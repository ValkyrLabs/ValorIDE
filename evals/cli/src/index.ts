...
program
  .name("valoride-eval")
  .description(
    "CLI tool for orchestrating ValorIDE evaluations across multiple benchmarks",
  )
  .version("0.1.0");
...
program
  ...
  .option("-k, --api-key <apiKey>", "ValorIDE API key to use for evaluations")
  .action(async (options) => {
    try {
      ...
      console.error(
        chalk.red(
          `Error during run: ${error instanceof Error ? error.message : String(error)}`,
        ),
      );
      process.exit(1);
    }
...
program
  ...
  .option(
    ...
    "Directory to create/remove/check evals.env file in (defaults to current directory)",
  )
  .action(async (action, options) => {
    try {
      ...
      console.error(
        chalk.red(
          `Error managing evals.env file: ${error instanceof Error ? error.message : String(error)}`,
        ),
      );
      process.exit(1);
    }
