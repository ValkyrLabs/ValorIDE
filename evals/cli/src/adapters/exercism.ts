...
export class ExercismAdapter implements BenchmarkAdapter {
  ...
  async setup(): Promise<void> {
    ...
    if (!fs.existsSync(exercismDir)) {
      ...
      await execa("git", [
        "clone",
        "https://github.com/pashpashpash/evals.git",
        exercismDir,
      ]);
  ...
  async listTasks(): Promise<Task[]> {
    ...
    if (!fs.existsSync(exercisesDir)) {
      throw new Error(
        `Exercism repository not found at ${exercisesDir}. Run setup first.`,
      );
    }
    ...
    const languages = fs
      ...
      .filter(
        (dir) =>
          !dir.startsWith(".") && !["node_modules", ".git"].includes(dir),
      );
    ...
    for (const language of languages) {
      ...
      const exercises = fs
        ...
        .filter((dir) =>
          fs.statSync(path.join(languageDir, dir)).isDirectory(),
        );
