...
export async function setupHandler(options: SetupOptions): Promise<void> {
  ...
  try {
    const adapters = getAllAdapters().filter((adapter: BenchmarkAdapter) =>
      benchmarks.includes(adapter.name),
    );

    if (adapters.length === 0) {
      console.warn(
        chalk.yellow("No valid benchmarks specified. Available benchmarks:"),
      );
      console.warn(
        chalk.yellow(
          getAllAdapters()
    ...
    for (const adapter of adapters) {
      ...
      try {
        ...
        setupSpinner.fail(
          `Failed to set up ${adapter.name}: ${(error as Error).message}`,
        );
        throw error;
      }
    }
