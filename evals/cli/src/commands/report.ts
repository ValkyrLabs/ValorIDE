  ...
  try {
    ...
    for (const run of runs) {
      ...
      for (const task of tasks) {
        ...
        const tokensOut =
          metrics.find((m) => m.name === "tokensOut")?.value || 0;
        ...
        for (const toolCall of toolCalls) {
          ...
          summary.toolUsage[toolCall.tool_name].failures +=
            toolCall.failure_count;
        }
      }
    }
    ...
    summary.toolSuccessRate =
      totalToolCalls > 0 ? 1 - totalToolFailures / totalToolCalls : 1.0;
    ...
    for (const benchmark of summary.benchmarks) {
      ...
      for (const run of benchmarkRuns) {
        ...
        for (const task of tasks) {
          ...
          const tokensIn =
            metrics.find((m) => m.name === "tokensIn")?.value || 0;
          const tokensOut =
            metrics.find((m) => m.name === "tokensOut")?.value || 0;
          benchmarkTotalTokens += tokensIn + tokensOut;

          benchmarkTotalCost +=
            metrics.find((m) => m.name === "cost")?.value || 0;
          benchmarkTotalDuration +=
            metrics.find((m) => m.name === "duration")?.value || 0;
        }
      }

      benchmarkSummary.tasks = benchmarkTasks;
      benchmarkSummary.successRate =
        benchmarkTasks > 0 ? benchmarkSuccessfulTasks / benchmarkTasks : 0;
      benchmarkSummary.averageTokens =
        benchmarkTasks > 0 ? benchmarkTotalTokens / benchmarkTasks : 0;
      benchmarkSummary.averageCost =
        benchmarkTasks > 0 ? benchmarkTotalCost / benchmarkTasks : 0;
      benchmarkSummary.averageDuration =
        benchmarkTasks > 0 ? benchmarkTotalDuration / benchmarkTasks : 0;
    ...
    for (const model of summary.models) {
      ...
      modelSummary.successRate =
        modelTasks > 0 ? modelSuccessfulTasks / modelTasks : 0;
      modelSummary.averageTokens =
        modelTasks > 0 ? modelTotalTokens / modelTasks : 0;
      modelSummary.averageCost =
        modelTasks > 0 ? modelTotalCost / modelTasks : 0;
      modelSummary.averageDuration =
        modelTasks > 0 ? modelTotalDuration / modelTasks : 0;
    ...
    if (format === "json") {
      ...
      const outputPath =
        options.output || path.join(reportDir, `report-${timestamp}.md`);
