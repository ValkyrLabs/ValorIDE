...
/**
 * Spawn a VSCode instance with the ValorIDE extension
 ...
 */
export async function spawnVSCode(
  workspacePath: string,
  vsixPath?: string,
): Promise<VSCodeResources> {
  ...
  if (!vsixPath) {
    try {
      ...
      const valorideRoot = path.resolve(process.cwd(), "..", "..");
      ...
      if (vsixFiles.length > 0) {
        ...
        console.log(
          `Using most recent VSIX: ${vsixPath} (modified ${vsixFilesWithStats[0].mtime.toISOString()})`,
        );
  ...
  const tempExtensionsDir = path.join(
    os.tmpdir(),
    `vscode-valoride-eval-ext-${Date.now()}`,
  );
  ...
  fs.writeFileSync(
    evalsEnvPath,
    `# This file activates ValorIDE test mode
# Created at: ${new Date().toISOString()}
# 
# This file is automatically detected by the ValorIDE extension
...
`,
  ...
  // and configure ValorIDE to auto-open on startup
  ...
  const settings = {
    ...
    // Auto-open ValorIDE on startup
    "valoride.autoOpenOnStartup": true,
  ...
  console.log(
    `Created settings.json to disable workspace trust and auto-open ValorIDE`,
  );

  // Create keybindings.json to automatically open ValorIDE on startup
  const keybindingsPath = path.join(settingsDir, "keybindings.json");
  const keybindings = [
    {
      key: "alt+c",
      command:
        "workbench.view.extension.valkyrlabsinc.valoride-dev-ActivityBar",
      when: "viewContainer.workbench.view.extension.valkyrlabsinc.valoride-dev-ActivityBar.enabled",
    },
    {
      key: "alt+shift+c",
      command: "valoride.openInNewTab",
      when: "viewContainer.workbench.view.extension.valkyrlabsinc.valoride-dev-ActivityBar.enabled",
    },
  ];
  fs.writeFileSync(keybindingsPath, JSON.stringify(keybindings, null, 2));
  console.log(`Created keybindings.json to help with ValorIDE activation`);
  ...
  const args = [
    ...
    // Run a command on startup to open ValorIDE
    "--command",
    "workbench.view.extension.valkyrlabsinc.valoride-dev-ActivityBar",
  ...
  const startupScript = `
		...
		setTimeout(() => {
			// Try to open ValorIDE in the sidebar
			require('vscode').commands.executeCommand('workbench.view.extension.valkyrlabsinc.valoride-dev-ActivityBar');
			
			// Also try to open ValorIDE in a tab as a fallback
			setTimeout(() => {
				require('vscode').commands.executeCommand('valoride.openInNewTab');
			}, 5000);
		}, 5000);
	`;
  fs.writeFileSync(startupScriptPath, startupScript);
  console.log(`Created startup script to activate ValorIDE`);
  ...
  try {
    ...
    const extensionDir = path.join(tempExtensionsDir, "valoride-activator");
    fs.mkdirSync(extensionDir, { recursive: true });

    // Create package.json for the extension
    const packageJsonPath = path.join(extensionDir, "package.json");
    const packageJson = {
      name: "valoride-activator",
      username: "ValorIDE Activator",
      description: "Activates ValorIDE and starts the test server",
      version: "0.0.1",
      engines: {
        vscode: "^1.60.0",
      },
      ...
      contributes: {
        commands: [
          {
            command: "valoride-activator.activate",
            title: "Activate ValorIDE",
          },
        ],
      },
    };
    ...
    const extensionJs = `
			...
			function activate(context) {
				console.log('ValorIDE Activator is now active!');
				
				// Register the command to activate ValorIDE
				let disposable = vscode.commands.registerCommand('valoride-activator.activate', async function () {
					try {
						// Make sure the ValorIDE extension is activated
						const extension = vscode.extensions.getExtension('valkyrlabsinc.valoride-dev');
						if (!extension) {
							console.error('ValorIDE extension not found');
							return;
						}
						
						if (!extension.isActive) {
							console.log('Activating ValorIDE extension...');
							await extension.activate();
						}
						
						// Show the ValorIDE sidebar
						console.log('Opening ValorIDE sidebar...');
						await vscode.commands.executeCommand('workbench.view.extension.valkyrlabsinc.valoride-dev-ActivityBar');
						
						// Wait a moment for the sidebar to initialize
						await new Promise(resolve => setTimeout(resolve, 2000));
						
						// Also open ValorIDE in a tab as a fallback
						console.log('Opening ValorIDE in a tab...');
						await vscode.commands.executeCommand('valoride.openInNewTab');
						...
						const valorideRootPath = '${path.resolve(process.cwd(), "..", "..")}';
						const visibleWebview = require(path.join(valorideRootPath, 'src', 'core', 'webview')).WebviewProvider.getVisibleInstance();
						if (visibleWebview) {
							require(path.join(valorideRootPath, 'src', 'services', 'test', 'TestServer')).createTestServer(visibleWebview);
							console.log('Test server created successfully');
						} else {
							console.error('No visible webview instance found');
						}
					} catch (error) {
						console.error('Error activating ValorIDE:', error);
					}
				});
				...
				setTimeout(() => {
					vscode.commands.executeCommand('valoride-activator.activate');
				}, 5000);
			}
    ...
    console.log(`Created ValorIDE Activator extension`);

    // Try multiple approaches to activate the extension
    let serverStarted = false;

    // Create an activation script to run in VS Code
    const activationScriptPath = path.join(settingsDir, "activate-valoride.js");
    const activationScript = `
			// This script will be executed to activate ValorIDE and start the test server
			const vscode = require('vscode');
			
			// Execute the valoride-activator.activate command
			vscode.commands.executeCommand('valoride-activator.activate');
		`;
    ...
    try {
      console.log(
        "Executing activation script to start ValorIDE and test server...",
      );
      await execa(
        "code",
        [
    ...
    if (!serverStarted) {
      ...
      console.log(
        "You may need to manually open the ValorIDE extension in VS Code",
      );
    }
...
export async function cleanupVSCode(workspacePath: string): Promise<void> {
  ...
  try {
    ...
    if (process.platform === "darwin") {
      try {
        ...
        console.warn(
          `Error using AppleScript to quit VS Code: ${appleScriptError}`,
        );
      }
      ...
      try {
        ...
        for (const line of lines) {
          if (line.includes(resources.tempUserDataDir)) {
            ...
            if (pid && !isNaN(pid)) {
              console.log(
                `Sending SIGTERM to VS Code process with PID: ${pid}`,
              );
              try {
                ...
                console.warn(
                  `Failed to terminate process ${pid}: ${killError}`,
                );
              }
            }
          }
    ...
    if (vsCodeStillRunning) {
      console.log(
        "Graceful shutdown failed, falling back to forceful termination...",
      );
        ...
        try {
          ...
          for (const line of lines) {
            if (line.includes(resources.tempUserDataDir)) {
              ...
              if (pid && !isNaN(pid)) {
                console.log(
                  `Forcefully killing VS Code process with PID: ${pid}`,
                );
                try {
  ...
  try {
    console.log(
      `Removing temporary user data directory: ${resources.tempUserDataDir}`,
    );
  ...
  try {
    console.log(
      `Removing temporary extensions directory: ${resources.tempExtensionsDir}`,
    );
