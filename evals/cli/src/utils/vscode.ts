...
export async function spawnVSCode(workspacePath: string, vsixPath?: string): Promise<VSCodeResources> {
	...
	const tempUserDataDir = path.join(os.tmpdir(), `vscode-valoride-eval-${Date.now()}`)
	...
	const tempExtensionsDir = path.join(os.tmpdir(), `vscode-valoride-eval-ext-${Date.now()}`)
...
`,
	...
	console.log(`Created settings.json to disable workspace trust and auto-open ValorIDE`)
	...
	const keybindings = [
		{
			key: "alt+c",
			command: "workbench.view.extension.valkyrlabsinc.valoride-dev-ActivityBar",
	...
	try {
		...
		try {
			console.log("Executing activation script to start ValorIDE and test server...")
			await execa(
				"code",
				[
					"--user-data-dir",
		...
		if (!serverStarted) {
			...
			console.log("You may need to manually open the ValorIDE extension in VS Code")
		}
