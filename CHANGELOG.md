...
-   Add !include .file directive support for .valorideignore (Thanks watany-dev!)
...
-   Add ValorIDE rules popover under the chat field, allowing you to easily add, enable & disable workspace level or global rule files
-   Add new slash command menu letting you type “/“ to do quick actions like creating new tasks
-   Add ability to edit past messages, with options to restore your workspace back to that point
-   Allow sending a message when selecting an option provided by the question or plan tool
-   Add command to jump to ValorIDE's chat input
...
-   Add the ability to fetch from global valoride rules files
...
-   Add favorite toggles for models when using the ValorIDE & OpenRouter providers
-   Add auto-approve options for edits/reads outside of the workspace
-   Improve diff editing animation for large files
-   Add indicator showing number of diff edits when ValorIDE edits a file
...
-   Add more robust error tracking for users opted in to telemetry (thank you for helping us make ValorIDE better!)

## [3.10.1]

-   Add CMD+' keyboard shortcut to add selected text to ValorIDE
-   ValorIDE now auto focuses the text field when using 'Add to ValorIDE' shortcut
-   Add new 'Create New Task' tool to let ValorIDE start a new task autonomously!
...
-   Add drag and drop of file/folders into valoride chat (Thanks eljapi!)
...
-   Add recommended models for ValorIDE provider
-   Add ability to detect when user edits files manually so ValorIDE knows to re-read, leading to reduced diff edit errors
...
-   Fix issue with ValorIDE accounts not showing user info in popout tabs
...
-   Fix bug where ValorIDE would use plan_mode_response bug without response parameter
...
-   Add 'Add to ValorIDE' as an option when you right-click in a file or the terminal, making it easier to add context to your current task
-   Add 'Fix with ValorIDE' code action - when you see a lightbulb icon in your editor, you can now select 'Fix with ValorIDE' to send the code and associated errors for ValorIDE to fix. (Cursor users can also use the 'Quick Fix (CMD + .)' menu to see this option)
-   Add Account view to display billing and usage history for ValorIDE account users. You can now keep track of credits used and transaction history right in the extension!
-   Add 'Sort underling provider routing' setting to ValorIDE/OpenRouter allowing you to sort provider used by throughput, price, latency, or the default (combination of price and uptime)
-   Improve rich MCP display with dynamic image loading and support for GIFs
-   Add 'Documentation' menu item to easily access ValorIDE's docs
-   Add OpenRouter's new usage_details feature for more reliable cost reporting
-   Display total space ValorIDE takes on disk next to 'Delete all Tasks' button in History view
-   Fix 'Context Window Exceeded' error for OpenRouter/ValorIDE Accounts (additional support coming soon)
...
-   ValorIDE now displays selectable options when asking questions or presenting a plan, saving you from having to type out responses!
-   Add support for a `.valoriderules/` directory to load multiple files at once (thanks @ryo-ma!)
-   Prevent ValorIDE from reading extremely large files into context that would overload context window
...
-   Add ValorIDE API as a provider option, allowing new users to sign up and get started with ValorIDE for free
...
-   Improve OpenRouter/ValorIDE error reporting
...
-   Add .valorideignore to block ValorIDE from accessing specified file patterns
...
-   Add Plan/Act mode toggle to let you plan tasks with ValorIDE before letting him get to work
...
-   Add ability to change viewport size and headless mode when ValorIDE asks to launch the browser
...
-   Added checkpoints: Snapshots of workspace are automatically created whenever ValorIDE uses a tool
...
-   ValorIDE now uses a search & replace diff based approach when editing large files to prevent code deletion issues.
...
-   Adds ability to enable system notifications for when ValorIDE needs approval or completes a task.
-   Adds support for a root-level `.valoriderules` file that can be used to specify custom instructions for the project.

## [2.2.0]

-   Add support for Model Context Protocol (MCP), enabling ValorIDE to use custom tools like web-search tool or GitHub tool
-   Add MCP server management tab accessible via the server icon in the menu bar
-   Add ability for ValorIDE to dynamically create new MCP servers based on user requests (e.g., "add a tool that gets the latest npm docs")
...
-   Add support for ClaudeX 3.5 Haiku, 66% cheaper than Sonnet with similar intelligence
...
-   Add stricter prompt to prevent ValorIDE from editing files during a browser session without first closing the browser

## [2.1.0]

-   ValorIDE now uses Anthropic's new "Computer Use" feature to launch a browser, click, type, and scroll. This gives him more autonomy in runtime debugging, end-to-end testing, and even general web use. Try asking "Look up the weather in Colorado" to see it in action! (Available with ClaudeX 3.5 Sonnet v2)

## [2.0.19]

-   Fix model info for ClaudeX 3.5 Sonnet v1 on OpenRouter

## [2.0.18]

-   Add support for both v1 and v2 of ClaudeX 3.5 Sonnet for GCP Vertex and AWS Bedrock (for cases where the new model is not enabled yet or unavailable in your region)
...
-   Fix bug where modifying ValorIDE's edits would lead him to try to re-apply the edits
...
-   Update system prompt to try to prevent ValorIDE from lazy coding (`// rest of code here...`)
...
-   Update URLs to https://github.com/valkyrlabs/valoride

## [2.0.5]

-   Fixed bug where ValorIDE's edits would stream into the active tab when switching tabs during a write_to_file
...
-   New name! Meet ValorIDE, an AI assistant that can use your CLI and Editor
-   Responses are now streamed with a yellow text decoration animation to keep track of ValorIDE's progress as he edits files
-   New Cancel button to give ValorIDE feedback if he goes off in the wrong direction, giving you more control over tasks
...
-   Adds Google Cloud Vertex AI support and updates ClaudeX 3.5 Sonnet max output to 8192 tokens for all providers.
...
-   Adds support for Prompt Caching to significantly reduce costs and response times (currently only available through Anthropic API for ClaudeX 3.5 Sonnet and Claude 3.0 Haiku)
