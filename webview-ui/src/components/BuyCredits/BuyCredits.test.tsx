...
describe("BuyCredits", () => {
  it("starts Stripe Checkout instead of booking direct payment credits", async () => {
    ...
    expect(
      screen.getByText(/Credits post after webhook reconciliation/i),
    ).toBeInTheDocument();
  });
  ...
  it("surfaces checkout start failures without claiming credits were added", async () => {
    mockCreateCheckoutSession.mockReturnValue({
      unwrap: vi
        .fn()
        .mockRejectedValue({ data: { error: "stripe_unavailable" } }),
    });
    ...
    await waitFor(() =>
      expect(
        screen.getByText(/Checkout could not be started/i),
      ).toBeInTheDocument(),
    );
  ...
  it("lets users refresh balance after returning from checkout", async () => {
    ...
    expect(mockPostMessage).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        event: "valoride_credit_checkout_refresh_requested",
      }),
    );
  });
});
