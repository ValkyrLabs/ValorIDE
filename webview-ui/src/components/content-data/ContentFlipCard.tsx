...
const ContentFlipCard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const materials = [
      new THREE.MeshStandardMaterial({ color: 0x001f1f }),
      new THREE.MeshStandardMaterial({ color: 0x001f1f }),
      new THREE.MeshStandardMaterial({ color: 0x001f1f }),
      new THREE.MeshStandardMaterial({ color: 0x001f1f }),
      new THREE.MeshStandardMaterial({
        map: createTextureForIndex(currentIndex),
      }),
      new THREE.MeshStandardMaterial({ color: 0x081826 }),
    ];
  ...
  const flipToIndex = (idx: number) => {
    ...
    const animate = (t: number) => {
      ...
      mesh.rotation.y =
        initialRotation + (finalRotation - initialRotation) * easeInOutCubic(p);

      // At halfway, update texture to next content
      if (p >= 0.5 && mesh.material instanceof Array) {
        (mesh.material as THREE.Material[])[4]["map"] =
          createCanvasTextureForIndex(targetIndex, cards);
  ...
  if (isLoading || isFetching) {
    return (
      <div style={{ display: "flex", justifyContent: "center" }}>
        <LoadingSpinner label="Loading flip card..." />
      </div>
    );
  }
  ...
  return (
    <div className="contentflip-wrapper">
      <div className="contentflip-header">
        <h4>Spotlight</h4>
        <div>
          <CoolButton className="me-2" onClick={handlePrevious}>
            <FaChevronLeft />
          </CoolButton>
          <CoolButton onClick={handleNext}>
            <FaChevronRight />
          </CoolButton>
        </div>
      </div>
      ...
      <div className="contentflip-footer">
        ...
        <div className="contentflip-desc">
          {cards[currentIndex]?.description?.substring(0, 110)}...
        </div>
      </div>
    </div>
  );
