...
export const UserAvatar = ({ avatarUrl, firstName, lastName, username, size = "md", editable = false, onAvatarChange, onUploaded, showStatus = false, isOnline = false, className = "", style, title, onClick, borderColor, showRing = false, ringColor = "#0acffe", }) => {
    ...
    const initials = useMemo(() => getInitials(firstName, lastName, username), [firstName, lastName, username]);
    ...
    return (_jsxs("div", { className: `user-avatar-container ${className} ${editable ? "editable" : ""} ${isDragOver ? "drag-over" : ""} ${isUploading ? "uploading" : ""}`, style: containerStyle, onDragOver: handleDragOver, onDragLeave: handleDragLeave, onDrop: handleDrop, onClick: handleClick, onKeyDown: handleKeyDown, role: editable || onClick ? "button" : undefined, tabIndex: editable || onClick ? 0 : undefined, title: title || (editable ? "Click or drag to upload avatar" : undefined), "aria-label": `Avatar for ${firstName || username || "user"}`, children: [editable && (_jsx("input", { ref: fileInputRef, type: "file", accept: "image/*", onChange: handleFileChange, style: { display: "none" }, "aria-hidden": "true" })), _jsx("div", { className: "user-avatar-content", children: showImage ? (_jsx(Image, { src: avatarUrl, alt: `${firstName || username || "User"} avatar`, className: "user-avatar-image", roundedCircle: true, onError: handleImageError })) : (_jsx("div", { className: "user-avatar-initials", style: initialsStyle, children: initials })) }), editable && !isUploading && (_jsx("div", { className: "user-avatar-overlay", children: _jsx(FaCamera, { size: sizeConfig.iconSize }) })), isUploading && (_jsx("div", { className: "user-avatar-spinner", children: _jsx(Spinner, { animation: "border", size: "sm", style: { width: sizeConfig.iconSize, height: sizeConfig.iconSize } }) })), uploadSuccess && (_jsx("div", { className: "user-avatar-success", children: _jsx(FaCheck, { size: sizeConfig.iconSize }) })), uploadError && (_jsx("div", { className: "user-avatar-error", children: _jsx(FaTimes, { size: sizeConfig.iconSize }) })), showStatus && (_jsx("div", { className: `user-avatar-status ${isOnline ? "online" : "offline"}`, style: {
