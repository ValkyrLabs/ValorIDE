    ...
    const doSearch = useMemo(() => debounce(async (text) => {
        ...
        try {
            ...
            if (!q) {
                ...
                const res = await authFetch(withValkyraiBase(`${entity}?page=1&limit=50`));
                if (res.ok) {
                ...
                try {
                    const lu = await authFetch(withValkyraiBase(`lookup/entity/${encodeURIComponent(entity)}?q=${encodeURIComponent(q)}&limit=20`));
                    if (lu.ok) {
                    ...
                    for (const exObj of queries) {
                        ...
                        const res = await authFetch(withValkyraiBase(`${entity}?page=1&limit=20&example=${ex}`));
                        if (res.ok) {
                            const arr = await res.json();
                            for (const o of arr || []) {
                                const opt = {
