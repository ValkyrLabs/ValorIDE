...
export const SwarmPanel: React.FC = () => {
  ...
  const getSenderId = useCallback(() => {
    return (
      (window as any).__valoride_instanceId ||
  ...
  const doRollcall = useCallback(
    () =>
      sendApp("presence:rollcall", {
        id: (window as any).__valoride_instanceId || "ui",
      }),
    [],
  );
  ...
  const sendCommand = useCallback(
    (targetId?: string) => {
      const payload = {
        ...
        type: "command",
        data: { cmd: command },
        sourceInstanceId: (window as any).__valoride_instanceId || "ui",
  ...
  const renderHierarchyTree = (
    ...
    return nodes.map((node) => (
      <div key={node.agentId} style={{ marginLeft: depth * 20, padding: 4 }}>
        ...
        >
          {"▶".repeat(Math.max(0, depth))} <strong>{node.agentId}</strong>{" "}
          (depth: {node.depth})
        </div>
        {node.children &&
        ...
        {activeTab === "hierarchy" && (
          ...
          >
            <h4 style={{ marginTop: 0 }}>
              Agent Tree (Max 12 children, 12 levels)
            </h4>
            {hierarchy.length > 0 ? (
                      ...
                      {chatMessages.map((msg) => (
                        <div
                          ...
                          style={{
                            ...
                            padding: 8,
                            backgroundColor:
                              msg.senderType === "USER" ? "#e3f2fd" : "#f0f0f0",
                            borderRadius: 4,
                          }}
                        >
                          <div
