...
describe('SWARM v2 Integration Tests', () => {
    ...
    describe('GET /agent/{id}/chat/history', () => {
        it('should return paginated message history', async () => {
            const response = await fetch(`${API_BASE}/agent/${TEST_AGENT_ID}/chat/history?organizationId=${encodeURIComponent(TEST_ORG)}&conversationId=conv-test&page=0`);
            if (response.ok) {
        ...
        it('should include readBy array in messages', async () => {
            const response = await fetch(`${API_BASE}/agent/${TEST_AGENT_ID}/chat/history?organizationId=${encodeURIComponent(TEST_ORG)}&conversationId=conv-test`);
            if (response.ok) {
                const data = await response.json();
                const messages = data.content || data;
    ...
    describe('GET /billing/status', () => {
        it('should return billing status with correct schema', async () => {
            const response = await fetch(`${API_BASE}/billing/status?organizationId=${encodeURIComponent(TEST_ORG)}`);
        ...
        it('should return 402 when billing suspended', async () => {
            ...
            const response = await fetch(`${API_BASE}/billing/status?organizationId=suspended-org`);
...
//# sourceMappingURL=SwarmPanel.integration.test.js.map