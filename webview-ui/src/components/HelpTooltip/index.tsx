...
// ============================================================================
export const DOC_URLS: Record<string, DocEntry> = {
  // === WORKFLOW ENGINE ===
  "workflow-engine": {
...
export interface HelpTooltipProps {
  /** Key from DOC_URLS registry, or custom DocEntry */
  topic: string | DocEntry;
  ...
  /** Show as inline icon or block with label */
  inline?: boolean;
  /** Custom label text (only for non-inline mode) */
  label?: string;
  /** Additional CSS class */
  className?: string;
  /** Show the category badge */
  showCategory?: boolean;
  /** Children to wrap (if provided, tooltip wraps children instead of showing icon) */
...
const HelpTooltip: React.FC<HelpTooltipProps> = ({
  ...
  const docEntry: DocEntry | undefined =
    typeof topic === "string" ? DOC_URLS[topic] : topic;
  ...
  const popoverContent = (overlayProps: any) => (
    <Popover
      ...
      {...overlayProps}
    >
      <Popover.Header as="h6" className="help-tooltip-header">
        <FaBook className="me-2" />
        {docEntry.title}
        {showCategory && (
          <span className="help-tooltip-category">{docEntry.category}</span>
        )}
      </Popover.Header>
      <Popover.Body className="help-tooltip-body">
        <p className="help-tooltip-summary">{docEntry.summary}</p>
        <button
          type="button"
  ...
  const helpIcon = icon || <FaQuestionCircle />;
  ...
  return (
    <>
      ...
      >
        {helpIcon}
        <span className="help-tooltip-label">{label || docEntry.title}</span>
      </span>
      {overlay}
    </>
  );
...
// ============================================================================
// SECTION HELP HEADER - For dashboard sections
// ============================================================================
...
export const SectionHelp: React.FC<SectionHelpProps> = ({
  title,
  topic,
  children,
  className = "",
  ...
  return (
    <div className={`section-help-header ${className}`}>
      <h5 className="section-help-title">
        {title}
        <HelpTooltip topic={topic} size="sm" className="ms-2" />
      </h5>
      {children}
    </div>
  );
...
// ============================================================================
// QUICK HELP BADGE - Compact inline help
// ============================================================================
...
export const QuickHelp: React.FC<QuickHelpProps> = ({
  ...
  return (
    <HelpTooltip topic={topic} className={`quick-help-badge ${className}`}>
