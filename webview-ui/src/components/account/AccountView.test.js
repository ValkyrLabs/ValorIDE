...
vi.mock("@/services/creditsApi", () => ({
    useGetAccountBalanceQuery: (...args) => mockUseGetAccountBalanceQuery(...args),
}));
vi.mock("@/thor/redux/services/UsageTransactionService", () => ({
    useAddUsageTransactionMutation: () => [vi.fn()],
    useGetUsageTransactionsQuery: (...args) => mockUseGetUsageTransactionsQuery(...args),
}));
vi.mock("@/thor/redux/services/PaymentTransactionService", () => ({
    useGetPaymentTransactionsQuery: (...args) => mockUseGetPaymentTransactionsQuery(...args),
}));
...
vi.mock("@/components/BuyCredits", () => ({
    ...
    default: (props) => {
        ...
        return (_jsxs("div", { "data-testid": "buy-credits-component", children: ["buy-credits for ", props.authenticatedPrincipal?.id || "no-user"] }));
    },
}));
...
describe("AccountView - BuyCredits integration", () => {
    ...
    it("renders the embedded BuyCredits component instead of an external link when authenticated", () => {
        ...
        expect(mockBuyCredits).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it("fetches balance using userInfo when authenticatedUser is missing", () => {
        ...
        render(_jsx(AccountView, { onDone: () => { } }));
        expect(mockUseGetAccountBalanceQuery).toHaveBeenCalledWith("user-123", expect.objectContaining({ skip: false }));
        expect(mockBuyCredits).toHaveBeenCalledWith(expect.objectContaining({
            authenticatedPrincipal: expect.objectContaining({ id: "user-123" }),
        }));
    });
