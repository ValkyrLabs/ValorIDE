...
const AccountView = ({ onDone }: AccountViewProps) => {
	...
	const { userInfo, authenticatedPrincipal, isLoggedIn, jwtToken } = useExtensionState()
	...
	const isAuthenticated = Boolean(isLoggedIn || (authenticatedPrincipal || userInfo || jwtToken))
	...
	const { data: usageData, isLoading: isUsageLoading } = useGetUsageTransactionsQuery(undefined, {
		skip: !isAuthenticated,
	})
	const { data: paymentsData, isLoading: isPaymentsLoading } = useGetPaymentTransactionsQuery(undefined, {
		skip: !isAuthenticated,
	})
			...
			<div className="h-full flex flex-col pr-3 overflow-y-auto">
				...
				<div className="w-full flex gap-2 flex-col min-[225px]:flex-row mt-4">
					<div className="w-full min-[225px]:w-1/2">
						<VSCodeButtonLink href="https://valkyrlabs.com/dashboard" appearance="primary" className="w-full">
							Dashboard
						</VSCodeButtonLink>
					</div>
				...
				<div className="w-full flex flex-col items-center">
					...
					<div className="text-4xl font-bold text-[var(--vscode-foreground)] mb-6 flex items-center gap-2">
						{loading ? (
							...
							<>
								<span>$</span>
								<CountUp end={balanceData?.[0]?.currentBalance || 0} duration={0.66} decimals={2} />
								<VSCodeButton appearance="icon" className="mt-1" onClick={() => refetchBalance()}>
					...
					<div className="w-full">
						<VSCodeButtonLink href="https://app.valkyrlabs.com/v1/credits/#buy" className="w-full">
							Add Credits
						</VSCodeButtonLink>
					</div>
				</div>
