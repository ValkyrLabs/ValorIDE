...
const ApplicationsList: React.FC<ApplicationsListProps> = ({ showTitle = true, title = "Available ThorAPI Applications" }) => {
	...
	const isAuthenticated = !!(sessionToken || jwtToken || authenticatedPrincipal || userInfo)
	...
	const [completedApplications, setCompletedApplications] = useState<Set<string>>(new Set())
	...
	const handleGenerate = async (applicationId: string) => {
		...
		try {
			...
			console.log('ApplicationsList: Sending to extension with filename:', extractedFilename)
		...
		<div className="applications-list">
			...
			<div className="applications-container">
				{applications.map((app: any) => (
					<div key={app.id || app.name || JSON.stringify(app)} className="application-card">
						<div className="application-card-content">
							<div className="application-info">
								<div className="application-name">{app.name || app.title || app.id}</div>
								{app.description && <div className="application-description">{app.description}</div>}
								<div className="application-meta">
									{app.type && <span className="application-type">{app.type}</span>}
									{app.status && (
							...
							{app.id && (
								<div className="application-buttons">
									<VSCodeButton appearance="secondary" onClick={() => handleApplicationSelect(app)}>
										Open
									</VSCodeButton>
									<VSCodeButton
										appearance="primary"
										...
										{loadingStates[app.id]?.generating ? "Generating..." : "Generate"}
									</VSCodeButton>
									<VSCodeButton
										appearance="secondary"
										onClick={() => handleDeploy(app.id)}
										disabled={loadingStates[app.id]?.deploying}>
										{loadingStates[app.id]?.deploying ? "Deploying..." : "Deploy"}
									</VSCodeButton>
								</div>
							)}
							{(loadingStates[app.id]?.generating || loadingStates[app.id]?.steps?.receiving || loadingStates[app.id]?.steps?.processing || loadingStates[app.id]?.steps?.extracting || loadingStates[app.id]?.steps?.finalizing) && (
								<div className="loading-steps" style={{ marginTop: '16px', padding: '12px', border: '1px solid var(--vscode-panel-border)', borderRadius: '4px' }}>
									<div className="loading-step" style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
										{loadingStates[app.id]?.steps?.receiving ? (
											<span style={{ color: 'var(--vscode-charts-green)', marginRight: '8px' }}>✅</span>
										) : (
											<VSCodeProgressRing style={{ width: '16px', height: '16px', marginRight: '8px' }} />
										)}
										<span>Receiving Application</span>
										{!loadingStates[app.id]?.steps?.receiving && <span style={{ marginLeft: '8px', fontSize: '12px', opacity: 0.7 }}>Downloading application payload...</span>}
									</div>
									<div className="loading-step" style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
										{loadingStates[app.id]?.steps?.processing ? (
											...
											<span style={{ color: 'var(--vscode-descriptionForeground)', marginRight: '8px' }}>⏳</span>
										)}
										<span>Processing Data</span>
										{loadingStates[app.id]?.steps?.receiving && !loadingStates[app.id]?.steps?.processing && <span style={{ marginLeft: '8px', fontSize: '12px', opacity: 0.7 }}>Analyzing application structure...</span>}
									</div>
									<div className="loading-step" style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
										...
										{loadingStates[app.id]?.steps?.processing && !loadingStates[app.id]?.steps?.extracting && <span style={{ marginLeft: '8px', fontSize: '12px', opacity: 0.7 }}>Creating project structure...</span>}
									</div>
									<div className="loading-step" style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
										...
										{loadingStates[app.id]?.steps?.extracting && !loadingStates[app.id]?.steps?.finalizing && <span style={{ marginLeft: '8px', fontSize: '12px', opacity: 0.7 }}>Preparing development environment...</span>}
									</div>
									{loadingStates[app.id]?.generating ? (
										...
										<div style={{ marginTop: '12px', fontSize: '12px', color: 'var(--vscode-charts-green)' }}>
			...
			{showFileExplorer && (
				<div style={{ marginTop: '20px' }}>
					<div className="file-explorer-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
					...
					{hasCompletedApplications && (
						<div style={{ marginBottom: '20px' }}>
							...
							{Array.from(completedApplications).map(appId => {
								...
								return (
									<div key={appId} className="application-card" style={{ marginBottom: '12px' }}>
										<div className="application-card-content">
											<div className="application-info">
												<div className="application-name">{appName}</div>
												<div style={{ fontSize: '12px', color: 'var(--vscode-charts-green)', marginTop: '4px' }}>
													✅ Generation completed successfully
												</div>
											</div>
											<div className="loading-steps" style={{ marginTop: '12px', padding: '12px', border: '1px solid var(--vscode-panel-border)', borderRadius: '4px' }}>
												<div className="loading-step" style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
													<span style={{ color: 'var(--vscode-charts-green)', marginRight: '8px' }}>✅</span>
													<span>Receiving Application</span>
												</div>
