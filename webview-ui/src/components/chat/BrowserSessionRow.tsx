...
import { BrowserAction, BrowserActionResult, ValorIDEMessage, ValorIDESayBrowserAction } from "@shared/ExtensionMessage"
	...
	const pages = useMemo(() => {
		const result: {
			currentState: {
				...
				consoleLogs?: string
				messages: ValorIDEMessage[] // messages up to and including the result
			}
			nextAction?: {
				messages: ValorIDEMessage[] // messages leading to next result
			}
		}[] = []

		let currentStateMessages: ValorIDEMessage[] = []
		let nextActionMessages: ValorIDEMessage[] = []
		...
		<BrowserSessionRowContainer style={{ marginBottom: -10 }}>
			<div style={browserSessionRowContainerInnerStyle}>
				...
				<span style={approveTextStyle}>
					<>{isAutoApproved ? "ValorIDE is using the browser:" : "ValorIDE wants to use the browser:"}</>
				</span>
			</div>
			<div
				style={{
