...
import { ContextMenuOptionType, getContextMenuOptions, insertMention, insertMentionDirectly, removeMention, shouldShowContextMenu, } from "@/utils/context-mentions";
import { slashCommandDeleteRegex, shouldShowSlashCommandsMenu, getMatchingSlashCommands, insertSlashCommand, removeSlashCommand, validateSlashCommand, } from "@/utils/slash-commands";
...
`;
const ModelSelectorTooltip = styled.div `
  position: fixed;
  bottom: calc(100% + 9px);
  left: 15px;
  right: 15px;
...
`;
const ModelDisplayButton = styled.a `
  padding: 0px 0px;
  height: 20px;
  width: 100%;
  min-width: 0;
...
const ModelButtonContent = styled.div `
  ...
  white-space: nowrap;
`;
const ChatTextArea = forwardRef(({ inputValue, setInputValue, textAreaDisabled, placeholderText, selectedImages, setSelectedImages, onSend, onSelectImages, shouldDisableImages, onHeightChange, }, ref) => {
    const { filePaths, chatSettings, apiConfiguration, openRouterModels, platform, valorideMessages, } = useExtensionState();
    const [isTextAreaFocused, setIsTextAreaFocused] = useState(false);
    const [gitCommits, setGitCommits] = useState([]);
    const [showSlashCommandsMenu, setShowSlashCommandsMenu] = useState(false);
    const [selectedSlashCommandsIndex, setSelectedSlashCommandsIndex] = useState(0);
    const [slashCommandsQuery, setSlashCommandsQuery] = useState("");
    const slashCommandsMenuContainerRef = useRef(null);
    ...
    const prevImagesCountRef = useRef(selectedImages.length);
    ...
    const handleMessage = useCallback((event) => {
        ...
        switch (message.type) {
            ...
            case "relativePathsResponse": {
                ...
                const validPaths = message.paths?.filter((path) => !!path) || [];
            ...
            case "fileSearchResults": {
                ...
                if (!message.mentionsRequestId ||
                    message.mentionsRequestId === currentSearchQueryRef.current) {
    ...
    const handleSlashCommandsSelect = useCallback((command) => {
    ...
    const handleKeyDown = useCallback((event) => {
        ...
        if (showContextMenu) {
            ...
            if (event.key === "ArrowUp" || event.key === "ArrowDown") {
                ...
                setSelectedMenuIndex((prevIndex) => {
                    ...
                    const newSelectableIndex = (currentSelectableIndex +
                        direction +
            ...
            if ((event.key === "Enter" || event.key === "Tab") &&
                selectedMenuIndex !== -1) {
                event.preventDefault();
                const selectedOption = getContextMenuOptions(searchQuery, selectedType, queryItems, fileSearchResults)[selectedMenuIndex];
                if (selectedOption &&
        ...
        if (event.key === "Backspace" && !isComposing) {
            ...
            const charBeforeIsWhitespace = charBeforeCursor === " " ||
                charBeforeCursor === "\n" ||
                charBeforeCursor === "\r\n";
            const charAfterIsWhitespace = charAfterCursor === " " ||
                charAfterCursor === "\n" ||
                charAfterCursor === "\r\n";
            ...
            else if (charBeforeIsWhitespace &&
                inputValue
                    .slice(0, cursorPosition - 1)
    ...
    useEffect(() => {
        ...
        const currentCursorPos = intendedCursorPosition ??
     ...
     */
    const readImageFiles = (imageFiles) => {
    ...
    return (_jsxs("div", { children: [_jsxs("div", { style: {
                    ...
                    position: "relative",
                    display: "flex",
                }, onDrop: onDrop, onDragOver: onDragOver, children: [showSlashCommandsMenu && (_jsx("div", { ref: slashCommandsMenuContainerRef, children: _jsx(SlashCommandMenu, { onSelect: handleSlashCommandsSelect, selectedIndex: selectedSlashCommandsIndex, setSelectedIndex: setSelectedSlashCommandsIndex, onMouseDown: handleMenuMouseDown, query: slashCommandsQuery }) })), showContextMenu && (_jsx("div", { ref: contextMenuContainerRef, children: _jsx(ContextMenu, { onSelect: handleMentionSelect, searchQuery: searchQuery, onMouseDown: handleMenuMouseDown, selectedIndex: selectedMenuIndex, setSelectedIndex: setSelectedMenuIndex, selectedType: selectedType, queryItems: queryItems, dynamicSearchResults: fileSearchResults, isLoading: searchLoading }) })), !isTextAreaFocused && (_jsx("div", { style: {
                            position: "absolute",
                            inset: "10px 15px",
                            borderRadius: 10,
                            pointerEvents: "none",
                            zIndex: 5,
                        } })), _jsx("div", { ref: highlightLayerRef, style: {
                            position: "absolute",
                            top: 10,
                            left: 15,
                            right: 15,
                        ...
                        } }), _jsx(GlowingTextArea, { "data-testid": "chat-input", ref: (el) => {
                        ...
                        }, value: inputValue, disabled: textAreaDisabled, borderState: (() => {
                            ...
                            try {
                                ...
                                for (let i = msgs.length - 1; i >= 0; i--) {
                                    const m = msgs[i];
                                    if (lastStarted === -1 && m.type === "say" && m.say === "api_req_started")
                                        lastStarted = i;
                                    if (lastFinished === -1 && m.type === "say" && (m.say === "api_req_finished" || m.say === "api_req_retried"))
                                        lastFinished = i;
                                    if (lastStarted !== -1 && lastFinished !== -1)
                                        break;
                                }
                        ...
                        })(), isStubborn: !!chatSettings.stubbornMode, onChange: (e) => {
                            handleInputChange(e);
                            updateHighlights();
                        }, onKeyDown: handleKeyDown, onKeyUp: handleKeyUp, onFocus: () => setIsTextAreaFocused(true), onBlur: handleBlur, onPaste: handlePaste, onSelect: updateCursorPosition, onMouseUp: updateCursorPosition, onHeightChange: (height) => {
                            if (textAreaBaseHeight === undefined ||
                            ...
                            outline: isTextAreaFocused
                                ...
                                : "none",
                        }, onScroll: () => updateHighlights() }), selectedImages.length > 0 && (_jsx(Thumbnails, { images: selectedImages, setImages: setSelectedImages, onHeightChange: handleThumbnailsHeightChange, style: {
                            position: "absolute",
                            paddingTop: 4,
                            bottom: 14,
                            left: 22,
                            ...
                            zIndex: 2,
                                ...
                                flexDirection: "row",
                                alignItems: "center",
                            }, children: _jsx("button", { style: { cursor: "pointer", padding: "3px", width: "28px", height: "28px", backgroundColor: "darkblue", borderRadius: "14px" }, "data-testid": "send-button", disabled: textAreaDisabled, onClick: () => {
                                    if (!textAreaDisabled) {
                                        setIsTextAreaFocused(false);
                                        onSend(inputValue, selectedImages);
                                ...
                                }, children: _jsx(FaPaperPlane, {}) }) }) })] }), _jsxs(ControlsContainer, { children: [imagesToastVisible && (_jsx("div", { style: {
                            position: "absolute",
                            bottom: 42,
                            left: 20,
                            ...
                            pointerEvents: "none",
                            zIndex: 10,
                        }, "aria-live": "polite", children: imagesToastText })), _jsxs(ButtonGroup, { children: [_jsx(Tooltip, { tipText: "Add Context", style: { left: 0 }, children: _jsx(VSCodeButton, { "data-testid": "context-button", appearance: "icon", "aria-label": "Add Context", disabled: textAreaDisabled, onClick: handleContextButtonClick, style: { padding: "0px 0px", height: "20px" }, children: _jsx(ButtonContainer, { children: _jsx("span", { className: "flex items-center", style: { fontSize: "13px", marginBottom: 1 }, children: "@" }) }) }) }), _jsx(Tooltip, { tipText: "Add Images", children: _jsx(VSCodeButton, { "data-testid": "images-button", appearance: "icon", "aria-label": "Add Images", disabled: shouldDisableImages, onClick: () => {
                                        if (!shouldDisableImages) {
                                            onSelectImages();
                                        }
                                    }, style: { padding: "0px 0px", height: "20px" }, children: _jsx(ButtonContainer, { children: _jsx(FaCamera, { className: "flex items-center", style: { fontSize: "14px", marginBottom: -3 } }) }) }) }), _jsx(ServersToggleModal, {}), _jsx(ValorIDERulesToggleModal, {}), _jsx(Tooltip, { tipText: "Auto Repeat Check [stubborn mode]", children: _jsx(VSCodeButton, { appearance: chatSettings.stubbornMode ? "primary" : "secondary", "aria-label": "Toggle Stubborn Mode", onClick: () => {
                                        ...
                                        vscode.postMessage({
                                    ...
                                    }, style: { padding: "0px 6px", height: 20, marginLeft: 4 }, children: _jsx(ButtonContainer, { children: _jsx(FaAngry, { className: "flex items-center", style: { fontSize: 12, marginBottom: -2 } }) }) }) }), _jsxs(ModelContainer, { ref: modelSelectorRef, children: [_jsx(ModelButtonWrapper, { ref: buttonRef, children: _jsx(ModelDisplayButton, { role: "button", isActive: showModelSelector, disabled: false, title: "Select Model / API Provider", onClick: handleModelButtonClick, 
                                            // onKeyDown={(e) => {
                                            ...
                                            // 	}
                                            // }}
                                            tabIndex: 0, children: _jsx(ModelButtonContent, { children: modelDisplayName }) }) }), showModelSelector && (_jsx(ModelSelectorTooltip, { arrowPosition: arrowPosition, menuPosition: menuPosition, style: {
                                            bottom: `calc(100vh - ${menuPosition}px + 6px)`,
                                        }, children: _jsx(ApiOptions, { showModelOptions: true, apiErrorMessage: undefined, modelIdErrorMessage: undefined, isPopup: true, saveImmediately: true }) }))] })] }), _jsx(Tooltip, { style: { zIndex: 1000 }, visible: shownTooltipMode !== null, tipText: `In ${shownTooltipMode === "act" ? "Act" : "Plan"}  mode, ValorIDE will ${shownTooltipMode === "act" ? "complete the task immediately" : "gather information to architect a plan"}`, hintText: `Toggle w/ ${metaKeyChar}+Shift+A`, children: _jsxs(SwitchContainer, { "data-testid": "mode-switch", disabled: false, onClick: onModeToggle, children: [_jsx(Slider, { isAct: chatSettings.mode === "act", isPlan: chatSettings.mode === "plan" }), _jsx(SwitchOption, { isActive: chatSettings.mode === "plan", onMouseOver: () => setShownTooltipMode("plan"), onMouseLeave: () => setShownTooltipMode(null), children: "Plan" }), _jsx(SwitchOption, { isActive: chatSettings.mode === "act", onMouseOver: () => setShownTooltipMode("act"), onMouseLeave: () => setShownTooltipMode(null), children: "Act" })] }) })] })] }));
});
