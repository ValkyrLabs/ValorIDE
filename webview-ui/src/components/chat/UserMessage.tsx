...
const UserMessage: React.FC<UserMessageProps> = ({
  ...
  const handleRestoreWorkspace = async (type: ValorIDECheckpointRestore) => {
  ...
  return (
    ...
    >
      ...
      {images && images.length > 0 && (
        <Thumbnails images={images} style={{ marginTop: "8px" }} />
      )}
    </div>
  );
};
...
interface RestoreButtonProps {
  type: ValorIDECheckpointRestore;
  label: string;
  isPrimary: boolean;
  onClick: (type: ValorIDECheckpointRestore) => void;
  title?: string;
}
