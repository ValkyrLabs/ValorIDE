...
const ChatView = ({ isHidden, showAnnouncement, hideAnnouncement, showHistoryView }: ChatViewProps) => {
	const { version, valorideMessages: messages, taskHistory, apiConfiguration, telemetrySetting } = useExtensionState()

	//const task = messages.length > 0 ? (messages[0].say === "task" ? messages[0] : undefined) : undefined) : undefined
	const task = useMemo(() => messages.at(0), [messages]) // leaving this less safe version here since if the first message is not a task, then the extension is in a bad state and needs to be debugged (see ValorIDE.abort)
	...
	const isStreaming = useMemo(() => {
		const isLastAsk = !!modifiedMessages.at(-1)?.ask // checking valorideAsk isn't enough since messages effect may be called again for a tool for example, set valorideAsk to its value, and if the next message is not an ask then it doesn't reset. This is likely due to how much more often we're updating messages as compared to before, and should be resolved with optimizations as it's likely a rendering bug. but as a final guard for now, the cancel button will show if the last message is not an ask
		const isToolCurrentlyAsking = isLastAsk && valorideAsk !== undefined && enableButtons && primaryButtonText !== undefined
		...
		return false
	}, [modifiedMessages, valorideAsk, enableButtons, primaryButtonText])
	...
	/*
	This logic depends on the useEffect[messages] above to set valorideAsk, after which buttons are shown and we then send an askResponse to the extension.
	*/
		...
		async (text?: string, images?: string[]) => {
			...
			switch (valorideAsk) {
				...
				case "resume_completed_task":
					...
					break
				case "new_task":
					console.info("new task button clicked!", { lastMessage, messages, valorideAsk, text })
	...
	const visibleMessages = useMemo(() => {
		return modifiedMessages.filter((message) => {
			switch (message.ask) {
				case "completion_result":
					// don't show a chat row for a completion_result ask without text. This specific type of message only occurs if valoride wants to execute a command as part of its completion result, in which case we interject the completion_result tool with the execute_command tool.
					if (message.text === "") {
						return false
					}
					break
			...
			switch (message.say) {
				...
				case "deleted_api_reqs": // aggregated api_req metrics from deleted messages
					return false
				case "text":
					// Sometimes valoride returns an empty text message, we don't want to render these. (We also use a say text for user messages, so in case they just sent images we still render that)
					...
					<div style={{ padding: "0 20px", flexShrink: 0 }}>
						<div style={{ backgroundColor: "#222222", padding: "0 20px", flexShrink: 0 }}>
							<a href="https://valkyrlabs.com/valoride">
								<img alt="Valkyr Labs" src="https://valkyrlabs.com/assets/VALKYR_LABS_INC_LOGO-7kbl90lk.png" />
							</a>
						</div>
						<p>
							Agentic Software Powered by{" "}
