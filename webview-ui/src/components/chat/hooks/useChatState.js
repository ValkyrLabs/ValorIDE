...
const USER_MESSAGE_SAY_VALUES = ["text", "task"];
...
const USER_MESSAGE_SAY_TYPES = new Set(USER_MESSAGE_SAY_VALUES);
const ACKNOWLEDGEMENT_SAY_TYPES = new Set(ACKNOWLEDGEMENT_SAY_VALUES);
const normalizeForComparison = (value) => value.replace(/\s+/g, " ").trim();
const buildMessageKey = (text, images) => {
    const normalized = normalizeForComparison(text ?? "");
    const imageCount = images?.length ?? 0;
    return `${normalized}::${imageCount}`;
};
const isUserMessageSayType = (say) => (say ? USER_MESSAGE_SAY_TYPES.has(say) : false);
export const useChatState = ({ messages, chatSettings }) => {
    const [valorideAsk, setValorIDEAsk] = useState(undefined);
    const [enableButtons, setEnableButtons] = useState(false);
    const [primaryButtonText, setPrimaryButtonText] = useState("Approve");
    const [secondaryButtonText, setSecondaryButtonText] = useState("Reject");
    ...
    const markUserMessagePending = useCallback((text, images) => {
                ...
                case "ask":
                    ...
                    switch (lastMessage.ask) {
                        ...
                        case "completion_result":
                            ...
                            const stubbornAttempts = messages.filter(m => m.type === "ask" && m.ask === "followup" &&
                                m.text?.includes("Are you sure you completed all")).length;
    ...
    useEffect(() => {
        if (!pendingUserMessage || !lastMessage)
            return;
        ...
        if (!pendingUserMessage.hasUserMessageEchoed) {
            const isUserEcho = lastMessage.type === "say" &&
                isUserMessageSayType(lastMessageSay) &&
                lastMessageKey === pendingUserMessage.key;
            if (isUserEcho) {
                setPendingUserMessage((prev) => prev ? { ...prev, hasUserMessageEchoed: true, userMessageTs: lastMessage.ts } : null);
            }
            return;
        }
        const isAcknowledgement = lastMessage.type === "ask" ||
