...
export const useWebSocketConnection = ({
  ...
  const ourSenderId = useMemo(
    () => (communicationService as any)?.senderId || "unknown",
  ...
  const broadcastLLMResponse = useCallback(
    (response: string) => {
      try {
  ...
  useEffect(() => {
    ...
    const handleWebSocketMessage = (event: any) => {
      ...
      if (message?.type === "BROADCAST") {
        if (
          message.action === "rollcall_request" &&
          message.senderId !== (communicationService as any).senderId
        ) {
          const ackMessage = {
            type: "BROADCAST",
            action: "rollcall_ack",
