...
export const useWebSocketConnection = ({ messages, containsValorIDEMention, clearChatInput, }) => {
    ...
    const ourSenderId = useMemo(() => communicationService?.senderId || "unknown", [communicationService]);
    ...
    const broadcastLLMResponse = useCallback((response) => {
        try {
            const wsMessage = {
        ...
        try {
            ...
            setTimeout(() => {
                const broadcastMessage = {
                    ...
                    timestamp: Date.now(),
                    payload: {
                        instanceId: communicationService.senderId || "unknown",
                ...
                vscode.postMessage({
                    ...
                    text: `Reconnecting to mothership… ${new Date().toLocaleTimeString()}`,
                });
            }, 500);
    ...
    useEffect(() => {
        ...
        const handleWebSocketMessage = (event) => {
            ...
            if (message?.type === "BROADCAST") {
                if (message.action === "rollcall_request" &&
                    message.senderId !== communicationService.senderId) {
                    const ackMessage = {
                        type: "BROADCAST",
                        action: "rollcall_ack",
                        senderId: communicationService.senderId || "unknown",
                ...
                if (payloadStr && containsValorIDEMention(payloadStr)) {
                    ...
                    if (senderId !== ourSenderId) {
                        ...
                        if (cleaned) {
                            ...
                            vscode.postMessage({
                                type: "displayVSCodeInfo",
                                text: `Received @valoride prompt from ${String(senderId).slice(-4)}`,
                            });
                        }
                    }
                }
