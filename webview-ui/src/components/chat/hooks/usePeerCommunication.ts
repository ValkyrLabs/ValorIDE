...
export const usePeerCommunication = () => {
  ...
  useEffect(() => {
    const handlePresence = (list: string[] | number) =>
      setPeerCount(
        Array.isArray(list) ? list.length : typeof list === "number" ? list : 0,
      );
    const handleP2PStatus = (s: any) =>
  ...
  useEffect(() => {
    const handleCommMessage = (message: any) => {
      ...
      if (message.type === "ping") {
        ...
        setHandshakeMessages((msgs) => [
          ...msgs,
          `Received ping at ${new Date().toLocaleTimeString()}`,
        ]);
      } else if (message.type === "ack") {
        setHandshakeMessages((msgs) => [
          ...msgs,
          `Received ack at ${new Date().toLocaleTimeString()}`,
        ]);
      } else if (message.type === "nack") {
        setHandshakeMessages((msgs) => [
          ...msgs,
          `Received nack at ${new Date().toLocaleTimeString()}`,
        ]);
      }
    };
  ...
  const connectToPeers = useCallback(() => {
    ...
    vscode.postMessage({
      ...
      text: `Broadcasting presence & connecting to broker… ${new Date().toLocaleTimeString()}`,
    });
