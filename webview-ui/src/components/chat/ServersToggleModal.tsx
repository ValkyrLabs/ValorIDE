			...
			{isVisible && (
				<div
					...
					<div className="flex justify-between items-center mb-2.5">
						...
						<div className="flex gap-1">
							<Tooltip tipText="Refresh MCP Servers">
								<VSCodeButton appearance="icon" onClick={handleRefresh} disabled={isLoading}>
									{isLoading ? (
										<VSCodeProgressRing style={{ width: "12px", height: "12px" }} />
									) : (
					...
					{isLoading && !mcpServers ? (
						...
						<div className="error-message py-2" style={{ color: "var(--vscode-errorForeground)" }}>
							<div style={{ marginBottom: "8px" }}>
								<strong>Failed to load MCP servers:</strong>
							</div>
							<div style={{ fontSize: "12px", opacity: 0.9 }}>{formatError(error)}</div>
