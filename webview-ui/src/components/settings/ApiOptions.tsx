    ...
    >
      ...
      {selectedProvider === "requesty" && (
        <div>
          ...
          {!apiConfiguration?.requestyApiKey && (
            <a href="https://app.requesty.ai/manage-api">Get API Key</a>
          )}
        </div>
      )}
      ...
      {selectedProvider === "xai" && (
        <div>
          ...
          {/* Note: To fully implement this, you would need to add a handler in ValorIDEProvider.ts */}
          {/* {apiConfiguration?.xaiApiKey && (
						<button
							onClick={() => {
								vscode.postMessage({
      ...
      {selectedProvider === "requesty" && showModelOptions && (
        <RequestyModelPicker isPopup={isPopup} />
      )}
...
export const ModelInfoView = ({
  ...
  if (
    ...
    outputPriceElement = (
      <Fragment key="outputPriceConditional">
        <span style={{ fontWeight: 500 }}>Output price (Standard):</span>{" "}
        {formatPrice(modelInfo.outputPrice)}/million tokens
        <br />
        <span style={{ fontWeight: 500 }}>
          Output price (Thinking Budget &gt; 0):
        </span>{" "}
  ...
  const infoItems = [
    ...
    <ModelInfoSupportsItem
      ...
      isSupported={modelInfo.supportsImages ?? false} // valoride browser tool uses image recognition for navigation (requires model image support).
