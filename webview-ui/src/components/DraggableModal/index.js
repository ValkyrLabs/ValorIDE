...
const computePlacementPosition = (placement, size) => {
    ...
    if (placement === "bottom-right") {
        ...
        const verticalOffset = ICON_FOOTPRINT.margin + ICON_FOOTPRINT.size + ICON_FOOTPRINT.gap;
...
const DraggableModal = ({ title, toggle, body, showModal, initialPlacement = "center", enableExpand = false, expandEventName, preferenceKey, forceCenterOnOpen = false, }) => {
    const memKey = preferenceKey ?? `UX:Floating:${String(title)}:${initialPlacement}`;
    ...
    const hasStoredPosition = typeof state.x === "number" &&
        !Number.isNaN(state.x) &&
        typeof state.y === "number" &&
        !Number.isNaN(state.y);
    const placementPosition = useMemo(() => computePlacementPosition(initialPlacement, size), [initialPlacement, size]);
    const clampPositionToViewport = useCallback((next) => {
        const { width: vw, height: vh } = getViewport();
        const gutter = 16;
    ...
    useEffect(() => {
        ...
        if (forceCenterOnOpen && next && !open) {
            const centered = clampPositionToViewport(computePlacementPosition("center", sizeRef.current));
    ...
    const toggleExpand = useCallback(() => {
        ...
        if (isExpanded) {
            ...
            const collapsedHasPosition = typeof state.collapsedX === "number" &&
    ...
    const handleResizeStart = (event) => {
        ...
        const handlePointerMove = (moveEvent) => {
            nextWidth = Math.max(MIN_WIDTH, startWidth + (moveEvent.clientX - startX));
            nextHeight = Math.max(MIN_HEIGHT, startHeight + (moveEvent.clientY - startY));
    ...
    const modalContent = (_jsx(Draggable, { handle: ".modal-header", cancel: ".draggable-modal-contents, .btn, .form-control, textarea, input, select", position: position, onDrag: handleDrag, onStop: handleStop, children: _jsx("div", { style: wrapperStyle, children: _jsxs(Modal, { show: true, onHide: () => {
                    setOpen(false);
                    toggle();
                }, backdrop: false, animation: false, dialogClassName: "draggable-modal-dialog", contentClassName: "draggable-modal-content", children: [_jsxs(Modal.Header, { closeButton: true, className: "draggable-modal-header", children: [_jsx(Modal.Title, { className: "draggable-modal-title", children: title }), enableExpand && (_jsxs("button", { type: "button", className: "draggable-modal-expand", onClick: toggleExpand, children: [isExpanded ? _jsx(FaCompress, { size: 34 }) : _jsx(FaExpand, { size: 34 }), _jsx("span", { children: isExpanded ? "Collapse window" : "Expand window" })] }))] }), _jsx(Modal.Body, { className: "draggable-modal-contents", children: body }), _jsx("div", { className: "draggable-modal-resize-handle", role: "presentation", onPointerDown: handleResizeStart })] }) }) }));
    const launcherButton = (_jsx("div", { className: "draggable-modal-launcher", children: _jsx("button", { style: {
                width: 56,
                height: 56,
                borderRadius: "50%",
                display: "flex",
                ...
                setOpen(true);
                toggle();
            }, "aria-label": "Open SageChat", children: _jsx(FaRobot, {}) }) }));
    return (_jsxs(_Fragment, { children: [!open &&
                typeof window !== "undefined" &&
                ReactDOM.createPortal(launcherButton, getOrCreatePortalRoot()), open &&
                typeof window !== "undefined" &&
