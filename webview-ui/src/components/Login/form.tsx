...
const Form: React.FC = () => {
	...
	if (loginSuccess) {
		...
		if (loginUserResult.data && loginUserResult.data.token) {
			...
			sessionStorage.setItem("authenticatedPrincipal", JSON.stringify(loginUserResult.data.authenticatedPrincipal))
		}
	}
	...
	const handleSubmit = (values: FormikValues, { setSubmitting }: FormikHelpers<Login>) => {
		setTimeout(() => {
			console.log(values)
			loginUser(values)
			setSubmitting(false)
