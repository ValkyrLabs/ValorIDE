	...
	return (
		<div style={{ padding: "16px 20px" }}>
			<div
				...
				ValorIDE's capabilities. You can use{" "}
			...
			{isLoading && !mcpServers ? (
				...
				<div
					...
					<div style={{ fontSize: "12px", opacity: 0.9, marginBottom: "12px" }}>{formatError(error)}</div>
					{isRetryableError(error) && (
						<VSCodeButton
							...
							<span className="codicon codicon-refresh" style={{ marginRight: "4px" }}></span>
							{isLoading ? "Retrying..." : "Retry"}
						</VSCodeButton>
					)}
				</div>
			) : (
				<ServersToggleList servers={sharedMcpServers} isExpandable={true} hasTrashIcon={false} />
			)}
