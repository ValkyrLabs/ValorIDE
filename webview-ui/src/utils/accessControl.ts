...
function dispatchJwtTokenUpdated(token: string | null, source: string) {
  if (!inBrowser) return;
  try {
...
export function shouldPersistJwt(): boolean {
...
function storePrincipalInStorage(
 ...
 */
export function writeStoredPrincipal(principal: StoredPrincipal): void {
  if (!inBrowser) return;
  try {
    if (!principal) {
 ...
 */
export function readStoredPrincipal(): StoredPrincipal | null {
  if (!inBrowser) return null;
  try {
    let principalData =
      safeGet(window.sessionStorage, PRINCIPAL_PRIMARY_KEY) ??
 ...
 */
export function clearStoredPrincipal(source: string = "manual"): void {
  if (!inBrowser) return;
  try {
...
export function hydrateStoredCredentials(
  ...
  let principal: StoredPrincipal | undefined;
  let principalRaw =
    safeGet(window.sessionStorage, PRINCIPAL_PRIMARY_KEY) ??
    safeGet(window.sessionStorage, PRINCIPAL_FALLBACK_KEY);
 ...
 */
export function clearStoredJwtToken(source: string = "manual"): void {
  if (!inBrowser) return;
