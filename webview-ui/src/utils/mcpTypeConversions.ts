...
function normalizeServerStatus(status: unknown): McpServerStatusEnum {
    ...
    if (normalizedStatus === McpServerStatusEnum.CONNECTED ||
    ...
    try {
        return {
            ...
            tools: thorServer.tools
                ...
                .filter(Boolean) || [],
            resources: thorServer.resources
                ?.map((resource) => {
                if (!resource)
                    return null;
                try {
                ...
                .filter(Boolean) || [],
            resourceTemplates: thorServer.resourceTemplates
                ?.map((template) => {
                if (!template)
                    return null;
                try {
...
export function convertThorMcpMarketplaceItemsToShared(thorItems) {
    if (!thorItems || !Array.isArray(thorItems)) {
        console.warn("Attempted to convert null/undefined/invalid ThorMcpMarketplaceItem array");
        return [];
    }
    try {
        ...
        console.error("Failed to convert ThorMcpMarketplaceItem array to SharedMcpMarketplaceItem array:", error, thorItems);
        return [];
    }
}
/**
