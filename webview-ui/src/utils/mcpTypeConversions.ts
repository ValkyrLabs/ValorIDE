import { McpServerStatusEnum } from "@thorapi/model/McpServer";

/**
 * Safely parse JSON string with error handling
 */
function safeJsonParse(jsonString: unknown) {
 ...
 */
function normalizeServerStatus(status: unknown): McpServerStatusEnum {
  if (!status || typeof status !== "string")
    return McpServerStatusEnum.DISCONNECTED;
  const normalizedStatus = status.toLowerCase();
  if (
    normalizedStatus === McpServerStatusEnum.CONNECTED ||
    normalizedStatus === McpServerStatusEnum.CONNECTING ||
    normalizedStatus === McpServerStatusEnum.DISCONNECTED
  ) {
    return normalizedStatus as McpServerStatusEnum;
  }
  return McpServerStatusEnum.DISCONNECTED;
}
/**
 ...
 */
export function convertThorMcpServerToShared(thorServer: any) {
  if (!thorServer) {
  ...
  try {
        ...
        thorServer.resourceTemplates
          ?.map((template) => {
            ...
            try {
              ...
              console.warn(
                "Failed to convert resource template:",
                error,
                template,
              );
              return null;
    ...
    console.error(
      "Failed to convert ThorMcpServer to SharedMcpServer:",
      error,
      thorServer,
    );
    return null;
 ...
 */
export function convertThorMcpMarketplaceItemToShared(thorItem) {
  if (!thorItem) {
  ...
  try {
    ...
    console.error(
      "Failed to convert ThorMcpMarketplaceItem to SharedMcpMarketplaceItem:",
      error,
      thorItem,
    );
    return null;
 ...
 */
export function convertThorMcpMarketplaceCatalogToShared(thorCatalog) {
  if (!thorCatalog) {
    console.warn(
      "Attempted to convert null/undefined ThorMcpMarketplaceCatalog",
    );
    return { items: [] };
  }
  try {
    return {
      items:
        thorCatalog.items
          ?.map(convertThorMcpMarketplaceItemToShared)
          .filter(Boolean) || [],
    };
  } catch (error) {
    console.error(
      "Failed to convert ThorMcpMarketplaceCatalog to SharedMcpMarketplaceCatalog:",
      error,
      thorCatalog,
    );
    return { items: [] };
 ...
 */
export function convertThorMcpServersToShared(thorServers) {
  if (!thorServers || !Array.isArray(thorServers)) {
    console.warn(
      "Attempted to convert null/undefined/invalid ThorMcpServer array",
    );
    return [];
  }
  try {
    ...
    console.error(
      "Failed to convert ThorMcpServer array to SharedMcpServer array:",
      error,
      thorServers,
    );
    return [];
 ...
 */
export function convertThorMcpMarketplaceItemsToShared(thorItems) {
 ...
 */
export function convertThorMcpMarketplaceCatalogsToShared(thorCatalogs) {
  if (!thorCatalogs || !Array.isArray(thorCatalogs)) {
    console.warn(
      "Attempted to convert null/undefined/invalid ThorMcpMarketplaceCatalog array",
    );
    return { items: [] };
  }
  try {
    ...
    console.error(
      "Failed to convert ThorMcpMarketplaceCatalog array to SharedMcpMarketplaceCatalog:",
      error,
      thorCatalogs,
    );
    return { items: [] };
