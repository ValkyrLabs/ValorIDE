/**
 * Node Sanitization for Redux Storage
 *
 * React Flow nodes can contain React components (icons) with Symbol properties.
 * These cannot be serialized to Redux. This module provides utilities to:
 * 1. Strip React components from node data before storage
 * 2. Reconstruct them on load (if needed)
 */
 ...
 */
export function sanitizeNodeForRedux<T extends FlowNodeLike>(node: T): T {
  if (!node) {
    return node;
  }
 ...
 */
export function sanitizeNodeData(data: any, depth = 0, maxDepth = 50): any {
  if (depth > maxDepth) {
    return data;
  }
 ...
 */
export function sanitizeNodesForRedux(nodes: FlowNodeLike[]): FlowNodeLike[] {
