...
export function sanitizeForRedux(obj, depth = 0, maxDepth = 100) {
    ...
    if (obj instanceof Set) {
        return Array.from(obj).map((item) => sanitizeForRedux(item, depth + 1, maxDepth));
    }
    ...
    return (async (args, api, extraOptions) => {
        try {
            ...
            if (error instanceof TypeError && String(error).includes("symbol")) {
                console.warn("[sanitizeBaseQuery] Symbol detected in response, attempting sanitization", error);
