...
import { Fzf } from "fzf";
export function insertMention(text, position, value) {
...
export function insertMentionDirectly(text, position, value) {
    const beforeCursor = text.slice(0, position);
    const afterCursor = text.slice(position);
...
export function removeMention(text, position) {
    ...
    if (matchEnd) {
        ...
        const newText = text.slice(0, position - matchEnd[0].length) +
...
export function getContextMenuOptions(query, selectedType = null, queryItems, dynamicSearchResults = []) {
    ...
    if (/^[a-f0-9]{7,40}$/i.test(lowerQuery)) {
        const exactMatches = queryItems.filter((item) => item.type === ContextMenuOptionType.Git &&
    ...
    const fileMatches = matchingItems.filter((item) => item.type === ContextMenuOptionType.File ||
        item.type === ContextMenuOptionType.Folder);
    const gitMatches = matchingItems.filter((item) => item.type === ContextMenuOptionType.Git);
    const otherMatches = matchingItems.filter((item) => item.type !== ContextMenuOptionType.File &&
        item.type !== ContextMenuOptionType.Folder &&
    ...
    if (suggestions.length > 0 || matchingItems.length > 0) {
        ...
        const deduped = allItems.filter((item) => {
            ...
            const normalizedValue = item.value && !item.value.startsWith("/")
                ? `/${item.value}`
                : item.value;
