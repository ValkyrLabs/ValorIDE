 ...
 */
export function removeSlashCommand(text, position) {
  ...
  if (matchEnd) {
    ...
    const newText =
      ...
      afterCursor.replace(" ", ""); // removes the first space after the command
 ...
 */
export function shouldShowSlashCommandsMenu(text, cursorPosition) {
 ...
 */
export function getMatchingSlashCommands(query) {
  if (!query) {
 ...
 */
export function insertSlashCommand(text, commandName) {
...
export function validateSlashCommand(command) {
  ...
  const exactMatch = SUPPORTED_SLASH_COMMANDS.some(
    (cmd) => cmd.name === command,
  );
  if (exactMatch) {
    return "full";
  }
  const partialMatch = SUPPORTED_SLASH_COMMANDS.some((cmd) =>
