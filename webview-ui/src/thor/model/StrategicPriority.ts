...
import { Goal, GoalFromJSON, GoalToJSON } from "./";
 ...
 */
export type StrategicPriority = DataObject & {
  /**
   *
   * @type {string}
...
export function StrategicPriorityFromJSON(json: any): StrategicPriority {
  return {
    ...
    goals: !exists(json, "goals")
      ? undefined
      : (json["goals"] as Array<any>).map(GoalFromJSON),
  ...
  return {
    ...
    goals:
      value.goals === undefined
        ? undefined
        : (value.goals as Array<any>).map(GoalToJSON),
    status: value.status,
    id: value.id,
