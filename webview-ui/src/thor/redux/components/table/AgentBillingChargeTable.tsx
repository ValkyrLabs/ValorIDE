...
import { AgentBillingCharge } from "@thor/model";
...
const AgentBillingChargeTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<AgentBillingCharge[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<AgentBillingCharge>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<AgentBillingCharge | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAgentBillingCharge(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<AgentBillingCharge> = { ...source };
      delete (payload as any).id;
      const created = await addAgentBillingCharge(payload).unwrap();
      setAllData((prev) => [created as AgentBillingCharge, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<AgentBillingCharge> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addAgentBillingCharge(payload).unwrap();
      setAllData((prev) => [created as AgentBillingCharge, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="AgentBillingChargeTable"
        ...
        >
          <FloatingControlPanel
            description="AgentBillingCharge Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create AgentBillingCharge</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <AgentBillingChargeForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter AgentBillingCharge (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing AgentBillingCharge</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof AgentBillingCharge;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of AgentBillingCharges.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "AgentBillingCharge"}
