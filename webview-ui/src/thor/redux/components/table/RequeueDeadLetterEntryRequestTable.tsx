...
import { RequeueDeadLetterEntryRequest } from '@thor/model';
...
import {
  useLazyGetRequeueDeadLetterEntryRequestsPagedQuery,
...
} from '@thor/redux/services/RequeueDeadLetterEntryRequestService';
...
const RequeueDeadLetterEntryRequestTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<RequeueDeadLetterEntryRequest[]>([]);
...
// AddressToJSON(value?: Address)
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RequeueDeadLetterEntryRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRequeueDeadLetterEntryRequest(payload).unwrap();
      setAllData((prev) => [created as RequeueDeadLetterEntryRequest, ...prev]);
    } catch (error) {
