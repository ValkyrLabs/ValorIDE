...
import { ToolFailure } from "@thor/model";
...
const ToolFailureTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ToolFailure[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<ToolFailure>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ToolFailure | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteToolFailure(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ToolFailure> = { ...source };
      delete (payload as any).id;
      const created = await addToolFailure(payload).unwrap();
      setAllData((prev) => [created as ToolFailure, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ToolFailure> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addToolFailure(payload).unwrap();
      setAllData((prev) => [created as ToolFailure, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <FloatingControlPanel
            description="ToolFailure Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create ToolFailure</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <ToolFailureForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter ToolFailure (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing ToolFailure</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof ToolFailure;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of ToolFailures.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "ToolFailure"}
