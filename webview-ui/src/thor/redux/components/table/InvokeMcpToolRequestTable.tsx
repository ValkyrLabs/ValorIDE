...
const InvokeMcpToolRequestTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<InvokeMcpToolRequest[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<InvokeMcpToolRequest>>({});
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteInvokeMcpToolRequest(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<InvokeMcpToolRequest> = { ...source };
      delete (payload as any).id;
      const created = await addInvokeMcpToolRequest(payload).unwrap();
      setAllData((prev) => [created as InvokeMcpToolRequest, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<InvokeMcpToolRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addInvokeMcpToolRequest(payload).unwrap();
      setAllData((prev) => [created as InvokeMcpToolRequest, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="InvokeMcpToolRequestTable"
        ...
        >
          <FloatingControlPanel
            description="InvokeMcpToolRequest Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create InvokeMcpToolRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <InvokeMcpToolRequestForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter InvokeMcpToolRequest (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing InvokeMcpToolRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof InvokeMcpToolRequest;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of InvokeMcpToolRequests.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "InvokeMcpToolRequest"}
