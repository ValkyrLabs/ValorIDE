...
import { WebsocketSession } from '@thor/model';
...
const WebsocketSessionTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<WebsocketSession[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<WebsocketSession>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<WebsocketSession | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWebsocketSession(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<WebsocketSession> = { ...source };
      delete (payload as any).id;
      const created = await addWebsocketSession(payload).unwrap();
      setAllData((prev) => [created as WebsocketSession, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<WebsocketSession> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addWebsocketSession(payload).unwrap();
      setAllData((prev) => [created as WebsocketSession, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="WebsocketSessionTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="WebsocketSession Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create WebsocketSession</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <WebsocketSessionForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter WebsocketSession (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing WebsocketSession</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof WebsocketSession;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of WebsocketSessions.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'WebsocketSession'}
