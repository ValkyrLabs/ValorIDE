...
import { ContentMediaLink } from '@thor/model';
...
const ContentMediaLinkTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<ContentMediaLink[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteContentMediaLink(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ContentMediaLink> = { ...source };
      delete (payload as any).id;
      const created = await addContentMediaLink(payload).unwrap();
      setAllData((prev) => [created as ContentMediaLink, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ContentMediaLink> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addContentMediaLink(payload).unwrap();
      setAllData((prev) => [created as ContentMediaLink, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="ContentMediaLinkTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ContentMediaLink Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ContentMediaLink</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ContentMediaLink (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ContentMediaLink;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ContentMediaLinks.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ContentMediaLink'}
