...
import { SalesActivity } from "@thor/model";
...
const SalesActivityTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<SalesActivity[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSalesActivity(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SalesActivity> = { ...source };
      delete (payload as any).id;
      const created = await addSalesActivity(payload).unwrap();
      setAllData((prev) => [created as SalesActivity, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SalesActivity> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSalesActivity(payload).unwrap();
      setAllData((prev) => [created as SalesActivity, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <FloatingControlPanel
            description="SalesActivity Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create SalesActivity</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <SalesActivityForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter SalesActivity (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof SalesActivity;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of SalesActivitys.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "SalesActivity"}
