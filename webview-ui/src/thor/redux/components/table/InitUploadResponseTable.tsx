...
const InitUploadResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<InitUploadResponse[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<InitUploadResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<InitUploadResponse | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteInitUploadResponse(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<InitUploadResponse> = { ...source };
      delete (payload as any).id;
      const created = await addInitUploadResponse(payload).unwrap();
      setAllData((prev) => [created as InitUploadResponse, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<InitUploadResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addInitUploadResponse(payload).unwrap();
      setAllData((prev) => [created as InitUploadResponse, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="InitUploadResponseTable"
        ...
        >
          <FloatingControlPanel
            description="InitUploadResponse Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create InitUploadResponse</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <InitUploadResponseForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter InitUploadResponse (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing InitUploadResponse</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof InitUploadResponse;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of InitUploadResponses.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "InitUploadResponse"}
