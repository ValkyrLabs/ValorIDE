...
const PublishWorkflowRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<PublishWorkflowRequest[]>([]);
  ...
  const [updatePublishWorkflowRequest] =
    useUpdatePublishWorkflowRequestMutation();
  const [deletePublishWorkflowRequest] =
    useDeletePublishWorkflowRequestMutation();
  ...
  const [formData, setFormData] = useState<Partial<PublishWorkflowRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<PublishWorkflowRequest | null>(
    null,
  );
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePublishWorkflowRequest(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<PublishWorkflowRequest> = { ...source };
      delete (payload as any).id;
      const created = await addPublishWorkflowRequest(payload).unwrap();
      setAllData((prev) => [created as PublishWorkflowRequest, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<PublishWorkflowRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addPublishWorkflowRequest(payload).unwrap();
      setAllData((prev) => [created as PublishWorkflowRequest, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="PublishWorkflowRequestTable"
        ...
        >
          <FloatingControlPanel
            description="PublishWorkflowRequest Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create PublishWorkflowRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <PublishWorkflowRequestForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter PublishWorkflowRequest (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing PublishWorkflowRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof PublishWorkflowRequest;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of PublishWorkflowRequests.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "PublishWorkflowRequest"}
