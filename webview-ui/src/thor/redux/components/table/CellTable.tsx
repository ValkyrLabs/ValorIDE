...
import {
  useLazyGetCellsPagedQuery,
  useAddCellMutation,
  useUpdateCellMutation,
  useDeleteCellMutation
} from '@thor/redux/services/CellService';
...
const CellTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Cell[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCell(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Cell> = { ...source };
      delete (payload as any).id;
      const created = await addCell(payload).unwrap();
      setAllData((prev) => [created as Cell, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Cell> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addCell(payload).unwrap();
      setAllData((prev) => [created as Cell, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create Cell</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <CellForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter Cell (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof Cell;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of Cells.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'Cell'}
