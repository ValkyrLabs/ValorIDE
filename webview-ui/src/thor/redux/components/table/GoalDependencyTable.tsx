...
const GoalDependencyTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<GoalDependency[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteGoalDependency(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<GoalDependency> = { ...source };
      delete (payload as any).id;
      const created = await addGoalDependency(payload).unwrap();
      setAllData((prev) => [created as GoalDependency, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<GoalDependency> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addGoalDependency(payload).unwrap();
      setAllData((prev) => [created as GoalDependency, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="GoalDependencyTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="GoalDependency Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create GoalDependency</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <GoalDependencyForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter GoalDependency (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof GoalDependency;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of GoalDependencys.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'GoalDependency'}
