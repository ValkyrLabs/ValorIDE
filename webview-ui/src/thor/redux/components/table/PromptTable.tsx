...
import {
  useLazyGetPromptsPagedQuery,
  useAddPromptMutation,
  useUpdatePromptMutation,
  useDeletePromptMutation,
} from "@thor/redux/services/PromptService";
...
const PromptTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Prompt[]>([]);
  ...
  const [copiedRow, setCopiedRow] = useState<Prompt | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePrompt(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Prompt> = { ...source };
      delete (payload as any).id;
      const created = await addPrompt(payload).unwrap();
      setAllData((prev) => [created as Prompt, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Prompt> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addPrompt(payload).unwrap();
      setAllData((prev) => [created as Prompt, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter Prompt (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing Prompt</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof Prompt;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of Prompts.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "Prompt"}
