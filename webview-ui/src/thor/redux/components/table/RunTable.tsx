...
import {
  useLazyGetRunsPagedQuery,
  useAddRunMutation,
  useUpdateRunMutation,
  useDeleteRunMutation
} from '@thor/redux/services/RunService';
...
const RunTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Run[]>([]);
  ...
  const [copiedRow, setCopiedRow] = useState<Run | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRun(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Run> = { ...source };
      delete (payload as any).id;
      const created = await addRun(payload).unwrap();
      setAllData((prev) => [created as Run, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Run> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRun(payload).unwrap();
      setAllData((prev) => [created as Run, ...prev]);
    } catch (error) {
        ...
        <div className="tableContainer">
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create Run</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RunForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter Run (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing Run</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof Run;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of Runs.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
