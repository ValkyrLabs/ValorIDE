...
const SpaceFileTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<SpaceFile[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<SpaceFile>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SpaceFile | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSpaceFile(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SpaceFile> = { ...source };
      delete (payload as any).id;
      const created = await addSpaceFile(payload).unwrap();
      setAllData((prev) => [created as SpaceFile, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SpaceFile> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSpaceFile(payload).unwrap();
      setAllData((prev) => [created as SpaceFile, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SpaceFile</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SpaceFileForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SpaceFile (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SpaceFile</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SpaceFile;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SpaceFiles.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SpaceFile'}
