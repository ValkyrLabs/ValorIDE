...
const ChatCompletionRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ChatCompletionRequest[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<ChatCompletionRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ChatCompletionRequest | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteChatCompletionRequest(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ChatCompletionRequest> = { ...source };
      delete (payload as any).id;
      const created = await addChatCompletionRequest(payload).unwrap();
      setAllData((prev) => [created as ChatCompletionRequest, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ChatCompletionRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addChatCompletionRequest(payload).unwrap();
      setAllData((prev) => [created as ChatCompletionRequest, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="ChatCompletionRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ChatCompletionRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ChatCompletionRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ChatCompletionRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ChatCompletionRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ChatCompletionRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ChatCompletionRequest;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ChatCompletionRequests.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ChatCompletionRequest'}
