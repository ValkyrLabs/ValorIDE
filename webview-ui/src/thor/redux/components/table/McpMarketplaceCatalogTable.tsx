...
import { McpMarketplaceCatalog } from '@thor/model';
...
const McpMarketplaceCatalogTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<McpMarketplaceCatalog[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<McpMarketplaceCatalog>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<McpMarketplaceCatalog | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpMarketplaceCatalog(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<McpMarketplaceCatalog> = { ...source };
      delete (payload as any).id;
      const created = await addMcpMarketplaceCatalog(payload).unwrap();
      setAllData((prev) => [created as McpMarketplaceCatalog, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<McpMarketplaceCatalog> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMcpMarketplaceCatalog(payload).unwrap();
      setAllData((prev) => [created as McpMarketplaceCatalog, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="McpMarketplaceCatalogTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="McpMarketplaceCatalog Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create McpMarketplaceCatalog</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <McpMarketplaceCatalogForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter McpMarketplaceCatalog (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing McpMarketplaceCatalog</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof McpMarketplaceCatalog;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of McpMarketplaceCatalogs.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'McpMarketplaceCatalog'}
