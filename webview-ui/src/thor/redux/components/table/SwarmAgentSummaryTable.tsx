...
import { SwarmAgentSummary } from '@thor/model';
...
const SwarmAgentSummaryTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SwarmAgentSummary[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<SwarmAgentSummary>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SwarmAgentSummary | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmAgentSummary(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SwarmAgentSummary> = { ...source };
      delete (payload as any).id;
      const created = await addSwarmAgentSummary(payload).unwrap();
      setAllData((prev) => [created as SwarmAgentSummary, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SwarmAgentSummary> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSwarmAgentSummary(payload).unwrap();
      setAllData((prev) => [created as SwarmAgentSummary, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="SwarmAgentSummaryTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="SwarmAgentSummary Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SwarmAgentSummary</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SwarmAgentSummaryForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SwarmAgentSummary (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SwarmAgentSummary</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SwarmAgentSummary;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SwarmAgentSummarys.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SwarmAgentSummary'}
