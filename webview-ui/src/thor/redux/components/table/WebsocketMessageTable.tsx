...
const WebsocketMessageTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<WebsocketMessage[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<WebsocketMessage>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<WebsocketMessage | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWebsocketMessage(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<WebsocketMessage> = { ...source };
      delete (payload as any).id;
      const created = await addWebsocketMessage(payload).unwrap();
      setAllData((prev) => [created as WebsocketMessage, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<WebsocketMessage> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addWebsocketMessage(payload).unwrap();
      setAllData((prev) => [created as WebsocketMessage, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="WebsocketMessageTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="WebsocketMessage Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create WebsocketMessage</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <WebsocketMessageForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter WebsocketMessage (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing WebsocketMessage</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof WebsocketMessage;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of WebsocketMessages.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'WebsocketMessage'}
