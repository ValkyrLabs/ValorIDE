...
import { FileMetadata } from "@thor/model";
...
const FileMetadataTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<FileMetadata[]>([]);
  ...
  const [copiedRow, setCopiedRow] = useState<FileMetadata | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteFileMetadata(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<FileMetadata> = { ...source };
      delete (payload as any).id;
      const created = await addFileMetadata(payload).unwrap();
      setAllData((prev) => [created as FileMetadata, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<FileMetadata> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addFileMetadata(payload).unwrap();
      setAllData((prev) => [created as FileMetadata, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <FloatingControlPanel
            description="FileMetadata Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create FileMetadata</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <FileMetadataForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter FileMetadata (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing FileMetadata</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof FileMetadata;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of FileMetadatas.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "FileMetadata"}
