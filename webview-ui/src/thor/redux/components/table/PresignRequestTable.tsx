...
import { PresignRequest } from "@thor/model";
...
const PresignRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<PresignRequest[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<PresignRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<PresignRequest | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePresignRequest(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<PresignRequest> = { ...source };
      delete (payload as any).id;
      const created = await addPresignRequest(payload).unwrap();
      setAllData((prev) => [created as PresignRequest, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<PresignRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addPresignRequest(payload).unwrap();
      setAllData((prev) => [created as PresignRequest, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="PresignRequestTable"
        ...
        >
          <FloatingControlPanel
            description="PresignRequest Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create PresignRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <PresignRequestForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter PresignRequest (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing PresignRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof PresignRequest;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of PresignRequests.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "PresignRequest"}
