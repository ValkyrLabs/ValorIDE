...
import { McpTransportConfig } from "@thor/model";
...
const McpTransportConfigTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<McpTransportConfig[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<McpTransportConfig>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<McpTransportConfig | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpTransportConfig(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<McpTransportConfig> = { ...source };
      delete (payload as any).id;
      const created = await addMcpTransportConfig(payload).unwrap();
      setAllData((prev) => [created as McpTransportConfig, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<McpTransportConfig> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMcpTransportConfig(payload).unwrap();
      setAllData((prev) => [created as McpTransportConfig, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="McpTransportConfigTable"
        ...
        >
          <FloatingControlPanel
            description="McpTransportConfig Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create McpTransportConfig</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <McpTransportConfigForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter McpTransportConfig (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing McpTransportConfig</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof McpTransportConfig;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of McpTransportConfigs.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "McpTransportConfig"}
