...
import { BlankRange } from '@thor/model';
...
const BlankRangeTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<BlankRange[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteBlankRange(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<BlankRange> = { ...source };
      delete (payload as any).id;
      const created = await addBlankRange(payload).unwrap();
      setAllData((prev) => [created as BlankRange, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<BlankRange> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addBlankRange(payload).unwrap();
      setAllData((prev) => [created as BlankRange, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          >
            ...
            <FloatingControlPanel
              description="BlankRange Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create BlankRange</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <BlankRangeForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter BlankRange (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof BlankRange;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of BlankRanges.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'BlankRange'}
