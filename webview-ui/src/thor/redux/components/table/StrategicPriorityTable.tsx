...
import { StrategicPriority } from '@thor/model';
...
const StrategicPriorityTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<StrategicPriority[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteStrategicPriority(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<StrategicPriority> = { ...source };
      delete (payload as any).id;
      const created = await addStrategicPriority(payload).unwrap();
      setAllData((prev) => [created as StrategicPriority, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<StrategicPriority> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addStrategicPriority(payload).unwrap();
      setAllData((prev) => [created as StrategicPriority, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="StrategicPriorityTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="StrategicPriority Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create StrategicPriority</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter StrategicPriority (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof StrategicPriority;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of StrategicPrioritys.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'StrategicPriority'}
