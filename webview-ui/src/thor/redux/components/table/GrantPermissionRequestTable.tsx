...
import { GrantPermissionRequest } from "@thor/model";
...
const GrantPermissionRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<GrantPermissionRequest[]>([]);
  ...
  const [updateGrantPermissionRequest] =
    useUpdateGrantPermissionRequestMutation();
  const [deleteGrantPermissionRequest] =
    useDeleteGrantPermissionRequestMutation();
  ...
  const [formData, setFormData] = useState<Partial<GrantPermissionRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<GrantPermissionRequest | null>(
    null,
  );
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteGrantPermissionRequest(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<GrantPermissionRequest> = { ...source };
      delete (payload as any).id;
      const created = await addGrantPermissionRequest(payload).unwrap();
      setAllData((prev) => [created as GrantPermissionRequest, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<GrantPermissionRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addGrantPermissionRequest(payload).unwrap();
      setAllData((prev) => [created as GrantPermissionRequest, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="GrantPermissionRequestTable"
        ...
        >
          <FloatingControlPanel
            description="GrantPermissionRequest Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create GrantPermissionRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <GrantPermissionRequestForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter GrantPermissionRequest (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing GrantPermissionRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof GrantPermissionRequest;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of GrantPermissionRequests.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "GrantPermissionRequest"}
