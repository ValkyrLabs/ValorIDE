...
import { OasObjectSchema } from '@thor/model';
...
const OasObjectSchemaTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<OasObjectSchema[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOasObjectSchema(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<OasObjectSchema> = { ...source };
      delete (payload as any).id;
      const created = await addOasObjectSchema(payload).unwrap();
      setAllData((prev) => [created as OasObjectSchema, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<OasObjectSchema> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addOasObjectSchema(payload).unwrap();
      setAllData((prev) => [created as OasObjectSchema, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="OasObjectSchemaTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="OasObjectSchema Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create OasObjectSchema</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <OasObjectSchemaForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter OasObjectSchema (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof OasObjectSchema;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of OasObjectSchemas.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'OasObjectSchema'}
