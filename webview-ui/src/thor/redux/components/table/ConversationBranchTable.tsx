...
import { ConversationBranch } from "@thor/model";
...
const ConversationBranchTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ConversationBranch[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<ConversationBranch>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ConversationBranch | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteConversationBranch(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ConversationBranch> = { ...source };
      delete (payload as any).id;
      const created = await addConversationBranch(payload).unwrap();
      setAllData((prev) => [created as ConversationBranch, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ConversationBranch> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addConversationBranch(payload).unwrap();
      setAllData((prev) => [created as ConversationBranch, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="ConversationBranchTable"
        ...
        >
          <FloatingControlPanel
            description="ConversationBranch Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create ConversationBranch</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <ConversationBranchForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter ConversationBranch (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing ConversationBranch</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof ConversationBranch;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of ConversationBranchs.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "ConversationBranch"}
