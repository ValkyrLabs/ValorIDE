...
const BuildOutputTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<BuildOutput[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteBuildOutput(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<BuildOutput> = { ...source };
      delete (payload as any).id;
      const created = await addBuildOutput(payload).unwrap();
      setAllData((prev) => [created as BuildOutput, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<BuildOutput> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addBuildOutput(payload).unwrap();
      setAllData((prev) => [created as BuildOutput, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          >
            ...
            <FloatingControlPanel
              description="BuildOutput Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create BuildOutput</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <BuildOutputForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter BuildOutput (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof BuildOutput;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of BuildOutputs.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'BuildOutput'}
