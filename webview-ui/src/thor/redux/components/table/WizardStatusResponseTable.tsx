...
import { WizardStatusResponse } from '@thor/model';
...
const WizardStatusResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<WizardStatusResponse[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<WizardStatusResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<WizardStatusResponse | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWizardStatusResponse(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<WizardStatusResponse> = { ...source };
      delete (payload as any).id;
      const created = await addWizardStatusResponse(payload).unwrap();
      setAllData((prev) => [created as WizardStatusResponse, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<WizardStatusResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addWizardStatusResponse(payload).unwrap();
      setAllData((prev) => [created as WizardStatusResponse, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="WizardStatusResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="WizardStatusResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create WizardStatusResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <WizardStatusResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter WizardStatusResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing WizardStatusResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof WizardStatusResponse;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of WizardStatusResponses.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'WizardStatusResponse'}
