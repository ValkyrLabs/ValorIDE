...
import { CareerOpportunity } from "@thor/model";
...
const CareerOpportunityTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<CareerOpportunity[]>([]);
  ...
  const [copiedRow, setCopiedRow] = useState<CareerOpportunity | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCareerOpportunity(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<CareerOpportunity> = { ...source };
      delete (payload as any).id;
      const created = await addCareerOpportunity(payload).unwrap();
      setAllData((prev) => [created as CareerOpportunity, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<CareerOpportunity> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addCareerOpportunity(payload).unwrap();
      setAllData((prev) => [created as CareerOpportunity, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="CareerOpportunityTable"
        ...
        >
          <FloatingControlPanel
            description="CareerOpportunity Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create CareerOpportunity</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter CareerOpportunity (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing CareerOpportunity</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof CareerOpportunity;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of CareerOpportunitys.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "CareerOpportunity"}
