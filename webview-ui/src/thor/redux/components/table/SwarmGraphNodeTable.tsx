...
import { SwarmGraphNode } from '@thor/model';
...
const SwarmGraphNodeTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SwarmGraphNode[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<SwarmGraphNode>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SwarmGraphNode | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmGraphNode(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SwarmGraphNode> = { ...source };
      delete (payload as any).id;
      const created = await addSwarmGraphNode(payload).unwrap();
      setAllData((prev) => [created as SwarmGraphNode, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SwarmGraphNode> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSwarmGraphNode(payload).unwrap();
      setAllData((prev) => [created as SwarmGraphNode, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="SwarmGraphNodeTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="SwarmGraphNode Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SwarmGraphNode</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SwarmGraphNodeForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SwarmGraphNode (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SwarmGraphNode</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SwarmGraphNode;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SwarmGraphNodes.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SwarmGraphNode'}
