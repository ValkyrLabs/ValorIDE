...
import { JobSeeker } from "@thor/model";
...
const JobSeekerTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<JobSeeker[]>([]);
  ...
  const [copiedRow, setCopiedRow] = useState<JobSeeker | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteJobSeeker(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<JobSeeker> = { ...source };
      delete (payload as any).id;
      const created = await addJobSeeker(payload).unwrap();
      setAllData((prev) => [created as JobSeeker, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<JobSeeker> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addJobSeeker(payload).unwrap();
      setAllData((prev) => [created as JobSeeker, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create JobSeeker</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <JobSeekerForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter JobSeeker (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing JobSeeker</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof JobSeeker;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of JobSeekers.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "JobSeeker"}
