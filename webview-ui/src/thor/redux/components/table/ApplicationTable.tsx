...
import {
  useLazyGetApplicationsPagedQuery,
...
const ApplicationTable: React.FC = () => {
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteApplication(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Application> = { ...source };
      delete (payload as any).id;
      const created = await addApplication(payload).unwrap();
      setAllData((prev) => [created as Application, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Application> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addApplication(payload).unwrap();
      setAllData((prev) => [created as Application, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter Application (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof Application;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of Applications.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "Application"}
