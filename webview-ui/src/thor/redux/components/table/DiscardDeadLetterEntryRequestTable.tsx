...
import { DiscardDeadLetterEntryRequest } from '@thor/model';
...
import {
  useLazyGetDiscardDeadLetterEntryRequestsPagedQuery,
...
} from '@thor/redux/services/DiscardDeadLetterEntryRequestService';
...
const DiscardDeadLetterEntryRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<DiscardDeadLetterEntryRequest[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<DiscardDeadLetterEntryRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<DiscardDeadLetterEntryRequest | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteDiscardDeadLetterEntryRequest(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<DiscardDeadLetterEntryRequest> = { ...source };
      delete (payload as any).id;
      const created = await addDiscardDeadLetterEntryRequest(payload).unwrap();
      setAllData((prev) => [created as DiscardDeadLetterEntryRequest, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<DiscardDeadLetterEntryRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addDiscardDeadLetterEntryRequest(payload).unwrap();
      setAllData((prev) => [created as DiscardDeadLetterEntryRequest, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="DiscardDeadLetterEntryRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="DiscardDeadLetterEntryRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create DiscardDeadLetterEntryRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <DiscardDeadLetterEntryRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter DiscardDeadLetterEntryRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing DiscardDeadLetterEntryRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof DiscardDeadLetterEntryRequest;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of DiscardDeadLetterEntryRequests.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'DiscardDeadLetterEntryRequest'}
