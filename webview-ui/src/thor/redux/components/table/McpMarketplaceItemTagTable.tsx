...
import { McpMarketplaceItemTag } from '@thor/model';
...
const McpMarketplaceItemTagTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<McpMarketplaceItemTag[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<McpMarketplaceItemTag>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<McpMarketplaceItemTag | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpMarketplaceItemTag(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<McpMarketplaceItemTag> = { ...source };
      delete (payload as any).id;
      const created = await addMcpMarketplaceItemTag(payload).unwrap();
      setAllData((prev) => [created as McpMarketplaceItemTag, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<McpMarketplaceItemTag> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMcpMarketplaceItemTag(payload).unwrap();
      setAllData((prev) => [created as McpMarketplaceItemTag, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="McpMarketplaceItemTagTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="McpMarketplaceItemTag Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create McpMarketplaceItemTag</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <McpMarketplaceItemTagForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter McpMarketplaceItemTag (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing McpMarketplaceItemTag</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof McpMarketplaceItemTag;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of McpMarketplaceItemTags.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'McpMarketplaceItemTag'}
