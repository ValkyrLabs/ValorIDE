...
import { OasSecurityScheme } from '@thor/model';
...
const OasSecuritySchemeTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<OasSecurityScheme[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOasSecurityScheme(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<OasSecurityScheme> = { ...source };
      delete (payload as any).id;
      const created = await addOasSecurityScheme(payload).unwrap();
      setAllData((prev) => [created as OasSecurityScheme, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<OasSecurityScheme> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addOasSecurityScheme(payload).unwrap();
      setAllData((prev) => [created as OasSecurityScheme, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="OasSecuritySchemeTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="OasSecurityScheme Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create OasSecurityScheme</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter OasSecurityScheme (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof OasSecurityScheme;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of OasSecuritySchemes.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'OasSecurityScheme'}
