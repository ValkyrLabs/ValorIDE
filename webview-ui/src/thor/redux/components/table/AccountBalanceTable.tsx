...
import { AccountBalance } from '@thor/model';
...
const AccountBalanceTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<AccountBalance[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<AccountBalance>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<AccountBalance | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAccountBalance(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<AccountBalance> = { ...source };
      delete (payload as any).id;
      const created = await addAccountBalance(payload).unwrap();
      setAllData((prev) => [created as AccountBalance, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<AccountBalance> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addAccountBalance(payload).unwrap();
      setAllData((prev) => [created as AccountBalance, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="AccountBalanceTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="AccountBalance Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create AccountBalance</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <AccountBalanceForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter AccountBalance (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing AccountBalance</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof AccountBalance;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of AccountBalances.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'AccountBalance'}
