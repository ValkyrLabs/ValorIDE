...
import { ApiMetricsResponse } from '@thor/model';
...
const ApiMetricsResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ApiMetricsResponse[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<ApiMetricsResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ApiMetricsResponse | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteApiMetricsResponse(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ApiMetricsResponse> = { ...source };
      delete (payload as any).id;
      const created = await addApiMetricsResponse(payload).unwrap();
      setAllData((prev) => [created as ApiMetricsResponse, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ApiMetricsResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addApiMetricsResponse(payload).unwrap();
      setAllData((prev) => [created as ApiMetricsResponse, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="ApiMetricsResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ApiMetricsResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ApiMetricsResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ApiMetricsResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ApiMetricsResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ApiMetricsResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ApiMetricsResponse;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ApiMetricsResponses.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ApiMetricsResponse'}
