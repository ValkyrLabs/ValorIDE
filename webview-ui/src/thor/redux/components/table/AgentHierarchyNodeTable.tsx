...
const AgentHierarchyNodeTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<AgentHierarchyNode[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<AgentHierarchyNode>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<AgentHierarchyNode | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAgentHierarchyNode(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<AgentHierarchyNode> = { ...source };
      delete (payload as any).id;
      const created = await addAgentHierarchyNode(payload).unwrap();
      setAllData((prev) => [created as AgentHierarchyNode, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<AgentHierarchyNode> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addAgentHierarchyNode(payload).unwrap();
      setAllData((prev) => [created as AgentHierarchyNode, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="AgentHierarchyNodeTable"
        ...
        >
          <FloatingControlPanel
            description="AgentHierarchyNode Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create AgentHierarchyNode</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <AgentHierarchyNodeForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter AgentHierarchyNode (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing AgentHierarchyNode</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof AgentHierarchyNode;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of AgentHierarchyNodes.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "AgentHierarchyNode"}
