...
import { ChannelSubscription } from '@thor/model';
...
const ChannelSubscriptionTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ChannelSubscription[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<ChannelSubscription>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ChannelSubscription | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteChannelSubscription(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ChannelSubscription> = { ...source };
      delete (payload as any).id;
      const created = await addChannelSubscription(payload).unwrap();
      setAllData((prev) => [created as ChannelSubscription, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ChannelSubscription> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addChannelSubscription(payload).unwrap();
      setAllData((prev) => [created as ChannelSubscription, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="ChannelSubscriptionTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ChannelSubscription Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ChannelSubscription</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ChannelSubscription (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ChannelSubscription</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ChannelSubscription;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ChannelSubscriptions.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ChannelSubscription'}
