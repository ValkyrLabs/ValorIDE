...
import {
  useLazyGetContentDatasPagedQuery,
...
const ContentDataTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<ContentData[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteContentData(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ContentData> = { ...source };
      delete (payload as any).id;
      const created = await addContentData(payload).unwrap();
      setAllData((prev) => [created as ContentData, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ContentData> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addContentData(payload).unwrap();
      setAllData((prev) => [created as ContentData, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          >
            ...
            <FloatingControlPanel
              description="ContentData Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ContentData</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ContentDataForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ContentData (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ContentData;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ContentDatas.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ContentData'}
