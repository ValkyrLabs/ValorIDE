...
import { OasOpenAPISpec } from '@thor/model';
...
const OasOpenAPISpecTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<OasOpenAPISpec[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOasOpenAPISpec(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<OasOpenAPISpec> = { ...source };
      delete (payload as any).id;
      const created = await addOasOpenAPISpec(payload).unwrap();
      setAllData((prev) => [created as OasOpenAPISpec, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<OasOpenAPISpec> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addOasOpenAPISpec(payload).unwrap();
      setAllData((prev) => [created as OasOpenAPISpec, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="OasOpenAPISpecTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="OasOpenAPISpec Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create OasOpenAPISpec</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <OasOpenAPISpecForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter OasOpenAPISpec (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof OasOpenAPISpec;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of OasOpenAPISpecs.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'OasOpenAPISpec'}
