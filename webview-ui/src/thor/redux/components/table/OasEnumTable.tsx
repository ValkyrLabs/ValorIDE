...
import { OasEnum } from '@thor/model';
...
const OasEnumTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<OasEnum[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOasEnum(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<OasEnum> = { ...source };
      delete (payload as any).id;
      const created = await addOasEnum(payload).unwrap();
      setAllData((prev) => [created as OasEnum, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<OasEnum> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addOasEnum(payload).unwrap();
      setAllData((prev) => [created as OasEnum, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create OasEnum</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <OasEnumForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter OasEnum (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof OasEnum;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of OasEnums.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'OasEnum'}
