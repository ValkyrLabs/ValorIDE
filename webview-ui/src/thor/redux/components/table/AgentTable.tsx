...
import {
  useLazyGetAgentsPagedQuery,
  useAddAgentMutation,
  useUpdateAgentMutation,
  useDeleteAgentMutation
} from '@thor/redux/services/AgentService';
...
const AgentTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Agent[]>([]);
  ...
  const [copiedRow, setCopiedRow] = useState<Agent | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAgent(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Agent> = { ...source };
      delete (payload as any).id;
      const created = await addAgent(payload).unwrap();
      setAllData((prev) => [created as Agent, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Agent> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addAgent(payload).unwrap();
      setAllData((prev) => [created as Agent, ...prev]);
    } catch (error) {
    ...
    <>
        <div className="tableContainer">
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter Agent (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing Agent</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof Agent;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of Agents.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'Agent'}
