...
import {
  useLazyGetTagsPagedQuery,
  useAddTagMutation,
  useUpdateTagMutation,
  useDeleteTagMutation,
} from "@thor/redux/services/TagService";
...
const TagTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Tag[]>([]);
  ...
  const [copiedRow, setCopiedRow] = useState<Tag | null>(null);
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Tag> = { ...source };
      delete (payload as any).id;
      const created = await addTag(payload).unwrap();
      setAllData((prev) => [created as Tag, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Tag> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTag(payload).unwrap();
      setAllData((prev) => [created as Tag, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter Tag (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof Tag;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of Tags.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
