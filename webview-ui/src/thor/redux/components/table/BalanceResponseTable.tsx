...
import { BalanceResponse } from '@thor/model';
...
const BalanceResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<BalanceResponse[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<BalanceResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<BalanceResponse | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteBalanceResponse(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<BalanceResponse> = { ...source };
      delete (payload as any).id;
      const created = await addBalanceResponse(payload).unwrap();
      setAllData((prev) => [created as BalanceResponse, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<BalanceResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addBalanceResponse(payload).unwrap();
      setAllData((prev) => [created as BalanceResponse, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="BalanceResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="BalanceResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create BalanceResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <BalanceResponseForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter BalanceResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing BalanceResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof BalanceResponse;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of BalanceResponses.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'BalanceResponse'}
