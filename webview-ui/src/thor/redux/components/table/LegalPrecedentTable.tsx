...
import { LegalPrecedent } from '@thor/model';
...
const LegalPrecedentTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<LegalPrecedent[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<LegalPrecedent>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<LegalPrecedent | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteLegalPrecedent(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<LegalPrecedent> = { ...source };
      delete (payload as any).id;
      const created = await addLegalPrecedent(payload).unwrap();
      setAllData((prev) => [created as LegalPrecedent, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<LegalPrecedent> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addLegalPrecedent(payload).unwrap();
      setAllData((prev) => [created as LegalPrecedent, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="LegalPrecedentTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="LegalPrecedent Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create LegalPrecedent</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <LegalPrecedentForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter LegalPrecedent (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing LegalPrecedent</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof LegalPrecedent;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of LegalPrecedents.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'LegalPrecedent'}
