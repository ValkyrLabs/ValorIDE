...
import { AgentChatMessageRequest } from "@thor/model";
...
const AgentChatMessageRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<AgentChatMessageRequest[]>([]);
  ...
  const [updateAgentChatMessageRequest] =
    useUpdateAgentChatMessageRequestMutation();
  const [deleteAgentChatMessageRequest] =
    useDeleteAgentChatMessageRequestMutation();
  ...
  const [formData, setFormData] = useState<Partial<AgentChatMessageRequest>>(
    {},
  );
  ...
  const [copiedRow, setCopiedRow] = useState<AgentChatMessageRequest | null>(
    null,
  );
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAgentChatMessageRequest(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<AgentChatMessageRequest> = { ...source };
      delete (payload as any).id;
      const created = await addAgentChatMessageRequest(payload).unwrap();
      setAllData((prev) => [created as AgentChatMessageRequest, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<AgentChatMessageRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addAgentChatMessageRequest(payload).unwrap();
      setAllData((prev) => [created as AgentChatMessageRequest, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="AgentChatMessageRequestTable"
        ...
        >
          <FloatingControlPanel
            description="AgentChatMessageRequest Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create AgentChatMessageRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <AgentChatMessageRequestForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter AgentChatMessageRequest (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing AgentChatMessageRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof AgentChatMessageRequest;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of AgentChatMessageRequests.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "AgentChatMessageRequest"}
