...
import { SwarmCommandResponse } from '@thor/model';
...
const SwarmCommandResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SwarmCommandResponse[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<SwarmCommandResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SwarmCommandResponse | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmCommandResponse(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SwarmCommandResponse> = { ...source };
      delete (payload as any).id;
      const created = await addSwarmCommandResponse(payload).unwrap();
      setAllData((prev) => [created as SwarmCommandResponse, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SwarmCommandResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSwarmCommandResponse(payload).unwrap();
      setAllData((prev) => [created as SwarmCommandResponse, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="SwarmCommandResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="SwarmCommandResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SwarmCommandResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SwarmCommandResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SwarmCommandResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SwarmCommandResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SwarmCommandResponse;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SwarmCommandResponses.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SwarmCommandResponse'}
