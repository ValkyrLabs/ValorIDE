...
const ICON_SIZE = 22;
import InitiateTwoFactorRequestForm from "@thor/redux/components/form/InitiateTwoFactorRequestForm";
import CoolButton from "@valkyr/component-library/CoolButton";
// Removed SplitPane usage; using floating toolbar for edit form
// ** Import the LAZY paged hook only **
import { useLazyGetInitiateTwoFactorRequestsPagedQuery, useAddInitiateTwoFactorRequestMutation, useUpdateInitiateTwoFactorRequestMutation, useDeleteInitiateTwoFactorRequestMutation, useDeleteInitiateTwoFactorRequestCascadeMutation, } from "@thor/redux/services/InitiateTwoFactorRequestService";
...
const InitiateTwoFactorRequestTable = () => {
    // -------------------------------------------------------------------
    // 1) "Lazy" RTK Query Hook: We manually trigger page fetches
    // -------------------------------------------------------------------
    // destructure: [triggerFn, { data, isFetching, isError }]
    const [triggerGetPage, { data: fetchedPageData = [], isFetching, isError }] = useLazyGetInitiateTwoFactorRequestsPagedQuery();
    ...
    const confirmDelete = async () => {
        ...
        try {
            await Promise.all(ids.map(async (id) => {
                try {
                    await deleteInitiateTwoFactorRequestCascade({
                        id,
                        cascade: true,
                        trash: true,
                    }).unwrap();
                    ...
                    try {
                        await deleteInitiateTwoFactorRequest(id).unwrap();
                    }
                    catch (inner) {
                    ...
                    if (picked) {
                        ...
                        setTimeout(() => {
                            setFormData((prev) => {
                                ...
                                try {
                                    updateInitiateTwoFactorRequest(updated);
    ...
    const handleSave = async (editKey) => {
        ...
        try {
            await updateInitiateTwoFactorRequest(updatedItem).unwrap();
            ...
            const errorMsg = error instanceof Error
                ? error.message
                : "Failed to update InitiateTwoFactorRequest";
            console.error("Failed to update InitiateTwoFactorRequest:", error);
    ...
    return (_jsxs(_Fragment, { children: [_jsx(RBGrid, { data: tableData, columns: columns, columnSchema: resolvedColumnSchema, selectedRows: selectedRows, onToggleRow: handleRowSelect, onToggleAll: handleToggleAll, onRowPermissions: handleRowPermissions, editCellId: editRowId, formData: formData, onInputChange: handleInputChange, onKeyDownEdit: handleKeyDownEdit, onBlurEdit: handleBlur, onCellDoubleClick: handleDoubleClick, activeCell: activeCell, onCellFocus: handleCellFocus, onCellKeyDownNav: handleCellKeyDownNav, expandedObjects: expandedObjects, onToggleExpandObject: handleToggleExpandObject, onObjectFieldChange: handleObjectFieldChange, showAllFields: showAllFields, onToggleShowAllFields: () => setShowAllFields(!showAllFields), storageKey: "InitiateTwoFactorRequestTable", onReferencePick: onReferencePick, onRequestMoreRows: (dir) => {
                    ...
                    zIndex: 9999,
                    pointerEvents: "auto",
                }, children: _jsxs(FloatingControlPanel, { description: "InitiateTwoFactorRequest Actions", className: "grid-toolbar", style: {
                        pointerEvents: "auto",
                        width: 360,
                        maxWidth: 420,
...
//# sourceMappingURL=InitiateTwoFactorRequestTable.js.map