...
import { SwarmUnregisterResponse } from '@thor/model';
...
const SwarmUnregisterResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SwarmUnregisterResponse[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<SwarmUnregisterResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SwarmUnregisterResponse | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmUnregisterResponse(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SwarmUnregisterResponse> = { ...source };
      delete (payload as any).id;
      const created = await addSwarmUnregisterResponse(payload).unwrap();
      setAllData((prev) => [created as SwarmUnregisterResponse, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SwarmUnregisterResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSwarmUnregisterResponse(payload).unwrap();
      setAllData((prev) => [created as SwarmUnregisterResponse, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="SwarmUnregisterResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="SwarmUnregisterResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SwarmUnregisterResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SwarmUnregisterResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SwarmUnregisterResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SwarmUnregisterResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SwarmUnregisterResponse;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SwarmUnregisterResponses.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SwarmUnregisterResponse'}
