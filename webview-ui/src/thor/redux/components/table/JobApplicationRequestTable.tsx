...
const JobApplicationRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<JobApplicationRequest[]>([]);
  ...
  const [updateJobApplicationRequest] =
    useUpdateJobApplicationRequestMutation();
  const [deleteJobApplicationRequest] =
    useDeleteJobApplicationRequestMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<JobApplicationRequest | null>(
    null,
  );
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteJobApplicationRequest(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<JobApplicationRequest> = { ...source };
      delete (payload as any).id;
      const created = await addJobApplicationRequest(payload).unwrap();
      setAllData((prev) => [created as JobApplicationRequest, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<JobApplicationRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addJobApplicationRequest(payload).unwrap();
      setAllData((prev) => [created as JobApplicationRequest, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="JobApplicationRequestTable"
        ...
        >
          <FloatingControlPanel
            description="JobApplicationRequest Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create JobApplicationRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <JobApplicationRequestForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter JobApplicationRequest (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing JobApplicationRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof JobApplicationRequest;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of JobApplicationRequests.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "JobApplicationRequest"}
