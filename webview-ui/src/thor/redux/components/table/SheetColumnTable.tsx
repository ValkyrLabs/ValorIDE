...
import { SheetColumn } from '@thor/model';
...
const SheetColumnTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<SheetColumn[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSheetColumn(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SheetColumn> = { ...source };
      delete (payload as any).id;
      const created = await addSheetColumn(payload).unwrap();
      setAllData((prev) => [created as SheetColumn, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SheetColumn> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSheetColumn(payload).unwrap();
      setAllData((prev) => [created as SheetColumn, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          >
            ...
            <FloatingControlPanel
              description="SheetColumn Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SheetColumn</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SheetColumnForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SheetColumn (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SheetColumn;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SheetColumns.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SheetColumn'}
