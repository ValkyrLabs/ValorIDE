...
import { SecureKey } from "@thor/model";
...
const SecureKeyTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<SecureKey[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSecureKey(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SecureKey> = { ...source };
      delete (payload as any).id;
      const created = await addSecureKey(payload).unwrap();
      setAllData((prev) => [created as SecureKey, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SecureKey> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSecureKey(payload).unwrap();
      setAllData((prev) => [created as SecureKey, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create SecureKey</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <SecureKeyForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter SecureKey (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof SecureKey;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of SecureKeys.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "SecureKey"}
