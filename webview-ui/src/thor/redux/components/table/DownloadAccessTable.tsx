...
const DownloadAccessTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<DownloadAccess[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<DownloadAccess>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<DownloadAccess | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteDownloadAccess(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<DownloadAccess> = { ...source };
      delete (payload as any).id;
      const created = await addDownloadAccess(payload).unwrap();
      setAllData((prev) => [created as DownloadAccess, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<DownloadAccess> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addDownloadAccess(payload).unwrap();
      setAllData((prev) => [created as DownloadAccess, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="DownloadAccessTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="DownloadAccess Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create DownloadAccess</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <DownloadAccessForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter DownloadAccess (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing DownloadAccess</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof DownloadAccess;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of DownloadAccesss.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'DownloadAccess'}
