...
const ICON_SIZE = 22;
import TwoFactorSecretForm from "@thor/redux/components/form/TwoFactorSecretForm";
import CoolButton from "@valkyr/component-library/CoolButton";
// Removed SplitPane usage; using floating toolbar for edit form
// ** Import the LAZY paged hook only **
import { useLazyGetTwoFactorSecretsPagedQuery, useAddTwoFactorSecretMutation, useUpdateTwoFactorSecretMutation, useDeleteTwoFactorSecretMutation, useDeleteTwoFactorSecretCascadeMutation, } from "@thor/redux/services/TwoFactorSecretService";
...
const columnSchema = {
    algorithm: {
        type: "enum",
        enumValues: ["HmacSHA1", "HmacSHA256", "HmacSHA512"],
...
const TwoFactorSecretTable = () => {
    // -------------------------------------------------------------------
    // 1) "Lazy" RTK Query Hook: We manually trigger page fetches
    // -------------------------------------------------------------------
    // destructure: [triggerFn, { data, isFetching, isError }]
    const [triggerGetPage, { data: fetchedPageData = [], isFetching, isError }] = useLazyGetTwoFactorSecretsPagedQuery();
    ...
    const confirmDelete = async () => {
        ...
        try {
            await Promise.all(ids.map(async (id) => {
                try {
                    await deleteTwoFactorSecretCascade({
                        id,
                        cascade: true,
                        trash: true,
                    }).unwrap();
                    ...
                    try {
                        await deleteTwoFactorSecret(id).unwrap();
                    }
                    catch (inner) {
    ...
    const handleSave = async (editKey) => {
        ...
        try {
            await updateTwoFactorSecret(updatedItem).unwrap();
            ...
            const errorMsg = error instanceof Error
                ? error.message
                : "Failed to update TwoFactorSecret";
            console.error("Failed to update TwoFactorSecret:", error);
    ...
    return (_jsxs(_Fragment, { children: [_jsx(RBGrid, { data: tableData, columns: columns, columnSchema: resolvedColumnSchema, selectedRows: selectedRows, onToggleRow: handleRowSelect, onToggleAll: handleToggleAll, onRowPermissions: handleRowPermissions, editCellId: editRowId, formData: formData, onInputChange: handleInputChange, onKeyDownEdit: handleKeyDownEdit, onBlurEdit: handleBlur, onCellDoubleClick: handleDoubleClick, activeCell: activeCell, onCellFocus: handleCellFocus, onCellKeyDownNav: handleCellKeyDownNav, expandedObjects: expandedObjects, onToggleExpandObject: handleToggleExpandObject, onObjectFieldChange: handleObjectFieldChange, showAllFields: showAllFields, onToggleShowAllFields: () => setShowAllFields(!showAllFields), storageKey: "TwoFactorSecretTable", onReferencePick: onReferencePick, onRequestMoreRows: (dir) => {
                    ...
                    zIndex: 9999,
                    pointerEvents: "auto",
                }, children: _jsxs(FloatingControlPanel, { description: "TwoFactorSecret Actions", className: "grid-toolbar", style: {
                        pointerEvents: "auto",
                        width: 360,
                        maxWidth: 420,
...
//# sourceMappingURL=TwoFactorSecretTable.js.map