...
const RequeueDeadLetterEntryResponseTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addRequeueDeadLetterEntryResponse] =
    useAddRequeueDeadLetterEntryResponseMutation();
  const [updateRequeueDeadLetterEntryResponse] =
    useUpdateRequeueDeadLetterEntryResponseMutation();
  const [deleteRequeueDeadLetterEntryResponse] =
    useDeleteRequeueDeadLetterEntryResponseMutation();
  ...
  const [copiedRow, setCopiedRow] =
    useState<RequeueDeadLetterEntryResponse | null>(null);
  ...
  const handleAddRow = async () => {
    try {
      ...
      setAllData((prev) => [
        created as RequeueDeadLetterEntryResponse,
        ...prev,
      ]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) RequeueDeadLetterEntryResponse:",
        error,
      );
    ...
    <>
      <div className="tableContainer">
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter RequeueDeadLetterEntryResponse (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field =
                richModal.field as keyof RequeueDeadLetterEntryResponse;
              if (!rid || !field) {
