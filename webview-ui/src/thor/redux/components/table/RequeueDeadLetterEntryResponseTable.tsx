...
import { RequeueDeadLetterEntryResponse } from '@thor/model';
...
import {
  useLazyGetRequeueDeadLetterEntryResponsesPagedQuery,
...
} from '@thor/redux/services/RequeueDeadLetterEntryResponseService';
...
const RequeueDeadLetterEntryResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RequeueDeadLetterEntryResponse[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<RequeueDeadLetterEntryResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RequeueDeadLetterEntryResponse | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRequeueDeadLetterEntryResponse(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RequeueDeadLetterEntryResponse> = { ...source };
      delete (payload as any).id;
      const created = await addRequeueDeadLetterEntryResponse(payload).unwrap();
      setAllData((prev) => [created as RequeueDeadLetterEntryResponse, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RequeueDeadLetterEntryResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRequeueDeadLetterEntryResponse(payload).unwrap();
      setAllData((prev) => [created as RequeueDeadLetterEntryResponse, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="RequeueDeadLetterEntryResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="RequeueDeadLetterEntryResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RequeueDeadLetterEntryResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RequeueDeadLetterEntryResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RequeueDeadLetterEntryResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RequeueDeadLetterEntryResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RequeueDeadLetterEntryResponse;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RequeueDeadLetterEntryResponses.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RequeueDeadLetterEntryResponse'}
