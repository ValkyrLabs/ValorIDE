...
const ICON_SIZE = 22;
import RequestPasswordResetResponseForm from "@thor/redux/components/form/RequestPasswordResetResponseForm";
import CoolButton from "@valkyr/component-library/CoolButton";
// Removed SplitPane usage; using floating toolbar for edit form
// ** Import the LAZY paged hook only **
import { useLazyGetRequestPasswordResetResponsesPagedQuery, useAddRequestPasswordResetResponseMutation, useUpdateRequestPasswordResetResponseMutation, useDeleteRequestPasswordResetResponseMutation, useDeleteRequestPasswordResetResponseCascadeMutation, } from "@thor/redux/services/RequestPasswordResetResponseService";
...
const columnSchema = {
    status: {
        type: "enum",
        enumValues: ["SENT", "USER_NOT_FOUND", "ACCOUNT_LOCKED"],
...
const RequestPasswordResetResponseTable = () => {
    // -------------------------------------------------------------------
    // 1) "Lazy" RTK Query Hook: We manually trigger page fetches
    // -------------------------------------------------------------------
    // destructure: [triggerFn, { data, isFetching, isError }]
    const [triggerGetPage, { data: fetchedPageData = [], isFetching, isError }] = useLazyGetRequestPasswordResetResponsesPagedQuery();
    ...
    const confirmDelete = async () => {
        ...
        try {
            await Promise.all(ids.map(async (id) => {
                try {
                    await deleteRequestPasswordResetResponseCascade({
                        id,
                        cascade: true,
                        trash: true,
                    }).unwrap();
                    ...
                    try {
                        await deleteRequestPasswordResetResponse(id).unwrap();
                    }
                    catch (inner) {
                    ...
                    if (picked) {
                        ...
                        setTimeout(() => {
                            setFormData((prev) => {
                                ...
                                try {
                                    updateRequestPasswordResetResponse(updated);
    ...
    const handleSave = async (editKey) => {
        ...
        try {
            await updateRequestPasswordResetResponse(updatedItem).unwrap();
            ...
            const errorMsg = error instanceof Error
                ? error.message
                : "Failed to update RequestPasswordResetResponse";
            console.error("Failed to update RequestPasswordResetResponse:", error);
    ...
    return (_jsxs(_Fragment, { children: [_jsx(RBGrid, { data: tableData, columns: columns, columnSchema: resolvedColumnSchema, selectedRows: selectedRows, onToggleRow: handleRowSelect, onToggleAll: handleToggleAll, onRowPermissions: handleRowPermissions, editCellId: editRowId, formData: formData, onInputChange: handleInputChange, onKeyDownEdit: handleKeyDownEdit, onBlurEdit: handleBlur, onCellDoubleClick: handleDoubleClick, activeCell: activeCell, onCellFocus: handleCellFocus, onCellKeyDownNav: handleCellKeyDownNav, expandedObjects: expandedObjects, onToggleExpandObject: handleToggleExpandObject, onObjectFieldChange: handleObjectFieldChange, showAllFields: showAllFields, onToggleShowAllFields: () => setShowAllFields(!showAllFields), storageKey: "RequestPasswordResetResponseTable", onReferencePick: onReferencePick, onRequestMoreRows: (dir) => {
                    ...
                    zIndex: 9999,
                    pointerEvents: "auto",
                }, children: _jsxs(FloatingControlPanel, { description: "RequestPasswordResetResponse Actions", className: "grid-toolbar", style: {
                        pointerEvents: "auto",
                        width: 360,
                        maxWidth: 420,
...
//# sourceMappingURL=RequestPasswordResetResponseTable.js.map