...
import { ApiTrafficEvent } from '@thor/model';
...
const ApiTrafficEventTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ApiTrafficEvent[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<ApiTrafficEvent>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ApiTrafficEvent | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteApiTrafficEvent(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ApiTrafficEvent> = { ...source };
      delete (payload as any).id;
      const created = await addApiTrafficEvent(payload).unwrap();
      setAllData((prev) => [created as ApiTrafficEvent, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ApiTrafficEvent> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addApiTrafficEvent(payload).unwrap();
      setAllData((prev) => [created as ApiTrafficEvent, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="ApiTrafficEventTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ApiTrafficEvent Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ApiTrafficEvent</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ApiTrafficEventForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ApiTrafficEvent (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ApiTrafficEvent</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ApiTrafficEvent;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ApiTrafficEvents.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ApiTrafficEvent'}
