...
import { OasParameter } from "@thor/model";
...
const OasParameterTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<OasParameter[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOasParameter(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<OasParameter> = { ...source };
      delete (payload as any).id;
      const created = await addOasParameter(payload).unwrap();
      setAllData((prev) => [created as OasParameter, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<OasParameter> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addOasParameter(payload).unwrap();
      setAllData((prev) => [created as OasParameter, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <FloatingControlPanel
            description="OasParameter Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create OasParameter</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <OasParameterForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter OasParameter (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof OasParameter;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of OasParameters.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "OasParameter"}
