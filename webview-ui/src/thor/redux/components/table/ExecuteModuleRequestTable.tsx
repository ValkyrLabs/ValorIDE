...
const ExecuteModuleRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ExecuteModuleRequest[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<ExecuteModuleRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ExecuteModuleRequest | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecuteModuleRequest(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ExecuteModuleRequest> = { ...source };
      delete (payload as any).id;
      const created = await addExecuteModuleRequest(payload).unwrap();
      setAllData((prev) => [created as ExecuteModuleRequest, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ExecuteModuleRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addExecuteModuleRequest(payload).unwrap();
      setAllData((prev) => [created as ExecuteModuleRequest, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="ExecuteModuleRequestTable"
        ...
        >
          <FloatingControlPanel
            description="ExecuteModuleRequest Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create ExecuteModuleRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <ExecuteModuleRequestForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter ExecuteModuleRequest (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing ExecuteModuleRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof ExecuteModuleRequest;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of ExecuteModuleRequests.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "ExecuteModuleRequest"}
