...
import { LineItem } from "@thor/model";
...
import {
  useLazyGetLineItemsPagedQuery,
  useAddLineItemMutation,
  useUpdateLineItemMutation,
  useDeleteLineItemMutation,
} from "@thor/redux/services/LineItemService";
...
const LineItemTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<LineItem[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteLineItem(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<LineItem> = { ...source };
      delete (payload as any).id;
      const created = await addLineItem(payload).unwrap();
      setAllData((prev) => [created as LineItem, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<LineItem> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addLineItem(payload).unwrap();
      setAllData((prev) => [created as LineItem, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create LineItem</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <LineItemForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter LineItem (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof LineItem;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of LineItems.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "LineItem"}
