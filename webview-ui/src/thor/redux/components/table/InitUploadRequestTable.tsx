...
const InitUploadRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<InitUploadRequest[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<InitUploadRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<InitUploadRequest | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteInitUploadRequest(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<InitUploadRequest> = { ...source };
      delete (payload as any).id;
      const created = await addInitUploadRequest(payload).unwrap();
      setAllData((prev) => [created as InitUploadRequest, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<InitUploadRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addInitUploadRequest(payload).unwrap();
      setAllData((prev) => [created as InitUploadRequest, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="InitUploadRequestTable"
        ...
        >
          <FloatingControlPanel
            description="InitUploadRequest Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create InitUploadRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <InitUploadRequestForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter InitUploadRequest (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing InitUploadRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof InitUploadRequest;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of InitUploadRequests.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "InitUploadRequest"}
