...
import { EventLog } from "@thor/model";
...
const EventLogTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<EventLog[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteEventLog(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<EventLog> = { ...source };
      delete (payload as any).id;
      const created = await addEventLog(payload).unwrap();
      setAllData((prev) => [created as EventLog, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<EventLog> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addEventLog(payload).unwrap();
      setAllData((prev) => [created as EventLog, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create EventLog</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <EventLogForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter EventLog (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof EventLog;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of EventLogs.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "EventLog"}
