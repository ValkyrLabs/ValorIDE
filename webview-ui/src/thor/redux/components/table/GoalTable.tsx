...
import {
  useLazyGetGoalsPagedQuery,
  useAddGoalMutation,
  useUpdateGoalMutation,
  useDeleteGoalMutation
} from '@thor/redux/services/GoalService';
...
const GoalTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Goal[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteGoal(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Goal> = { ...source };
      delete (payload as any).id;
      const created = await addGoal(payload).unwrap();
      setAllData((prev) => [created as Goal, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Goal> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addGoal(payload).unwrap();
      setAllData((prev) => [created as Goal, ...prev]);
    } catch (error) {
    ...
    <>
        <div className="tableContainer">
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter Goal (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof Goal;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of Goals.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'Goal'}
