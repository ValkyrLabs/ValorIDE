...
import {
  useLazyGetSheetsPagedQuery,
  useAddSheetMutation,
  useUpdateSheetMutation,
  useDeleteSheetMutation
} from '@thor/redux/services/SheetService';
...
const SheetTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Sheet[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSheet(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Sheet> = { ...source };
      delete (payload as any).id;
      const created = await addSheet(payload).unwrap();
      setAllData((prev) => [created as Sheet, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Sheet> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSheet(payload).unwrap();
      setAllData((prev) => [created as Sheet, ...prev]);
    } catch (error) {
    ...
    <>
        <div className="tableContainer">
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter Sheet (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof Sheet;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of Sheets.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'Sheet'}
