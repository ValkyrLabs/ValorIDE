...
const ICON_SIZE = 22;
import RequestPasswordResetRequestForm from "@thor/redux/components/form/RequestPasswordResetRequestForm";
import CoolButton from "@valkyr/component-library/CoolButton";
// Removed SplitPane usage; using floating toolbar for edit form
// ** Import the LAZY paged hook only **
import { useLazyGetRequestPasswordResetRequestsPagedQuery, useAddRequestPasswordResetRequestMutation, useUpdateRequestPasswordResetRequestMutation, useDeleteRequestPasswordResetRequestMutation, useDeleteRequestPasswordResetRequestCascadeMutation, } from "@thor/redux/services/RequestPasswordResetRequestService";
...
const RequestPasswordResetRequestTable = () => {
    // -------------------------------------------------------------------
    // 1) "Lazy" RTK Query Hook: We manually trigger page fetches
    // -------------------------------------------------------------------
    // destructure: [triggerFn, { data, isFetching, isError }]
    const [triggerGetPage, { data: fetchedPageData = [], isFetching, isError }] = useLazyGetRequestPasswordResetRequestsPagedQuery();
    ...
    const confirmDelete = async () => {
        ...
        try {
            await Promise.all(ids.map(async (id) => {
                try {
                    await deleteRequestPasswordResetRequestCascade({
                        id,
                        cascade: true,
                        trash: true,
                    }).unwrap();
                    ...
                    try {
                        await deleteRequestPasswordResetRequest(id).unwrap();
                    }
                    catch (inner) {
                    ...
                    if (picked) {
                        ...
                        setTimeout(() => {
                            setFormData((prev) => {
                                ...
                                try {
                                    updateRequestPasswordResetRequest(updated);
    ...
    const handleSave = async (editKey) => {
        ...
        try {
            await updateRequestPasswordResetRequest(updatedItem).unwrap();
            ...
            const errorMsg = error instanceof Error
                ? error.message
                : "Failed to update RequestPasswordResetRequest";
            console.error("Failed to update RequestPasswordResetRequest:", error);
    ...
    return (_jsxs(_Fragment, { children: [_jsx(RBGrid, { data: tableData, columns: columns, columnSchema: resolvedColumnSchema, selectedRows: selectedRows, onToggleRow: handleRowSelect, onToggleAll: handleToggleAll, onRowPermissions: handleRowPermissions, editCellId: editRowId, formData: formData, onInputChange: handleInputChange, onKeyDownEdit: handleKeyDownEdit, onBlurEdit: handleBlur, onCellDoubleClick: handleDoubleClick, activeCell: activeCell, onCellFocus: handleCellFocus, onCellKeyDownNav: handleCellKeyDownNav, expandedObjects: expandedObjects, onToggleExpandObject: handleToggleExpandObject, onObjectFieldChange: handleObjectFieldChange, showAllFields: showAllFields, onToggleShowAllFields: () => setShowAllFields(!showAllFields), storageKey: "RequestPasswordResetRequestTable", onReferencePick: onReferencePick, onRequestMoreRows: (dir) => {
                    ...
                    zIndex: 9999,
                    pointerEvents: "auto",
                }, children: _jsxs(FloatingControlPanel, { description: "RequestPasswordResetRequest Actions", className: "grid-toolbar", style: {
                        pointerEvents: "auto",
                        width: 360,
                        maxWidth: 420,
...
//# sourceMappingURL=RequestPasswordResetRequestTable.js.map