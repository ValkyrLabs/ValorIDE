...
import { SwarmUnregisterRequest } from '@thor/model';
...
const SwarmUnregisterRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SwarmUnregisterRequest[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<SwarmUnregisterRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SwarmUnregisterRequest | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmUnregisterRequest(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SwarmUnregisterRequest> = { ...source };
      delete (payload as any).id;
      const created = await addSwarmUnregisterRequest(payload).unwrap();
      setAllData((prev) => [created as SwarmUnregisterRequest, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SwarmUnregisterRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSwarmUnregisterRequest(payload).unwrap();
      setAllData((prev) => [created as SwarmUnregisterRequest, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="SwarmUnregisterRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="SwarmUnregisterRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SwarmUnregisterRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SwarmUnregisterRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SwarmUnregisterRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SwarmUnregisterRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SwarmUnregisterRequest;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SwarmUnregisterRequests.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SwarmUnregisterRequest'}
