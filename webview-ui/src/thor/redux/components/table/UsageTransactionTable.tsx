...
import {
  useLazyGetUsageTransactionsPagedQuery,
...
const UsageTransactionTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<UsageTransaction[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<UsageTransaction>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<UsageTransaction | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteUsageTransaction(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<UsageTransaction> = { ...source };
      delete (payload as any).id;
      const created = await addUsageTransaction(payload).unwrap();
      setAllData((prev) => [created as UsageTransaction, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<UsageTransaction> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addUsageTransaction(payload).unwrap();
      setAllData((prev) => [created as UsageTransaction, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="UsageTransactionTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="UsageTransaction Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create UsageTransaction</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <UsageTransactionForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter UsageTransaction (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing UsageTransaction</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof UsageTransaction;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of UsageTransactions.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'UsageTransaction'}
