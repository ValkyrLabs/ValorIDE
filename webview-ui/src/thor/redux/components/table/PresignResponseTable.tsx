...
import { PresignResponse } from "@thor/model";
...
const PresignResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<PresignResponse[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<PresignResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<PresignResponse | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePresignResponse(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<PresignResponse> = { ...source };
      delete (payload as any).id;
      const created = await addPresignResponse(payload).unwrap();
      setAllData((prev) => [created as PresignResponse, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<PresignResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addPresignResponse(payload).unwrap();
      setAllData((prev) => [created as PresignResponse, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="PresignResponseTable"
        ...
        >
          <FloatingControlPanel
            description="PresignResponse Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create PresignResponse</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <PresignResponseForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter PresignResponse (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing PresignResponse</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof PresignResponse;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of PresignResponses.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "PresignResponse"}
