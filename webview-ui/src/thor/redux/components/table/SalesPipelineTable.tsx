...
import { SalesPipeline } from "@thor/model";
...
const SalesPipelineTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<SalesPipeline[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSalesPipeline(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SalesPipeline> = { ...source };
      delete (payload as any).id;
      const created = await addSalesPipeline(payload).unwrap();
      setAllData((prev) => [created as SalesPipeline, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SalesPipeline> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSalesPipeline(payload).unwrap();
      setAllData((prev) => [created as SalesPipeline, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <FloatingControlPanel
            description="SalesPipeline Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create SalesPipeline</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <SalesPipelineForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter SalesPipeline (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof SalesPipeline;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of SalesPipelines.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "SalesPipeline"}
