...
const KeyMetricTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<KeyMetric[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteKeyMetric(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<KeyMetric> = { ...source };
      delete (payload as any).id;
      const created = await addKeyMetric(payload).unwrap();
      setAllData((prev) => [created as KeyMetric, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<KeyMetric> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addKeyMetric(payload).unwrap();
      setAllData((prev) => [created as KeyMetric, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create KeyMetric</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <KeyMetricForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter KeyMetric (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof KeyMetric;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of KeyMetrics.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'KeyMetric'}
