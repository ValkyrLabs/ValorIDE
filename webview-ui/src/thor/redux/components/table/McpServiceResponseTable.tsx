...
import { McpServiceResponse } from "@thor/model";
...
const McpServiceResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<McpServiceResponse[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<McpServiceResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<McpServiceResponse | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpServiceResponse(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<McpServiceResponse> = { ...source };
      delete (payload as any).id;
      const created = await addMcpServiceResponse(payload).unwrap();
      setAllData((prev) => [created as McpServiceResponse, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<McpServiceResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMcpServiceResponse(payload).unwrap();
      setAllData((prev) => [created as McpServiceResponse, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="McpServiceResponseTable"
        ...
        >
          <FloatingControlPanel
            description="McpServiceResponse Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create McpServiceResponse</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <McpServiceResponseForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter McpServiceResponse (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing McpServiceResponse</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof McpServiceResponse;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of McpServiceResponses.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "McpServiceResponse"}
