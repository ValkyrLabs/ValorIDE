...
import { SwarmGraphEdge } from '@thor/model';
...
const SwarmGraphEdgeTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SwarmGraphEdge[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<SwarmGraphEdge>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SwarmGraphEdge | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmGraphEdge(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SwarmGraphEdge> = { ...source };
      delete (payload as any).id;
      const created = await addSwarmGraphEdge(payload).unwrap();
      setAllData((prev) => [created as SwarmGraphEdge, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SwarmGraphEdge> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSwarmGraphEdge(payload).unwrap();
      setAllData((prev) => [created as SwarmGraphEdge, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="SwarmGraphEdgeTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="SwarmGraphEdge Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SwarmGraphEdge</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SwarmGraphEdgeForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SwarmGraphEdge (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SwarmGraphEdge</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SwarmGraphEdge;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SwarmGraphEdges.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SwarmGraphEdge'}
