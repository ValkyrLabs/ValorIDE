...
import {
  useLazyGetLoginsPagedQuery,
  useAddLoginMutation,
  useUpdateLoginMutation,
  useDeleteLoginMutation
} from '@thor/redux/services/LoginService';
...
const LoginTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Login[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteLogin(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Login> = { ...source };
      delete (payload as any).id;
      const created = await addLogin(payload).unwrap();
      setAllData((prev) => [created as Login, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Login> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addLogin(payload).unwrap();
      setAllData((prev) => [created as Login, ...prev]);
    } catch (error) {
    ...
    <>
        <div className="tableContainer">
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter Login (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof Login;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of Logins.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'Login'}
