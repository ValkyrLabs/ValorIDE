...
const PtgRefTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<PtgRef[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePtgRef(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<PtgRef> = { ...source };
      delete (payload as any).id;
      const created = await addPtgRef(payload).unwrap();
      setAllData((prev) => [created as PtgRef, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<PtgRef> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addPtgRef(payload).unwrap();
      setAllData((prev) => [created as PtgRef, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create PtgRef</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <PtgRefForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter PtgRef (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof PtgRef;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of PtgRefs.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "PtgRef"}
