...
import { BackupConfig } from "@thor/model";
...
const BackupConfigTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<BackupConfig[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteBackupConfig(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<BackupConfig> = { ...source };
      delete (payload as any).id;
      const created = await addBackupConfig(payload).unwrap();
      setAllData((prev) => [created as BackupConfig, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<BackupConfig> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addBackupConfig(payload).unwrap();
      setAllData((prev) => [created as BackupConfig, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <FloatingControlPanel
            description="BackupConfig Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create BackupConfig</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <BackupConfigForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter BackupConfig (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof BackupConfig;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of BackupConfigs.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "BackupConfig"}
