...
import { McpToolPreset } from '@thor/model';
...
const McpToolPresetTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<McpToolPreset[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<McpToolPreset>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<McpToolPreset | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpToolPreset(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<McpToolPreset> = { ...source };
      delete (payload as any).id;
      const created = await addMcpToolPreset(payload).unwrap();
      setAllData((prev) => [created as McpToolPreset, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<McpToolPreset> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMcpToolPreset(payload).unwrap();
      setAllData((prev) => [created as McpToolPreset, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          >
            ...
            <FloatingControlPanel
              description="McpToolPreset Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create McpToolPreset</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <McpToolPresetForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter McpToolPreset (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing McpToolPreset</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof McpToolPreset;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of McpToolPresets.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'McpToolPreset'}
