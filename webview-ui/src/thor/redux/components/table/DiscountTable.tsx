...
import { Discount } from "@thor/model";
...
import {
  useLazyGetDiscountsPagedQuery,
  useAddDiscountMutation,
  useUpdateDiscountMutation,
  useDeleteDiscountMutation,
} from "@thor/redux/services/DiscountService";
...
const DiscountTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Discount[]>([]);
  ...
  const [copiedRow, setCopiedRow] = useState<Discount | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteDiscount(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Discount> = { ...source };
      delete (payload as any).id;
      const created = await addDiscount(payload).unwrap();
      setAllData((prev) => [created as Discount, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Discount> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addDiscount(payload).unwrap();
      setAllData((prev) => [created as Discount, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter Discount (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing Discount</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof Discount;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of Discounts.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "Discount"}
