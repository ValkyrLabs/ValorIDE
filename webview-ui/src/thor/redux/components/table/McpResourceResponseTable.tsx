...
import { McpResourceResponse } from '@thor/model';
...
const McpResourceResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<McpResourceResponse[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<McpResourceResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<McpResourceResponse | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpResourceResponse(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<McpResourceResponse> = { ...source };
      delete (payload as any).id;
      const created = await addMcpResourceResponse(payload).unwrap();
      setAllData((prev) => [created as McpResourceResponse, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<McpResourceResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMcpResourceResponse(payload).unwrap();
      setAllData((prev) => [created as McpResourceResponse, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="McpResourceResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="McpResourceResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create McpResourceResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <McpResourceResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter McpResourceResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing McpResourceResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof McpResourceResponse;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of McpResourceResponses.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'McpResourceResponse'}
