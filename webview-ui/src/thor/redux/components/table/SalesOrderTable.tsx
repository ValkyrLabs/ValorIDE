...
import { SalesOrder } from "@thor/model";
...
import {
  useLazyGetSalesOrdersPagedQuery,
  useAddSalesOrderMutation,
  useUpdateSalesOrderMutation,
  useDeleteSalesOrderMutation,
} from "@thor/redux/services/SalesOrderService";
...
const SalesOrderTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<SalesOrder[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSalesOrder(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SalesOrder> = { ...source };
      delete (payload as any).id;
      const created = await addSalesOrder(payload).unwrap();
      setAllData((prev) => [created as SalesOrder, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SalesOrder> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSalesOrder(payload).unwrap();
      setAllData((prev) => [created as SalesOrder, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <FloatingControlPanel
            description="SalesOrder Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create SalesOrder</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <SalesOrderForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter SalesOrder (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof SalesOrder;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of SalesOrders.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "SalesOrder"}
