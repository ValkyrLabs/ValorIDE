...
import { SwarmRegisterRequest } from '@thor/model';
...
const SwarmRegisterRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SwarmRegisterRequest[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<SwarmRegisterRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SwarmRegisterRequest | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmRegisterRequest(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SwarmRegisterRequest> = { ...source };
      delete (payload as any).id;
      const created = await addSwarmRegisterRequest(payload).unwrap();
      setAllData((prev) => [created as SwarmRegisterRequest, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SwarmRegisterRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSwarmRegisterRequest(payload).unwrap();
      setAllData((prev) => [created as SwarmRegisterRequest, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="SwarmRegisterRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="SwarmRegisterRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SwarmRegisterRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SwarmRegisterRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SwarmRegisterRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SwarmRegisterRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SwarmRegisterRequest;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SwarmRegisterRequests.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SwarmRegisterRequest'}
