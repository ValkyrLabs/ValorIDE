...
import { AclObjectIdentity } from '@thor/model';
...
const AclObjectIdentityTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<AclObjectIdentity[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAclObjectIdentity(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<AclObjectIdentity> = { ...source };
      delete (payload as any).id;
      const created = await addAclObjectIdentity(payload).unwrap();
      setAllData((prev) => [created as AclObjectIdentity, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<AclObjectIdentity> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addAclObjectIdentity(payload).unwrap();
      setAllData((prev) => [created as AclObjectIdentity, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="AclObjectIdentityTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="AclObjectIdentity Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create AclObjectIdentity</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter AclObjectIdentity (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof AclObjectIdentity;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of AclObjectIdentitys.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'AclObjectIdentity'}
