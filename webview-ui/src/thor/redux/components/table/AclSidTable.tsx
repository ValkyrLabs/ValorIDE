...
const AclSidTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<AclSid[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAclSid(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<AclSid> = { ...source };
      delete (payload as any).id;
      const created = await addAclSid(payload).unwrap();
      setAllData((prev) => [created as AclSid, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<AclSid> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addAclSid(payload).unwrap();
      setAllData((prev) => [created as AclSid, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create AclSid</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <AclSidForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter AclSid (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof AclSid;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of AclSids.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'AclSid'}
