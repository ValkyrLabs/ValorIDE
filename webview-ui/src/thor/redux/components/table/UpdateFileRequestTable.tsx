...
const UpdateFileRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<UpdateFileRequest[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<UpdateFileRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<UpdateFileRequest | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteUpdateFileRequest(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<UpdateFileRequest> = { ...source };
      delete (payload as any).id;
      const created = await addUpdateFileRequest(payload).unwrap();
      setAllData((prev) => [created as UpdateFileRequest, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<UpdateFileRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addUpdateFileRequest(payload).unwrap();
      setAllData((prev) => [created as UpdateFileRequest, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="UpdateFileRequestTable"
        ...
        >
          <FloatingControlPanel
            description="UpdateFileRequest Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create UpdateFileRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <UpdateFileRequestForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter UpdateFileRequest (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing UpdateFileRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof UpdateFileRequest;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of UpdateFileRequests.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "UpdateFileRequest"}
