...
const PrincipalRolesTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<PrincipalRoles[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<PrincipalRoles>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<PrincipalRoles | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePrincipalRoles(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<PrincipalRoles> = { ...source };
      delete (payload as any).id;
      const created = await addPrincipalRoles(payload).unwrap();
      setAllData((prev) => [created as PrincipalRoles, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<PrincipalRoles> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addPrincipalRoles(payload).unwrap();
      setAllData((prev) => [created as PrincipalRoles, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="PrincipalRolesTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="PrincipalRoles Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create PrincipalRoles</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <PrincipalRolesForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter PrincipalRoles (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing PrincipalRoles</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof PrincipalRoles;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of PrincipalRoless.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'PrincipalRoles'}
