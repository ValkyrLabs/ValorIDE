...
import { OrderFulfillmentTask } from '@thor/model';
...
const OrderFulfillmentTaskTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<OrderFulfillmentTask[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<OrderFulfillmentTask>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<OrderFulfillmentTask | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOrderFulfillmentTask(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<OrderFulfillmentTask> = { ...source };
      delete (payload as any).id;
      const created = await addOrderFulfillmentTask(payload).unwrap();
      setAllData((prev) => [created as OrderFulfillmentTask, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<OrderFulfillmentTask> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addOrderFulfillmentTask(payload).unwrap();
      setAllData((prev) => [created as OrderFulfillmentTask, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="OrderFulfillmentTaskTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="OrderFulfillmentTask Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create OrderFulfillmentTask</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <OrderFulfillmentTaskForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter OrderFulfillmentTask (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing OrderFulfillmentTask</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof OrderFulfillmentTask;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of OrderFulfillmentTasks.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'OrderFulfillmentTask'}
