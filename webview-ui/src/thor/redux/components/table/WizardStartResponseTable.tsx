...
import { WizardStartResponse } from "@thor/model";
...
const WizardStartResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<WizardStartResponse[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<WizardStartResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<WizardStartResponse | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWizardStartResponse(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<WizardStartResponse> = { ...source };
      delete (payload as any).id;
      const created = await addWizardStartResponse(payload).unwrap();
      setAllData((prev) => [created as WizardStartResponse, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<WizardStartResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addWizardStartResponse(payload).unwrap();
      setAllData((prev) => [created as WizardStartResponse, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="WizardStartResponseTable"
        ...
        >
          <FloatingControlPanel
            description="WizardStartResponse Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create WizardStartResponse</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <WizardStartResponseForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter WizardStartResponse (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing WizardStartResponse</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof WizardStartResponse;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of WizardStartResponses.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "WizardStartResponse"}
