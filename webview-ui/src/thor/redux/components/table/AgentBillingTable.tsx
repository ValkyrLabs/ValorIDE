...
import { AgentBilling } from "@thor/model";
...
const AgentBillingTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<AgentBilling[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<AgentBilling>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<AgentBilling | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAgentBilling(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<AgentBilling> = { ...source };
      delete (payload as any).id;
      const created = await addAgentBilling(payload).unwrap();
      setAllData((prev) => [created as AgentBilling, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<AgentBilling> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addAgentBilling(payload).unwrap();
      setAllData((prev) => [created as AgentBilling, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <FloatingControlPanel
            description="AgentBilling Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create AgentBilling</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <AgentBillingForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter AgentBilling (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing AgentBilling</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof AgentBilling;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of AgentBillings.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "AgentBilling"}
