...
import { Workbook } from "@thor/model";
...
const WorkbookTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Workbook[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkbook(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Workbook> = { ...source };
      delete (payload as any).id;
      const created = await addWorkbook(payload).unwrap();
      setAllData((prev) => [created as Workbook, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Workbook> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addWorkbook(payload).unwrap();
      setAllData((prev) => [created as Workbook, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter Workbook (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof Workbook;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of Workbooks.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "Workbook"}
