...
import { MediaObject } from "@thor/model";
...
const MediaObjectTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<MediaObject[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMediaObject(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<MediaObject> = { ...source };
      delete (payload as any).id;
      const created = await addMediaObject(payload).unwrap();
      setAllData((prev) => [created as MediaObject, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<MediaObject> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMediaObject(payload).unwrap();
      setAllData((prev) => [created as MediaObject, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <FloatingControlPanel
            description="MediaObject Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create MediaObject</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <MediaObjectForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter MediaObject (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof MediaObject;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of MediaObjects.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "MediaObject"}
