...
import { PersistentLogin } from '@thor/model';
...
const PersistentLoginTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<PersistentLogin[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePersistentLogin(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<PersistentLogin> = { ...source };
      delete (payload as any).id;
      const created = await addPersistentLogin(payload).unwrap();
      setAllData((prev) => [created as PersistentLogin, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<PersistentLogin> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addPersistentLogin(payload).unwrap();
      setAllData((prev) => [created as PersistentLogin, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="PersistentLoginTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="PersistentLogin Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create PersistentLogin</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <PersistentLoginForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter PersistentLogin (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof PersistentLogin;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of PersistentLogins.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'PersistentLogin'}
