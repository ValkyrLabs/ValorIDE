...
const DiscardDeadLetterEntryResponseTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addDiscardDeadLetterEntryResponse] =
    useAddDiscardDeadLetterEntryResponseMutation();
  const [updateDiscardDeadLetterEntryResponse] =
    useUpdateDiscardDeadLetterEntryResponseMutation();
  const [deleteDiscardDeadLetterEntryResponse] =
    useDeleteDiscardDeadLetterEntryResponseMutation();
  ...
  const [copiedRow, setCopiedRow] =
    useState<DiscardDeadLetterEntryResponse | null>(null);
  ...
  const handleAddRow = async () => {
    try {
      ...
      setAllData((prev) => [
        created as DiscardDeadLetterEntryResponse,
        ...prev,
      ]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) DiscardDeadLetterEntryResponse:",
        error,
      );
    ...
    <>
      <div className="tableContainer">
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter DiscardDeadLetterEntryResponse (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field =
                richModal.field as keyof DiscardDeadLetterEntryResponse;
              if (!rid || !field) {
