...
import { DiscardDeadLetterEntryResponse } from '@thor/model';
...
import {
  useLazyGetDiscardDeadLetterEntryResponsesPagedQuery,
...
} from '@thor/redux/services/DiscardDeadLetterEntryResponseService';
...
const DiscardDeadLetterEntryResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<DiscardDeadLetterEntryResponse[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<DiscardDeadLetterEntryResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<DiscardDeadLetterEntryResponse | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteDiscardDeadLetterEntryResponse(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<DiscardDeadLetterEntryResponse> = { ...source };
      delete (payload as any).id;
      const created = await addDiscardDeadLetterEntryResponse(payload).unwrap();
      setAllData((prev) => [created as DiscardDeadLetterEntryResponse, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<DiscardDeadLetterEntryResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addDiscardDeadLetterEntryResponse(payload).unwrap();
      setAllData((prev) => [created as DiscardDeadLetterEntryResponse, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="DiscardDeadLetterEntryResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="DiscardDeadLetterEntryResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create DiscardDeadLetterEntryResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <DiscardDeadLetterEntryResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter DiscardDeadLetterEntryResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing DiscardDeadLetterEntryResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof DiscardDeadLetterEntryResponse;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of DiscardDeadLetterEntryResponses.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'DiscardDeadLetterEntryResponse'}
