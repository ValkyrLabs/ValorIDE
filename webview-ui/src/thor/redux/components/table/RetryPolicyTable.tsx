...
import { RetryPolicy } from '@thor/model';
...
const RetryPolicyTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RetryPolicy[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<RetryPolicy>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RetryPolicy | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRetryPolicy(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RetryPolicy> = { ...source };
      delete (payload as any).id;
      const created = await addRetryPolicy(payload).unwrap();
      setAllData((prev) => [created as RetryPolicy, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RetryPolicy> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRetryPolicy(payload).unwrap();
      setAllData((prev) => [created as RetryPolicy, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          >
            ...
            <FloatingControlPanel
              description="RetryPolicy Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RetryPolicy</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RetryPolicyForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RetryPolicy (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RetryPolicy</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RetryPolicy;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RetryPolicys.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RetryPolicy'}
