...
import {
  useLazyGetMcpsPagedQuery,
  useAddMcpMutation,
  useUpdateMcpMutation,
  useDeleteMcpMutation
} from '@thor/redux/services/McpService';
...
const McpTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Mcp[]>([]);
  ...
  const [copiedRow, setCopiedRow] = useState<Mcp | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcp(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Mcp> = { ...source };
      delete (payload as any).id;
      const created = await addMcp(payload).unwrap();
      setAllData((prev) => [created as Mcp, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Mcp> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMcp(payload).unwrap();
      setAllData((prev) => [created as Mcp, ...prev]);
    } catch (error) {
        ...
        <div className="tableContainer">
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create Mcp</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <McpForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter Mcp (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing Mcp</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof Mcp;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of Mcps.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
