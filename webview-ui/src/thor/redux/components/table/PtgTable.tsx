...
import {
  useLazyGetPtgsPagedQuery,
  useAddPtgMutation,
  useUpdatePtgMutation,
  useDeletePtgMutation
} from '@thor/redux/services/PtgService';
...
const PtgTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Ptg[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePtg(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Ptg> = { ...source };
      delete (payload as any).id;
      const created = await addPtg(payload).unwrap();
      setAllData((prev) => [created as Ptg, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Ptg> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addPtg(payload).unwrap();
      setAllData((prev) => [created as Ptg, ...prev]);
    } catch (error) {
        ...
        <div className="tableContainer">
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create Ptg</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <PtgForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter Ptg (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof Ptg;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of Ptgs.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
