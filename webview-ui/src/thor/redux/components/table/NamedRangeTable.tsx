...
import { NamedRange } from '@thor/model';
...
const NamedRangeTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<NamedRange[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteNamedRange(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<NamedRange> = { ...source };
      delete (payload as any).id;
      const created = await addNamedRange(payload).unwrap();
      setAllData((prev) => [created as NamedRange, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<NamedRange> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addNamedRange(payload).unwrap();
      setAllData((prev) => [created as NamedRange, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          >
            ...
            <FloatingControlPanel
              description="NamedRange Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create NamedRange</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <NamedRangeForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter NamedRange (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof NamedRange;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of NamedRanges.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'NamedRange'}
