...
import { SwarmCommandRequest } from '@thor/model';
...
const SwarmCommandRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SwarmCommandRequest[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<SwarmCommandRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SwarmCommandRequest | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmCommandRequest(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SwarmCommandRequest> = { ...source };
      delete (payload as any).id;
      const created = await addSwarmCommandRequest(payload).unwrap();
      setAllData((prev) => [created as SwarmCommandRequest, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SwarmCommandRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSwarmCommandRequest(payload).unwrap();
      setAllData((prev) => [created as SwarmCommandRequest, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="SwarmCommandRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="SwarmCommandRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SwarmCommandRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SwarmCommandRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SwarmCommandRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SwarmCommandRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SwarmCommandRequest;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SwarmCommandRequests.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SwarmCommandRequest'}
