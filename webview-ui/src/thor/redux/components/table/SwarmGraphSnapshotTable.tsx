...
import { SwarmGraphSnapshot } from '@thor/model';
...
const SwarmGraphSnapshotTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SwarmGraphSnapshot[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<SwarmGraphSnapshot>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SwarmGraphSnapshot | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmGraphSnapshot(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SwarmGraphSnapshot> = { ...source };
      delete (payload as any).id;
      const created = await addSwarmGraphSnapshot(payload).unwrap();
      setAllData((prev) => [created as SwarmGraphSnapshot, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SwarmGraphSnapshot> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSwarmGraphSnapshot(payload).unwrap();
      setAllData((prev) => [created as SwarmGraphSnapshot, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="SwarmGraphSnapshotTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="SwarmGraphSnapshot Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SwarmGraphSnapshot</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SwarmGraphSnapshotForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SwarmGraphSnapshot (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SwarmGraphSnapshot</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SwarmGraphSnapshot;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SwarmGraphSnapshots.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SwarmGraphSnapshot'}
