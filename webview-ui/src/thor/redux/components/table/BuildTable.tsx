...
import {
  useLazyGetBuildsPagedQuery,
  useAddBuildMutation,
  useUpdateBuildMutation,
  useDeleteBuildMutation,
} from "@thor/redux/services/BuildService";
...
const BuildTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Build[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteBuild(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Build> = { ...source };
      delete (payload as any).id;
      const created = await addBuild(payload).unwrap();
      setAllData((prev) => [created as Build, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Build> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addBuild(payload).unwrap();
      setAllData((prev) => [created as Build, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter Build (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof Build;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of Builds.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "Build"}
