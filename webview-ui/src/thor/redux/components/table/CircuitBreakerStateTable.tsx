...
import { CircuitBreakerState } from '@thor/model';
...
const CircuitBreakerStateTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<CircuitBreakerState[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<CircuitBreakerState>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<CircuitBreakerState | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCircuitBreakerState(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<CircuitBreakerState> = { ...source };
      delete (payload as any).id;
      const created = await addCircuitBreakerState(payload).unwrap();
      setAllData((prev) => [created as CircuitBreakerState, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<CircuitBreakerState> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addCircuitBreakerState(payload).unwrap();
      setAllData((prev) => [created as CircuitBreakerState, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="CircuitBreakerStateTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="CircuitBreakerState Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create CircuitBreakerState</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <CircuitBreakerStateForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter CircuitBreakerState (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing CircuitBreakerState</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof CircuitBreakerState;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of CircuitBreakerStates.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'CircuitBreakerState'}
