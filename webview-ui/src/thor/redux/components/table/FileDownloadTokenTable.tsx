...
import { FileDownloadToken } from '@thor/model';
...
const FileDownloadTokenTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<FileDownloadToken[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<FileDownloadToken>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<FileDownloadToken | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteFileDownloadToken(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<FileDownloadToken> = { ...source };
      delete (payload as any).id;
      const created = await addFileDownloadToken(payload).unwrap();
      setAllData((prev) => [created as FileDownloadToken, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<FileDownloadToken> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addFileDownloadToken(payload).unwrap();
      setAllData((prev) => [created as FileDownloadToken, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="FileDownloadTokenTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="FileDownloadToken Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create FileDownloadToken</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <FileDownloadTokenForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter FileDownloadToken (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing FileDownloadToken</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof FileDownloadToken;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of FileDownloadTokens.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'FileDownloadToken'}
