...
import { Campaign } from "@thor/model";
...
import {
  useLazyGetCampaignsPagedQuery,
  useAddCampaignMutation,
  useUpdateCampaignMutation,
  useDeleteCampaignMutation,
} from "@thor/redux/services/CampaignService";
...
const CampaignTable: React.FC = () => {
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCampaign(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Campaign> = { ...source };
      delete (payload as any).id;
      const created = await addCampaign(payload).unwrap();
      setAllData((prev) => [created as Campaign, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Campaign> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addCampaign(payload).unwrap();
      setAllData((prev) => [created as Campaign, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter Campaign (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof Campaign;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of Campaigns.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "Campaign"}
