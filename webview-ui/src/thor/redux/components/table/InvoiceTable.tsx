...
import { Invoice } from "@thor/model";
...
import {
  useLazyGetInvoicesPagedQuery,
  useAddInvoiceMutation,
  useUpdateInvoiceMutation,
  useDeleteInvoiceMutation,
} from "@thor/redux/services/InvoiceService";
...
const InvoiceTable: React.FC = () => {
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteInvoice(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Invoice> = { ...source };
      delete (payload as any).id;
      const created = await addInvoice(payload).unwrap();
      setAllData((prev) => [created as Invoice, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Invoice> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addInvoice(payload).unwrap();
      setAllData((prev) => [created as Invoice, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter Invoice (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof Invoice;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of Invoices.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "Invoice"}
