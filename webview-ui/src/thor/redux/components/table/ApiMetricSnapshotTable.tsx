...
import { ApiMetricSnapshot } from '@thor/model';
...
const ApiMetricSnapshotTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ApiMetricSnapshot[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<ApiMetricSnapshot>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ApiMetricSnapshot | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteApiMetricSnapshot(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ApiMetricSnapshot> = { ...source };
      delete (payload as any).id;
      const created = await addApiMetricSnapshot(payload).unwrap();
      setAllData((prev) => [created as ApiMetricSnapshot, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ApiMetricSnapshot> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addApiMetricSnapshot(payload).unwrap();
      setAllData((prev) => [created as ApiMetricSnapshot, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="ApiMetricSnapshotTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ApiMetricSnapshot Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ApiMetricSnapshot</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ApiMetricSnapshotForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ApiMetricSnapshot (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ApiMetricSnapshot</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ApiMetricSnapshot;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ApiMetricSnapshots.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ApiMetricSnapshot'}
