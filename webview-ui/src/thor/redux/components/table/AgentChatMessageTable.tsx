...
import { AgentChatMessage } from "@thor/model";
...
const AgentChatMessageTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<AgentChatMessage[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<AgentChatMessage>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<AgentChatMessage | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAgentChatMessage(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<AgentChatMessage> = { ...source };
      delete (payload as any).id;
      const created = await addAgentChatMessage(payload).unwrap();
      setAllData((prev) => [created as AgentChatMessage, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<AgentChatMessage> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addAgentChatMessage(payload).unwrap();
      setAllData((prev) => [created as AgentChatMessage, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="AgentChatMessageTable"
        ...
        >
          <FloatingControlPanel
            description="AgentChatMessage Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create AgentChatMessage</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <AgentChatMessageForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter AgentChatMessage (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing AgentChatMessage</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof AgentChatMessage;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of AgentChatMessages.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "AgentChatMessage"}
