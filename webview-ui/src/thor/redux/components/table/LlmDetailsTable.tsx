...
import { LlmDetails } from '@thor/model';
...
const LlmDetailsTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<LlmDetails[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteLlmDetails(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<LlmDetails> = { ...source };
      delete (payload as any).id;
      const created = await addLlmDetails(payload).unwrap();
      setAllData((prev) => [created as LlmDetails, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<LlmDetails> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addLlmDetails(payload).unwrap();
      setAllData((prev) => [created as LlmDetails, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          >
            ...
            <FloatingControlPanel
              description="LlmDetails Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create LlmDetails</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <LlmDetailsForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter LlmDetails (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof LlmDetails;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of LlmDetailss.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'LlmDetails'}
