...
const JobApplicationResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<JobApplicationResponse[]>([]);
  ...
  const [updateJobApplicationResponse] =
    useUpdateJobApplicationResponseMutation();
  const [deleteJobApplicationResponse] =
    useDeleteJobApplicationResponseMutation();
  ...
  const [formData, setFormData] = useState<Partial<JobApplicationResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<JobApplicationResponse | null>(
    null,
  );
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteJobApplicationResponse(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<JobApplicationResponse> = { ...source };
      delete (payload as any).id;
      const created = await addJobApplicationResponse(payload).unwrap();
      setAllData((prev) => [created as JobApplicationResponse, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<JobApplicationResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addJobApplicationResponse(payload).unwrap();
      setAllData((prev) => [created as JobApplicationResponse, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="JobApplicationResponseTable"
        ...
        >
          <FloatingControlPanel
            description="JobApplicationResponse Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create JobApplicationResponse</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <JobApplicationResponseForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter JobApplicationResponse (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing JobApplicationResponse</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof JobApplicationResponse;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of JobApplicationResponses.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "JobApplicationResponse"}
