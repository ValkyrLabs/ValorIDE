...
import { ConversationMemoryNode } from "@thor/model";
...
const ConversationMemoryNodeTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ConversationMemoryNode[]>([]);
  ...
  const [updateConversationMemoryNode] =
    useUpdateConversationMemoryNodeMutation();
  const [deleteConversationMemoryNode] =
    useDeleteConversationMemoryNodeMutation();
  ...
  const [formData, setFormData] = useState<Partial<ConversationMemoryNode>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ConversationMemoryNode | null>(
    null,
  );
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteConversationMemoryNode(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ConversationMemoryNode> = { ...source };
      delete (payload as any).id;
      const created = await addConversationMemoryNode(payload).unwrap();
      setAllData((prev) => [created as ConversationMemoryNode, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ConversationMemoryNode> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addConversationMemoryNode(payload).unwrap();
      setAllData((prev) => [created as ConversationMemoryNode, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="ConversationMemoryNodeTable"
        ...
        >
          <FloatingControlPanel
            description="ConversationMemoryNode Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create ConversationMemoryNode</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <ConversationMemoryNodeForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter ConversationMemoryNode (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing ConversationMemoryNode</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof ConversationMemoryNode;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of ConversationMemoryNodes.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "ConversationMemoryNode"}
