...
const ICON_SIZE = 22;
import PhoneVerificationResponseForm from "@thor/redux/components/form/PhoneVerificationResponseForm";
import CoolButton from "@valkyr/component-library/CoolButton";
// Removed SplitPane usage; using floating toolbar for edit form
// ** Import the LAZY paged hook only **
import { useLazyGetPhoneVerificationResponsesPagedQuery, useAddPhoneVerificationResponseMutation, useUpdatePhoneVerificationResponseMutation, useDeletePhoneVerificationResponseMutation, useDeletePhoneVerificationResponseCascadeMutation, } from "@thor/redux/services/PhoneVerificationResponseService";
...
const columnSchema = {
    status: {
        type: "enum",
        enumValues: ["OTP_SENT", "ALREADY_VERIFIED"],
...
const PhoneVerificationResponseTable = () => {
    // -------------------------------------------------------------------
    // 1) "Lazy" RTK Query Hook: We manually trigger page fetches
    // -------------------------------------------------------------------
    // destructure: [triggerFn, { data, isFetching, isError }]
    const [triggerGetPage, { data: fetchedPageData = [], isFetching, isError }] = useLazyGetPhoneVerificationResponsesPagedQuery();
    ...
    const confirmDelete = async () => {
        ...
        try {
            await Promise.all(ids.map(async (id) => {
                try {
                    await deletePhoneVerificationResponseCascade({
                        id,
                        cascade: true,
                        trash: true,
                    }).unwrap();
                    ...
                    try {
                        await deletePhoneVerificationResponse(id).unwrap();
                    }
                    catch (inner) {
                    ...
                    if (picked) {
                        ...
                        setTimeout(() => {
                            setFormData((prev) => {
                                ...
                                try {
                                    updatePhoneVerificationResponse(updated);
    ...
    const handleSave = async (editKey) => {
        ...
        try {
            await updatePhoneVerificationResponse(updatedItem).unwrap();
            ...
            const errorMsg = error instanceof Error
                ? error.message
                : "Failed to update PhoneVerificationResponse";
            console.error("Failed to update PhoneVerificationResponse:", error);
    ...
    return (_jsxs(_Fragment, { children: [_jsx(RBGrid, { data: tableData, columns: columns, columnSchema: resolvedColumnSchema, selectedRows: selectedRows, onToggleRow: handleRowSelect, onToggleAll: handleToggleAll, onRowPermissions: handleRowPermissions, editCellId: editRowId, formData: formData, onInputChange: handleInputChange, onKeyDownEdit: handleKeyDownEdit, onBlurEdit: handleBlur, onCellDoubleClick: handleDoubleClick, activeCell: activeCell, onCellFocus: handleCellFocus, onCellKeyDownNav: handleCellKeyDownNav, expandedObjects: expandedObjects, onToggleExpandObject: handleToggleExpandObject, onObjectFieldChange: handleObjectFieldChange, showAllFields: showAllFields, onToggleShowAllFields: () => setShowAllFields(!showAllFields), storageKey: "PhoneVerificationResponseTable", onReferencePick: onReferencePick, onRequestMoreRows: (dir) => {
                    ...
                    zIndex: 9999,
                    pointerEvents: "auto",
                }, children: _jsxs(FloatingControlPanel, { description: "PhoneVerificationResponse Actions", className: "grid-toolbar", style: {
                        pointerEvents: "auto",
                        width: 360,
                        maxWidth: 420,
...
//# sourceMappingURL=PhoneVerificationResponseTable.js.map