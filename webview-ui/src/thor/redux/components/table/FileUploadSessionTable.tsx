...
import { FileUploadSession } from '@thor/model';
...
const FileUploadSessionTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<FileUploadSession[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<FileUploadSession>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<FileUploadSession | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteFileUploadSession(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<FileUploadSession> = { ...source };
      delete (payload as any).id;
      const created = await addFileUploadSession(payload).unwrap();
      setAllData((prev) => [created as FileUploadSession, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<FileUploadSession> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addFileUploadSession(payload).unwrap();
      setAllData((prev) => [created as FileUploadSession, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="FileUploadSessionTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="FileUploadSession Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create FileUploadSession</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <FileUploadSessionForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter FileUploadSession (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing FileUploadSession</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof FileUploadSession;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of FileUploadSessions.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'FileUploadSession'}
