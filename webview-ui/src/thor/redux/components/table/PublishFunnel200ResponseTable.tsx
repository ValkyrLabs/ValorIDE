...
import { PublishFunnel200Response } from '@thor/model';
...
const PublishFunnel200ResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<PublishFunnel200Response[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<PublishFunnel200Response>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<PublishFunnel200Response | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePublishFunnel200Response(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<PublishFunnel200Response> = { ...source };
      delete (payload as any).id;
      const created = await addPublishFunnel200Response(payload).unwrap();
      setAllData((prev) => [created as PublishFunnel200Response, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<PublishFunnel200Response> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addPublishFunnel200Response(payload).unwrap();
      setAllData((prev) => [created as PublishFunnel200Response, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="PublishFunnel200ResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="PublishFunnel200Response Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create PublishFunnel200Response</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <PublishFunnel200ResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter PublishFunnel200Response (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing PublishFunnel200Response</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof PublishFunnel200Response;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of PublishFunnel200Responses.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'PublishFunnel200Response'}
