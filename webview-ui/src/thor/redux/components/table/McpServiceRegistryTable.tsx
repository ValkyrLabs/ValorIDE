...
import { McpServiceRegistry } from "@thor/model";
...
const McpServiceRegistryTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<McpServiceRegistry[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<McpServiceRegistry>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<McpServiceRegistry | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpServiceRegistry(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<McpServiceRegistry> = { ...source };
      delete (payload as any).id;
      const created = await addMcpServiceRegistry(payload).unwrap();
      setAllData((prev) => [created as McpServiceRegistry, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<McpServiceRegistry> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMcpServiceRegistry(payload).unwrap();
      setAllData((prev) => [created as McpServiceRegistry, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="McpServiceRegistryTable"
        ...
        >
          <FloatingControlPanel
            description="McpServiceRegistry Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create McpServiceRegistry</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <McpServiceRegistryForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter McpServiceRegistry (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing McpServiceRegistry</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof McpServiceRegistry;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of McpServiceRegistrys.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "McpServiceRegistry"}
