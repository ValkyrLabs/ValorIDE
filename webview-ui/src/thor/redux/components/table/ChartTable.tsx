...
import {
  useLazyGetChartsPagedQuery,
  useAddChartMutation,
  useUpdateChartMutation,
  useDeleteChartMutation
} from '@thor/redux/services/ChartService';
...
const ChartTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Chart[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteChart(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Chart> = { ...source };
      delete (payload as any).id;
      const created = await addChart(payload).unwrap();
      setAllData((prev) => [created as Chart, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Chart> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addChart(payload).unwrap();
      setAllData((prev) => [created as Chart, ...prev]);
    } catch (error) {
    ...
    <>
        <div className="tableContainer">
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter Chart (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof Chart;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of Charts.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'Chart'}
