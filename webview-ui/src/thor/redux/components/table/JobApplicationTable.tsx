...
import { JobApplication } from "@thor/model";
...
import {
  useLazyGetJobApplicationsPagedQuery,
  useAddJobApplicationMutation,
  useUpdateJobApplicationMutation,
  useDeleteJobApplicationMutation,
} from "@thor/redux/services/JobApplicationService";
...
const JobApplicationTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<JobApplication[]>([]);
  ...
  const [copiedRow, setCopiedRow] = useState<JobApplication | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteJobApplication(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<JobApplication> = { ...source };
      delete (payload as any).id;
      const created = await addJobApplication(payload).unwrap();
      setAllData((prev) => [created as JobApplication, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<JobApplication> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addJobApplication(payload).unwrap();
      setAllData((prev) => [created as JobApplication, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="JobApplicationTable"
        ...
        >
          <FloatingControlPanel
            description="JobApplication Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create JobApplication</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <JobApplicationForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter JobApplication (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing JobApplication</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof JobApplication;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of JobApplications.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "JobApplication"}
