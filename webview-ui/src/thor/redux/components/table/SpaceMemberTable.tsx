...
import { SpaceMember } from "@thor/model";
...
import {
  useLazyGetSpaceMembersPagedQuery,
  useAddSpaceMemberMutation,
  useUpdateSpaceMemberMutation,
  useDeleteSpaceMemberMutation,
} from "@thor/redux/services/SpaceMemberService";
...
const SpaceMemberTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SpaceMember[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<SpaceMember>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SpaceMember | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSpaceMember(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SpaceMember> = { ...source };
      delete (payload as any).id;
      const created = await addSpaceMember(payload).unwrap();
      setAllData((prev) => [created as SpaceMember, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SpaceMember> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSpaceMember(payload).unwrap();
      setAllData((prev) => [created as SpaceMember, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <FloatingControlPanel
            description="SpaceMember Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create SpaceMember</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <SpaceMemberForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter SpaceMember (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing SpaceMember</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof SpaceMember;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of SpaceMembers.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "SpaceMember"}
