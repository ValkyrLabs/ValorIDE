...
import { McpMarketplaceItem } from '@thor/model';
...
const McpMarketplaceItemTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<McpMarketplaceItem[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<McpMarketplaceItem>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<McpMarketplaceItem | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpMarketplaceItem(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<McpMarketplaceItem> = { ...source };
      delete (payload as any).id;
      const created = await addMcpMarketplaceItem(payload).unwrap();
      setAllData((prev) => [created as McpMarketplaceItem, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<McpMarketplaceItem> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMcpMarketplaceItem(payload).unwrap();
      setAllData((prev) => [created as McpMarketplaceItem, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="McpMarketplaceItemTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="McpMarketplaceItem Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create McpMarketplaceItem</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <McpMarketplaceItemForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter McpMarketplaceItem (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing McpMarketplaceItem</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof McpMarketplaceItem;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of McpMarketplaceItems.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'McpMarketplaceItem'}
