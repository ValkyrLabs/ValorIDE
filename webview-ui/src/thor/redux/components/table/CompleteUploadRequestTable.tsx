...
const CompleteUploadRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<CompleteUploadRequest[]>([]);
  ...
  const [updateCompleteUploadRequest] =
    useUpdateCompleteUploadRequestMutation();
  const [deleteCompleteUploadRequest] =
    useDeleteCompleteUploadRequestMutation();
  ...
  const [formData, setFormData] = useState<Partial<CompleteUploadRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<CompleteUploadRequest | null>(
    null,
  );
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCompleteUploadRequest(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<CompleteUploadRequest> = { ...source };
      delete (payload as any).id;
      const created = await addCompleteUploadRequest(payload).unwrap();
      setAllData((prev) => [created as CompleteUploadRequest, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<CompleteUploadRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addCompleteUploadRequest(payload).unwrap();
      setAllData((prev) => [created as CompleteUploadRequest, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="CompleteUploadRequestTable"
        ...
        >
          <FloatingControlPanel
            description="CompleteUploadRequest Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create CompleteUploadRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <CompleteUploadRequestForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter CompleteUploadRequest (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing CompleteUploadRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof CompleteUploadRequest;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of CompleteUploadRequests.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "CompleteUploadRequest"}
