...
import { PromptRating } from "@thor/model";
...
const PromptRatingTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<PromptRating[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<PromptRating>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<PromptRating | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePromptRating(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<PromptRating> = { ...source };
      delete (payload as any).id;
      const created = await addPromptRating(payload).unwrap();
      setAllData((prev) => [created as PromptRating, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<PromptRating> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addPromptRating(payload).unwrap();
      setAllData((prev) => [created as PromptRating, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <FloatingControlPanel
            description="PromptRating Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create PromptRating</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <PromptRatingForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter PromptRating (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing PromptRating</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof PromptRating;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of PromptRatings.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "PromptRating"}
