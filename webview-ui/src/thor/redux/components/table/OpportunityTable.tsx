...
import { Opportunity } from '@thor/model';
...
import {
  useLazyGetOpportunitysPagedQuery,
  useAddOpportunityMutation,
  useUpdateOpportunityMutation,
  useDeleteOpportunityMutation
} from '@thor/redux/services/OpportunityService';
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOpportunity(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Opportunity> = { ...source };
      delete (payload as any).id;
      const created = await addOpportunity(payload).unwrap();
      setAllData((prev) => [created as Opportunity, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Opportunity> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addOpportunity(payload).unwrap();
      setAllData((prev) => [created as Opportunity, ...prev]);
    } catch (error) {
    ...
    <>
        <div className="tableContainer">
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter Opportunity (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof Opportunity;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of Opportunitys.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'Opportunity'}
