...
import { ChatMessage } from "@thor/model";
...
import {
  useLazyGetChatMessagesPagedQuery,
  useAddChatMessageMutation,
  useUpdateChatMessageMutation,
  useDeleteChatMessageMutation,
} from "@thor/redux/services/ChatMessageService";
...
const ChatMessageTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<ChatMessage[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteChatMessage(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ChatMessage> = { ...source };
      delete (payload as any).id;
      const created = await addChatMessage(payload).unwrap();
      setAllData((prev) => [created as ChatMessage, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ChatMessage> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addChatMessage(payload).unwrap();
      setAllData((prev) => [created as ChatMessage, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <FloatingControlPanel
            description="ChatMessage Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create ChatMessage</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <ChatMessageForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter ChatMessage (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof ChatMessage;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of ChatMessages.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "ChatMessage"}
