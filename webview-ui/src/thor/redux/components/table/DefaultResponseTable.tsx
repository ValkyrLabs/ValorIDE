...
import { DefaultResponse } from '@thor/model';
...
const DefaultResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<DefaultResponse[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<DefaultResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<DefaultResponse | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteDefaultResponse(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<DefaultResponse> = { ...source };
      delete (payload as any).id;
      const created = await addDefaultResponse(payload).unwrap();
      setAllData((prev) => [created as DefaultResponse, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<DefaultResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addDefaultResponse(payload).unwrap();
      setAllData((prev) => [created as DefaultResponse, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="DefaultResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="DefaultResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create DefaultResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <DefaultResponseForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter DefaultResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing DefaultResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof DefaultResponse;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of DefaultResponses.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'DefaultResponse'}
