...
import { Organization } from '@thor/model';
...
import {
  useLazyGetOrganizationsPagedQuery,
  useAddOrganizationMutation,
  useUpdateOrganizationMutation,
  useDeleteOrganizationMutation
} from '@thor/redux/services/OrganizationService';
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOrganization(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Organization> = { ...source };
      delete (payload as any).id;
      const created = await addOrganization(payload).unwrap();
      setAllData((prev) => [created as Organization, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Organization> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addOrganization(payload).unwrap();
      setAllData((prev) => [created as Organization, ...prev]);
    } catch (error) {
    ...
    <>
        <div className="tableContainer">
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter Organization (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof Organization;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of Organizations.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'Organization'}
