...
import { ActivateAgent200Response } from '@thor/model';
...
const ActivateAgent200ResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ActivateAgent200Response[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<ActivateAgent200Response>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ActivateAgent200Response | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteActivateAgent200Response(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ActivateAgent200Response> = { ...source };
      delete (payload as any).id;
      const created = await addActivateAgent200Response(payload).unwrap();
      setAllData((prev) => [created as ActivateAgent200Response, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ActivateAgent200Response> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addActivateAgent200Response(payload).unwrap();
      setAllData((prev) => [created as ActivateAgent200Response, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="ActivateAgent200ResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ActivateAgent200Response Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ActivateAgent200Response</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ActivateAgent200ResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ActivateAgent200Response (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ActivateAgent200Response</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ActivateAgent200Response;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ActivateAgent200Responses.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ActivateAgent200Response'}
