...
import { DeadLetterQueue } from '@thor/model';
...
const DeadLetterQueueTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<DeadLetterQueue[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<DeadLetterQueue>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<DeadLetterQueue | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteDeadLetterQueue(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<DeadLetterQueue> = { ...source };
      delete (payload as any).id;
      const created = await addDeadLetterQueue(payload).unwrap();
      setAllData((prev) => [created as DeadLetterQueue, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<DeadLetterQueue> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addDeadLetterQueue(payload).unwrap();
      setAllData((prev) => [created as DeadLetterQueue, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="DeadLetterQueueTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="DeadLetterQueue Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create DeadLetterQueue</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <DeadLetterQueueForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter DeadLetterQueue (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing DeadLetterQueue</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof DeadLetterQueue;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of DeadLetterQueues.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'DeadLetterQueue'}
