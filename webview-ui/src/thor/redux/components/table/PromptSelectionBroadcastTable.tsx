...
import { PromptSelectionBroadcast } from "@thor/model";
...
const PromptSelectionBroadcastTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<PromptSelectionBroadcast[]>([]);
  ...
  // -------------------------------------------------------------------
  const [addPromptSelectionBroadcast] =
    useAddPromptSelectionBroadcastMutation();
  const [updatePromptSelectionBroadcast] =
    useUpdatePromptSelectionBroadcastMutation();
  const [deletePromptSelectionBroadcast] =
    useDeletePromptSelectionBroadcastMutation();
  ...
  const [formData, setFormData] = useState<Partial<PromptSelectionBroadcast>>(
    {},
  );
  ...
  const [copiedRow, setCopiedRow] = useState<PromptSelectionBroadcast | null>(
    null,
  );
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePromptSelectionBroadcast(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<PromptSelectionBroadcast> = { ...source };
      delete (payload as any).id;
      const created = await addPromptSelectionBroadcast(payload).unwrap();
      setAllData((prev) => [created as PromptSelectionBroadcast, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<PromptSelectionBroadcast> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addPromptSelectionBroadcast(payload).unwrap();
      setAllData((prev) => [created as PromptSelectionBroadcast, ...prev]);
    } catch (error) {
      console.error(
        "Failed to paste (create) PromptSelectionBroadcast:",
        error,
      );
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="PromptSelectionBroadcastTable"
        ...
        >
          <FloatingControlPanel
            description="PromptSelectionBroadcast Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create PromptSelectionBroadcast</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <PromptSelectionBroadcastForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter PromptSelectionBroadcast (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing PromptSelectionBroadcast</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof PromptSelectionBroadcast;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of PromptSelectionBroadcasts.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "PromptSelectionBroadcast"}
