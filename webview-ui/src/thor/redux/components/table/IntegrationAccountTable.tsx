...
import { IntegrationAccount } from "@thor/model";
...
const IntegrationAccountTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<IntegrationAccount[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteIntegrationAccount(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<IntegrationAccount> = { ...source };
      delete (payload as any).id;
      const created = await addIntegrationAccount(payload).unwrap();
      setAllData((prev) => [created as IntegrationAccount, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<IntegrationAccount> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addIntegrationAccount(payload).unwrap();
      setAllData((prev) => [created as IntegrationAccount, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="IntegrationAccountTable"
        ...
        >
          <FloatingControlPanel
            description="IntegrationAccount Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create IntegrationAccount</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter IntegrationAccount (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof IntegrationAccount;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of IntegrationAccounts.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "IntegrationAccount"}
