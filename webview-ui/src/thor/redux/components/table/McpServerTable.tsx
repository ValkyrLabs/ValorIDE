...
import { McpServer } from "@thor/model";
...
import {
  useLazyGetMcpServersPagedQuery,
  useAddMcpServerMutation,
  useUpdateMcpServerMutation,
  useDeleteMcpServerMutation,
} from "@thor/redux/services/McpServerService";
...
const McpServerTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<McpServer[]>([]);
  ...
  const [copiedRow, setCopiedRow] = useState<McpServer | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpServer(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<McpServer> = { ...source };
      delete (payload as any).id;
      const created = await addMcpServer(payload).unwrap();
      setAllData((prev) => [created as McpServer, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<McpServer> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMcpServer(payload).unwrap();
      setAllData((prev) => [created as McpServer, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create McpServer</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <McpServerForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter McpServer (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing McpServer</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof McpServer;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of McpServers.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "McpServer"}
