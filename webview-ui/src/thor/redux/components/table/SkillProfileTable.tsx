...
import { SkillProfile } from "@thor/model";
...
const SkillProfileTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SkillProfile[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<SkillProfile>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SkillProfile | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSkillProfile(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SkillProfile> = { ...source };
      delete (payload as any).id;
      const created = await addSkillProfile(payload).unwrap();
      setAllData((prev) => [created as SkillProfile, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SkillProfile> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSkillProfile(payload).unwrap();
      setAllData((prev) => [created as SkillProfile, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <FloatingControlPanel
            description="SkillProfile Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create SkillProfile</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <SkillProfileForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter SkillProfile (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing SkillProfile</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof SkillProfile;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of SkillProfiles.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "SkillProfile"}
