...
import { McpResourceTemplate } from '@thor/model';
...
const McpResourceTemplateTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<McpResourceTemplate[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<McpResourceTemplate>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<McpResourceTemplate | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpResourceTemplate(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<McpResourceTemplate> = { ...source };
      delete (payload as any).id;
      const created = await addMcpResourceTemplate(payload).unwrap();
      setAllData((prev) => [created as McpResourceTemplate, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<McpResourceTemplate> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMcpResourceTemplate(payload).unwrap();
      setAllData((prev) => [created as McpResourceTemplate, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="McpResourceTemplateTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="McpResourceTemplate Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create McpResourceTemplate</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter McpResourceTemplate (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing McpResourceTemplate</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof McpResourceTemplate;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of McpResourceTemplates.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'McpResourceTemplate'}
