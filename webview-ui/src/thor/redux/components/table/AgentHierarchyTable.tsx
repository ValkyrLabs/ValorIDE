...
import { AgentHierarchy } from "@thor/model";
...
const AgentHierarchyTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<AgentHierarchy[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<AgentHierarchy>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<AgentHierarchy | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAgentHierarchy(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<AgentHierarchy> = { ...source };
      delete (payload as any).id;
      const created = await addAgentHierarchy(payload).unwrap();
      setAllData((prev) => [created as AgentHierarchy, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<AgentHierarchy> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addAgentHierarchy(payload).unwrap();
      setAllData((prev) => [created as AgentHierarchy, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="AgentHierarchyTable"
        ...
        >
          <FloatingControlPanel
            description="AgentHierarchy Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create AgentHierarchy</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <AgentHierarchyForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter AgentHierarchy (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing AgentHierarchy</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof AgentHierarchy;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of AgentHierarchys.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "AgentHierarchy"}
