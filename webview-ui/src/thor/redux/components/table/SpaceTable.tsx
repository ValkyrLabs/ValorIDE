...
import {
  useLazyGetSpacesPagedQuery,
  useAddSpaceMutation,
  useUpdateSpaceMutation,
  useDeleteSpaceMutation,
} from "@thor/redux/services/SpaceService";
...
const SpaceTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Space[]>([]);
  ...
  const [copiedRow, setCopiedRow] = useState<Space | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSpace(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Space> = { ...source };
      delete (payload as any).id;
      const created = await addSpace(payload).unwrap();
      setAllData((prev) => [created as Space, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Space> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSpace(payload).unwrap();
      setAllData((prev) => [created as Space, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter Space (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing Space</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof Space;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of Spaces.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "Space"}
