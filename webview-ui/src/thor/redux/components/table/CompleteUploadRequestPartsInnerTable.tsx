...
import {
  useLazyGetCompleteUploadRequestPartsInnersPagedQuery,
...
} from "@thor/redux/services/CompleteUploadRequestPartsInnerService";
...
const CompleteUploadRequestPartsInnerTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<CompleteUploadRequestPartsInner[]>([]);
  ...
  // -------------------------------------------------------------------
  const [addCompleteUploadRequestPartsInner] =
    useAddCompleteUploadRequestPartsInnerMutation();
  const [updateCompleteUploadRequestPartsInner] =
    useUpdateCompleteUploadRequestPartsInnerMutation();
  const [deleteCompleteUploadRequestPartsInner] =
    useDeleteCompleteUploadRequestPartsInnerMutation();
  ...
  const [copiedRow, setCopiedRow] =
    useState<CompleteUploadRequestPartsInner | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCompleteUploadRequestPartsInner(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<CompleteUploadRequestPartsInner> = { ...source };
      delete (payload as any).id;
      const created =
        await addCompleteUploadRequestPartsInner(payload).unwrap();
      setAllData((prev) => [
        created as CompleteUploadRequestPartsInner,
        ...prev,
      ]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<CompleteUploadRequestPartsInner> = {
        ...copiedRow,
      };
      delete (payload as any).id;
      const created =
      ...
      console.error(
        "Failed to paste (create) CompleteUploadRequestPartsInner:",
        error,
      );
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="CompleteUploadRequestPartsInnerTable"
        ...
        >
          <FloatingControlPanel
            description="CompleteUploadRequestPartsInner Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create CompleteUploadRequestPartsInner</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <CompleteUploadRequestPartsInnerForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter CompleteUploadRequestPartsInner (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing CompleteUploadRequestPartsInner</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field =
                richModal.field as keyof CompleteUploadRequestPartsInner;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of CompleteUploadRequestPartsInners.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "CompleteUploadRequestPartsInner"}
