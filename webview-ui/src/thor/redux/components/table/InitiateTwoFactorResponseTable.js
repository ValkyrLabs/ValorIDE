...
const ICON_SIZE = 22;
import InitiateTwoFactorResponseForm from "@thor/redux/components/form/InitiateTwoFactorResponseForm";
import CoolButton from "@valkyr/component-library/CoolButton";
// Removed SplitPane usage; using floating toolbar for edit form
// ** Import the LAZY paged hook only **
import { useLazyGetInitiateTwoFactorResponsesPagedQuery, useAddInitiateTwoFactorResponseMutation, useUpdateInitiateTwoFactorResponseMutation, useDeleteInitiateTwoFactorResponseMutation, useDeleteInitiateTwoFactorResponseCascadeMutation, } from "@thor/redux/services/InitiateTwoFactorResponseService";
...
const InitiateTwoFactorResponseTable = () => {
    // -------------------------------------------------------------------
    // 1) "Lazy" RTK Query Hook: We manually trigger page fetches
    // -------------------------------------------------------------------
    // destructure: [triggerFn, { data, isFetching, isError }]
    const [triggerGetPage, { data: fetchedPageData = [], isFetching, isError }] = useLazyGetInitiateTwoFactorResponsesPagedQuery();
    ...
    const confirmDelete = async () => {
        ...
        try {
            await Promise.all(ids.map(async (id) => {
                try {
                    await deleteInitiateTwoFactorResponseCascade({
                        id,
                        cascade: true,
                        trash: true,
                    }).unwrap();
                    ...
                    try {
                        await deleteInitiateTwoFactorResponse(id).unwrap();
                    }
                    catch (inner) {
                    ...
                    if (picked) {
                        ...
                        setTimeout(() => {
                            setFormData((prev) => {
                                ...
                                try {
                                    updateInitiateTwoFactorResponse(updated);
    ...
    const handleSave = async (editKey) => {
        ...
        try {
            await updateInitiateTwoFactorResponse(updatedItem).unwrap();
            ...
            const errorMsg = error instanceof Error
                ? error.message
                : "Failed to update InitiateTwoFactorResponse";
            console.error("Failed to update InitiateTwoFactorResponse:", error);
    ...
    return (_jsxs(_Fragment, { children: [_jsx(RBGrid, { data: tableData, columns: columns, columnSchema: resolvedColumnSchema, selectedRows: selectedRows, onToggleRow: handleRowSelect, onToggleAll: handleToggleAll, onRowPermissions: handleRowPermissions, editCellId: editRowId, formData: formData, onInputChange: handleInputChange, onKeyDownEdit: handleKeyDownEdit, onBlurEdit: handleBlur, onCellDoubleClick: handleDoubleClick, activeCell: activeCell, onCellFocus: handleCellFocus, onCellKeyDownNav: handleCellKeyDownNav, expandedObjects: expandedObjects, onToggleExpandObject: handleToggleExpandObject, onObjectFieldChange: handleObjectFieldChange, showAllFields: showAllFields, onToggleShowAllFields: () => setShowAllFields(!showAllFields), storageKey: "InitiateTwoFactorResponseTable", onReferencePick: onReferencePick, onRequestMoreRows: (dir) => {
                    ...
                    zIndex: 9999,
                    pointerEvents: "auto",
                }, children: _jsxs(FloatingControlPanel, { description: "InitiateTwoFactorResponse Actions", className: "grid-toolbar", style: {
                        pointerEvents: "auto",
                        width: 360,
                        maxWidth: 420,
...
//# sourceMappingURL=InitiateTwoFactorResponseTable.js.map