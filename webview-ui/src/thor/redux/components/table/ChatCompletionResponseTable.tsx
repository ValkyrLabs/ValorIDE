...
import { ChatCompletionResponse } from '@thor/model';
...
const ChatCompletionResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ChatCompletionResponse[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<ChatCompletionResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ChatCompletionResponse | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteChatCompletionResponse(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ChatCompletionResponse> = { ...source };
      delete (payload as any).id;
      const created = await addChatCompletionResponse(payload).unwrap();
      setAllData((prev) => [created as ChatCompletionResponse, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ChatCompletionResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addChatCompletionResponse(payload).unwrap();
      setAllData((prev) => [created as ChatCompletionResponse, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="ChatCompletionResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ChatCompletionResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ChatCompletionResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ChatCompletionResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ChatCompletionResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ChatCompletionResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ChatCompletionResponse;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ChatCompletionResponses.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ChatCompletionResponse'}
