...
import ExecuteModuleRequestForm from "@thor/redux/components/form/ExecuteModuleRequestForm";
import CoolButton from "@valkyr/component-library/CoolButton";
// Removed SplitPane usage; using floating toolbar for edit form
// ** Import the LAZY paged hook only **
import { useLazyGetExecuteModuleRequestsPagedQuery, useAddExecuteModuleRequestMutation, useUpdateExecuteModuleRequestMutation, useDeleteExecuteModuleRequestMutation, } from "@thor/redux/services/ExecuteModuleRequestService";
...
const ExecuteModuleRequestTable = () => {
    // -------------------------------------------------------------------
    // 1) "Lazy" RTK Query Hook: We manually trigger page fetches
    // -------------------------------------------------------------------
    // destructure: [triggerFn, { data, isFetching, isError }]
    const [triggerGetPage, { data: fetchedPageData = [], isFetching, isError }] = useLazyGetExecuteModuleRequestsPagedQuery();
    ...
    // -------------------------------------------------------------------
    const [addExecuteModuleRequest] = useAddExecuteModuleRequestMutation();
    const [updateExecuteModuleRequest] = useUpdateExecuteModuleRequestMutation();
    const [deleteExecuteModuleRequest] = useDeleteExecuteModuleRequestMutation();
    // -------------------------------------------------------------------
    ...
    const confirmDelete = async () => {
        try {
            ...
            await Promise.all(ids.map(async (id) => {
                try {
                    await deleteExecuteModuleRequest(id).unwrap();
                }
                catch (e) {
    ...
    const handleAddRow = async () => {
        try {
            ...
            delete payload.id;
            const created = await addExecuteModuleRequest(payload).unwrap();
            setAllData((prev) => [created, ...prev]);
        }
        catch (error) {
            console.error("Failed to add ExecuteModuleRequest:", error);
    ...
    const handlePasteRow = async () => {
        ...
        try {
            ...
            console.error("Failed to paste (create) ExecuteModuleRequest:", error);
    ...
    const handleSave = async (editKey) => {
        ...
        try {
            await updateExecuteModuleRequest(updatedItem).unwrap();
            setAllData((prev) => prev.map((i) => (i.id === id ? updatedItem : i)));
        }
        catch (error) {
            console.error("Failed to update ExecuteModuleRequest:", error);
        }
        setEditRowId(null);
        setFormData({});
    };
    ...
    return (_jsxs(_Fragment, { children: [_jsxs("div", { className: "tableContainer", children: [_jsxs("div", { className: "spreadsheet-container", children: [_jsx(RBGrid, { data: tableData, columns: columns, columnSchema: resolvedColumnSchema, selectedRows: selectedRows, onToggleRow: handleRowSelect, onToggleAll: handleToggleAll, onRowPermissions: handleRowPermissions, editCellId: editRowId, formData: formData, onInputChange: handleInputChange, onKeyDownEdit: handleKeyDownEdit, onBlurEdit: handleBlur, onCellDoubleClick: handleDoubleClick, activeCell: activeCell, onCellFocus: handleCellFocus, onCellKeyDownNav: handleCellKeyDownNav, expandedObjects: expandedObjects, onToggleExpandObject: handleToggleExpandObject, onObjectFieldChange: handleObjectFieldChange, showAllFields: showAllFields, onToggleShowAllFields: () => setShowAllFields(!showAllFields), storageKey: "ExecuteModuleRequestTable", onReferencePick: onReferencePick, onRequestMoreRows: (dir) => {
                            ...
                            zIndex: 9999,
                            pointerEvents: "auto",
                        }, children: _jsxs(FloatingControlPanel, { description: "ExecuteModuleRequest Actions", className: "grid-toolbar", style: {
                                pointerEvents: "auto",
                                width: 360,
                                maxWidth: 420,
                                boxShadow: "0 6px 20px rgba(0,0,0,0.3)",
                                    ...
                                    }, children: toolbarOpen ? _jsx(FaChevronRight, {}) : _jsx(FaChevronLeft, {}) }), _jsx("div", { children: _jsxs(ButtonGroup, { children: [_jsx(CoolButton, { variant: "secondary", onClick: handleLoadData, children: _jsx(FaSync, {}) }), _jsx(CoolButton, { variant: "primary", onClick: () => setShowCreateModal(true), children: _jsx(FaPlus, {}) }), _jsx(CoolButton, { disabled: selectedRows.size !== 1, variant: "secondary", onClick: handleCopyRow, children: _jsx(FaCopy, {}) }), _jsx(CoolButton, { variant: "secondary", onClick: handlePasteRow, children: _jsx(FaPaste, {}) }), _jsxs(CoolButton, { variant: "secondary", onClick: () => setShowQbeModal(true), children: [_jsx(FaFilter, {}), " Filter (QBE)"] }), _jsx(CoolButton, { variant: "warning", onClick: handleDelete, disabled: selectedRows.size !== 1, children: _jsx(FaTrash, {}) }), _jsx(CoolButton, { variant: "info", onClick: handleManagePermissions, disabled: selectedRows.size !== 1, children: _jsx(FaUserShield, {}) })] }) })] }) }), _jsxs(Modal, { show: showCreateModal, onHide: () => setShowCreateModal(false), centered: true, size: "lg", children: [_jsx(Modal.Header, { closeButton: true, children: _jsx(Modal.Title, { children: "Create ExecuteModuleRequest" }) }), _jsx(Modal.Body, { children: _jsx(ExecuteModuleRequestForm, {}) })] }), _jsxs(Modal, { show: showQbeModal, onHide: () => setShowQbeModal(false), centered: true, size: "lg", children: [_jsx(Modal.Header, { closeButton: true, children: _jsx(Modal.Title, { children: "Filter ExecuteModuleRequest (Query By Example)" }) }), _jsxs(Modal.Body, { children: [_jsx("p", { children: "Enter a JSON example. Non-null fields will be matched. Strings use case-insensitive contains; exact match for non-strings." }), _jsx(BSForm.Control, { as: "textarea", rows: 10, value: qbeText, onChange: (e) => setQbeText(e.target.value), placeholder: '{"name":"acme","status":"active"}' })] }), _jsxs(Modal.Footer, { children: [_jsx(CoolButton, { variant: "secondary", onClick: () => {
                                            setQbeText("");
                                            setQbeExample(null);
                            ...
                            try {
                                ...
                                await updateExecuteModuleRequest(updated).unwrap();
