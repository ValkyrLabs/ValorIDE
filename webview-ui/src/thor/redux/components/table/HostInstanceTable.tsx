...
import { HostInstance } from "@thor/model";
...
const HostInstanceTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<HostInstance[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<HostInstance>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<HostInstance | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteHostInstance(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<HostInstance> = { ...source };
      delete (payload as any).id;
      const created = await addHostInstance(payload).unwrap();
      setAllData((prev) => [created as HostInstance, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<HostInstance> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addHostInstance(payload).unwrap();
      setAllData((prev) => [created as HostInstance, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <FloatingControlPanel
            description="HostInstance Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create HostInstance</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <HostInstanceForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter HostInstance (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing HostInstance</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof HostInstance;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of HostInstances.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "HostInstance"}
