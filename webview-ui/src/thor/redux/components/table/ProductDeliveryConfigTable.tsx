...
import { ProductDeliveryConfig } from '@thor/model';
...
const ProductDeliveryConfigTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ProductDeliveryConfig[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<ProductDeliveryConfig>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ProductDeliveryConfig | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteProductDeliveryConfig(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ProductDeliveryConfig> = { ...source };
      delete (payload as any).id;
      const created = await addProductDeliveryConfig(payload).unwrap();
      setAllData((prev) => [created as ProductDeliveryConfig, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ProductDeliveryConfig> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addProductDeliveryConfig(payload).unwrap();
      setAllData((prev) => [created as ProductDeliveryConfig, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="ProductDeliveryConfigTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ProductDeliveryConfig Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ProductDeliveryConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ProductDeliveryConfigForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ProductDeliveryConfig (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ProductDeliveryConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ProductDeliveryConfig;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ProductDeliveryConfigs.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ProductDeliveryConfig'}
