...
import {
  useLazyGetSwarmsPagedQuery,
  useAddSwarmMutation,
  useUpdateSwarmMutation,
  useDeleteSwarmMutation
} from '@thor/redux/services/SwarmService';
...
const SwarmTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Swarm[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<Swarm>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<Swarm | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarm(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Swarm> = { ...source };
      delete (payload as any).id;
      const created = await addSwarm(payload).unwrap();
      setAllData((prev) => [created as Swarm, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Swarm> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSwarm(payload).unwrap();
      setAllData((prev) => [created as Swarm, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create Swarm</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SwarmForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter Swarm (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing Swarm</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof Swarm;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of Swarms.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'Swarm'}
