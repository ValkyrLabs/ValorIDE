...
import { AgentDiscoveryItem } from "@thor/model";
...
const AgentDiscoveryItemTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<AgentDiscoveryItem[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<AgentDiscoveryItem>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<AgentDiscoveryItem | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAgentDiscoveryItem(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<AgentDiscoveryItem> = { ...source };
      delete (payload as any).id;
      const created = await addAgentDiscoveryItem(payload).unwrap();
      setAllData((prev) => [created as AgentDiscoveryItem, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<AgentDiscoveryItem> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addAgentDiscoveryItem(payload).unwrap();
      setAllData((prev) => [created as AgentDiscoveryItem, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="AgentDiscoveryItemTable"
        ...
        >
          <FloatingControlPanel
            description="AgentDiscoveryItem Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create AgentDiscoveryItem</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <AgentDiscoveryItemForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter AgentDiscoveryItem (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing AgentDiscoveryItem</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof AgentDiscoveryItem;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of AgentDiscoveryItems.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "AgentDiscoveryItem"}
