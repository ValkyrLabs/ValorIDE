...
import { AgentEventTrigger } from '@thor/model';
...
const AgentEventTriggerTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<AgentEventTrigger[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<AgentEventTrigger>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<AgentEventTrigger | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAgentEventTrigger(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<AgentEventTrigger> = { ...source };
      delete (payload as any).id;
      const created = await addAgentEventTrigger(payload).unwrap();
      setAllData((prev) => [created as AgentEventTrigger, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<AgentEventTrigger> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addAgentEventTrigger(payload).unwrap();
      setAllData((prev) => [created as AgentEventTrigger, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="AgentEventTriggerTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="AgentEventTrigger Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create AgentEventTrigger</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <AgentEventTriggerForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter AgentEventTrigger (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing AgentEventTrigger</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof AgentEventTrigger;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of AgentEventTriggers.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'AgentEventTrigger'}
