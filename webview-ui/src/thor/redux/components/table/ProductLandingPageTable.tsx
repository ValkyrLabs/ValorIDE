...
import { ProductLandingPage } from '@thor/model';
...
const ProductLandingPageTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ProductLandingPage[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<ProductLandingPage>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ProductLandingPage | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteProductLandingPage(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ProductLandingPage> = { ...source };
      delete (payload as any).id;
      const created = await addProductLandingPage(payload).unwrap();
      setAllData((prev) => [created as ProductLandingPage, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ProductLandingPage> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addProductLandingPage(payload).unwrap();
      setAllData((prev) => [created as ProductLandingPage, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="ProductLandingPageTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ProductLandingPage Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ProductLandingPage</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ProductLandingPage (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ProductLandingPage</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ProductLandingPage;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ProductLandingPages.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ProductLandingPage'}
