...
import { AgentChatMessageResponse } from "@thor/model";
...
const AgentChatMessageResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<AgentChatMessageResponse[]>([]);
  ...
  // -------------------------------------------------------------------
  const [addAgentChatMessageResponse] =
    useAddAgentChatMessageResponseMutation();
  const [updateAgentChatMessageResponse] =
    useUpdateAgentChatMessageResponseMutation();
  const [deleteAgentChatMessageResponse] =
    useDeleteAgentChatMessageResponseMutation();
  ...
  const [formData, setFormData] = useState<Partial<AgentChatMessageResponse>>(
    {},
  );
  ...
  const [copiedRow, setCopiedRow] = useState<AgentChatMessageResponse | null>(
    null,
  );
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAgentChatMessageResponse(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<AgentChatMessageResponse> = { ...source };
      delete (payload as any).id;
      const created = await addAgentChatMessageResponse(payload).unwrap();
      setAllData((prev) => [created as AgentChatMessageResponse, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<AgentChatMessageResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addAgentChatMessageResponse(payload).unwrap();
      setAllData((prev) => [created as AgentChatMessageResponse, ...prev]);
    } catch (error) {
      console.error(
        "Failed to paste (create) AgentChatMessageResponse:",
        error,
      );
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="AgentChatMessageResponseTable"
        ...
        >
          <FloatingControlPanel
            description="AgentChatMessageResponse Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create AgentChatMessageResponse</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <AgentChatMessageResponseForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter AgentChatMessageResponse (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing AgentChatMessageResponse</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof AgentChatMessageResponse;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of AgentChatMessageResponses.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "AgentChatMessageResponse"}
