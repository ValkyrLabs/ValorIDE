...
import { ConsultingProfile } from "@thor/model";
...
const ConsultingProfileTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ConsultingProfile[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<ConsultingProfile>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ConsultingProfile | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteConsultingProfile(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ConsultingProfile> = { ...source };
      delete (payload as any).id;
      const created = await addConsultingProfile(payload).unwrap();
      setAllData((prev) => [created as ConsultingProfile, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ConsultingProfile> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addConsultingProfile(payload).unwrap();
      setAllData((prev) => [created as ConsultingProfile, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="ConsultingProfileTable"
        ...
        >
          <FloatingControlPanel
            description="ConsultingProfile Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create ConsultingProfile</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <ConsultingProfileForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter ConsultingProfile (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing ConsultingProfile</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof ConsultingProfile;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of ConsultingProfiles.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "ConsultingProfile"}
