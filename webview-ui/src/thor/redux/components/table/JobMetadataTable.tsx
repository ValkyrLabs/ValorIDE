...
import { JobMetadata } from "@thor/model";
...
const JobMetadataTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<JobMetadata[]>([]);
  ...
  const [copiedRow, setCopiedRow] = useState<JobMetadata | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteJobMetadata(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<JobMetadata> = { ...source };
      delete (payload as any).id;
      const created = await addJobMetadata(payload).unwrap();
      setAllData((prev) => [created as JobMetadata, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<JobMetadata> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addJobMetadata(payload).unwrap();
      setAllData((prev) => [created as JobMetadata, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <FloatingControlPanel
            description="JobMetadata Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create JobMetadata</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <JobMetadataForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter JobMetadata (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing JobMetadata</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof JobMetadata;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of JobMetadatas.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "JobMetadata"}
