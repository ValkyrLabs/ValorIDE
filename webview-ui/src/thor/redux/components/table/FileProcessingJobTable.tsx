...
import { FileProcessingJob } from '@thor/model';
...
const FileProcessingJobTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<FileProcessingJob[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<FileProcessingJob>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<FileProcessingJob | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteFileProcessingJob(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<FileProcessingJob> = { ...source };
      delete (payload as any).id;
      const created = await addFileProcessingJob(payload).unwrap();
      setAllData((prev) => [created as FileProcessingJob, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<FileProcessingJob> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addFileProcessingJob(payload).unwrap();
      setAllData((prev) => [created as FileProcessingJob, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="FileProcessingJobTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="FileProcessingJob Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create FileProcessingJob</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <FileProcessingJobForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter FileProcessingJob (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing FileProcessingJob</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof FileProcessingJob;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of FileProcessingJobs.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'FileProcessingJob'}
