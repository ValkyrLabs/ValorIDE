...
import { FileAuditLog } from '@thor/model';
...
const FileAuditLogTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<FileAuditLog[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<FileAuditLog>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<FileAuditLog | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteFileAuditLog(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<FileAuditLog> = { ...source };
      delete (payload as any).id;
      const created = await addFileAuditLog(payload).unwrap();
      setAllData((prev) => [created as FileAuditLog, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<FileAuditLog> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addFileAuditLog(payload).unwrap();
      setAllData((prev) => [created as FileAuditLog, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          >
            ...
            <FloatingControlPanel
              description="FileAuditLog Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create FileAuditLog</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <FileAuditLogForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter FileAuditLog (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing FileAuditLog</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof FileAuditLog;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of FileAuditLogs.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'FileAuditLog'}
