...
import { McpToolCallResponse } from '@thor/model';
...
const McpToolCallResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<McpToolCallResponse[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<McpToolCallResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<McpToolCallResponse | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpToolCallResponse(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<McpToolCallResponse> = { ...source };
      delete (payload as any).id;
      const created = await addMcpToolCallResponse(payload).unwrap();
      setAllData((prev) => [created as McpToolCallResponse, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<McpToolCallResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMcpToolCallResponse(payload).unwrap();
      setAllData((prev) => [created as McpToolCallResponse, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="McpToolCallResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="McpToolCallResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create McpToolCallResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <McpToolCallResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter McpToolCallResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing McpToolCallResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof McpToolCallResponse;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of McpToolCallResponses.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'McpToolCallResponse'}
