...
import { PaymentTransaction } from '@thor/model';
...
const PaymentTransactionTable: React.FC = () => {
  ...
  const [copiedRow, setCopiedRow] = useState<PaymentTransaction | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePaymentTransaction(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<PaymentTransaction> = { ...source };
      delete (payload as any).id;
      const created = await addPaymentTransaction(payload).unwrap();
      setAllData((prev) => [created as PaymentTransaction, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<PaymentTransaction> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addPaymentTransaction(payload).unwrap();
      setAllData((prev) => [created as PaymentTransaction, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="PaymentTransactionTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="PaymentTransaction Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create PaymentTransaction</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter PaymentTransaction (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing PaymentTransaction</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof PaymentTransaction;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of PaymentTransactions.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'PaymentTransaction'}
