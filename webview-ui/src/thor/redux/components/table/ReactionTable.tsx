...
import { Reaction } from "@thor/model";
...
import {
  useLazyGetReactionsPagedQuery,
  useAddReactionMutation,
  useUpdateReactionMutation,
  useDeleteReactionMutation,
} from "@thor/redux/services/ReactionService";
...
const ReactionTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Reaction[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteReaction(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Reaction> = { ...source };
      delete (payload as any).id;
      const created = await addReaction(payload).unwrap();
      setAllData((prev) => [created as Reaction, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Reaction> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addReaction(payload).unwrap();
      setAllData((prev) => [created as Reaction, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter Reaction (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof Reaction;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of Reactions.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "Reaction"}
