...
import { ValkyrJob } from '@thor/model';
...
const ValkyrJobTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<ValkyrJob[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<ValkyrJob>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ValkyrJob | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteValkyrJob(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ValkyrJob> = { ...source };
      delete (payload as any).id;
      const created = await addValkyrJob(payload).unwrap();
      setAllData((prev) => [created as ValkyrJob, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ValkyrJob> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addValkyrJob(payload).unwrap();
      setAllData((prev) => [created as ValkyrJob, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ValkyrJob</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ValkyrJobForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ValkyrJob (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ValkyrJob</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ValkyrJob;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ValkyrJobs.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ValkyrJob'}
