...
import { SwarmRegisterResponse } from '@thor/model';
...
const SwarmRegisterResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SwarmRegisterResponse[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<SwarmRegisterResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SwarmRegisterResponse | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmRegisterResponse(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SwarmRegisterResponse> = { ...source };
      delete (payload as any).id;
      const created = await addSwarmRegisterResponse(payload).unwrap();
      setAllData((prev) => [created as SwarmRegisterResponse, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SwarmRegisterResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSwarmRegisterResponse(payload).unwrap();
      setAllData((prev) => [created as SwarmRegisterResponse, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="SwarmRegisterResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="SwarmRegisterResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SwarmRegisterResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SwarmRegisterResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SwarmRegisterResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SwarmRegisterResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SwarmRegisterResponse;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SwarmRegisterResponses.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SwarmRegisterResponse'}
