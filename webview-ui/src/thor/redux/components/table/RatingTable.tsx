...
import {
  useLazyGetRatingsPagedQuery,
  useAddRatingMutation,
  useUpdateRatingMutation,
  useDeleteRatingMutation
} from '@thor/redux/services/RatingService';
...
const RatingTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Rating[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRating(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Rating> = { ...source };
      delete (payload as any).id;
      const created = await addRating(payload).unwrap();
      setAllData((prev) => [created as Rating, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Rating> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRating(payload).unwrap();
      setAllData((prev) => [created as Rating, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create Rating</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RatingForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter Rating (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof Rating;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of Ratings.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'Rating'}
