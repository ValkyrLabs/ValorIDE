...
import {
  useLazyGetPublishRestEndpointRequestsPagedQuery,
...
} from "@thor/redux/services/PublishRestEndpointRequestService";
...
const PublishRestEndpointRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<PublishRestEndpointRequest[]>([]);
  ...
  // -------------------------------------------------------------------
  const [addPublishRestEndpointRequest] =
    useAddPublishRestEndpointRequestMutation();
  const [updatePublishRestEndpointRequest] =
    useUpdatePublishRestEndpointRequestMutation();
  const [deletePublishRestEndpointRequest] =
    useDeletePublishRestEndpointRequestMutation();
  ...
  const [formData, setFormData] = useState<Partial<PublishRestEndpointRequest>>(
    {},
  );
  ...
  const [copiedRow, setCopiedRow] = useState<PublishRestEndpointRequest | null>(
    null,
  );
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePublishRestEndpointRequest(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<PublishRestEndpointRequest> = { ...source };
      delete (payload as any).id;
      const created = await addPublishRestEndpointRequest(payload).unwrap();
      setAllData((prev) => [created as PublishRestEndpointRequest, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<PublishRestEndpointRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addPublishRestEndpointRequest(payload).unwrap();
      setAllData((prev) => [created as PublishRestEndpointRequest, ...prev]);
    } catch (error) {
      console.error(
        "Failed to paste (create) PublishRestEndpointRequest:",
        error,
      );
  ...
  return (
    <>
      <div className="tableContainer">
        <div className="spreadsheet-container">
          <RBGrid
            ...
            storageKey="PublishRestEndpointRequestTable"
        ...
        >
          <FloatingControlPanel
            description="PublishRestEndpointRequest Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create PublishRestEndpointRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <PublishRestEndpointRequestForm />
          </Modal.Body>
        </Modal>
        {/* QBE Modal */}
        <Modal
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              Filter PublishRestEndpointRequest (Query By Example)
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Editing PublishRestEndpointRequest</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof PublishRestEndpointRequest;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of PublishRestEndpointRequests.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "PublishRestEndpointRequest"}
