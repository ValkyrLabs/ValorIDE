...
import { ProductFunnelWizard } from '@thor/model';
...
const ProductFunnelWizardTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ProductFunnelWizard[]>([]);
  ...
  const [formData, setFormData] = useState<Partial<ProductFunnelWizard>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ProductFunnelWizard | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteProductFunnelWizard(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ProductFunnelWizard> = { ...source };
      delete (payload as any).id;
      const created = await addProductFunnelWizard(payload).unwrap();
      setAllData((prev) => [created as ProductFunnelWizard, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ProductFunnelWizard> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addProductFunnelWizard(payload).unwrap();
      setAllData((prev) => [created as ProductFunnelWizard, ...prev]);
    } catch (error) {
  ...
  return (
    <>
        <div className="tableContainer">
          ...
          <div className="spreadsheet-container">
            <RBGrid
              ...
              storageKey="ProductFunnelWizardTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ProductFunnelWizard Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ProductFunnelWizard</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ProductFunnelWizardForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ProductFunnelWizard (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ProductFunnelWizard</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ProductFunnelWizard;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ProductFunnelWizards.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ProductFunnelWizard'}
