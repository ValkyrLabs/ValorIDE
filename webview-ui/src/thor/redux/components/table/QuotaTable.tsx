...
import {
  useLazyGetQuotasPagedQuery,
  useAddQuotaMutation,
  useUpdateQuotaMutation,
  useDeleteQuotaMutation
} from '@thor/redux/services/QuotaService';
...
const QuotaTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Quota[]>([]);
  ...
  const [copiedRow, setCopiedRow] = useState<Quota | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteQuota(id as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Quota> = { ...source };
      delete (payload as any).id;
      const created = await addQuota(payload).unwrap();
      setAllData((prev) => [created as Quota, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Quota> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addQuota(payload).unwrap();
      setAllData((prev) => [created as Quota, ...prev]);
    } catch (error) {
    ...
    <>
        <div className="tableContainer">
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter Quota (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing Quota</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof Quota;
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of Quotas.
            </div>
          )}
        </div>
        {/* Generic QBE Picker */}
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'Quota'}
