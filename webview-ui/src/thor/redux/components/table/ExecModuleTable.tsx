...
import { ExecModule } from "@thor/model";
...
const ExecModuleTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<ExecModule[]>([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecModule(id as any).unwrap();
          } catch (e) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ExecModule> = { ...source };
      delete (payload as any).id;
      const created = await addExecModule(payload).unwrap();
      setAllData((prev) => [created as ExecModule, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ExecModule> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addExecModule(payload).unwrap();
      setAllData((prev) => [created as ExecModule, ...prev]);
    } catch (error) {
  ...
  return (
    <>
      <div className="tableContainer">
        ...
        >
          <FloatingControlPanel
            description="ExecModule Actions"
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Create ExecModule</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <ExecModuleForm />
          </Modal.Body>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>Filter ExecModule (Query By Example)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
        ...
        <MarkdownEditorModal
          ...
          onSave={async (updatedText) => {
            try {
              ...
              const field = richModal.field as keyof ExecModule;
              if (!rid || !field) {
        ...
        {isError && (
          <div style={{ color: "red", marginTop: 12 }}>
            Error fetching page {page} of ExecModules.
          </div>
        )}
      </div>
      {/* Generic QBE Picker */}
      <QBEPicker
        show={qbePicker.show}
        refType={qbePicker.refType || "ExecModule"}
