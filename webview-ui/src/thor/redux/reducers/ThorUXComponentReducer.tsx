...
const ThorUXComponentSlice = createSlice({
	...
	reducers: {
		...
		ThorUXComponentValueToggled(state, action) {
			...
			const ThorUXComponent: ThorUXComponent = state.find(
				(ThorUXComponent) => ThorUXComponent.id === action.payload.ThorUXComponentId,
			)
			if (ThorUXComponent) {
...
export const { ThorUXComponentAdded, ThorUXComponentValueToggled, ThorUXComponentpropertySet } = ThorUXComponentSlice.actions
