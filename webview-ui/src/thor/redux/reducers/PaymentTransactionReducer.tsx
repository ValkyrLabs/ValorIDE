	...
	reducers: {
		...
		PaymentTransactionValueToggled(state, action) {
			...
			const PaymentTransaction: PaymentTransaction = state.find(
				(PaymentTransaction) => PaymentTransaction.id === action.payload.PaymentTransactionId,
			)
...
export const { PaymentTransactionAdded, PaymentTransactionValueToggled, PaymentTransactionpropertySet } =
