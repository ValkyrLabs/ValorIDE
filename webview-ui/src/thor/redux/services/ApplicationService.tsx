...
export const ApplicationService = createApi({
	...
	endpoints: (build) => ({
		...
		getApplicationsPaged: build.query<ApplicationResponse, { page: number; limit?: number }>({
		...
		generateApplication: build.mutation<{ blob: Blob; filename: string }, string>({
			query: (applicationId) => ({
			...
			invalidatesTags: (result, error, applicationId) => [{ type: "Application", id: applicationId }],
		}),
