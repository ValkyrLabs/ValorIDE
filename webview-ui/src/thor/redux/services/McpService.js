// tslint:disable
/**
############################## DO NOT EDIT: GENERATED FILE ##############################

Generated by Valkyr Labs ThorAPI: https://valkyrlabs.com/thorapi
Powered by Swagger Codegen: http://swagger.io

Generated Details:
**GENERATOR VERSION:** 7.5.0
**GENERATED DATE:** 2025-12-07T16:29:11.456024-08:00[America/Los_Angeles]
**GENERATOR CLASS:** org.openapitools.codegen.languages.TypeScriptReduxQueryClientCodegen

Template file: typescript-redux-query/modelService.mustache

############################## DO NOT EDIT: GENERATED FILE ##############################
*/
import { createApi } from "@reduxjs/toolkit/query/react";
import customBaseQuery from "../customBaseQuery"; // Import the custom base query
export const McpService = createApi({
    reducerPath: "Mcp", // This should remain unique
    baseQuery: customBaseQuery,
    tagTypes: ["Mcp"],
    endpoints: (build) => ({
        // 1) Paged Query Endpoint
        // Standardized pagination: page (0-based), size (page size)
        getMcpsPaged: build.query({
            query: ({ page, size = 20, example }) => {
                const q = [`page=${page}`, `size=${size}`];
                if (example)
                    q.push(`example=${encodeURIComponent(JSON.stringify(example))}`);
                return `Mcp?${q.join("&")}`;
            },
            providesTags: (result, error, { page }) => result
                ? [
                    ...result.map(({ id }) => ({ type: "Mcp", id })),
                    { type: "Mcp", id: `PAGE_${page}` },
                ]
                : [],
        }),
        // 2) Simple "get all" Query (optional)
        getMcps: build.query({
            query: (arg) => {
                if (arg && arg.example) {
                    const ex = arg.example;
                    return `Mcp?example=${encodeURIComponent(JSON.stringify(ex))}`;
                }
                return `Mcp`;
            },
            providesTags: (result) => result
                ? [
                    ...result.map(({ id }) => ({ type: "Mcp", id })),
                    { type: "Mcp", id: "LIST" },
                ]
                : [{ type: "Mcp", id: "LIST" }],
        }),
        // 3) Create
        addMcp: build.mutation({
            query: (body) => ({
                url: `Mcp`,
                method: "POST",
                body,
            }),
            invalidatesTags: [{ type: "Mcp", id: "LIST" }],
        }),
        // 4) Get single by ID
        getMcp: build.query({
            query: (id) => `Mcp/${id}`,
            providesTags: (result, error, id) => [{ type: "Mcp", id }],
        }),
        // 5) Update
        updateMcp: build.mutation({
            query: ({ id, ...patch }) => ({
                url: `Mcp/${id}`,
                method: "PUT",
                body: patch,
            }),
            async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
                if (id) {
                    const patchResult = dispatch(McpService.util.updateQueryData("getMcp", id, (draft) => {
                        Object.assign(draft, patch);
                    }));
                    try {
                        await queryFulfilled;
                    }
                    catch {
                        patchResult.undo();
                    }
                }
            },
            invalidatesTags: (result, error, { id }) => [
                { type: "Mcp", id },
                { type: "Mcp", id: "LIST" },
            ],
        }),
        // 6) Delete
        deleteMcp: build.mutation({
            query(id) {
                return {
                    url: `Mcp/${id}`,
                    method: "DELETE",
                };
            },
            invalidatesTags: (result, error, id) => [{ type: "Mcp", id }],
        }),
        // 7) Cascade / soft-delete (marks trashed, cascades children)
        deleteMcpCascade: build.mutation({
            query({ id, cascade = true, trash = true }) {
                const params = [`cascade=${cascade}`, `trash=${trash}`].join("&");
                return {
                    url: `Mcp/${id}?${params}`,
                    method: "DELETE",
                };
            },
            invalidatesTags: (result, error, { id }) => [
                { type: "Mcp", id },
                { type: "Mcp", id: "LIST" },
            ],
        }),
    }),
});
// Notice we now also export `useLazyGetMcpsPagedQuery`
export const { useGetMcpsPagedQuery, // immediate fetch
useLazyGetMcpsPagedQuery, // lazy fetch
useGetMcpQuery, useGetMcpsQuery, useAddMcpMutation, useUpdateMcpMutation, useDeleteMcpMutation, useDeleteMcpCascadeMutation, } = McpService;
//# sourceMappingURL=McpService.js.map