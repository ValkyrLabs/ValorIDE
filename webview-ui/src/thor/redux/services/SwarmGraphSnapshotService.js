...
export const SwarmGraphSnapshotService = createApi({
    ...
    endpoints: (build) => ({
        ...
        getSwarmGraphSnapshotsPaged: build.query({
            ...
            providesTags: (result, error, { page }) => result
                ? [
                    ...result.map(({ id }) => ({ type: 'SwarmGraphSnapshot', id })),
                    { type: 'SwarmGraphSnapshot', id: `PAGE_${page}` },
                ]
                : [],
        }),
        // 2) Simple "get all" Query (optional)
        getSwarmGraphSnapshots: build.query({
            query: (arg) => {
                if (arg && arg.example) {
        ...
        // 3) Create
        addSwarmGraphSnapshot: build.mutation({
            query: (body) => ({
        ...
        updateSwarmGraphSnapshot: build.mutation({
            ...
            async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
                if (id) {
                    const patchResult = dispatch(SwarmGraphSnapshotService.util.updateQueryData('getSwarmGraphSnapshot', id, (draft) => {
        ...
        // 6) Delete
        deleteSwarmGraphSnapshot: build.mutation({
            query(id) {
                return {
                    url: `SwarmGraphSnapshot/${id}`,
                    method: 'DELETE',
...
export const { useGetSwarmGraphSnapshotsPagedQuery, // immediate fetch
useLazyGetSwarmGraphSnapshotsPagedQuery, // lazy fetch
useGetSwarmGraphSnapshotQuery, useGetSwarmGraphSnapshotsQuery, useAddSwarmGraphSnapshotMutation, useUpdateSwarmGraphSnapshotMutation, useDeleteSwarmGraphSnapshotMutation, } = SwarmGraphSnapshotService;
//# sourceMappingURL=SwarmGraphSnapshotService.js.map