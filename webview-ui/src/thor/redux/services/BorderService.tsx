...
export const BorderService = createApi({
	...
	endpoints: (build) => ({
		...
		getBordersPaged: build.query<BorderResponse, { page: number; limit?: number }>({
			query: ({ page, limit = 20 }) => `Border?page=${page}&limit=${limit}`,
			providesTags: (result, error, { page }) =>
				result
					? [...result.map(({ id }) => ({ type: "Border" as const, id })), { type: "Border", id: `PAGE_${page}` }]
					: [],
		}),
		...
		getBorders: build.query<BorderResponse, void>({
			...
			providesTags: (result) =>
				result
					? [...result.map(({ id }) => ({ type: "Border" as const, id })), { type: "Border", id: "LIST" }]
					: [{ type: "Border", id: "LIST" }],
		}),
