...
export const InvoiceService = createApi({
	...
	endpoints: (build) => ({
		...
		getInvoicesPaged: build.query<InvoiceResponse, { page: number; limit?: number }>({
			query: ({ page, limit = 20 }) => `Invoice?page=${page}&limit=${limit}`,
			providesTags: (result, error, { page }) =>
				result
					? [...result.map(({ id }) => ({ type: "Invoice" as const, id })), { type: "Invoice", id: `PAGE_${page}` }]
					: [],
		}),
		...
		getInvoices: build.query<InvoiceResponse, void>({
			...
			providesTags: (result) =>
				result
					? [...result.map(({ id }) => ({ type: "Invoice" as const, id })), { type: "Invoice", id: "LIST" }]
