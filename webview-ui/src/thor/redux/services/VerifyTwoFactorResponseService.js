// tslint:disable
/**
############################## DO NOT EDIT: GENERATED FILE ##############################

Generated by Valkyr Labs ThorAPI: https://valkyrlabs.com/thorapi
Powered by Swagger Codegen: http://swagger.io

Generated Details:
**GENERATOR VERSION:** 7.5.0
**GENERATED DATE:** 2025-12-07T16:29:11.456024-08:00[America/Los_Angeles]
**GENERATOR CLASS:** org.openapitools.codegen.languages.TypeScriptReduxQueryClientCodegen

Template file: typescript-redux-query/modelService.mustache

############################## DO NOT EDIT: GENERATED FILE ##############################
*/
import { createApi } from "@reduxjs/toolkit/query/react";
import customBaseQuery from "../customBaseQuery"; // Import the custom base query
export const VerifyTwoFactorResponseService = createApi({
    reducerPath: "VerifyTwoFactorResponse", // This should remain unique
    baseQuery: customBaseQuery,
    tagTypes: ["VerifyTwoFactorResponse"],
    endpoints: (build) => ({
        // 1) Paged Query Endpoint
        // Standardized pagination: page (0-based), size (page size)
        getVerifyTwoFactorResponsesPaged: build.query({
            query: ({ page, size = 20, example }) => {
                const q = [`page=${page}`, `size=${size}`];
                if (example)
                    q.push(`example=${encodeURIComponent(JSON.stringify(example))}`);
                return `VerifyTwoFactorResponse?${q.join("&")}`;
            },
            providesTags: (result, error, { page }) => result
                ? [
                    ...result.map(({ id }) => ({
                        type: "VerifyTwoFactorResponse",
                        id,
                    })),
                    { type: "VerifyTwoFactorResponse", id: `PAGE_${page}` },
                ]
                : [],
        }),
        // 2) Simple "get all" Query (optional)
        getVerifyTwoFactorResponses: build.query({
            query: (arg) => {
                if (arg && arg.example) {
                    const ex = arg.example;
                    return `VerifyTwoFactorResponse?example=${encodeURIComponent(JSON.stringify(ex))}`;
                }
                return `VerifyTwoFactorResponse`;
            },
            providesTags: (result) => result
                ? [
                    ...result.map(({ id }) => ({
                        type: "VerifyTwoFactorResponse",
                        id,
                    })),
                    { type: "VerifyTwoFactorResponse", id: "LIST" },
                ]
                : [{ type: "VerifyTwoFactorResponse", id: "LIST" }],
        }),
        // 3) Create
        addVerifyTwoFactorResponse: build.mutation({
            query: (body) => ({
                url: `VerifyTwoFactorResponse`,
                method: "POST",
                body,
            }),
            invalidatesTags: [{ type: "VerifyTwoFactorResponse", id: "LIST" }],
        }),
        // 4) Get single by ID
        getVerifyTwoFactorResponse: build.query({
            query: (id) => `VerifyTwoFactorResponse/${id}`,
            providesTags: (result, error, id) => [
                { type: "VerifyTwoFactorResponse", id },
            ],
        }),
        // 5) Update
        updateVerifyTwoFactorResponse: build.mutation({
            query: ({ id, ...patch }) => ({
                url: `VerifyTwoFactorResponse/${id}`,
                method: "PUT",
                body: patch,
            }),
            async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
                if (id) {
                    const patchResult = dispatch(VerifyTwoFactorResponseService.util.updateQueryData("getVerifyTwoFactorResponse", id, (draft) => {
                        Object.assign(draft, patch);
                    }));
                    try {
                        await queryFulfilled;
                    }
                    catch {
                        patchResult.undo();
                    }
                }
            },
            invalidatesTags: (result, error, { id }) => [
                { type: "VerifyTwoFactorResponse", id },
                { type: "VerifyTwoFactorResponse", id: "LIST" },
            ],
        }),
        // 6) Delete
        deleteVerifyTwoFactorResponse: build.mutation({
            query(id) {
                return {
                    url: `VerifyTwoFactorResponse/${id}`,
                    method: "DELETE",
                };
            },
            invalidatesTags: (result, error, id) => [
                { type: "VerifyTwoFactorResponse", id },
            ],
        }),
        // 7) Cascade / soft-delete (marks trashed, cascades children)
        deleteVerifyTwoFactorResponseCascade: build.mutation({
            query({ id, cascade = true, trash = true }) {
                const params = [`cascade=${cascade}`, `trash=${trash}`].join("&");
                return {
                    url: `VerifyTwoFactorResponse/${id}?${params}`,
                    method: "DELETE",
                };
            },
            invalidatesTags: (result, error, { id }) => [
                { type: "VerifyTwoFactorResponse", id },
                { type: "VerifyTwoFactorResponse", id: "LIST" },
            ],
        }),
    }),
});
// Notice we now also export `useLazyGetVerifyTwoFactorResponsesPagedQuery`
export const { useGetVerifyTwoFactorResponsesPagedQuery, // immediate fetch
useLazyGetVerifyTwoFactorResponsesPagedQuery, // lazy fetch
useGetVerifyTwoFactorResponseQuery, useGetVerifyTwoFactorResponsesQuery, useAddVerifyTwoFactorResponseMutation, useUpdateVerifyTwoFactorResponseMutation, useDeleteVerifyTwoFactorResponseMutation, useDeleteVerifyTwoFactorResponseCascadeMutation, } = VerifyTwoFactorResponseService;
//# sourceMappingURL=VerifyTwoFactorResponseService.js.map