...
export const WizardStatusResponseService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getWizardStatusResponsesPaged: build.query<
      WizardStatusResponseResponse,
      { page: number; size?: number; example?: Partial<WizardStatusResponse> }
    >({
      ...
      providesTags: (result, error, { page }) =>
        result
          ? [
              ...result.map(({ id }) => ({
                type: "WizardStatusResponse" as const,
                id,
              })),
    ...
    getWizardStatusResponses: build.query<
      WizardStatusResponseResponse,
      { example?: Partial<WizardStatusResponse> } | void
    >({
      query: (arg) => {
    ...
    updateWizardStatusResponse: build.mutation<
      void,
      Pick<WizardStatusResponse, "id"> & Partial<WizardStatusResponse>
    >({
      query: ({ id, ...patch }) => ({
      ...
      invalidatesTags: (
        result,
        error,
        { id }: Pick<WizardStatusResponse, "id">,
      ) => [
        { type: "WizardStatusResponse", id },
        { type: "WizardStatusResponse", id: "LIST" },
      ],
