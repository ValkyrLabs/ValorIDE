...
export const FileVersionService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getFileVersionsPaged: build.query<
      FileVersionResponse,
      { page: number; size?: number; example?: Partial<FileVersion> }
    >({
    ...
    getFileVersions: build.query<
      FileVersionResponse,
      { example?: Partial<FileVersion> } | void
    >({
      query: (arg) => {
    ...
    updateFileVersion: build.mutation<
      void,
      Pick<FileVersion, "id"> & Partial<FileVersion>
    >({
      query: ({ id, ...patch }) => ({
        url: `FileVersion/${id}`,
        method: "PUT",
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            FileVersionService.util.updateQueryData(
              "getFileVersion",
              id,
              (draft) => {
      ...
      invalidatesTags: (result, error, { id }: Pick<FileVersion, "id">) => [
        { type: "FileVersion", id },
        { type: "FileVersion", id: "LIST" },
      ],
    }),
