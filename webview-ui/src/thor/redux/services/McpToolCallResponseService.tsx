...
export const McpToolCallResponseService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getMcpToolCallResponsesPaged: build.query<
      McpToolCallResponseResponse,
      { page: number; size?: number; example?: Partial<McpToolCallResponse> }
    >({
      ...
      providesTags: (result, error, { page }) =>
        result
          ? [
              ...result.map(({ id }) => ({
                type: "McpToolCallResponse" as const,
                id,
              })),
    ...
    getMcpToolCallResponses: build.query<
      McpToolCallResponseResponse,
      { example?: Partial<McpToolCallResponse> } | void
    >({
      query: (arg) => {
    ...
    updateMcpToolCallResponse: build.mutation<
      void,
      Pick<McpToolCallResponse, "id"> & Partial<McpToolCallResponse>
    >({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            McpToolCallResponseService.util.updateQueryData(
      ...
      invalidatesTags: (
        result,
        error,
        { id }: Pick<McpToolCallResponse, "id">,
      ) => [
        { type: "McpToolCallResponse", id },
        { type: "McpToolCallResponse", id: "LIST" },
      ],
