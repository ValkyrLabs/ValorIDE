...
export const SwarmGraphSnapshotService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getSwarmGraphSnapshotsPaged: build.query<
      SwarmGraphSnapshotResponse,
      { page: number; size?: number; example?: Partial<SwarmGraphSnapshot> }
    >({
      ...
      providesTags: (result, error, { page }) =>
        result
          ? [
              ...result.map(({ id }) => ({
                type: "SwarmGraphSnapshot" as const,
                id,
              })),
    ...
    getSwarmGraphSnapshots: build.query<
      SwarmGraphSnapshotResponse,
      { example?: Partial<SwarmGraphSnapshot> } | void
    >({
      query: (arg) => {
    ...
    updateSwarmGraphSnapshot: build.mutation<
      void,
      Pick<SwarmGraphSnapshot, "id"> & Partial<SwarmGraphSnapshot>
    >({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            SwarmGraphSnapshotService.util.updateQueryData(
              "getSwarmGraphSnapshot",
              id,
              (draft) => {
