    ...
    endpoints: (build) => ({
        ...
        getChatResponsesPaged: build.query({
            ...
            providesTags: (result, error, { page }) => result
                ? [
                    ...result.map(({ id }) => ({ type: 'ChatResponse', id })),
        ...
        // 3) Create
        addChatResponse: build.mutation({
            query: (body) => ({
                url: `ChatResponse`,
                method: 'POST',
                body,
        ...
        updateChatResponse: build.mutation({
            ...
            async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
                if (id) {
                    const patchResult = dispatch(ChatResponseService.util.updateQueryData('getChatResponse', id, (draft) => {
...
export const { useGetChatResponsesPagedQuery, // immediate fetch
useLazyGetChatResponsesPagedQuery, // lazy fetch
useGetChatResponseQuery, useGetChatResponsesQuery, useAddChatResponseMutation, useUpdateChatResponseMutation, useDeleteChatResponseMutation, } = ChatResponseService;
//# sourceMappingURL=ChatResponseService.js.map