  ...
  endpoints: (build) => ({
    ...
    getProductFeaturesPaged: build.query<ProductFeatureResponse, { page: number; size?: number; example?: Partial<ProductFeature> }>({
      ...
      providesTags: (result, error, { page }) =>
        result
          ? [
              ...result.map(({ id }) => ({ type: 'ProductFeature' as const, id })),
    ...
    // 5) Update
    updateProductFeature: build.mutation<void, Pick<ProductFeature, 'id'> & Partial<ProductFeature>>({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            ProductFeatureService.util.updateQueryData('getProductFeature', id, (draft) => {
