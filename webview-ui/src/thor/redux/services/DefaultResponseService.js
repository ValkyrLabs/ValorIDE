    ...
    endpoints: (build) => ({
        ...
        getDefaultResponsesPaged: build.query({
            ...
            providesTags: (result, error, { page }) => result
                ? [
                    ...result.map(({ id }) => ({ type: 'DefaultResponse', id })),
        ...
        // 4) Get single by ID
        getDefaultResponse: build.query({
        ...
        updateDefaultResponse: build.mutation({
            ...
            async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
                if (id) {
                    const patchResult = dispatch(DefaultResponseService.util.updateQueryData('getDefaultResponse', id, (draft) => {
...
export const { useGetDefaultResponsesPagedQuery, // immediate fetch
useLazyGetDefaultResponsesPagedQuery, // lazy fetch
useGetDefaultResponseQuery, useGetDefaultResponsesQuery, useAddDefaultResponseMutation, useUpdateDefaultResponseMutation, useDeleteDefaultResponseMutation, } = DefaultResponseService;
//# sourceMappingURL=DefaultResponseService.js.map