...
export const RequeueDeadLetterEntryResponseService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getRequeueDeadLetterEntryResponsesPaged: build.query<
      RequeueDeadLetterEntryResponseResponse,
      {
        page: number;
        size?: number;
        example?: Partial<RequeueDeadLetterEntryResponse>;
      }
    >({
      ...
      providesTags: (result, error, { page }) =>
        result
          ? [
              ...result.map(({ id }) => ({
                type: "RequeueDeadLetterEntryResponse" as const,
                id,
              })),
    ...
    getRequeueDeadLetterEntryResponses: build.query<
      RequeueDeadLetterEntryResponseResponse,
      { example?: Partial<RequeueDeadLetterEntryResponse> } | void
    >({
      query: (arg) => {
    ...
    updateRequeueDeadLetterEntryResponse: build.mutation<
      void,
      Pick<RequeueDeadLetterEntryResponse, "id"> &
        Partial<RequeueDeadLetterEntryResponse>
    >({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            RequeueDeadLetterEntryResponseService.util.updateQueryData(
              "getRequeueDeadLetterEntryResponse",
              id,
              (draft) => {
      ...
      invalidatesTags: (
        result,
        error,
        { id }: Pick<RequeueDeadLetterEntryResponse, "id">,
      ) => [
        { type: "RequeueDeadLetterEntryResponse", id },
        { type: "RequeueDeadLetterEntryResponse", id: "LIST" },
      ],
