...
export const FormatService = createApi({
    ...
    endpoints: (build) => ({
        ...
        // 4) Get single by ID
        getFormat: build.query({
            query: (id) => `Format/${id}`,
            providesTags: (result, error, id) => [{ type: 'Format', id }],
        }),
        // 5) Update
        updateFormat: build.mutation({
            query: ({ id, ...patch }) => ({
                url: `Format/${id}`,
                method: 'PUT',
                body: patch,
            ...
            async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
                if (id) {
                    const patchResult = dispatch(FormatService.util.updateQueryData('getFormat', id, (draft) => {
...
export const { useGetFormatsPagedQuery, // immediate fetch
useLazyGetFormatsPagedQuery, // lazy fetch
useGetFormatQuery, useGetFormatsQuery, useAddFormatMutation, useUpdateFormatMutation, useDeleteFormatMutation, } = FormatService;
