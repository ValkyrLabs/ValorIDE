...
export const ProductLandingPageService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getProductLandingPagesPaged: build.query<
      ...
      { page: number; size?: number; example?: Partial<ProductLandingPage> }
    >({
      ...
      providesTags: (result, error, { page }) =>
        result
          ? [
              ...result.map(({ id }) => ({
                type: "ProductLandingPage" as const,
                id,
              })),
    ...
    getProductLandingPages: build.query<
      ProductLandingPageResponse,
      { example?: Partial<ProductLandingPage> } | void
    >({
      query: (arg) => {
    ...
    updateProductLandingPage: build.mutation<
      void,
      Pick<ProductLandingPage, "id"> & Partial<ProductLandingPage>
    >({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            ProductLandingPageService.util.updateQueryData(
