...
export const GoalDependencyService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getGoalDependencysPaged: build.query<
      ...
      { page: number; size?: number; example?: Partial<GoalDependency> }
    >({
    ...
    getGoalDependencys: build.query<
      GoalDependencyResponse,
      { example?: Partial<GoalDependency> } | void
    >({
      query: (arg) => {
    ...
    updateGoalDependency: build.mutation<
      void,
      Pick<GoalDependency, "id"> & Partial<GoalDependency>
    >({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            GoalDependencyService.util.updateQueryData(
              "getGoalDependency",
              id,
              (draft) => {
      ...
      invalidatesTags: (result, error, { id }: Pick<GoalDependency, "id">) => [
