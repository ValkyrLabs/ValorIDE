...
import { ThorUXMeta } from '@thor/model/ThorUXMeta'
...
export const ThorUXMetaService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getThorUXMetasPaged: build.query<ThorUXMetaResponse, { page: number; size?: number; example?: Partial<ThorUXMeta> }>({
      query: ({ page, size = 20, example }) => {
        const q: string[] = [`page=${page}`, `size=${size}`];
        if (example) q.push(`example=${encodeURIComponent(JSON.stringify(example))}`);
        return `ThorUXMeta?${q.join('&')}`;
      },
    ...
    getThorUXMetas: build.query<ThorUXMetaResponse, { example?: Partial<ThorUXMeta> } | void>({
      query: (arg) => {
        if (arg && (arg as any).example) {
          const ex = (arg as any).example;
          return `ThorUXMeta?example=${encodeURIComponent(JSON.stringify(ex))}`;
        }
        return `ThorUXMeta`;
      },
      providesTags: (result) =>
