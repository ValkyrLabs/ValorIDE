...
export const SwarmUnregisterResponseService = createApi({
    ...
    endpoints: (build) => ({
        ...
        getSwarmUnregisterResponsesPaged: build.query({
            ...
            providesTags: (result, error, { page }) => result
                ? [
                    ...result.map(({ id }) => ({ type: 'SwarmUnregisterResponse', id })),
                    { type: 'SwarmUnregisterResponse', id: `PAGE_${page}` },
                ]
                : [],
        }),
        // 2) Simple "get all" Query (optional)
        getSwarmUnregisterResponses: build.query({
            query: (arg) => {
                if (arg && arg.example) {
        ...
        // 3) Create
        addSwarmUnregisterResponse: build.mutation({
            query: (body) => ({
        ...
        updateSwarmUnregisterResponse: build.mutation({
            ...
            async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
                if (id) {
                    const patchResult = dispatch(SwarmUnregisterResponseService.util.updateQueryData('getSwarmUnregisterResponse', id, (draft) => {
        ...
        // 6) Delete
        deleteSwarmUnregisterResponse: build.mutation({
            query(id) {
                return {
                    url: `SwarmUnregisterResponse/${id}`,
                    method: 'DELETE',
...
export const { useGetSwarmUnregisterResponsesPagedQuery, // immediate fetch
useLazyGetSwarmUnregisterResponsesPagedQuery, // lazy fetch
useGetSwarmUnregisterResponseQuery, useGetSwarmUnregisterResponsesQuery, useAddSwarmUnregisterResponseMutation, useUpdateSwarmUnregisterResponseMutation, useDeleteSwarmUnregisterResponseMutation, } = SwarmUnregisterResponseService;
//# sourceMappingURL=SwarmUnregisterResponseService.js.map