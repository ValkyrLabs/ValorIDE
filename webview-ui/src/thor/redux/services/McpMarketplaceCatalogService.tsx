...
export const McpMarketplaceCatalogService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getMcpMarketplaceCatalogsPaged: build.query<McpMarketplaceCatalogResponse, { page: number; size?: number; example?: Partial<McpMarketplaceCatalog> }>({
      ...
      providesTags: (result, error, { page }) =>
        result
          ? [
              ...result.map(({ id }) => ({ type: 'McpMarketplaceCatalog' as const, id })),
    ...
    getMcpMarketplaceCatalogs: build.query<McpMarketplaceCatalogResponse, { example?: Partial<McpMarketplaceCatalog> } | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateMcpMarketplaceCatalog: build.mutation<void, Pick<McpMarketplaceCatalog, 'id'> & Partial<McpMarketplaceCatalog>>({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            McpMarketplaceCatalogService.util.updateQueryData('getMcpMarketplaceCatalog', id, (draft) => {
