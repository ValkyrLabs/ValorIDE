...
export const FormatService = createApi({
	...
	endpoints: (build) => ({
		...
		getFormatsPaged: build.query<FormatResponse, { page: number; limit?: number }>({
			query: ({ page, limit = 20 }) => `Format?page=${page}&limit=${limit}`,
			providesTags: (result, error, { page }) =>
				result
					? [...result.map(({ id }) => ({ type: "Format" as const, id })), { type: "Format", id: `PAGE_${page}` }]
					: [],
		}),
		...
		getFormats: build.query<FormatResponse, void>({
			...
			providesTags: (result) =>
				result
					? [...result.map(({ id }) => ({ type: "Format" as const, id })), { type: "Format", id: "LIST" }]
					: [{ type: "Format", id: "LIST" }],
		}),
