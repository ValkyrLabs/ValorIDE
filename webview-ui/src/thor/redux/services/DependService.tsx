...
export const DependService = createApi({
	...
	endpoints: (build) => ({
		...
		getDependsPaged: build.query<DependResponse, { page: number; limit?: number }>({
			query: ({ page, limit = 20 }) => `Depend?page=${page}&limit=${limit}`,
			providesTags: (result, error, { page }) =>
				result
					? [...result.map(({ id }) => ({ type: "Depend" as const, id })), { type: "Depend", id: `PAGE_${page}` }]
					: [],
		}),
		...
		getDepends: build.query<DependResponse, void>({
			...
			providesTags: (result) =>
				result
					? [...result.map(({ id }) => ({ type: "Depend" as const, id })), { type: "Depend", id: "LIST" }]
					: [{ type: "Depend", id: "LIST" }],
		}),
