...
export const SalesActivityService = createApi({
	...
	endpoints: (build) => ({
		...
		getSalesActivitysPaged: build.query<SalesActivityResponse, { page: number; limit?: number }>({
		...
		getSalesActivitys: build.query<SalesActivityResponse, void>({
			...
			providesTags: (result) =>
				result
					? [...result.map(({ id }) => ({ type: "SalesActivity" as const, id })), { type: "SalesActivity", id: "LIST" }]
