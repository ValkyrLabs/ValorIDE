...
function getMcpServiceRaw(requestParameters, requestConfig = {}) {
    ...
    if (requestTransform) {
        config.transform = (body, text) => requestTransform(McpServiceRegistryFromJSON(body), text);
    }
    return config;
}
/**
...
function invokeMcpToolRaw(requestParameters, requestConfig = {}) {
    ...
    if (requestParameters.invokeMcpToolRequest === null ||
...
function listMcpServicesRaw(requestParameters, requestConfig = {}) {
    ...
    const { transform: requestTransform } = requestConfig;
    if (requestTransform) {
        config.transform = (body, text) => requestTransform(body.map(McpServiceRegistryFromJSON), text);
    }
    return config;
}
/**
...
function publishRestEndpointAsMcpRaw(requestParameters, requestConfig = {}) {
    if (requestParameters.publishRestEndpointRequest === null ||
    ...
    const config = {
        ...
        body: queryParameters ||
            PublishRestEndpointRequestToJSON(requestParameters.publishRestEndpointRequest),
    };
    const { transform: requestTransform } = requestConfig;
    if (requestTransform) {
        config.transform = (body, text) => requestTransform(McpServiceResponseFromJSON(body), text);
    }
    return config;
}
/**
...
function publishWorkflowAsMcpRaw(requestParameters, requestConfig = {}) {
    if (requestParameters.publishWorkflowRequest === null ||
...
function updateMcpServiceRaw(requestParameters, requestConfig = {}) {
    ...
    if (requestParameters.mcpServiceRegistry === null ||
