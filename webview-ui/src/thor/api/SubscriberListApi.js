...
function getSubscriberListRaw(requestParameters, requestConfig = {}) {
    ...
    const { transform: requestTransform } = requestConfig;
    if (requestTransform) {
        config.transform = (body, text) => requestTransform(SubscriberListFromJSON(body), text);
    }
    return config;
}
/**
...
function getSubscriberListListRaw(requestParameters, requestConfig = {}) {
    ...
    const { transform: requestTransform } = requestConfig;
    if (requestTransform) {
        config.transform = (body, text) => requestTransform(body.map(SubscriberListFromJSON), text);
    }
    return config;
}
/**
...
function postSubscriberListRaw(requestParameters, requestConfig = {}) {
    if (requestParameters.subscriberList === null ||
