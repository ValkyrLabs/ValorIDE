...
import { AgentChatMessageRequestToJSON, AgentChatMessageResponseFromJSON, SwarmCommandRequestToJSON, SwarmCommandResponseFromJSON, SwarmRegisterRequestToJSON, SwarmRegisterResponseFromJSON, SwarmUnregisterRequestToJSON, SwarmUnregisterResponseFromJSON, } from "../model";
/**
...
function forwardSwarmCommandRaw(requestParameters, requestConfig = {}) {
    if (requestParameters.swarmCommandRequest === null ||
...
function registerSwarmAgentRaw(requestParameters, requestConfig = {}) {
    if (requestParameters.swarmRegisterRequest === null ||
 ...
 */
function sendAgentChatMessageRaw(requestParameters, requestConfig = {}) {
    ...
    if (requestParameters.agentChatMessageRequest === null ||
    ...
    const { meta = {} } = requestConfig;
    const config = {
        url: `${runtime.Configuration.basePath}/SwarmOps/agent/{agentId}/chat`.replace(`{${"agentId"}}`, encodeURIComponent(String(requestParameters.agentId))),
        meta,
        update: requestConfig.update,
        queryKey: requestConfig.queryKey,
        optimisticUpdate: requestConfig.optimisticUpdate,
    ...
    const { transform: requestTransform } = requestConfig;
    if (requestTransform) {
        config.transform = (body, text) => requestTransform(AgentChatMessageResponseFromJSON(body), text);
    }
    return config;
}
/**
 ...
 * Send chat message to agent
 */
export function sendAgentChatMessage(requestParameters, requestConfig) {
...
function unregisterSwarmAgentRaw(requestParameters, requestConfig = {}) {
    if (requestParameters.swarmUnregisterRequest === null ||
