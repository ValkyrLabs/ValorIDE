...
export class UserPreferenceService {
    ...
    async loadUserPreferences() {
        try {
            ...
            const result = await store.dispatch(UserPreferenceApi.endpoints.getUserPreferences.initiate());
            if (result.data) {
    ...
    async setThemePreference(mode) {
        try {
            ...
            const existingPref = this.preferences.get(UserPreferencePreferenceTypeEnum.UXTHEME);
            ...
            if (existingPref?.id) {
                ...
                result = await store.dispatch(UserPreferenceApi.endpoints.updateUserPreference.initiate({
                    id: existingPref.id,
                    ...preferenceData,
                }));
            }
                ...
                result = await store.dispatch(UserPreferenceApi.endpoints.addUserPreference.initiate(preferenceData));
            }
            // Update local preferences map with the saved preference
            if (result.data) {
                this.preferences.set(UserPreferencePreferenceTypeEnum.UXTHEME, result.data);
            }
        }
        catch (error) {
    ...
    async setLayoutPreference(layout) {
        try {
            ...
            // Save to backend using RTK Query
            const existingPref = this.preferences.get(UserPreferencePreferenceTypeEnum.UXLAYOUT);
            ...
            // Update local preferences map with the saved preference
            if (result.data) {
                this.preferences.set(UserPreferencePreferenceTypeEnum.UXLAYOUT, result.data);
            }
        }
        catch (error) {
    ...
    getThemePreference() {
        ...
        const backendPref = this.preferences.get(UserPreferencePreferenceTypeEnum.UXTHEME);
    ...
    getLayoutPreference() {
        // First try to get from loaded preferences
        const backendPref = this.preferences.get(UserPreferencePreferenceTypeEnum.UXLAYOUT);
        if (backendPref?.preference) {
    ...
    async setGridPrefsKey(storageKey, patch) {
        try {
            let existingPref = this.preferences.get(UserPreferencePreferenceTypeEnum.UXLAYOUT);
            const principalId = (() => {
                try {
                    const raw = sessionStorage.getItem("authenticatedPrincipal") || "{}";
                    const parsed = JSON.parse(raw);
            ...
            if (!existingPref) {
                const res = await store.dispatch(UserPreferenceApi.endpoints.getUserPreferences.initiate());
                const items = res.data;
                if (items && items.length) {
                    const found = items.find((p) => p.preferenceType === UserPreferencePreferenceTypeEnum.UXLAYOUT &&
                        (!principalId || p.principalId === principalId));
                    if (found) {
                        existingPref = found;
                        this.preferences.set(UserPreferencePreferenceTypeEnum.UXLAYOUT, found);
                    }
                }
            }
