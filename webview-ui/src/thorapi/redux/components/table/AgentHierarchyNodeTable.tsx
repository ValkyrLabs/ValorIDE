...
import AgentHierarchyNodeForm from '@thorapi/redux/components/form/AgentHierarchyNodeForm';
...
} from '@thorapi/redux/services/AgentHierarchyNodeService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AgentHierarchyNode update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AgentHierarchyNode/${rowId}`;
...
const AgentHierarchyNodeTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): AgentHierarchyNode[] => {
    if (Array.isArray(result)) return result as AgentHierarchyNode[];
    ...
    return Array.isArray(candidate) ? (candidate as AgentHierarchyNode[]) : [];
  };

  const mergeFetchedRows = (incomingRows: AgentHierarchyNode[], requestedPage: number) => {
  ...
  const [deleteAgentHierarchyNodeCascade] = useDeleteAgentHierarchyNodeCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<AgentHierarchyNode | null> => {
    ...
    try {
      ...
      return (await response.json()) as AgentHierarchyNode;
    } catch (err) {
      console.warn('Could not reload AgentHierarchyNode row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AgentHierarchyNode>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAgentHierarchyNode(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AgentHierarchyNode[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AgentHierarchyNode';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAgentHierarchyNodeCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AgentHierarchyNode-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAgentHierarchyNode(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAgentHierarchyNode(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AgentHierarchyNode[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AgentHierarchyNode[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AgentHierarchyNode[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AgentHierarchyNode[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter AgentHierarchyNode (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
