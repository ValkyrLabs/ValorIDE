...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/MemoryCostStats/${rowId}`;
...
const MemoryCostStatsTable: React.FC = () => {
  ...
  const [deleteMemoryCostStatsCascade] =
    useDeleteMemoryCostStatsCascadeMutation();
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update MemoryCostStats";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMemoryCostStatsCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
