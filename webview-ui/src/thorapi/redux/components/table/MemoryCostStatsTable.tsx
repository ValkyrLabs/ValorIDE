...
import MemoryCostStatsForm from '@thorapi/redux/components/form/MemoryCostStatsForm';
...
import {
  useLazyGetMemoryCostStatssPagedQuery,
...
} from '@thorapi/redux/services/MemoryCostStatsService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'retrievalCost': { type: 'number' },
  'writesCost': { type: 'number' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `MemoryCostStats update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/MemoryCostStats/${rowId}`;

const MemoryCostStatsTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<MemoryCostStats[]>([]);
  ...
  const normalizePageRows = (result: unknown): MemoryCostStats[] => {
    if (Array.isArray(result)) return result as MemoryCostStats[];
    ...
    return Array.isArray(candidate) ? (candidate as MemoryCostStats[]) : [];
  };

  const mergeFetchedRows = (incomingRows: MemoryCostStats[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addMemoryCostStats] = useAddMemoryCostStatsMutation();
  const [updateMemoryCostStats] = useUpdateMemoryCostStatsMutation();
  const [deleteMemoryCostStats] = useDeleteMemoryCostStatsMutation();
  const [deleteMemoryCostStatsCascade] = useDeleteMemoryCostStatsCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<MemoryCostStats>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<MemoryCostStats | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<MemoryCostStats | null> => {
    ...
    try {
      ...
      return (await response.json()) as MemoryCostStats;
    } catch (err) {
      console.warn('Could not reload MemoryCostStats row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<MemoryCostStats>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMemoryCostStats(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as MemoryCostStats[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update MemoryCostStats';
      console.error('Failed to update MemoryCostStats:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMemoryCostStatsCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteMemoryCostStats(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<MemoryCostStats> = { ...source };
      delete (payload as any).id;
      const created = await addMemoryCostStats(payload).unwrap();
      setAllData((prev) => [created as MemoryCostStats, ...prev]);
    } catch (error) {
      console.error('Failed to add MemoryCostStats:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<MemoryCostStats> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMemoryCostStats(payload).unwrap();
      setAllData((prev) => [created as MemoryCostStats, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) MemoryCostStats:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `MemoryCostStats-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMemoryCostStats(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMemoryCostStats(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as MemoryCostStats[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as MemoryCostStats[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as MemoryCostStats[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as MemoryCostStats[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="MemoryCostStatsTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="MemoryCostStats Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create MemoryCostStats</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <MemoryCostStatsForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter MemoryCostStats (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing MemoryCostStats</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof MemoryCostStats;
                ...
                await updateMemoryCostStats(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of MemoryCostStatss.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'MemoryCostStats'}
...
export default MemoryCostStatsTable;
