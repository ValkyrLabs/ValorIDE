...
import { TrustVerificationResult } from '@thorapi/model';
import TrustVerificationResultForm from '@thorapi/redux/components/form/TrustVerificationResultForm';
...
import {
  useLazyGetTrustVerificationResultsPagedQuery,
...
} from '@thorapi/redux/services/TrustVerificationResultService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'verificationStatus': { type: 'enum', enumValues: [
    'VALID', 'INVALID', 'INCONCLUSIVE', 'UNAUTHORIZED', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TrustVerificationResult update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TrustVerificationResult/${rowId}`;

const TrustVerificationResultTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TrustVerificationResult[]>([]);
  ...
  const normalizePageRows = (result: unknown): TrustVerificationResult[] => {
    if (Array.isArray(result)) return result as TrustVerificationResult[];
    ...
    return Array.isArray(candidate) ? (candidate as TrustVerificationResult[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TrustVerificationResult[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addTrustVerificationResult] = useAddTrustVerificationResultMutation();
  const [updateTrustVerificationResult] = useUpdateTrustVerificationResultMutation();
  const [deleteTrustVerificationResult] = useDeleteTrustVerificationResultMutation();
  const [deleteTrustVerificationResultCascade] = useDeleteTrustVerificationResultCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TrustVerificationResult>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TrustVerificationResult | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TrustVerificationResult | null> => {
    ...
    try {
      ...
      return (await response.json()) as TrustVerificationResult;
    } catch (err) {
      console.warn('Could not reload TrustVerificationResult row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TrustVerificationResult>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTrustVerificationResult(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TrustVerificationResult[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TrustVerificationResult';
      console.error('Failed to update TrustVerificationResult:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustVerificationResultCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTrustVerificationResult(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TrustVerificationResult> = { ...source };
      delete (payload as any).id;
      const created = await addTrustVerificationResult(payload).unwrap();
      setAllData((prev) => [created as TrustVerificationResult, ...prev]);
    } catch (error) {
      console.error('Failed to add TrustVerificationResult:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TrustVerificationResult> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTrustVerificationResult(payload).unwrap();
      setAllData((prev) => [created as TrustVerificationResult, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TrustVerificationResult:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TrustVerificationResult-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTrustVerificationResult(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTrustVerificationResult(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TrustVerificationResult[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TrustVerificationResult[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TrustVerificationResult[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TrustVerificationResult[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="TrustVerificationResultTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="TrustVerificationResult Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TrustVerificationResult</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TrustVerificationResultForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TrustVerificationResult (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TrustVerificationResult</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TrustVerificationResult;
                ...
                await updateTrustVerificationResult(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TrustVerificationResults.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TrustVerificationResult'}
...
export default TrustVerificationResultTable;
