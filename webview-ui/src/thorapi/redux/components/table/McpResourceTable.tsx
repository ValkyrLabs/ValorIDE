...
import { McpResource } from '@thorapi/model';
import McpResourceForm from '@thorapi/redux/components/form/McpResourceForm';
...
} from '@thorapi/redux/services/McpResourceService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `McpResource update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/McpResource/${rowId}`;
...
const McpResourceTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): McpResource[] => {
    if (Array.isArray(result)) return result as McpResource[];
    ...
    return Array.isArray(candidate) ? (candidate as McpResource[]) : [];
  };

  const mergeFetchedRows = (incomingRows: McpResource[], requestedPage: number) => {
  ...
  const [deleteMcpResourceCascade] = useDeleteMcpResourceCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<McpResource | null> => {
    ...
    try {
      ...
      return (await response.json()) as McpResource;
    } catch (err) {
      console.warn('Could not reload McpResource row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<McpResource>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMcpResource(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as McpResource[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update McpResource';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpResourceCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `McpResource-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMcpResource(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMcpResource(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as McpResource[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as McpResource[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as McpResource[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as McpResource[]);
    }
  };
