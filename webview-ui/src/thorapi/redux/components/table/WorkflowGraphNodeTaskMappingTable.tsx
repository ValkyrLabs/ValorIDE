...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/WorkflowGraphNodeTaskMapping/${rowId}`;
...
const WorkflowGraphNodeTaskMappingTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addWorkflowGraphNodeTaskMapping] =
    useAddWorkflowGraphNodeTaskMappingMutation();
  const [updateWorkflowGraphNodeTaskMapping] =
    useUpdateWorkflowGraphNodeTaskMappingMutation();
  const [deleteWorkflowGraphNodeTaskMapping] =
    useDeleteWorkflowGraphNodeTaskMappingMutation();
  const [deleteWorkflowGraphNodeTaskMappingCascade] =
    useDeleteWorkflowGraphNodeTaskMappingCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] =
    useState<WorkflowGraphNodeTaskMapping | null>(null);
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<WorkflowGraphNodeTaskMapping | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload WorkflowGraphNodeTaskMapping row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateWorkflowGraphNodeTaskMapping(
        updatedItem as any,
      ).unwrap();
      ...
      setAllData(
        (prev) =>
          mergeUpdatedRow(
            ...
            rowId,
            hydrated as any,
          ) as WorkflowGraphNodeTaskMapping[],
      );
      if (successField) {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update WorkflowGraphNodeTaskMapping";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphNodeTaskMappingCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) WorkflowGraphNodeTaskMapping:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter WorkflowGraphNodeTaskMapping (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
