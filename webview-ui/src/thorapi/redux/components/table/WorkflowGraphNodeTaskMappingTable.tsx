...
import { WorkflowGraphNodeTaskMapping } from '@thorapi/model';
import WorkflowGraphNodeTaskMappingForm from '@thorapi/redux/components/form/WorkflowGraphNodeTaskMappingForm';
...
import {
  useLazyGetWorkflowGraphNodeTaskMappingsPagedQuery,
...
} from '@thorapi/redux/services/WorkflowGraphNodeTaskMappingService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `WorkflowGraphNodeTaskMapping update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/WorkflowGraphNodeTaskMapping/${rowId}`;

const WorkflowGraphNodeTaskMappingTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<WorkflowGraphNodeTaskMapping[]>([]);
  ...
  const normalizePageRows = (result: unknown): WorkflowGraphNodeTaskMapping[] => {
    if (Array.isArray(result)) return result as WorkflowGraphNodeTaskMapping[];
    ...
    return Array.isArray(candidate) ? (candidate as WorkflowGraphNodeTaskMapping[]) : [];
  };

  const mergeFetchedRows = (incomingRows: WorkflowGraphNodeTaskMapping[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addWorkflowGraphNodeTaskMapping] = useAddWorkflowGraphNodeTaskMappingMutation();
  const [updateWorkflowGraphNodeTaskMapping] = useUpdateWorkflowGraphNodeTaskMappingMutation();
  const [deleteWorkflowGraphNodeTaskMapping] = useDeleteWorkflowGraphNodeTaskMappingMutation();
  const [deleteWorkflowGraphNodeTaskMappingCascade] = useDeleteWorkflowGraphNodeTaskMappingCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<WorkflowGraphNodeTaskMapping>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<WorkflowGraphNodeTaskMapping | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<WorkflowGraphNodeTaskMapping | null> => {
    ...
    try {
      ...
      return (await response.json()) as WorkflowGraphNodeTaskMapping;
    } catch (err) {
      console.warn('Could not reload WorkflowGraphNodeTaskMapping row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<WorkflowGraphNodeTaskMapping>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWorkflowGraphNodeTaskMapping(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as WorkflowGraphNodeTaskMapping[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update WorkflowGraphNodeTaskMapping';
      console.error('Failed to update WorkflowGraphNodeTaskMapping:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphNodeTaskMappingCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteWorkflowGraphNodeTaskMapping(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<WorkflowGraphNodeTaskMapping> = { ...source };
      delete (payload as any).id;
      const created = await addWorkflowGraphNodeTaskMapping(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphNodeTaskMapping, ...prev]);
    } catch (error) {
      console.error('Failed to add WorkflowGraphNodeTaskMapping:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<WorkflowGraphNodeTaskMapping> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addWorkflowGraphNodeTaskMapping(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphNodeTaskMapping, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) WorkflowGraphNodeTaskMapping:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `WorkflowGraphNodeTaskMapping-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWorkflowGraphNodeTaskMapping(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWorkflowGraphNodeTaskMapping(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as WorkflowGraphNodeTaskMapping[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as WorkflowGraphNodeTaskMapping[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as WorkflowGraphNodeTaskMapping[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as WorkflowGraphNodeTaskMapping[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.WorkflowGraphNodeTaskMapping',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="WorkflowGraphNodeTaskMappingTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="WorkflowGraphNodeTaskMapping Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create WorkflowGraphNodeTaskMapping</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <WorkflowGraphNodeTaskMappingForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter WorkflowGraphNodeTaskMapping (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing WorkflowGraphNodeTaskMapping</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof WorkflowGraphNodeTaskMapping;
                ...
                await updateWorkflowGraphNodeTaskMapping(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of WorkflowGraphNodeTaskMappings.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'WorkflowGraphNodeTaskMapping'}
...
export default WorkflowGraphNodeTaskMappingTable;
