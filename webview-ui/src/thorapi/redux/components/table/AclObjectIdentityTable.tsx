...
import { AclObjectIdentity } from '@thorapi/model';
import AclObjectIdentityForm from '@thorapi/redux/components/form/AclObjectIdentityForm';
...
} from '@thorapi/redux/services/AclObjectIdentityService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AclObjectIdentity update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AclObjectIdentity/${rowId}`;
...
const AclObjectIdentityTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): AclObjectIdentity[] => {
    if (Array.isArray(result)) return result as AclObjectIdentity[];
    ...
    return Array.isArray(candidate) ? (candidate as AclObjectIdentity[]) : [];
  };

  const mergeFetchedRows = (incomingRows: AclObjectIdentity[], requestedPage: number) => {
  ...
  const [deleteAclObjectIdentityCascade] = useDeleteAclObjectIdentityCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<AclObjectIdentity | null> => {
    ...
    try {
      ...
      return (await response.json()) as AclObjectIdentity;
    } catch (err) {
      console.warn('Could not reload AclObjectIdentity row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AclObjectIdentity>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAclObjectIdentity(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AclObjectIdentity[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AclObjectIdentity';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAclObjectIdentityCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AclObjectIdentity-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAclObjectIdentity(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAclObjectIdentity(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AclObjectIdentity[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AclObjectIdentity[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AclObjectIdentity[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AclObjectIdentity[]);
    }
  };
