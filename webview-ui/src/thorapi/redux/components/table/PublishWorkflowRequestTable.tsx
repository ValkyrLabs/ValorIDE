...
import PublishWorkflowRequestForm from '@thorapi/redux/components/form/PublishWorkflowRequestForm';
...
} from '@thorapi/redux/services/PublishWorkflowRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `PublishWorkflowRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/PublishWorkflowRequest/${rowId}`;
...
const PublishWorkflowRequestTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): PublishWorkflowRequest[] => {
    if (Array.isArray(result)) return result as PublishWorkflowRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as PublishWorkflowRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: PublishWorkflowRequest[], requestedPage: number) => {
  ...
  const [deletePublishWorkflowRequestCascade] = useDeletePublishWorkflowRequestCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<PublishWorkflowRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<PublishWorkflowRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as PublishWorkflowRequest;
    } catch (err) {
      console.warn('Could not reload PublishWorkflowRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<PublishWorkflowRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updatePublishWorkflowRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as PublishWorkflowRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update PublishWorkflowRequest';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePublishWorkflowRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `PublishWorkflowRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addPublishWorkflowRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updatePublishWorkflowRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as PublishWorkflowRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as PublishWorkflowRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as PublishWorkflowRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as PublishWorkflowRequest[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter PublishWorkflowRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
