...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/PublishWorkflowRequest/${rowId}`;
...
const PublishWorkflowRequestTable: React.FC = () => {
  ...
  const [deletePublishWorkflowRequest] =
    useDeletePublishWorkflowRequestMutation();
  const [deletePublishWorkflowRequestCascade] =
    useDeletePublishWorkflowRequestCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<PublishWorkflowRequest | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updatePublishWorkflowRequest(
        updatedItem as any,
      ).unwrap();
