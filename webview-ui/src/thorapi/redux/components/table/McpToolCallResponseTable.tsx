...
import { McpToolCallResponse } from '@thorapi/model';
import McpToolCallResponseForm from '@thorapi/redux/components/form/McpToolCallResponseForm';
...
} from '@thorapi/redux/services/McpToolCallResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `McpToolCallResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/McpToolCallResponse/${rowId}`;
...
const McpToolCallResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): McpToolCallResponse[] => {
    if (Array.isArray(result)) return result as McpToolCallResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as McpToolCallResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: McpToolCallResponse[], requestedPage: number) => {
  ...
  const [deleteMcpToolCallResponseCascade] = useDeleteMcpToolCallResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<McpToolCallResponse | null> => {
    ...
    try {
      ...
      console.warn('Could not reload McpToolCallResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<McpToolCallResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMcpToolCallResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as McpToolCallResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update McpToolCallResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpToolCallResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `McpToolCallResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMcpToolCallResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMcpToolCallResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as McpToolCallResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as McpToolCallResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as McpToolCallResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as McpToolCallResponse[]);
    }
  };
