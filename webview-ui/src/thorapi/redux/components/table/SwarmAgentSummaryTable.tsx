...
import SwarmAgentSummaryForm from '@thorapi/redux/components/form/SwarmAgentSummaryForm';
...
} from '@thorapi/redux/services/SwarmAgentSummaryService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SwarmAgentSummary update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SwarmAgentSummary/${rowId}`;
...
const SwarmAgentSummaryTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SwarmAgentSummary[] => {
    if (Array.isArray(result)) return result as SwarmAgentSummary[];
    ...
    return Array.isArray(candidate) ? (candidate as SwarmAgentSummary[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SwarmAgentSummary[], requestedPage: number) => {
  ...
  const [deleteSwarmAgentSummaryCascade] = useDeleteSwarmAgentSummaryCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SwarmAgentSummary | null> => {
    ...
    try {
      ...
      return (await response.json()) as SwarmAgentSummary;
    } catch (err) {
      console.warn('Could not reload SwarmAgentSummary row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SwarmAgentSummary>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSwarmAgentSummary(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SwarmAgentSummary[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SwarmAgentSummary';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmAgentSummaryCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SwarmAgentSummary-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSwarmAgentSummary(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSwarmAgentSummary(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SwarmAgentSummary[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SwarmAgentSummary[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SwarmAgentSummary[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SwarmAgentSummary[]);
    }
  };
