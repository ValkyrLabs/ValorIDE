...
import { TrustKey } from '@thorapi/model';
import TrustKeyForm from '@thorapi/redux/components/form/TrustKeyForm';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TrustKey update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TrustKey/${rowId}`;

const TrustKeyTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TrustKey[]>([]);
  ...
  const normalizePageRows = (result: unknown): TrustKey[] => {
    if (Array.isArray(result)) return result as TrustKey[];
    ...
    return Array.isArray(candidate) ? (candidate as TrustKey[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TrustKey[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addTrustKey] = useAddTrustKeyMutation();
  const [updateTrustKey] = useUpdateTrustKeyMutation();
  const [deleteTrustKey] = useDeleteTrustKeyMutation();
  const [deleteTrustKeyCascade] = useDeleteTrustKeyCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TrustKey>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TrustKey | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TrustKey | null> => {
    ...
    try {
      ...
      return (await response.json()) as TrustKey;
    } catch (err) {
      console.warn('Could not reload TrustKey row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TrustKey>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTrustKey(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TrustKey[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TrustKey';
      console.error('Failed to update TrustKey:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustKeyCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTrustKey(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TrustKey> = { ...source };
      delete (payload as any).id;
      const created = await addTrustKey(payload).unwrap();
      setAllData((prev) => [created as TrustKey, ...prev]);
    } catch (error) {
      console.error('Failed to add TrustKey:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TrustKey> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTrustKey(payload).unwrap();
      setAllData((prev) => [created as TrustKey, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TrustKey:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TrustKey-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTrustKey(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTrustKey(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TrustKey[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TrustKey[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TrustKey[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TrustKey[]);
    }
  };
    ...
    <>
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TrustKey</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TrustKeyForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TrustKey (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TrustKey</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TrustKey;
                ...
                await updateTrustKey(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TrustKeys.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TrustKey'}
