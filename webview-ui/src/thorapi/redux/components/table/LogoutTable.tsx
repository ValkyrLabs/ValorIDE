...
import LogoutForm from '@thorapi/redux/components/form/LogoutForm';
...
} from '@thorapi/redux/services/LogoutService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Logout update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Logout/${rowId}`;
...
const LogoutTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Logout[] => {
    if (Array.isArray(result)) return result as Logout[];
    ...
    return Array.isArray(candidate) ? (candidate as Logout[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Logout[], requestedPage: number) => {
  ...
  const [deleteLogoutCascade] = useDeleteLogoutCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Logout | null> => {
    ...
    try {
      ...
      return (await response.json()) as Logout;
    } catch (err) {
      console.warn('Could not reload Logout row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Logout>,
    successField?: string,
  ) => {
    try {
      const saved = await updateLogout(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Logout[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Logout';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteLogoutCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Logout-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addLogout(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateLogout(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Logout[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Logout[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Logout[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Logout[]);
    }
  };
