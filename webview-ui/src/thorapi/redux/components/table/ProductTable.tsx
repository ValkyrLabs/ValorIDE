...
import { Product } from '@thorapi/model';
import ProductForm from '@thorapi/redux/components/form/ProductForm';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Product update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Product/${rowId}`;
...
const ProductTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Product[] => {
    ...
    return Array.isArray(candidate) ? (candidate as Product[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Product[], requestedPage: number) => {
  ...
  const reloadRowById = async (rowId: string): Promise<Product | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Product row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateProduct(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Product[]);
      if (successField) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteProductCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Product-export-${stamp}.json`;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Product[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Product[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Product[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Product[]);
    }
  };
