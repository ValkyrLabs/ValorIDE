...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ApiIdentityKpiSnapshot/${rowId}`;
...
const ApiIdentityKpiSnapshotTable: React.FC = () => {
  ...
  const [updateApiIdentityKpiSnapshot] =
    useUpdateApiIdentityKpiSnapshotMutation();
  const [deleteApiIdentityKpiSnapshot] =
    useDeleteApiIdentityKpiSnapshotMutation();
  const [deleteApiIdentityKpiSnapshotCascade] =
    useDeleteApiIdentityKpiSnapshotCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<ApiIdentityKpiSnapshot | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<ApiIdentityKpiSnapshot | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateApiIdentityKpiSnapshot(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ApiIdentityKpiSnapshot";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteApiIdentityKpiSnapshotCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter ApiIdentityKpiSnapshot (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
