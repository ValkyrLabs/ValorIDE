...
import { Formula } from '@thorapi/model';
import FormulaForm from '@thorapi/redux/components/form/FormulaForm';
...
} from '@thorapi/redux/services/FormulaService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Formula update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Formula/${rowId}`;
...
const FormulaTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Formula[] => {
    if (Array.isArray(result)) return result as Formula[];
    ...
    return Array.isArray(candidate) ? (candidate as Formula[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Formula[], requestedPage: number) => {
  ...
  const [deleteFormulaCascade] = useDeleteFormulaCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Formula | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Formula row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Formula>,
    successField?: string,
  ) => {
    try {
      const saved = await updateFormula(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Formula[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Formula';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteFormulaCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Formula-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addFormula(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateFormula(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Formula[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Formula[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Formula[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Formula[]);
    }
  };
