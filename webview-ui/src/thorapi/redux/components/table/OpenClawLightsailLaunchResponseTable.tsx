...
import { OpenClawLightsailLaunchResponse } from '@thorapi/model';
import OpenClawLightsailLaunchResponseForm from '@thorapi/redux/components/form/OpenClawLightsailLaunchResponseForm';
...
import {
  useLazyGetOpenClawLightsailLaunchResponsesPagedQuery,
...
} from '@thorapi/redux/services/OpenClawLightsailLaunchResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `OpenClawLightsailLaunchResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/OpenClawLightsailLaunchResponse/${rowId}`;

const OpenClawLightsailLaunchResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<OpenClawLightsailLaunchResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): OpenClawLightsailLaunchResponse[] => {
    if (Array.isArray(result)) return result as OpenClawLightsailLaunchResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as OpenClawLightsailLaunchResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: OpenClawLightsailLaunchResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addOpenClawLightsailLaunchResponse] = useAddOpenClawLightsailLaunchResponseMutation();
  const [updateOpenClawLightsailLaunchResponse] = useUpdateOpenClawLightsailLaunchResponseMutation();
  const [deleteOpenClawLightsailLaunchResponse] = useDeleteOpenClawLightsailLaunchResponseMutation();
  const [deleteOpenClawLightsailLaunchResponseCascade] = useDeleteOpenClawLightsailLaunchResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<OpenClawLightsailLaunchResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<OpenClawLightsailLaunchResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<OpenClawLightsailLaunchResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as OpenClawLightsailLaunchResponse;
    } catch (err) {
      console.warn('Could not reload OpenClawLightsailLaunchResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<OpenClawLightsailLaunchResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateOpenClawLightsailLaunchResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as OpenClawLightsailLaunchResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update OpenClawLightsailLaunchResponse';
      console.error('Failed to update OpenClawLightsailLaunchResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOpenClawLightsailLaunchResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteOpenClawLightsailLaunchResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<OpenClawLightsailLaunchResponse> = { ...source };
      delete (payload as any).id;
      const created = await addOpenClawLightsailLaunchResponse(payload).unwrap();
      setAllData((prev) => [created as OpenClawLightsailLaunchResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add OpenClawLightsailLaunchResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<OpenClawLightsailLaunchResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addOpenClawLightsailLaunchResponse(payload).unwrap();
      setAllData((prev) => [created as OpenClawLightsailLaunchResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) OpenClawLightsailLaunchResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `OpenClawLightsailLaunchResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addOpenClawLightsailLaunchResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateOpenClawLightsailLaunchResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as OpenClawLightsailLaunchResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as OpenClawLightsailLaunchResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as OpenClawLightsailLaunchResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as OpenClawLightsailLaunchResponse[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="OpenClawLightsailLaunchResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="OpenClawLightsailLaunchResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create OpenClawLightsailLaunchResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <OpenClawLightsailLaunchResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter OpenClawLightsailLaunchResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing OpenClawLightsailLaunchResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof OpenClawLightsailLaunchResponse;
                ...
                await updateOpenClawLightsailLaunchResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of OpenClawLightsailLaunchResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'OpenClawLightsailLaunchResponse'}
...
export default OpenClawLightsailLaunchResponseTable;
