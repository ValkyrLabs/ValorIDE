...
import WorkflowGraphModuleForm from '@thorapi/redux/components/form/WorkflowGraphModuleForm';
...
import {
  useLazyGetWorkflowGraphModulesPagedQuery,
...
} from '@thorapi/redux/services/WorkflowGraphModuleService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `WorkflowGraphModule update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/WorkflowGraphModule/${rowId}`;

const WorkflowGraphModuleTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<WorkflowGraphModule[]>([]);
  ...
  const normalizePageRows = (result: unknown): WorkflowGraphModule[] => {
    if (Array.isArray(result)) return result as WorkflowGraphModule[];
    ...
    return Array.isArray(candidate) ? (candidate as WorkflowGraphModule[]) : [];
  };

  const mergeFetchedRows = (incomingRows: WorkflowGraphModule[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addWorkflowGraphModule] = useAddWorkflowGraphModuleMutation();
  const [updateWorkflowGraphModule] = useUpdateWorkflowGraphModuleMutation();
  const [deleteWorkflowGraphModule] = useDeleteWorkflowGraphModuleMutation();
  const [deleteWorkflowGraphModuleCascade] = useDeleteWorkflowGraphModuleCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<WorkflowGraphModule>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<WorkflowGraphModule | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<WorkflowGraphModule | null> => {
    ...
    try {
      ...
      return (await response.json()) as WorkflowGraphModule;
    } catch (err) {
      console.warn('Could not reload WorkflowGraphModule row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<WorkflowGraphModule>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWorkflowGraphModule(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as WorkflowGraphModule[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update WorkflowGraphModule';
      console.error('Failed to update WorkflowGraphModule:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphModuleCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteWorkflowGraphModule(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<WorkflowGraphModule> = { ...source };
      delete (payload as any).id;
      const created = await addWorkflowGraphModule(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphModule, ...prev]);
    } catch (error) {
      console.error('Failed to add WorkflowGraphModule:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<WorkflowGraphModule> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addWorkflowGraphModule(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphModule, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) WorkflowGraphModule:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `WorkflowGraphModule-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWorkflowGraphModule(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWorkflowGraphModule(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as WorkflowGraphModule[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as WorkflowGraphModule[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as WorkflowGraphModule[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as WorkflowGraphModule[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="WorkflowGraphModuleTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="WorkflowGraphModule Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create WorkflowGraphModule</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <WorkflowGraphModuleForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter WorkflowGraphModule (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing WorkflowGraphModule</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof WorkflowGraphModule;
                ...
                await updateWorkflowGraphModule(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of WorkflowGraphModules.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'WorkflowGraphModule'}
...
export default WorkflowGraphModuleTable;
