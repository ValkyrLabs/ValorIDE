...
import SwarmCommandResponseForm from '@thorapi/redux/components/form/SwarmCommandResponseForm';
...
} from '@thorapi/redux/services/SwarmCommandResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SwarmCommandResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SwarmCommandResponse/${rowId}`;
...
const SwarmCommandResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SwarmCommandResponse[] => {
    if (Array.isArray(result)) return result as SwarmCommandResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as SwarmCommandResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SwarmCommandResponse[], requestedPage: number) => {
  ...
  const [deleteSwarmCommandResponseCascade] = useDeleteSwarmCommandResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SwarmCommandResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as SwarmCommandResponse;
    } catch (err) {
      console.warn('Could not reload SwarmCommandResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SwarmCommandResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSwarmCommandResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SwarmCommandResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SwarmCommandResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmCommandResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SwarmCommandResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSwarmCommandResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSwarmCommandResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SwarmCommandResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SwarmCommandResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SwarmCommandResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SwarmCommandResponse[]);
    }
  };
