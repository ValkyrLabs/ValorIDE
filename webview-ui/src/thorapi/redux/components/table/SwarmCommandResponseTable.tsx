...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/SwarmCommandResponse/${rowId}`;
...
const SwarmCommandResponseTable: React.FC = () => {
  ...
  const [deleteSwarmCommandResponseCascade] =
    useDeleteSwarmCommandResponseCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<SwarmCommandResponse | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateSwarmCommandResponse(
        updatedItem as any,
      ).unwrap();
