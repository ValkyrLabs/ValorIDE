...
import { RetrievalReceiptRequest } from '@thorapi/model';
import RetrievalReceiptRequestForm from '@thorapi/redux/components/form/RetrievalReceiptRequestForm';
...
import {
  useLazyGetRetrievalReceiptRequestsPagedQuery,
...
} from '@thorapi/redux/services/RetrievalReceiptRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RetrievalReceiptRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RetrievalReceiptRequest/${rowId}`;

const RetrievalReceiptRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RetrievalReceiptRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): RetrievalReceiptRequest[] => {
    if (Array.isArray(result)) return result as RetrievalReceiptRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as RetrievalReceiptRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RetrievalReceiptRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addRetrievalReceiptRequest] = useAddRetrievalReceiptRequestMutation();
  const [updateRetrievalReceiptRequest] = useUpdateRetrievalReceiptRequestMutation();
  const [deleteRetrievalReceiptRequest] = useDeleteRetrievalReceiptRequestMutation();
  const [deleteRetrievalReceiptRequestCascade] = useDeleteRetrievalReceiptRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<RetrievalReceiptRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RetrievalReceiptRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<RetrievalReceiptRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as RetrievalReceiptRequest;
    } catch (err) {
      console.warn('Could not reload RetrievalReceiptRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RetrievalReceiptRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRetrievalReceiptRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RetrievalReceiptRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RetrievalReceiptRequest';
      console.error('Failed to update RetrievalReceiptRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRetrievalReceiptRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteRetrievalReceiptRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RetrievalReceiptRequest> = { ...source };
      delete (payload as any).id;
      const created = await addRetrievalReceiptRequest(payload).unwrap();
      setAllData((prev) => [created as RetrievalReceiptRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add RetrievalReceiptRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RetrievalReceiptRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRetrievalReceiptRequest(payload).unwrap();
      setAllData((prev) => [created as RetrievalReceiptRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) RetrievalReceiptRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RetrievalReceiptRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRetrievalReceiptRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRetrievalReceiptRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RetrievalReceiptRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RetrievalReceiptRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RetrievalReceiptRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RetrievalReceiptRequest[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="RetrievalReceiptRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="RetrievalReceiptRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RetrievalReceiptRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RetrievalReceiptRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RetrievalReceiptRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RetrievalReceiptRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RetrievalReceiptRequest;
                ...
                await updateRetrievalReceiptRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RetrievalReceiptRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RetrievalReceiptRequest'}
...
export default RetrievalReceiptRequestTable;
