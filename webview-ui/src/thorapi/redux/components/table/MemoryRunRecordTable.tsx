...
import { MemoryRunRecord } from '@thorapi/model';
import MemoryRunRecordForm from '@thorapi/redux/components/form/MemoryRunRecordForm';
...
import {
  useLazyGetMemoryRunRecordsPagedQuery,
...
} from '@thorapi/redux/services/MemoryRunRecordService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'creditsBurned': { type: 'number' },
  'qualitySignal': { type: 'enum', enumValues: [
    'good', 'noisy', 'improved', 'degraded', 
  ], enumValueType: 'string' },
  'contextUsedPct': { type: 'number' },
  'wastedPct': { type: 'number' },
  'memoriesUsed': { type: 'number' },
  'runAt': { type: 'datetime' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `MemoryRunRecord update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/MemoryRunRecord/${rowId}`;

const MemoryRunRecordTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<MemoryRunRecord[]>([]);
  ...
  const normalizePageRows = (result: unknown): MemoryRunRecord[] => {
    if (Array.isArray(result)) return result as MemoryRunRecord[];
    ...
    return Array.isArray(candidate) ? (candidate as MemoryRunRecord[]) : [];
  };

  const mergeFetchedRows = (incomingRows: MemoryRunRecord[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addMemoryRunRecord] = useAddMemoryRunRecordMutation();
  const [updateMemoryRunRecord] = useUpdateMemoryRunRecordMutation();
  const [deleteMemoryRunRecord] = useDeleteMemoryRunRecordMutation();
  const [deleteMemoryRunRecordCascade] = useDeleteMemoryRunRecordCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<MemoryRunRecord>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<MemoryRunRecord | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<MemoryRunRecord | null> => {
    ...
    try {
      ...
      return (await response.json()) as MemoryRunRecord;
    } catch (err) {
      console.warn('Could not reload MemoryRunRecord row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<MemoryRunRecord>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMemoryRunRecord(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as MemoryRunRecord[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update MemoryRunRecord';
      console.error('Failed to update MemoryRunRecord:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMemoryRunRecordCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteMemoryRunRecord(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<MemoryRunRecord> = { ...source };
      delete (payload as any).id;
      const created = await addMemoryRunRecord(payload).unwrap();
      setAllData((prev) => [created as MemoryRunRecord, ...prev]);
    } catch (error) {
      console.error('Failed to add MemoryRunRecord:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<MemoryRunRecord> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMemoryRunRecord(payload).unwrap();
      setAllData((prev) => [created as MemoryRunRecord, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) MemoryRunRecord:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `MemoryRunRecord-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMemoryRunRecord(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMemoryRunRecord(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as MemoryRunRecord[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as MemoryRunRecord[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as MemoryRunRecord[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as MemoryRunRecord[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="MemoryRunRecordTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="MemoryRunRecord Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create MemoryRunRecord</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <MemoryRunRecordForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter MemoryRunRecord (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing MemoryRunRecord</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof MemoryRunRecord;
                ...
                await updateMemoryRunRecord(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of MemoryRunRecords.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'MemoryRunRecord'}
...
export default MemoryRunRecordTable;
