...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  qualitySignal: {
    type: "enum",
    enumValues: ["good", "noisy", "improved", "degraded"],
...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/MemoryRunRecord/${rowId}`;
...
const MemoryRunRecordTable: React.FC = () => {
  ...
  const [deleteMemoryRunRecordCascade] =
    useDeleteMemoryRunRecordCascadeMutation();
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update MemoryRunRecord";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMemoryRunRecordCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
