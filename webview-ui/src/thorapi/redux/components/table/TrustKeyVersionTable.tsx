...
import { TrustKeyVersion } from '@thorapi/model';
import TrustKeyVersionForm from '@thorapi/redux/components/form/TrustKeyVersionForm';
...
import {
  useLazyGetTrustKeyVersionsPagedQuery,
...
} from '@thorapi/redux/services/TrustKeyVersionService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'algorithm': { type: 'enum', enumValues: [
    'AES_256_GCM', 'XCHACHA20_POLY1305', 'ED25519', 'ECDSA_P256', 'RSA_OAEP_3072', 
  ], enumValueType: 'string' },
  'versionStatus': { type: 'enum', enumValues: [
    'PENDING_ACTIVATION', 'ACTIVE', 'DECRYPT_ONLY', 'RETIRED', 'DESTROY_SCHEDULED', 'DESTROYED', 
  ], enumValueType: 'string' },
  'notBefore': { type: 'datetime' },
  'notAfter': { type: 'datetime' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TrustKeyVersion update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TrustKeyVersion/${rowId}`;

const TrustKeyVersionTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TrustKeyVersion[]>([]);
  ...
  const normalizePageRows = (result: unknown): TrustKeyVersion[] => {
    if (Array.isArray(result)) return result as TrustKeyVersion[];
    ...
    return Array.isArray(candidate) ? (candidate as TrustKeyVersion[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TrustKeyVersion[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addTrustKeyVersion] = useAddTrustKeyVersionMutation();
  const [updateTrustKeyVersion] = useUpdateTrustKeyVersionMutation();
  const [deleteTrustKeyVersion] = useDeleteTrustKeyVersionMutation();
  const [deleteTrustKeyVersionCascade] = useDeleteTrustKeyVersionCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TrustKeyVersion>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TrustKeyVersion | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TrustKeyVersion | null> => {
    ...
    try {
      ...
      return (await response.json()) as TrustKeyVersion;
    } catch (err) {
      console.warn('Could not reload TrustKeyVersion row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TrustKeyVersion>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTrustKeyVersion(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TrustKeyVersion[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TrustKeyVersion';
      console.error('Failed to update TrustKeyVersion:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustKeyVersionCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTrustKeyVersion(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TrustKeyVersion> = { ...source };
      delete (payload as any).id;
      const created = await addTrustKeyVersion(payload).unwrap();
      setAllData((prev) => [created as TrustKeyVersion, ...prev]);
    } catch (error) {
      console.error('Failed to add TrustKeyVersion:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TrustKeyVersion> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTrustKeyVersion(payload).unwrap();
      setAllData((prev) => [created as TrustKeyVersion, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TrustKeyVersion:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TrustKeyVersion-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTrustKeyVersion(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTrustKeyVersion(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TrustKeyVersion[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TrustKeyVersion[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TrustKeyVersion[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TrustKeyVersion[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="TrustKeyVersionTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="TrustKeyVersion Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TrustKeyVersion</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TrustKeyVersionForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TrustKeyVersion (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TrustKeyVersion</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TrustKeyVersion;
                ...
                await updateTrustKeyVersion(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TrustKeyVersions.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TrustKeyVersion'}
...
export default TrustKeyVersionTable;
