...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/TrustKeyVersion/${rowId}`;
...
const TrustKeyVersionTable: React.FC = () => {
  ...
  const [deleteTrustKeyVersionCascade] =
    useDeleteTrustKeyVersionCascadeMutation();
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update TrustKeyVersion";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustKeyVersionCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
