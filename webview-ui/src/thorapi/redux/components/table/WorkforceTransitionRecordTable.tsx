...
import { WorkforceTransitionRecord } from '@thorapi/model';
import WorkforceTransitionRecordForm from '@thorapi/redux/components/form/WorkforceTransitionRecordForm';
...
import {
  useLazyGetWorkforceTransitionRecordsPagedQuery,
...
} from '@thorapi/redux/services/WorkforceTransitionRecordService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'certificationStatus': { type: 'enum', enumValues: [
    'NOT_STARTED', 'IN_PROGRESS', 'CERTIFIED', 
  ], enumValueType: 'string' },
  'wageDelta': { type: 'number' },
  'transitionDate': { type: 'datetime' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `WorkforceTransitionRecord update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/WorkforceTransitionRecord/${rowId}`;

const WorkforceTransitionRecordTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<WorkforceTransitionRecord[]>([]);
  ...
  const normalizePageRows = (result: unknown): WorkforceTransitionRecord[] => {
    if (Array.isArray(result)) return result as WorkforceTransitionRecord[];
    ...
    return Array.isArray(candidate) ? (candidate as WorkforceTransitionRecord[]) : [];
  };

  const mergeFetchedRows = (incomingRows: WorkforceTransitionRecord[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addWorkforceTransitionRecord] = useAddWorkforceTransitionRecordMutation();
  const [updateWorkforceTransitionRecord] = useUpdateWorkforceTransitionRecordMutation();
  const [deleteWorkforceTransitionRecord] = useDeleteWorkforceTransitionRecordMutation();
  const [deleteWorkforceTransitionRecordCascade] = useDeleteWorkforceTransitionRecordCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<WorkforceTransitionRecord>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<WorkforceTransitionRecord | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<WorkforceTransitionRecord | null> => {
    ...
    try {
      ...
      return (await response.json()) as WorkforceTransitionRecord;
    } catch (err) {
      console.warn('Could not reload WorkforceTransitionRecord row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<WorkforceTransitionRecord>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWorkforceTransitionRecord(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as WorkforceTransitionRecord[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update WorkforceTransitionRecord';
      console.error('Failed to update WorkforceTransitionRecord:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkforceTransitionRecordCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteWorkforceTransitionRecord(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<WorkforceTransitionRecord> = { ...source };
      delete (payload as any).id;
      const created = await addWorkforceTransitionRecord(payload).unwrap();
      setAllData((prev) => [created as WorkforceTransitionRecord, ...prev]);
    } catch (error) {
      console.error('Failed to add WorkforceTransitionRecord:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<WorkforceTransitionRecord> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addWorkforceTransitionRecord(payload).unwrap();
      setAllData((prev) => [created as WorkforceTransitionRecord, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) WorkforceTransitionRecord:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `WorkforceTransitionRecord-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWorkforceTransitionRecord(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWorkforceTransitionRecord(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as WorkforceTransitionRecord[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as WorkforceTransitionRecord[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as WorkforceTransitionRecord[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as WorkforceTransitionRecord[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="WorkforceTransitionRecordTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="WorkforceTransitionRecord Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create WorkforceTransitionRecord</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <WorkforceTransitionRecordForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter WorkforceTransitionRecord (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing WorkforceTransitionRecord</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof WorkforceTransitionRecord;
                ...
                await updateWorkforceTransitionRecord(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of WorkforceTransitionRecords.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'WorkforceTransitionRecord'}
...
export default WorkforceTransitionRecordTable;
