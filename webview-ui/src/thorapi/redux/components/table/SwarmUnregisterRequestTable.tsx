...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/SwarmUnregisterRequest/${rowId}`;
...
const SwarmUnregisterRequestTable: React.FC = () => {
  ...
  const [deleteSwarmUnregisterRequest] =
    useDeleteSwarmUnregisterRequestMutation();
  const [deleteSwarmUnregisterRequestCascade] =
    useDeleteSwarmUnregisterRequestCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<SwarmUnregisterRequest | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateSwarmUnregisterRequest(
        updatedItem as any,
      ).unwrap();
