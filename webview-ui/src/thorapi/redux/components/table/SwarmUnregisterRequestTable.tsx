...
import { SwarmUnregisterRequest } from '@thorapi/model';
import SwarmUnregisterRequestForm from '@thorapi/redux/components/form/SwarmUnregisterRequestForm';
...
import {
  ...
  useDeleteSwarmUnregisterRequestCascadeMutation
} from '@thorapi/redux/services/SwarmUnregisterRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SwarmUnregisterRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SwarmUnregisterRequest/${rowId}`;
...
const SwarmUnregisterRequestTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SwarmUnregisterRequest[] => {
    if (Array.isArray(result)) return result as SwarmUnregisterRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as SwarmUnregisterRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SwarmUnregisterRequest[], requestedPage: number) => {
  ...
  const [deleteSwarmUnregisterRequestCascade] = useDeleteSwarmUnregisterRequestCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SwarmUnregisterRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as SwarmUnregisterRequest;
    } catch (err) {
      console.warn('Could not reload SwarmUnregisterRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SwarmUnregisterRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSwarmUnregisterRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SwarmUnregisterRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SwarmUnregisterRequest';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmUnregisterRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SwarmUnregisterRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSwarmUnregisterRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSwarmUnregisterRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SwarmUnregisterRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SwarmUnregisterRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SwarmUnregisterRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SwarmUnregisterRequest[]);
    }
  };
