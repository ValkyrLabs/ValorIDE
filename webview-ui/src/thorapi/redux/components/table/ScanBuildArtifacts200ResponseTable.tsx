...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ScanBuildArtifacts200Response/${rowId}`;
...
const ScanBuildArtifacts200ResponseTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addScanBuildArtifacts200Response] =
    useAddScanBuildArtifacts200ResponseMutation();
  const [updateScanBuildArtifacts200Response] =
    useUpdateScanBuildArtifacts200ResponseMutation();
  const [deleteScanBuildArtifacts200Response] =
    useDeleteScanBuildArtifacts200ResponseMutation();
  const [deleteScanBuildArtifacts200ResponseCascade] =
    useDeleteScanBuildArtifacts200ResponseCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] =
    useState<ScanBuildArtifacts200Response | null>(null);
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<ScanBuildArtifacts200Response | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload ScanBuildArtifacts200Response row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateScanBuildArtifacts200Response(
        updatedItem as any,
      ).unwrap();
      ...
      setAllData(
        (prev) =>
          mergeUpdatedRow(
            ...
            rowId,
            hydrated as any,
          ) as ScanBuildArtifacts200Response[],
      );
      if (successField) {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ScanBuildArtifacts200Response";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteScanBuildArtifacts200ResponseCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) ScanBuildArtifacts200Response:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter ScanBuildArtifacts200Response (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
      ...
      <MarkdownEditorModal
        ...
        onSave={async (updatedText) => {
          try {
            ...
            const field =
              richModal.field as keyof ScanBuildArtifacts200Response;
            if (!rid || !field) {
