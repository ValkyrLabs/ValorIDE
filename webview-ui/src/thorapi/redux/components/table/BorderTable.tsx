...
import { Border } from '@thorapi/model';
import BorderForm from '@thorapi/redux/components/form/BorderForm';
...
import {
  ...
  useDeleteBorderCascadeMutation
} from '@thorapi/redux/services/BorderService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Border update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Border/${rowId}`;
...
const BorderTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Border[] => {
    if (Array.isArray(result)) return result as Border[];
    ...
    return Array.isArray(candidate) ? (candidate as Border[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Border[], requestedPage: number) => {
  ...
  const [deleteBorderCascade] = useDeleteBorderCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Border | null> => {
    ...
    try {
      ...
      return (await response.json()) as Border;
    } catch (err) {
      console.warn('Could not reload Border row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Border>,
    successField?: string,
  ) => {
    try {
      const saved = await updateBorder(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Border[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Border';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteBorderCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Border-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addBorder(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateBorder(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Border[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Border[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Border[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Border[]);
    }
  };
