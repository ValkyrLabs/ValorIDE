...
import { MemoryEntry } from '@thorapi/model';
import MemoryEntryForm from '@thorapi/redux/components/form/MemoryEntryForm';
...
import {
  useLazyGetMemoryEntrysPagedQuery,
...
const columnSchema: Record<string, ColumnSchema> = {
  'type': { type: 'enum', enumValues: [
    'configuration', 'preference', 'decision', 'todo', 'context', 'artifact', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `MemoryEntry update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/MemoryEntry/${rowId}`;

const MemoryEntryTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<MemoryEntry[]>([]);
  ...
  const normalizePageRows = (result: unknown): MemoryEntry[] => {
    if (Array.isArray(result)) return result as MemoryEntry[];
    ...
    return Array.isArray(candidate) ? (candidate as MemoryEntry[]) : [];
  };

  const mergeFetchedRows = (incomingRows: MemoryEntry[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addMemoryEntry] = useAddMemoryEntryMutation();
  const [updateMemoryEntry] = useUpdateMemoryEntryMutation();
  const [deleteMemoryEntry] = useDeleteMemoryEntryMutation();
  const [deleteMemoryEntryCascade] = useDeleteMemoryEntryCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<MemoryEntry | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<MemoryEntry | null> => {
    ...
    try {
      ...
      console.warn('Could not reload MemoryEntry row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<MemoryEntry>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMemoryEntry(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as MemoryEntry[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update MemoryEntry';
      console.error('Failed to update MemoryEntry:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMemoryEntryCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteMemoryEntry(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<MemoryEntry> = { ...source };
      delete (payload as any).id;
      const created = await addMemoryEntry(payload).unwrap();
      setAllData((prev) => [created as MemoryEntry, ...prev]);
    } catch (error) {
      console.error('Failed to add MemoryEntry:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<MemoryEntry> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMemoryEntry(payload).unwrap();
      setAllData((prev) => [created as MemoryEntry, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) MemoryEntry:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `MemoryEntry-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMemoryEntry(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMemoryEntry(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as MemoryEntry[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as MemoryEntry[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as MemoryEntry[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as MemoryEntry[]);
    }
  };
    ...
    <>
          ...
          >
            ...
            <FloatingControlPanel
              description="MemoryEntry Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create MemoryEntry</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <MemoryEntryForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter MemoryEntry (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing MemoryEntry</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof MemoryEntry;
                ...
                await updateMemoryEntry(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of MemoryEntrys.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'MemoryEntry'}
...
export default MemoryEntryTable;
