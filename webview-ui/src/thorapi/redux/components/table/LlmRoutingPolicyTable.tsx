...
import { LlmRoutingPolicy } from '@thorapi/model';
import LlmRoutingPolicyForm from '@thorapi/redux/components/form/LlmRoutingPolicyForm';
...
import {
  useLazyGetLlmRoutingPolicysPagedQuery,
...
} from '@thorapi/redux/services/LlmRoutingPolicyService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'strategy': { type: 'enum', enumValues: [
    'single_provider', 'fallback_chain', 
  ], enumValueType: 'string' },
  'primaryProvider': { type: 'enum', enumValues: [
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `LlmRoutingPolicy update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/LlmRoutingPolicy/${rowId}`;

const LlmRoutingPolicyTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<LlmRoutingPolicy[]>([]);
  ...
  const normalizePageRows = (result: unknown): LlmRoutingPolicy[] => {
    if (Array.isArray(result)) return result as LlmRoutingPolicy[];
    ...
    return Array.isArray(candidate) ? (candidate as LlmRoutingPolicy[]) : [];
  };

  const mergeFetchedRows = (incomingRows: LlmRoutingPolicy[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addLlmRoutingPolicy] = useAddLlmRoutingPolicyMutation();
  const [updateLlmRoutingPolicy] = useUpdateLlmRoutingPolicyMutation();
  const [deleteLlmRoutingPolicy] = useDeleteLlmRoutingPolicyMutation();
  const [deleteLlmRoutingPolicyCascade] = useDeleteLlmRoutingPolicyCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<LlmRoutingPolicy>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<LlmRoutingPolicy | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<LlmRoutingPolicy | null> => {
    ...
    try {
      ...
      return (await response.json()) as LlmRoutingPolicy;
    } catch (err) {
      console.warn('Could not reload LlmRoutingPolicy row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<LlmRoutingPolicy>,
    successField?: string,
  ) => {
    try {
      const saved = await updateLlmRoutingPolicy(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as LlmRoutingPolicy[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update LlmRoutingPolicy';
      console.error('Failed to update LlmRoutingPolicy:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteLlmRoutingPolicyCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteLlmRoutingPolicy(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<LlmRoutingPolicy> = { ...source };
      delete (payload as any).id;
      const created = await addLlmRoutingPolicy(payload).unwrap();
      setAllData((prev) => [created as LlmRoutingPolicy, ...prev]);
    } catch (error) {
      console.error('Failed to add LlmRoutingPolicy:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<LlmRoutingPolicy> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addLlmRoutingPolicy(payload).unwrap();
      setAllData((prev) => [created as LlmRoutingPolicy, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) LlmRoutingPolicy:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `LlmRoutingPolicy-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addLlmRoutingPolicy(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateLlmRoutingPolicy(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as LlmRoutingPolicy[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as LlmRoutingPolicy[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as LlmRoutingPolicy[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as LlmRoutingPolicy[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="LlmRoutingPolicyTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="LlmRoutingPolicy Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create LlmRoutingPolicy</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <LlmRoutingPolicyForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter LlmRoutingPolicy (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing LlmRoutingPolicy</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof LlmRoutingPolicy;
                ...
                await updateLlmRoutingPolicy(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of LlmRoutingPolicys.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'LlmRoutingPolicy'}
...
export default LlmRoutingPolicyTable;
