...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/DiscardDeadLetterEntryRequest/${rowId}`;
...
const DiscardDeadLetterEntryRequestTable: React.FC = () => {
  ...
  const [deleteDiscardDeadLetterEntryRequest] =
    useDeleteDiscardDeadLetterEntryRequestMutation();
  const [deleteDiscardDeadLetterEntryRequestCascade] =
    useDeleteDiscardDeadLetterEntryRequestCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<DiscardDeadLetterEntryRequest | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload DiscardDeadLetterEntryRequest row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateDiscardDeadLetterEntryRequest(
        updatedItem as any,
      ).unwrap();
      ...
      setAllData(
        (prev) =>
          mergeUpdatedRow(
            ...
            rowId,
            hydrated as any,
          ) as DiscardDeadLetterEntryRequest[],
      );
      if (successField) {
