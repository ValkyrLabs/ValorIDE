...
import { FileMetadata } from '@thorapi/model';
import FileMetadataForm from '@thorapi/redux/components/form/FileMetadataForm';
...
} from '@thorapi/redux/services/FileMetadataService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `FileMetadata update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/FileMetadata/${rowId}`;
...
const FileMetadataTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): FileMetadata[] => {
    if (Array.isArray(result)) return result as FileMetadata[];
    ...
    return Array.isArray(candidate) ? (candidate as FileMetadata[]) : [];
  };

  const mergeFetchedRows = (incomingRows: FileMetadata[], requestedPage: number) => {
  ...
  const [deleteFileMetadataCascade] = useDeleteFileMetadataCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<FileMetadata | null> => {
    ...
    try {
      ...
      console.warn('Could not reload FileMetadata row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<FileMetadata>,
    successField?: string,
  ) => {
    try {
      const saved = await updateFileMetadata(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as FileMetadata[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update FileMetadata';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteFileMetadataCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `FileMetadata-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addFileMetadata(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateFileMetadata(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as FileMetadata[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as FileMetadata[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as FileMetadata[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as FileMetadata[]);
    }
  };
