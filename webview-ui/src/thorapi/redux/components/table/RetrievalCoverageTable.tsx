...
import { RetrievalCoverage } from '@thorapi/model';
import RetrievalCoverageForm from '@thorapi/redux/components/form/RetrievalCoverageForm';
...
import {
  useLazyGetRetrievalCoveragesPagedQuery,
...
} from '@thorapi/redux/services/RetrievalCoverageService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'coverageStatus': { type: 'enum', enumValues: [
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RetrievalCoverage update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RetrievalCoverage/${rowId}`;

const RetrievalCoverageTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RetrievalCoverage[]>([]);
  ...
  const normalizePageRows = (result: unknown): RetrievalCoverage[] => {
    if (Array.isArray(result)) return result as RetrievalCoverage[];
    ...
    return Array.isArray(candidate) ? (candidate as RetrievalCoverage[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RetrievalCoverage[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addRetrievalCoverage] = useAddRetrievalCoverageMutation();
  const [updateRetrievalCoverage] = useUpdateRetrievalCoverageMutation();
  const [deleteRetrievalCoverage] = useDeleteRetrievalCoverageMutation();
  const [deleteRetrievalCoverageCascade] = useDeleteRetrievalCoverageCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<RetrievalCoverage>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RetrievalCoverage | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<RetrievalCoverage | null> => {
    ...
    try {
      ...
      return (await response.json()) as RetrievalCoverage;
    } catch (err) {
      console.warn('Could not reload RetrievalCoverage row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RetrievalCoverage>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRetrievalCoverage(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RetrievalCoverage[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RetrievalCoverage';
      console.error('Failed to update RetrievalCoverage:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRetrievalCoverageCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteRetrievalCoverage(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RetrievalCoverage> = { ...source };
      delete (payload as any).id;
      const created = await addRetrievalCoverage(payload).unwrap();
      setAllData((prev) => [created as RetrievalCoverage, ...prev]);
    } catch (error) {
      console.error('Failed to add RetrievalCoverage:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RetrievalCoverage> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRetrievalCoverage(payload).unwrap();
      setAllData((prev) => [created as RetrievalCoverage, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) RetrievalCoverage:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RetrievalCoverage-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRetrievalCoverage(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRetrievalCoverage(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RetrievalCoverage[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RetrievalCoverage[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RetrievalCoverage[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RetrievalCoverage[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="RetrievalCoverageTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="RetrievalCoverage Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RetrievalCoverage</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RetrievalCoverageForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RetrievalCoverage (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RetrievalCoverage</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RetrievalCoverage;
                ...
                await updateRetrievalCoverage(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RetrievalCoverages.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RetrievalCoverage'}
...
export default RetrievalCoverageTable;
