...
import { TrustKeyRotation } from '@thorapi/model';
import TrustKeyRotationForm from '@thorapi/redux/components/form/TrustKeyRotationForm';
...
import {
  useLazyGetTrustKeyRotationsPagedQuery,
...
} from '@thorapi/redux/services/TrustKeyRotationService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'rotationStatus': { type: 'enum', enumValues: [
    'REQUESTED', 'APPROVED', 'ACTIVATED', 'REWRAP_RUNNING', 'REWRAP_COMPLETE', 'RETIREMENT_RUNNING', 'COMPLETE', 'FAILED', 
  ], enumValueType: 'string' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TrustKeyRotation update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TrustKeyRotation/${rowId}`;

const TrustKeyRotationTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TrustKeyRotation[]>([]);
  ...
  const normalizePageRows = (result: unknown): TrustKeyRotation[] => {
    if (Array.isArray(result)) return result as TrustKeyRotation[];
    ...
    return Array.isArray(candidate) ? (candidate as TrustKeyRotation[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TrustKeyRotation[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addTrustKeyRotation] = useAddTrustKeyRotationMutation();
  const [updateTrustKeyRotation] = useUpdateTrustKeyRotationMutation();
  const [deleteTrustKeyRotation] = useDeleteTrustKeyRotationMutation();
  const [deleteTrustKeyRotationCascade] = useDeleteTrustKeyRotationCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TrustKeyRotation>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TrustKeyRotation | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TrustKeyRotation | null> => {
    ...
    try {
      ...
      return (await response.json()) as TrustKeyRotation;
    } catch (err) {
      console.warn('Could not reload TrustKeyRotation row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TrustKeyRotation>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTrustKeyRotation(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TrustKeyRotation[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TrustKeyRotation';
      console.error('Failed to update TrustKeyRotation:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustKeyRotationCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTrustKeyRotation(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TrustKeyRotation> = { ...source };
      delete (payload as any).id;
      const created = await addTrustKeyRotation(payload).unwrap();
      setAllData((prev) => [created as TrustKeyRotation, ...prev]);
    } catch (error) {
      console.error('Failed to add TrustKeyRotation:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TrustKeyRotation> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTrustKeyRotation(payload).unwrap();
      setAllData((prev) => [created as TrustKeyRotation, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TrustKeyRotation:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TrustKeyRotation-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTrustKeyRotation(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTrustKeyRotation(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TrustKeyRotation[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TrustKeyRotation[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TrustKeyRotation[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TrustKeyRotation[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="TrustKeyRotationTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="TrustKeyRotation Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TrustKeyRotation</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TrustKeyRotationForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TrustKeyRotation (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TrustKeyRotation</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TrustKeyRotation;
                ...
                await updateTrustKeyRotation(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TrustKeyRotations.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TrustKeyRotation'}
...
export default TrustKeyRotationTable;
