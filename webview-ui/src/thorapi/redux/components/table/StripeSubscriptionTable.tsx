...
import { StripeSubscription } from '@thorapi/model';
import StripeSubscriptionForm from '@thorapi/redux/components/form/StripeSubscriptionForm';
...
import {
  useLazyGetStripeSubscriptionsPagedQuery,
...
} from '@thorapi/redux/services/StripeSubscriptionService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'status': { type: 'enum', enumValues: [
    'incomplete', 'trialing', 'active', 'past_due', 'canceled', 'unpaid', 'paused', 'deleted', 'unknown', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `StripeSubscription update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/StripeSubscription/${rowId}`;

const StripeSubscriptionTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<StripeSubscription[]>([]);
  ...
  const normalizePageRows = (result: unknown): StripeSubscription[] => {
    if (Array.isArray(result)) return result as StripeSubscription[];
    ...
    return Array.isArray(candidate) ? (candidate as StripeSubscription[]) : [];
  };

  const mergeFetchedRows = (incomingRows: StripeSubscription[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addStripeSubscription] = useAddStripeSubscriptionMutation();
  const [updateStripeSubscription] = useUpdateStripeSubscriptionMutation();
  const [deleteStripeSubscription] = useDeleteStripeSubscriptionMutation();
  const [deleteStripeSubscriptionCascade] = useDeleteStripeSubscriptionCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<StripeSubscription>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<StripeSubscription | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<StripeSubscription | null> => {
    ...
    try {
      ...
      console.warn('Could not reload StripeSubscription row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<StripeSubscription>,
    successField?: string,
  ) => {
    try {
      const saved = await updateStripeSubscription(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as StripeSubscription[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update StripeSubscription';
      console.error('Failed to update StripeSubscription:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteStripeSubscriptionCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteStripeSubscription(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<StripeSubscription> = { ...source };
      delete (payload as any).id;
      const created = await addStripeSubscription(payload).unwrap();
      setAllData((prev) => [created as StripeSubscription, ...prev]);
    } catch (error) {
      console.error('Failed to add StripeSubscription:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<StripeSubscription> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addStripeSubscription(payload).unwrap();
      setAllData((prev) => [created as StripeSubscription, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) StripeSubscription:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `StripeSubscription-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addStripeSubscription(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateStripeSubscription(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as StripeSubscription[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as StripeSubscription[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as StripeSubscription[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as StripeSubscription[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="StripeSubscriptionTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="StripeSubscription Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create StripeSubscription</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter StripeSubscription (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing StripeSubscription</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof StripeSubscription;
                ...
                await updateStripeSubscription(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of StripeSubscriptions.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'StripeSubscription'}
...
export default StripeSubscriptionTable;
