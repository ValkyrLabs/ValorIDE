...
import { WorkflowGraphNode } from '@thorapi/model';
import WorkflowGraphNodeForm from '@thorapi/redux/components/form/WorkflowGraphNodeForm';
...
import {
  useLazyGetWorkflowGraphNodesPagedQuery,
...
} from '@thorapi/redux/services/WorkflowGraphNodeService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `WorkflowGraphNode update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/WorkflowGraphNode/${rowId}`;

const WorkflowGraphNodeTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<WorkflowGraphNode[]>([]);
  ...
  const normalizePageRows = (result: unknown): WorkflowGraphNode[] => {
    if (Array.isArray(result)) return result as WorkflowGraphNode[];
    ...
    return Array.isArray(candidate) ? (candidate as WorkflowGraphNode[]) : [];
  };

  const mergeFetchedRows = (incomingRows: WorkflowGraphNode[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addWorkflowGraphNode] = useAddWorkflowGraphNodeMutation();
  const [updateWorkflowGraphNode] = useUpdateWorkflowGraphNodeMutation();
  const [deleteWorkflowGraphNode] = useDeleteWorkflowGraphNodeMutation();
  const [deleteWorkflowGraphNodeCascade] = useDeleteWorkflowGraphNodeCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<WorkflowGraphNode>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<WorkflowGraphNode | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<WorkflowGraphNode | null> => {
    ...
    try {
      ...
      return (await response.json()) as WorkflowGraphNode;
    } catch (err) {
      console.warn('Could not reload WorkflowGraphNode row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<WorkflowGraphNode>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWorkflowGraphNode(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as WorkflowGraphNode[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update WorkflowGraphNode';
      console.error('Failed to update WorkflowGraphNode:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphNodeCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteWorkflowGraphNode(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<WorkflowGraphNode> = { ...source };
      delete (payload as any).id;
      const created = await addWorkflowGraphNode(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphNode, ...prev]);
    } catch (error) {
      console.error('Failed to add WorkflowGraphNode:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<WorkflowGraphNode> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addWorkflowGraphNode(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphNode, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) WorkflowGraphNode:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `WorkflowGraphNode-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWorkflowGraphNode(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWorkflowGraphNode(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as WorkflowGraphNode[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as WorkflowGraphNode[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as WorkflowGraphNode[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as WorkflowGraphNode[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="WorkflowGraphNodeTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="WorkflowGraphNode Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create WorkflowGraphNode</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter WorkflowGraphNode (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing WorkflowGraphNode</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof WorkflowGraphNode;
                ...
                await updateWorkflowGraphNode(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of WorkflowGraphNodes.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'WorkflowGraphNode'}
...
export default WorkflowGraphNodeTable;
