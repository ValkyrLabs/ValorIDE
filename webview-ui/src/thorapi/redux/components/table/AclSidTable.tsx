...
import { AclSid } from '@thorapi/model';
import AclSidForm from '@thorapi/redux/components/form/AclSidForm';
...
} from '@thorapi/redux/services/AclSidService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AclSid update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AclSid/${rowId}`;
...
const AclSidTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): AclSid[] => {
    if (Array.isArray(result)) return result as AclSid[];
    ...
    return Array.isArray(candidate) ? (candidate as AclSid[]) : [];
  };

  const mergeFetchedRows = (incomingRows: AclSid[], requestedPage: number) => {
  ...
  const [deleteAclSidCascade] = useDeleteAclSidCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<AclSid | null> => {
    ...
    try {
      ...
      return (await response.json()) as AclSid;
    } catch (err) {
      console.warn('Could not reload AclSid row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AclSid>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAclSid(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AclSid[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AclSid';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAclSidCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AclSid-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAclSid(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAclSid(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AclSid[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AclSid[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AclSid[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AclSid[]);
    }
  };
