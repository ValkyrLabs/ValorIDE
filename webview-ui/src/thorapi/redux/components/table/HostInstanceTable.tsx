...
import { HostInstance } from '@thorapi/model';
import HostInstanceForm from '@thorapi/redux/components/form/HostInstanceForm';
...
} from '@thorapi/redux/services/HostInstanceService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `HostInstance update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/HostInstance/${rowId}`;
...
const HostInstanceTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): HostInstance[] => {
    if (Array.isArray(result)) return result as HostInstance[];
    ...
    return Array.isArray(candidate) ? (candidate as HostInstance[]) : [];
  };

  const mergeFetchedRows = (incomingRows: HostInstance[], requestedPage: number) => {
  ...
  const [deleteHostInstanceCascade] = useDeleteHostInstanceCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<HostInstance | null> => {
    ...
    try {
      ...
      return (await response.json()) as HostInstance;
    } catch (err) {
      console.warn('Could not reload HostInstance row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<HostInstance>,
    successField?: string,
  ) => {
    try {
      const saved = await updateHostInstance(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as HostInstance[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update HostInstance';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteHostInstanceCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `HostInstance-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addHostInstance(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateHostInstance(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as HostInstance[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as HostInstance[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as HostInstance[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as HostInstance[]);
    }
  };
