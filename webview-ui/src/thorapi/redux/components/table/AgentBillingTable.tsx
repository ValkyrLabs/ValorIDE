...
import { AgentBilling } from '@thorapi/model';
import AgentBillingForm from '@thorapi/redux/components/form/AgentBillingForm';
...
} from '@thorapi/redux/services/AgentBillingService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'chargingModel': { type: 'enum', enumValues: [
    'PER_INSTANCE', 'USAGE_BASED', 'FLAT_RATE', 
  ], enumValueType: 'string' },
  'billingStatus': { type: 'enum', enumValues: [
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AgentBilling update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AgentBilling/${rowId}`;
...
const AgentBillingTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): AgentBilling[] => {
    if (Array.isArray(result)) return result as AgentBilling[];
    ...
    return Array.isArray(candidate) ? (candidate as AgentBilling[]) : [];
  };

  const mergeFetchedRows = (incomingRows: AgentBilling[], requestedPage: number) => {
  ...
  const [deleteAgentBillingCascade] = useDeleteAgentBillingCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<AgentBilling | null> => {
    ...
    try {
      ...
      return (await response.json()) as AgentBilling;
    } catch (err) {
      console.warn('Could not reload AgentBilling row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AgentBilling>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAgentBilling(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AgentBilling[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AgentBilling';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAgentBillingCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AgentBilling-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAgentBilling(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAgentBilling(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AgentBilling[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AgentBilling[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AgentBilling[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AgentBilling[]);
    }
  };
