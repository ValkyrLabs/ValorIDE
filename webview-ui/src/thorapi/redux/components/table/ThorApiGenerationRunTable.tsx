...
import { ThorApiGenerationRun } from '@thorapi/model';
import ThorApiGenerationRunForm from '@thorapi/redux/components/form/ThorApiGenerationRunForm';
...
import {
  useLazyGetThorApiGenerationRunsPagedQuery,
...
} from '@thorapi/redux/services/ThorApiGenerationRunService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ThorApiGenerationRun update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ThorApiGenerationRun/${rowId}`;

const ThorApiGenerationRunTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ThorApiGenerationRun[]>([]);
  ...
  const normalizePageRows = (result: unknown): ThorApiGenerationRun[] => {
    if (Array.isArray(result)) return result as ThorApiGenerationRun[];
    ...
    return Array.isArray(candidate) ? (candidate as ThorApiGenerationRun[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ThorApiGenerationRun[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addThorApiGenerationRun] = useAddThorApiGenerationRunMutation();
  const [updateThorApiGenerationRun] = useUpdateThorApiGenerationRunMutation();
  const [deleteThorApiGenerationRun] = useDeleteThorApiGenerationRunMutation();
  const [deleteThorApiGenerationRunCascade] = useDeleteThorApiGenerationRunCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ThorApiGenerationRun>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ThorApiGenerationRun | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ThorApiGenerationRun | null> => {
    ...
    try {
      ...
      return (await response.json()) as ThorApiGenerationRun;
    } catch (err) {
      console.warn('Could not reload ThorApiGenerationRun row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ThorApiGenerationRun>,
    successField?: string,
  ) => {
    try {
      const saved = await updateThorApiGenerationRun(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ThorApiGenerationRun[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ThorApiGenerationRun';
      console.error('Failed to update ThorApiGenerationRun:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteThorApiGenerationRunCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteThorApiGenerationRun(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ThorApiGenerationRun> = { ...source };
      delete (payload as any).id;
      const created = await addThorApiGenerationRun(payload).unwrap();
      setAllData((prev) => [created as ThorApiGenerationRun, ...prev]);
    } catch (error) {
      console.error('Failed to add ThorApiGenerationRun:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ThorApiGenerationRun> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addThorApiGenerationRun(payload).unwrap();
      setAllData((prev) => [created as ThorApiGenerationRun, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ThorApiGenerationRun:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ThorApiGenerationRun-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addThorApiGenerationRun(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateThorApiGenerationRun(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ThorApiGenerationRun[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ThorApiGenerationRun[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ThorApiGenerationRun[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ThorApiGenerationRun[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ThorApiGenerationRunTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ThorApiGenerationRun Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ThorApiGenerationRun</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ThorApiGenerationRunForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ThorApiGenerationRun (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ThorApiGenerationRun</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ThorApiGenerationRun;
                ...
                await updateThorApiGenerationRun(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ThorApiGenerationRuns.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ThorApiGenerationRun'}
...
export default ThorApiGenerationRunTable;
