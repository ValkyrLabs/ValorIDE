...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/WorkflowGraphWorkflowData/${rowId}`;
...
const WorkflowGraphWorkflowDataTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addWorkflowGraphWorkflowData] =
    useAddWorkflowGraphWorkflowDataMutation();
  const [updateWorkflowGraphWorkflowData] =
    useUpdateWorkflowGraphWorkflowDataMutation();
  const [deleteWorkflowGraphWorkflowData] =
    useDeleteWorkflowGraphWorkflowDataMutation();
  const [deleteWorkflowGraphWorkflowDataCascade] =
    useDeleteWorkflowGraphWorkflowDataCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<WorkflowGraphWorkflowData | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<WorkflowGraphWorkflowData | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload WorkflowGraphWorkflowData row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateWorkflowGraphWorkflowData(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update WorkflowGraphWorkflowData";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphWorkflowDataCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) WorkflowGraphWorkflowData:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter WorkflowGraphWorkflowData (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
