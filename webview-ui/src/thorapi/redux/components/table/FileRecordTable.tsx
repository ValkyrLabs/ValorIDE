...
import { FileRecord } from '@thorapi/model';
import FileRecordForm from '@thorapi/redux/components/form/FileRecordForm';
...
} from '@thorapi/redux/services/FileRecordService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'virusScanStatus': { type: 'enum', enumValues: [
    'PENDING', 'IN_PROGRESS', 'CLEAN', 'SUSPICIOUS', 'INFECTED', 'SCAN_FAILED', 'QUARANTINED', 'BYPASSED_BY_POLICY', 'SKIPPED', 
  ], enumValueType: 'string' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `FileRecord update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/FileRecord/${rowId}`;
...
const FileRecordTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): FileRecord[] => {
    if (Array.isArray(result)) return result as FileRecord[];
    ...
    return Array.isArray(candidate) ? (candidate as FileRecord[]) : [];
  };

  const mergeFetchedRows = (incomingRows: FileRecord[], requestedPage: number) => {
  ...
  const [deleteFileRecordCascade] = useDeleteFileRecordCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<FileRecord | null> => {
    ...
    try {
      ...
      console.warn('Could not reload FileRecord row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<FileRecord>,
    successField?: string,
  ) => {
    try {
      const saved = await updateFileRecord(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as FileRecord[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update FileRecord';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteFileRecordCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `FileRecord-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addFileRecord(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateFileRecord(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as FileRecord[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as FileRecord[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as FileRecord[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as FileRecord[]);
    }
  };
