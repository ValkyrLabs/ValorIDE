...
import { StrategicPriority } from '@thorapi/model';
import StrategicPriorityForm from '@thorapi/redux/components/form/StrategicPriorityForm';
...
} from '@thorapi/redux/services/StrategicPriorityService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `StrategicPriority update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/StrategicPriority/${rowId}`;
...
const StrategicPriorityTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): StrategicPriority[] => {
    if (Array.isArray(result)) return result as StrategicPriority[];
    ...
    return Array.isArray(candidate) ? (candidate as StrategicPriority[]) : [];
  };

  const mergeFetchedRows = (incomingRows: StrategicPriority[], requestedPage: number) => {
  ...
  const [deleteStrategicPriorityCascade] = useDeleteStrategicPriorityCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<StrategicPriority | null> => {
    ...
    try {
      ...
      return (await response.json()) as StrategicPriority;
    } catch (err) {
      console.warn('Could not reload StrategicPriority row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<StrategicPriority>,
    successField?: string,
  ) => {
    try {
      const saved = await updateStrategicPriority(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as StrategicPriority[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update StrategicPriority';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteStrategicPriorityCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `StrategicPriority-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addStrategicPriority(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateStrategicPriority(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as StrategicPriority[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as StrategicPriority[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as StrategicPriority[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as StrategicPriority[]);
    }
  };
