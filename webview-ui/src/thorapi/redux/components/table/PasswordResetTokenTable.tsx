...
import { PasswordResetToken } from '@thorapi/model';
import PasswordResetTokenForm from '@thorapi/redux/components/form/PasswordResetTokenForm';
...
import {
  useLazyGetPasswordResetTokensPagedQuery,
...
} from '@thorapi/redux/services/PasswordResetTokenService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `PasswordResetToken update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/PasswordResetToken/${rowId}`;

const PasswordResetTokenTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<PasswordResetToken[]>([]);
  ...
  const normalizePageRows = (result: unknown): PasswordResetToken[] => {
    if (Array.isArray(result)) return result as PasswordResetToken[];
    ...
    return Array.isArray(candidate) ? (candidate as PasswordResetToken[]) : [];
  };

  const mergeFetchedRows = (incomingRows: PasswordResetToken[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addPasswordResetToken] = useAddPasswordResetTokenMutation();
  const [updatePasswordResetToken] = useUpdatePasswordResetTokenMutation();
  const [deletePasswordResetToken] = useDeletePasswordResetTokenMutation();
  const [deletePasswordResetTokenCascade] = useDeletePasswordResetTokenCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<PasswordResetToken>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<PasswordResetToken | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<PasswordResetToken | null> => {
    ...
    try {
      ...
      return (await response.json()) as PasswordResetToken;
    } catch (err) {
      console.warn('Could not reload PasswordResetToken row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<PasswordResetToken>,
    successField?: string,
  ) => {
    try {
      const saved = await updatePasswordResetToken(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as PasswordResetToken[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update PasswordResetToken';
      console.error('Failed to update PasswordResetToken:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePasswordResetTokenCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deletePasswordResetToken(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<PasswordResetToken> = { ...source };
      delete (payload as any).id;
      const created = await addPasswordResetToken(payload).unwrap();
      setAllData((prev) => [created as PasswordResetToken, ...prev]);
    } catch (error) {
      console.error('Failed to add PasswordResetToken:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<PasswordResetToken> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addPasswordResetToken(payload).unwrap();
      setAllData((prev) => [created as PasswordResetToken, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) PasswordResetToken:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `PasswordResetToken-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addPasswordResetToken(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updatePasswordResetToken(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as PasswordResetToken[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as PasswordResetToken[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as PasswordResetToken[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as PasswordResetToken[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.PasswordResetToken',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="PasswordResetTokenTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="PasswordResetToken Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create PasswordResetToken</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter PasswordResetToken (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing PasswordResetToken</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof PasswordResetToken;
                ...
                await updatePasswordResetToken(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of PasswordResetTokens.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'PasswordResetToken'}
...
export default PasswordResetTokenTable;
