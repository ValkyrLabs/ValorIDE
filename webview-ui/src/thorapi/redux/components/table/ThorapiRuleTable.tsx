...
import { ThorapiRule } from '@thorapi/model';
import ThorapiRuleForm from '@thorapi/redux/components/form/ThorapiRuleForm';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ThorapiRule update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ThorapiRule/${rowId}`;

const ThorapiRuleTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ThorapiRule[]>([]);
  ...
  const normalizePageRows = (result: unknown): ThorapiRule[] => {
    if (Array.isArray(result)) return result as ThorapiRule[];
    ...
    return Array.isArray(candidate) ? (candidate as ThorapiRule[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ThorapiRule[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addThorapiRule] = useAddThorapiRuleMutation();
  const [updateThorapiRule] = useUpdateThorapiRuleMutation();
  const [deleteThorapiRule] = useDeleteThorapiRuleMutation();
  const [deleteThorapiRuleCascade] = useDeleteThorapiRuleCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ThorapiRule>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ThorapiRule | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ThorapiRule | null> => {
    ...
    try {
      ...
      return (await response.json()) as ThorapiRule;
    } catch (err) {
      console.warn('Could not reload ThorapiRule row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ThorapiRule>,
    successField?: string,
  ) => {
    try {
      const saved = await updateThorapiRule(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ThorapiRule[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ThorapiRule';
      console.error('Failed to update ThorapiRule:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteThorapiRuleCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteThorapiRule(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ThorapiRule> = { ...source };
      delete (payload as any).id;
      const created = await addThorapiRule(payload).unwrap();
      setAllData((prev) => [created as ThorapiRule, ...prev]);
    } catch (error) {
      console.error('Failed to add ThorapiRule:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ThorapiRule> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addThorapiRule(payload).unwrap();
      setAllData((prev) => [created as ThorapiRule, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ThorapiRule:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ThorapiRule-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addThorapiRule(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateThorapiRule(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ThorapiRule[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ThorapiRule[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ThorapiRule[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ThorapiRule[]);
    }
  };
    ...
    <>
          ...
          >
            ...
            <FloatingControlPanel
              description="ThorapiRule Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ThorapiRule</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ThorapiRuleForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ThorapiRule (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ThorapiRule</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ThorapiRule;
                ...
                await updateThorapiRule(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ThorapiRules.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ThorapiRule'}
...
export default ThorapiRuleTable;
