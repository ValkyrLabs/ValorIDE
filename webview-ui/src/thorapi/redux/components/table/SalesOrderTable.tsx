...
import { SalesOrder } from '@thorapi/model';
import SalesOrderForm from '@thorapi/redux/components/form/SalesOrderForm';
...
} from '@thorapi/redux/services/SalesOrderService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SalesOrder update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SalesOrder/${rowId}`;
...
const SalesOrderTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SalesOrder[] => {
    ...
    return Array.isArray(candidate) ? (candidate as SalesOrder[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SalesOrder[], requestedPage: number) => {
  ...
  const reloadRowById = async (rowId: string): Promise<SalesOrder | null> => {
    ...
    try {
      ...
      console.warn('Could not reload SalesOrder row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SalesOrder>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSalesOrder(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SalesOrder[]);
      if (successField) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSalesOrderCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SalesOrder-export-${stamp}.json`;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SalesOrder[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SalesOrder[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SalesOrder[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SalesOrder[]);
    }
  };
