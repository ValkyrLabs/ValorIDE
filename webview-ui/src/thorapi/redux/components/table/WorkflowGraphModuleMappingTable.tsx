...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/WorkflowGraphModuleMapping/${rowId}`;
...
const WorkflowGraphModuleMappingTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addWorkflowGraphModuleMapping] =
    useAddWorkflowGraphModuleMappingMutation();
  const [updateWorkflowGraphModuleMapping] =
    useUpdateWorkflowGraphModuleMappingMutation();
  const [deleteWorkflowGraphModuleMapping] =
    useDeleteWorkflowGraphModuleMappingMutation();
  const [deleteWorkflowGraphModuleMappingCascade] =
    useDeleteWorkflowGraphModuleMappingCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<WorkflowGraphModuleMapping | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<WorkflowGraphModuleMapping | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload WorkflowGraphModuleMapping row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateWorkflowGraphModuleMapping(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update WorkflowGraphModuleMapping";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphModuleMappingCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) WorkflowGraphModuleMapping:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter WorkflowGraphModuleMapping (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
