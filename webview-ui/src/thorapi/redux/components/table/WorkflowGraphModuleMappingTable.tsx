...
import WorkflowGraphModuleMappingForm from '@thorapi/redux/components/form/WorkflowGraphModuleMappingForm';
...
import {
  useLazyGetWorkflowGraphModuleMappingsPagedQuery,
...
} from '@thorapi/redux/services/WorkflowGraphModuleMappingService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `WorkflowGraphModuleMapping update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/WorkflowGraphModuleMapping/${rowId}`;

const WorkflowGraphModuleMappingTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<WorkflowGraphModuleMapping[]>([]);
  ...
  const normalizePageRows = (result: unknown): WorkflowGraphModuleMapping[] => {
    if (Array.isArray(result)) return result as WorkflowGraphModuleMapping[];
    ...
    return Array.isArray(candidate) ? (candidate as WorkflowGraphModuleMapping[]) : [];
  };

  const mergeFetchedRows = (incomingRows: WorkflowGraphModuleMapping[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addWorkflowGraphModuleMapping] = useAddWorkflowGraphModuleMappingMutation();
  const [updateWorkflowGraphModuleMapping] = useUpdateWorkflowGraphModuleMappingMutation();
  const [deleteWorkflowGraphModuleMapping] = useDeleteWorkflowGraphModuleMappingMutation();
  const [deleteWorkflowGraphModuleMappingCascade] = useDeleteWorkflowGraphModuleMappingCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<WorkflowGraphModuleMapping>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<WorkflowGraphModuleMapping | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<WorkflowGraphModuleMapping | null> => {
    ...
    try {
      ...
      return (await response.json()) as WorkflowGraphModuleMapping;
    } catch (err) {
      console.warn('Could not reload WorkflowGraphModuleMapping row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<WorkflowGraphModuleMapping>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWorkflowGraphModuleMapping(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as WorkflowGraphModuleMapping[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update WorkflowGraphModuleMapping';
      console.error('Failed to update WorkflowGraphModuleMapping:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphModuleMappingCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteWorkflowGraphModuleMapping(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<WorkflowGraphModuleMapping> = { ...source };
      delete (payload as any).id;
      const created = await addWorkflowGraphModuleMapping(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphModuleMapping, ...prev]);
    } catch (error) {
      console.error('Failed to add WorkflowGraphModuleMapping:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<WorkflowGraphModuleMapping> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addWorkflowGraphModuleMapping(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphModuleMapping, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) WorkflowGraphModuleMapping:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `WorkflowGraphModuleMapping-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWorkflowGraphModuleMapping(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWorkflowGraphModuleMapping(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as WorkflowGraphModuleMapping[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as WorkflowGraphModuleMapping[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as WorkflowGraphModuleMapping[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as WorkflowGraphModuleMapping[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="WorkflowGraphModuleMappingTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="WorkflowGraphModuleMapping Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create WorkflowGraphModuleMapping</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <WorkflowGraphModuleMappingForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter WorkflowGraphModuleMapping (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing WorkflowGraphModuleMapping</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof WorkflowGraphModuleMapping;
                ...
                await updateWorkflowGraphModuleMapping(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of WorkflowGraphModuleMappings.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'WorkflowGraphModuleMapping'}
...
export default WorkflowGraphModuleMappingTable;
