...
import { SubscriberList } from '@thorapi/model';
import SubscriberListForm from '@thorapi/redux/components/form/SubscriberListForm';
...
import {
  useLazyGetSubscriberListsPagedQuery,
...
} from '@thorapi/redux/services/SubscriberListService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SubscriberList update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SubscriberList/${rowId}`;

const SubscriberListTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SubscriberList[]>([]);
  ...
  const normalizePageRows = (result: unknown): SubscriberList[] => {
    if (Array.isArray(result)) return result as SubscriberList[];
    ...
    return Array.isArray(candidate) ? (candidate as SubscriberList[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SubscriberList[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addSubscriberList] = useAddSubscriberListMutation();
  const [updateSubscriberList] = useUpdateSubscriberListMutation();
  const [deleteSubscriberList] = useDeleteSubscriberListMutation();
  const [deleteSubscriberListCascade] = useDeleteSubscriberListCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<SubscriberList>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SubscriberList | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<SubscriberList | null> => {
    ...
    try {
      ...
      console.warn('Could not reload SubscriberList row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SubscriberList>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSubscriberList(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SubscriberList[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SubscriberList';
      console.error('Failed to update SubscriberList:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSubscriberListCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteSubscriberList(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SubscriberList> = { ...source };
      delete (payload as any).id;
      const created = await addSubscriberList(payload).unwrap();
      setAllData((prev) => [created as SubscriberList, ...prev]);
    } catch (error) {
      console.error('Failed to add SubscriberList:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SubscriberList> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSubscriberList(payload).unwrap();
      setAllData((prev) => [created as SubscriberList, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) SubscriberList:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SubscriberList-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSubscriberList(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSubscriberList(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SubscriberList[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SubscriberList[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SubscriberList[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SubscriberList[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.SubscriberList',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="SubscriberListTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="SubscriberList Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SubscriberList</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SubscriberListForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SubscriberList (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SubscriberList</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SubscriberList;
                ...
                await updateSubscriberList(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SubscriberLists.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SubscriberList'}
