...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/MemoryActionRequest/${rowId}`;
...
const MemoryActionRequestTable: React.FC = () => {
  ...
  const [deleteMemoryActionRequestCascade] =
    useDeleteMemoryActionRequestCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<MemoryActionRequest | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateMemoryActionRequest(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update MemoryActionRequest";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMemoryActionRequestCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter MemoryActionRequest (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
