...
import { MemoryActionRequest } from '@thorapi/model';
import MemoryActionRequestForm from '@thorapi/redux/components/form/MemoryActionRequestForm';
...
import {
  useLazyGetMemoryActionRequestsPagedQuery,
...
} from '@thorapi/redux/services/MemoryActionRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `MemoryActionRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/MemoryActionRequest/${rowId}`;

const MemoryActionRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<MemoryActionRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): MemoryActionRequest[] => {
    if (Array.isArray(result)) return result as MemoryActionRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as MemoryActionRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: MemoryActionRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addMemoryActionRequest] = useAddMemoryActionRequestMutation();
  const [updateMemoryActionRequest] = useUpdateMemoryActionRequestMutation();
  const [deleteMemoryActionRequest] = useDeleteMemoryActionRequestMutation();
  const [deleteMemoryActionRequestCascade] = useDeleteMemoryActionRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<MemoryActionRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<MemoryActionRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<MemoryActionRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as MemoryActionRequest;
    } catch (err) {
      console.warn('Could not reload MemoryActionRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<MemoryActionRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMemoryActionRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as MemoryActionRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update MemoryActionRequest';
      console.error('Failed to update MemoryActionRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMemoryActionRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteMemoryActionRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<MemoryActionRequest> = { ...source };
      delete (payload as any).id;
      const created = await addMemoryActionRequest(payload).unwrap();
      setAllData((prev) => [created as MemoryActionRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add MemoryActionRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<MemoryActionRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMemoryActionRequest(payload).unwrap();
      setAllData((prev) => [created as MemoryActionRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) MemoryActionRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `MemoryActionRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMemoryActionRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMemoryActionRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as MemoryActionRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as MemoryActionRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as MemoryActionRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as MemoryActionRequest[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="MemoryActionRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="MemoryActionRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create MemoryActionRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <MemoryActionRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter MemoryActionRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing MemoryActionRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof MemoryActionRequest;
                ...
                await updateMemoryActionRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of MemoryActionRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'MemoryActionRequest'}
...
export default MemoryActionRequestTable;
