...
import { AclClass } from '@thorapi/model';
import AclClassForm from '@thorapi/redux/components/form/AclClassForm';
...
} from '@thorapi/redux/services/AclClassService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AclClass update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AclClass/${rowId}`;
...
const AclClassTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): AclClass[] => {
    if (Array.isArray(result)) return result as AclClass[];
    ...
    return Array.isArray(candidate) ? (candidate as AclClass[]) : [];
  };

  const mergeFetchedRows = (incomingRows: AclClass[], requestedPage: number) => {
  ...
  const [deleteAclClassCascade] = useDeleteAclClassCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<AclClass | null> => {
    ...
    try {
      ...
      return (await response.json()) as AclClass;
    } catch (err) {
      console.warn('Could not reload AclClass row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AclClass>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAclClass(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AclClass[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AclClass';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAclClassCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AclClass-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAclClass(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAclClass(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AclClass[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AclClass[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AclClass[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AclClass[]);
    }
  };
