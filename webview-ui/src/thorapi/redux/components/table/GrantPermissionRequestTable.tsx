...
import { GrantPermissionRequest } from '@thorapi/model';
import GrantPermissionRequestForm from '@thorapi/redux/components/form/GrantPermissionRequestForm';
...
} from '@thorapi/redux/services/GrantPermissionRequestService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'subjectType': { type: 'enum', enumValues: [
    'user', 'org', 'space', 'role', 
  ], enumValueType: 'string' },
  'permission': { type: 'enum', enumValues: [
    'read', 'write', 'share', 'delete', 'admin', 
  ], enumValueType: 'string' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `GrantPermissionRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/GrantPermissionRequest/${rowId}`;
...
const GrantPermissionRequestTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): GrantPermissionRequest[] => {
    if (Array.isArray(result)) return result as GrantPermissionRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as GrantPermissionRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: GrantPermissionRequest[], requestedPage: number) => {
  ...
  const [deleteGrantPermissionRequestCascade] = useDeleteGrantPermissionRequestCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<GrantPermissionRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<GrantPermissionRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as GrantPermissionRequest;
    } catch (err) {
      console.warn('Could not reload GrantPermissionRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<GrantPermissionRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateGrantPermissionRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as GrantPermissionRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update GrantPermissionRequest';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteGrantPermissionRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `GrantPermissionRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addGrantPermissionRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateGrantPermissionRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as GrantPermissionRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as GrantPermissionRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as GrantPermissionRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as GrantPermissionRequest[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter GrantPermissionRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
