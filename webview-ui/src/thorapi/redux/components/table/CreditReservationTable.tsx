...
import { CreditReservation } from '@thorapi/model';
import CreditReservationForm from '@thorapi/redux/components/form/CreditReservationForm';
...
import {
  useLazyGetCreditReservationsPagedQuery,
...
} from '@thorapi/redux/services/CreditReservationService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'actionType': { type: 'enum', enumValues: [
    'thorapi_generation', 'graymatter_context_compile', 'graymatter_hydration', 'graymatter_recompression', 'workflow_run', 'model_call', 'openclaw_lightsail_launch', 'tenant_app_generation', 'other', 
  ], enumValueType: 'string' },
  'status': { type: 'enum', enumValues: [
    'reserved', 'debited', 'released', 'expired', 'failed', 
  ], enumValueType: 'string' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `CreditReservation update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/CreditReservation/${rowId}`;

const CreditReservationTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<CreditReservation[]>([]);
  ...
  const normalizePageRows = (result: unknown): CreditReservation[] => {
    if (Array.isArray(result)) return result as CreditReservation[];
    ...
    return Array.isArray(candidate) ? (candidate as CreditReservation[]) : [];
  };

  const mergeFetchedRows = (incomingRows: CreditReservation[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addCreditReservation] = useAddCreditReservationMutation();
  const [updateCreditReservation] = useUpdateCreditReservationMutation();
  const [deleteCreditReservation] = useDeleteCreditReservationMutation();
  const [deleteCreditReservationCascade] = useDeleteCreditReservationCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<CreditReservation>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<CreditReservation | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<CreditReservation | null> => {
    ...
    try {
      ...
      return (await response.json()) as CreditReservation;
    } catch (err) {
      console.warn('Could not reload CreditReservation row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<CreditReservation>,
    successField?: string,
  ) => {
    try {
      const saved = await updateCreditReservation(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as CreditReservation[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update CreditReservation';
      console.error('Failed to update CreditReservation:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCreditReservationCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteCreditReservation(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<CreditReservation> = { ...source };
      delete (payload as any).id;
      const created = await addCreditReservation(payload).unwrap();
      setAllData((prev) => [created as CreditReservation, ...prev]);
    } catch (error) {
      console.error('Failed to add CreditReservation:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<CreditReservation> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addCreditReservation(payload).unwrap();
      setAllData((prev) => [created as CreditReservation, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) CreditReservation:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `CreditReservation-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addCreditReservation(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateCreditReservation(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as CreditReservation[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as CreditReservation[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as CreditReservation[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as CreditReservation[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="CreditReservationTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="CreditReservation Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create CreditReservation</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <CreditReservationForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter CreditReservation (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing CreditReservation</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof CreditReservation;
                ...
                await updateCreditReservation(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of CreditReservations.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'CreditReservation'}
...
export default CreditReservationTable;
