...
import { PaymentTransaction } from '@thorapi/model';
import PaymentTransactionForm from '@thorapi/redux/components/form/PaymentTransactionForm';
...
} from '@thorapi/redux/services/PaymentTransactionService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `PaymentTransaction update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/PaymentTransaction/${rowId}`;
...
const PaymentTransactionTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): PaymentTransaction[] => {
    if (Array.isArray(result)) return result as PaymentTransaction[];
    ...
    return Array.isArray(candidate) ? (candidate as PaymentTransaction[]) : [];
  };

  const mergeFetchedRows = (incomingRows: PaymentTransaction[], requestedPage: number) => {
  ...
  const [deletePaymentTransactionCascade] = useDeletePaymentTransactionCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<PaymentTransaction | null> => {
    ...
    try {
      ...
      console.warn('Could not reload PaymentTransaction row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<PaymentTransaction>,
    successField?: string,
  ) => {
    try {
      const saved = await updatePaymentTransaction(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as PaymentTransaction[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update PaymentTransaction';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePaymentTransactionCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `PaymentTransaction-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addPaymentTransaction(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updatePaymentTransaction(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as PaymentTransaction[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as PaymentTransaction[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as PaymentTransaction[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as PaymentTransaction[]);
    }
  };
