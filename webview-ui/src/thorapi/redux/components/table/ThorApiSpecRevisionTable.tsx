...
import { ThorApiSpecRevision } from '@thorapi/model';
import ThorApiSpecRevisionForm from '@thorapi/redux/components/form/ThorApiSpecRevisionForm';
...
import {
  useLazyGetThorApiSpecRevisionsPagedQuery,
...
} from '@thorapi/redux/services/ThorApiSpecRevisionService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'status': { type: 'enum', enumValues: [
    'draft', 'generated', 'applied', 'failed', 'superseded', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ThorApiSpecRevision update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ThorApiSpecRevision/${rowId}`;

const ThorApiSpecRevisionTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ThorApiSpecRevision[]>([]);
  ...
  const normalizePageRows = (result: unknown): ThorApiSpecRevision[] => {
    if (Array.isArray(result)) return result as ThorApiSpecRevision[];
    ...
    return Array.isArray(candidate) ? (candidate as ThorApiSpecRevision[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ThorApiSpecRevision[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addThorApiSpecRevision] = useAddThorApiSpecRevisionMutation();
  const [updateThorApiSpecRevision] = useUpdateThorApiSpecRevisionMutation();
  const [deleteThorApiSpecRevision] = useDeleteThorApiSpecRevisionMutation();
  const [deleteThorApiSpecRevisionCascade] = useDeleteThorApiSpecRevisionCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ThorApiSpecRevision>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ThorApiSpecRevision | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ThorApiSpecRevision | null> => {
    ...
    try {
      ...
      return (await response.json()) as ThorApiSpecRevision;
    } catch (err) {
      console.warn('Could not reload ThorApiSpecRevision row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ThorApiSpecRevision>,
    successField?: string,
  ) => {
    try {
      const saved = await updateThorApiSpecRevision(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ThorApiSpecRevision[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ThorApiSpecRevision';
      console.error('Failed to update ThorApiSpecRevision:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteThorApiSpecRevisionCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteThorApiSpecRevision(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ThorApiSpecRevision> = { ...source };
      delete (payload as any).id;
      const created = await addThorApiSpecRevision(payload).unwrap();
      setAllData((prev) => [created as ThorApiSpecRevision, ...prev]);
    } catch (error) {
      console.error('Failed to add ThorApiSpecRevision:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ThorApiSpecRevision> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addThorApiSpecRevision(payload).unwrap();
      setAllData((prev) => [created as ThorApiSpecRevision, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ThorApiSpecRevision:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ThorApiSpecRevision-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addThorApiSpecRevision(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateThorApiSpecRevision(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ThorApiSpecRevision[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ThorApiSpecRevision[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ThorApiSpecRevision[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ThorApiSpecRevision[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ThorApiSpecRevisionTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ThorApiSpecRevision Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ThorApiSpecRevision</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ThorApiSpecRevisionForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ThorApiSpecRevision (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ThorApiSpecRevision</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ThorApiSpecRevision;
                ...
                await updateThorApiSpecRevision(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ThorApiSpecRevisions.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ThorApiSpecRevision'}
...
export default ThorApiSpecRevisionTable;
