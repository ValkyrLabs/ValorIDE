...
import { SwarmCommandRequest } from '@thorapi/model';
import SwarmCommandRequestForm from '@thorapi/redux/components/form/SwarmCommandRequestForm';
...
} from '@thorapi/redux/services/SwarmCommandRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SwarmCommandRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SwarmCommandRequest/${rowId}`;
...
const SwarmCommandRequestTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SwarmCommandRequest[] => {
    if (Array.isArray(result)) return result as SwarmCommandRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as SwarmCommandRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SwarmCommandRequest[], requestedPage: number) => {
  ...
  const [deleteSwarmCommandRequestCascade] = useDeleteSwarmCommandRequestCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SwarmCommandRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as SwarmCommandRequest;
    } catch (err) {
      console.warn('Could not reload SwarmCommandRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SwarmCommandRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSwarmCommandRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SwarmCommandRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SwarmCommandRequest';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmCommandRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SwarmCommandRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSwarmCommandRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSwarmCommandRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SwarmCommandRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SwarmCommandRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SwarmCommandRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SwarmCommandRequest[]);
    }
  };
