...
import { GeneratedArtifactSet } from '@thorapi/model';
import GeneratedArtifactSetForm from '@thorapi/redux/components/form/GeneratedArtifactSetForm';
...
import {
  useLazyGetGeneratedArtifactSetsPagedQuery,
...
} from '@thorapi/redux/services/GeneratedArtifactSetService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'status': { type: 'enum', enumValues: [
    'created', 'packaged', 'failed', 'superseded', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `GeneratedArtifactSet update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/GeneratedArtifactSet/${rowId}`;

const GeneratedArtifactSetTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<GeneratedArtifactSet[]>([]);
  ...
  const normalizePageRows = (result: unknown): GeneratedArtifactSet[] => {
    if (Array.isArray(result)) return result as GeneratedArtifactSet[];
    ...
    return Array.isArray(candidate) ? (candidate as GeneratedArtifactSet[]) : [];
  };

  const mergeFetchedRows = (incomingRows: GeneratedArtifactSet[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addGeneratedArtifactSet] = useAddGeneratedArtifactSetMutation();
  const [updateGeneratedArtifactSet] = useUpdateGeneratedArtifactSetMutation();
  const [deleteGeneratedArtifactSet] = useDeleteGeneratedArtifactSetMutation();
  const [deleteGeneratedArtifactSetCascade] = useDeleteGeneratedArtifactSetCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<GeneratedArtifactSet>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<GeneratedArtifactSet | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<GeneratedArtifactSet | null> => {
    ...
    try {
      ...
      return (await response.json()) as GeneratedArtifactSet;
    } catch (err) {
      console.warn('Could not reload GeneratedArtifactSet row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<GeneratedArtifactSet>,
    successField?: string,
  ) => {
    try {
      const saved = await updateGeneratedArtifactSet(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as GeneratedArtifactSet[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update GeneratedArtifactSet';
      console.error('Failed to update GeneratedArtifactSet:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteGeneratedArtifactSetCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteGeneratedArtifactSet(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<GeneratedArtifactSet> = { ...source };
      delete (payload as any).id;
      const created = await addGeneratedArtifactSet(payload).unwrap();
      setAllData((prev) => [created as GeneratedArtifactSet, ...prev]);
    } catch (error) {
      console.error('Failed to add GeneratedArtifactSet:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<GeneratedArtifactSet> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addGeneratedArtifactSet(payload).unwrap();
      setAllData((prev) => [created as GeneratedArtifactSet, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) GeneratedArtifactSet:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `GeneratedArtifactSet-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addGeneratedArtifactSet(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateGeneratedArtifactSet(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as GeneratedArtifactSet[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as GeneratedArtifactSet[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as GeneratedArtifactSet[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as GeneratedArtifactSet[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="GeneratedArtifactSetTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="GeneratedArtifactSet Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create GeneratedArtifactSet</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <GeneratedArtifactSetForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter GeneratedArtifactSet (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing GeneratedArtifactSet</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof GeneratedArtifactSet;
                ...
                await updateGeneratedArtifactSet(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of GeneratedArtifactSets.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'GeneratedArtifactSet'}
...
export default GeneratedArtifactSetTable;
