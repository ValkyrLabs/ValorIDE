...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/McpMarketplaceItemTag/${rowId}`;
...
const McpMarketplaceItemTagTable: React.FC = () => {
  ...
  const [deleteMcpMarketplaceItemTag] =
    useDeleteMcpMarketplaceItemTagMutation();
  const [deleteMcpMarketplaceItemTagCascade] =
    useDeleteMcpMarketplaceItemTagCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<McpMarketplaceItemTag | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateMcpMarketplaceItemTag(
        updatedItem as any,
      ).unwrap();
