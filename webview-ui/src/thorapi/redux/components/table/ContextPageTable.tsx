...
import { ContextPage } from '@thorapi/model';
import ContextPageForm from '@thorapi/redux/components/form/ContextPageForm';
...
const columnSchema: Record<string, ColumnSchema> = {
  'status': { type: 'enum', enumValues: [
    'compiled', 'hydrated', 'traversed', 'recompressed', 'expired', 'failed', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ContextPage update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ContextPage/${rowId}`;

const ContextPageTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ContextPage[]>([]);
  ...
  const normalizePageRows = (result: unknown): ContextPage[] => {
    if (Array.isArray(result)) return result as ContextPage[];
    ...
    return Array.isArray(candidate) ? (candidate as ContextPage[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ContextPage[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addContextPage] = useAddContextPageMutation();
  const [updateContextPage] = useUpdateContextPageMutation();
  const [deleteContextPage] = useDeleteContextPageMutation();
  const [deleteContextPageCascade] = useDeleteContextPageCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ContextPage>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ContextPage | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ContextPage | null> => {
    ...
    try {
      ...
      console.warn('Could not reload ContextPage row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ContextPage>,
    successField?: string,
  ) => {
    try {
      const saved = await updateContextPage(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ContextPage[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ContextPage';
      console.error('Failed to update ContextPage:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteContextPageCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteContextPage(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ContextPage> = { ...source };
      delete (payload as any).id;
      const created = await addContextPage(payload).unwrap();
      setAllData((prev) => [created as ContextPage, ...prev]);
    } catch (error) {
      console.error('Failed to add ContextPage:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ContextPage> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addContextPage(payload).unwrap();
      setAllData((prev) => [created as ContextPage, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ContextPage:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ContextPage-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addContextPage(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateContextPage(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ContextPage[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ContextPage[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ContextPage[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ContextPage[]);
    }
  };
    ...
    <>
          ...
          >
            ...
            <FloatingControlPanel
              description="ContextPage Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ContextPage</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ContextPageForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ContextPage (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ContextPage</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ContextPage;
                ...
                await updateContextPage(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ContextPages.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ContextPage'}
...
export default ContextPageTable;
