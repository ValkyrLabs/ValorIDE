...
import { WizardStatusResponse } from '@thorapi/model';
import WizardStatusResponseForm from '@thorapi/redux/components/form/WizardStatusResponseForm';
...
} from '@thorapi/redux/services/WizardStatusResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `WizardStatusResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/WizardStatusResponse/${rowId}`;
...
const WizardStatusResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): WizardStatusResponse[] => {
    if (Array.isArray(result)) return result as WizardStatusResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as WizardStatusResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: WizardStatusResponse[], requestedPage: number) => {
  ...
  const [deleteWizardStatusResponseCascade] = useDeleteWizardStatusResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<WizardStatusResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as WizardStatusResponse;
    } catch (err) {
      console.warn('Could not reload WizardStatusResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<WizardStatusResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWizardStatusResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as WizardStatusResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update WizardStatusResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWizardStatusResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `WizardStatusResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWizardStatusResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWizardStatusResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as WizardStatusResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as WizardStatusResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as WizardStatusResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as WizardStatusResponse[]);
    }
  };
