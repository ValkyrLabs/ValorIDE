...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/WizardStatusResponse/${rowId}`;
...
const WizardStatusResponseTable: React.FC = () => {
  ...
  const [deleteWizardStatusResponseCascade] =
    useDeleteWizardStatusResponseCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<WizardStatusResponse | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateWizardStatusResponse(
        updatedItem as any,
      ).unwrap();
