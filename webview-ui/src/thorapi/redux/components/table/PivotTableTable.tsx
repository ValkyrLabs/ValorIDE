...
import { PivotTable } from '@thorapi/model';
import PivotTableForm from '@thorapi/redux/components/form/PivotTableForm';
...
} from '@thorapi/redux/services/PivotTableService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `PivotTable update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/PivotTable/${rowId}`;
...
const PivotTableTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): PivotTable[] => {
    if (Array.isArray(result)) return result as PivotTable[];
    ...
    return Array.isArray(candidate) ? (candidate as PivotTable[]) : [];
  };

  const mergeFetchedRows = (incomingRows: PivotTable[], requestedPage: number) => {
  ...
  const [deletePivotTableCascade] = useDeletePivotTableCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<PivotTable | null> => {
    ...
    try {
      ...
      return (await response.json()) as PivotTable;
    } catch (err) {
      console.warn('Could not reload PivotTable row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<PivotTable>,
    successField?: string,
  ) => {
    try {
      const saved = await updatePivotTable(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as PivotTable[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update PivotTable';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePivotTableCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `PivotTable-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addPivotTable(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updatePivotTable(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as PivotTable[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as PivotTable[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as PivotTable[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as PivotTable[]);
    }
  };
