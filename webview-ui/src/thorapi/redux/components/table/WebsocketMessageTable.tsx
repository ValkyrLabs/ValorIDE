...
import { WebsocketMessage } from '@thorapi/model';
import WebsocketMessageForm from '@thorapi/redux/components/form/WebsocketMessageForm';
...
} from '@thorapi/redux/services/WebsocketMessageService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `WebsocketMessage update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/WebsocketMessage/${rowId}`;
...
const WebsocketMessageTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): WebsocketMessage[] => {
    if (Array.isArray(result)) return result as WebsocketMessage[];
    ...
    return Array.isArray(candidate) ? (candidate as WebsocketMessage[]) : [];
  };

  const mergeFetchedRows = (incomingRows: WebsocketMessage[], requestedPage: number) => {
  ...
  const [deleteWebsocketMessageCascade] = useDeleteWebsocketMessageCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<WebsocketMessage | null> => {
    ...
    try {
      ...
      return (await response.json()) as WebsocketMessage;
    } catch (err) {
      console.warn('Could not reload WebsocketMessage row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<WebsocketMessage>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWebsocketMessage(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as WebsocketMessage[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update WebsocketMessage';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWebsocketMessageCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `WebsocketMessage-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWebsocketMessage(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWebsocketMessage(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as WebsocketMessage[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as WebsocketMessage[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as WebsocketMessage[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as WebsocketMessage[]);
    }
  };
