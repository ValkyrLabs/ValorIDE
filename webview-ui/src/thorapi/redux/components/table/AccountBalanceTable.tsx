...
import { AccountBalance } from '@thorapi/model';
import AccountBalanceForm from '@thorapi/redux/components/form/AccountBalanceForm';
...
} from '@thorapi/redux/services/AccountBalanceService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AccountBalance update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AccountBalance/${rowId}`;
...
const AccountBalanceTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): AccountBalance[] => {
    if (Array.isArray(result)) return result as AccountBalance[];
    ...
    return Array.isArray(candidate) ? (candidate as AccountBalance[]) : [];
  };

  const mergeFetchedRows = (incomingRows: AccountBalance[], requestedPage: number) => {
  ...
  const [deleteAccountBalanceCascade] = useDeleteAccountBalanceCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<AccountBalance | null> => {
    ...
    try {
      ...
      console.warn('Could not reload AccountBalance row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AccountBalance>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAccountBalance(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AccountBalance[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AccountBalance';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAccountBalanceCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AccountBalance-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAccountBalance(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAccountBalance(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AccountBalance[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AccountBalance[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AccountBalance[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AccountBalance[]);
    }
  };
