...
import { TrustRuntimeEvent } from '@thorapi/model';
import TrustRuntimeEventForm from '@thorapi/redux/components/form/TrustRuntimeEventForm';
...
import {
  useLazyGetTrustRuntimeEventsPagedQuery,
...
} from '@thorapi/redux/services/TrustRuntimeEventService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'eventType': { type: 'enum', enumValues: [
    'EXECUTION_STARTED', 'RBAC_CHECK', 'ACL_CHECK', 'API_CALL', 'GRAYMATTER_READ', 'GRAYMATTER_WRITE', 'SECUREFIELD_ENCRYPT', 'SECUREFIELD_DECRYPT', 'KEY_LEASE', 'MODEL_CALL', 'TOOL_CALL', 'PROOF_GENERATED', 'EXECUTION_COMPLETED', 'EXECUTION_FAILED', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TrustRuntimeEvent update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TrustRuntimeEvent/${rowId}`;

const TrustRuntimeEventTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TrustRuntimeEvent[]>([]);
  ...
  const normalizePageRows = (result: unknown): TrustRuntimeEvent[] => {
    if (Array.isArray(result)) return result as TrustRuntimeEvent[];
    ...
    return Array.isArray(candidate) ? (candidate as TrustRuntimeEvent[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TrustRuntimeEvent[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addTrustRuntimeEvent] = useAddTrustRuntimeEventMutation();
  const [updateTrustRuntimeEvent] = useUpdateTrustRuntimeEventMutation();
  const [deleteTrustRuntimeEvent] = useDeleteTrustRuntimeEventMutation();
  const [deleteTrustRuntimeEventCascade] = useDeleteTrustRuntimeEventCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TrustRuntimeEvent>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TrustRuntimeEvent | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TrustRuntimeEvent | null> => {
    ...
    try {
      ...
      return (await response.json()) as TrustRuntimeEvent;
    } catch (err) {
      console.warn('Could not reload TrustRuntimeEvent row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TrustRuntimeEvent>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTrustRuntimeEvent(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TrustRuntimeEvent[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TrustRuntimeEvent';
      console.error('Failed to update TrustRuntimeEvent:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustRuntimeEventCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTrustRuntimeEvent(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TrustRuntimeEvent> = { ...source };
      delete (payload as any).id;
      const created = await addTrustRuntimeEvent(payload).unwrap();
      setAllData((prev) => [created as TrustRuntimeEvent, ...prev]);
    } catch (error) {
      console.error('Failed to add TrustRuntimeEvent:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TrustRuntimeEvent> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTrustRuntimeEvent(payload).unwrap();
      setAllData((prev) => [created as TrustRuntimeEvent, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TrustRuntimeEvent:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TrustRuntimeEvent-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTrustRuntimeEvent(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTrustRuntimeEvent(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TrustRuntimeEvent[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TrustRuntimeEvent[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TrustRuntimeEvent[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TrustRuntimeEvent[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="TrustRuntimeEventTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="TrustRuntimeEvent Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TrustRuntimeEvent</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TrustRuntimeEventForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TrustRuntimeEvent (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TrustRuntimeEvent</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TrustRuntimeEvent;
                ...
                await updateTrustRuntimeEvent(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TrustRuntimeEvents.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TrustRuntimeEvent'}
...
export default TrustRuntimeEventTable;
