...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/TrustRuntimeEvent/${rowId}`;
...
const TrustRuntimeEventTable: React.FC = () => {
  ...
  const [deleteTrustRuntimeEventCascade] =
    useDeleteTrustRuntimeEventCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<TrustRuntimeEvent | null> => {
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update TrustRuntimeEvent";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustRuntimeEventCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
