...
import VerifyTwoFactorResponseForm from '@thorapi/redux/components/form/VerifyTwoFactorResponseForm';
...
import {
  useLazyGetVerifyTwoFactorResponsesPagedQuery,
...
} from '@thorapi/redux/services/VerifyTwoFactorResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `VerifyTwoFactorResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/VerifyTwoFactorResponse/${rowId}`;

const VerifyTwoFactorResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<VerifyTwoFactorResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): VerifyTwoFactorResponse[] => {
    if (Array.isArray(result)) return result as VerifyTwoFactorResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as VerifyTwoFactorResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: VerifyTwoFactorResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addVerifyTwoFactorResponse] = useAddVerifyTwoFactorResponseMutation();
  const [updateVerifyTwoFactorResponse] = useUpdateVerifyTwoFactorResponseMutation();
  const [deleteVerifyTwoFactorResponse] = useDeleteVerifyTwoFactorResponseMutation();
  const [deleteVerifyTwoFactorResponseCascade] = useDeleteVerifyTwoFactorResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<VerifyTwoFactorResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<VerifyTwoFactorResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<VerifyTwoFactorResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as VerifyTwoFactorResponse;
    } catch (err) {
      console.warn('Could not reload VerifyTwoFactorResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<VerifyTwoFactorResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateVerifyTwoFactorResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as VerifyTwoFactorResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update VerifyTwoFactorResponse';
      console.error('Failed to update VerifyTwoFactorResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteVerifyTwoFactorResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteVerifyTwoFactorResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<VerifyTwoFactorResponse> = { ...source };
      delete (payload as any).id;
      const created = await addVerifyTwoFactorResponse(payload).unwrap();
      setAllData((prev) => [created as VerifyTwoFactorResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add VerifyTwoFactorResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<VerifyTwoFactorResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addVerifyTwoFactorResponse(payload).unwrap();
      setAllData((prev) => [created as VerifyTwoFactorResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) VerifyTwoFactorResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `VerifyTwoFactorResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addVerifyTwoFactorResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateVerifyTwoFactorResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as VerifyTwoFactorResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as VerifyTwoFactorResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as VerifyTwoFactorResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as VerifyTwoFactorResponse[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.VerifyTwoFactorResponse',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="VerifyTwoFactorResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="VerifyTwoFactorResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create VerifyTwoFactorResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <VerifyTwoFactorResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter VerifyTwoFactorResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing VerifyTwoFactorResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof VerifyTwoFactorResponse;
                ...
                await updateVerifyTwoFactorResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of VerifyTwoFactorResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'VerifyTwoFactorResponse'}
...
export default VerifyTwoFactorResponseTable;
