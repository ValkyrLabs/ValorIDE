...
import { RetrievalReceipt } from '@thorapi/model';
import RetrievalReceiptForm from '@thorapi/redux/components/form/RetrievalReceiptForm';
...
import {
  useLazyGetRetrievalReceiptsPagedQuery,
...
} from '@thorapi/redux/services/RetrievalReceiptService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RetrievalReceipt update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RetrievalReceipt/${rowId}`;

const RetrievalReceiptTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RetrievalReceipt[]>([]);
  ...
  const normalizePageRows = (result: unknown): RetrievalReceipt[] => {
    if (Array.isArray(result)) return result as RetrievalReceipt[];
    ...
    return Array.isArray(candidate) ? (candidate as RetrievalReceipt[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RetrievalReceipt[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addRetrievalReceipt] = useAddRetrievalReceiptMutation();
  const [updateRetrievalReceipt] = useUpdateRetrievalReceiptMutation();
  const [deleteRetrievalReceipt] = useDeleteRetrievalReceiptMutation();
  const [deleteRetrievalReceiptCascade] = useDeleteRetrievalReceiptCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<RetrievalReceipt>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RetrievalReceipt | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<RetrievalReceipt | null> => {
    ...
    try {
      ...
      return (await response.json()) as RetrievalReceipt;
    } catch (err) {
      console.warn('Could not reload RetrievalReceipt row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RetrievalReceipt>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRetrievalReceipt(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RetrievalReceipt[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RetrievalReceipt';
      console.error('Failed to update RetrievalReceipt:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRetrievalReceiptCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteRetrievalReceipt(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RetrievalReceipt> = { ...source };
      delete (payload as any).id;
      const created = await addRetrievalReceipt(payload).unwrap();
      setAllData((prev) => [created as RetrievalReceipt, ...prev]);
    } catch (error) {
      console.error('Failed to add RetrievalReceipt:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RetrievalReceipt> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRetrievalReceipt(payload).unwrap();
      setAllData((prev) => [created as RetrievalReceipt, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) RetrievalReceipt:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RetrievalReceipt-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRetrievalReceipt(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRetrievalReceipt(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RetrievalReceipt[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RetrievalReceipt[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RetrievalReceipt[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RetrievalReceipt[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="RetrievalReceiptTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="RetrievalReceipt Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RetrievalReceipt</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RetrievalReceiptForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RetrievalReceipt (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RetrievalReceipt</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RetrievalReceipt;
                ...
                await updateRetrievalReceipt(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RetrievalReceipts.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RetrievalReceipt'}
...
export default RetrievalReceiptTable;
