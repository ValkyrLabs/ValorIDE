...
import { McpMarketplaceCatalog } from '@thorapi/model';
import McpMarketplaceCatalogForm from '@thorapi/redux/components/form/McpMarketplaceCatalogForm';
...
} from '@thorapi/redux/services/McpMarketplaceCatalogService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `McpMarketplaceCatalog update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/McpMarketplaceCatalog/${rowId}`;
...
const McpMarketplaceCatalogTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): McpMarketplaceCatalog[] => {
    if (Array.isArray(result)) return result as McpMarketplaceCatalog[];
    ...
    return Array.isArray(candidate) ? (candidate as McpMarketplaceCatalog[]) : [];
  };

  const mergeFetchedRows = (incomingRows: McpMarketplaceCatalog[], requestedPage: number) => {
  ...
  const [deleteMcpMarketplaceCatalogCascade] = useDeleteMcpMarketplaceCatalogCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<McpMarketplaceCatalog | null> => {
    ...
    try {
      ...
      return (await response.json()) as McpMarketplaceCatalog;
    } catch (err) {
      console.warn('Could not reload McpMarketplaceCatalog row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<McpMarketplaceCatalog>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMcpMarketplaceCatalog(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as McpMarketplaceCatalog[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update McpMarketplaceCatalog';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpMarketplaceCatalogCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `McpMarketplaceCatalog-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMcpMarketplaceCatalog(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMcpMarketplaceCatalog(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as McpMarketplaceCatalog[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as McpMarketplaceCatalog[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as McpMarketplaceCatalog[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as McpMarketplaceCatalog[]);
    }
  };
