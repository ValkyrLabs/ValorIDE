...
import { RetrievalQualityScore } from '@thorapi/model';
import RetrievalQualityScoreForm from '@thorapi/redux/components/form/RetrievalQualityScoreForm';
...
import {
  useLazyGetRetrievalQualityScoresPagedQuery,
...
} from '@thorapi/redux/services/RetrievalQualityScoreService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'meanSimilarityScore': { type: 'number' },
  'minSimilarityScore': { type: 'number' },
  'resultCount': { type: 'number' },
  'sourceDiversityScore': { type: 'number' },
  'freshnessScore': { type: 'number' },
  'schemaCoverageScore': { type: 'number' },
  'authorityScore': { type: 'number' },
  'contradictionScore': { type: 'number' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RetrievalQualityScore update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RetrievalQualityScore/${rowId}`;

const RetrievalQualityScoreTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RetrievalQualityScore[]>([]);
  ...
  const normalizePageRows = (result: unknown): RetrievalQualityScore[] => {
    if (Array.isArray(result)) return result as RetrievalQualityScore[];
    ...
    return Array.isArray(candidate) ? (candidate as RetrievalQualityScore[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RetrievalQualityScore[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addRetrievalQualityScore] = useAddRetrievalQualityScoreMutation();
  const [updateRetrievalQualityScore] = useUpdateRetrievalQualityScoreMutation();
  const [deleteRetrievalQualityScore] = useDeleteRetrievalQualityScoreMutation();
  const [deleteRetrievalQualityScoreCascade] = useDeleteRetrievalQualityScoreCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<RetrievalQualityScore>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RetrievalQualityScore | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<RetrievalQualityScore | null> => {
    ...
    try {
      ...
      return (await response.json()) as RetrievalQualityScore;
    } catch (err) {
      console.warn('Could not reload RetrievalQualityScore row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RetrievalQualityScore>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRetrievalQualityScore(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RetrievalQualityScore[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RetrievalQualityScore';
      console.error('Failed to update RetrievalQualityScore:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRetrievalQualityScoreCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteRetrievalQualityScore(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RetrievalQualityScore> = { ...source };
      delete (payload as any).id;
      const created = await addRetrievalQualityScore(payload).unwrap();
      setAllData((prev) => [created as RetrievalQualityScore, ...prev]);
    } catch (error) {
      console.error('Failed to add RetrievalQualityScore:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RetrievalQualityScore> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRetrievalQualityScore(payload).unwrap();
      setAllData((prev) => [created as RetrievalQualityScore, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) RetrievalQualityScore:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RetrievalQualityScore-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRetrievalQualityScore(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRetrievalQualityScore(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RetrievalQualityScore[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RetrievalQualityScore[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RetrievalQualityScore[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RetrievalQualityScore[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="RetrievalQualityScoreTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="RetrievalQualityScore Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RetrievalQualityScore</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RetrievalQualityScoreForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RetrievalQualityScore (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RetrievalQualityScore</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RetrievalQualityScore;
                ...
                await updateRetrievalQualityScore(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RetrievalQualityScores.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RetrievalQualityScore'}
...
export default RetrievalQualityScoreTable;
