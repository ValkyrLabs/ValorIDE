...
import { PersistentLogin } from '@thorapi/model';
import PersistentLoginForm from '@thorapi/redux/components/form/PersistentLoginForm';
...
} from '@thorapi/redux/services/PersistentLoginService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `PersistentLogin update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/PersistentLogin/${rowId}`;
...
const PersistentLoginTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): PersistentLogin[] => {
    if (Array.isArray(result)) return result as PersistentLogin[];
    ...
    return Array.isArray(candidate) ? (candidate as PersistentLogin[]) : [];
  };

  const mergeFetchedRows = (incomingRows: PersistentLogin[], requestedPage: number) => {
  ...
  const [deletePersistentLoginCascade] = useDeletePersistentLoginCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<PersistentLogin | null> => {
    ...
    try {
      ...
      return (await response.json()) as PersistentLogin;
    } catch (err) {
      console.warn('Could not reload PersistentLogin row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<PersistentLogin>,
    successField?: string,
  ) => {
    try {
      const saved = await updatePersistentLogin(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as PersistentLogin[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update PersistentLogin';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePersistentLoginCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `PersistentLogin-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addPersistentLogin(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updatePersistentLogin(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as PersistentLogin[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as PersistentLogin[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as PersistentLogin[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as PersistentLogin[]);
    }
  };
