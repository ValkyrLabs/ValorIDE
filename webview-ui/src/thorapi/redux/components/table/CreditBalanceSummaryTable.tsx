...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/CreditBalanceSummary/${rowId}`;
...
const CreditBalanceSummaryTable: React.FC = () => {
  ...
  const [deleteCreditBalanceSummaryCascade] =
    useDeleteCreditBalanceSummaryCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<CreditBalanceSummary | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateCreditBalanceSummary(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update CreditBalanceSummary";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCreditBalanceSummaryCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter CreditBalanceSummary (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
