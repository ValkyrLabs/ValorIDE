...
import CreditBalanceSummaryForm from '@thorapi/redux/components/form/CreditBalanceSummaryForm';
...
import {
  useLazyGetCreditBalanceSummarysPagedQuery,
...
} from '@thorapi/redux/services/CreditBalanceSummaryService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'paymentTransactionCount': { type: 'number' },
  'usageTransactionCount': { type: 'number' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `CreditBalanceSummary update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/CreditBalanceSummary/${rowId}`;

const CreditBalanceSummaryTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<CreditBalanceSummary[]>([]);
  ...
  const normalizePageRows = (result: unknown): CreditBalanceSummary[] => {
    if (Array.isArray(result)) return result as CreditBalanceSummary[];
    ...
    return Array.isArray(candidate) ? (candidate as CreditBalanceSummary[]) : [];
  };

  const mergeFetchedRows = (incomingRows: CreditBalanceSummary[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addCreditBalanceSummary] = useAddCreditBalanceSummaryMutation();
  const [updateCreditBalanceSummary] = useUpdateCreditBalanceSummaryMutation();
  const [deleteCreditBalanceSummary] = useDeleteCreditBalanceSummaryMutation();
  const [deleteCreditBalanceSummaryCascade] = useDeleteCreditBalanceSummaryCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<CreditBalanceSummary>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<CreditBalanceSummary | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<CreditBalanceSummary | null> => {
    ...
    try {
      ...
      return (await response.json()) as CreditBalanceSummary;
    } catch (err) {
      console.warn('Could not reload CreditBalanceSummary row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<CreditBalanceSummary>,
    successField?: string,
  ) => {
    try {
      const saved = await updateCreditBalanceSummary(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as CreditBalanceSummary[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update CreditBalanceSummary';
      console.error('Failed to update CreditBalanceSummary:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCreditBalanceSummaryCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteCreditBalanceSummary(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<CreditBalanceSummary> = { ...source };
      delete (payload as any).id;
      const created = await addCreditBalanceSummary(payload).unwrap();
      setAllData((prev) => [created as CreditBalanceSummary, ...prev]);
    } catch (error) {
      console.error('Failed to add CreditBalanceSummary:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<CreditBalanceSummary> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addCreditBalanceSummary(payload).unwrap();
      setAllData((prev) => [created as CreditBalanceSummary, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) CreditBalanceSummary:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `CreditBalanceSummary-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addCreditBalanceSummary(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateCreditBalanceSummary(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as CreditBalanceSummary[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as CreditBalanceSummary[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as CreditBalanceSummary[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as CreditBalanceSummary[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="CreditBalanceSummaryTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="CreditBalanceSummary Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create CreditBalanceSummary</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <CreditBalanceSummaryForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter CreditBalanceSummary (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing CreditBalanceSummary</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof CreditBalanceSummary;
                ...
                await updateCreditBalanceSummary(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of CreditBalanceSummarys.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'CreditBalanceSummary'}
...
export default CreditBalanceSummaryTable;
