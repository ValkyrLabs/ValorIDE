...
import { SwarmPayload } from '@thorapi/model';
import SwarmPayloadForm from '@thorapi/redux/components/form/SwarmPayloadForm';
...
} from '@thorapi/redux/services/SwarmPayloadService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SwarmPayload update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SwarmPayload/${rowId}`;
...
const SwarmPayloadTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SwarmPayload[] => {
    if (Array.isArray(result)) return result as SwarmPayload[];
    ...
    return Array.isArray(candidate) ? (candidate as SwarmPayload[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SwarmPayload[], requestedPage: number) => {
  ...
  const [deleteSwarmPayloadCascade] = useDeleteSwarmPayloadCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SwarmPayload | null> => {
    ...
    try {
      ...
      return (await response.json()) as SwarmPayload;
    } catch (err) {
      console.warn('Could not reload SwarmPayload row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SwarmPayload>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSwarmPayload(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SwarmPayload[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SwarmPayload';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmPayloadCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SwarmPayload-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSwarmPayload(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSwarmPayload(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SwarmPayload[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SwarmPayload[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SwarmPayload[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SwarmPayload[]);
    }
  };
