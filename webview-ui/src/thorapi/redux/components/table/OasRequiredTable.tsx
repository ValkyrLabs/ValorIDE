...
import { OasRequired } from '@thorapi/model';
import OasRequiredForm from '@thorapi/redux/components/form/OasRequiredForm';
...
} from '@thorapi/redux/services/OasRequiredService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `OasRequired update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/OasRequired/${rowId}`;
...
const OasRequiredTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): OasRequired[] => {
    if (Array.isArray(result)) return result as OasRequired[];
    ...
    return Array.isArray(candidate) ? (candidate as OasRequired[]) : [];
  };

  const mergeFetchedRows = (incomingRows: OasRequired[], requestedPage: number) => {
  ...
  const [deleteOasRequiredCascade] = useDeleteOasRequiredCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<OasRequired | null> => {
    ...
    try {
      ...
      return (await response.json()) as OasRequired;
    } catch (err) {
      console.warn('Could not reload OasRequired row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<OasRequired>,
    successField?: string,
  ) => {
    try {
      const saved = await updateOasRequired(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as OasRequired[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update OasRequired';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOasRequiredCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `OasRequired-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addOasRequired(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateOasRequired(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as OasRequired[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as OasRequired[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as OasRequired[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as OasRequired[]);
    }
  };
