...
import { WebsocketSession } from '@thorapi/model';
import WebsocketSessionForm from '@thorapi/redux/components/form/WebsocketSessionForm';
...
} from '@thorapi/redux/services/WebsocketSessionService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `WebsocketSession update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/WebsocketSession/${rowId}`;
...
const WebsocketSessionTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): WebsocketSession[] => {
    if (Array.isArray(result)) return result as WebsocketSession[];
    ...
    return Array.isArray(candidate) ? (candidate as WebsocketSession[]) : [];
  };

  const mergeFetchedRows = (incomingRows: WebsocketSession[], requestedPage: number) => {
  ...
  const [deleteWebsocketSessionCascade] = useDeleteWebsocketSessionCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<WebsocketSession | null> => {
    ...
    try {
      ...
      return (await response.json()) as WebsocketSession;
    } catch (err) {
      console.warn('Could not reload WebsocketSession row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<WebsocketSession>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWebsocketSession(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as WebsocketSession[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update WebsocketSession';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWebsocketSessionCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `WebsocketSession-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWebsocketSession(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWebsocketSession(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as WebsocketSession[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as WebsocketSession[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as WebsocketSession[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as WebsocketSession[]);
    }
  };
