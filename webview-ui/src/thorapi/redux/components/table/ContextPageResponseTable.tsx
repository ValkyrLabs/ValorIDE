...
import { ContextPageResponse } from '@thorapi/model';
import ContextPageResponseForm from '@thorapi/redux/components/form/ContextPageResponseForm';
...
import {
  useLazyGetContextPageResponsesPagedQuery,
...
} from '@thorapi/redux/services/ContextPageResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ContextPageResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ContextPageResponse/${rowId}`;

const ContextPageResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ContextPageResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): ContextPageResponse[] => {
    if (Array.isArray(result)) return result as ContextPageResponse[];
  ...
  const mergeFetchedRows = (incomingRows: ContextPageResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addContextPageResponse] = useAddContextPageResponseMutation();
  const [updateContextPageResponse] = useUpdateContextPageResponseMutation();
  const [deleteContextPageResponse] = useDeleteContextPageResponseMutation();
  const [deleteContextPageResponseCascade] = useDeleteContextPageResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ContextPageResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ContextPageResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ContextPageResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as ContextPageResponse;
    } catch (err) {
      console.warn('Could not reload ContextPageResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ContextPageResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateContextPageResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ContextPageResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ContextPageResponse';
      console.error('Failed to update ContextPageResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteContextPageResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteContextPageResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ContextPageResponse> = { ...source };
      delete (payload as any).id;
      const created = await addContextPageResponse(payload).unwrap();
      setAllData((prev) => [created as ContextPageResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add ContextPageResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ContextPageResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addContextPageResponse(payload).unwrap();
      setAllData((prev) => [created as ContextPageResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ContextPageResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ContextPageResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addContextPageResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateContextPageResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ContextPageResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ContextPageResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ContextPageResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ContextPageResponse[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.ContextPageResponse',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ContextPageResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ContextPageResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ContextPageResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ContextPageResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ContextPageResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ContextPageResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ContextPageResponse;
                ...
                await updateContextPageResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ContextPageResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ContextPageResponse'}
...
export default ContextPageResponseTable;
