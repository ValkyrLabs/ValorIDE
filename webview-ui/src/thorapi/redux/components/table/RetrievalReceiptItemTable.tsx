...
import { RetrievalReceiptItem } from '@thorapi/model';
import RetrievalReceiptItemForm from '@thorapi/redux/components/form/RetrievalReceiptItemForm';
...
import {
  useLazyGetRetrievalReceiptItemsPagedQuery,
...
} from '@thorapi/redux/services/RetrievalReceiptItemService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RetrievalReceiptItem update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RetrievalReceiptItem/${rowId}`;

const RetrievalReceiptItemTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RetrievalReceiptItem[]>([]);
  ...
  const normalizePageRows = (result: unknown): RetrievalReceiptItem[] => {
    if (Array.isArray(result)) return result as RetrievalReceiptItem[];
    ...
    return Array.isArray(candidate) ? (candidate as RetrievalReceiptItem[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RetrievalReceiptItem[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addRetrievalReceiptItem] = useAddRetrievalReceiptItemMutation();
  const [updateRetrievalReceiptItem] = useUpdateRetrievalReceiptItemMutation();
  const [deleteRetrievalReceiptItem] = useDeleteRetrievalReceiptItemMutation();
  const [deleteRetrievalReceiptItemCascade] = useDeleteRetrievalReceiptItemCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<RetrievalReceiptItem>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RetrievalReceiptItem | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<RetrievalReceiptItem | null> => {
    ...
    try {
      ...
      return (await response.json()) as RetrievalReceiptItem;
    } catch (err) {
      console.warn('Could not reload RetrievalReceiptItem row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RetrievalReceiptItem>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRetrievalReceiptItem(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RetrievalReceiptItem[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RetrievalReceiptItem';
      console.error('Failed to update RetrievalReceiptItem:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRetrievalReceiptItemCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteRetrievalReceiptItem(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RetrievalReceiptItem> = { ...source };
      delete (payload as any).id;
      const created = await addRetrievalReceiptItem(payload).unwrap();
      setAllData((prev) => [created as RetrievalReceiptItem, ...prev]);
    } catch (error) {
      console.error('Failed to add RetrievalReceiptItem:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RetrievalReceiptItem> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRetrievalReceiptItem(payload).unwrap();
      setAllData((prev) => [created as RetrievalReceiptItem, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) RetrievalReceiptItem:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RetrievalReceiptItem-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRetrievalReceiptItem(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRetrievalReceiptItem(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RetrievalReceiptItem[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RetrievalReceiptItem[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RetrievalReceiptItem[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RetrievalReceiptItem[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="RetrievalReceiptItemTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="RetrievalReceiptItem Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RetrievalReceiptItem</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RetrievalReceiptItemForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RetrievalReceiptItem (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RetrievalReceiptItem</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RetrievalReceiptItem;
                ...
                await updateRetrievalReceiptItem(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RetrievalReceiptItems.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RetrievalReceiptItem'}
...
export default RetrievalReceiptItemTable;
