...
import { Depend } from '@thorapi/model';
import DependForm from '@thorapi/redux/components/form/DependForm';
...
} from '@thorapi/redux/services/DependService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Depend update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Depend/${rowId}`;
...
const DependTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Depend[] => {
    if (Array.isArray(result)) return result as Depend[];
    ...
    return Array.isArray(candidate) ? (candidate as Depend[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Depend[], requestedPage: number) => {
  ...
  const [deleteDependCascade] = useDeleteDependCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Depend | null> => {
    ...
    try {
      ...
      return (await response.json()) as Depend;
    } catch (err) {
      console.warn('Could not reload Depend row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Depend>,
    successField?: string,
  ) => {
    try {
      const saved = await updateDepend(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Depend[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Depend';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteDependCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Depend-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addDepend(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateDepend(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Depend[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Depend[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Depend[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Depend[]);
    }
  };
