...
import { AccountPlan } from '@thorapi/model';
import AccountPlanForm from '@thorapi/redux/components/form/AccountPlanForm';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'customOrganizationLimit': { type: 'number' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AccountPlan update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AccountPlan/${rowId}`;

const AccountPlanTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<AccountPlan[]>([]);
  ...
  const normalizePageRows = (result: unknown): AccountPlan[] => {
    if (Array.isArray(result)) return result as AccountPlan[];
    ...
    return Array.isArray(candidate) ? (candidate as AccountPlan[]) : [];
  };

  const mergeFetchedRows = (incomingRows: AccountPlan[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addAccountPlan] = useAddAccountPlanMutation();
  const [updateAccountPlan] = useUpdateAccountPlanMutation();
  const [deleteAccountPlan] = useDeleteAccountPlanMutation();
  const [deleteAccountPlanCascade] = useDeleteAccountPlanCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<AccountPlan | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<AccountPlan | null> => {
    ...
    try {
      ...
      console.warn('Could not reload AccountPlan row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AccountPlan>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAccountPlan(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AccountPlan[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AccountPlan';
      console.error('Failed to update AccountPlan:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAccountPlanCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteAccountPlan(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<AccountPlan> = { ...source };
      delete (payload as any).id;
      const created = await addAccountPlan(payload).unwrap();
      setAllData((prev) => [created as AccountPlan, ...prev]);
    } catch (error) {
      console.error('Failed to add AccountPlan:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<AccountPlan> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addAccountPlan(payload).unwrap();
      setAllData((prev) => [created as AccountPlan, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) AccountPlan:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AccountPlan-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAccountPlan(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAccountPlan(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AccountPlan[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AccountPlan[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AccountPlan[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AccountPlan[]);
    }
  };
    ...
    <>
          ...
          >
            ...
            <FloatingControlPanel
              description="AccountPlan Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create AccountPlan</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <AccountPlanForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter AccountPlan (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing AccountPlan</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof AccountPlan;
                ...
                await updateAccountPlan(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of AccountPlans.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'AccountPlan'}
