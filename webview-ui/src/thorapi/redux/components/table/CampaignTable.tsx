...
import { Campaign } from '@thorapi/model';
import CampaignForm from '@thorapi/redux/components/form/CampaignForm';
...
} from '@thorapi/redux/services/CampaignService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Campaign update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Campaign/${rowId}`;
...
const CampaignTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Campaign[] => {
    if (Array.isArray(result)) return result as Campaign[];
    ...
    return Array.isArray(candidate) ? (candidate as Campaign[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Campaign[], requestedPage: number) => {
  ...
  const reloadRowById = async (rowId: string): Promise<Campaign | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Campaign row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Campaign>,
    successField?: string,
  ) => {
    try {
      const saved = await updateCampaign(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Campaign[]);
      if (successField) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCampaignCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Campaign-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addCampaign(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Campaign[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Campaign[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Campaign[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Campaign[]);
    }
  };
