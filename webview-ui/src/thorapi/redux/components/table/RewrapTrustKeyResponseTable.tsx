...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/RewrapTrustKeyResponse/${rowId}`;
...
const RewrapTrustKeyResponseTable: React.FC = () => {
  ...
  const [updateRewrapTrustKeyResponse] =
    useUpdateRewrapTrustKeyResponseMutation();
  const [deleteRewrapTrustKeyResponse] =
    useDeleteRewrapTrustKeyResponseMutation();
  const [deleteRewrapTrustKeyResponseCascade] =
    useDeleteRewrapTrustKeyResponseCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<RewrapTrustKeyResponse | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<RewrapTrustKeyResponse | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateRewrapTrustKeyResponse(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update RewrapTrustKeyResponse";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRewrapTrustKeyResponseCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter RewrapTrustKeyResponse (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
