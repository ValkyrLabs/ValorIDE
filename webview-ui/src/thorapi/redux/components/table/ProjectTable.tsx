...
import { Project } from '@thorapi/model';
import ProjectForm from '@thorapi/redux/components/form/ProjectForm';
...
import {
  useLazyGetProjectsPagedQuery,
  useAddProjectMutation,
  useUpdateProjectMutation,
  useDeleteProjectMutation,
  useDeleteProjectCascadeMutation
} from '@thorapi/redux/services/ProjectService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'projectType': { type: 'enum', enumValues: [
    'app-generation', 'api-builder', 'workflow-mcp', 'graymatter-memory', 'valoride-coding', 'deployment', 'content-publishing', 'business-in-a-box', 
  ], enumValueType: 'string' },
  'status': { type: 'enum', enumValues: [
    'planned', 'active', 'blocked', 'review', 'completed', 'archived', 
  ], enumValueType: 'string' },
  'currentStage': { type: 'enum', enumValues: [
    'define-strategy', 'define-goals', 'plan-tasks', 'generate-api', 'generate-app', 'vibe-code', 'deploy', 'publish-mcp', 'operate', 'complete', 
  ], enumValueType: 'string' },
  'progressPercent': { type: 'number' },
  'sourceSurface': { type: 'enum', enumValues: [
    'dashboard', 'graymatter', 'valor', 'valoride', 'api', 'workflow', 'swarm', 
  ], enumValueType: 'string' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Project update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Project/${rowId}`;
...
const ProjectTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Project[] => {
  ...
  const mergeFetchedRows = (incomingRows: Project[], requestedPage: number) => {
  ...
  const [deleteProjectCascade] = useDeleteProjectCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<Project | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<Project | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Project row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Project>,
    successField?: string,
  ) => {
    try {
      const saved = await updateProject(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Project[]);
      if (successField) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteProjectCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteProject(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Project> = { ...source };
      delete (payload as any).id;
      const created = await addProject(payload).unwrap();
      setAllData((prev) => [created as Project, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Project> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addProject(payload).unwrap();
      setAllData((prev) => [created as Project, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) Project:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Project-export-${stamp}.json`;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Project[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Project[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Project[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Project[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter Project (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof Project;
                ...
                await updateProject(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of Projects.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'Project'}
