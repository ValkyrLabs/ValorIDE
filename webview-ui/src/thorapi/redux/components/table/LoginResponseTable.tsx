...
import { LoginResponse } from '@thorapi/model';
import LoginResponseForm from '@thorapi/redux/components/form/LoginResponseForm';
...
import {
  useLazyGetLoginResponsesPagedQuery,
...
} from '@thorapi/redux/services/LoginResponseService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'status': { type: 'enum', enumValues: [
    'SUCCESS', 'REQUIRES_2FA', 'INVALID_CREDENTIALS', 'ACCOUNT_LOCKED', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `LoginResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/LoginResponse/${rowId}`;

const LoginResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<LoginResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): LoginResponse[] => {
    if (Array.isArray(result)) return result as LoginResponse[];
  ...
  const mergeFetchedRows = (incomingRows: LoginResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addLoginResponse] = useAddLoginResponseMutation();
  const [updateLoginResponse] = useUpdateLoginResponseMutation();
  const [deleteLoginResponse] = useDeleteLoginResponseMutation();
  const [deleteLoginResponseCascade] = useDeleteLoginResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<LoginResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<LoginResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<LoginResponse | null> => {
    ...
    try {
      ...
      console.warn('Could not reload LoginResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<LoginResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateLoginResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as LoginResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update LoginResponse';
      console.error('Failed to update LoginResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteLoginResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteLoginResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<LoginResponse> = { ...source };
      delete (payload as any).id;
      const created = await addLoginResponse(payload).unwrap();
      setAllData((prev) => [created as LoginResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add LoginResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<LoginResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addLoginResponse(payload).unwrap();
      setAllData((prev) => [created as LoginResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) LoginResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `LoginResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addLoginResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateLoginResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as LoginResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as LoginResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as LoginResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as LoginResponse[]);
    }
  };
    ...
    <>
          ...
          >
            ...
            <FloatingControlPanel
              description="LoginResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create LoginResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <LoginResponseForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter LoginResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing LoginResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof LoginResponse;
                ...
                await updateLoginResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of LoginResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'LoginResponse'}
...
export default LoginResponseTable;
