...
import { NamedRange } from '@thorapi/model';
import NamedRangeForm from '@thorapi/redux/components/form/NamedRangeForm';
...
import {
  ...
  useDeleteNamedRangeCascadeMutation
} from '@thorapi/redux/services/NamedRangeService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `NamedRange update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/NamedRange/${rowId}`;
...
const NamedRangeTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): NamedRange[] => {
    if (Array.isArray(result)) return result as NamedRange[];
    ...
    return Array.isArray(candidate) ? (candidate as NamedRange[]) : [];
  };

  const mergeFetchedRows = (incomingRows: NamedRange[], requestedPage: number) => {
  ...
  const [deleteNamedRangeCascade] = useDeleteNamedRangeCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<NamedRange | null> => {
    ...
    try {
      ...
      return (await response.json()) as NamedRange;
    } catch (err) {
      console.warn('Could not reload NamedRange row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<NamedRange>,
    successField?: string,
  ) => {
    try {
      const saved = await updateNamedRange(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as NamedRange[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update NamedRange';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteNamedRangeCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `NamedRange-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addNamedRange(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateNamedRange(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as NamedRange[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as NamedRange[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as NamedRange[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as NamedRange[]);
    }
  };
