...
import ExecModuleAuthConfigForm from '@thorapi/redux/components/form/ExecModuleAuthConfigForm';
...
import {
  useLazyGetExecModuleAuthConfigsPagedQuery,
...
} from '@thorapi/redux/services/ExecModuleAuthConfigService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'authStrategy': { type: 'number' },
  'storeEnrollment': { type: 'boolean' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ExecModuleAuthConfig update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ExecModuleAuthConfig/${rowId}`;

const ExecModuleAuthConfigTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ExecModuleAuthConfig[]>([]);
  ...
  const normalizePageRows = (result: unknown): ExecModuleAuthConfig[] => {
    if (Array.isArray(result)) return result as ExecModuleAuthConfig[];
    ...
    return Array.isArray(candidate) ? (candidate as ExecModuleAuthConfig[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ExecModuleAuthConfig[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addExecModuleAuthConfig] = useAddExecModuleAuthConfigMutation();
  const [updateExecModuleAuthConfig] = useUpdateExecModuleAuthConfigMutation();
  const [deleteExecModuleAuthConfig] = useDeleteExecModuleAuthConfigMutation();
  const [deleteExecModuleAuthConfigCascade] = useDeleteExecModuleAuthConfigCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ExecModuleAuthConfig>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ExecModuleAuthConfig | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ExecModuleAuthConfig | null> => {
    ...
    try {
      ...
      return (await response.json()) as ExecModuleAuthConfig;
    } catch (err) {
      console.warn('Could not reload ExecModuleAuthConfig row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ExecModuleAuthConfig>,
    successField?: string,
  ) => {
    try {
      const saved = await updateExecModuleAuthConfig(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ExecModuleAuthConfig[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ExecModuleAuthConfig';
      console.error('Failed to update ExecModuleAuthConfig:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecModuleAuthConfigCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteExecModuleAuthConfig(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ExecModuleAuthConfig> = { ...source };
      delete (payload as any).id;
      const created = await addExecModuleAuthConfig(payload).unwrap();
      setAllData((prev) => [created as ExecModuleAuthConfig, ...prev]);
    } catch (error) {
      console.error('Failed to add ExecModuleAuthConfig:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ExecModuleAuthConfig> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addExecModuleAuthConfig(payload).unwrap();
      setAllData((prev) => [created as ExecModuleAuthConfig, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ExecModuleAuthConfig:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ExecModuleAuthConfig-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addExecModuleAuthConfig(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateExecModuleAuthConfig(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ExecModuleAuthConfig[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ExecModuleAuthConfig[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ExecModuleAuthConfig[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ExecModuleAuthConfig[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ExecModuleAuthConfigTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ExecModuleAuthConfig Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ExecModuleAuthConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ExecModuleAuthConfigForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ExecModuleAuthConfig (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ExecModuleAuthConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ExecModuleAuthConfig;
                ...
                await updateExecModuleAuthConfig(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ExecModuleAuthConfigs.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ExecModuleAuthConfig'}
...
export default ExecModuleAuthConfigTable;
