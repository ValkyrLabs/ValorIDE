...
import { ProductLandingPage } from '@thorapi/model';
import ProductLandingPageForm from '@thorapi/redux/components/form/ProductLandingPageForm';
...
} from '@thorapi/redux/services/ProductLandingPageService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ProductLandingPage update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ProductLandingPage/${rowId}`;
...
const ProductLandingPageTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ProductLandingPage[] => {
    if (Array.isArray(result)) return result as ProductLandingPage[];
    ...
    return Array.isArray(candidate) ? (candidate as ProductLandingPage[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ProductLandingPage[], requestedPage: number) => {
  ...
  const [deleteProductLandingPageCascade] = useDeleteProductLandingPageCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ProductLandingPage | null> => {
    ...
    try {
      ...
      return (await response.json()) as ProductLandingPage;
    } catch (err) {
      console.warn('Could not reload ProductLandingPage row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ProductLandingPage>,
    successField?: string,
  ) => {
    try {
      const saved = await updateProductLandingPage(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ProductLandingPage[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ProductLandingPage';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteProductLandingPageCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ProductLandingPage-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addProductLandingPage(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateProductLandingPage(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ProductLandingPage[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ProductLandingPage[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ProductLandingPage[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ProductLandingPage[]);
    }
  };
