...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ProductFunnelWizard/${rowId}`;
...
const ProductFunnelWizardTable: React.FC = () => {
  ...
  const [deleteProductFunnelWizardCascade] =
    useDeleteProductFunnelWizardCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<ProductFunnelWizard | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateProductFunnelWizard(
        updatedItem as any,
      ).unwrap();
