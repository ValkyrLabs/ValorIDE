...
import { ProductFunnelWizard } from '@thorapi/model';
import ProductFunnelWizardForm from '@thorapi/redux/components/form/ProductFunnelWizardForm';
...
} from '@thorapi/redux/services/ProductFunnelWizardService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ProductFunnelWizard update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ProductFunnelWizard/${rowId}`;
...
const ProductFunnelWizardTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ProductFunnelWizard[] => {
    if (Array.isArray(result)) return result as ProductFunnelWizard[];
    ...
    return Array.isArray(candidate) ? (candidate as ProductFunnelWizard[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ProductFunnelWizard[], requestedPage: number) => {
  ...
  const [deleteProductFunnelWizardCascade] = useDeleteProductFunnelWizardCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ProductFunnelWizard | null> => {
    ...
    try {
      ...
      return (await response.json()) as ProductFunnelWizard;
    } catch (err) {
      console.warn('Could not reload ProductFunnelWizard row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ProductFunnelWizard>,
    successField?: string,
  ) => {
    try {
      const saved = await updateProductFunnelWizard(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ProductFunnelWizard[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ProductFunnelWizard';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteProductFunnelWizardCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ProductFunnelWizard-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addProductFunnelWizard(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateProductFunnelWizard(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ProductFunnelWizard[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ProductFunnelWizard[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ProductFunnelWizard[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ProductFunnelWizard[]);
    }
  };
