...
import StartTrustExecutionRequestForm from '@thorapi/redux/components/form/StartTrustExecutionRequestForm';
...
import {
  useLazyGetStartTrustExecutionRequestsPagedQuery,
...
} from '@thorapi/redux/services/StartTrustExecutionRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `StartTrustExecutionRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/StartTrustExecutionRequest/${rowId}`;

const StartTrustExecutionRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<StartTrustExecutionRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): StartTrustExecutionRequest[] => {
    if (Array.isArray(result)) return result as StartTrustExecutionRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as StartTrustExecutionRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: StartTrustExecutionRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addStartTrustExecutionRequest] = useAddStartTrustExecutionRequestMutation();
  const [updateStartTrustExecutionRequest] = useUpdateStartTrustExecutionRequestMutation();
  const [deleteStartTrustExecutionRequest] = useDeleteStartTrustExecutionRequestMutation();
  const [deleteStartTrustExecutionRequestCascade] = useDeleteStartTrustExecutionRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<StartTrustExecutionRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<StartTrustExecutionRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<StartTrustExecutionRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as StartTrustExecutionRequest;
    } catch (err) {
      console.warn('Could not reload StartTrustExecutionRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<StartTrustExecutionRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateStartTrustExecutionRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as StartTrustExecutionRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update StartTrustExecutionRequest';
      console.error('Failed to update StartTrustExecutionRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteStartTrustExecutionRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteStartTrustExecutionRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<StartTrustExecutionRequest> = { ...source };
      delete (payload as any).id;
      const created = await addStartTrustExecutionRequest(payload).unwrap();
      setAllData((prev) => [created as StartTrustExecutionRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add StartTrustExecutionRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<StartTrustExecutionRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addStartTrustExecutionRequest(payload).unwrap();
      setAllData((prev) => [created as StartTrustExecutionRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) StartTrustExecutionRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `StartTrustExecutionRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addStartTrustExecutionRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateStartTrustExecutionRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as StartTrustExecutionRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as StartTrustExecutionRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as StartTrustExecutionRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as StartTrustExecutionRequest[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="StartTrustExecutionRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="StartTrustExecutionRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create StartTrustExecutionRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <StartTrustExecutionRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter StartTrustExecutionRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing StartTrustExecutionRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof StartTrustExecutionRequest;
                ...
                await updateStartTrustExecutionRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of StartTrustExecutionRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'StartTrustExecutionRequest'}
...
export default StartTrustExecutionRequestTable;
