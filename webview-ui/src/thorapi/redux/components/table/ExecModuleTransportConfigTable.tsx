...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ExecModuleTransportConfig/${rowId}`;
...
const ExecModuleTransportConfigTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addExecModuleTransportConfig] =
    useAddExecModuleTransportConfigMutation();
  const [updateExecModuleTransportConfig] =
    useUpdateExecModuleTransportConfigMutation();
  const [deleteExecModuleTransportConfig] =
    useDeleteExecModuleTransportConfigMutation();
  const [deleteExecModuleTransportConfigCascade] =
    useDeleteExecModuleTransportConfigCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<ExecModuleTransportConfig | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<ExecModuleTransportConfig | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload ExecModuleTransportConfig row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateExecModuleTransportConfig(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ExecModuleTransportConfig";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecModuleTransportConfigCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) ExecModuleTransportConfig:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter ExecModuleTransportConfig (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
