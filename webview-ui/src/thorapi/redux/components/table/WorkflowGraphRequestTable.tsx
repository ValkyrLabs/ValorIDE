...
import { WorkflowGraphRequest } from '@thorapi/model';
import WorkflowGraphRequestForm from '@thorapi/redux/components/form/WorkflowGraphRequestForm';
...
import {
  useLazyGetWorkflowGraphRequestsPagedQuery,
...
} from '@thorapi/redux/services/WorkflowGraphRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `WorkflowGraphRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/WorkflowGraphRequest/${rowId}`;

const WorkflowGraphRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<WorkflowGraphRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): WorkflowGraphRequest[] => {
    if (Array.isArray(result)) return result as WorkflowGraphRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as WorkflowGraphRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: WorkflowGraphRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addWorkflowGraphRequest] = useAddWorkflowGraphRequestMutation();
  const [updateWorkflowGraphRequest] = useUpdateWorkflowGraphRequestMutation();
  const [deleteWorkflowGraphRequest] = useDeleteWorkflowGraphRequestMutation();
  const [deleteWorkflowGraphRequestCascade] = useDeleteWorkflowGraphRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<WorkflowGraphRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<WorkflowGraphRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<WorkflowGraphRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as WorkflowGraphRequest;
    } catch (err) {
      console.warn('Could not reload WorkflowGraphRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<WorkflowGraphRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWorkflowGraphRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as WorkflowGraphRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update WorkflowGraphRequest';
      console.error('Failed to update WorkflowGraphRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteWorkflowGraphRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<WorkflowGraphRequest> = { ...source };
      delete (payload as any).id;
      const created = await addWorkflowGraphRequest(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add WorkflowGraphRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<WorkflowGraphRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addWorkflowGraphRequest(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) WorkflowGraphRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `WorkflowGraphRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWorkflowGraphRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWorkflowGraphRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as WorkflowGraphRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as WorkflowGraphRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as WorkflowGraphRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as WorkflowGraphRequest[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.WorkflowGraphRequest',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="WorkflowGraphRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="WorkflowGraphRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create WorkflowGraphRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <WorkflowGraphRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter WorkflowGraphRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing WorkflowGraphRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof WorkflowGraphRequest;
                ...
                await updateWorkflowGraphRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of WorkflowGraphRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'WorkflowGraphRequest'}
...
export default WorkflowGraphRequestTable;
