...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/WorkflowGraphRequest/${rowId}`;
...
const WorkflowGraphRequestTable: React.FC = () => {
  ...
  const [deleteWorkflowGraphRequestCascade] =
    useDeleteWorkflowGraphRequestCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<WorkflowGraphRequest | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateWorkflowGraphRequest(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update WorkflowGraphRequest";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphRequestCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter WorkflowGraphRequest (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
