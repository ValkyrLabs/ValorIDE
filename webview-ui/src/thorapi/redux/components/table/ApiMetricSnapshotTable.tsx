...
import ApiMetricSnapshotForm from '@thorapi/redux/components/form/ApiMetricSnapshotForm';
...
} from '@thorapi/redux/services/ApiMetricSnapshotService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ApiMetricSnapshot update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ApiMetricSnapshot/${rowId}`;
...
const ApiMetricSnapshotTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ApiMetricSnapshot[] => {
    if (Array.isArray(result)) return result as ApiMetricSnapshot[];
    ...
    return Array.isArray(candidate) ? (candidate as ApiMetricSnapshot[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ApiMetricSnapshot[], requestedPage: number) => {
  ...
  const [deleteApiMetricSnapshotCascade] = useDeleteApiMetricSnapshotCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ApiMetricSnapshot | null> => {
    ...
    try {
      ...
      return (await response.json()) as ApiMetricSnapshot;
    } catch (err) {
      console.warn('Could not reload ApiMetricSnapshot row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ApiMetricSnapshot>,
    successField?: string,
  ) => {
    try {
      const saved = await updateApiMetricSnapshot(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ApiMetricSnapshot[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ApiMetricSnapshot';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteApiMetricSnapshotCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ApiMetricSnapshot-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addApiMetricSnapshot(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateApiMetricSnapshot(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ApiMetricSnapshot[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ApiMetricSnapshot[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ApiMetricSnapshot[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ApiMetricSnapshot[]);
    }
  };
