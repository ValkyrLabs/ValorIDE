...
import { SecureKey } from '@thorapi/model';
import SecureKeyForm from '@thorapi/redux/components/form/SecureKeyForm';
...
} from '@thorapi/redux/services/SecureKeyService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SecureKey update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SecureKey/${rowId}`;
...
const SecureKeyTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SecureKey[] => {
    if (Array.isArray(result)) return result as SecureKey[];
    ...
    return Array.isArray(candidate) ? (candidate as SecureKey[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SecureKey[], requestedPage: number) => {
  ...
  const [deleteSecureKeyCascade] = useDeleteSecureKeyCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SecureKey | null> => {
    ...
    try {
      ...
      return (await response.json()) as SecureKey;
    } catch (err) {
      console.warn('Could not reload SecureKey row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SecureKey>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSecureKey(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SecureKey[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SecureKey';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSecureKeyCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SecureKey-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSecureKey(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSecureKey(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SecureKey[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SecureKey[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SecureKey[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SecureKey[]);
    }
  };
