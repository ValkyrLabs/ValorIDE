...
import GraphLinkForm from '@thorapi/redux/components/form/GraphLinkForm';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `GraphLink update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/GraphLink/${rowId}`;

const GraphLinkTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<GraphLink[]>([]);
  ...
  const normalizePageRows = (result: unknown): GraphLink[] => {
    if (Array.isArray(result)) return result as GraphLink[];
    ...
    return Array.isArray(candidate) ? (candidate as GraphLink[]) : [];
  };

  const mergeFetchedRows = (incomingRows: GraphLink[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addGraphLink] = useAddGraphLinkMutation();
  const [updateGraphLink] = useUpdateGraphLinkMutation();
  const [deleteGraphLink] = useDeleteGraphLinkMutation();
  const [deleteGraphLinkCascade] = useDeleteGraphLinkCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<GraphLink>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<GraphLink | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<GraphLink | null> => {
    ...
    try {
      ...
      console.warn('Could not reload GraphLink row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<GraphLink>,
    successField?: string,
  ) => {
    try {
      const saved = await updateGraphLink(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as GraphLink[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update GraphLink';
      console.error('Failed to update GraphLink:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteGraphLinkCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteGraphLink(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<GraphLink> = { ...source };
      delete (payload as any).id;
      const created = await addGraphLink(payload).unwrap();
      setAllData((prev) => [created as GraphLink, ...prev]);
    } catch (error) {
      console.error('Failed to add GraphLink:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<GraphLink> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addGraphLink(payload).unwrap();
      setAllData((prev) => [created as GraphLink, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) GraphLink:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `GraphLink-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addGraphLink(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateGraphLink(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as GraphLink[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as GraphLink[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as GraphLink[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as GraphLink[]);
    }
  };
    ...
    <>
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create GraphLink</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <GraphLinkForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter GraphLink (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing GraphLink</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof GraphLink;
                ...
                await updateGraphLink(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of GraphLinks.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'GraphLink'}
