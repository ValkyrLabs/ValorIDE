...
import { PresignResponse } from '@thorapi/model';
import PresignResponseForm from '@thorapi/redux/components/form/PresignResponseForm';
...
} from '@thorapi/redux/services/PresignResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `PresignResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/PresignResponse/${rowId}`;
...
const PresignResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): PresignResponse[] => {
    if (Array.isArray(result)) return result as PresignResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as PresignResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: PresignResponse[], requestedPage: number) => {
  ...
  const [deletePresignResponseCascade] = useDeletePresignResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<PresignResponse | null> => {
    ...
    try {
      ...
      console.warn('Could not reload PresignResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<PresignResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updatePresignResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as PresignResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update PresignResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePresignResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `PresignResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addPresignResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updatePresignResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as PresignResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as PresignResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as PresignResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as PresignResponse[]);
    }
  };
