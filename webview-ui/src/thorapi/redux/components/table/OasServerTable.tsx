...
import { OasServer } from '@thorapi/model';
import OasServerForm from '@thorapi/redux/components/form/OasServerForm';
...
} from '@thorapi/redux/services/OasServerService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `OasServer update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/OasServer/${rowId}`;
...
const OasServerTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): OasServer[] => {
    if (Array.isArray(result)) return result as OasServer[];
    ...
    return Array.isArray(candidate) ? (candidate as OasServer[]) : [];
  };

  const mergeFetchedRows = (incomingRows: OasServer[], requestedPage: number) => {
  ...
  const [deleteOasServerCascade] = useDeleteOasServerCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<OasServer | null> => {
    ...
    try {
      ...
      return (await response.json()) as OasServer;
    } catch (err) {
      console.warn('Could not reload OasServer row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<OasServer>,
    successField?: string,
  ) => {
    try {
      const saved = await updateOasServer(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as OasServer[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update OasServer';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOasServerCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `OasServer-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addOasServer(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateOasServer(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as OasServer[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as OasServer[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as OasServer[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as OasServer[]);
    }
  };
