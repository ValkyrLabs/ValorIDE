...
import { MediaObject } from '@thorapi/model';
import MediaObjectForm from '@thorapi/redux/components/form/MediaObjectForm';
...
} from '@thorapi/redux/services/MediaObjectService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `MediaObject update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/MediaObject/${rowId}`;
...
const MediaObjectTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): MediaObject[] => {
    if (Array.isArray(result)) return result as MediaObject[];
    ...
    return Array.isArray(candidate) ? (candidate as MediaObject[]) : [];
  };

  const mergeFetchedRows = (incomingRows: MediaObject[], requestedPage: number) => {
  ...
  const [deleteMediaObjectCascade] = useDeleteMediaObjectCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<MediaObject | null> => {
    ...
    try {
      ...
      console.warn('Could not reload MediaObject row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<MediaObject>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMediaObject(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as MediaObject[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update MediaObject';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMediaObjectCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `MediaObject-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMediaObject(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMediaObject(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as MediaObject[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as MediaObject[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as MediaObject[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as MediaObject[]);
    }
  };
