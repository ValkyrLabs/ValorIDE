...
import { Agent } from '@thorapi/model';
import AgentForm from '@thorapi/redux/components/form/AgentForm';
...
} from '@thorapi/redux/services/AgentService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'autonomousTableCreationLevel': { type: 'enum', enumValues: [
    'none', 'guided', 'bounded', 'high', 'unlimited', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Agent update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Agent/${rowId}`;
...
const AgentTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Agent[] => {
    ...
    return Array.isArray(candidate) ? (candidate as Agent[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Agent[], requestedPage: number) => {
  ...
  const [deleteAgentCascade] = useDeleteAgentCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Agent | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Agent row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Agent>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAgent(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Agent[]);
      if (successField) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAgentCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Agent-export-${stamp}.json`;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Agent[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Agent[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Agent[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Agent[]);
    }
  };
