...
import { JobApplicationResponse } from '@thorapi/model';
import JobApplicationResponseForm from '@thorapi/redux/components/form/JobApplicationResponseForm';
...
} from '@thorapi/redux/services/JobApplicationResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `JobApplicationResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/JobApplicationResponse/${rowId}`;
...
const JobApplicationResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): JobApplicationResponse[] => {
    if (Array.isArray(result)) return result as JobApplicationResponse[];
  ...
  const mergeFetchedRows = (incomingRows: JobApplicationResponse[], requestedPage: number) => {
  ...
  const [deleteJobApplicationResponseCascade] = useDeleteJobApplicationResponseCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<JobApplicationResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<JobApplicationResponse | null> => {
    ...
    try {
      ...
      console.warn('Could not reload JobApplicationResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<JobApplicationResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateJobApplicationResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as JobApplicationResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update JobApplicationResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteJobApplicationResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `JobApplicationResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addJobApplicationResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateJobApplicationResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as JobApplicationResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as JobApplicationResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as JobApplicationResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as JobApplicationResponse[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter JobApplicationResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
