...
import { Cell } from '@thorapi/model';
import CellForm from '@thorapi/redux/components/form/CellForm';
...
} from '@thorapi/redux/services/CellService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Cell update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Cell/${rowId}`;
...
const CellTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Cell[] => {
    if (Array.isArray(result)) return result as Cell[];
    ...
    return Array.isArray(candidate) ? (candidate as Cell[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Cell[], requestedPage: number) => {
  ...
  const [deleteCellCascade] = useDeleteCellCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Cell | null> => {
    ...
    try {
      ...
      return (await response.json()) as Cell;
    } catch (err) {
      console.warn('Could not reload Cell row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Cell>,
    successField?: string,
  ) => {
    try {
      const saved = await updateCell(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Cell[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Cell';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCellCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Cell-export-${stamp}.json`;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Cell[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Cell[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Cell[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Cell[]);
    }
  };
