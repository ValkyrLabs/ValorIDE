...
import { OasComponent } from '@thorapi/model';
import OasComponentForm from '@thorapi/redux/components/form/OasComponentForm';
...
} from '@thorapi/redux/services/OasComponentService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `OasComponent update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/OasComponent/${rowId}`;
...
const OasComponentTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): OasComponent[] => {
    if (Array.isArray(result)) return result as OasComponent[];
    ...
    return Array.isArray(candidate) ? (candidate as OasComponent[]) : [];
  };

  const mergeFetchedRows = (incomingRows: OasComponent[], requestedPage: number) => {
  ...
  const [deleteOasComponentCascade] = useDeleteOasComponentCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<OasComponent | null> => {
    ...
    try {
      ...
      return (await response.json()) as OasComponent;
    } catch (err) {
      console.warn('Could not reload OasComponent row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<OasComponent>,
    successField?: string,
  ) => {
    try {
      const saved = await updateOasComponent(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as OasComponent[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update OasComponent';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOasComponentCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `OasComponent-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addOasComponent(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateOasComponent(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as OasComponent[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as OasComponent[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as OasComponent[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as OasComponent[]);
    }
  };
