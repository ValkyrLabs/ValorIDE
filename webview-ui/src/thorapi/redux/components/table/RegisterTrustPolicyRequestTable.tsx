...
import { RegisterTrustPolicyRequest } from '@thorapi/model';
import RegisterTrustPolicyRequestForm from '@thorapi/redux/components/form/RegisterTrustPolicyRequestForm';
...
import {
  useLazyGetRegisterTrustPolicyRequestsPagedQuery,
...
} from '@thorapi/redux/services/RegisterTrustPolicyRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RegisterTrustPolicyRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RegisterTrustPolicyRequest/${rowId}`;

const RegisterTrustPolicyRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RegisterTrustPolicyRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): RegisterTrustPolicyRequest[] => {
    if (Array.isArray(result)) return result as RegisterTrustPolicyRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as RegisterTrustPolicyRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RegisterTrustPolicyRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addRegisterTrustPolicyRequest] = useAddRegisterTrustPolicyRequestMutation();
  const [updateRegisterTrustPolicyRequest] = useUpdateRegisterTrustPolicyRequestMutation();
  const [deleteRegisterTrustPolicyRequest] = useDeleteRegisterTrustPolicyRequestMutation();
  const [deleteRegisterTrustPolicyRequestCascade] = useDeleteRegisterTrustPolicyRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<RegisterTrustPolicyRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RegisterTrustPolicyRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<RegisterTrustPolicyRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as RegisterTrustPolicyRequest;
    } catch (err) {
      console.warn('Could not reload RegisterTrustPolicyRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RegisterTrustPolicyRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRegisterTrustPolicyRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RegisterTrustPolicyRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RegisterTrustPolicyRequest';
      console.error('Failed to update RegisterTrustPolicyRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRegisterTrustPolicyRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteRegisterTrustPolicyRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RegisterTrustPolicyRequest> = { ...source };
      delete (payload as any).id;
      const created = await addRegisterTrustPolicyRequest(payload).unwrap();
      setAllData((prev) => [created as RegisterTrustPolicyRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add RegisterTrustPolicyRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RegisterTrustPolicyRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRegisterTrustPolicyRequest(payload).unwrap();
      setAllData((prev) => [created as RegisterTrustPolicyRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) RegisterTrustPolicyRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RegisterTrustPolicyRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRegisterTrustPolicyRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRegisterTrustPolicyRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RegisterTrustPolicyRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RegisterTrustPolicyRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RegisterTrustPolicyRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RegisterTrustPolicyRequest[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.RegisterTrustPolicyRequest',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="RegisterTrustPolicyRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="RegisterTrustPolicyRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RegisterTrustPolicyRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RegisterTrustPolicyRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RegisterTrustPolicyRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RegisterTrustPolicyRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RegisterTrustPolicyRequest;
                ...
                await updateRegisterTrustPolicyRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RegisterTrustPolicyRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RegisterTrustPolicyRequest'}
...
export default RegisterTrustPolicyRequestTable;
