...
import { GrayMatter } from '@thorapi/model';
import GrayMatterForm from '@thorapi/redux/components/form/GrayMatterForm';
...
import {
  useLazyGetGrayMattersPagedQuery,
  useAddGrayMatterMutation,
  useUpdateGrayMatterMutation,
  useDeleteGrayMatterMutation,
  useDeleteGrayMatterCascadeMutation
} from '@thorapi/redux/services/GrayMatterService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `GrayMatter update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/GrayMatter/${rowId}`;

const GrayMatterTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<GrayMatter[]>([]);
  ...
  const normalizePageRows = (result: unknown): GrayMatter[] => {
    if (Array.isArray(result)) return result as GrayMatter[];
    ...
    return Array.isArray(candidate) ? (candidate as GrayMatter[]) : [];
  };

  const mergeFetchedRows = (incomingRows: GrayMatter[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addGrayMatter] = useAddGrayMatterMutation();
  const [updateGrayMatter] = useUpdateGrayMatterMutation();
  const [deleteGrayMatter] = useDeleteGrayMatterMutation();
  const [deleteGrayMatterCascade] = useDeleteGrayMatterCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<GrayMatter>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<GrayMatter | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<GrayMatter | null> => {
    ...
    try {
      ...
      return (await response.json()) as GrayMatter;
    } catch (err) {
      console.warn('Could not reload GrayMatter row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<GrayMatter>,
    successField?: string,
  ) => {
    try {
      const saved = await updateGrayMatter(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as GrayMatter[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update GrayMatter';
      console.error('Failed to update GrayMatter:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteGrayMatterCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteGrayMatter(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<GrayMatter> = { ...source };
      delete (payload as any).id;
      const created = await addGrayMatter(payload).unwrap();
      setAllData((prev) => [created as GrayMatter, ...prev]);
    } catch (error) {
      console.error('Failed to add GrayMatter:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<GrayMatter> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addGrayMatter(payload).unwrap();
      setAllData((prev) => [created as GrayMatter, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) GrayMatter:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `GrayMatter-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addGrayMatter(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateGrayMatter(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as GrayMatter[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as GrayMatter[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as GrayMatter[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as GrayMatter[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.GrayMatter',
        objectId: rowId,
      });
    ...
    <>
          ...
          >
            ...
            <FloatingControlPanel
              description="GrayMatter Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create GrayMatter</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <GrayMatterForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter GrayMatter (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing GrayMatter</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof GrayMatter;
                ...
                await updateGrayMatter(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of GrayMatters.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'GrayMatter'}
...
export default GrayMatterTable;
