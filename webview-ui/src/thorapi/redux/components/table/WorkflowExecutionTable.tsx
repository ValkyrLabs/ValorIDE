...
import { WorkflowExecution } from '@thorapi/model';
import WorkflowExecutionForm from '@thorapi/redux/components/form/WorkflowExecutionForm';
...
} from '@thorapi/redux/services/WorkflowExecutionService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `WorkflowExecution update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/WorkflowExecution/${rowId}`;
...
const WorkflowExecutionTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): WorkflowExecution[] => {
    if (Array.isArray(result)) return result as WorkflowExecution[];
    ...
    return Array.isArray(candidate) ? (candidate as WorkflowExecution[]) : [];
  };

  const mergeFetchedRows = (incomingRows: WorkflowExecution[], requestedPage: number) => {
  ...
  const [deleteWorkflowExecutionCascade] = useDeleteWorkflowExecutionCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<WorkflowExecution | null> => {
    ...
    try {
      ...
      console.warn('Could not reload WorkflowExecution row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<WorkflowExecution>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWorkflowExecution(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as WorkflowExecution[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update WorkflowExecution';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowExecutionCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `WorkflowExecution-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWorkflowExecution(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWorkflowExecution(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as WorkflowExecution[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as WorkflowExecution[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as WorkflowExecution[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as WorkflowExecution[]);
    }
  };
