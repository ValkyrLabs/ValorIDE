...
import { RetryPolicy } from '@thorapi/model';
import RetryPolicyForm from '@thorapi/redux/components/form/RetryPolicyForm';
...
} from '@thorapi/redux/services/RetryPolicyService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RetryPolicy update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RetryPolicy/${rowId}`;
...
const RetryPolicyTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): RetryPolicy[] => {
    if (Array.isArray(result)) return result as RetryPolicy[];
    ...
    return Array.isArray(candidate) ? (candidate as RetryPolicy[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RetryPolicy[], requestedPage: number) => {
  ...
  const [deleteRetryPolicyCascade] = useDeleteRetryPolicyCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<RetryPolicy | null> => {
    ...
    try {
      ...
      return (await response.json()) as RetryPolicy;
    } catch (err) {
      console.warn('Could not reload RetryPolicy row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RetryPolicy>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRetryPolicy(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RetryPolicy[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RetryPolicy';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRetryPolicyCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RetryPolicy-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRetryPolicy(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRetryPolicy(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RetryPolicy[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RetryPolicy[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RetryPolicy[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RetryPolicy[]);
    }
  };
