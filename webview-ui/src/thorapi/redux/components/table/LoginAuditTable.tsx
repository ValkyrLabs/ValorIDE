...
import { LoginAudit } from '@thorapi/model';
import LoginAuditForm from '@thorapi/redux/components/form/LoginAuditForm';
...
const columnSchema: Record<string, ColumnSchema> = {
  'loginStatus': { type: 'enum', enumValues: [
    'SUCCESS', 'FAILED_INVALID_CREDENTIALS', 'FAILED_2FA_REQUIRED', 'FAILED_2FA_INVALID', 'FAILED_ACCOUNT_LOCKED', 'FAILED_ACCOUNT_DISABLED', 'SUCCESS_2FA_VERIFIED', 
  ], enumValueType: 'string' },
  'loginMethod': { type: 'enum', enumValues: [
    'USERNAME_PASSWORD', 'GITHUB_OAUTH', 'GOOGLE_OAUTH', 'SMS_OTP', 'TOTP', 
  ], enumValueType: 'string' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `LoginAudit update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/LoginAudit/${rowId}`;

const LoginAuditTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<LoginAudit[]>([]);
  ...
  const normalizePageRows = (result: unknown): LoginAudit[] => {
    if (Array.isArray(result)) return result as LoginAudit[];
    ...
    return Array.isArray(candidate) ? (candidate as LoginAudit[]) : [];
  };

  const mergeFetchedRows = (incomingRows: LoginAudit[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addLoginAudit] = useAddLoginAuditMutation();
  const [updateLoginAudit] = useUpdateLoginAuditMutation();
  const [deleteLoginAudit] = useDeleteLoginAuditMutation();
  const [deleteLoginAuditCascade] = useDeleteLoginAuditCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<LoginAudit>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<LoginAudit | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<LoginAudit | null> => {
    ...
    try {
      ...
      return (await response.json()) as LoginAudit;
    } catch (err) {
      console.warn('Could not reload LoginAudit row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<LoginAudit>,
    successField?: string,
  ) => {
    try {
      const saved = await updateLoginAudit(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as LoginAudit[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update LoginAudit';
      console.error('Failed to update LoginAudit:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteLoginAuditCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteLoginAudit(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<LoginAudit> = { ...source };
      delete (payload as any).id;
      const created = await addLoginAudit(payload).unwrap();
      setAllData((prev) => [created as LoginAudit, ...prev]);
    } catch (error) {
      console.error('Failed to add LoginAudit:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<LoginAudit> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addLoginAudit(payload).unwrap();
      setAllData((prev) => [created as LoginAudit, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) LoginAudit:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `LoginAudit-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addLoginAudit(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateLoginAudit(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as LoginAudit[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as LoginAudit[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as LoginAudit[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as LoginAudit[]);
    }
  };
    ...
    <>
          ...
          >
            ...
            <FloatingControlPanel
              description="LoginAudit Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create LoginAudit</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <LoginAuditForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter LoginAudit (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing LoginAudit</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof LoginAudit;
                ...
                await updateLoginAudit(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of LoginAudits.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'LoginAudit'}
