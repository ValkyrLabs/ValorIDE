...
import { TenantSchemaRegistry } from '@thorapi/model';
import TenantSchemaRegistryForm from '@thorapi/redux/components/form/TenantSchemaRegistryForm';
...
import {
  useLazyGetTenantSchemaRegistrysPagedQuery,
...
} from '@thorapi/redux/services/TenantSchemaRegistryService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'databaseScope': { type: 'enum', enumValues: [
    'valkyr_managed', 'customer_runtime', 'app_database', 'byoc_database', 
  ], enumValueType: 'string' },
  'isolationMode': { type: 'enum', enumValues: [
    'org_schema', 'app_schema', 'app_database', 'byoc_database', 
  ], enumValueType: 'string' },
  'status': { type: 'enum', enumValues: [
    'requested', 'provisioning', 'active', 'validating', 'suspended', 'failed', 'retired', 
  ], enumValueType: 'string' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TenantSchemaRegistry update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TenantSchemaRegistry/${rowId}`;

const TenantSchemaRegistryTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TenantSchemaRegistry[]>([]);
  ...
  const normalizePageRows = (result: unknown): TenantSchemaRegistry[] => {
    if (Array.isArray(result)) return result as TenantSchemaRegistry[];
    ...
    return Array.isArray(candidate) ? (candidate as TenantSchemaRegistry[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TenantSchemaRegistry[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addTenantSchemaRegistry] = useAddTenantSchemaRegistryMutation();
  const [updateTenantSchemaRegistry] = useUpdateTenantSchemaRegistryMutation();
  const [deleteTenantSchemaRegistry] = useDeleteTenantSchemaRegistryMutation();
  const [deleteTenantSchemaRegistryCascade] = useDeleteTenantSchemaRegistryCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TenantSchemaRegistry>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TenantSchemaRegistry | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TenantSchemaRegistry | null> => {
    ...
    try {
      ...
      return (await response.json()) as TenantSchemaRegistry;
    } catch (err) {
      console.warn('Could not reload TenantSchemaRegistry row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TenantSchemaRegistry>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTenantSchemaRegistry(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TenantSchemaRegistry[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TenantSchemaRegistry';
      console.error('Failed to update TenantSchemaRegistry:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTenantSchemaRegistryCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTenantSchemaRegistry(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TenantSchemaRegistry> = { ...source };
      delete (payload as any).id;
      const created = await addTenantSchemaRegistry(payload).unwrap();
      setAllData((prev) => [created as TenantSchemaRegistry, ...prev]);
    } catch (error) {
      console.error('Failed to add TenantSchemaRegistry:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TenantSchemaRegistry> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTenantSchemaRegistry(payload).unwrap();
      setAllData((prev) => [created as TenantSchemaRegistry, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TenantSchemaRegistry:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TenantSchemaRegistry-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTenantSchemaRegistry(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTenantSchemaRegistry(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TenantSchemaRegistry[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TenantSchemaRegistry[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TenantSchemaRegistry[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TenantSchemaRegistry[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="TenantSchemaRegistryTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="TenantSchemaRegistry Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TenantSchemaRegistry</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TenantSchemaRegistryForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TenantSchemaRegistry (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TenantSchemaRegistry</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TenantSchemaRegistry;
                ...
                await updateTenantSchemaRegistry(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TenantSchemaRegistrys.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TenantSchemaRegistry'}
...
export default TenantSchemaRegistryTable;
