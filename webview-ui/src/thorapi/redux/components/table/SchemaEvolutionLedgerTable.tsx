...
import { SchemaEvolutionLedger } from '@thorapi/model';
import SchemaEvolutionLedgerForm from '@thorapi/redux/components/form/SchemaEvolutionLedgerForm';
...
import {
  useLazyGetSchemaEvolutionLedgersPagedQuery,
...
} from '@thorapi/redux/services/SchemaEvolutionLedgerService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'category': { type: 'enum', enumValues: [
    'string_widening', 'enum_constraint_expansion', 'nullable_column_addition', 'data_backfill', 'manual_review', 
  ], enumValueType: 'string' },
  'sourceKind': { type: 'enum', enumValues: [
    'thorapi_openapi', 'generated_entity_metadata', 'tenant_bundle', 
  ], enumValueType: 'string' },
  'status': { type: 'enum', enumValues: [
    'planned', 'applying', 'applied', 'skipped', 'failed', 'manual_review_required', 
  ], enumValueType: 'string' },
  'safeAutoApply': { type: 'boolean' },
  'autoApplied': { type: 'boolean' },
  'approvalRequired': { type: 'boolean' },
  'plannedAt': { type: 'datetime' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SchemaEvolutionLedger update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SchemaEvolutionLedger/${rowId}`;

const SchemaEvolutionLedgerTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SchemaEvolutionLedger[]>([]);
  ...
  const normalizePageRows = (result: unknown): SchemaEvolutionLedger[] => {
    if (Array.isArray(result)) return result as SchemaEvolutionLedger[];
    ...
    return Array.isArray(candidate) ? (candidate as SchemaEvolutionLedger[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SchemaEvolutionLedger[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addSchemaEvolutionLedger] = useAddSchemaEvolutionLedgerMutation();
  const [updateSchemaEvolutionLedger] = useUpdateSchemaEvolutionLedgerMutation();
  const [deleteSchemaEvolutionLedger] = useDeleteSchemaEvolutionLedgerMutation();
  const [deleteSchemaEvolutionLedgerCascade] = useDeleteSchemaEvolutionLedgerCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<SchemaEvolutionLedger>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SchemaEvolutionLedger | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<SchemaEvolutionLedger | null> => {
    ...
    try {
      ...
      return (await response.json()) as SchemaEvolutionLedger;
    } catch (err) {
      console.warn('Could not reload SchemaEvolutionLedger row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SchemaEvolutionLedger>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSchemaEvolutionLedger(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SchemaEvolutionLedger[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SchemaEvolutionLedger';
      console.error('Failed to update SchemaEvolutionLedger:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSchemaEvolutionLedgerCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteSchemaEvolutionLedger(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SchemaEvolutionLedger> = { ...source };
      delete (payload as any).id;
      const created = await addSchemaEvolutionLedger(payload).unwrap();
      setAllData((prev) => [created as SchemaEvolutionLedger, ...prev]);
    } catch (error) {
      console.error('Failed to add SchemaEvolutionLedger:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SchemaEvolutionLedger> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSchemaEvolutionLedger(payload).unwrap();
      setAllData((prev) => [created as SchemaEvolutionLedger, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) SchemaEvolutionLedger:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SchemaEvolutionLedger-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSchemaEvolutionLedger(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSchemaEvolutionLedger(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SchemaEvolutionLedger[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SchemaEvolutionLedger[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SchemaEvolutionLedger[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SchemaEvolutionLedger[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="SchemaEvolutionLedgerTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="SchemaEvolutionLedger Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SchemaEvolutionLedger</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SchemaEvolutionLedgerForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SchemaEvolutionLedger (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SchemaEvolutionLedger</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SchemaEvolutionLedger;
                ...
                await updateSchemaEvolutionLedger(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SchemaEvolutionLedgers.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SchemaEvolutionLedger'}
...
export default SchemaEvolutionLedgerTable;
