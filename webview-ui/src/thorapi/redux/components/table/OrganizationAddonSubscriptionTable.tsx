...
import { OrganizationAddonSubscription } from '@thorapi/model';
import OrganizationAddonSubscriptionForm from '@thorapi/redux/components/form/OrganizationAddonSubscriptionForm';
...
import {
  useLazyGetOrganizationAddonSubscriptionsPagedQuery,
...
} from '@thorapi/redux/services/OrganizationAddonSubscriptionService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'planTier': { type: 'enum', enumValues: [
    'FREE', 'SOLO', 'TEAM', 'PRO', 'RESELLER', 'ENTERPRISE', 
  ], enumValueType: 'string' },
  'monthlyPriceCredits': { type: 'number' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `OrganizationAddonSubscription update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/OrganizationAddonSubscription/${rowId}`;

const OrganizationAddonSubscriptionTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<OrganizationAddonSubscription[]>([]);
  ...
  const normalizePageRows = (result: unknown): OrganizationAddonSubscription[] => {
    if (Array.isArray(result)) return result as OrganizationAddonSubscription[];
    ...
    return Array.isArray(candidate) ? (candidate as OrganizationAddonSubscription[]) : [];
  };

  const mergeFetchedRows = (incomingRows: OrganizationAddonSubscription[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addOrganizationAddonSubscription] = useAddOrganizationAddonSubscriptionMutation();
  const [updateOrganizationAddonSubscription] = useUpdateOrganizationAddonSubscriptionMutation();
  const [deleteOrganizationAddonSubscription] = useDeleteOrganizationAddonSubscriptionMutation();
  const [deleteOrganizationAddonSubscriptionCascade] = useDeleteOrganizationAddonSubscriptionCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<OrganizationAddonSubscription>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<OrganizationAddonSubscription | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<OrganizationAddonSubscription | null> => {
    ...
    try {
      ...
      return (await response.json()) as OrganizationAddonSubscription;
    } catch (err) {
      console.warn('Could not reload OrganizationAddonSubscription row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<OrganizationAddonSubscription>,
    successField?: string,
  ) => {
    try {
      const saved = await updateOrganizationAddonSubscription(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as OrganizationAddonSubscription[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update OrganizationAddonSubscription';
      console.error('Failed to update OrganizationAddonSubscription:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOrganizationAddonSubscriptionCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteOrganizationAddonSubscription(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<OrganizationAddonSubscription> = { ...source };
      delete (payload as any).id;
      const created = await addOrganizationAddonSubscription(payload).unwrap();
      setAllData((prev) => [created as OrganizationAddonSubscription, ...prev]);
    } catch (error) {
      console.error('Failed to add OrganizationAddonSubscription:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<OrganizationAddonSubscription> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addOrganizationAddonSubscription(payload).unwrap();
      setAllData((prev) => [created as OrganizationAddonSubscription, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) OrganizationAddonSubscription:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `OrganizationAddonSubscription-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addOrganizationAddonSubscription(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateOrganizationAddonSubscription(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as OrganizationAddonSubscription[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as OrganizationAddonSubscription[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as OrganizationAddonSubscription[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as OrganizationAddonSubscription[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="OrganizationAddonSubscriptionTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="OrganizationAddonSubscription Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create OrganizationAddonSubscription</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <OrganizationAddonSubscriptionForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter OrganizationAddonSubscription (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing OrganizationAddonSubscription</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof OrganizationAddonSubscription;
                ...
                await updateOrganizationAddonSubscription(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of OrganizationAddonSubscriptions.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'OrganizationAddonSubscription'}
...
export default OrganizationAddonSubscriptionTable;
