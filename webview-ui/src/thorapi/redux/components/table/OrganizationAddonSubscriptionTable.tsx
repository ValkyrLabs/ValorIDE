...
const columnSchema: Record<string, ColumnSchema> = {
  planTier: {
    type: "enum",
    enumValues: ["FREE", "SOLO", "TEAM", "PRO", "RESELLER", "ENTERPRISE"],
...
const stringFieldCandidates = ["planTier", "status", "id"];
...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/OrganizationAddonSubscription/${rowId}`;
...
const OrganizationAddonSubscriptionTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addOrganizationAddonSubscription] =
    useAddOrganizationAddonSubscriptionMutation();
  const [updateOrganizationAddonSubscription] =
    useUpdateOrganizationAddonSubscriptionMutation();
  const [deleteOrganizationAddonSubscription] =
    useDeleteOrganizationAddonSubscriptionMutation();
  const [deleteOrganizationAddonSubscriptionCascade] =
    useDeleteOrganizationAddonSubscriptionCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] =
    useState<OrganizationAddonSubscription | null>(null);
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<OrganizationAddonSubscription | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload OrganizationAddonSubscription row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateOrganizationAddonSubscription(
        updatedItem as any,
      ).unwrap();
      ...
      setAllData(
        (prev) =>
          mergeUpdatedRow(
            ...
            rowId,
            hydrated as any,
          ) as OrganizationAddonSubscription[],
      );
      if (successField) {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update OrganizationAddonSubscription";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOrganizationAddonSubscriptionCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) OrganizationAddonSubscription:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter OrganizationAddonSubscription (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
      ...
      <MarkdownEditorModal
        ...
        onSave={async (updatedText) => {
          try {
            ...
            const field =
              richModal.field as keyof OrganizationAddonSubscription;
            if (!rid || !field) {
