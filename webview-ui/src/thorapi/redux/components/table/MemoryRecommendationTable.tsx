...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/MemoryRecommendation/${rowId}`;
...
const MemoryRecommendationTable: React.FC = () => {
  ...
  const [deleteMemoryRecommendationCascade] =
    useDeleteMemoryRecommendationCascadeMutation();
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateMemoryRecommendation(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update MemoryRecommendation";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMemoryRecommendationCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter MemoryRecommendation (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
