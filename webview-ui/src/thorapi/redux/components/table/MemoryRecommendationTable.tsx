...
import { MemoryRecommendation } from '@thorapi/model';
import MemoryRecommendationForm from '@thorapi/redux/components/form/MemoryRecommendationForm';
...
import {
  useLazyGetMemoryRecommendationsPagedQuery,
...
} from '@thorapi/redux/services/MemoryRecommendationService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `MemoryRecommendation update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/MemoryRecommendation/${rowId}`;

const MemoryRecommendationTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<MemoryRecommendation[]>([]);
  ...
  const normalizePageRows = (result: unknown): MemoryRecommendation[] => {
    if (Array.isArray(result)) return result as MemoryRecommendation[];
    ...
    return Array.isArray(candidate) ? (candidate as MemoryRecommendation[]) : [];
  };

  const mergeFetchedRows = (incomingRows: MemoryRecommendation[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addMemoryRecommendation] = useAddMemoryRecommendationMutation();
  const [updateMemoryRecommendation] = useUpdateMemoryRecommendationMutation();
  const [deleteMemoryRecommendation] = useDeleteMemoryRecommendationMutation();
  const [deleteMemoryRecommendationCascade] = useDeleteMemoryRecommendationCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<MemoryRecommendation>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<MemoryRecommendation | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<MemoryRecommendation | null> => {
    ...
    try {
      ...
      return (await response.json()) as MemoryRecommendation;
    } catch (err) {
      console.warn('Could not reload MemoryRecommendation row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<MemoryRecommendation>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMemoryRecommendation(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as MemoryRecommendation[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update MemoryRecommendation';
      console.error('Failed to update MemoryRecommendation:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMemoryRecommendationCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteMemoryRecommendation(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<MemoryRecommendation> = { ...source };
      delete (payload as any).id;
      const created = await addMemoryRecommendation(payload).unwrap();
      setAllData((prev) => [created as MemoryRecommendation, ...prev]);
    } catch (error) {
      console.error('Failed to add MemoryRecommendation:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<MemoryRecommendation> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMemoryRecommendation(payload).unwrap();
      setAllData((prev) => [created as MemoryRecommendation, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) MemoryRecommendation:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `MemoryRecommendation-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMemoryRecommendation(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMemoryRecommendation(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as MemoryRecommendation[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as MemoryRecommendation[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as MemoryRecommendation[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as MemoryRecommendation[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="MemoryRecommendationTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="MemoryRecommendation Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create MemoryRecommendation</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <MemoryRecommendationForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter MemoryRecommendation (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing MemoryRecommendation</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof MemoryRecommendation;
                ...
                await updateMemoryRecommendation(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of MemoryRecommendations.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'MemoryRecommendation'}
...
export default MemoryRecommendationTable;
