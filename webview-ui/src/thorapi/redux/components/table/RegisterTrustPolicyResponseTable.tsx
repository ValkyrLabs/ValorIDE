...
import { RegisterTrustPolicyResponse } from '@thorapi/model';
import RegisterTrustPolicyResponseForm from '@thorapi/redux/components/form/RegisterTrustPolicyResponseForm';
...
import {
  useLazyGetRegisterTrustPolicyResponsesPagedQuery,
...
} from '@thorapi/redux/services/RegisterTrustPolicyResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RegisterTrustPolicyResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RegisterTrustPolicyResponse/${rowId}`;

const RegisterTrustPolicyResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RegisterTrustPolicyResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): RegisterTrustPolicyResponse[] => {
    if (Array.isArray(result)) return result as RegisterTrustPolicyResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as RegisterTrustPolicyResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RegisterTrustPolicyResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addRegisterTrustPolicyResponse] = useAddRegisterTrustPolicyResponseMutation();
  const [updateRegisterTrustPolicyResponse] = useUpdateRegisterTrustPolicyResponseMutation();
  const [deleteRegisterTrustPolicyResponse] = useDeleteRegisterTrustPolicyResponseMutation();
  const [deleteRegisterTrustPolicyResponseCascade] = useDeleteRegisterTrustPolicyResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<RegisterTrustPolicyResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RegisterTrustPolicyResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<RegisterTrustPolicyResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as RegisterTrustPolicyResponse;
    } catch (err) {
      console.warn('Could not reload RegisterTrustPolicyResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RegisterTrustPolicyResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRegisterTrustPolicyResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RegisterTrustPolicyResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RegisterTrustPolicyResponse';
      console.error('Failed to update RegisterTrustPolicyResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRegisterTrustPolicyResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteRegisterTrustPolicyResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RegisterTrustPolicyResponse> = { ...source };
      delete (payload as any).id;
      const created = await addRegisterTrustPolicyResponse(payload).unwrap();
      setAllData((prev) => [created as RegisterTrustPolicyResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add RegisterTrustPolicyResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RegisterTrustPolicyResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRegisterTrustPolicyResponse(payload).unwrap();
      setAllData((prev) => [created as RegisterTrustPolicyResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) RegisterTrustPolicyResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RegisterTrustPolicyResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRegisterTrustPolicyResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRegisterTrustPolicyResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RegisterTrustPolicyResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RegisterTrustPolicyResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RegisterTrustPolicyResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RegisterTrustPolicyResponse[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.RegisterTrustPolicyResponse',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="RegisterTrustPolicyResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="RegisterTrustPolicyResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RegisterTrustPolicyResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RegisterTrustPolicyResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RegisterTrustPolicyResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RegisterTrustPolicyResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RegisterTrustPolicyResponse;
                ...
                await updateRegisterTrustPolicyResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RegisterTrustPolicyResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RegisterTrustPolicyResponse'}
...
export default RegisterTrustPolicyResponseTable;
