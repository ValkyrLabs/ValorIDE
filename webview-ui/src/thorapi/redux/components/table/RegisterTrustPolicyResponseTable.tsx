...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/RegisterTrustPolicyResponse/${rowId}`;
...
const RegisterTrustPolicyResponseTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addRegisterTrustPolicyResponse] =
    useAddRegisterTrustPolicyResponseMutation();
  const [updateRegisterTrustPolicyResponse] =
    useUpdateRegisterTrustPolicyResponseMutation();
  const [deleteRegisterTrustPolicyResponse] =
    useDeleteRegisterTrustPolicyResponseMutation();
  const [deleteRegisterTrustPolicyResponseCascade] =
    useDeleteRegisterTrustPolicyResponseCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] =
    useState<RegisterTrustPolicyResponse | null>(null);
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<RegisterTrustPolicyResponse | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload RegisterTrustPolicyResponse row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateRegisterTrustPolicyResponse(
        updatedItem as any,
      ).unwrap();
      ...
      setAllData(
        (prev) =>
          mergeUpdatedRow(
            ...
            rowId,
            hydrated as any,
          ) as RegisterTrustPolicyResponse[],
      );
      if (successField) {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update RegisterTrustPolicyResponse";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRegisterTrustPolicyResponseCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) RegisterTrustPolicyResponse:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter RegisterTrustPolicyResponse (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
