...
import ExecModuleAudienceConfigForm from '@thorapi/redux/components/form/ExecModuleAudienceConfigForm';
...
import {
  useLazyGetExecModuleAudienceConfigsPagedQuery,
...
} from '@thorapi/redux/services/ExecModuleAudienceConfigService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ExecModuleAudienceConfig update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ExecModuleAudienceConfig/${rowId}`;

const ExecModuleAudienceConfigTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ExecModuleAudienceConfig[]>([]);
  ...
  const normalizePageRows = (result: unknown): ExecModuleAudienceConfig[] => {
    if (Array.isArray(result)) return result as ExecModuleAudienceConfig[];
    ...
    return Array.isArray(candidate) ? (candidate as ExecModuleAudienceConfig[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ExecModuleAudienceConfig[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addExecModuleAudienceConfig] = useAddExecModuleAudienceConfigMutation();
  const [updateExecModuleAudienceConfig] = useUpdateExecModuleAudienceConfigMutation();
  const [deleteExecModuleAudienceConfig] = useDeleteExecModuleAudienceConfigMutation();
  const [deleteExecModuleAudienceConfigCascade] = useDeleteExecModuleAudienceConfigCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ExecModuleAudienceConfig>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ExecModuleAudienceConfig | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ExecModuleAudienceConfig | null> => {
    ...
    try {
      ...
      return (await response.json()) as ExecModuleAudienceConfig;
    } catch (err) {
      console.warn('Could not reload ExecModuleAudienceConfig row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ExecModuleAudienceConfig>,
    successField?: string,
  ) => {
    try {
      const saved = await updateExecModuleAudienceConfig(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ExecModuleAudienceConfig[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ExecModuleAudienceConfig';
      console.error('Failed to update ExecModuleAudienceConfig:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecModuleAudienceConfigCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteExecModuleAudienceConfig(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ExecModuleAudienceConfig> = { ...source };
      delete (payload as any).id;
      const created = await addExecModuleAudienceConfig(payload).unwrap();
      setAllData((prev) => [created as ExecModuleAudienceConfig, ...prev]);
    } catch (error) {
      console.error('Failed to add ExecModuleAudienceConfig:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ExecModuleAudienceConfig> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addExecModuleAudienceConfig(payload).unwrap();
      setAllData((prev) => [created as ExecModuleAudienceConfig, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ExecModuleAudienceConfig:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ExecModuleAudienceConfig-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addExecModuleAudienceConfig(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateExecModuleAudienceConfig(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ExecModuleAudienceConfig[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ExecModuleAudienceConfig[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ExecModuleAudienceConfig[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ExecModuleAudienceConfig[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ExecModuleAudienceConfigTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ExecModuleAudienceConfig Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ExecModuleAudienceConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ExecModuleAudienceConfigForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ExecModuleAudienceConfig (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ExecModuleAudienceConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ExecModuleAudienceConfig;
                ...
                await updateExecModuleAudienceConfig(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ExecModuleAudienceConfigs.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ExecModuleAudienceConfig'}
...
export default ExecModuleAudienceConfigTable;
