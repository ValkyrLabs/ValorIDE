...
import { Workflow } from '@thorapi/model';
import WorkflowForm from '@thorapi/redux/components/form/WorkflowForm';
...
} from '@thorapi/redux/services/WorkflowService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Workflow update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Workflow/${rowId}`;
...
const WorkflowTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Workflow[] => {
    if (Array.isArray(result)) return result as Workflow[];
    ...
    return Array.isArray(candidate) ? (candidate as Workflow[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Workflow[], requestedPage: number) => {
  ...
  const [deleteWorkflowCascade] = useDeleteWorkflowCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Workflow | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Workflow row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Workflow>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWorkflow(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Workflow[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Workflow';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Workflow-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWorkflow(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWorkflow(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Workflow[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Workflow[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Workflow[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Workflow[]);
    }
  };
