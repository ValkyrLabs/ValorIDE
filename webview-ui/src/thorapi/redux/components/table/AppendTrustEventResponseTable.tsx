...
import AppendTrustEventResponseForm from '@thorapi/redux/components/form/AppendTrustEventResponseForm';
...
import {
  useLazyGetAppendTrustEventResponsesPagedQuery,
...
} from '@thorapi/redux/services/AppendTrustEventResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AppendTrustEventResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AppendTrustEventResponse/${rowId}`;

const AppendTrustEventResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<AppendTrustEventResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): AppendTrustEventResponse[] => {
    if (Array.isArray(result)) return result as AppendTrustEventResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as AppendTrustEventResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: AppendTrustEventResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addAppendTrustEventResponse] = useAddAppendTrustEventResponseMutation();
  const [updateAppendTrustEventResponse] = useUpdateAppendTrustEventResponseMutation();
  const [deleteAppendTrustEventResponse] = useDeleteAppendTrustEventResponseMutation();
  const [deleteAppendTrustEventResponseCascade] = useDeleteAppendTrustEventResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<AppendTrustEventResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<AppendTrustEventResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<AppendTrustEventResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as AppendTrustEventResponse;
    } catch (err) {
      console.warn('Could not reload AppendTrustEventResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AppendTrustEventResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAppendTrustEventResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AppendTrustEventResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AppendTrustEventResponse';
      console.error('Failed to update AppendTrustEventResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAppendTrustEventResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteAppendTrustEventResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<AppendTrustEventResponse> = { ...source };
      delete (payload as any).id;
      const created = await addAppendTrustEventResponse(payload).unwrap();
      setAllData((prev) => [created as AppendTrustEventResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add AppendTrustEventResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<AppendTrustEventResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addAppendTrustEventResponse(payload).unwrap();
      setAllData((prev) => [created as AppendTrustEventResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) AppendTrustEventResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AppendTrustEventResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAppendTrustEventResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAppendTrustEventResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AppendTrustEventResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AppendTrustEventResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AppendTrustEventResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AppendTrustEventResponse[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="AppendTrustEventResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="AppendTrustEventResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create AppendTrustEventResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <AppendTrustEventResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter AppendTrustEventResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing AppendTrustEventResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof AppendTrustEventResponse;
                ...
                await updateAppendTrustEventResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of AppendTrustEventResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'AppendTrustEventResponse'}
...
export default AppendTrustEventResponseTable;
