...
import McpMarketplaceItemForm from '@thorapi/redux/components/form/McpMarketplaceItemForm';
...
} from '@thorapi/redux/services/McpMarketplaceItemService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `McpMarketplaceItem update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/McpMarketplaceItem/${rowId}`;
...
const McpMarketplaceItemTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): McpMarketplaceItem[] => {
    if (Array.isArray(result)) return result as McpMarketplaceItem[];
    ...
    return Array.isArray(candidate) ? (candidate as McpMarketplaceItem[]) : [];
  };

  const mergeFetchedRows = (incomingRows: McpMarketplaceItem[], requestedPage: number) => {
  ...
  const [deleteMcpMarketplaceItemCascade] = useDeleteMcpMarketplaceItemCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<McpMarketplaceItem | null> => {
    ...
    try {
      ...
      return (await response.json()) as McpMarketplaceItem;
    } catch (err) {
      console.warn('Could not reload McpMarketplaceItem row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<McpMarketplaceItem>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMcpMarketplaceItem(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as McpMarketplaceItem[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update McpMarketplaceItem';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpMarketplaceItemCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `McpMarketplaceItem-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMcpMarketplaceItem(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMcpMarketplaceItem(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as McpMarketplaceItem[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as McpMarketplaceItem[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as McpMarketplaceItem[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as McpMarketplaceItem[]);
    }
  };
