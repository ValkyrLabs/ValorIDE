...
import { BalanceResponse } from '@thorapi/model';
import BalanceResponseForm from '@thorapi/redux/components/form/BalanceResponseForm';
...
import {
  ...
  useDeleteBalanceResponseCascadeMutation
} from '@thorapi/redux/services/BalanceResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `BalanceResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/BalanceResponse/${rowId}`;
...
const BalanceResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): BalanceResponse[] => {
    if (Array.isArray(result)) return result as BalanceResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as BalanceResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: BalanceResponse[], requestedPage: number) => {
  ...
  const [deleteBalanceResponseCascade] = useDeleteBalanceResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<BalanceResponse | null> => {
    ...
    try {
      ...
      console.warn('Could not reload BalanceResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<BalanceResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateBalanceResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as BalanceResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update BalanceResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteBalanceResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `BalanceResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addBalanceResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateBalanceResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as BalanceResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as BalanceResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as BalanceResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as BalanceResponse[]);
    }
  };
