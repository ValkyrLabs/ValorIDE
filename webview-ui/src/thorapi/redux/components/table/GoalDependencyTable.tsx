...
import { GoalDependency } from '@thorapi/model';
import GoalDependencyForm from '@thorapi/redux/components/form/GoalDependencyForm';
...
import {
  ...
  useDeleteGoalDependencyCascadeMutation
} from '@thorapi/redux/services/GoalDependencyService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `GoalDependency update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/GoalDependency/${rowId}`;
...
const GoalDependencyTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): GoalDependency[] => {
    if (Array.isArray(result)) return result as GoalDependency[];
    ...
    return Array.isArray(candidate) ? (candidate as GoalDependency[]) : [];
  };

  const mergeFetchedRows = (incomingRows: GoalDependency[], requestedPage: number) => {
  ...
  const [deleteGoalDependencyCascade] = useDeleteGoalDependencyCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<GoalDependency | null> => {
    ...
    try {
      ...
      return (await response.json()) as GoalDependency;
    } catch (err) {
      console.warn('Could not reload GoalDependency row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<GoalDependency>,
    successField?: string,
  ) => {
    try {
      const saved = await updateGoalDependency(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as GoalDependency[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update GoalDependency';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteGoalDependencyCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `GoalDependency-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addGoalDependency(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateGoalDependency(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as GoalDependency[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as GoalDependency[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as GoalDependency[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as GoalDependency[]);
    }
  };
