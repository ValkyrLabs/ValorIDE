...
import { LlmCommand } from '@thorapi/model';
import LlmCommandForm from '@thorapi/redux/components/form/LlmCommandForm';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `LlmCommand update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/LlmCommand/${rowId}`;

const LlmCommandTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<LlmCommand[]>([]);
  ...
  const normalizePageRows = (result: unknown): LlmCommand[] => {
    if (Array.isArray(result)) return result as LlmCommand[];
    ...
    return Array.isArray(candidate) ? (candidate as LlmCommand[]) : [];
  };

  const mergeFetchedRows = (incomingRows: LlmCommand[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addLlmCommand] = useAddLlmCommandMutation();
  const [updateLlmCommand] = useUpdateLlmCommandMutation();
  const [deleteLlmCommand] = useDeleteLlmCommandMutation();
  const [deleteLlmCommandCascade] = useDeleteLlmCommandCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<LlmCommand>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<LlmCommand | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<LlmCommand | null> => {
    ...
    try {
      ...
      return (await response.json()) as LlmCommand;
    } catch (err) {
      console.warn('Could not reload LlmCommand row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<LlmCommand>,
    successField?: string,
  ) => {
    try {
      const saved = await updateLlmCommand(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as LlmCommand[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update LlmCommand';
      console.error('Failed to update LlmCommand:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteLlmCommandCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteLlmCommand(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<LlmCommand> = { ...source };
      delete (payload as any).id;
      const created = await addLlmCommand(payload).unwrap();
      setAllData((prev) => [created as LlmCommand, ...prev]);
    } catch (error) {
      console.error('Failed to add LlmCommand:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<LlmCommand> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addLlmCommand(payload).unwrap();
      setAllData((prev) => [created as LlmCommand, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) LlmCommand:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `LlmCommand-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addLlmCommand(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateLlmCommand(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as LlmCommand[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as LlmCommand[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as LlmCommand[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as LlmCommand[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.LlmCommand',
        objectId: rowId,
      });
    ...
    <>
          ...
          >
            ...
            <FloatingControlPanel
              description="LlmCommand Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create LlmCommand</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <LlmCommandForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter LlmCommand (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing LlmCommand</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof LlmCommand;
                ...
                await updateLlmCommand(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of LlmCommands.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'LlmCommand'}
...
export default LlmCommandTable;
