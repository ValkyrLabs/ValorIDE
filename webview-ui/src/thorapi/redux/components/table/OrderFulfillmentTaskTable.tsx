...
import { OrderFulfillmentTask } from '@thorapi/model';
import OrderFulfillmentTaskForm from '@thorapi/redux/components/form/OrderFulfillmentTaskForm';
...
} from '@thorapi/redux/services/OrderFulfillmentTaskService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `OrderFulfillmentTask update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/OrderFulfillmentTask/${rowId}`;
...
const OrderFulfillmentTaskTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): OrderFulfillmentTask[] => {
    if (Array.isArray(result)) return result as OrderFulfillmentTask[];
    ...
    return Array.isArray(candidate) ? (candidate as OrderFulfillmentTask[]) : [];
  };

  const mergeFetchedRows = (incomingRows: OrderFulfillmentTask[], requestedPage: number) => {
  ...
  const [deleteOrderFulfillmentTaskCascade] = useDeleteOrderFulfillmentTaskCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<OrderFulfillmentTask | null> => {
    ...
    try {
      ...
      return (await response.json()) as OrderFulfillmentTask;
    } catch (err) {
      console.warn('Could not reload OrderFulfillmentTask row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<OrderFulfillmentTask>,
    successField?: string,
  ) => {
    try {
      const saved = await updateOrderFulfillmentTask(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as OrderFulfillmentTask[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update OrderFulfillmentTask';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOrderFulfillmentTaskCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `OrderFulfillmentTask-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addOrderFulfillmentTask(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateOrderFulfillmentTask(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as OrderFulfillmentTask[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as OrderFulfillmentTask[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as OrderFulfillmentTask[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as OrderFulfillmentTask[]);
    }
  };
