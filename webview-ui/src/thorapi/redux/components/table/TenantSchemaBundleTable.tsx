...
import { TenantSchemaBundle } from '@thorapi/model';
import TenantSchemaBundleForm from '@thorapi/redux/components/form/TenantSchemaBundleForm';
...
import {
  useLazyGetTenantSchemaBundlesPagedQuery,
...
} from '@thorapi/redux/services/TenantSchemaBundleService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'status': { type: 'enum', enumValues: [
    'generated', 'applying', 'active', 'superseded', 'failed', 'retired', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TenantSchemaBundle update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TenantSchemaBundle/${rowId}`;

const TenantSchemaBundleTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TenantSchemaBundle[]>([]);
  ...
  const normalizePageRows = (result: unknown): TenantSchemaBundle[] => {
    if (Array.isArray(result)) return result as TenantSchemaBundle[];
    ...
    return Array.isArray(candidate) ? (candidate as TenantSchemaBundle[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TenantSchemaBundle[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addTenantSchemaBundle] = useAddTenantSchemaBundleMutation();
  const [updateTenantSchemaBundle] = useUpdateTenantSchemaBundleMutation();
  const [deleteTenantSchemaBundle] = useDeleteTenantSchemaBundleMutation();
  const [deleteTenantSchemaBundleCascade] = useDeleteTenantSchemaBundleCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TenantSchemaBundle>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TenantSchemaBundle | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TenantSchemaBundle | null> => {
    ...
    try {
      ...
      return (await response.json()) as TenantSchemaBundle;
    } catch (err) {
      console.warn('Could not reload TenantSchemaBundle row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TenantSchemaBundle>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTenantSchemaBundle(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TenantSchemaBundle[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TenantSchemaBundle';
      console.error('Failed to update TenantSchemaBundle:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTenantSchemaBundleCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTenantSchemaBundle(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TenantSchemaBundle> = { ...source };
      delete (payload as any).id;
      const created = await addTenantSchemaBundle(payload).unwrap();
      setAllData((prev) => [created as TenantSchemaBundle, ...prev]);
    } catch (error) {
      console.error('Failed to add TenantSchemaBundle:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TenantSchemaBundle> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTenantSchemaBundle(payload).unwrap();
      setAllData((prev) => [created as TenantSchemaBundle, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TenantSchemaBundle:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TenantSchemaBundle-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTenantSchemaBundle(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTenantSchemaBundle(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TenantSchemaBundle[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TenantSchemaBundle[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TenantSchemaBundle[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TenantSchemaBundle[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="TenantSchemaBundleTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="TenantSchemaBundle Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TenantSchemaBundle</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TenantSchemaBundleForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TenantSchemaBundle (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TenantSchemaBundle</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TenantSchemaBundle;
                ...
                await updateTenantSchemaBundle(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TenantSchemaBundles.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TenantSchemaBundle'}
...
export default TenantSchemaBundleTable;
