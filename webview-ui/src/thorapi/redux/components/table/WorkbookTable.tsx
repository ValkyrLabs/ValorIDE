...
import { Workbook } from '@thorapi/model';
import WorkbookForm from '@thorapi/redux/components/form/WorkbookForm';
...
} from '@thorapi/redux/services/WorkbookService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Workbook update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Workbook/${rowId}`;
...
const WorkbookTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Workbook[] => {
    if (Array.isArray(result)) return result as Workbook[];
    ...
    return Array.isArray(candidate) ? (candidate as Workbook[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Workbook[], requestedPage: number) => {
  ...
  const reloadRowById = async (rowId: string): Promise<Workbook | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Workbook row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Workbook>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWorkbook(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Workbook[]);
      if (successField) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkbookCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Workbook-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWorkbook(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWorkbook(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Workbook[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Workbook[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Workbook[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Workbook[]);
    }
  };
