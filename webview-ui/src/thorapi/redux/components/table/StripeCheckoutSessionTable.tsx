...
import { StripeCheckoutSession } from '@thorapi/model';
import StripeCheckoutSessionForm from '@thorapi/redux/components/form/StripeCheckoutSessionForm';
...
import {
  useLazyGetStripeCheckoutSessionsPagedQuery,
...
} from '@thorapi/redux/services/StripeCheckoutSessionService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'status': { type: 'enum', enumValues: [
    'pending', 'paid', 'duplicate', 'failed', 'refunded', 'ignored', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `StripeCheckoutSession update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/StripeCheckoutSession/${rowId}`;

const StripeCheckoutSessionTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<StripeCheckoutSession[]>([]);
  ...
  const normalizePageRows = (result: unknown): StripeCheckoutSession[] => {
    if (Array.isArray(result)) return result as StripeCheckoutSession[];
    ...
    return Array.isArray(candidate) ? (candidate as StripeCheckoutSession[]) : [];
  };

  const mergeFetchedRows = (incomingRows: StripeCheckoutSession[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addStripeCheckoutSession] = useAddStripeCheckoutSessionMutation();
  const [updateStripeCheckoutSession] = useUpdateStripeCheckoutSessionMutation();
  const [deleteStripeCheckoutSession] = useDeleteStripeCheckoutSessionMutation();
  const [deleteStripeCheckoutSessionCascade] = useDeleteStripeCheckoutSessionCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<StripeCheckoutSession>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<StripeCheckoutSession | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<StripeCheckoutSession | null> => {
    ...
    try {
      ...
      console.warn('Could not reload StripeCheckoutSession row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<StripeCheckoutSession>,
    successField?: string,
  ) => {
    try {
      const saved = await updateStripeCheckoutSession(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as StripeCheckoutSession[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update StripeCheckoutSession';
      console.error('Failed to update StripeCheckoutSession:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteStripeCheckoutSessionCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteStripeCheckoutSession(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<StripeCheckoutSession> = { ...source };
      delete (payload as any).id;
      const created = await addStripeCheckoutSession(payload).unwrap();
      setAllData((prev) => [created as StripeCheckoutSession, ...prev]);
    } catch (error) {
      console.error('Failed to add StripeCheckoutSession:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<StripeCheckoutSession> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addStripeCheckoutSession(payload).unwrap();
      setAllData((prev) => [created as StripeCheckoutSession, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) StripeCheckoutSession:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `StripeCheckoutSession-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addStripeCheckoutSession(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateStripeCheckoutSession(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as StripeCheckoutSession[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as StripeCheckoutSession[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as StripeCheckoutSession[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as StripeCheckoutSession[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="StripeCheckoutSessionTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="StripeCheckoutSession Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create StripeCheckoutSession</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter StripeCheckoutSession (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing StripeCheckoutSession</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof StripeCheckoutSession;
                ...
                await updateStripeCheckoutSession(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of StripeCheckoutSessions.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'StripeCheckoutSession'}
...
export default StripeCheckoutSessionTable;
