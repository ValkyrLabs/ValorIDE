...
import { PhoneVerificationRequest } from '@thorapi/model';
import PhoneVerificationRequestForm from '@thorapi/redux/components/form/PhoneVerificationRequestForm';
...
import {
  useLazyGetPhoneVerificationRequestsPagedQuery,
...
} from '@thorapi/redux/services/PhoneVerificationRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `PhoneVerificationRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/PhoneVerificationRequest/${rowId}`;

const PhoneVerificationRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<PhoneVerificationRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): PhoneVerificationRequest[] => {
    if (Array.isArray(result)) return result as PhoneVerificationRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as PhoneVerificationRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: PhoneVerificationRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addPhoneVerificationRequest] = useAddPhoneVerificationRequestMutation();
  const [updatePhoneVerificationRequest] = useUpdatePhoneVerificationRequestMutation();
  const [deletePhoneVerificationRequest] = useDeletePhoneVerificationRequestMutation();
  const [deletePhoneVerificationRequestCascade] = useDeletePhoneVerificationRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<PhoneVerificationRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<PhoneVerificationRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<PhoneVerificationRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as PhoneVerificationRequest;
    } catch (err) {
      console.warn('Could not reload PhoneVerificationRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<PhoneVerificationRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updatePhoneVerificationRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as PhoneVerificationRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update PhoneVerificationRequest';
      console.error('Failed to update PhoneVerificationRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePhoneVerificationRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deletePhoneVerificationRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<PhoneVerificationRequest> = { ...source };
      delete (payload as any).id;
      const created = await addPhoneVerificationRequest(payload).unwrap();
      setAllData((prev) => [created as PhoneVerificationRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add PhoneVerificationRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<PhoneVerificationRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addPhoneVerificationRequest(payload).unwrap();
      setAllData((prev) => [created as PhoneVerificationRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) PhoneVerificationRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `PhoneVerificationRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addPhoneVerificationRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updatePhoneVerificationRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as PhoneVerificationRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as PhoneVerificationRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as PhoneVerificationRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as PhoneVerificationRequest[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="PhoneVerificationRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="PhoneVerificationRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create PhoneVerificationRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter PhoneVerificationRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing PhoneVerificationRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof PhoneVerificationRequest;
                ...
                await updatePhoneVerificationRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of PhoneVerificationRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'PhoneVerificationRequest'}
