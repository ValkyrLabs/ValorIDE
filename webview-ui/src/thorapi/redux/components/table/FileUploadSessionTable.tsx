...
import FileUploadSessionForm from '@thorapi/redux/components/form/FileUploadSessionForm';
...
} from '@thorapi/redux/services/FileUploadSessionService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `FileUploadSession update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/FileUploadSession/${rowId}`;
...
const FileUploadSessionTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): FileUploadSession[] => {
    if (Array.isArray(result)) return result as FileUploadSession[];
    ...
    return Array.isArray(candidate) ? (candidate as FileUploadSession[]) : [];
  };

  const mergeFetchedRows = (incomingRows: FileUploadSession[], requestedPage: number) => {
  ...
  const [deleteFileUploadSessionCascade] = useDeleteFileUploadSessionCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<FileUploadSession | null> => {
    ...
    try {
      ...
      return (await response.json()) as FileUploadSession;
    } catch (err) {
      console.warn('Could not reload FileUploadSession row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<FileUploadSession>,
    successField?: string,
  ) => {
    try {
      const saved = await updateFileUploadSession(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as FileUploadSession[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update FileUploadSession';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteFileUploadSessionCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `FileUploadSession-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addFileUploadSession(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateFileUploadSession(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as FileUploadSession[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as FileUploadSession[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as FileUploadSession[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as FileUploadSession[]);
    }
  };
