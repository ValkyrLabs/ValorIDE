...
import { RetrievalReceiptResponse } from '@thorapi/model';
import RetrievalReceiptResponseForm from '@thorapi/redux/components/form/RetrievalReceiptResponseForm';
...
import {
  useLazyGetRetrievalReceiptResponsesPagedQuery,
...
} from '@thorapi/redux/services/RetrievalReceiptResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RetrievalReceiptResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RetrievalReceiptResponse/${rowId}`;

const RetrievalReceiptResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RetrievalReceiptResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): RetrievalReceiptResponse[] => {
    if (Array.isArray(result)) return result as RetrievalReceiptResponse[];
  ...
  const mergeFetchedRows = (incomingRows: RetrievalReceiptResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addRetrievalReceiptResponse] = useAddRetrievalReceiptResponseMutation();
  const [updateRetrievalReceiptResponse] = useUpdateRetrievalReceiptResponseMutation();
  const [deleteRetrievalReceiptResponse] = useDeleteRetrievalReceiptResponseMutation();
  const [deleteRetrievalReceiptResponseCascade] = useDeleteRetrievalReceiptResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<RetrievalReceiptResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RetrievalReceiptResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<RetrievalReceiptResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as RetrievalReceiptResponse;
    } catch (err) {
      console.warn('Could not reload RetrievalReceiptResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RetrievalReceiptResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRetrievalReceiptResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RetrievalReceiptResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RetrievalReceiptResponse';
      console.error('Failed to update RetrievalReceiptResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRetrievalReceiptResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteRetrievalReceiptResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RetrievalReceiptResponse> = { ...source };
      delete (payload as any).id;
      const created = await addRetrievalReceiptResponse(payload).unwrap();
      setAllData((prev) => [created as RetrievalReceiptResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add RetrievalReceiptResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RetrievalReceiptResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRetrievalReceiptResponse(payload).unwrap();
      setAllData((prev) => [created as RetrievalReceiptResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) RetrievalReceiptResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RetrievalReceiptResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRetrievalReceiptResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRetrievalReceiptResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RetrievalReceiptResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RetrievalReceiptResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RetrievalReceiptResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RetrievalReceiptResponse[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="RetrievalReceiptResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="RetrievalReceiptResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RetrievalReceiptResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RetrievalReceiptResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RetrievalReceiptResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RetrievalReceiptResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RetrievalReceiptResponse;
                ...
                await updateRetrievalReceiptResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RetrievalReceiptResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RetrievalReceiptResponse'}
...
export default RetrievalReceiptResponseTable;
