...
import { GenerateTrustProofResponse } from '@thorapi/model';
import GenerateTrustProofResponseForm from '@thorapi/redux/components/form/GenerateTrustProofResponseForm';
...
import {
  useLazyGetGenerateTrustProofResponsesPagedQuery,
...
} from '@thorapi/redux/services/GenerateTrustProofResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `GenerateTrustProofResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/GenerateTrustProofResponse/${rowId}`;

const GenerateTrustProofResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<GenerateTrustProofResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): GenerateTrustProofResponse[] => {
    if (Array.isArray(result)) return result as GenerateTrustProofResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as GenerateTrustProofResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: GenerateTrustProofResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addGenerateTrustProofResponse] = useAddGenerateTrustProofResponseMutation();
  const [updateGenerateTrustProofResponse] = useUpdateGenerateTrustProofResponseMutation();
  const [deleteGenerateTrustProofResponse] = useDeleteGenerateTrustProofResponseMutation();
  const [deleteGenerateTrustProofResponseCascade] = useDeleteGenerateTrustProofResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<GenerateTrustProofResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<GenerateTrustProofResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<GenerateTrustProofResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as GenerateTrustProofResponse;
    } catch (err) {
      console.warn('Could not reload GenerateTrustProofResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<GenerateTrustProofResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateGenerateTrustProofResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as GenerateTrustProofResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update GenerateTrustProofResponse';
      console.error('Failed to update GenerateTrustProofResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteGenerateTrustProofResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteGenerateTrustProofResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<GenerateTrustProofResponse> = { ...source };
      delete (payload as any).id;
      const created = await addGenerateTrustProofResponse(payload).unwrap();
      setAllData((prev) => [created as GenerateTrustProofResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add GenerateTrustProofResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<GenerateTrustProofResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addGenerateTrustProofResponse(payload).unwrap();
      setAllData((prev) => [created as GenerateTrustProofResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) GenerateTrustProofResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `GenerateTrustProofResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addGenerateTrustProofResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateGenerateTrustProofResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as GenerateTrustProofResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as GenerateTrustProofResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as GenerateTrustProofResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as GenerateTrustProofResponse[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.GenerateTrustProofResponse',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="GenerateTrustProofResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="GenerateTrustProofResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create GenerateTrustProofResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <GenerateTrustProofResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter GenerateTrustProofResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing GenerateTrustProofResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof GenerateTrustProofResponse;
                ...
                await updateGenerateTrustProofResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of GenerateTrustProofResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'GenerateTrustProofResponse'}
...
export default GenerateTrustProofResponseTable;
