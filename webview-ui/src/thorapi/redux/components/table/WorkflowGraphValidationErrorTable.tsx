...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/WorkflowGraphValidationError/${rowId}`;
...
const WorkflowGraphValidationErrorTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addWorkflowGraphValidationError] =
    useAddWorkflowGraphValidationErrorMutation();
  const [updateWorkflowGraphValidationError] =
    useUpdateWorkflowGraphValidationErrorMutation();
  const [deleteWorkflowGraphValidationError] =
    useDeleteWorkflowGraphValidationErrorMutation();
  const [deleteWorkflowGraphValidationErrorCascade] =
    useDeleteWorkflowGraphValidationErrorCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] =
    useState<WorkflowGraphValidationError | null>(null);
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<WorkflowGraphValidationError | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload WorkflowGraphValidationError row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateWorkflowGraphValidationError(
        updatedItem as any,
      ).unwrap();
      ...
      setAllData(
        (prev) =>
          mergeUpdatedRow(
            ...
            rowId,
            hydrated as any,
          ) as WorkflowGraphValidationError[],
      );
      if (successField) {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update WorkflowGraphValidationError";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphValidationErrorCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) WorkflowGraphValidationError:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter WorkflowGraphValidationError (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
