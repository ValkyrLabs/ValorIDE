...
import { Tag } from '@thorapi/model';
import TagForm from '@thorapi/redux/components/form/TagForm';
...
} from '@thorapi/redux/services/TagService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'type': { type: 'enum', enumValues: [
    'category', 'keyword', 'topic', 'genre', 'audience', 'score_band', 'other', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Tag update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Tag/${rowId}`;
...
const TagTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Tag[] => {
    ...
    return Array.isArray(candidate) ? (candidate as Tag[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Tag[], requestedPage: number) => {
  ...
  const [deleteTagCascade] = useDeleteTagCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Tag | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Tag row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Tag>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTag(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Tag[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Tag';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTagCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Tag-export-${stamp}.json`;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Tag[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Tag[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Tag[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Tag[]);
    }
  };
