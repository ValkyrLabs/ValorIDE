...
import { SwarmGraphNode } from '@thorapi/model';
import SwarmGraphNodeForm from '@thorapi/redux/components/form/SwarmGraphNodeForm';
...
} from '@thorapi/redux/services/SwarmGraphNodeService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SwarmGraphNode update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SwarmGraphNode/${rowId}`;
...
const SwarmGraphNodeTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SwarmGraphNode[] => {
    if (Array.isArray(result)) return result as SwarmGraphNode[];
    ...
    return Array.isArray(candidate) ? (candidate as SwarmGraphNode[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SwarmGraphNode[], requestedPage: number) => {
  ...
  const [deleteSwarmGraphNodeCascade] = useDeleteSwarmGraphNodeCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SwarmGraphNode | null> => {
    ...
    try {
      ...
      return (await response.json()) as SwarmGraphNode;
    } catch (err) {
      console.warn('Could not reload SwarmGraphNode row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SwarmGraphNode>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSwarmGraphNode(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SwarmGraphNode[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SwarmGraphNode';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmGraphNodeCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SwarmGraphNode-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSwarmGraphNode(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSwarmGraphNode(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SwarmGraphNode[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SwarmGraphNode[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SwarmGraphNode[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SwarmGraphNode[]);
    }
  };
