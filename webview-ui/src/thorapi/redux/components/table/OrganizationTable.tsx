...
import { Organization } from '@thorapi/model';
import OrganizationForm from '@thorapi/redux/components/form/OrganizationForm';
...
} from '@thorapi/redux/services/OrganizationService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Organization update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Organization/${rowId}`;
...
const OrganizationTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Organization[] => {
    ...
    return Array.isArray(candidate) ? (candidate as Organization[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Organization[], requestedPage: number) => {
  ...
  const [deleteOrganizationCascade] = useDeleteOrganizationCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Organization | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Organization row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Organization>,
    successField?: string,
  ) => {
    try {
      const saved = await updateOrganization(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Organization[]);
      if (successField) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOrganizationCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Organization-export-${stamp}.json`;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Organization[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Organization[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Organization[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Organization[]);
    }
  };
