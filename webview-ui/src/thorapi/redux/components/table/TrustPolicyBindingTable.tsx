...
import { TrustPolicyBinding } from '@thorapi/model';
import TrustPolicyBindingForm from '@thorapi/redux/components/form/TrustPolicyBindingForm';
...
import {
  useLazyGetTrustPolicyBindingsPagedQuery,
...
} from '@thorapi/redux/services/TrustPolicyBindingService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'bindingStatus': { type: 'enum', enumValues: [
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TrustPolicyBinding update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TrustPolicyBinding/${rowId}`;

const TrustPolicyBindingTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TrustPolicyBinding[]>([]);
  ...
  const normalizePageRows = (result: unknown): TrustPolicyBinding[] => {
    if (Array.isArray(result)) return result as TrustPolicyBinding[];
    ...
    return Array.isArray(candidate) ? (candidate as TrustPolicyBinding[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TrustPolicyBinding[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addTrustPolicyBinding] = useAddTrustPolicyBindingMutation();
  const [updateTrustPolicyBinding] = useUpdateTrustPolicyBindingMutation();
  const [deleteTrustPolicyBinding] = useDeleteTrustPolicyBindingMutation();
  const [deleteTrustPolicyBindingCascade] = useDeleteTrustPolicyBindingCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TrustPolicyBinding>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TrustPolicyBinding | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TrustPolicyBinding | null> => {
    ...
    try {
      ...
      return (await response.json()) as TrustPolicyBinding;
    } catch (err) {
      console.warn('Could not reload TrustPolicyBinding row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TrustPolicyBinding>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTrustPolicyBinding(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TrustPolicyBinding[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TrustPolicyBinding';
      console.error('Failed to update TrustPolicyBinding:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustPolicyBindingCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTrustPolicyBinding(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TrustPolicyBinding> = { ...source };
      delete (payload as any).id;
      const created = await addTrustPolicyBinding(payload).unwrap();
      setAllData((prev) => [created as TrustPolicyBinding, ...prev]);
    } catch (error) {
      console.error('Failed to add TrustPolicyBinding:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TrustPolicyBinding> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTrustPolicyBinding(payload).unwrap();
      setAllData((prev) => [created as TrustPolicyBinding, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TrustPolicyBinding:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TrustPolicyBinding-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTrustPolicyBinding(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTrustPolicyBinding(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TrustPolicyBinding[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TrustPolicyBinding[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TrustPolicyBinding[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TrustPolicyBinding[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.TrustPolicyBinding',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="TrustPolicyBindingTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="TrustPolicyBinding Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TrustPolicyBinding</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TrustPolicyBindingForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TrustPolicyBinding (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TrustPolicyBinding</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TrustPolicyBinding;
                ...
                await updateTrustPolicyBinding(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TrustPolicyBindings.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TrustPolicyBinding'}
...
export default TrustPolicyBindingTable;
