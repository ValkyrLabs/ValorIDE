...
import { SkillOptRouteOutcomeRequest } from '@thorapi/model';
import SkillOptRouteOutcomeRequestForm from '@thorapi/redux/components/form/SkillOptRouteOutcomeRequestForm';
...
import {
  useLazyGetSkillOptRouteOutcomeRequestsPagedQuery,
  useAddSkillOptRouteOutcomeRequestMutation,
  useUpdateSkillOptRouteOutcomeRequestMutation,
  useDeleteSkillOptRouteOutcomeRequestMutation,
  useDeleteSkillOptRouteOutcomeRequestCascadeMutation
} from '@thorapi/redux/services/SkillOptRouteOutcomeRequestService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'outcome': { type: 'enum', enumValues: [
    'success', 'failure', 'partial', 'canceled', 
  ], enumValueType: 'string' },
  'ratingScore': { type: 'number' },
  'actualCreditCost': { type: 'number' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SkillOptRouteOutcomeRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SkillOptRouteOutcomeRequest/${rowId}`;

const SkillOptRouteOutcomeRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SkillOptRouteOutcomeRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): SkillOptRouteOutcomeRequest[] => {
    if (Array.isArray(result)) return result as SkillOptRouteOutcomeRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as SkillOptRouteOutcomeRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SkillOptRouteOutcomeRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addSkillOptRouteOutcomeRequest] = useAddSkillOptRouteOutcomeRequestMutation();
  const [updateSkillOptRouteOutcomeRequest] = useUpdateSkillOptRouteOutcomeRequestMutation();
  const [deleteSkillOptRouteOutcomeRequest] = useDeleteSkillOptRouteOutcomeRequestMutation();
  const [deleteSkillOptRouteOutcomeRequestCascade] = useDeleteSkillOptRouteOutcomeRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<SkillOptRouteOutcomeRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SkillOptRouteOutcomeRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<SkillOptRouteOutcomeRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as SkillOptRouteOutcomeRequest;
    } catch (err) {
      console.warn('Could not reload SkillOptRouteOutcomeRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SkillOptRouteOutcomeRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSkillOptRouteOutcomeRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SkillOptRouteOutcomeRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SkillOptRouteOutcomeRequest';
      console.error('Failed to update SkillOptRouteOutcomeRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSkillOptRouteOutcomeRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteSkillOptRouteOutcomeRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SkillOptRouteOutcomeRequest> = { ...source };
      delete (payload as any).id;
      const created = await addSkillOptRouteOutcomeRequest(payload).unwrap();
      setAllData((prev) => [created as SkillOptRouteOutcomeRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add SkillOptRouteOutcomeRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SkillOptRouteOutcomeRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSkillOptRouteOutcomeRequest(payload).unwrap();
      setAllData((prev) => [created as SkillOptRouteOutcomeRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) SkillOptRouteOutcomeRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SkillOptRouteOutcomeRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSkillOptRouteOutcomeRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSkillOptRouteOutcomeRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SkillOptRouteOutcomeRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SkillOptRouteOutcomeRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SkillOptRouteOutcomeRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SkillOptRouteOutcomeRequest[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="SkillOptRouteOutcomeRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="SkillOptRouteOutcomeRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SkillOptRouteOutcomeRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SkillOptRouteOutcomeRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SkillOptRouteOutcomeRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SkillOptRouteOutcomeRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SkillOptRouteOutcomeRequest;
                ...
                await updateSkillOptRouteOutcomeRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SkillOptRouteOutcomeRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SkillOptRouteOutcomeRequest'}
...
export default SkillOptRouteOutcomeRequestTable;
