...
import PublishFunnel200ResponseForm from '@thorapi/redux/components/form/PublishFunnel200ResponseForm';
...
} from '@thorapi/redux/services/PublishFunnel200ResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `PublishFunnel200Response update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/PublishFunnel200Response/${rowId}`;
...
const PublishFunnel200ResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): PublishFunnel200Response[] => {
    if (Array.isArray(result)) return result as PublishFunnel200Response[];
    ...
    return Array.isArray(candidate) ? (candidate as PublishFunnel200Response[]) : [];
  };

  const mergeFetchedRows = (incomingRows: PublishFunnel200Response[], requestedPage: number) => {
  ...
  const [deletePublishFunnel200ResponseCascade] = useDeletePublishFunnel200ResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<PublishFunnel200Response | null> => {
    ...
    try {
      ...
      return (await response.json()) as PublishFunnel200Response;
    } catch (err) {
      console.warn('Could not reload PublishFunnel200Response row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<PublishFunnel200Response>,
    successField?: string,
  ) => {
    try {
      const saved = await updatePublishFunnel200Response(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as PublishFunnel200Response[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update PublishFunnel200Response';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePublishFunnel200ResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `PublishFunnel200Response-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addPublishFunnel200Response(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updatePublishFunnel200Response(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as PublishFunnel200Response[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as PublishFunnel200Response[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as PublishFunnel200Response[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as PublishFunnel200Response[]);
    }
  };
