...
import { OasParameter } from '@thorapi/model';
import OasParameterForm from '@thorapi/redux/components/form/OasParameterForm';
...
} from '@thorapi/redux/services/OasParameterService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `OasParameter update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/OasParameter/${rowId}`;
...
const OasParameterTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): OasParameter[] => {
    if (Array.isArray(result)) return result as OasParameter[];
    ...
    return Array.isArray(candidate) ? (candidate as OasParameter[]) : [];
  };

  const mergeFetchedRows = (incomingRows: OasParameter[], requestedPage: number) => {
  ...
  const [deleteOasParameterCascade] = useDeleteOasParameterCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<OasParameter | null> => {
    ...
    try {
      ...
      return (await response.json()) as OasParameter;
    } catch (err) {
      console.warn('Could not reload OasParameter row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<OasParameter>,
    successField?: string,
  ) => {
    try {
      const saved = await updateOasParameter(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as OasParameter[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update OasParameter';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOasParameterCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `OasParameter-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addOasParameter(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateOasParameter(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as OasParameter[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as OasParameter[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as OasParameter[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as OasParameter[]);
    }
  };
