...
import FileDownloadTokenForm from '@thorapi/redux/components/form/FileDownloadTokenForm';
...
} from '@thorapi/redux/services/FileDownloadTokenService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `FileDownloadToken update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/FileDownloadToken/${rowId}`;
...
const FileDownloadTokenTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): FileDownloadToken[] => {
    if (Array.isArray(result)) return result as FileDownloadToken[];
    ...
    return Array.isArray(candidate) ? (candidate as FileDownloadToken[]) : [];
  };

  const mergeFetchedRows = (incomingRows: FileDownloadToken[], requestedPage: number) => {
  ...
  const [deleteFileDownloadTokenCascade] = useDeleteFileDownloadTokenCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<FileDownloadToken | null> => {
    ...
    try {
      ...
      return (await response.json()) as FileDownloadToken;
    } catch (err) {
      console.warn('Could not reload FileDownloadToken row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<FileDownloadToken>,
    successField?: string,
  ) => {
    try {
      const saved = await updateFileDownloadToken(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as FileDownloadToken[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update FileDownloadToken';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteFileDownloadTokenCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `FileDownloadToken-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addFileDownloadToken(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateFileDownloadToken(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as FileDownloadToken[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as FileDownloadToken[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as FileDownloadToken[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as FileDownloadToken[]);
    }
  };
