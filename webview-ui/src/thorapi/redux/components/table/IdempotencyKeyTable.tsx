...
import { IdempotencyKey } from '@thorapi/model';
import IdempotencyKeyForm from '@thorapi/redux/components/form/IdempotencyKeyForm';
...
import {
  useLazyGetIdempotencyKeysPagedQuery,
...
} from '@thorapi/redux/services/IdempotencyKeyService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `IdempotencyKey update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/IdempotencyKey/${rowId}`;

const IdempotencyKeyTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<IdempotencyKey[]>([]);
  ...
  const normalizePageRows = (result: unknown): IdempotencyKey[] => {
    if (Array.isArray(result)) return result as IdempotencyKey[];
    ...
    return Array.isArray(candidate) ? (candidate as IdempotencyKey[]) : [];
  };

  const mergeFetchedRows = (incomingRows: IdempotencyKey[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addIdempotencyKey] = useAddIdempotencyKeyMutation();
  const [updateIdempotencyKey] = useUpdateIdempotencyKeyMutation();
  const [deleteIdempotencyKey] = useDeleteIdempotencyKeyMutation();
  const [deleteIdempotencyKeyCascade] = useDeleteIdempotencyKeyCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<IdempotencyKey>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<IdempotencyKey | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<IdempotencyKey | null> => {
    ...
    try {
      ...
      return (await response.json()) as IdempotencyKey;
    } catch (err) {
      console.warn('Could not reload IdempotencyKey row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<IdempotencyKey>,
    successField?: string,
  ) => {
    try {
      const saved = await updateIdempotencyKey(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as IdempotencyKey[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update IdempotencyKey';
      console.error('Failed to update IdempotencyKey:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteIdempotencyKeyCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteIdempotencyKey(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<IdempotencyKey> = { ...source };
      delete (payload as any).id;
      const created = await addIdempotencyKey(payload).unwrap();
      setAllData((prev) => [created as IdempotencyKey, ...prev]);
    } catch (error) {
      console.error('Failed to add IdempotencyKey:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<IdempotencyKey> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addIdempotencyKey(payload).unwrap();
      setAllData((prev) => [created as IdempotencyKey, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) IdempotencyKey:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `IdempotencyKey-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addIdempotencyKey(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateIdempotencyKey(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as IdempotencyKey[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as IdempotencyKey[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as IdempotencyKey[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as IdempotencyKey[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.IdempotencyKey',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="IdempotencyKeyTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="IdempotencyKey Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create IdempotencyKey</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <IdempotencyKeyForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter IdempotencyKey (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing IdempotencyKey</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof IdempotencyKey;
                ...
                await updateIdempotencyKey(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of IdempotencyKeys.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'IdempotencyKey'}
...
export default IdempotencyKeyTable;
