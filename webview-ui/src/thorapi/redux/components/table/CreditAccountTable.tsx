...
import CreditAccountForm from '@thorapi/redux/components/form/CreditAccountForm';
...
import {
  useLazyGetCreditAccountsPagedQuery,
...
} from '@thorapi/redux/services/CreditAccountService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `CreditAccount update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/CreditAccount/${rowId}`;

const CreditAccountTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<CreditAccount[]>([]);
  ...
  const normalizePageRows = (result: unknown): CreditAccount[] => {
    if (Array.isArray(result)) return result as CreditAccount[];
    ...
    return Array.isArray(candidate) ? (candidate as CreditAccount[]) : [];
  };

  const mergeFetchedRows = (incomingRows: CreditAccount[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addCreditAccount] = useAddCreditAccountMutation();
  const [updateCreditAccount] = useUpdateCreditAccountMutation();
  const [deleteCreditAccount] = useDeleteCreditAccountMutation();
  const [deleteCreditAccountCascade] = useDeleteCreditAccountCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<CreditAccount>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<CreditAccount | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<CreditAccount | null> => {
    ...
    try {
      ...
      return (await response.json()) as CreditAccount;
    } catch (err) {
      console.warn('Could not reload CreditAccount row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<CreditAccount>,
    successField?: string,
  ) => {
    try {
      const saved = await updateCreditAccount(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as CreditAccount[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update CreditAccount';
      console.error('Failed to update CreditAccount:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCreditAccountCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteCreditAccount(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<CreditAccount> = { ...source };
      delete (payload as any).id;
      const created = await addCreditAccount(payload).unwrap();
      setAllData((prev) => [created as CreditAccount, ...prev]);
    } catch (error) {
      console.error('Failed to add CreditAccount:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<CreditAccount> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addCreditAccount(payload).unwrap();
      setAllData((prev) => [created as CreditAccount, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) CreditAccount:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `CreditAccount-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addCreditAccount(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateCreditAccount(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as CreditAccount[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as CreditAccount[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as CreditAccount[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as CreditAccount[]);
    }
  };
    ...
    <>
          ...
          >
            ...
            <FloatingControlPanel
              description="CreditAccount Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create CreditAccount</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <CreditAccountForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter CreditAccount (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing CreditAccount</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof CreditAccount;
                ...
                await updateCreditAccount(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of CreditAccounts.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'CreditAccount'}
