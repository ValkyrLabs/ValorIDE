...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RequeueDeadLetterEntryRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RequeueDeadLetterEntryRequest/${rowId}`;
...
const RequeueDeadLetterEntryRequestTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): RequeueDeadLetterEntryRequest[] => {
    if (Array.isArray(result)) return result as RequeueDeadLetterEntryRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as RequeueDeadLetterEntryRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RequeueDeadLetterEntryRequest[], requestedPage: number) => {
  ...
  const reloadRowById = async (rowId: string): Promise<RequeueDeadLetterEntryRequest | null> => {
    ...
    try {
      ...
      console.warn('Could not reload RequeueDeadLetterEntryRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateRequeueDeadLetterEntryRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RequeueDeadLetterEntryRequest[]);
      if (successField) {
...
// AddressToJSON(value?: Address)
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RequeueDeadLetterEntryRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RequeueDeadLetterEntryRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RequeueDeadLetterEntryRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RequeueDeadLetterEntryRequest[]);
    }
  };
