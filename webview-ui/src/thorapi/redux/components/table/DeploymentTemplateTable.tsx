...
import DeploymentTemplateForm from '@thorapi/redux/components/form/DeploymentTemplateForm';
...
import {
  useLazyGetDeploymentTemplatesPagedQuery,
...
} from '@thorapi/redux/services/DeploymentTemplateService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `DeploymentTemplate update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/DeploymentTemplate/${rowId}`;

const DeploymentTemplateTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<DeploymentTemplate[]>([]);
  ...
  const normalizePageRows = (result: unknown): DeploymentTemplate[] => {
    if (Array.isArray(result)) return result as DeploymentTemplate[];
    ...
    return Array.isArray(candidate) ? (candidate as DeploymentTemplate[]) : [];
  };

  const mergeFetchedRows = (incomingRows: DeploymentTemplate[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addDeploymentTemplate] = useAddDeploymentTemplateMutation();
  const [updateDeploymentTemplate] = useUpdateDeploymentTemplateMutation();
  const [deleteDeploymentTemplate] = useDeleteDeploymentTemplateMutation();
  const [deleteDeploymentTemplateCascade] = useDeleteDeploymentTemplateCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<DeploymentTemplate>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<DeploymentTemplate | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<DeploymentTemplate | null> => {
    ...
    try {
      ...
      return (await response.json()) as DeploymentTemplate;
    } catch (err) {
      console.warn('Could not reload DeploymentTemplate row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<DeploymentTemplate>,
    successField?: string,
  ) => {
    try {
      const saved = await updateDeploymentTemplate(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as DeploymentTemplate[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update DeploymentTemplate';
      console.error('Failed to update DeploymentTemplate:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteDeploymentTemplateCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteDeploymentTemplate(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<DeploymentTemplate> = { ...source };
      delete (payload as any).id;
      const created = await addDeploymentTemplate(payload).unwrap();
      setAllData((prev) => [created as DeploymentTemplate, ...prev]);
    } catch (error) {
      console.error('Failed to add DeploymentTemplate:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<DeploymentTemplate> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addDeploymentTemplate(payload).unwrap();
      setAllData((prev) => [created as DeploymentTemplate, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) DeploymentTemplate:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `DeploymentTemplate-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addDeploymentTemplate(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateDeploymentTemplate(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as DeploymentTemplate[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as DeploymentTemplate[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as DeploymentTemplate[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as DeploymentTemplate[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="DeploymentTemplateTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="DeploymentTemplate Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create DeploymentTemplate</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter DeploymentTemplate (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing DeploymentTemplate</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof DeploymentTemplate;
                ...
                await updateDeploymentTemplate(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of DeploymentTemplates.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'DeploymentTemplate'}
...
export default DeploymentTemplateTable;
