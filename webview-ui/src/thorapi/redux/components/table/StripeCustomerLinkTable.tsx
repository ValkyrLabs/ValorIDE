...
import { StripeCustomerLink } from '@thorapi/model';
import StripeCustomerLinkForm from '@thorapi/redux/components/form/StripeCustomerLinkForm';
...
import {
  useLazyGetStripeCustomerLinksPagedQuery,
...
} from '@thorapi/redux/services/StripeCustomerLinkService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'status': { type: 'enum', enumValues: [
    'active', 'inactive', 'replaced', 'disputed', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `StripeCustomerLink update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/StripeCustomerLink/${rowId}`;

const StripeCustomerLinkTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<StripeCustomerLink[]>([]);
  ...
  const normalizePageRows = (result: unknown): StripeCustomerLink[] => {
    if (Array.isArray(result)) return result as StripeCustomerLink[];
    ...
    return Array.isArray(candidate) ? (candidate as StripeCustomerLink[]) : [];
  };

  const mergeFetchedRows = (incomingRows: StripeCustomerLink[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addStripeCustomerLink] = useAddStripeCustomerLinkMutation();
  const [updateStripeCustomerLink] = useUpdateStripeCustomerLinkMutation();
  const [deleteStripeCustomerLink] = useDeleteStripeCustomerLinkMutation();
  const [deleteStripeCustomerLinkCascade] = useDeleteStripeCustomerLinkCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<StripeCustomerLink>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<StripeCustomerLink | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<StripeCustomerLink | null> => {
    ...
    try {
      ...
      return (await response.json()) as StripeCustomerLink;
    } catch (err) {
      console.warn('Could not reload StripeCustomerLink row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<StripeCustomerLink>,
    successField?: string,
  ) => {
    try {
      const saved = await updateStripeCustomerLink(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as StripeCustomerLink[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update StripeCustomerLink';
      console.error('Failed to update StripeCustomerLink:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteStripeCustomerLinkCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteStripeCustomerLink(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<StripeCustomerLink> = { ...source };
      delete (payload as any).id;
      const created = await addStripeCustomerLink(payload).unwrap();
      setAllData((prev) => [created as StripeCustomerLink, ...prev]);
    } catch (error) {
      console.error('Failed to add StripeCustomerLink:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<StripeCustomerLink> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addStripeCustomerLink(payload).unwrap();
      setAllData((prev) => [created as StripeCustomerLink, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) StripeCustomerLink:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `StripeCustomerLink-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addStripeCustomerLink(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateStripeCustomerLink(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as StripeCustomerLink[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as StripeCustomerLink[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as StripeCustomerLink[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as StripeCustomerLink[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="StripeCustomerLinkTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="StripeCustomerLink Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create StripeCustomerLink</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <StripeCustomerLinkForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter StripeCustomerLink (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing StripeCustomerLink</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof StripeCustomerLink;
                ...
                await updateStripeCustomerLink(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of StripeCustomerLinks.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'StripeCustomerLink'}
...
export default StripeCustomerLinkTable;
