...
import { TrustSecretAccessEvent } from '@thorapi/model';
import TrustSecretAccessEventForm from '@thorapi/redux/components/form/TrustSecretAccessEventForm';
...
import {
  useLazyGetTrustSecretAccessEventsPagedQuery,
...
} from '@thorapi/redux/services/TrustSecretAccessEventService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TrustSecretAccessEvent update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TrustSecretAccessEvent/${rowId}`;

const TrustSecretAccessEventTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TrustSecretAccessEvent[]>([]);
  ...
  const normalizePageRows = (result: unknown): TrustSecretAccessEvent[] => {
    if (Array.isArray(result)) return result as TrustSecretAccessEvent[];
    ...
    return Array.isArray(candidate) ? (candidate as TrustSecretAccessEvent[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TrustSecretAccessEvent[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addTrustSecretAccessEvent] = useAddTrustSecretAccessEventMutation();
  const [updateTrustSecretAccessEvent] = useUpdateTrustSecretAccessEventMutation();
  const [deleteTrustSecretAccessEvent] = useDeleteTrustSecretAccessEventMutation();
  const [deleteTrustSecretAccessEventCascade] = useDeleteTrustSecretAccessEventCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TrustSecretAccessEvent>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TrustSecretAccessEvent | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TrustSecretAccessEvent | null> => {
    ...
    try {
      ...
      return (await response.json()) as TrustSecretAccessEvent;
    } catch (err) {
      console.warn('Could not reload TrustSecretAccessEvent row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TrustSecretAccessEvent>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTrustSecretAccessEvent(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TrustSecretAccessEvent[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TrustSecretAccessEvent';
      console.error('Failed to update TrustSecretAccessEvent:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustSecretAccessEventCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTrustSecretAccessEvent(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TrustSecretAccessEvent> = { ...source };
      delete (payload as any).id;
      const created = await addTrustSecretAccessEvent(payload).unwrap();
      setAllData((prev) => [created as TrustSecretAccessEvent, ...prev]);
    } catch (error) {
      console.error('Failed to add TrustSecretAccessEvent:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TrustSecretAccessEvent> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTrustSecretAccessEvent(payload).unwrap();
      setAllData((prev) => [created as TrustSecretAccessEvent, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TrustSecretAccessEvent:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TrustSecretAccessEvent-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTrustSecretAccessEvent(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTrustSecretAccessEvent(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TrustSecretAccessEvent[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TrustSecretAccessEvent[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TrustSecretAccessEvent[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TrustSecretAccessEvent[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="TrustSecretAccessEventTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="TrustSecretAccessEvent Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TrustSecretAccessEvent</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TrustSecretAccessEventForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TrustSecretAccessEvent (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TrustSecretAccessEvent</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TrustSecretAccessEvent;
                ...
                await updateTrustSecretAccessEvent(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TrustSecretAccessEvents.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TrustSecretAccessEvent'}
...
export default TrustSecretAccessEventTable;
