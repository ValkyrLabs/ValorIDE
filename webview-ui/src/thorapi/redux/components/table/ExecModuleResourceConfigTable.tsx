...
import ExecModuleResourceConfigForm from '@thorapi/redux/components/form/ExecModuleResourceConfigForm';
...
import {
  useLazyGetExecModuleResourceConfigsPagedQuery,
...
} from '@thorapi/redux/services/ExecModuleResourceConfigService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ExecModuleResourceConfig update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ExecModuleResourceConfig/${rowId}`;

const ExecModuleResourceConfigTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ExecModuleResourceConfig[]>([]);
  ...
  const normalizePageRows = (result: unknown): ExecModuleResourceConfig[] => {
    if (Array.isArray(result)) return result as ExecModuleResourceConfig[];
    ...
    return Array.isArray(candidate) ? (candidate as ExecModuleResourceConfig[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ExecModuleResourceConfig[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addExecModuleResourceConfig] = useAddExecModuleResourceConfigMutation();
  const [updateExecModuleResourceConfig] = useUpdateExecModuleResourceConfigMutation();
  const [deleteExecModuleResourceConfig] = useDeleteExecModuleResourceConfigMutation();
  const [deleteExecModuleResourceConfigCascade] = useDeleteExecModuleResourceConfigCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ExecModuleResourceConfig>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ExecModuleResourceConfig | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ExecModuleResourceConfig | null> => {
    ...
    try {
      ...
      return (await response.json()) as ExecModuleResourceConfig;
    } catch (err) {
      console.warn('Could not reload ExecModuleResourceConfig row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ExecModuleResourceConfig>,
    successField?: string,
  ) => {
    try {
      const saved = await updateExecModuleResourceConfig(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ExecModuleResourceConfig[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ExecModuleResourceConfig';
      console.error('Failed to update ExecModuleResourceConfig:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecModuleResourceConfigCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteExecModuleResourceConfig(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ExecModuleResourceConfig> = { ...source };
      delete (payload as any).id;
      const created = await addExecModuleResourceConfig(payload).unwrap();
      setAllData((prev) => [created as ExecModuleResourceConfig, ...prev]);
    } catch (error) {
      console.error('Failed to add ExecModuleResourceConfig:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ExecModuleResourceConfig> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addExecModuleResourceConfig(payload).unwrap();
      setAllData((prev) => [created as ExecModuleResourceConfig, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ExecModuleResourceConfig:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ExecModuleResourceConfig-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addExecModuleResourceConfig(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateExecModuleResourceConfig(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ExecModuleResourceConfig[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ExecModuleResourceConfig[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ExecModuleResourceConfig[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ExecModuleResourceConfig[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ExecModuleResourceConfigTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ExecModuleResourceConfig Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ExecModuleResourceConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ExecModuleResourceConfigForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ExecModuleResourceConfig (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ExecModuleResourceConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ExecModuleResourceConfig;
                ...
                await updateExecModuleResourceConfig(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ExecModuleResourceConfigs.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ExecModuleResourceConfig'}
...
export default ExecModuleResourceConfigTable;
