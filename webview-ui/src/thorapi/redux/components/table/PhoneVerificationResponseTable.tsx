...
import { PhoneVerificationResponse } from '@thorapi/model';
import PhoneVerificationResponseForm from '@thorapi/redux/components/form/PhoneVerificationResponseForm';
...
import {
  useLazyGetPhoneVerificationResponsesPagedQuery,
...
} from '@thorapi/redux/services/PhoneVerificationResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `PhoneVerificationResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/PhoneVerificationResponse/${rowId}`;

const PhoneVerificationResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<PhoneVerificationResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): PhoneVerificationResponse[] => {
    if (Array.isArray(result)) return result as PhoneVerificationResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as PhoneVerificationResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: PhoneVerificationResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addPhoneVerificationResponse] = useAddPhoneVerificationResponseMutation();
  const [updatePhoneVerificationResponse] = useUpdatePhoneVerificationResponseMutation();
  const [deletePhoneVerificationResponse] = useDeletePhoneVerificationResponseMutation();
  const [deletePhoneVerificationResponseCascade] = useDeletePhoneVerificationResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<PhoneVerificationResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<PhoneVerificationResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<PhoneVerificationResponse | null> => {
    ...
    try {
      ...
      console.warn('Could not reload PhoneVerificationResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<PhoneVerificationResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updatePhoneVerificationResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as PhoneVerificationResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update PhoneVerificationResponse';
      console.error('Failed to update PhoneVerificationResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePhoneVerificationResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deletePhoneVerificationResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<PhoneVerificationResponse> = { ...source };
      delete (payload as any).id;
      const created = await addPhoneVerificationResponse(payload).unwrap();
      setAllData((prev) => [created as PhoneVerificationResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add PhoneVerificationResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<PhoneVerificationResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addPhoneVerificationResponse(payload).unwrap();
      setAllData((prev) => [created as PhoneVerificationResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) PhoneVerificationResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `PhoneVerificationResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addPhoneVerificationResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updatePhoneVerificationResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as PhoneVerificationResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as PhoneVerificationResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as PhoneVerificationResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as PhoneVerificationResponse[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="PhoneVerificationResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="PhoneVerificationResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create PhoneVerificationResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <PhoneVerificationResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter PhoneVerificationResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing PhoneVerificationResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof PhoneVerificationResponse;
                ...
                await updatePhoneVerificationResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of PhoneVerificationResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'PhoneVerificationResponse'}
...
export default PhoneVerificationResponseTable;
