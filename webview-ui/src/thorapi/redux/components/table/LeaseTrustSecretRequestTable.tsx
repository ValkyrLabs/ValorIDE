...
import { LeaseTrustSecretRequest } from '@thorapi/model';
import LeaseTrustSecretRequestForm from '@thorapi/redux/components/form/LeaseTrustSecretRequestForm';
...
import {
  useLazyGetLeaseTrustSecretRequestsPagedQuery,
...
} from '@thorapi/redux/services/LeaseTrustSecretRequestService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'leasePurpose': { type: 'enum', enumValues: [
    'ENCRYPT', 'DECRYPT', 'SIGN', 'VERIFY', 'WRAP', 'UNWRAP', 'REWRAP', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `LeaseTrustSecretRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/LeaseTrustSecretRequest/${rowId}`;

const LeaseTrustSecretRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<LeaseTrustSecretRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): LeaseTrustSecretRequest[] => {
    if (Array.isArray(result)) return result as LeaseTrustSecretRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as LeaseTrustSecretRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: LeaseTrustSecretRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addLeaseTrustSecretRequest] = useAddLeaseTrustSecretRequestMutation();
  const [updateLeaseTrustSecretRequest] = useUpdateLeaseTrustSecretRequestMutation();
  const [deleteLeaseTrustSecretRequest] = useDeleteLeaseTrustSecretRequestMutation();
  const [deleteLeaseTrustSecretRequestCascade] = useDeleteLeaseTrustSecretRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<LeaseTrustSecretRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<LeaseTrustSecretRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<LeaseTrustSecretRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as LeaseTrustSecretRequest;
    } catch (err) {
      console.warn('Could not reload LeaseTrustSecretRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<LeaseTrustSecretRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateLeaseTrustSecretRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as LeaseTrustSecretRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update LeaseTrustSecretRequest';
      console.error('Failed to update LeaseTrustSecretRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteLeaseTrustSecretRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteLeaseTrustSecretRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<LeaseTrustSecretRequest> = { ...source };
      delete (payload as any).id;
      const created = await addLeaseTrustSecretRequest(payload).unwrap();
      setAllData((prev) => [created as LeaseTrustSecretRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add LeaseTrustSecretRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<LeaseTrustSecretRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addLeaseTrustSecretRequest(payload).unwrap();
      setAllData((prev) => [created as LeaseTrustSecretRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) LeaseTrustSecretRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `LeaseTrustSecretRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addLeaseTrustSecretRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateLeaseTrustSecretRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as LeaseTrustSecretRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as LeaseTrustSecretRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as LeaseTrustSecretRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as LeaseTrustSecretRequest[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="LeaseTrustSecretRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="LeaseTrustSecretRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create LeaseTrustSecretRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <LeaseTrustSecretRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter LeaseTrustSecretRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing LeaseTrustSecretRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof LeaseTrustSecretRequest;
                ...
                await updateLeaseTrustSecretRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of LeaseTrustSecretRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'LeaseTrustSecretRequest'}
...
export default LeaseTrustSecretRequestTable;
