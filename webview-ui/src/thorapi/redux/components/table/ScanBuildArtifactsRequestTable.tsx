...
const stringFieldCandidates = ["applicationId", "searchPath"];
...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ScanBuildArtifactsRequest/${rowId}`;
...
const ScanBuildArtifactsRequestTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addScanBuildArtifactsRequest] =
    useAddScanBuildArtifactsRequestMutation();
  const [updateScanBuildArtifactsRequest] =
    useUpdateScanBuildArtifactsRequestMutation();
  const [deleteScanBuildArtifactsRequest] =
    useDeleteScanBuildArtifactsRequestMutation();
  const [deleteScanBuildArtifactsRequestCascade] =
    useDeleteScanBuildArtifactsRequestCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<ScanBuildArtifactsRequest | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<ScanBuildArtifactsRequest | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload ScanBuildArtifactsRequest row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateScanBuildArtifactsRequest(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ScanBuildArtifactsRequest";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteScanBuildArtifactsRequestCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) ScanBuildArtifactsRequest:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter ScanBuildArtifactsRequest (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
