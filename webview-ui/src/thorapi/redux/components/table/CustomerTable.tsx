...
import { Customer } from '@thorapi/model';
import CustomerForm from '@thorapi/redux/components/form/CustomerForm';
...
} from '@thorapi/redux/services/CustomerService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Customer update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Customer/${rowId}`;
...
const CustomerTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Customer[] => {
    ...
    return Array.isArray(candidate) ? (candidate as Customer[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Customer[], requestedPage: number) => {
  ...
  const [deleteCustomerCascade] = useDeleteCustomerCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Customer | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Customer row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Customer>,
    successField?: string,
  ) => {
    try {
      const saved = await updateCustomer(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Customer[]);
      if (successField) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCustomerCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Customer-export-${stamp}.json`;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Customer[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Customer[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Customer[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Customer[]);
    }
  };
