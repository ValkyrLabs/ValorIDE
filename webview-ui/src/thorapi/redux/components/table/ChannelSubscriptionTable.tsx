...
import ChannelSubscriptionForm from '@thorapi/redux/components/form/ChannelSubscriptionForm';
...
} from '@thorapi/redux/services/ChannelSubscriptionService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ChannelSubscription update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ChannelSubscription/${rowId}`;
...
const ChannelSubscriptionTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ChannelSubscription[] => {
    if (Array.isArray(result)) return result as ChannelSubscription[];
    ...
    return Array.isArray(candidate) ? (candidate as ChannelSubscription[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ChannelSubscription[], requestedPage: number) => {
  ...
  const [deleteChannelSubscriptionCascade] = useDeleteChannelSubscriptionCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ChannelSubscription | null> => {
    ...
    try {
      ...
      return (await response.json()) as ChannelSubscription;
    } catch (err) {
      console.warn('Could not reload ChannelSubscription row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ChannelSubscription>,
    successField?: string,
  ) => {
    try {
      const saved = await updateChannelSubscription(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ChannelSubscription[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ChannelSubscription';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteChannelSubscriptionCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ChannelSubscription-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addChannelSubscription(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateChannelSubscription(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ChannelSubscription[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ChannelSubscription[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ChannelSubscription[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ChannelSubscription[]);
    }
  };
