...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  providerStatus: {
    type: "enum",
    enumValues: ["ACTIVE", "DISABLED", "DEGRADED", "RETIRED"],
    enumValueType: "string",
  },
...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/TrustKeyProvider/${rowId}`;
...
const TrustKeyProviderTable: React.FC = () => {
  ...
  const [deleteTrustKeyProviderCascade] =
    useDeleteTrustKeyProviderCascadeMutation();
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update TrustKeyProvider";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustKeyProviderCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
