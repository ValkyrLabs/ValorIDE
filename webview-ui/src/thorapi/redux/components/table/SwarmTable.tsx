...
import { Swarm } from '@thorapi/model';
import SwarmForm from '@thorapi/redux/components/form/SwarmForm';
...
} from '@thorapi/redux/services/SwarmService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Swarm update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Swarm/${rowId}`;
...
const SwarmTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Swarm[] => {
    if (Array.isArray(result)) return result as Swarm[];
    ...
    return Array.isArray(candidate) ? (candidate as Swarm[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Swarm[], requestedPage: number) => {
  ...
  const [deleteSwarmCascade] = useDeleteSwarmCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Swarm | null> => {
    ...
    try {
      ...
      return (await response.json()) as Swarm;
    } catch (err) {
      console.warn('Could not reload Swarm row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Swarm>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSwarm(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Swarm[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Swarm';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Swarm-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        try {
          ...
          try {
            await updateSwarm(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Swarm[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Swarm[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Swarm[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Swarm[]);
    }
  };
