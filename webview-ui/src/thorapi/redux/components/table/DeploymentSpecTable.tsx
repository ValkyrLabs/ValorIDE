...
import DeploymentSpecForm from '@thorapi/redux/components/form/DeploymentSpecForm';
...
import {
  useLazyGetDeploymentSpecsPagedQuery,
...
} from '@thorapi/redux/services/DeploymentSpecService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `DeploymentSpec update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/DeploymentSpec/${rowId}`;

const DeploymentSpecTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<DeploymentSpec[]>([]);
  ...
  const normalizePageRows = (result: unknown): DeploymentSpec[] => {
    if (Array.isArray(result)) return result as DeploymentSpec[];
    ...
    return Array.isArray(candidate) ? (candidate as DeploymentSpec[]) : [];
  };

  const mergeFetchedRows = (incomingRows: DeploymentSpec[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addDeploymentSpec] = useAddDeploymentSpecMutation();
  const [updateDeploymentSpec] = useUpdateDeploymentSpecMutation();
  const [deleteDeploymentSpec] = useDeleteDeploymentSpecMutation();
  const [deleteDeploymentSpecCascade] = useDeleteDeploymentSpecCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<DeploymentSpec>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<DeploymentSpec | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<DeploymentSpec | null> => {
    ...
    try {
      ...
      return (await response.json()) as DeploymentSpec;
    } catch (err) {
      console.warn('Could not reload DeploymentSpec row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<DeploymentSpec>,
    successField?: string,
  ) => {
    try {
      const saved = await updateDeploymentSpec(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as DeploymentSpec[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update DeploymentSpec';
      console.error('Failed to update DeploymentSpec:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteDeploymentSpecCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteDeploymentSpec(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<DeploymentSpec> = { ...source };
      delete (payload as any).id;
      const created = await addDeploymentSpec(payload).unwrap();
      setAllData((prev) => [created as DeploymentSpec, ...prev]);
    } catch (error) {
      console.error('Failed to add DeploymentSpec:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<DeploymentSpec> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addDeploymentSpec(payload).unwrap();
      setAllData((prev) => [created as DeploymentSpec, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) DeploymentSpec:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `DeploymentSpec-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addDeploymentSpec(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateDeploymentSpec(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as DeploymentSpec[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as DeploymentSpec[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as DeploymentSpec[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as DeploymentSpec[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="DeploymentSpecTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="DeploymentSpec Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create DeploymentSpec</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <DeploymentSpecForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter DeploymentSpec (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing DeploymentSpec</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof DeploymentSpec;
                ...
                await updateDeploymentSpec(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of DeploymentSpecs.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'DeploymentSpec'}
...
export default DeploymentSpecTable;
