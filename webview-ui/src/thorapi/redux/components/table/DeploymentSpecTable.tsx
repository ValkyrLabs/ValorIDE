...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/DeploymentSpec/${rowId}`;
...
const DeploymentSpecTable: React.FC = () => {
  ...
  const [deleteDeploymentSpecCascade] =
    useDeleteDeploymentSpecCascadeMutation();
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update DeploymentSpec";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteDeploymentSpecCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
