...
import { TicketComment } from '@thorapi/model';
import TicketCommentForm from '@thorapi/redux/components/form/TicketCommentForm';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TicketComment update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TicketComment/${rowId}`;

const TicketCommentTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TicketComment[]>([]);
  ...
  const normalizePageRows = (result: unknown): TicketComment[] => {
    if (Array.isArray(result)) return result as TicketComment[];
    ...
    return Array.isArray(candidate) ? (candidate as TicketComment[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TicketComment[], requestedPage: number) => {
  ...
  const [updateTicketComment] = useUpdateTicketCommentMutation();
  const [deleteTicketComment] = useDeleteTicketCommentMutation();
  const [deleteTicketCommentCascade] = useDeleteTicketCommentCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TicketComment>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TicketComment | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TicketComment | null> => {
    ...
    try {
      ...
      console.warn('Could not reload TicketComment row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TicketComment>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTicketComment(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TicketComment[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TicketComment';
      console.error('Failed to update TicketComment:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTicketCommentCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTicketComment(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TicketComment> = { ...source };
      delete (payload as any).id;
      const created = await addTicketComment(payload).unwrap();
      setAllData((prev) => [created as TicketComment, ...prev]);
    } catch (error) {
      console.error('Failed to add TicketComment:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TicketComment> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTicketComment(payload).unwrap();
      setAllData((prev) => [created as TicketComment, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TicketComment:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TicketComment-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTicketComment(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTicketComment(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TicketComment[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TicketComment[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TicketComment[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TicketComment[]);
    }
  };
    ...
    <>
          ...
          >
            ...
            <FloatingControlPanel
              description="TicketComment Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TicketComment</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TicketCommentForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TicketComment (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TicketComment</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TicketComment;
                ...
                await updateTicketComment(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TicketComments.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TicketComment'}
...
export default TicketCommentTable;
