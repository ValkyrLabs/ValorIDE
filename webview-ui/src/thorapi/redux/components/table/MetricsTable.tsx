...
import { Metrics } from '@thorapi/model';
import MetricsForm from '@thorapi/redux/components/form/MetricsForm';
...
} from '@thorapi/redux/services/MetricsService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Metrics update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Metrics/${rowId}`;
...
const MetricsTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Metrics[] => {
    if (Array.isArray(result)) return result as Metrics[];
    ...
    return Array.isArray(candidate) ? (candidate as Metrics[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Metrics[], requestedPage: number) => {
  ...
  const [deleteMetricsCascade] = useDeleteMetricsCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Metrics | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Metrics row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Metrics>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMetrics(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Metrics[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Metrics';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMetricsCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Metrics-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMetrics(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Metrics[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Metrics[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Metrics[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Metrics[]);
    }
  };
