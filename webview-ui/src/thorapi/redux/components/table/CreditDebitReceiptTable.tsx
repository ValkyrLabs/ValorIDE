...
import { CreditDebitReceipt } from '@thorapi/model';
import CreditDebitReceiptForm from '@thorapi/redux/components/form/CreditDebitReceiptForm';
...
import {
  useLazyGetCreditDebitReceiptsPagedQuery,
...
} from '@thorapi/redux/services/CreditDebitReceiptService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'status': { type: 'enum', enumValues: [
    'debited', 'replayed', 'partial', 'failed', 
  ], enumValueType: 'string' },
  'meteredUnits': { type: 'number' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `CreditDebitReceipt update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/CreditDebitReceipt/${rowId}`;

const CreditDebitReceiptTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<CreditDebitReceipt[]>([]);
  ...
  const normalizePageRows = (result: unknown): CreditDebitReceipt[] => {
    if (Array.isArray(result)) return result as CreditDebitReceipt[];
    ...
    return Array.isArray(candidate) ? (candidate as CreditDebitReceipt[]) : [];
  };

  const mergeFetchedRows = (incomingRows: CreditDebitReceipt[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addCreditDebitReceipt] = useAddCreditDebitReceiptMutation();
  const [updateCreditDebitReceipt] = useUpdateCreditDebitReceiptMutation();
  const [deleteCreditDebitReceipt] = useDeleteCreditDebitReceiptMutation();
  const [deleteCreditDebitReceiptCascade] = useDeleteCreditDebitReceiptCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<CreditDebitReceipt>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<CreditDebitReceipt | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<CreditDebitReceipt | null> => {
    ...
    try {
      ...
      return (await response.json()) as CreditDebitReceipt;
    } catch (err) {
      console.warn('Could not reload CreditDebitReceipt row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<CreditDebitReceipt>,
    successField?: string,
  ) => {
    try {
      const saved = await updateCreditDebitReceipt(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as CreditDebitReceipt[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update CreditDebitReceipt';
      console.error('Failed to update CreditDebitReceipt:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCreditDebitReceiptCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteCreditDebitReceipt(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<CreditDebitReceipt> = { ...source };
      delete (payload as any).id;
      const created = await addCreditDebitReceipt(payload).unwrap();
      setAllData((prev) => [created as CreditDebitReceipt, ...prev]);
    } catch (error) {
      console.error('Failed to add CreditDebitReceipt:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<CreditDebitReceipt> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addCreditDebitReceipt(payload).unwrap();
      setAllData((prev) => [created as CreditDebitReceipt, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) CreditDebitReceipt:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `CreditDebitReceipt-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addCreditDebitReceipt(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateCreditDebitReceipt(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as CreditDebitReceipt[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as CreditDebitReceipt[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as CreditDebitReceipt[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as CreditDebitReceipt[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="CreditDebitReceiptTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="CreditDebitReceipt Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create CreditDebitReceipt</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <CreditDebitReceiptForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter CreditDebitReceipt (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing CreditDebitReceipt</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof CreditDebitReceipt;
                ...
                await updateCreditDebitReceipt(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of CreditDebitReceipts.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'CreditDebitReceipt'}
...
export default CreditDebitReceiptTable;
