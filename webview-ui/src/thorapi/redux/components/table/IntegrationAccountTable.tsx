...
import { IntegrationAccount } from '@thorapi/model';
import IntegrationAccountForm from '@thorapi/redux/components/form/IntegrationAccountForm';
...
} from '@thorapi/redux/services/IntegrationAccountService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `IntegrationAccount update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/IntegrationAccount/${rowId}`;
...
const IntegrationAccountTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): IntegrationAccount[] => {
    if (Array.isArray(result)) return result as IntegrationAccount[];
    ...
    return Array.isArray(candidate) ? (candidate as IntegrationAccount[]) : [];
  };

  const mergeFetchedRows = (incomingRows: IntegrationAccount[], requestedPage: number) => {
  ...
  const [deleteIntegrationAccountCascade] = useDeleteIntegrationAccountCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<IntegrationAccount | null> => {
    ...
    try {
      ...
      return (await response.json()) as IntegrationAccount;
    } catch (err) {
      console.warn('Could not reload IntegrationAccount row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<IntegrationAccount>,
    successField?: string,
  ) => {
    try {
      const saved = await updateIntegrationAccount(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as IntegrationAccount[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update IntegrationAccount';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteIntegrationAccountCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `IntegrationAccount-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addIntegrationAccount(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateIntegrationAccount(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as IntegrationAccount[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as IntegrationAccount[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as IntegrationAccount[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as IntegrationAccount[]);
    }
  };
