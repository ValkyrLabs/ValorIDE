...
import ChatCompletionRequestForm from '@thorapi/redux/components/form/ChatCompletionRequestForm';
...
} from '@thorapi/redux/services/ChatCompletionRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ChatCompletionRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ChatCompletionRequest/${rowId}`;
...
const ChatCompletionRequestTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ChatCompletionRequest[] => {
    if (Array.isArray(result)) return result as ChatCompletionRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as ChatCompletionRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ChatCompletionRequest[], requestedPage: number) => {
  ...
  const [deleteChatCompletionRequestCascade] = useDeleteChatCompletionRequestCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ChatCompletionRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as ChatCompletionRequest;
    } catch (err) {
      console.warn('Could not reload ChatCompletionRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ChatCompletionRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateChatCompletionRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ChatCompletionRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ChatCompletionRequest';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteChatCompletionRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ChatCompletionRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addChatCompletionRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateChatCompletionRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ChatCompletionRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ChatCompletionRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ChatCompletionRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ChatCompletionRequest[]);
    }
  };
