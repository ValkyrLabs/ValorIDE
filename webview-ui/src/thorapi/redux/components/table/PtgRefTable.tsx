...
import { PtgRef } from '@thorapi/model';
import PtgRefForm from '@thorapi/redux/components/form/PtgRefForm';
...
} from '@thorapi/redux/services/PtgRefService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `PtgRef update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/PtgRef/${rowId}`;
...
const PtgRefTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): PtgRef[] => {
    if (Array.isArray(result)) return result as PtgRef[];
    ...
    return Array.isArray(candidate) ? (candidate as PtgRef[]) : [];
  };

  const mergeFetchedRows = (incomingRows: PtgRef[], requestedPage: number) => {
  ...
  const [deletePtgRefCascade] = useDeletePtgRefCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<PtgRef | null> => {
    ...
    try {
      ...
      return (await response.json()) as PtgRef;
    } catch (err) {
      console.warn('Could not reload PtgRef row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<PtgRef>,
    successField?: string,
  ) => {
    try {
      const saved = await updatePtgRef(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as PtgRef[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update PtgRef';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePtgRefCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `PtgRef-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addPtgRef(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updatePtgRef(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as PtgRef[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as PtgRef[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as PtgRef[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as PtgRef[]);
    }
  };
