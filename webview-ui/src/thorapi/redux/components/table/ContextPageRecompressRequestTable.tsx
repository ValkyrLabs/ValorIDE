...
import { ContextPageRecompressRequest } from '@thorapi/model';
import ContextPageRecompressRequestForm from '@thorapi/redux/components/form/ContextPageRecompressRequestForm';
...
import {
  useLazyGetContextPageRecompressRequestsPagedQuery,
...
} from '@thorapi/redux/services/ContextPageRecompressRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ContextPageRecompressRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ContextPageRecompressRequest/${rowId}`;

const ContextPageRecompressRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ContextPageRecompressRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): ContextPageRecompressRequest[] => {
    if (Array.isArray(result)) return result as ContextPageRecompressRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as ContextPageRecompressRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ContextPageRecompressRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addContextPageRecompressRequest] = useAddContextPageRecompressRequestMutation();
  const [updateContextPageRecompressRequest] = useUpdateContextPageRecompressRequestMutation();
  const [deleteContextPageRecompressRequest] = useDeleteContextPageRecompressRequestMutation();
  const [deleteContextPageRecompressRequestCascade] = useDeleteContextPageRecompressRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ContextPageRecompressRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ContextPageRecompressRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ContextPageRecompressRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as ContextPageRecompressRequest;
    } catch (err) {
      console.warn('Could not reload ContextPageRecompressRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ContextPageRecompressRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateContextPageRecompressRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ContextPageRecompressRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ContextPageRecompressRequest';
      console.error('Failed to update ContextPageRecompressRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteContextPageRecompressRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteContextPageRecompressRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ContextPageRecompressRequest> = { ...source };
      delete (payload as any).id;
      const created = await addContextPageRecompressRequest(payload).unwrap();
      setAllData((prev) => [created as ContextPageRecompressRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add ContextPageRecompressRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ContextPageRecompressRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addContextPageRecompressRequest(payload).unwrap();
      setAllData((prev) => [created as ContextPageRecompressRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ContextPageRecompressRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ContextPageRecompressRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addContextPageRecompressRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateContextPageRecompressRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ContextPageRecompressRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ContextPageRecompressRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ContextPageRecompressRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ContextPageRecompressRequest[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.ContextPageRecompressRequest',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ContextPageRecompressRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ContextPageRecompressRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ContextPageRecompressRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ContextPageRecompressRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ContextPageRecompressRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ContextPageRecompressRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ContextPageRecompressRequest;
                ...
                await updateContextPageRecompressRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ContextPageRecompressRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ContextPageRecompressRequest'}
...
export default ContextPageRecompressRequestTable;
