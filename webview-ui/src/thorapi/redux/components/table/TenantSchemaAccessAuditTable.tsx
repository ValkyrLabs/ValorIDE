...
import { TenantSchemaAccessAudit } from '@thorapi/model';
import TenantSchemaAccessAuditForm from '@thorapi/redux/components/form/TenantSchemaAccessAuditForm';
...
import {
  useLazyGetTenantSchemaAccessAuditsPagedQuery,
...
} from '@thorapi/redux/services/TenantSchemaAccessAuditService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'action': { type: 'enum', enumValues: [
    'read', 'search', 'export', 'update', 'delete', 'validate', 'impersonation_denied', 
  ], enumValueType: 'string' },
  'result': { type: 'enum', enumValues: [
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TenantSchemaAccessAudit update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TenantSchemaAccessAudit/${rowId}`;

const TenantSchemaAccessAuditTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TenantSchemaAccessAudit[]>([]);
  ...
  const normalizePageRows = (result: unknown): TenantSchemaAccessAudit[] => {
    if (Array.isArray(result)) return result as TenantSchemaAccessAudit[];
    ...
    return Array.isArray(candidate) ? (candidate as TenantSchemaAccessAudit[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TenantSchemaAccessAudit[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addTenantSchemaAccessAudit] = useAddTenantSchemaAccessAuditMutation();
  const [updateTenantSchemaAccessAudit] = useUpdateTenantSchemaAccessAuditMutation();
  const [deleteTenantSchemaAccessAudit] = useDeleteTenantSchemaAccessAuditMutation();
  const [deleteTenantSchemaAccessAuditCascade] = useDeleteTenantSchemaAccessAuditCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TenantSchemaAccessAudit>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TenantSchemaAccessAudit | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TenantSchemaAccessAudit | null> => {
    ...
    try {
      ...
      return (await response.json()) as TenantSchemaAccessAudit;
    } catch (err) {
      console.warn('Could not reload TenantSchemaAccessAudit row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TenantSchemaAccessAudit>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTenantSchemaAccessAudit(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TenantSchemaAccessAudit[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TenantSchemaAccessAudit';
      console.error('Failed to update TenantSchemaAccessAudit:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTenantSchemaAccessAuditCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTenantSchemaAccessAudit(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TenantSchemaAccessAudit> = { ...source };
      delete (payload as any).id;
      const created = await addTenantSchemaAccessAudit(payload).unwrap();
      setAllData((prev) => [created as TenantSchemaAccessAudit, ...prev]);
    } catch (error) {
      console.error('Failed to add TenantSchemaAccessAudit:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TenantSchemaAccessAudit> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTenantSchemaAccessAudit(payload).unwrap();
      setAllData((prev) => [created as TenantSchemaAccessAudit, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TenantSchemaAccessAudit:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TenantSchemaAccessAudit-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTenantSchemaAccessAudit(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTenantSchemaAccessAudit(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TenantSchemaAccessAudit[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TenantSchemaAccessAudit[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TenantSchemaAccessAudit[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TenantSchemaAccessAudit[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="TenantSchemaAccessAuditTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="TenantSchemaAccessAudit Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TenantSchemaAccessAudit</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TenantSchemaAccessAuditForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TenantSchemaAccessAudit (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TenantSchemaAccessAudit</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TenantSchemaAccessAudit;
                ...
                await updateTenantSchemaAccessAudit(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TenantSchemaAccessAudits.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TenantSchemaAccessAudit'}
...
export default TenantSchemaAccessAuditTable;
