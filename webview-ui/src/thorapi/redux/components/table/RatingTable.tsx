...
import { Rating } from '@thorapi/model';
import RatingForm from '@thorapi/redux/components/form/RatingForm';
...
} from '@thorapi/redux/services/RatingService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'targetType': { type: 'enum', enumValues: [
    'homepage', 'webpage', 'application', 'function', 'api', 'service', 'content', 'schema', 'feature', 'helpfulness', 'satisfaction', 'product', 'workflow', 'workflow_run', 'chat_message', 'agent_task', 'generated_code', 'skill_artifact', 'memory_entry', 'none', 
  ], enumValueType: 'string' },
  'rating': { type: 'number' },
  'raterType': { type: 'enum', enumValues: [
    'user', 'agent', 'system', 'evaluator', 'test', 'none', 
  ], enumValueType: 'string' },
  'ratingType': { type: 'enum', enumValues: [
    'user_feedback', 'agent_observation', 'error_report', 'validation_result', 'quality_score', 'satisfaction_score', 'helpfulness_score', 'none', 
  ], enumValueType: 'string' },
  'status': { type: 'enum', enumValues: [
    'new', 'analyzed', 'action_proposed', 'action_applied', 'ignored', 'archived', 
  ], enumValueType: 'string' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Rating update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Rating/${rowId}`;
...
const RatingTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Rating[] => {
    if (Array.isArray(result)) return result as Rating[];
    ...
    return Array.isArray(candidate) ? (candidate as Rating[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Rating[], requestedPage: number) => {
  ...
  const [deleteRatingCascade] = useDeleteRatingCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Rating | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Rating row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Rating>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRating(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Rating[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Rating';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRatingCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Rating-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRating(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Rating[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Rating[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Rating[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Rating[]);
    }
  };
