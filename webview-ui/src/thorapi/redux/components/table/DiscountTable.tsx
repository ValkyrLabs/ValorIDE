...
import { Discount } from '@thorapi/model';
import DiscountForm from '@thorapi/redux/components/form/DiscountForm';
...
} from '@thorapi/redux/services/DiscountService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Discount update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Discount/${rowId}`;
...
const DiscountTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Discount[] => {
    if (Array.isArray(result)) return result as Discount[];
    ...
    return Array.isArray(candidate) ? (candidate as Discount[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Discount[], requestedPage: number) => {
  ...
  const [deleteDiscountCascade] = useDeleteDiscountCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Discount | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Discount row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Discount>,
    successField?: string,
  ) => {
    try {
      const saved = await updateDiscount(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Discount[]);
      if (successField) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteDiscountCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Discount-export-${stamp}.json`;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Discount[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Discount[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Discount[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Discount[]);
    }
  };
