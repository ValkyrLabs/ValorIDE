...
import BuildArtifactForm from '@thorapi/redux/components/form/BuildArtifactForm';
...
import {
  useLazyGetBuildArtifactsPagedQuery,
...
} from '@thorapi/redux/services/BuildArtifactService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'jarSize': { type: 'number' },
  'frontendSize': { type: 'number' },
  'buildTimestamp': { type: 'datetime' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `BuildArtifact update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/BuildArtifact/${rowId}`;

const BuildArtifactTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<BuildArtifact[]>([]);
  ...
  const normalizePageRows = (result: unknown): BuildArtifact[] => {
    if (Array.isArray(result)) return result as BuildArtifact[];
    ...
    return Array.isArray(candidate) ? (candidate as BuildArtifact[]) : [];
  };

  const mergeFetchedRows = (incomingRows: BuildArtifact[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addBuildArtifact] = useAddBuildArtifactMutation();
  const [updateBuildArtifact] = useUpdateBuildArtifactMutation();
  const [deleteBuildArtifact] = useDeleteBuildArtifactMutation();
  const [deleteBuildArtifactCascade] = useDeleteBuildArtifactCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<BuildArtifact>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<BuildArtifact | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<BuildArtifact | null> => {
    ...
    try {
      ...
      return (await response.json()) as BuildArtifact;
    } catch (err) {
      console.warn('Could not reload BuildArtifact row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<BuildArtifact>,
    successField?: string,
  ) => {
    try {
      const saved = await updateBuildArtifact(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as BuildArtifact[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update BuildArtifact';
      console.error('Failed to update BuildArtifact:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteBuildArtifactCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteBuildArtifact(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<BuildArtifact> = { ...source };
      delete (payload as any).id;
      const created = await addBuildArtifact(payload).unwrap();
      setAllData((prev) => [created as BuildArtifact, ...prev]);
    } catch (error) {
      console.error('Failed to add BuildArtifact:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<BuildArtifact> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addBuildArtifact(payload).unwrap();
      setAllData((prev) => [created as BuildArtifact, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) BuildArtifact:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `BuildArtifact-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addBuildArtifact(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateBuildArtifact(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as BuildArtifact[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as BuildArtifact[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as BuildArtifact[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as BuildArtifact[]);
    }
  };
    ...
    <>
          ...
          >
            ...
            <FloatingControlPanel
              description="BuildArtifact Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create BuildArtifact</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <BuildArtifactForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter BuildArtifact (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing BuildArtifact</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof BuildArtifact;
                ...
                await updateBuildArtifact(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of BuildArtifacts.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'BuildArtifact'}
...
export default BuildArtifactTable;
