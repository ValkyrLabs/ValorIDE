...
import { PublishMcpToolResponse } from '@thorapi/model';
import PublishMcpToolResponseForm from '@thorapi/redux/components/form/PublishMcpToolResponseForm';
...
import {
  useLazyGetPublishMcpToolResponsesPagedQuery,
  useAddPublishMcpToolResponseMutation,
  useUpdatePublishMcpToolResponseMutation,
  useDeletePublishMcpToolResponseMutation,
  useDeletePublishMcpToolResponseCascadeMutation
} from '@thorapi/redux/services/PublishMcpToolResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `PublishMcpToolResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/PublishMcpToolResponse/${rowId}`;

const PublishMcpToolResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<PublishMcpToolResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): PublishMcpToolResponse[] => {
    if (Array.isArray(result)) return result as PublishMcpToolResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as PublishMcpToolResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: PublishMcpToolResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addPublishMcpToolResponse] = useAddPublishMcpToolResponseMutation();
  const [updatePublishMcpToolResponse] = useUpdatePublishMcpToolResponseMutation();
  const [deletePublishMcpToolResponse] = useDeletePublishMcpToolResponseMutation();
  const [deletePublishMcpToolResponseCascade] = useDeletePublishMcpToolResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<PublishMcpToolResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<PublishMcpToolResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<PublishMcpToolResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as PublishMcpToolResponse;
    } catch (err) {
      console.warn('Could not reload PublishMcpToolResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<PublishMcpToolResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updatePublishMcpToolResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as PublishMcpToolResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update PublishMcpToolResponse';
      console.error('Failed to update PublishMcpToolResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePublishMcpToolResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deletePublishMcpToolResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<PublishMcpToolResponse> = { ...source };
      delete (payload as any).id;
      const created = await addPublishMcpToolResponse(payload).unwrap();
      setAllData((prev) => [created as PublishMcpToolResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add PublishMcpToolResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<PublishMcpToolResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addPublishMcpToolResponse(payload).unwrap();
      setAllData((prev) => [created as PublishMcpToolResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) PublishMcpToolResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `PublishMcpToolResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addPublishMcpToolResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updatePublishMcpToolResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as PublishMcpToolResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as PublishMcpToolResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as PublishMcpToolResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as PublishMcpToolResponse[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="PublishMcpToolResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="PublishMcpToolResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create PublishMcpToolResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <PublishMcpToolResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter PublishMcpToolResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing PublishMcpToolResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof PublishMcpToolResponse;
                ...
                await updatePublishMcpToolResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of PublishMcpToolResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'PublishMcpToolResponse'}
...
export default PublishMcpToolResponseTable;
