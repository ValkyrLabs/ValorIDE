...
import { ReferralLink } from '@thorapi/model';
import ReferralLinkForm from '@thorapi/redux/components/form/ReferralLinkForm';
...
} from '@thorapi/redux/services/ReferralLinkService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ReferralLink update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ReferralLink/${rowId}`;
...
const ReferralLinkTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ReferralLink[] => {
    if (Array.isArray(result)) return result as ReferralLink[];
    ...
    return Array.isArray(candidate) ? (candidate as ReferralLink[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ReferralLink[], requestedPage: number) => {
  ...
  const [deleteReferralLinkCascade] = useDeleteReferralLinkCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ReferralLink | null> => {
    ...
    try {
      ...
      console.warn('Could not reload ReferralLink row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ReferralLink>,
    successField?: string,
  ) => {
    try {
      const saved = await updateReferralLink(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ReferralLink[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ReferralLink';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteReferralLinkCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ReferralLink-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addReferralLink(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateReferralLink(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ReferralLink[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ReferralLink[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ReferralLink[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ReferralLink[]);
    }
  };
