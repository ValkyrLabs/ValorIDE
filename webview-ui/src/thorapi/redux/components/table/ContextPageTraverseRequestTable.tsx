...
import { ContextPageTraverseRequest } from '@thorapi/model';
import ContextPageTraverseRequestForm from '@thorapi/redux/components/form/ContextPageTraverseRequestForm';
...
import {
  useLazyGetContextPageTraverseRequestsPagedQuery,
...
} from '@thorapi/redux/services/ContextPageTraverseRequestService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'traversalType': { type: 'enum', enumValues: [
    'intent_shift', 'app_spec_to_schema', 'schema_to_generation', 'billing_to_entitlement', 'hydrate_source', 'recompress_after_use', 'user_clarification', 'failure_analysis', 'other', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ContextPageTraverseRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ContextPageTraverseRequest/${rowId}`;

const ContextPageTraverseRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ContextPageTraverseRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): ContextPageTraverseRequest[] => {
    if (Array.isArray(result)) return result as ContextPageTraverseRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as ContextPageTraverseRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ContextPageTraverseRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addContextPageTraverseRequest] = useAddContextPageTraverseRequestMutation();
  const [updateContextPageTraverseRequest] = useUpdateContextPageTraverseRequestMutation();
  const [deleteContextPageTraverseRequest] = useDeleteContextPageTraverseRequestMutation();
  const [deleteContextPageTraverseRequestCascade] = useDeleteContextPageTraverseRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ContextPageTraverseRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ContextPageTraverseRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ContextPageTraverseRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as ContextPageTraverseRequest;
    } catch (err) {
      console.warn('Could not reload ContextPageTraverseRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ContextPageTraverseRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateContextPageTraverseRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ContextPageTraverseRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ContextPageTraverseRequest';
      console.error('Failed to update ContextPageTraverseRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteContextPageTraverseRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteContextPageTraverseRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ContextPageTraverseRequest> = { ...source };
      delete (payload as any).id;
      const created = await addContextPageTraverseRequest(payload).unwrap();
      setAllData((prev) => [created as ContextPageTraverseRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add ContextPageTraverseRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ContextPageTraverseRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addContextPageTraverseRequest(payload).unwrap();
      setAllData((prev) => [created as ContextPageTraverseRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ContextPageTraverseRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ContextPageTraverseRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addContextPageTraverseRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateContextPageTraverseRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ContextPageTraverseRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ContextPageTraverseRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ContextPageTraverseRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ContextPageTraverseRequest[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.ContextPageTraverseRequest',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ContextPageTraverseRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ContextPageTraverseRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ContextPageTraverseRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ContextPageTraverseRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ContextPageTraverseRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ContextPageTraverseRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ContextPageTraverseRequest;
                ...
                await updateContextPageTraverseRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ContextPageTraverseRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ContextPageTraverseRequest'}
...
export default ContextPageTraverseRequestTable;
