...
import { Quota } from '@thorapi/model';
import QuotaForm from '@thorapi/redux/components/form/QuotaForm';
...
} from '@thorapi/redux/services/QuotaService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Quota update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Quota/${rowId}`;
...
const QuotaTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Quota[] => {
    if (Array.isArray(result)) return result as Quota[];
    ...
    return Array.isArray(candidate) ? (candidate as Quota[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Quota[], requestedPage: number) => {
  ...
  const [deleteQuotaCascade] = useDeleteQuotaCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Quota | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Quota row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Quota>,
    successField?: string,
  ) => {
    try {
      const saved = await updateQuota(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Quota[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Quota';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteQuotaCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Quota-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        try {
          ...
          try {
            await updateQuota(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Quota[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Quota[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Quota[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Quota[]);
    }
  };
