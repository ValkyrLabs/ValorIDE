...
import { ExecModuleIOConfig } from '@thorapi/model';
import ExecModuleIOConfigForm from '@thorapi/redux/components/form/ExecModuleIOConfigForm';
...
import {
  useLazyGetExecModuleIOConfigsPagedQuery,
...
} from '@thorapi/redux/services/ExecModuleIOConfigService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ExecModuleIOConfig update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ExecModuleIOConfig/${rowId}`;

const ExecModuleIOConfigTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ExecModuleIOConfig[]>([]);
  ...
  const normalizePageRows = (result: unknown): ExecModuleIOConfig[] => {
    if (Array.isArray(result)) return result as ExecModuleIOConfig[];
    ...
    return Array.isArray(candidate) ? (candidate as ExecModuleIOConfig[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ExecModuleIOConfig[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addExecModuleIOConfig] = useAddExecModuleIOConfigMutation();
  const [updateExecModuleIOConfig] = useUpdateExecModuleIOConfigMutation();
  const [deleteExecModuleIOConfig] = useDeleteExecModuleIOConfigMutation();
  const [deleteExecModuleIOConfigCascade] = useDeleteExecModuleIOConfigCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ExecModuleIOConfig>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ExecModuleIOConfig | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ExecModuleIOConfig | null> => {
    ...
    try {
      ...
      return (await response.json()) as ExecModuleIOConfig;
    } catch (err) {
      console.warn('Could not reload ExecModuleIOConfig row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ExecModuleIOConfig>,
    successField?: string,
  ) => {
    try {
      const saved = await updateExecModuleIOConfig(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ExecModuleIOConfig[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ExecModuleIOConfig';
      console.error('Failed to update ExecModuleIOConfig:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecModuleIOConfigCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteExecModuleIOConfig(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ExecModuleIOConfig> = { ...source };
      delete (payload as any).id;
      const created = await addExecModuleIOConfig(payload).unwrap();
      setAllData((prev) => [created as ExecModuleIOConfig, ...prev]);
    } catch (error) {
      console.error('Failed to add ExecModuleIOConfig:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ExecModuleIOConfig> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addExecModuleIOConfig(payload).unwrap();
      setAllData((prev) => [created as ExecModuleIOConfig, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ExecModuleIOConfig:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ExecModuleIOConfig-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addExecModuleIOConfig(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateExecModuleIOConfig(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ExecModuleIOConfig[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ExecModuleIOConfig[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ExecModuleIOConfig[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ExecModuleIOConfig[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ExecModuleIOConfigTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ExecModuleIOConfig Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ExecModuleIOConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ExecModuleIOConfigForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ExecModuleIOConfig (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ExecModuleIOConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ExecModuleIOConfig;
                ...
                await updateExecModuleIOConfig(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ExecModuleIOConfigs.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ExecModuleIOConfig'}
...
export default ExecModuleIOConfigTable;
