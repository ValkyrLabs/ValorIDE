...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ExecModuleIOConfig/${rowId}`;
...
const ExecModuleIOConfigTable: React.FC = () => {
  ...
  const [deleteExecModuleIOConfigCascade] =
    useDeleteExecModuleIOConfigCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<ExecModuleIOConfig | null> => {
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ExecModuleIOConfig";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecModuleIOConfigCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter ExecModuleIOConfig (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
