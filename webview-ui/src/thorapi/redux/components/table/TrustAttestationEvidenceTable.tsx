...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  evidenceStatus: {
    type: "enum",
    enumValues: ["ACCEPTED", "REJECTED", "EXPIRED"],
    enumValueType: "string",
  },
...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/TrustAttestationEvidence/${rowId}`;
...
const TrustAttestationEvidenceTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addTrustAttestationEvidence] =
    useAddTrustAttestationEvidenceMutation();
  const [updateTrustAttestationEvidence] =
    useUpdateTrustAttestationEvidenceMutation();
  const [deleteTrustAttestationEvidence] =
    useDeleteTrustAttestationEvidenceMutation();
  const [deleteTrustAttestationEvidenceCascade] =
    useDeleteTrustAttestationEvidenceCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<TrustAttestationEvidence | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<TrustAttestationEvidence | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateTrustAttestationEvidence(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update TrustAttestationEvidence";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustAttestationEvidenceCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) TrustAttestationEvidence:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter TrustAttestationEvidence (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
