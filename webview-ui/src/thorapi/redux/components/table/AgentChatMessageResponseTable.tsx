...
import { AgentChatMessageResponse } from '@thorapi/model';
import AgentChatMessageResponseForm from '@thorapi/redux/components/form/AgentChatMessageResponseForm';
...
} from '@thorapi/redux/services/AgentChatMessageResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AgentChatMessageResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AgentChatMessageResponse/${rowId}`;
...
const AgentChatMessageResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): AgentChatMessageResponse[] => {
    if (Array.isArray(result)) return result as AgentChatMessageResponse[];
  ...
  const mergeFetchedRows = (incomingRows: AgentChatMessageResponse[], requestedPage: number) => {
  ...
  const [deleteAgentChatMessageResponseCascade] = useDeleteAgentChatMessageResponseCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<AgentChatMessageResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<AgentChatMessageResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as AgentChatMessageResponse;
    } catch (err) {
      console.warn('Could not reload AgentChatMessageResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AgentChatMessageResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAgentChatMessageResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AgentChatMessageResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AgentChatMessageResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAgentChatMessageResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AgentChatMessageResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAgentChatMessageResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAgentChatMessageResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AgentChatMessageResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AgentChatMessageResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AgentChatMessageResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AgentChatMessageResponse[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter AgentChatMessageResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
