...
import ExecModulePayloadConfigForm from '@thorapi/redux/components/form/ExecModulePayloadConfigForm';
...
import {
  useLazyGetExecModulePayloadConfigsPagedQuery,
...
} from '@thorapi/redux/services/ExecModulePayloadConfigService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'actionSize': { type: 'number' },
  'activate': { type: 'boolean' },
  'allowCreate': { type: 'boolean' },
  'allowUpdate': { type: 'boolean' },
  'augmentData': { type: 'boolean' },
  'bidirectional': { type: 'boolean' },
  'd': { type: 'number' },
  'dataAugmentation': { type: 'boolean' },
  'dateOffsetDays': { type: 'number' },
  'earlyStoppingPatience': { type: 'number' },
  'embeddingDim': { type: 'number' },
  'episodes': { type: 'number' },
  'epochs': { type: 'number' },
  'flattenNested': { type: 'boolean' },
  'frozenLayers': { type: 'number' },
  'includeIntercept': { type: 'boolean' },
  'inferTypes': { type: 'boolean' },
  'introspection': { type: 'boolean' },
  'latentDim': { type: 'number' },
  'lstmUnits': { type: 'number' },
  'maxLength': { type: 'number' },
  'memorySize': { type: 'number' },
  'minWordFreq': { type: 'number' },
  'numLayers': { type: 'number' },
  'numUnits': { type: 'number' },
  'p': { type: 'number' },
  'polynomialDegree': { type: 'number' },
  'published': { type: 'boolean' },
  'q': { type: 'number' },
  'requireHashMatch': { type: 'boolean' },
  'robust': { type: 'boolean' },
  'stateSize': { type: 'number' },
  'stripNamespaces': { type: 'boolean' },
  'transaction': { type: 'boolean' },
  'transferLearning': { type: 'boolean' },
  'useAttention': { type: 'boolean' },
  'useVariational': { type: 'boolean' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ExecModulePayloadConfig update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ExecModulePayloadConfig/${rowId}`;

const ExecModulePayloadConfigTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ExecModulePayloadConfig[]>([]);
  ...
  const normalizePageRows = (result: unknown): ExecModulePayloadConfig[] => {
    if (Array.isArray(result)) return result as ExecModulePayloadConfig[];
    ...
    return Array.isArray(candidate) ? (candidate as ExecModulePayloadConfig[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ExecModulePayloadConfig[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addExecModulePayloadConfig] = useAddExecModulePayloadConfigMutation();
  const [updateExecModulePayloadConfig] = useUpdateExecModulePayloadConfigMutation();
  const [deleteExecModulePayloadConfig] = useDeleteExecModulePayloadConfigMutation();
  const [deleteExecModulePayloadConfigCascade] = useDeleteExecModulePayloadConfigCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ExecModulePayloadConfig>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ExecModulePayloadConfig | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ExecModulePayloadConfig | null> => {
    ...
    try {
      ...
      return (await response.json()) as ExecModulePayloadConfig;
    } catch (err) {
      console.warn('Could not reload ExecModulePayloadConfig row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ExecModulePayloadConfig>,
    successField?: string,
  ) => {
    try {
      const saved = await updateExecModulePayloadConfig(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ExecModulePayloadConfig[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ExecModulePayloadConfig';
      console.error('Failed to update ExecModulePayloadConfig:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecModulePayloadConfigCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteExecModulePayloadConfig(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ExecModulePayloadConfig> = { ...source };
      delete (payload as any).id;
      const created = await addExecModulePayloadConfig(payload).unwrap();
      setAllData((prev) => [created as ExecModulePayloadConfig, ...prev]);
    } catch (error) {
      console.error('Failed to add ExecModulePayloadConfig:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ExecModulePayloadConfig> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addExecModulePayloadConfig(payload).unwrap();
      setAllData((prev) => [created as ExecModulePayloadConfig, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ExecModulePayloadConfig:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ExecModulePayloadConfig-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addExecModulePayloadConfig(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateExecModulePayloadConfig(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ExecModulePayloadConfig[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ExecModulePayloadConfig[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ExecModulePayloadConfig[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ExecModulePayloadConfig[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ExecModulePayloadConfigTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ExecModulePayloadConfig Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ExecModulePayloadConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ExecModulePayloadConfigForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ExecModulePayloadConfig (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ExecModulePayloadConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ExecModulePayloadConfig;
                ...
                await updateExecModulePayloadConfig(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ExecModulePayloadConfigs.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ExecModulePayloadConfig'}
...
export default ExecModulePayloadConfigTable;
