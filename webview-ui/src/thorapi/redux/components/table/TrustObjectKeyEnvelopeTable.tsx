...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TrustObjectKeyEnvelope update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};
...
const TrustObjectKeyEnvelopeTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): TrustObjectKeyEnvelope[] => {
    if (Array.isArray(result)) return result as TrustObjectKeyEnvelope[];
    ...
    return Array.isArray(candidate) ? (candidate as TrustObjectKeyEnvelope[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TrustObjectKeyEnvelope[], requestedPage: number) => {
...
// AddressToJSON(value?: Address)
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TrustObjectKeyEnvelope[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TrustObjectKeyEnvelope[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TrustObjectKeyEnvelope[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TrustObjectKeyEnvelope[]);
    }
  };
