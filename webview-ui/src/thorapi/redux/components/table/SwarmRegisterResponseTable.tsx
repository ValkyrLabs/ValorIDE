...
import { SwarmRegisterResponse } from '@thorapi/model';
import SwarmRegisterResponseForm from '@thorapi/redux/components/form/SwarmRegisterResponseForm';
...
} from '@thorapi/redux/services/SwarmRegisterResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SwarmRegisterResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SwarmRegisterResponse/${rowId}`;
...
const SwarmRegisterResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SwarmRegisterResponse[] => {
    if (Array.isArray(result)) return result as SwarmRegisterResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as SwarmRegisterResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SwarmRegisterResponse[], requestedPage: number) => {
  ...
  const [deleteSwarmRegisterResponseCascade] = useDeleteSwarmRegisterResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SwarmRegisterResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as SwarmRegisterResponse;
    } catch (err) {
      console.warn('Could not reload SwarmRegisterResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SwarmRegisterResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSwarmRegisterResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SwarmRegisterResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SwarmRegisterResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmRegisterResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SwarmRegisterResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSwarmRegisterResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSwarmRegisterResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SwarmRegisterResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SwarmRegisterResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SwarmRegisterResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SwarmRegisterResponse[]);
    }
  };
