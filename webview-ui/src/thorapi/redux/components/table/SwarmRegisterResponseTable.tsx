...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/SwarmRegisterResponse/${rowId}`;
...
const SwarmRegisterResponseTable: React.FC = () => {
  ...
  const [deleteSwarmRegisterResponse] =
    useDeleteSwarmRegisterResponseMutation();
  const [deleteSwarmRegisterResponseCascade] =
    useDeleteSwarmRegisterResponseCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<SwarmRegisterResponse | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateSwarmRegisterResponse(
        updatedItem as any,
      ).unwrap();
