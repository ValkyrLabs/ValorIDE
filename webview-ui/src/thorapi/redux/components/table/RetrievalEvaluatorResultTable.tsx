...
import { RetrievalEvaluatorResult } from '@thorapi/model';
import RetrievalEvaluatorResultForm from '@thorapi/redux/components/form/RetrievalEvaluatorResultForm';
...
import {
  useLazyGetRetrievalEvaluatorResultsPagedQuery,
...
} from '@thorapi/redux/services/RetrievalEvaluatorResultService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RetrievalEvaluatorResult update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RetrievalEvaluatorResult/${rowId}`;

const RetrievalEvaluatorResultTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RetrievalEvaluatorResult[]>([]);
  ...
  const normalizePageRows = (result: unknown): RetrievalEvaluatorResult[] => {
    if (Array.isArray(result)) return result as RetrievalEvaluatorResult[];
    ...
    return Array.isArray(candidate) ? (candidate as RetrievalEvaluatorResult[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RetrievalEvaluatorResult[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addRetrievalEvaluatorResult] = useAddRetrievalEvaluatorResultMutation();
  const [updateRetrievalEvaluatorResult] = useUpdateRetrievalEvaluatorResultMutation();
  const [deleteRetrievalEvaluatorResult] = useDeleteRetrievalEvaluatorResultMutation();
  const [deleteRetrievalEvaluatorResultCascade] = useDeleteRetrievalEvaluatorResultCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<RetrievalEvaluatorResult>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RetrievalEvaluatorResult | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<RetrievalEvaluatorResult | null> => {
    ...
    try {
      ...
      return (await response.json()) as RetrievalEvaluatorResult;
    } catch (err) {
      console.warn('Could not reload RetrievalEvaluatorResult row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RetrievalEvaluatorResult>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRetrievalEvaluatorResult(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RetrievalEvaluatorResult[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RetrievalEvaluatorResult';
      console.error('Failed to update RetrievalEvaluatorResult:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRetrievalEvaluatorResultCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteRetrievalEvaluatorResult(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RetrievalEvaluatorResult> = { ...source };
      delete (payload as any).id;
      const created = await addRetrievalEvaluatorResult(payload).unwrap();
      setAllData((prev) => [created as RetrievalEvaluatorResult, ...prev]);
    } catch (error) {
      console.error('Failed to add RetrievalEvaluatorResult:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RetrievalEvaluatorResult> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRetrievalEvaluatorResult(payload).unwrap();
      setAllData((prev) => [created as RetrievalEvaluatorResult, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) RetrievalEvaluatorResult:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RetrievalEvaluatorResult-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRetrievalEvaluatorResult(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRetrievalEvaluatorResult(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RetrievalEvaluatorResult[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RetrievalEvaluatorResult[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RetrievalEvaluatorResult[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RetrievalEvaluatorResult[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="RetrievalEvaluatorResultTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="RetrievalEvaluatorResult Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RetrievalEvaluatorResult</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RetrievalEvaluatorResultForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RetrievalEvaluatorResult (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RetrievalEvaluatorResult</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RetrievalEvaluatorResult;
                ...
                await updateRetrievalEvaluatorResult(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RetrievalEvaluatorResults.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RetrievalEvaluatorResult'}
...
export default RetrievalEvaluatorResultTable;
