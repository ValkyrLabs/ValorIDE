...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/WorkflowGraphEdgeConfig/${rowId}`;
...
const WorkflowGraphEdgeConfigTable: React.FC = () => {
  ...
  const [updateWorkflowGraphEdgeConfig] =
    useUpdateWorkflowGraphEdgeConfigMutation();
  const [deleteWorkflowGraphEdgeConfig] =
    useDeleteWorkflowGraphEdgeConfigMutation();
  const [deleteWorkflowGraphEdgeConfigCascade] =
    useDeleteWorkflowGraphEdgeConfigCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<WorkflowGraphEdgeConfig | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<WorkflowGraphEdgeConfig | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateWorkflowGraphEdgeConfig(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update WorkflowGraphEdgeConfig";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphEdgeConfigCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter WorkflowGraphEdgeConfig (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
