...
import { WorkflowGraphEdgeConfig } from '@thorapi/model';
import WorkflowGraphEdgeConfigForm from '@thorapi/redux/components/form/WorkflowGraphEdgeConfigForm';
...
import {
  useLazyGetWorkflowGraphEdgeConfigsPagedQuery,
...
} from '@thorapi/redux/services/WorkflowGraphEdgeConfigService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `WorkflowGraphEdgeConfig update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/WorkflowGraphEdgeConfig/${rowId}`;

const WorkflowGraphEdgeConfigTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<WorkflowGraphEdgeConfig[]>([]);
  ...
  const normalizePageRows = (result: unknown): WorkflowGraphEdgeConfig[] => {
    if (Array.isArray(result)) return result as WorkflowGraphEdgeConfig[];
    ...
    return Array.isArray(candidate) ? (candidate as WorkflowGraphEdgeConfig[]) : [];
  };

  const mergeFetchedRows = (incomingRows: WorkflowGraphEdgeConfig[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addWorkflowGraphEdgeConfig] = useAddWorkflowGraphEdgeConfigMutation();
  const [updateWorkflowGraphEdgeConfig] = useUpdateWorkflowGraphEdgeConfigMutation();
  const [deleteWorkflowGraphEdgeConfig] = useDeleteWorkflowGraphEdgeConfigMutation();
  const [deleteWorkflowGraphEdgeConfigCascade] = useDeleteWorkflowGraphEdgeConfigCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<WorkflowGraphEdgeConfig>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<WorkflowGraphEdgeConfig | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<WorkflowGraphEdgeConfig | null> => {
    ...
    try {
      ...
      return (await response.json()) as WorkflowGraphEdgeConfig;
    } catch (err) {
      console.warn('Could not reload WorkflowGraphEdgeConfig row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<WorkflowGraphEdgeConfig>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWorkflowGraphEdgeConfig(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as WorkflowGraphEdgeConfig[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update WorkflowGraphEdgeConfig';
      console.error('Failed to update WorkflowGraphEdgeConfig:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphEdgeConfigCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteWorkflowGraphEdgeConfig(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<WorkflowGraphEdgeConfig> = { ...source };
      delete (payload as any).id;
      const created = await addWorkflowGraphEdgeConfig(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphEdgeConfig, ...prev]);
    } catch (error) {
      console.error('Failed to add WorkflowGraphEdgeConfig:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<WorkflowGraphEdgeConfig> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addWorkflowGraphEdgeConfig(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphEdgeConfig, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) WorkflowGraphEdgeConfig:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `WorkflowGraphEdgeConfig-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWorkflowGraphEdgeConfig(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWorkflowGraphEdgeConfig(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as WorkflowGraphEdgeConfig[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as WorkflowGraphEdgeConfig[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as WorkflowGraphEdgeConfig[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as WorkflowGraphEdgeConfig[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.WorkflowGraphEdgeConfig',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="WorkflowGraphEdgeConfigTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="WorkflowGraphEdgeConfig Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create WorkflowGraphEdgeConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <WorkflowGraphEdgeConfigForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter WorkflowGraphEdgeConfig (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing WorkflowGraphEdgeConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof WorkflowGraphEdgeConfig;
                ...
                await updateWorkflowGraphEdgeConfig(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of WorkflowGraphEdgeConfigs.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'WorkflowGraphEdgeConfig'}
...
export default WorkflowGraphEdgeConfigTable;
