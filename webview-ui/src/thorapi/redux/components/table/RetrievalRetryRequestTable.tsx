...
import { RetrievalRetryRequest } from '@thorapi/model';
import RetrievalRetryRequestForm from '@thorapi/redux/components/form/RetrievalRetryRequestForm';
...
import {
  useLazyGetRetrievalRetryRequestsPagedQuery,
...
} from '@thorapi/redux/services/RetrievalRetryRequestService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'retryStrategy': { type: 'enum', enumValues: [
    'SAME_QUERY', 'EXPAND_QUERY', 'SCHEMA_FILTER', 'RECENCY_BIAS', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RetrievalRetryRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RetrievalRetryRequest/${rowId}`;

const RetrievalRetryRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RetrievalRetryRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): RetrievalRetryRequest[] => {
    if (Array.isArray(result)) return result as RetrievalRetryRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as RetrievalRetryRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RetrievalRetryRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addRetrievalRetryRequest] = useAddRetrievalRetryRequestMutation();
  const [updateRetrievalRetryRequest] = useUpdateRetrievalRetryRequestMutation();
  const [deleteRetrievalRetryRequest] = useDeleteRetrievalRetryRequestMutation();
  const [deleteRetrievalRetryRequestCascade] = useDeleteRetrievalRetryRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<RetrievalRetryRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RetrievalRetryRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<RetrievalRetryRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as RetrievalRetryRequest;
    } catch (err) {
      console.warn('Could not reload RetrievalRetryRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RetrievalRetryRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRetrievalRetryRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RetrievalRetryRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RetrievalRetryRequest';
      console.error('Failed to update RetrievalRetryRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRetrievalRetryRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteRetrievalRetryRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RetrievalRetryRequest> = { ...source };
      delete (payload as any).id;
      const created = await addRetrievalRetryRequest(payload).unwrap();
      setAllData((prev) => [created as RetrievalRetryRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add RetrievalRetryRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RetrievalRetryRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRetrievalRetryRequest(payload).unwrap();
      setAllData((prev) => [created as RetrievalRetryRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) RetrievalRetryRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RetrievalRetryRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRetrievalRetryRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRetrievalRetryRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RetrievalRetryRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RetrievalRetryRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RetrievalRetryRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RetrievalRetryRequest[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="RetrievalRetryRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="RetrievalRetryRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RetrievalRetryRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RetrievalRetryRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RetrievalRetryRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RetrievalRetryRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RetrievalRetryRequest;
                ...
                await updateRetrievalRetryRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RetrievalRetryRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RetrievalRetryRequest'}
...
export default RetrievalRetryRequestTable;
