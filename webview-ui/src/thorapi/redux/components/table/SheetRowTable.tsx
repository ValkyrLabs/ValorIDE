...
import SheetRowForm from '@thorapi/redux/components/form/SheetRowForm';
...
} from '@thorapi/redux/services/SheetRowService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SheetRow update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SheetRow/${rowId}`;
...
const SheetRowTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SheetRow[] => {
    if (Array.isArray(result)) return result as SheetRow[];
    ...
    return Array.isArray(candidate) ? (candidate as SheetRow[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SheetRow[], requestedPage: number) => {
  ...
  const [deleteSheetRowCascade] = useDeleteSheetRowCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SheetRow | null> => {
    ...
    try {
      ...
      return (await response.json()) as SheetRow;
    } catch (err) {
      console.warn('Could not reload SheetRow row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SheetRow>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSheetRow(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SheetRow[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SheetRow';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSheetRowCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SheetRow-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSheetRow(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSheetRow(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SheetRow[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SheetRow[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SheetRow[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SheetRow[]);
    }
  };
