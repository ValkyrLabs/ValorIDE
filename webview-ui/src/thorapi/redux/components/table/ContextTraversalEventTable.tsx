...
import { ContextTraversalEvent } from '@thorapi/model';
import ContextTraversalEventForm from '@thorapi/redux/components/form/ContextTraversalEventForm';
...
import {
  useLazyGetContextTraversalEventsPagedQuery,
...
} from '@thorapi/redux/services/ContextTraversalEventService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ContextTraversalEvent update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ContextTraversalEvent/${rowId}`;

const ContextTraversalEventTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ContextTraversalEvent[]>([]);
  ...
  const normalizePageRows = (result: unknown): ContextTraversalEvent[] => {
    if (Array.isArray(result)) return result as ContextTraversalEvent[];
    ...
    return Array.isArray(candidate) ? (candidate as ContextTraversalEvent[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ContextTraversalEvent[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addContextTraversalEvent] = useAddContextTraversalEventMutation();
  const [updateContextTraversalEvent] = useUpdateContextTraversalEventMutation();
  const [deleteContextTraversalEvent] = useDeleteContextTraversalEventMutation();
  const [deleteContextTraversalEventCascade] = useDeleteContextTraversalEventCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ContextTraversalEvent>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ContextTraversalEvent | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ContextTraversalEvent | null> => {
    ...
    try {
      ...
      return (await response.json()) as ContextTraversalEvent;
    } catch (err) {
      console.warn('Could not reload ContextTraversalEvent row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ContextTraversalEvent>,
    successField?: string,
  ) => {
    try {
      const saved = await updateContextTraversalEvent(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ContextTraversalEvent[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ContextTraversalEvent';
      console.error('Failed to update ContextTraversalEvent:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteContextTraversalEventCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteContextTraversalEvent(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ContextTraversalEvent> = { ...source };
      delete (payload as any).id;
      const created = await addContextTraversalEvent(payload).unwrap();
      setAllData((prev) => [created as ContextTraversalEvent, ...prev]);
    } catch (error) {
      console.error('Failed to add ContextTraversalEvent:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ContextTraversalEvent> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addContextTraversalEvent(payload).unwrap();
      setAllData((prev) => [created as ContextTraversalEvent, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ContextTraversalEvent:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ContextTraversalEvent-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addContextTraversalEvent(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateContextTraversalEvent(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ContextTraversalEvent[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ContextTraversalEvent[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ContextTraversalEvent[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ContextTraversalEvent[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ContextTraversalEventTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ContextTraversalEvent Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ContextTraversalEvent</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ContextTraversalEventForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ContextTraversalEvent (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ContextTraversalEvent</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ContextTraversalEvent;
                ...
                await updateContextTraversalEvent(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ContextTraversalEvents.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ContextTraversalEvent'}
...
export default ContextTraversalEventTable;
