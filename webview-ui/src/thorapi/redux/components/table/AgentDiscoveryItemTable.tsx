...
import { AgentDiscoveryItem } from '@thorapi/model';
import AgentDiscoveryItemForm from '@thorapi/redux/components/form/AgentDiscoveryItemForm';
...
} from '@thorapi/redux/services/AgentDiscoveryItemService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AgentDiscoveryItem update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AgentDiscoveryItem/${rowId}`;
...
const AgentDiscoveryItemTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): AgentDiscoveryItem[] => {
    if (Array.isArray(result)) return result as AgentDiscoveryItem[];
    ...
    return Array.isArray(candidate) ? (candidate as AgentDiscoveryItem[]) : [];
  };

  const mergeFetchedRows = (incomingRows: AgentDiscoveryItem[], requestedPage: number) => {
  ...
  const [deleteAgentDiscoveryItemCascade] = useDeleteAgentDiscoveryItemCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<AgentDiscoveryItem | null> => {
    ...
    try {
      ...
      return (await response.json()) as AgentDiscoveryItem;
    } catch (err) {
      console.warn('Could not reload AgentDiscoveryItem row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AgentDiscoveryItem>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAgentDiscoveryItem(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AgentDiscoveryItem[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AgentDiscoveryItem';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAgentDiscoveryItemCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AgentDiscoveryItem-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAgentDiscoveryItem(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAgentDiscoveryItem(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AgentDiscoveryItem[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AgentDiscoveryItem[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AgentDiscoveryItem[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AgentDiscoveryItem[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter AgentDiscoveryItem (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
