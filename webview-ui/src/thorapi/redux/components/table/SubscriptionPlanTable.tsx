...
import { SubscriptionPlan } from '@thorapi/model';
import SubscriptionPlanForm from '@thorapi/redux/components/form/SubscriptionPlanForm';
...
import {
  useLazyGetSubscriptionPlansPagedQuery,
  useAddSubscriptionPlanMutation,
  useUpdateSubscriptionPlanMutation,
  useDeleteSubscriptionPlanMutation,
  useDeleteSubscriptionPlanCascadeMutation
} from '@thorapi/redux/services/SubscriptionPlanService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SubscriptionPlan update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SubscriptionPlan/${rowId}`;
...
const SubscriptionPlanTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<SubscriptionPlan[]>([]);
  ...
  const normalizePageRows = (result: unknown): SubscriptionPlan[] => {
    if (Array.isArray(result)) return result as SubscriptionPlan[];
    ...
    return Array.isArray(candidate) ? (candidate as SubscriptionPlan[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SubscriptionPlan[], requestedPage: number) => {
  ...
  const [deleteSubscriptionPlanCascade] = useDeleteSubscriptionPlanCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<SubscriptionPlan | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<SubscriptionPlan | null> => {
    ...
    try {
      ...
      console.warn('Could not reload SubscriptionPlan row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SubscriptionPlan>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSubscriptionPlan(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SubscriptionPlan[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SubscriptionPlan';
      console.error('Failed to update SubscriptionPlan:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSubscriptionPlanCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteSubscriptionPlan(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SubscriptionPlan> = { ...source };
      delete (payload as any).id;
      const created = await addSubscriptionPlan(payload).unwrap();
      setAllData((prev) => [created as SubscriptionPlan, ...prev]);
    } catch (error) {
      console.error('Failed to add SubscriptionPlan:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SubscriptionPlan> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSubscriptionPlan(payload).unwrap();
      setAllData((prev) => [created as SubscriptionPlan, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) SubscriptionPlan:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SubscriptionPlan-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSubscriptionPlan(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SubscriptionPlan[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SubscriptionPlan[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SubscriptionPlan[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SubscriptionPlan[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="SubscriptionPlanTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="SubscriptionPlan Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SubscriptionPlan</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SubscriptionPlan (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SubscriptionPlan</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SubscriptionPlan;
                ...
                await updateSubscriptionPlan(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SubscriptionPlans.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SubscriptionPlan'}
