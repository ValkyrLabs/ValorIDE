...
import { SwarmSecurity } from '@thorapi/model';
import SwarmSecurityForm from '@thorapi/redux/components/form/SwarmSecurityForm';
...
} from '@thorapi/redux/services/SwarmSecurityService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SwarmSecurity update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SwarmSecurity/${rowId}`;
...
const SwarmSecurityTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SwarmSecurity[] => {
    if (Array.isArray(result)) return result as SwarmSecurity[];
    ...
    return Array.isArray(candidate) ? (candidate as SwarmSecurity[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SwarmSecurity[], requestedPage: number) => {
  ...
  const [deleteSwarmSecurityCascade] = useDeleteSwarmSecurityCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SwarmSecurity | null> => {
    ...
    try {
      ...
      return (await response.json()) as SwarmSecurity;
    } catch (err) {
      console.warn('Could not reload SwarmSecurity row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SwarmSecurity>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSwarmSecurity(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SwarmSecurity[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SwarmSecurity';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmSecurityCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SwarmSecurity-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSwarmSecurity(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSwarmSecurity(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SwarmSecurity[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SwarmSecurity[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SwarmSecurity[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SwarmSecurity[]);
    }
  };
