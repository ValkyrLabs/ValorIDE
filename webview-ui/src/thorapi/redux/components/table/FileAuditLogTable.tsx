...
import { FileAuditLog } from '@thorapi/model';
import FileAuditLogForm from '@thorapi/redux/components/form/FileAuditLogForm';
...
} from '@thorapi/redux/services/FileAuditLogService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `FileAuditLog update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/FileAuditLog/${rowId}`;
...
const FileAuditLogTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): FileAuditLog[] => {
    if (Array.isArray(result)) return result as FileAuditLog[];
    ...
    return Array.isArray(candidate) ? (candidate as FileAuditLog[]) : [];
  };

  const mergeFetchedRows = (incomingRows: FileAuditLog[], requestedPage: number) => {
  ...
  const [deleteFileAuditLogCascade] = useDeleteFileAuditLogCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<FileAuditLog | null> => {
    ...
    try {
      ...
      return (await response.json()) as FileAuditLog;
    } catch (err) {
      console.warn('Could not reload FileAuditLog row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<FileAuditLog>,
    successField?: string,
  ) => {
    try {
      const saved = await updateFileAuditLog(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as FileAuditLog[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update FileAuditLog';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteFileAuditLogCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `FileAuditLog-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addFileAuditLog(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateFileAuditLog(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as FileAuditLog[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as FileAuditLog[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as FileAuditLog[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as FileAuditLog[]);
    }
  };
