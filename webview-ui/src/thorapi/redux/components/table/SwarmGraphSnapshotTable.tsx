...
import { SwarmGraphSnapshot } from '@thorapi/model';
import SwarmGraphSnapshotForm from '@thorapi/redux/components/form/SwarmGraphSnapshotForm';
...
} from '@thorapi/redux/services/SwarmGraphSnapshotService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SwarmGraphSnapshot update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SwarmGraphSnapshot/${rowId}`;
...
const SwarmGraphSnapshotTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SwarmGraphSnapshot[] => {
    if (Array.isArray(result)) return result as SwarmGraphSnapshot[];
    ...
    return Array.isArray(candidate) ? (candidate as SwarmGraphSnapshot[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SwarmGraphSnapshot[], requestedPage: number) => {
  ...
  const [deleteSwarmGraphSnapshotCascade] = useDeleteSwarmGraphSnapshotCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SwarmGraphSnapshot | null> => {
    ...
    try {
      ...
      return (await response.json()) as SwarmGraphSnapshot;
    } catch (err) {
      console.warn('Could not reload SwarmGraphSnapshot row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SwarmGraphSnapshot>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSwarmGraphSnapshot(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SwarmGraphSnapshot[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SwarmGraphSnapshot';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmGraphSnapshotCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SwarmGraphSnapshot-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSwarmGraphSnapshot(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSwarmGraphSnapshot(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SwarmGraphSnapshot[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SwarmGraphSnapshot[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SwarmGraphSnapshot[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SwarmGraphSnapshot[]);
    }
  };
