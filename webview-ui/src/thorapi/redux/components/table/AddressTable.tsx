...
import { Address } from '@thorapi/model';
import AddressForm from '@thorapi/redux/components/form/AddressForm';
...
import {
  ...
  useDeleteAddressCascadeMutation
} from '@thorapi/redux/services/AddressService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Address update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Address/${rowId}`;
...
const AddressTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Address[] => {
    if (Array.isArray(result)) return result as Address[];
    ...
    return Array.isArray(candidate) ? (candidate as Address[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Address[], requestedPage: number) => {
  ...
  const [deleteAddressCascade] = useDeleteAddressCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Address | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Address row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Address>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAddress(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Address[]);
      if (successField) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAddressCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Address-export-${stamp}.json`;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Address[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Address[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Address[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Address[]);
    }
  };
