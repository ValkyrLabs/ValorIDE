...
import { RequestPasswordResetRequest } from '@thorapi/model';
import RequestPasswordResetRequestForm from '@thorapi/redux/components/form/RequestPasswordResetRequestForm';
...
import {
  useLazyGetRequestPasswordResetRequestsPagedQuery,
...
} from '@thorapi/redux/services/RequestPasswordResetRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RequestPasswordResetRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RequestPasswordResetRequest/${rowId}`;

const RequestPasswordResetRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RequestPasswordResetRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): RequestPasswordResetRequest[] => {
    if (Array.isArray(result)) return result as RequestPasswordResetRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as RequestPasswordResetRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RequestPasswordResetRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addRequestPasswordResetRequest] = useAddRequestPasswordResetRequestMutation();
  const [updateRequestPasswordResetRequest] = useUpdateRequestPasswordResetRequestMutation();
  const [deleteRequestPasswordResetRequest] = useDeleteRequestPasswordResetRequestMutation();
  const [deleteRequestPasswordResetRequestCascade] = useDeleteRequestPasswordResetRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<RequestPasswordResetRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RequestPasswordResetRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<RequestPasswordResetRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as RequestPasswordResetRequest;
    } catch (err) {
      console.warn('Could not reload RequestPasswordResetRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RequestPasswordResetRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRequestPasswordResetRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RequestPasswordResetRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RequestPasswordResetRequest';
      console.error('Failed to update RequestPasswordResetRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRequestPasswordResetRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteRequestPasswordResetRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RequestPasswordResetRequest> = { ...source };
      delete (payload as any).id;
      const created = await addRequestPasswordResetRequest(payload).unwrap();
      setAllData((prev) => [created as RequestPasswordResetRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add RequestPasswordResetRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RequestPasswordResetRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRequestPasswordResetRequest(payload).unwrap();
      setAllData((prev) => [created as RequestPasswordResetRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) RequestPasswordResetRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RequestPasswordResetRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRequestPasswordResetRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRequestPasswordResetRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RequestPasswordResetRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RequestPasswordResetRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RequestPasswordResetRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RequestPasswordResetRequest[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="RequestPasswordResetRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="RequestPasswordResetRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RequestPasswordResetRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RequestPasswordResetRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RequestPasswordResetRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RequestPasswordResetRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RequestPasswordResetRequest;
                ...
                await updateRequestPasswordResetRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RequestPasswordResetRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RequestPasswordResetRequest'}
...
export default RequestPasswordResetRequestTable;
