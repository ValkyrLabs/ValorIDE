...
import { InitiateTwoFactorRequest } from '@thorapi/model';
import InitiateTwoFactorRequestForm from '@thorapi/redux/components/form/InitiateTwoFactorRequestForm';
...
import {
  useLazyGetInitiateTwoFactorRequestsPagedQuery,
...
} from '@thorapi/redux/services/InitiateTwoFactorRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `InitiateTwoFactorRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/InitiateTwoFactorRequest/${rowId}`;

const InitiateTwoFactorRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<InitiateTwoFactorRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): InitiateTwoFactorRequest[] => {
    if (Array.isArray(result)) return result as InitiateTwoFactorRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as InitiateTwoFactorRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: InitiateTwoFactorRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addInitiateTwoFactorRequest] = useAddInitiateTwoFactorRequestMutation();
  const [updateInitiateTwoFactorRequest] = useUpdateInitiateTwoFactorRequestMutation();
  const [deleteInitiateTwoFactorRequest] = useDeleteInitiateTwoFactorRequestMutation();
  const [deleteInitiateTwoFactorRequestCascade] = useDeleteInitiateTwoFactorRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<InitiateTwoFactorRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<InitiateTwoFactorRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<InitiateTwoFactorRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as InitiateTwoFactorRequest;
    } catch (err) {
      console.warn('Could not reload InitiateTwoFactorRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<InitiateTwoFactorRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateInitiateTwoFactorRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as InitiateTwoFactorRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update InitiateTwoFactorRequest';
      console.error('Failed to update InitiateTwoFactorRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteInitiateTwoFactorRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteInitiateTwoFactorRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<InitiateTwoFactorRequest> = { ...source };
      delete (payload as any).id;
      const created = await addInitiateTwoFactorRequest(payload).unwrap();
      setAllData((prev) => [created as InitiateTwoFactorRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add InitiateTwoFactorRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<InitiateTwoFactorRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addInitiateTwoFactorRequest(payload).unwrap();
      setAllData((prev) => [created as InitiateTwoFactorRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) InitiateTwoFactorRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `InitiateTwoFactorRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addInitiateTwoFactorRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateInitiateTwoFactorRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as InitiateTwoFactorRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as InitiateTwoFactorRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as InitiateTwoFactorRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as InitiateTwoFactorRequest[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="InitiateTwoFactorRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="InitiateTwoFactorRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create InitiateTwoFactorRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <InitiateTwoFactorRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter InitiateTwoFactorRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing InitiateTwoFactorRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof InitiateTwoFactorRequest;
                ...
                await updateInitiateTwoFactorRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of InitiateTwoFactorRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'InitiateTwoFactorRequest'}
...
export default InitiateTwoFactorRequestTable;
