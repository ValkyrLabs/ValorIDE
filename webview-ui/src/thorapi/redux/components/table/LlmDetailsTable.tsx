...
import { LlmDetails } from '@thorapi/model';
import LlmDetailsForm from '@thorapi/redux/components/form/LlmDetailsForm';
...
} from '@thorapi/redux/services/LlmDetailsService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'useGrayMatter': { type: 'boolean' },
  'role': { type: 'enum', enumValues: [
    'user', 'assistant', 
  ], enumValueType: 'string' },
  'provider': { type: 'enum', enumValues: [
    'valkyrai', 'openai', 'ollama', 'phi', 'gemma', 'mistral', 'claude', 'grok', 'gemini', 'moonshot', 'deepseek', 'other', 'chatgpt_pass_through', 
  ], enumValueType: 'string' },
  'apiType': { type: 'enum', enumValues: [
    'openai', 'ollama', 'anthropic', 'grok', 'gemini', 'other', 
  ], enumValueType: 'string' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `LlmDetails update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/LlmDetails/${rowId}`;
...
const LlmDetailsTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): LlmDetails[] => {
    if (Array.isArray(result)) return result as LlmDetails[];
    ...
    return Array.isArray(candidate) ? (candidate as LlmDetails[]) : [];
  };

  const mergeFetchedRows = (incomingRows: LlmDetails[], requestedPage: number) => {
  ...
  const [deleteLlmDetailsCascade] = useDeleteLlmDetailsCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<LlmDetails | null> => {
    ...
    try {
      ...
      return (await response.json()) as LlmDetails;
    } catch (err) {
      console.warn('Could not reload LlmDetails row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<LlmDetails>,
    successField?: string,
  ) => {
    try {
      const saved = await updateLlmDetails(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as LlmDetails[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update LlmDetails';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteLlmDetailsCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `LlmDetails-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addLlmDetails(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateLlmDetails(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as LlmDetails[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as LlmDetails[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as LlmDetails[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as LlmDetails[]);
    }
  };
