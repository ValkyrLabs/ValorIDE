...
const columnSchema: Record<string, ColumnSchema> = {
  trustStatus: {
    type: "enum",
    enumValues: ["VERIFIED", "DEGRADED", "CRITICAL", "UNCONFIGURED"],
...
const TrustTable: React.FC = () => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      ...
      setAllData(
        (prev) =>
          mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Trust[],
      );
      if (successField) {
      ...
      const errorMsg =
        error instanceof Error ? error.message : "Failed to update Trust";
