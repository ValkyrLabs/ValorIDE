...
import { Trust } from '@thorapi/model';
import TrustForm from '@thorapi/redux/components/form/TrustForm';
...
const columnSchema: Record<string, ColumnSchema> = {
  'trustStatus': { type: 'enum', enumValues: [
    'VERIFIED', 'DEGRADED', 'CRITICAL', 'UNCONFIGURED', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Trust update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Trust/${rowId}`;

const TrustTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<Trust[]>([]);
  ...
  const normalizePageRows = (result: unknown): Trust[] => {
    if (Array.isArray(result)) return result as Trust[];
    ...
    return Array.isArray(candidate) ? (candidate as Trust[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Trust[], requestedPage: number) => {
  ...
  const [updateTrust] = useUpdateTrustMutation();
  const [deleteTrust] = useDeleteTrustMutation();
  const [deleteTrustCascade] = useDeleteTrustCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<Trust>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<Trust | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<Trust | null> => {
    ...
    try {
      ...
      return (await response.json()) as Trust;
    } catch (err) {
      console.warn('Could not reload Trust row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Trust>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTrust(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Trust[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Trust';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTrust(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Trust> = { ...source };
      delete (payload as any).id;
      const created = await addTrust(payload).unwrap();
      setAllData((prev) => [created as Trust, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Trust> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTrust(payload).unwrap();
      setAllData((prev) => [created as Trust, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) Trust:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Trust-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        try {
          ...
          try {
            await updateTrust(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Trust[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Trust[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Trust[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Trust[]);
    }
  };
    ...
    <>
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create Trust</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TrustForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter Trust (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing Trust</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof Trust;
                ...
                await updateTrust(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of Trusts.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'Trust'}
