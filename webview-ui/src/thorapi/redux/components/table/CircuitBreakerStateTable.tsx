...
import { CircuitBreakerState } from '@thorapi/model';
import CircuitBreakerStateForm from '@thorapi/redux/components/form/CircuitBreakerStateForm';
...
} from '@thorapi/redux/services/CircuitBreakerStateService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `CircuitBreakerState update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/CircuitBreakerState/${rowId}`;
...
const CircuitBreakerStateTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): CircuitBreakerState[] => {
    if (Array.isArray(result)) return result as CircuitBreakerState[];
    ...
    return Array.isArray(candidate) ? (candidate as CircuitBreakerState[]) : [];
  };

  const mergeFetchedRows = (incomingRows: CircuitBreakerState[], requestedPage: number) => {
  ...
  const [deleteCircuitBreakerStateCascade] = useDeleteCircuitBreakerStateCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<CircuitBreakerState | null> => {
    ...
    try {
      ...
      console.warn('Could not reload CircuitBreakerState row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<CircuitBreakerState>,
    successField?: string,
  ) => {
    try {
      const saved = await updateCircuitBreakerState(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as CircuitBreakerState[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update CircuitBreakerState';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCircuitBreakerStateCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `CircuitBreakerState-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addCircuitBreakerState(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateCircuitBreakerState(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as CircuitBreakerState[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as CircuitBreakerState[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as CircuitBreakerState[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as CircuitBreakerState[]);
    }
  };
