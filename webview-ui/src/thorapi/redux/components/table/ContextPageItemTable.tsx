...
import { ContextPageItem } from '@thorapi/model';
import ContextPageItemForm from '@thorapi/redux/components/form/ContextPageItemForm';
...
import {
  useLazyGetContextPageItemsPagedQuery,
...
} from '@thorapi/redux/services/ContextPageItemService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'sourceType': { type: 'enum', enumValues: [
    'memory', 'semantic_index', 'retrieval_receipt', 'file', 'model', 'schema', 'receipt', 'procedure', 'workflow', 'generated_artifact', 'tenant_app', 'external_source', 'other', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ContextPageItem update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ContextPageItem/${rowId}`;

const ContextPageItemTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ContextPageItem[]>([]);
  ...
  const normalizePageRows = (result: unknown): ContextPageItem[] => {
    if (Array.isArray(result)) return result as ContextPageItem[];
    ...
    return Array.isArray(candidate) ? (candidate as ContextPageItem[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ContextPageItem[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addContextPageItem] = useAddContextPageItemMutation();
  const [updateContextPageItem] = useUpdateContextPageItemMutation();
  const [deleteContextPageItem] = useDeleteContextPageItemMutation();
  const [deleteContextPageItemCascade] = useDeleteContextPageItemCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ContextPageItem>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ContextPageItem | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ContextPageItem | null> => {
    ...
    try {
      ...
      return (await response.json()) as ContextPageItem;
    } catch (err) {
      console.warn('Could not reload ContextPageItem row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ContextPageItem>,
    successField?: string,
  ) => {
    try {
      const saved = await updateContextPageItem(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ContextPageItem[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ContextPageItem';
      console.error('Failed to update ContextPageItem:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteContextPageItemCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteContextPageItem(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ContextPageItem> = { ...source };
      delete (payload as any).id;
      const created = await addContextPageItem(payload).unwrap();
      setAllData((prev) => [created as ContextPageItem, ...prev]);
    } catch (error) {
      console.error('Failed to add ContextPageItem:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ContextPageItem> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addContextPageItem(payload).unwrap();
      setAllData((prev) => [created as ContextPageItem, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ContextPageItem:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ContextPageItem-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addContextPageItem(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateContextPageItem(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ContextPageItem[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ContextPageItem[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ContextPageItem[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ContextPageItem[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ContextPageItemTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ContextPageItem Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ContextPageItem</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ContextPageItemForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ContextPageItem (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ContextPageItem</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ContextPageItem;
                ...
                await updateContextPageItem(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ContextPageItems.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ContextPageItem'}
...
export default ContextPageItemTable;
