...
import { Reaction } from '@thorapi/model';
import ReactionForm from '@thorapi/redux/components/form/ReactionForm';
...
} from '@thorapi/redux/services/ReactionService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Reaction update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Reaction/${rowId}`;
...
const ReactionTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Reaction[] => {
    if (Array.isArray(result)) return result as Reaction[];
    ...
    return Array.isArray(candidate) ? (candidate as Reaction[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Reaction[], requestedPage: number) => {
  ...
  const [deleteReactionCascade] = useDeleteReactionCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Reaction | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Reaction row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Reaction>,
    successField?: string,
  ) => {
    try {
      const saved = await updateReaction(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Reaction[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Reaction';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteReactionCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Reaction-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addReaction(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateReaction(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Reaction[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Reaction[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Reaction[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Reaction[]);
    }
  };
