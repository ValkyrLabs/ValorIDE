...
import { WizardStartResponse } from '@thorapi/model';
import WizardStartResponseForm from '@thorapi/redux/components/form/WizardStartResponseForm';
...
} from '@thorapi/redux/services/WizardStartResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `WizardStartResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/WizardStartResponse/${rowId}`;
...
const WizardStartResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): WizardStartResponse[] => {
    if (Array.isArray(result)) return result as WizardStartResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as WizardStartResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: WizardStartResponse[], requestedPage: number) => {
  ...
  const [deleteWizardStartResponseCascade] = useDeleteWizardStartResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<WizardStartResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as WizardStartResponse;
    } catch (err) {
      console.warn('Could not reload WizardStartResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<WizardStartResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWizardStartResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as WizardStartResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update WizardStartResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWizardStartResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `WizardStartResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWizardStartResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWizardStartResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as WizardStartResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as WizardStartResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as WizardStartResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as WizardStartResponse[]);
    }
  };
