...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/InvokeMcpToolRequest/${rowId}`;
...
const InvokeMcpToolRequestTable: React.FC = () => {
  ...
  const [deleteInvokeMcpToolRequestCascade] =
    useDeleteInvokeMcpToolRequestCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<InvokeMcpToolRequest | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateInvokeMcpToolRequest(
        updatedItem as any,
      ).unwrap();
