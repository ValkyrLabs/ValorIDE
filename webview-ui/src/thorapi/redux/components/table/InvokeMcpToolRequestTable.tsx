...
} from '@thorapi/redux/services/InvokeMcpToolRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `InvokeMcpToolRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};
...
const InvokeMcpToolRequestTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): InvokeMcpToolRequest[] => {
    if (Array.isArray(result)) return result as InvokeMcpToolRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as InvokeMcpToolRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: InvokeMcpToolRequest[], requestedPage: number) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteInvokeMcpToolRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as InvokeMcpToolRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as InvokeMcpToolRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as InvokeMcpToolRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as InvokeMcpToolRequest[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter InvokeMcpToolRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
