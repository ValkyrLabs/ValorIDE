...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ConversationMemoryNode/${rowId}`;
...
const ConversationMemoryNodeTable: React.FC = () => {
  ...
  const [deleteConversationMemoryNode] =
    useDeleteConversationMemoryNodeMutation();
  const [deleteConversationMemoryNodeCascade] =
    useDeleteConversationMemoryNodeCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<ConversationMemoryNode | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateConversationMemoryNode(
        updatedItem as any,
      ).unwrap();
