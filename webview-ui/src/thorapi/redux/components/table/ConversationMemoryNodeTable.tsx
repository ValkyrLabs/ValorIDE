...
import { ConversationMemoryNode } from '@thorapi/model';
import ConversationMemoryNodeForm from '@thorapi/redux/components/form/ConversationMemoryNodeForm';
...
} from '@thorapi/redux/services/ConversationMemoryNodeService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'messageRole': { type: 'enum', enumValues: [
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ConversationMemoryNode update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ConversationMemoryNode/${rowId}`;
...
const ConversationMemoryNodeTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ConversationMemoryNode[] => {
    if (Array.isArray(result)) return result as ConversationMemoryNode[];
    ...
    return Array.isArray(candidate) ? (candidate as ConversationMemoryNode[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ConversationMemoryNode[], requestedPage: number) => {
  ...
  const [deleteConversationMemoryNodeCascade] = useDeleteConversationMemoryNodeCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<ConversationMemoryNode | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ConversationMemoryNode | null> => {
    ...
    try {
      ...
      return (await response.json()) as ConversationMemoryNode;
    } catch (err) {
      console.warn('Could not reload ConversationMemoryNode row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ConversationMemoryNode>,
    successField?: string,
  ) => {
    try {
      const saved = await updateConversationMemoryNode(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ConversationMemoryNode[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ConversationMemoryNode';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteConversationMemoryNodeCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ConversationMemoryNode-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addConversationMemoryNode(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateConversationMemoryNode(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ConversationMemoryNode[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ConversationMemoryNode[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ConversationMemoryNode[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ConversationMemoryNode[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ConversationMemoryNode (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
