...
import { SupportTicket } from '@thorapi/model';
import SupportTicketForm from '@thorapi/redux/components/form/SupportTicketForm';
...
import {
  useLazyGetSupportTicketsPagedQuery,
  useAddSupportTicketMutation,
  useUpdateSupportTicketMutation,
  useDeleteSupportTicketMutation,
  useDeleteSupportTicketCascadeMutation
} from '@thorapi/redux/services/SupportTicketService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SupportTicket update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SupportTicket/${rowId}`;

const SupportTicketTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SupportTicket[]>([]);
  ...
  const normalizePageRows = (result: unknown): SupportTicket[] => {
    if (Array.isArray(result)) return result as SupportTicket[];
    ...
    return Array.isArray(candidate) ? (candidate as SupportTicket[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SupportTicket[], requestedPage: number) => {
  ...
  const [deleteSupportTicketCascade] = useDeleteSupportTicketCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<SupportTicket | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<SupportTicket | null> => {
    ...
    try {
      ...
      console.warn('Could not reload SupportTicket row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SupportTicket>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSupportTicket(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SupportTicket[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SupportTicket';
      console.error('Failed to update SupportTicket:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSupportTicketCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteSupportTicket(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SupportTicket> = { ...source };
      delete (payload as any).id;
      const created = await addSupportTicket(payload).unwrap();
      setAllData((prev) => [created as SupportTicket, ...prev]);
    } catch (error) {
      console.error('Failed to add SupportTicket:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SupportTicket> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSupportTicket(payload).unwrap();
      setAllData((prev) => [created as SupportTicket, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) SupportTicket:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SupportTicket-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        try {
          ...
          try {
            await updateSupportTicket(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SupportTicket[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SupportTicket[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SupportTicket[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SupportTicket[]);
    }
  };
    ...
    <>
          ...
          >
            ...
            <FloatingControlPanel
              description="SupportTicket Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SupportTicket</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SupportTicketForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SupportTicket (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SupportTicket</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SupportTicket;
                ...
                await updateSupportTicket(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SupportTickets.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SupportTicket'}
