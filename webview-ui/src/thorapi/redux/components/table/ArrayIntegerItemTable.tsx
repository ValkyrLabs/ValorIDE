...
import { ArrayIntegerItem } from '@thorapi/model';
import ArrayIntegerItemForm from '@thorapi/redux/components/form/ArrayIntegerItemForm';
...
import {
  useLazyGetArrayIntegerItemsPagedQuery,
...
} from '@thorapi/redux/services/ArrayIntegerItemService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ArrayIntegerItem update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ArrayIntegerItem/${rowId}`;

const ArrayIntegerItemTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ArrayIntegerItem[]>([]);
  ...
  const normalizePageRows = (result: unknown): ArrayIntegerItem[] => {
    if (Array.isArray(result)) return result as ArrayIntegerItem[];
    ...
    return Array.isArray(candidate) ? (candidate as ArrayIntegerItem[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ArrayIntegerItem[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addArrayIntegerItem] = useAddArrayIntegerItemMutation();
  const [updateArrayIntegerItem] = useUpdateArrayIntegerItemMutation();
  const [deleteArrayIntegerItem] = useDeleteArrayIntegerItemMutation();
  const [deleteArrayIntegerItemCascade] = useDeleteArrayIntegerItemCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ArrayIntegerItem>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ArrayIntegerItem | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ArrayIntegerItem | null> => {
    ...
    try {
      ...
      return (await response.json()) as ArrayIntegerItem;
    } catch (err) {
      console.warn('Could not reload ArrayIntegerItem row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ArrayIntegerItem>,
    successField?: string,
  ) => {
    try {
      const saved = await updateArrayIntegerItem(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ArrayIntegerItem[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ArrayIntegerItem';
      console.error('Failed to update ArrayIntegerItem:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteArrayIntegerItemCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteArrayIntegerItem(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ArrayIntegerItem> = { ...source };
      delete (payload as any).id;
      const created = await addArrayIntegerItem(payload).unwrap();
      setAllData((prev) => [created as ArrayIntegerItem, ...prev]);
    } catch (error) {
      console.error('Failed to add ArrayIntegerItem:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ArrayIntegerItem> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addArrayIntegerItem(payload).unwrap();
      setAllData((prev) => [created as ArrayIntegerItem, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ArrayIntegerItem:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ArrayIntegerItem-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addArrayIntegerItem(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateArrayIntegerItem(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ArrayIntegerItem[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ArrayIntegerItem[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ArrayIntegerItem[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ArrayIntegerItem[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.ArrayIntegerItem',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ArrayIntegerItemTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ArrayIntegerItem Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ArrayIntegerItem</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ArrayIntegerItemForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ArrayIntegerItem (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ArrayIntegerItem</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ArrayIntegerItem;
                ...
                await updateArrayIntegerItem(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ArrayIntegerItems.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ArrayIntegerItem'}
...
export default ArrayIntegerItemTable;
