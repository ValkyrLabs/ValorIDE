...
const columnSchema: Record<string, ColumnSchema> = {
  policyStatus: {
    type: "enum",
    enumValues: ["DRAFT", "ACTIVE", "SUSPENDED", "RETIRED"],
...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/TrustPolicyManifest/${rowId}`;
...
const TrustPolicyManifestTable: React.FC = () => {
  ...
  const [deleteTrustPolicyManifestCascade] =
    useDeleteTrustPolicyManifestCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<TrustPolicyManifest | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateTrustPolicyManifest(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update TrustPolicyManifest";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustPolicyManifestCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter TrustPolicyManifest (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
