...
import { TrustPolicyManifest } from '@thorapi/model';
import TrustPolicyManifestForm from '@thorapi/redux/components/form/TrustPolicyManifestForm';
...
import {
  useLazyGetTrustPolicyManifestsPagedQuery,
...
} from '@thorapi/redux/services/TrustPolicyManifestService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'policyStatus': { type: 'enum', enumValues: [
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TrustPolicyManifest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TrustPolicyManifest/${rowId}`;

const TrustPolicyManifestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TrustPolicyManifest[]>([]);
  ...
  const normalizePageRows = (result: unknown): TrustPolicyManifest[] => {
    if (Array.isArray(result)) return result as TrustPolicyManifest[];
    ...
    return Array.isArray(candidate) ? (candidate as TrustPolicyManifest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TrustPolicyManifest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addTrustPolicyManifest] = useAddTrustPolicyManifestMutation();
  const [updateTrustPolicyManifest] = useUpdateTrustPolicyManifestMutation();
  const [deleteTrustPolicyManifest] = useDeleteTrustPolicyManifestMutation();
  const [deleteTrustPolicyManifestCascade] = useDeleteTrustPolicyManifestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TrustPolicyManifest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TrustPolicyManifest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TrustPolicyManifest | null> => {
    ...
    try {
      ...
      return (await response.json()) as TrustPolicyManifest;
    } catch (err) {
      console.warn('Could not reload TrustPolicyManifest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TrustPolicyManifest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTrustPolicyManifest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TrustPolicyManifest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TrustPolicyManifest';
      console.error('Failed to update TrustPolicyManifest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustPolicyManifestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTrustPolicyManifest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TrustPolicyManifest> = { ...source };
      delete (payload as any).id;
      const created = await addTrustPolicyManifest(payload).unwrap();
      setAllData((prev) => [created as TrustPolicyManifest, ...prev]);
    } catch (error) {
      console.error('Failed to add TrustPolicyManifest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TrustPolicyManifest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTrustPolicyManifest(payload).unwrap();
      setAllData((prev) => [created as TrustPolicyManifest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TrustPolicyManifest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TrustPolicyManifest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTrustPolicyManifest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTrustPolicyManifest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TrustPolicyManifest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TrustPolicyManifest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TrustPolicyManifest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TrustPolicyManifest[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="TrustPolicyManifestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="TrustPolicyManifest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TrustPolicyManifest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TrustPolicyManifestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TrustPolicyManifest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TrustPolicyManifest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TrustPolicyManifest;
                ...
                await updateTrustPolicyManifest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TrustPolicyManifests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TrustPolicyManifest'}
...
export default TrustPolicyManifestTable;
