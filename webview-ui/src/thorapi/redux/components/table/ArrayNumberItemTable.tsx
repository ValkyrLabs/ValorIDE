...
import { ArrayNumberItem } from '@thorapi/model';
import ArrayNumberItemForm from '@thorapi/redux/components/form/ArrayNumberItemForm';
...
import {
  useLazyGetArrayNumberItemsPagedQuery,
...
} from '@thorapi/redux/services/ArrayNumberItemService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ArrayNumberItem update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ArrayNumberItem/${rowId}`;

const ArrayNumberItemTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ArrayNumberItem[]>([]);
  ...
  const normalizePageRows = (result: unknown): ArrayNumberItem[] => {
    if (Array.isArray(result)) return result as ArrayNumberItem[];
    ...
    return Array.isArray(candidate) ? (candidate as ArrayNumberItem[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ArrayNumberItem[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addArrayNumberItem] = useAddArrayNumberItemMutation();
  const [updateArrayNumberItem] = useUpdateArrayNumberItemMutation();
  const [deleteArrayNumberItem] = useDeleteArrayNumberItemMutation();
  const [deleteArrayNumberItemCascade] = useDeleteArrayNumberItemCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ArrayNumberItem>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ArrayNumberItem | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ArrayNumberItem | null> => {
    ...
    try {
      ...
      return (await response.json()) as ArrayNumberItem;
    } catch (err) {
      console.warn('Could not reload ArrayNumberItem row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ArrayNumberItem>,
    successField?: string,
  ) => {
    try {
      const saved = await updateArrayNumberItem(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ArrayNumberItem[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ArrayNumberItem';
      console.error('Failed to update ArrayNumberItem:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteArrayNumberItemCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteArrayNumberItem(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ArrayNumberItem> = { ...source };
      delete (payload as any).id;
      const created = await addArrayNumberItem(payload).unwrap();
      setAllData((prev) => [created as ArrayNumberItem, ...prev]);
    } catch (error) {
      console.error('Failed to add ArrayNumberItem:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ArrayNumberItem> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addArrayNumberItem(payload).unwrap();
      setAllData((prev) => [created as ArrayNumberItem, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ArrayNumberItem:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ArrayNumberItem-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addArrayNumberItem(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateArrayNumberItem(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ArrayNumberItem[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ArrayNumberItem[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ArrayNumberItem[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ArrayNumberItem[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.ArrayNumberItem',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ArrayNumberItemTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ArrayNumberItem Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ArrayNumberItem</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ArrayNumberItemForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ArrayNumberItem (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ArrayNumberItem</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ArrayNumberItem;
                ...
                await updateArrayNumberItem(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ArrayNumberItems.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ArrayNumberItem'}
...
export default ArrayNumberItemTable;
