...
import { OasEnum } from '@thorapi/model';
import OasEnumForm from '@thorapi/redux/components/form/OasEnumForm';
...
import {
  ...
  useDeleteOasEnumCascadeMutation
} from '@thorapi/redux/services/OasEnumService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `OasEnum update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/OasEnum/${rowId}`;
...
const OasEnumTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): OasEnum[] => {
    if (Array.isArray(result)) return result as OasEnum[];
    ...
    return Array.isArray(candidate) ? (candidate as OasEnum[]) : [];
  };

  const mergeFetchedRows = (incomingRows: OasEnum[], requestedPage: number) => {
  ...
  const [deleteOasEnumCascade] = useDeleteOasEnumCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<OasEnum | null> => {
    ...
    try {
      ...
      return (await response.json()) as OasEnum;
    } catch (err) {
      console.warn('Could not reload OasEnum row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<OasEnum>,
    successField?: string,
  ) => {
    try {
      const saved = await updateOasEnum(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as OasEnum[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update OasEnum';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOasEnumCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `OasEnum-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addOasEnum(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateOasEnum(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as OasEnum[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as OasEnum[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as OasEnum[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as OasEnum[]);
    }
  };
