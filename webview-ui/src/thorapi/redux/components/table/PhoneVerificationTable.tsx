...
import { PhoneVerification } from '@thorapi/model';
import PhoneVerificationForm from '@thorapi/redux/components/form/PhoneVerificationForm';
...
import {
  useLazyGetPhoneVerificationsPagedQuery,
...
} from '@thorapi/redux/services/PhoneVerificationService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'verificationCodeExpiresAt': { type: 'datetime' },
  'verifiedAt': { type: 'datetime' },
  'usedFor2FA': { type: 'boolean' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `PhoneVerification update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/PhoneVerification/${rowId}`;

const PhoneVerificationTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<PhoneVerification[]>([]);
  ...
  const normalizePageRows = (result: unknown): PhoneVerification[] => {
    if (Array.isArray(result)) return result as PhoneVerification[];
    ...
    return Array.isArray(candidate) ? (candidate as PhoneVerification[]) : [];
  };

  const mergeFetchedRows = (incomingRows: PhoneVerification[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addPhoneVerification] = useAddPhoneVerificationMutation();
  const [updatePhoneVerification] = useUpdatePhoneVerificationMutation();
  const [deletePhoneVerification] = useDeletePhoneVerificationMutation();
  const [deletePhoneVerificationCascade] = useDeletePhoneVerificationCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<PhoneVerification>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<PhoneVerification | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<PhoneVerification | null> => {
    ...
    try {
      ...
      return (await response.json()) as PhoneVerification;
    } catch (err) {
      console.warn('Could not reload PhoneVerification row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<PhoneVerification>,
    successField?: string,
  ) => {
    try {
      const saved = await updatePhoneVerification(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as PhoneVerification[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update PhoneVerification';
      console.error('Failed to update PhoneVerification:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePhoneVerificationCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deletePhoneVerification(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<PhoneVerification> = { ...source };
      delete (payload as any).id;
      const created = await addPhoneVerification(payload).unwrap();
      setAllData((prev) => [created as PhoneVerification, ...prev]);
    } catch (error) {
      console.error('Failed to add PhoneVerification:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<PhoneVerification> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addPhoneVerification(payload).unwrap();
      setAllData((prev) => [created as PhoneVerification, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) PhoneVerification:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `PhoneVerification-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addPhoneVerification(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updatePhoneVerification(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as PhoneVerification[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as PhoneVerification[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as PhoneVerification[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as PhoneVerification[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="PhoneVerificationTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="PhoneVerification Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create PhoneVerification</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter PhoneVerification (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing PhoneVerification</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof PhoneVerification;
                ...
                await updatePhoneVerification(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of PhoneVerifications.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'PhoneVerification'}
