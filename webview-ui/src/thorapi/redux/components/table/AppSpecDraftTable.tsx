...
import { AppSpecDraft } from '@thorapi/model';
import AppSpecDraftForm from '@thorapi/redux/components/form/AppSpecDraftForm';
...
const columnSchema: Record<string, ColumnSchema> = {
  'status': { type: 'enum', enumValues: [
    'draft', 'clarifying', 'ready', 'accepted', 'superseded', 'failed', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AppSpecDraft update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AppSpecDraft/${rowId}`;

const AppSpecDraftTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<AppSpecDraft[]>([]);
  ...
  const normalizePageRows = (result: unknown): AppSpecDraft[] => {
    if (Array.isArray(result)) return result as AppSpecDraft[];
    ...
    return Array.isArray(candidate) ? (candidate as AppSpecDraft[]) : [];
  };

  const mergeFetchedRows = (incomingRows: AppSpecDraft[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addAppSpecDraft] = useAddAppSpecDraftMutation();
  const [updateAppSpecDraft] = useUpdateAppSpecDraftMutation();
  const [deleteAppSpecDraft] = useDeleteAppSpecDraftMutation();
  const [deleteAppSpecDraftCascade] = useDeleteAppSpecDraftCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<AppSpecDraft>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<AppSpecDraft | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<AppSpecDraft | null> => {
    ...
    try {
      ...
      return (await response.json()) as AppSpecDraft;
    } catch (err) {
      console.warn('Could not reload AppSpecDraft row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AppSpecDraft>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAppSpecDraft(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AppSpecDraft[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AppSpecDraft';
      console.error('Failed to update AppSpecDraft:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAppSpecDraftCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteAppSpecDraft(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<AppSpecDraft> = { ...source };
      delete (payload as any).id;
      const created = await addAppSpecDraft(payload).unwrap();
      setAllData((prev) => [created as AppSpecDraft, ...prev]);
    } catch (error) {
      console.error('Failed to add AppSpecDraft:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<AppSpecDraft> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addAppSpecDraft(payload).unwrap();
      setAllData((prev) => [created as AppSpecDraft, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) AppSpecDraft:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AppSpecDraft-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAppSpecDraft(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAppSpecDraft(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AppSpecDraft[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AppSpecDraft[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AppSpecDraft[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AppSpecDraft[]);
    }
  };
    ...
    <>
          ...
          >
            ...
            <FloatingControlPanel
              description="AppSpecDraft Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create AppSpecDraft</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <AppSpecDraftForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter AppSpecDraft (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing AppSpecDraft</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof AppSpecDraft;
                ...
                await updateAppSpecDraft(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of AppSpecDrafts.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'AppSpecDraft'}
...
export default AppSpecDraftTable;
