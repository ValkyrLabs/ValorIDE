...
import { CreatorEarnings } from '@thorapi/model';
import CreatorEarningsForm from '@thorapi/redux/components/form/CreatorEarningsForm';
...
import {
  useLazyGetCreatorEarningssPagedQuery,
...
} from '@thorapi/redux/services/CreatorEarningsService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'totalInvocations': { type: 'number' },
  'payoutStatus': { type: 'enum', enumValues: [
    'PENDING', 'PROCESSING', 'PAID', 'FAILED', 
  ], enumValueType: 'string' },
  'payoutMethod': { type: 'enum', enumValues: [
    'STRIPE', 'CRYPTO_WALLET', 'BANK_TRANSFER', 
  ], enumValueType: 'string' },
  'payoutDate': { type: 'datetime' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `CreatorEarnings update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/CreatorEarnings/${rowId}`;

const CreatorEarningsTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<CreatorEarnings[]>([]);
  ...
  const normalizePageRows = (result: unknown): CreatorEarnings[] => {
    if (Array.isArray(result)) return result as CreatorEarnings[];
    ...
    return Array.isArray(candidate) ? (candidate as CreatorEarnings[]) : [];
  };

  const mergeFetchedRows = (incomingRows: CreatorEarnings[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addCreatorEarnings] = useAddCreatorEarningsMutation();
  const [updateCreatorEarnings] = useUpdateCreatorEarningsMutation();
  const [deleteCreatorEarnings] = useDeleteCreatorEarningsMutation();
  const [deleteCreatorEarningsCascade] = useDeleteCreatorEarningsCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<CreatorEarnings>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<CreatorEarnings | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<CreatorEarnings | null> => {
    ...
    try {
      ...
      return (await response.json()) as CreatorEarnings;
    } catch (err) {
      console.warn('Could not reload CreatorEarnings row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<CreatorEarnings>,
    successField?: string,
  ) => {
    try {
      const saved = await updateCreatorEarnings(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as CreatorEarnings[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update CreatorEarnings';
      console.error('Failed to update CreatorEarnings:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCreatorEarningsCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteCreatorEarnings(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<CreatorEarnings> = { ...source };
      delete (payload as any).id;
      const created = await addCreatorEarnings(payload).unwrap();
      setAllData((prev) => [created as CreatorEarnings, ...prev]);
    } catch (error) {
      console.error('Failed to add CreatorEarnings:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<CreatorEarnings> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addCreatorEarnings(payload).unwrap();
      setAllData((prev) => [created as CreatorEarnings, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) CreatorEarnings:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `CreatorEarnings-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addCreatorEarnings(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateCreatorEarnings(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as CreatorEarnings[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as CreatorEarnings[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as CreatorEarnings[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as CreatorEarnings[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="CreatorEarningsTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="CreatorEarnings Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create CreatorEarnings</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <CreatorEarningsForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter CreatorEarnings (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing CreatorEarnings</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof CreatorEarnings;
                ...
                await updateCreatorEarnings(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of CreatorEarningss.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'CreatorEarnings'}
...
export default CreatorEarningsTable;
