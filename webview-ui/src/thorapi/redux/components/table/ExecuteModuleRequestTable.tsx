...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ExecuteModuleRequest/${rowId}`;
...
const ExecuteModuleRequestTable: React.FC = () => {
  ...
  const [deleteExecuteModuleRequestCascade] =
    useDeleteExecuteModuleRequestCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<ExecuteModuleRequest | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateExecuteModuleRequest(
        updatedItem as any,
      ).unwrap();
