...
import { OpenClawLightsailProvisioningEvent } from '@thorapi/model';
import OpenClawLightsailProvisioningEventForm from '@thorapi/redux/components/form/OpenClawLightsailProvisioningEventForm';
...
import {
  useLazyGetOpenClawLightsailProvisioningEventsPagedQuery,
...
} from '@thorapi/redux/services/OpenClawLightsailProvisioningEventService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `OpenClawLightsailProvisioningEvent update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/OpenClawLightsailProvisioningEvent/${rowId}`;

const OpenClawLightsailProvisioningEventTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<OpenClawLightsailProvisioningEvent[]>([]);
  ...
  const normalizePageRows = (result: unknown): OpenClawLightsailProvisioningEvent[] => {
    if (Array.isArray(result)) return result as OpenClawLightsailProvisioningEvent[];
    ...
    return Array.isArray(candidate) ? (candidate as OpenClawLightsailProvisioningEvent[]) : [];
  };

  const mergeFetchedRows = (incomingRows: OpenClawLightsailProvisioningEvent[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addOpenClawLightsailProvisioningEvent] = useAddOpenClawLightsailProvisioningEventMutation();
  const [updateOpenClawLightsailProvisioningEvent] = useUpdateOpenClawLightsailProvisioningEventMutation();
  const [deleteOpenClawLightsailProvisioningEvent] = useDeleteOpenClawLightsailProvisioningEventMutation();
  const [deleteOpenClawLightsailProvisioningEventCascade] = useDeleteOpenClawLightsailProvisioningEventCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<OpenClawLightsailProvisioningEvent>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<OpenClawLightsailProvisioningEvent | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<OpenClawLightsailProvisioningEvent | null> => {
    ...
    try {
      ...
      return (await response.json()) as OpenClawLightsailProvisioningEvent;
    } catch (err) {
      console.warn('Could not reload OpenClawLightsailProvisioningEvent row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<OpenClawLightsailProvisioningEvent>,
    successField?: string,
  ) => {
    try {
      const saved = await updateOpenClawLightsailProvisioningEvent(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as OpenClawLightsailProvisioningEvent[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update OpenClawLightsailProvisioningEvent';
      console.error('Failed to update OpenClawLightsailProvisioningEvent:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOpenClawLightsailProvisioningEventCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteOpenClawLightsailProvisioningEvent(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<OpenClawLightsailProvisioningEvent> = { ...source };
      delete (payload as any).id;
      const created = await addOpenClawLightsailProvisioningEvent(payload).unwrap();
      setAllData((prev) => [created as OpenClawLightsailProvisioningEvent, ...prev]);
    } catch (error) {
      console.error('Failed to add OpenClawLightsailProvisioningEvent:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<OpenClawLightsailProvisioningEvent> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addOpenClawLightsailProvisioningEvent(payload).unwrap();
      setAllData((prev) => [created as OpenClawLightsailProvisioningEvent, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) OpenClawLightsailProvisioningEvent:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `OpenClawLightsailProvisioningEvent-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addOpenClawLightsailProvisioningEvent(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateOpenClawLightsailProvisioningEvent(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as OpenClawLightsailProvisioningEvent[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as OpenClawLightsailProvisioningEvent[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as OpenClawLightsailProvisioningEvent[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as OpenClawLightsailProvisioningEvent[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="OpenClawLightsailProvisioningEventTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="OpenClawLightsailProvisioningEvent Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create OpenClawLightsailProvisioningEvent</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <OpenClawLightsailProvisioningEventForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter OpenClawLightsailProvisioningEvent (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing OpenClawLightsailProvisioningEvent</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof OpenClawLightsailProvisioningEvent;
                ...
                await updateOpenClawLightsailProvisioningEvent(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of OpenClawLightsailProvisioningEvents.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'OpenClawLightsailProvisioningEvent'}
...
export default OpenClawLightsailProvisioningEventTable;
