...
import { McpServiceRegistry } from '@thorapi/model';
import McpServiceRegistryForm from '@thorapi/redux/components/form/McpServiceRegistryForm';
...
} from '@thorapi/redux/services/McpServiceRegistryService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'category': { type: 'enum', enumValues: [
    'code_generation', 'automation', 'optimization', 'integration', 'analytics', 'custom', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `McpServiceRegistry update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/McpServiceRegistry/${rowId}`;
...
const McpServiceRegistryTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): McpServiceRegistry[] => {
    if (Array.isArray(result)) return result as McpServiceRegistry[];
    ...
    return Array.isArray(candidate) ? (candidate as McpServiceRegistry[]) : [];
  };

  const mergeFetchedRows = (incomingRows: McpServiceRegistry[], requestedPage: number) => {
  ...
  const [deleteMcpServiceRegistryCascade] = useDeleteMcpServiceRegistryCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<McpServiceRegistry | null> => {
    ...
    try {
      ...
      return (await response.json()) as McpServiceRegistry;
    } catch (err) {
      console.warn('Could not reload McpServiceRegistry row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<McpServiceRegistry>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMcpServiceRegistry(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as McpServiceRegistry[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update McpServiceRegistry';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpServiceRegistryCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `McpServiceRegistry-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMcpServiceRegistry(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMcpServiceRegistry(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as McpServiceRegistry[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as McpServiceRegistry[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as McpServiceRegistry[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as McpServiceRegistry[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter McpServiceRegistry (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
