...
import { Space } from '@thorapi/model';
import SpaceForm from '@thorapi/redux/components/form/SpaceForm';
...
} from '@thorapi/redux/services/SpaceService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Space update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Space/${rowId}`;
...
const SpaceTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Space[] => {
    ...
    return Array.isArray(candidate) ? (candidate as Space[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Space[], requestedPage: number) => {
  ...
  const [deleteSpaceCascade] = useDeleteSpaceCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Space | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Space row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Space>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSpace(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Space[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Space';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSpaceCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Space-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        try {
          ...
          try {
            await updateSpace(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Space[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Space[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Space[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Space[]);
    }
  };
