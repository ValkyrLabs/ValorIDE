...
import { ChatResponse } from '@thorapi/model';
import ChatResponseForm from '@thorapi/redux/components/form/ChatResponseForm';
...
} from '@thorapi/redux/services/ChatResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ChatResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ChatResponse/${rowId}`;
...
const ChatResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ChatResponse[] => {
    if (Array.isArray(result)) return result as ChatResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as ChatResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ChatResponse[], requestedPage: number) => {
  ...
  const [deleteChatResponseCascade] = useDeleteChatResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ChatResponse | null> => {
    ...
    try {
      ...
      console.warn('Could not reload ChatResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ChatResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateChatResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ChatResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ChatResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteChatResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ChatResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addChatResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateChatResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ChatResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ChatResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ChatResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ChatResponse[]);
    }
  };
