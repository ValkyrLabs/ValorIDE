...
import { CareerOpportunity } from '@thorapi/model';
import CareerOpportunityForm from '@thorapi/redux/components/form/CareerOpportunityForm';
...
} from '@thorapi/redux/services/CareerOpportunityService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'opportunityType': { type: 'enum', enumValues: [
    'DIRECT_HIRE', 'CONSULTING_ENGAGEMENT', 'RESELLER_PARTNER', 'OPEN_SOURCE_CONTRIBUTOR', 
  ], enumValueType: 'string' },
  'experienceYearsMin': { type: 'number' },
  'experienceYearsMax': { type: 'number' },
  'commitment': { type: 'enum', enumValues: [
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `CareerOpportunity update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/CareerOpportunity/${rowId}`;
...
const CareerOpportunityTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): CareerOpportunity[] => {
    if (Array.isArray(result)) return result as CareerOpportunity[];
    ...
    return Array.isArray(candidate) ? (candidate as CareerOpportunity[]) : [];
  };

  const mergeFetchedRows = (incomingRows: CareerOpportunity[], requestedPage: number) => {
  ...
  const [deleteCareerOpportunityCascade] = useDeleteCareerOpportunityCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<CareerOpportunity | null> => {
    ...
    try {
      ...
      return (await response.json()) as CareerOpportunity;
    } catch (err) {
      console.warn('Could not reload CareerOpportunity row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<CareerOpportunity>,
    successField?: string,
  ) => {
    try {
      const saved = await updateCareerOpportunity(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as CareerOpportunity[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update CareerOpportunity';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCareerOpportunityCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `CareerOpportunity-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addCareerOpportunity(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateCareerOpportunity(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as CareerOpportunity[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as CareerOpportunity[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as CareerOpportunity[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as CareerOpportunity[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter CareerOpportunity (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
