...
import { PaymentType } from '@thorapi/model';
import PaymentTypeForm from '@thorapi/redux/components/form/PaymentTypeForm';
...
const columnSchema: Record<string, ColumnSchema> = {
  'type': { type: 'enum', enumValues: [
    'CREDIT_CARD', 'PAYPAL', 'CRYPTO_WALLET', 'STRIPE', 'BANK_TRANSFER', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `PaymentType update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/PaymentType/${rowId}`;
...
const PaymentTypeTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<PaymentType[]>([]);
  ...
  const normalizePageRows = (result: unknown): PaymentType[] => {
    if (Array.isArray(result)) return result as PaymentType[];
    ...
    return Array.isArray(candidate) ? (candidate as PaymentType[]) : [];
  };

  const mergeFetchedRows = (incomingRows: PaymentType[], requestedPage: number) => {
  ...
  const [deletePaymentTypeCascade] = useDeletePaymentTypeCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<PaymentType | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<PaymentType | null> => {
    ...
    try {
      ...
      console.warn('Could not reload PaymentType row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<PaymentType>,
    successField?: string,
  ) => {
    try {
      const saved = await updatePaymentType(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as PaymentType[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update PaymentType';
      console.error('Failed to update PaymentType:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePaymentTypeCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deletePaymentType(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<PaymentType> = { ...source };
      delete (payload as any).id;
      const created = await addPaymentType(payload).unwrap();
      setAllData((prev) => [created as PaymentType, ...prev]);
    } catch (error) {
      console.error('Failed to add PaymentType:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<PaymentType> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addPaymentType(payload).unwrap();
      setAllData((prev) => [created as PaymentType, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) PaymentType:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `PaymentType-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addPaymentType(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updatePaymentType(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as PaymentType[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as PaymentType[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as PaymentType[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as PaymentType[]);
    }
  };
    ...
    <>
          ...
          >
            ...
            <FloatingControlPanel
              description="PaymentType Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create PaymentType</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <PaymentTypeForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter PaymentType (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing PaymentType</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof PaymentType;
                ...
                await updatePaymentType(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of PaymentTypes.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'PaymentType'}
