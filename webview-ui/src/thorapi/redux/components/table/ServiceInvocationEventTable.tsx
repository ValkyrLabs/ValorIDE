...
import { ServiceInvocationEvent } from '@thorapi/model';
import ServiceInvocationEventForm from '@thorapi/redux/components/form/ServiceInvocationEventForm';
...
import {
  useLazyGetServiceInvocationEventsPagedQuery,
...
} from '@thorapi/redux/services/ServiceInvocationEventService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ServiceInvocationEvent update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ServiceInvocationEvent/${rowId}`;

const ServiceInvocationEventTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ServiceInvocationEvent[]>([]);
  ...
  const normalizePageRows = (result: unknown): ServiceInvocationEvent[] => {
    if (Array.isArray(result)) return result as ServiceInvocationEvent[];
    ...
    return Array.isArray(candidate) ? (candidate as ServiceInvocationEvent[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ServiceInvocationEvent[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addServiceInvocationEvent] = useAddServiceInvocationEventMutation();
  const [updateServiceInvocationEvent] = useUpdateServiceInvocationEventMutation();
  const [deleteServiceInvocationEvent] = useDeleteServiceInvocationEventMutation();
  const [deleteServiceInvocationEventCascade] = useDeleteServiceInvocationEventCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ServiceInvocationEvent>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ServiceInvocationEvent | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ServiceInvocationEvent | null> => {
    ...
    try {
      ...
      return (await response.json()) as ServiceInvocationEvent;
    } catch (err) {
      console.warn('Could not reload ServiceInvocationEvent row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ServiceInvocationEvent>,
    successField?: string,
  ) => {
    try {
      const saved = await updateServiceInvocationEvent(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ServiceInvocationEvent[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ServiceInvocationEvent';
      console.error('Failed to update ServiceInvocationEvent:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteServiceInvocationEventCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteServiceInvocationEvent(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ServiceInvocationEvent> = { ...source };
      delete (payload as any).id;
      const created = await addServiceInvocationEvent(payload).unwrap();
      setAllData((prev) => [created as ServiceInvocationEvent, ...prev]);
    } catch (error) {
      console.error('Failed to add ServiceInvocationEvent:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ServiceInvocationEvent> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addServiceInvocationEvent(payload).unwrap();
      setAllData((prev) => [created as ServiceInvocationEvent, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ServiceInvocationEvent:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ServiceInvocationEvent-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addServiceInvocationEvent(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateServiceInvocationEvent(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ServiceInvocationEvent[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ServiceInvocationEvent[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ServiceInvocationEvent[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ServiceInvocationEvent[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ServiceInvocationEventTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ServiceInvocationEvent Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ServiceInvocationEvent</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ServiceInvocationEventForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ServiceInvocationEvent (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ServiceInvocationEvent</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ServiceInvocationEvent;
                ...
                await updateServiceInvocationEvent(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ServiceInvocationEvents.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ServiceInvocationEvent'}
...
export default ServiceInvocationEventTable;
