...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ServiceInvocationEvent/${rowId}`;
...
const ServiceInvocationEventTable: React.FC = () => {
  ...
  const [updateServiceInvocationEvent] =
    useUpdateServiceInvocationEventMutation();
  const [deleteServiceInvocationEvent] =
    useDeleteServiceInvocationEventMutation();
  const [deleteServiceInvocationEventCascade] =
    useDeleteServiceInvocationEventCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<ServiceInvocationEvent | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<ServiceInvocationEvent | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateServiceInvocationEvent(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ServiceInvocationEvent";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteServiceInvocationEventCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter ServiceInvocationEvent (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
