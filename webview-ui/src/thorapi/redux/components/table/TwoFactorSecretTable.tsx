...
import { TwoFactorSecret } from '@thorapi/model';
import TwoFactorSecretForm from '@thorapi/redux/components/form/TwoFactorSecretForm';
...
import {
  useLazyGetTwoFactorSecretsPagedQuery,
...
} from '@thorapi/redux/services/TwoFactorSecretService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TwoFactorSecret update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TwoFactorSecret/${rowId}`;

const TwoFactorSecretTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TwoFactorSecret[]>([]);
  ...
  const normalizePageRows = (result: unknown): TwoFactorSecret[] => {
    if (Array.isArray(result)) return result as TwoFactorSecret[];
    ...
    return Array.isArray(candidate) ? (candidate as TwoFactorSecret[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TwoFactorSecret[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addTwoFactorSecret] = useAddTwoFactorSecretMutation();
  const [updateTwoFactorSecret] = useUpdateTwoFactorSecretMutation();
  const [deleteTwoFactorSecret] = useDeleteTwoFactorSecretMutation();
  const [deleteTwoFactorSecretCascade] = useDeleteTwoFactorSecretCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TwoFactorSecret>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TwoFactorSecret | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TwoFactorSecret | null> => {
    ...
    try {
      ...
      return (await response.json()) as TwoFactorSecret;
    } catch (err) {
      console.warn('Could not reload TwoFactorSecret row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TwoFactorSecret>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTwoFactorSecret(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TwoFactorSecret[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TwoFactorSecret';
      console.error('Failed to update TwoFactorSecret:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTwoFactorSecretCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTwoFactorSecret(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TwoFactorSecret> = { ...source };
      delete (payload as any).id;
      const created = await addTwoFactorSecret(payload).unwrap();
      setAllData((prev) => [created as TwoFactorSecret, ...prev]);
    } catch (error) {
      console.error('Failed to add TwoFactorSecret:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TwoFactorSecret> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTwoFactorSecret(payload).unwrap();
      setAllData((prev) => [created as TwoFactorSecret, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TwoFactorSecret:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TwoFactorSecret-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTwoFactorSecret(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTwoFactorSecret(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TwoFactorSecret[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TwoFactorSecret[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TwoFactorSecret[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TwoFactorSecret[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="TwoFactorSecretTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="TwoFactorSecret Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TwoFactorSecret</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TwoFactorSecretForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TwoFactorSecret (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TwoFactorSecret</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TwoFactorSecret;
                ...
                await updateTwoFactorSecret(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TwoFactorSecrets.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TwoFactorSecret'}
...
export default TwoFactorSecretTable;
