...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/CircuitBreakerConfig/${rowId}`;
...
const CircuitBreakerConfigTable: React.FC = () => {
  ...
  const [deleteCircuitBreakerConfigCascade] =
    useDeleteCircuitBreakerConfigCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<CircuitBreakerConfig | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateCircuitBreakerConfig(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update CircuitBreakerConfig";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCircuitBreakerConfigCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter CircuitBreakerConfig (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
