...
import CircuitBreakerConfigForm from '@thorapi/redux/components/form/CircuitBreakerConfigForm';
...
import {
  useLazyGetCircuitBreakerConfigsPagedQuery,
...
} from '@thorapi/redux/services/CircuitBreakerConfigService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `CircuitBreakerConfig update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/CircuitBreakerConfig/${rowId}`;

const CircuitBreakerConfigTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<CircuitBreakerConfig[]>([]);
  ...
  const normalizePageRows = (result: unknown): CircuitBreakerConfig[] => {
    if (Array.isArray(result)) return result as CircuitBreakerConfig[];
    ...
    return Array.isArray(candidate) ? (candidate as CircuitBreakerConfig[]) : [];
  };

  const mergeFetchedRows = (incomingRows: CircuitBreakerConfig[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addCircuitBreakerConfig] = useAddCircuitBreakerConfigMutation();
  const [updateCircuitBreakerConfig] = useUpdateCircuitBreakerConfigMutation();
  const [deleteCircuitBreakerConfig] = useDeleteCircuitBreakerConfigMutation();
  const [deleteCircuitBreakerConfigCascade] = useDeleteCircuitBreakerConfigCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<CircuitBreakerConfig | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<CircuitBreakerConfig | null> => {
    ...
    try {
      ...
      return (await response.json()) as CircuitBreakerConfig;
    } catch (err) {
      console.warn('Could not reload CircuitBreakerConfig row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<CircuitBreakerConfig>,
    successField?: string,
  ) => {
    try {
      const saved = await updateCircuitBreakerConfig(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as CircuitBreakerConfig[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update CircuitBreakerConfig';
      console.error('Failed to update CircuitBreakerConfig:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCircuitBreakerConfigCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteCircuitBreakerConfig(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<CircuitBreakerConfig> = { ...source };
      delete (payload as any).id;
      const created = await addCircuitBreakerConfig(payload).unwrap();
      setAllData((prev) => [created as CircuitBreakerConfig, ...prev]);
    } catch (error) {
      console.error('Failed to add CircuitBreakerConfig:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<CircuitBreakerConfig> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addCircuitBreakerConfig(payload).unwrap();
      setAllData((prev) => [created as CircuitBreakerConfig, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) CircuitBreakerConfig:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `CircuitBreakerConfig-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addCircuitBreakerConfig(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateCircuitBreakerConfig(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as CircuitBreakerConfig[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as CircuitBreakerConfig[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as CircuitBreakerConfig[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as CircuitBreakerConfig[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="CircuitBreakerConfigTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="CircuitBreakerConfig Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create CircuitBreakerConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <CircuitBreakerConfigForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter CircuitBreakerConfig (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing CircuitBreakerConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof CircuitBreakerConfig;
                ...
                await updateCircuitBreakerConfig(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of CircuitBreakerConfigs.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'CircuitBreakerConfig'}
...
export default CircuitBreakerConfigTable;
