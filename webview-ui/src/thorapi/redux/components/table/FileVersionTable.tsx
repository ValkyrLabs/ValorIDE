...
import FileVersionForm from '@thorapi/redux/components/form/FileVersionForm';
...
import {
  ...
  useDeleteFileVersionCascadeMutation
} from '@thorapi/redux/services/FileVersionService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `FileVersion update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/FileVersion/${rowId}`;
...
const FileVersionTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): FileVersion[] => {
    if (Array.isArray(result)) return result as FileVersion[];
    ...
    return Array.isArray(candidate) ? (candidate as FileVersion[]) : [];
  };

  const mergeFetchedRows = (incomingRows: FileVersion[], requestedPage: number) => {
  ...
  const [deleteFileVersionCascade] = useDeleteFileVersionCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<FileVersion | null> => {
    ...
    try {
      ...
      return (await response.json()) as FileVersion;
    } catch (err) {
      console.warn('Could not reload FileVersion row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<FileVersion>,
    successField?: string,
  ) => {
    try {
      const saved = await updateFileVersion(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as FileVersion[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update FileVersion';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteFileVersionCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `FileVersion-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addFileVersion(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateFileVersion(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as FileVersion[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as FileVersion[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as FileVersion[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as FileVersion[]);
    }
  };
