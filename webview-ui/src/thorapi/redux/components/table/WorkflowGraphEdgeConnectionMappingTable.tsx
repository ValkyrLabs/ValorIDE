...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/WorkflowGraphEdgeConnectionMapping/${rowId}`;
...
const WorkflowGraphEdgeConnectionMappingTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addWorkflowGraphEdgeConnectionMapping] =
    useAddWorkflowGraphEdgeConnectionMappingMutation();
  const [updateWorkflowGraphEdgeConnectionMapping] =
    useUpdateWorkflowGraphEdgeConnectionMappingMutation();
  const [deleteWorkflowGraphEdgeConnectionMapping] =
    useDeleteWorkflowGraphEdgeConnectionMappingMutation();
  const [deleteWorkflowGraphEdgeConnectionMappingCascade] =
    useDeleteWorkflowGraphEdgeConnectionMappingCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] =
    useState<WorkflowGraphEdgeConnectionMapping | null>(null);
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<WorkflowGraphEdgeConnectionMapping | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload WorkflowGraphEdgeConnectionMapping row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateWorkflowGraphEdgeConnectionMapping(
        updatedItem as any,
      ).unwrap();
      ...
      setAllData(
        (prev) =>
          mergeUpdatedRow(
            ...
            rowId,
            hydrated as any,
          ) as WorkflowGraphEdgeConnectionMapping[],
      );
      if (successField) {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update WorkflowGraphEdgeConnectionMapping";
      console.error(
        "Failed to update WorkflowGraphEdgeConnectionMapping:",
        error,
      );
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphEdgeConnectionMappingCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
            ...
            try {
              await deleteWorkflowGraphEdgeConnectionMapping(
                id as any,
              ).unwrap();
            } catch (inner) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<WorkflowGraphEdgeConnectionMapping> = {
        ...source,
      };
      ...
      setAllData((prev) => [
        created as WorkflowGraphEdgeConnectionMapping,
        ...prev,
      ]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) WorkflowGraphEdgeConnectionMapping:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter WorkflowGraphEdgeConnectionMapping (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
      ...
      <MarkdownEditorModal
        ...
        onSave={async (updatedText) => {
          try {
            ...
            const field =
              richModal.field as keyof WorkflowGraphEdgeConnectionMapping;
            if (!rid || !field) {
