...
import { WorkflowGraphEdgeConnectionMapping } from '@thorapi/model';
import WorkflowGraphEdgeConnectionMappingForm from '@thorapi/redux/components/form/WorkflowGraphEdgeConnectionMappingForm';
...
import {
  useLazyGetWorkflowGraphEdgeConnectionMappingsPagedQuery,
...
} from '@thorapi/redux/services/WorkflowGraphEdgeConnectionMappingService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `WorkflowGraphEdgeConnectionMapping update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/WorkflowGraphEdgeConnectionMapping/${rowId}`;

const WorkflowGraphEdgeConnectionMappingTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<WorkflowGraphEdgeConnectionMapping[]>([]);
  ...
  const normalizePageRows = (result: unknown): WorkflowGraphEdgeConnectionMapping[] => {
    if (Array.isArray(result)) return result as WorkflowGraphEdgeConnectionMapping[];
    ...
    return Array.isArray(candidate) ? (candidate as WorkflowGraphEdgeConnectionMapping[]) : [];
  };

  const mergeFetchedRows = (incomingRows: WorkflowGraphEdgeConnectionMapping[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addWorkflowGraphEdgeConnectionMapping] = useAddWorkflowGraphEdgeConnectionMappingMutation();
  const [updateWorkflowGraphEdgeConnectionMapping] = useUpdateWorkflowGraphEdgeConnectionMappingMutation();
  const [deleteWorkflowGraphEdgeConnectionMapping] = useDeleteWorkflowGraphEdgeConnectionMappingMutation();
  const [deleteWorkflowGraphEdgeConnectionMappingCascade] = useDeleteWorkflowGraphEdgeConnectionMappingCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<WorkflowGraphEdgeConnectionMapping>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<WorkflowGraphEdgeConnectionMapping | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<WorkflowGraphEdgeConnectionMapping | null> => {
    ...
    try {
      ...
      return (await response.json()) as WorkflowGraphEdgeConnectionMapping;
    } catch (err) {
      console.warn('Could not reload WorkflowGraphEdgeConnectionMapping row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<WorkflowGraphEdgeConnectionMapping>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWorkflowGraphEdgeConnectionMapping(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as WorkflowGraphEdgeConnectionMapping[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update WorkflowGraphEdgeConnectionMapping';
      console.error('Failed to update WorkflowGraphEdgeConnectionMapping:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphEdgeConnectionMappingCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteWorkflowGraphEdgeConnectionMapping(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<WorkflowGraphEdgeConnectionMapping> = { ...source };
      delete (payload as any).id;
      const created = await addWorkflowGraphEdgeConnectionMapping(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphEdgeConnectionMapping, ...prev]);
    } catch (error) {
      console.error('Failed to add WorkflowGraphEdgeConnectionMapping:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<WorkflowGraphEdgeConnectionMapping> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addWorkflowGraphEdgeConnectionMapping(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphEdgeConnectionMapping, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) WorkflowGraphEdgeConnectionMapping:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `WorkflowGraphEdgeConnectionMapping-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWorkflowGraphEdgeConnectionMapping(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWorkflowGraphEdgeConnectionMapping(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as WorkflowGraphEdgeConnectionMapping[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as WorkflowGraphEdgeConnectionMapping[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as WorkflowGraphEdgeConnectionMapping[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as WorkflowGraphEdgeConnectionMapping[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.WorkflowGraphEdgeConnectionMapping',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="WorkflowGraphEdgeConnectionMappingTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="WorkflowGraphEdgeConnectionMapping Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create WorkflowGraphEdgeConnectionMapping</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <WorkflowGraphEdgeConnectionMappingForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter WorkflowGraphEdgeConnectionMapping (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing WorkflowGraphEdgeConnectionMapping</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof WorkflowGraphEdgeConnectionMapping;
                ...
                await updateWorkflowGraphEdgeConnectionMapping(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of WorkflowGraphEdgeConnectionMappings.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'WorkflowGraphEdgeConnectionMapping'}
...
export default WorkflowGraphEdgeConnectionMappingTable;
