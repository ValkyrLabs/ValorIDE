...
import McpDownloadResponseForm from '@thorapi/redux/components/form/McpDownloadResponseForm';
...
import {
  ...
  useDeleteMcpDownloadResponseCascadeMutation
} from '@thorapi/redux/services/McpDownloadResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `McpDownloadResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/McpDownloadResponse/${rowId}`;
...
const McpDownloadResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): McpDownloadResponse[] => {
    if (Array.isArray(result)) return result as McpDownloadResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as McpDownloadResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: McpDownloadResponse[], requestedPage: number) => {
  ...
  const [deleteMcpDownloadResponseCascade] = useDeleteMcpDownloadResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<McpDownloadResponse | null> => {
    ...
    try {
      ...
      console.warn('Could not reload McpDownloadResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<McpDownloadResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMcpDownloadResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as McpDownloadResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update McpDownloadResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpDownloadResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `McpDownloadResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMcpDownloadResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMcpDownloadResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as McpDownloadResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as McpDownloadResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as McpDownloadResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as McpDownloadResponse[]);
    }
  };
