...
import { OasInfo } from '@thorapi/model';
import OasInfoForm from '@thorapi/redux/components/form/OasInfoForm';
...
} from '@thorapi/redux/services/OasInfoService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `OasInfo update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/OasInfo/${rowId}`;
...
const OasInfoTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): OasInfo[] => {
    if (Array.isArray(result)) return result as OasInfo[];
    ...
    return Array.isArray(candidate) ? (candidate as OasInfo[]) : [];
  };

  const mergeFetchedRows = (incomingRows: OasInfo[], requestedPage: number) => {
  ...
  const [deleteOasInfoCascade] = useDeleteOasInfoCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<OasInfo | null> => {
    ...
    try {
      ...
      return (await response.json()) as OasInfo;
    } catch (err) {
      console.warn('Could not reload OasInfo row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<OasInfo>,
    successField?: string,
  ) => {
    try {
      const saved = await updateOasInfo(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as OasInfo[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update OasInfo';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOasInfoCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `OasInfo-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addOasInfo(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateOasInfo(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as OasInfo[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as OasInfo[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as OasInfo[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as OasInfo[]);
    }
  };
