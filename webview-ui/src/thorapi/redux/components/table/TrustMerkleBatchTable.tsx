...
import TrustMerkleBatchForm from '@thorapi/redux/components/form/TrustMerkleBatchForm';
...
import {
  useLazyGetTrustMerkleBatchsPagedQuery,
...
} from '@thorapi/redux/services/TrustMerkleBatchService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TrustMerkleBatch update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TrustMerkleBatch/${rowId}`;

const TrustMerkleBatchTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TrustMerkleBatch[]>([]);
  ...
  const normalizePageRows = (result: unknown): TrustMerkleBatch[] => {
    if (Array.isArray(result)) return result as TrustMerkleBatch[];
    ...
    return Array.isArray(candidate) ? (candidate as TrustMerkleBatch[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TrustMerkleBatch[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addTrustMerkleBatch] = useAddTrustMerkleBatchMutation();
  const [updateTrustMerkleBatch] = useUpdateTrustMerkleBatchMutation();
  const [deleteTrustMerkleBatch] = useDeleteTrustMerkleBatchMutation();
  const [deleteTrustMerkleBatchCascade] = useDeleteTrustMerkleBatchCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TrustMerkleBatch>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TrustMerkleBatch | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TrustMerkleBatch | null> => {
    ...
    try {
      ...
      return (await response.json()) as TrustMerkleBatch;
    } catch (err) {
      console.warn('Could not reload TrustMerkleBatch row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TrustMerkleBatch>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTrustMerkleBatch(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TrustMerkleBatch[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TrustMerkleBatch';
      console.error('Failed to update TrustMerkleBatch:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustMerkleBatchCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTrustMerkleBatch(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TrustMerkleBatch> = { ...source };
      delete (payload as any).id;
      const created = await addTrustMerkleBatch(payload).unwrap();
      setAllData((prev) => [created as TrustMerkleBatch, ...prev]);
    } catch (error) {
      console.error('Failed to add TrustMerkleBatch:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TrustMerkleBatch> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTrustMerkleBatch(payload).unwrap();
      setAllData((prev) => [created as TrustMerkleBatch, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TrustMerkleBatch:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TrustMerkleBatch-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTrustMerkleBatch(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTrustMerkleBatch(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TrustMerkleBatch[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TrustMerkleBatch[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TrustMerkleBatch[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TrustMerkleBatch[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="TrustMerkleBatchTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="TrustMerkleBatch Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TrustMerkleBatch</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TrustMerkleBatchForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TrustMerkleBatch (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TrustMerkleBatch</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TrustMerkleBatch;
                ...
                await updateTrustMerkleBatch(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TrustMerkleBatchs.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TrustMerkleBatch'}
...
export default TrustMerkleBatchTable;
