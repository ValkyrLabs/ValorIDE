...
import { ArrayStringItem } from '@thorapi/model';
import ArrayStringItemForm from '@thorapi/redux/components/form/ArrayStringItemForm';
...
import {
  useLazyGetArrayStringItemsPagedQuery,
...
} from '@thorapi/redux/services/ArrayStringItemService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ArrayStringItem update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ArrayStringItem/${rowId}`;

const ArrayStringItemTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ArrayStringItem[]>([]);
  ...
  const normalizePageRows = (result: unknown): ArrayStringItem[] => {
    if (Array.isArray(result)) return result as ArrayStringItem[];
    ...
    return Array.isArray(candidate) ? (candidate as ArrayStringItem[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ArrayStringItem[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addArrayStringItem] = useAddArrayStringItemMutation();
  const [updateArrayStringItem] = useUpdateArrayStringItemMutation();
  const [deleteArrayStringItem] = useDeleteArrayStringItemMutation();
  const [deleteArrayStringItemCascade] = useDeleteArrayStringItemCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ArrayStringItem>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ArrayStringItem | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ArrayStringItem | null> => {
    ...
    try {
      ...
      return (await response.json()) as ArrayStringItem;
    } catch (err) {
      console.warn('Could not reload ArrayStringItem row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ArrayStringItem>,
    successField?: string,
  ) => {
    try {
      const saved = await updateArrayStringItem(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ArrayStringItem[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ArrayStringItem';
      console.error('Failed to update ArrayStringItem:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteArrayStringItemCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteArrayStringItem(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ArrayStringItem> = { ...source };
      delete (payload as any).id;
      const created = await addArrayStringItem(payload).unwrap();
      setAllData((prev) => [created as ArrayStringItem, ...prev]);
    } catch (error) {
      console.error('Failed to add ArrayStringItem:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ArrayStringItem> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addArrayStringItem(payload).unwrap();
      setAllData((prev) => [created as ArrayStringItem, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ArrayStringItem:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ArrayStringItem-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addArrayStringItem(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateArrayStringItem(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ArrayStringItem[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ArrayStringItem[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ArrayStringItem[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ArrayStringItem[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.ArrayStringItem',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ArrayStringItemTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ArrayStringItem Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ArrayStringItem</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ArrayStringItemForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ArrayStringItem (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ArrayStringItem</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ArrayStringItem;
                ...
                await updateArrayStringItem(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ArrayStringItems.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ArrayStringItem'}
...
export default ArrayStringItemTable;
