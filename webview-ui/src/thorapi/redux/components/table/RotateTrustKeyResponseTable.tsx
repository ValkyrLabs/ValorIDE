...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/RotateTrustKeyResponse/${rowId}`;
...
const RotateTrustKeyResponseTable: React.FC = () => {
  ...
  const [updateRotateTrustKeyResponse] =
    useUpdateRotateTrustKeyResponseMutation();
  const [deleteRotateTrustKeyResponse] =
    useDeleteRotateTrustKeyResponseMutation();
  const [deleteRotateTrustKeyResponseCascade] =
    useDeleteRotateTrustKeyResponseCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<RotateTrustKeyResponse | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<RotateTrustKeyResponse | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateRotateTrustKeyResponse(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update RotateTrustKeyResponse";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRotateTrustKeyResponseCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter RotateTrustKeyResponse (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
