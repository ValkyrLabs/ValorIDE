...
import ExecModuleRateLimitConfigForm from '@thorapi/redux/components/form/ExecModuleRateLimitConfigForm';
...
import {
  useLazyGetExecModuleRateLimitConfigsPagedQuery,
...
} from '@thorapi/redux/services/ExecModuleRateLimitConfigService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ExecModuleRateLimitConfig update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ExecModuleRateLimitConfig/${rowId}`;

const ExecModuleRateLimitConfigTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ExecModuleRateLimitConfig[]>([]);
  ...
  const normalizePageRows = (result: unknown): ExecModuleRateLimitConfig[] => {
    if (Array.isArray(result)) return result as ExecModuleRateLimitConfig[];
    ...
    return Array.isArray(candidate) ? (candidate as ExecModuleRateLimitConfig[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ExecModuleRateLimitConfig[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addExecModuleRateLimitConfig] = useAddExecModuleRateLimitConfigMutation();
  const [updateExecModuleRateLimitConfig] = useUpdateExecModuleRateLimitConfigMutation();
  const [deleteExecModuleRateLimitConfig] = useDeleteExecModuleRateLimitConfigMutation();
  const [deleteExecModuleRateLimitConfigCascade] = useDeleteExecModuleRateLimitConfigCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ExecModuleRateLimitConfig>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ExecModuleRateLimitConfig | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ExecModuleRateLimitConfig | null> => {
    ...
    try {
      ...
      return (await response.json()) as ExecModuleRateLimitConfig;
    } catch (err) {
      console.warn('Could not reload ExecModuleRateLimitConfig row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ExecModuleRateLimitConfig>,
    successField?: string,
  ) => {
    try {
      const saved = await updateExecModuleRateLimitConfig(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ExecModuleRateLimitConfig[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ExecModuleRateLimitConfig';
      console.error('Failed to update ExecModuleRateLimitConfig:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecModuleRateLimitConfigCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteExecModuleRateLimitConfig(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ExecModuleRateLimitConfig> = { ...source };
      delete (payload as any).id;
      const created = await addExecModuleRateLimitConfig(payload).unwrap();
      setAllData((prev) => [created as ExecModuleRateLimitConfig, ...prev]);
    } catch (error) {
      console.error('Failed to add ExecModuleRateLimitConfig:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ExecModuleRateLimitConfig> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addExecModuleRateLimitConfig(payload).unwrap();
      setAllData((prev) => [created as ExecModuleRateLimitConfig, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ExecModuleRateLimitConfig:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ExecModuleRateLimitConfig-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addExecModuleRateLimitConfig(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateExecModuleRateLimitConfig(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ExecModuleRateLimitConfig[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ExecModuleRateLimitConfig[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ExecModuleRateLimitConfig[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ExecModuleRateLimitConfig[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ExecModuleRateLimitConfigTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ExecModuleRateLimitConfig Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ExecModuleRateLimitConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ExecModuleRateLimitConfigForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ExecModuleRateLimitConfig (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ExecModuleRateLimitConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ExecModuleRateLimitConfig;
                ...
                await updateExecModuleRateLimitConfig(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ExecModuleRateLimitConfigs.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ExecModuleRateLimitConfig'}
...
export default ExecModuleRateLimitConfigTable;
