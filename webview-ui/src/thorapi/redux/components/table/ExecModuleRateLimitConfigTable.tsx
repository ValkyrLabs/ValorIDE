...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ExecModuleRateLimitConfig/${rowId}`;
...
const ExecModuleRateLimitConfigTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addExecModuleRateLimitConfig] =
    useAddExecModuleRateLimitConfigMutation();
  const [updateExecModuleRateLimitConfig] =
    useUpdateExecModuleRateLimitConfigMutation();
  const [deleteExecModuleRateLimitConfig] =
    useDeleteExecModuleRateLimitConfigMutation();
  const [deleteExecModuleRateLimitConfigCascade] =
    useDeleteExecModuleRateLimitConfigCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<ExecModuleRateLimitConfig | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<ExecModuleRateLimitConfig | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload ExecModuleRateLimitConfig row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateExecModuleRateLimitConfig(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ExecModuleRateLimitConfig";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecModuleRateLimitConfigCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) ExecModuleRateLimitConfig:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter ExecModuleRateLimitConfig (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
