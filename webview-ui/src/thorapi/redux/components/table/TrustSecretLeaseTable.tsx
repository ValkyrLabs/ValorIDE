...
import { TrustSecretLease } from '@thorapi/model';
import TrustSecretLeaseForm from '@thorapi/redux/components/form/TrustSecretLeaseForm';
...
import {
  useLazyGetTrustSecretLeasesPagedQuery,
...
} from '@thorapi/redux/services/TrustSecretLeaseService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'leaseStatus': { type: 'enum', enumValues: [
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TrustSecretLease update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TrustSecretLease/${rowId}`;

const TrustSecretLeaseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TrustSecretLease[]>([]);
  ...
  const normalizePageRows = (result: unknown): TrustSecretLease[] => {
    if (Array.isArray(result)) return result as TrustSecretLease[];
    ...
    return Array.isArray(candidate) ? (candidate as TrustSecretLease[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TrustSecretLease[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addTrustSecretLease] = useAddTrustSecretLeaseMutation();
  const [updateTrustSecretLease] = useUpdateTrustSecretLeaseMutation();
  const [deleteTrustSecretLease] = useDeleteTrustSecretLeaseMutation();
  const [deleteTrustSecretLeaseCascade] = useDeleteTrustSecretLeaseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TrustSecretLease>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TrustSecretLease | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TrustSecretLease | null> => {
    ...
    try {
      ...
      return (await response.json()) as TrustSecretLease;
    } catch (err) {
      console.warn('Could not reload TrustSecretLease row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TrustSecretLease>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTrustSecretLease(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TrustSecretLease[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TrustSecretLease';
      console.error('Failed to update TrustSecretLease:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustSecretLeaseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTrustSecretLease(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TrustSecretLease> = { ...source };
      delete (payload as any).id;
      const created = await addTrustSecretLease(payload).unwrap();
      setAllData((prev) => [created as TrustSecretLease, ...prev]);
    } catch (error) {
      console.error('Failed to add TrustSecretLease:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TrustSecretLease> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTrustSecretLease(payload).unwrap();
      setAllData((prev) => [created as TrustSecretLease, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TrustSecretLease:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TrustSecretLease-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTrustSecretLease(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTrustSecretLease(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TrustSecretLease[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TrustSecretLease[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TrustSecretLease[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TrustSecretLease[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="TrustSecretLeaseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="TrustSecretLease Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TrustSecretLease</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TrustSecretLeaseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TrustSecretLease (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TrustSecretLease</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TrustSecretLease;
                ...
                await updateTrustSecretLease(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TrustSecretLeases.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TrustSecretLease'}
...
export default TrustSecretLeaseTable;
