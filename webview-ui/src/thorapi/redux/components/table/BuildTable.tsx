...
import { Build } from '@thorapi/model';
import BuildForm from '@thorapi/redux/components/form/BuildForm';
...
} from '@thorapi/redux/services/BuildService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Build update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Build/${rowId}`;
...
const BuildTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Build[] => {
    if (Array.isArray(result)) return result as Build[];
    ...
    return Array.isArray(candidate) ? (candidate as Build[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Build[], requestedPage: number) => {
  ...
  const [deleteBuildCascade] = useDeleteBuildCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Build | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Build row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Build>,
    successField?: string,
  ) => {
    try {
      const saved = await updateBuild(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Build[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Build';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteBuildCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Build-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        try {
          ...
          try {
            await updateBuild(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Build[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Build[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Build[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Build[]);
    }
  };
