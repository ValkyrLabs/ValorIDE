...
import { StartTrustExecutionResponse } from '@thorapi/model';
import StartTrustExecutionResponseForm from '@thorapi/redux/components/form/StartTrustExecutionResponseForm';
...
import {
  useLazyGetStartTrustExecutionResponsesPagedQuery,
...
} from '@thorapi/redux/services/StartTrustExecutionResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `StartTrustExecutionResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/StartTrustExecutionResponse/${rowId}`;

const StartTrustExecutionResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<StartTrustExecutionResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): StartTrustExecutionResponse[] => {
    if (Array.isArray(result)) return result as StartTrustExecutionResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as StartTrustExecutionResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: StartTrustExecutionResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addStartTrustExecutionResponse] = useAddStartTrustExecutionResponseMutation();
  const [updateStartTrustExecutionResponse] = useUpdateStartTrustExecutionResponseMutation();
  const [deleteStartTrustExecutionResponse] = useDeleteStartTrustExecutionResponseMutation();
  const [deleteStartTrustExecutionResponseCascade] = useDeleteStartTrustExecutionResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<StartTrustExecutionResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<StartTrustExecutionResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<StartTrustExecutionResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as StartTrustExecutionResponse;
    } catch (err) {
      console.warn('Could not reload StartTrustExecutionResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<StartTrustExecutionResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateStartTrustExecutionResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as StartTrustExecutionResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update StartTrustExecutionResponse';
      console.error('Failed to update StartTrustExecutionResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteStartTrustExecutionResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteStartTrustExecutionResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<StartTrustExecutionResponse> = { ...source };
      delete (payload as any).id;
      const created = await addStartTrustExecutionResponse(payload).unwrap();
      setAllData((prev) => [created as StartTrustExecutionResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add StartTrustExecutionResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<StartTrustExecutionResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addStartTrustExecutionResponse(payload).unwrap();
      setAllData((prev) => [created as StartTrustExecutionResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) StartTrustExecutionResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `StartTrustExecutionResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addStartTrustExecutionResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateStartTrustExecutionResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as StartTrustExecutionResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as StartTrustExecutionResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as StartTrustExecutionResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as StartTrustExecutionResponse[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.StartTrustExecutionResponse',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="StartTrustExecutionResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="StartTrustExecutionResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create StartTrustExecutionResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <StartTrustExecutionResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter StartTrustExecutionResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing StartTrustExecutionResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof StartTrustExecutionResponse;
                ...
                await updateStartTrustExecutionResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of StartTrustExecutionResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'StartTrustExecutionResponse'}
...
export default StartTrustExecutionResponseTable;
