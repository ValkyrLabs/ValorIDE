...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/StartTrustExecutionResponse/${rowId}`;
...
const StartTrustExecutionResponseTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addStartTrustExecutionResponse] =
    useAddStartTrustExecutionResponseMutation();
  const [updateStartTrustExecutionResponse] =
    useUpdateStartTrustExecutionResponseMutation();
  const [deleteStartTrustExecutionResponse] =
    useDeleteStartTrustExecutionResponseMutation();
  const [deleteStartTrustExecutionResponseCascade] =
    useDeleteStartTrustExecutionResponseCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] =
    useState<StartTrustExecutionResponse | null>(null);
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<StartTrustExecutionResponse | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload StartTrustExecutionResponse row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateStartTrustExecutionResponse(
        updatedItem as any,
      ).unwrap();
      ...
      setAllData(
        (prev) =>
          mergeUpdatedRow(
            ...
            rowId,
            hydrated as any,
          ) as StartTrustExecutionResponse[],
      );
      if (successField) {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update StartTrustExecutionResponse";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteStartTrustExecutionResponseCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) StartTrustExecutionResponse:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter StartTrustExecutionResponse (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
