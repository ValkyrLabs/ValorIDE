...
import GenerateTrustKeyResponseForm from '@thorapi/redux/components/form/GenerateTrustKeyResponseForm';
...
import {
  useLazyGetGenerateTrustKeyResponsesPagedQuery,
...
} from '@thorapi/redux/services/GenerateTrustKeyResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `GenerateTrustKeyResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/GenerateTrustKeyResponse/${rowId}`;

const GenerateTrustKeyResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<GenerateTrustKeyResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): GenerateTrustKeyResponse[] => {
    if (Array.isArray(result)) return result as GenerateTrustKeyResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as GenerateTrustKeyResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: GenerateTrustKeyResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addGenerateTrustKeyResponse] = useAddGenerateTrustKeyResponseMutation();
  const [updateGenerateTrustKeyResponse] = useUpdateGenerateTrustKeyResponseMutation();
  const [deleteGenerateTrustKeyResponse] = useDeleteGenerateTrustKeyResponseMutation();
  const [deleteGenerateTrustKeyResponseCascade] = useDeleteGenerateTrustKeyResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<GenerateTrustKeyResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<GenerateTrustKeyResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<GenerateTrustKeyResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as GenerateTrustKeyResponse;
    } catch (err) {
      console.warn('Could not reload GenerateTrustKeyResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<GenerateTrustKeyResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateGenerateTrustKeyResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as GenerateTrustKeyResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update GenerateTrustKeyResponse';
      console.error('Failed to update GenerateTrustKeyResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteGenerateTrustKeyResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteGenerateTrustKeyResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<GenerateTrustKeyResponse> = { ...source };
      delete (payload as any).id;
      const created = await addGenerateTrustKeyResponse(payload).unwrap();
      setAllData((prev) => [created as GenerateTrustKeyResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add GenerateTrustKeyResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<GenerateTrustKeyResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addGenerateTrustKeyResponse(payload).unwrap();
      setAllData((prev) => [created as GenerateTrustKeyResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) GenerateTrustKeyResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `GenerateTrustKeyResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addGenerateTrustKeyResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateGenerateTrustKeyResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as GenerateTrustKeyResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as GenerateTrustKeyResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as GenerateTrustKeyResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as GenerateTrustKeyResponse[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="GenerateTrustKeyResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="GenerateTrustKeyResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create GenerateTrustKeyResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <GenerateTrustKeyResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter GenerateTrustKeyResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing GenerateTrustKeyResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof GenerateTrustKeyResponse;
                ...
                await updateGenerateTrustKeyResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of GenerateTrustKeyResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'GenerateTrustKeyResponse'}
...
export default GenerateTrustKeyResponseTable;
