...
import { McpServer } from '@thorapi/model';
import McpServerForm from '@thorapi/redux/components/form/McpServerForm';
...
} from '@thorapi/redux/services/McpServerService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `McpServer update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/McpServer/${rowId}`;
...
const McpServerTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): McpServer[] => {
    if (Array.isArray(result)) return result as McpServer[];
    ...
    return Array.isArray(candidate) ? (candidate as McpServer[]) : [];
  };

  const mergeFetchedRows = (incomingRows: McpServer[], requestedPage: number) => {
  ...
  const [deleteMcpServerCascade] = useDeleteMcpServerCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<McpServer | null> => {
    ...
    try {
      ...
      console.warn('Could not reload McpServer row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<McpServer>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMcpServer(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as McpServer[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update McpServer';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpServerCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `McpServer-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMcpServer(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMcpServer(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as McpServer[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as McpServer[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as McpServer[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as McpServer[]);
    }
  };
