...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/DiscardDeadLetterEntryResponse/${rowId}`;
...
const DiscardDeadLetterEntryResponseTable: React.FC = () => {
  ...
  const [deleteDiscardDeadLetterEntryResponse] =
    useDeleteDiscardDeadLetterEntryResponseMutation();
  const [deleteDiscardDeadLetterEntryResponseCascade] =
    useDeleteDiscardDeadLetterEntryResponseCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<DiscardDeadLetterEntryResponse | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload DiscardDeadLetterEntryResponse row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateDiscardDeadLetterEntryResponse(
        updatedItem as any,
      ).unwrap();
      ...
      setAllData(
        (prev) =>
          mergeUpdatedRow(
            ...
            rowId,
            hydrated as any,
          ) as DiscardDeadLetterEntryResponse[],
      );
      if (successField) {
