...
import DiscardDeadLetterEntryResponseForm from '@thorapi/redux/components/form/DiscardDeadLetterEntryResponseForm';
...
} from '@thorapi/redux/services/DiscardDeadLetterEntryResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `DiscardDeadLetterEntryResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/DiscardDeadLetterEntryResponse/${rowId}`;
...
const DiscardDeadLetterEntryResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): DiscardDeadLetterEntryResponse[] => {
    if (Array.isArray(result)) return result as DiscardDeadLetterEntryResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as DiscardDeadLetterEntryResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: DiscardDeadLetterEntryResponse[], requestedPage: number) => {
  ...
  const [deleteDiscardDeadLetterEntryResponseCascade] = useDeleteDiscardDeadLetterEntryResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<DiscardDeadLetterEntryResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as DiscardDeadLetterEntryResponse;
    } catch (err) {
      console.warn('Could not reload DiscardDeadLetterEntryResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<DiscardDeadLetterEntryResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateDiscardDeadLetterEntryResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as DiscardDeadLetterEntryResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update DiscardDeadLetterEntryResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteDiscardDeadLetterEntryResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `DiscardDeadLetterEntryResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addDiscardDeadLetterEntryResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateDiscardDeadLetterEntryResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as DiscardDeadLetterEntryResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as DiscardDeadLetterEntryResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as DiscardDeadLetterEntryResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as DiscardDeadLetterEntryResponse[]);
    }
  };
