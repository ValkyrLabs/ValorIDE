...
import { ManagedMcpService } from '@thorapi/model';
import ManagedMcpServiceForm from '@thorapi/redux/components/form/ManagedMcpServiceForm';
...
import {
  useLazyGetManagedMcpServicesPagedQuery,
...
} from '@thorapi/redux/services/ManagedMcpServiceService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'status': { type: 'enum', enumValues: [
    'DRAFT', 'PUBLISHED', 'MONETIZED', 'SUSPENDED', 
  ], enumValueType: 'string' },
  'pricingModel': { type: 'enum', enumValues: [
    'PER_CALL', 'PER_MONTH', 'TIERED', 'USAGE_BASED', 
  ], enumValueType: 'string' },
  'costPerCall': { type: 'number' },
  'costPerMonth': { type: 'number' },
  'tierName': { type: 'enum', enumValues: [
    'STARTER', 'PRO', 'ENTERPRISE', 
  ], enumValueType: 'string' },
  'hideFromMarketplace': { type: 'boolean' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ManagedMcpService update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ManagedMcpService/${rowId}`;

const ManagedMcpServiceTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ManagedMcpService[]>([]);
  ...
  const normalizePageRows = (result: unknown): ManagedMcpService[] => {
    if (Array.isArray(result)) return result as ManagedMcpService[];
    ...
    return Array.isArray(candidate) ? (candidate as ManagedMcpService[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ManagedMcpService[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addManagedMcpService] = useAddManagedMcpServiceMutation();
  const [updateManagedMcpService] = useUpdateManagedMcpServiceMutation();
  const [deleteManagedMcpService] = useDeleteManagedMcpServiceMutation();
  const [deleteManagedMcpServiceCascade] = useDeleteManagedMcpServiceCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ManagedMcpService>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ManagedMcpService | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ManagedMcpService | null> => {
    ...
    try {
      ...
      return (await response.json()) as ManagedMcpService;
    } catch (err) {
      console.warn('Could not reload ManagedMcpService row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ManagedMcpService>,
    successField?: string,
  ) => {
    try {
      const saved = await updateManagedMcpService(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ManagedMcpService[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ManagedMcpService';
      console.error('Failed to update ManagedMcpService:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteManagedMcpServiceCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteManagedMcpService(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ManagedMcpService> = { ...source };
      delete (payload as any).id;
      const created = await addManagedMcpService(payload).unwrap();
      setAllData((prev) => [created as ManagedMcpService, ...prev]);
    } catch (error) {
      console.error('Failed to add ManagedMcpService:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ManagedMcpService> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addManagedMcpService(payload).unwrap();
      setAllData((prev) => [created as ManagedMcpService, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ManagedMcpService:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ManagedMcpService-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addManagedMcpService(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateManagedMcpService(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ManagedMcpService[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ManagedMcpService[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ManagedMcpService[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ManagedMcpService[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ManagedMcpServiceTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ManagedMcpService Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ManagedMcpService</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ManagedMcpServiceForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ManagedMcpService (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ManagedMcpService</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ManagedMcpService;
                ...
                await updateManagedMcpService(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ManagedMcpServices.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ManagedMcpService'}
...
export default ManagedMcpServiceTable;
