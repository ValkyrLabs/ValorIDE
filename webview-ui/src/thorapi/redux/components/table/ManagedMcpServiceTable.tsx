...
const columnSchema: Record<string, ColumnSchema> = {
  status: {
    type: "enum",
    enumValues: ["DRAFT", "PUBLISHED", "MONETIZED", "SUSPENDED"],
    enumValueType: "string",
  },
  pricingModel: {
    type: "enum",
    enumValues: ["PER_CALL", "PER_MONTH", "TIERED", "USAGE_BASED"],
    enumValueType: "string",
  },
...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ManagedMcpService/${rowId}`;
...
const ManagedMcpServiceTable: React.FC = () => {
  ...
  const [deleteManagedMcpServiceCascade] =
    useDeleteManagedMcpServiceCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<ManagedMcpService | null> => {
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ManagedMcpService";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteManagedMcpServiceCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
