...
import { OpenClawLightsailLaunchEstimate } from '@thorapi/model';
import OpenClawLightsailLaunchEstimateForm from '@thorapi/redux/components/form/OpenClawLightsailLaunchEstimateForm';
...
import {
  useLazyGetOpenClawLightsailLaunchEstimatesPagedQuery,
...
} from '@thorapi/redux/services/OpenClawLightsailLaunchEstimateService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `OpenClawLightsailLaunchEstimate update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/OpenClawLightsailLaunchEstimate/${rowId}`;

const OpenClawLightsailLaunchEstimateTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<OpenClawLightsailLaunchEstimate[]>([]);
  ...
  const normalizePageRows = (result: unknown): OpenClawLightsailLaunchEstimate[] => {
    if (Array.isArray(result)) return result as OpenClawLightsailLaunchEstimate[];
    ...
    return Array.isArray(candidate) ? (candidate as OpenClawLightsailLaunchEstimate[]) : [];
  };

  const mergeFetchedRows = (incomingRows: OpenClawLightsailLaunchEstimate[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addOpenClawLightsailLaunchEstimate] = useAddOpenClawLightsailLaunchEstimateMutation();
  const [updateOpenClawLightsailLaunchEstimate] = useUpdateOpenClawLightsailLaunchEstimateMutation();
  const [deleteOpenClawLightsailLaunchEstimate] = useDeleteOpenClawLightsailLaunchEstimateMutation();
  const [deleteOpenClawLightsailLaunchEstimateCascade] = useDeleteOpenClawLightsailLaunchEstimateCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<OpenClawLightsailLaunchEstimate>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<OpenClawLightsailLaunchEstimate | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<OpenClawLightsailLaunchEstimate | null> => {
    ...
    try {
      ...
      return (await response.json()) as OpenClawLightsailLaunchEstimate;
    } catch (err) {
      console.warn('Could not reload OpenClawLightsailLaunchEstimate row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<OpenClawLightsailLaunchEstimate>,
    successField?: string,
  ) => {
    try {
      const saved = await updateOpenClawLightsailLaunchEstimate(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as OpenClawLightsailLaunchEstimate[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update OpenClawLightsailLaunchEstimate';
      console.error('Failed to update OpenClawLightsailLaunchEstimate:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOpenClawLightsailLaunchEstimateCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteOpenClawLightsailLaunchEstimate(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<OpenClawLightsailLaunchEstimate> = { ...source };
      delete (payload as any).id;
      const created = await addOpenClawLightsailLaunchEstimate(payload).unwrap();
      setAllData((prev) => [created as OpenClawLightsailLaunchEstimate, ...prev]);
    } catch (error) {
      console.error('Failed to add OpenClawLightsailLaunchEstimate:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<OpenClawLightsailLaunchEstimate> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addOpenClawLightsailLaunchEstimate(payload).unwrap();
      setAllData((prev) => [created as OpenClawLightsailLaunchEstimate, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) OpenClawLightsailLaunchEstimate:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `OpenClawLightsailLaunchEstimate-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addOpenClawLightsailLaunchEstimate(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateOpenClawLightsailLaunchEstimate(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as OpenClawLightsailLaunchEstimate[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as OpenClawLightsailLaunchEstimate[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as OpenClawLightsailLaunchEstimate[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as OpenClawLightsailLaunchEstimate[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="OpenClawLightsailLaunchEstimateTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="OpenClawLightsailLaunchEstimate Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create OpenClawLightsailLaunchEstimate</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <OpenClawLightsailLaunchEstimateForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter OpenClawLightsailLaunchEstimate (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing OpenClawLightsailLaunchEstimate</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof OpenClawLightsailLaunchEstimate;
                ...
                await updateOpenClawLightsailLaunchEstimate(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of OpenClawLightsailLaunchEstimates.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'OpenClawLightsailLaunchEstimate'}
...
export default OpenClawLightsailLaunchEstimateTable;
