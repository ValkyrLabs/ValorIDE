...
import { GenerateTrustKeyRequest } from '@thorapi/model';
import GenerateTrustKeyRequestForm from '@thorapi/redux/components/form/GenerateTrustKeyRequestForm';
...
import {
  useLazyGetGenerateTrustKeyRequestsPagedQuery,
...
} from '@thorapi/redux/services/GenerateTrustKeyRequestService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'keyPurpose': { type: 'enum', enumValues: [
    'FIELD_ENCRYPTION', 'EVENT_SIGNING', 'PROOF_SIGNING', 'TOKEN_SIGNING', 'ATTESTATION_SIGNING', 'DEK_WRAPPING', 
  ], enumValueType: 'string' },
  'keyStatus': { type: 'enum', enumValues: [
    'PENDING_ACTIVATION', 'ACTIVE', 'DECRYPT_ONLY', 'SUSPENDED', 'RETIRED', 'DESTROY_SCHEDULED', 'DESTROYED', 
  ], enumValueType: 'string' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `GenerateTrustKeyRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/GenerateTrustKeyRequest/${rowId}`;

const GenerateTrustKeyRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<GenerateTrustKeyRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): GenerateTrustKeyRequest[] => {
    if (Array.isArray(result)) return result as GenerateTrustKeyRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as GenerateTrustKeyRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: GenerateTrustKeyRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addGenerateTrustKeyRequest] = useAddGenerateTrustKeyRequestMutation();
  const [updateGenerateTrustKeyRequest] = useUpdateGenerateTrustKeyRequestMutation();
  const [deleteGenerateTrustKeyRequest] = useDeleteGenerateTrustKeyRequestMutation();
  const [deleteGenerateTrustKeyRequestCascade] = useDeleteGenerateTrustKeyRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<GenerateTrustKeyRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<GenerateTrustKeyRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<GenerateTrustKeyRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as GenerateTrustKeyRequest;
    } catch (err) {
      console.warn('Could not reload GenerateTrustKeyRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<GenerateTrustKeyRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateGenerateTrustKeyRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as GenerateTrustKeyRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update GenerateTrustKeyRequest';
      console.error('Failed to update GenerateTrustKeyRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteGenerateTrustKeyRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteGenerateTrustKeyRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<GenerateTrustKeyRequest> = { ...source };
      delete (payload as any).id;
      const created = await addGenerateTrustKeyRequest(payload).unwrap();
      setAllData((prev) => [created as GenerateTrustKeyRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add GenerateTrustKeyRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<GenerateTrustKeyRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addGenerateTrustKeyRequest(payload).unwrap();
      setAllData((prev) => [created as GenerateTrustKeyRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) GenerateTrustKeyRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `GenerateTrustKeyRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addGenerateTrustKeyRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateGenerateTrustKeyRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as GenerateTrustKeyRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as GenerateTrustKeyRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as GenerateTrustKeyRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as GenerateTrustKeyRequest[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.GenerateTrustKeyRequest',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="GenerateTrustKeyRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="GenerateTrustKeyRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create GenerateTrustKeyRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <GenerateTrustKeyRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter GenerateTrustKeyRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing GenerateTrustKeyRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof GenerateTrustKeyRequest;
                ...
                await updateGenerateTrustKeyRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of GenerateTrustKeyRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'GenerateTrustKeyRequest'}
...
export default GenerateTrustKeyRequestTable;
