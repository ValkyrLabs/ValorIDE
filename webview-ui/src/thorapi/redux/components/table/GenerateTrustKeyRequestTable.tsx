...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/GenerateTrustKeyRequest/${rowId}`;
...
const GenerateTrustKeyRequestTable: React.FC = () => {
  ...
  const [updateGenerateTrustKeyRequest] =
    useUpdateGenerateTrustKeyRequestMutation();
  const [deleteGenerateTrustKeyRequest] =
    useDeleteGenerateTrustKeyRequestMutation();
  const [deleteGenerateTrustKeyRequestCascade] =
    useDeleteGenerateTrustKeyRequestCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<GenerateTrustKeyRequest | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<GenerateTrustKeyRequest | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateGenerateTrustKeyRequest(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update GenerateTrustKeyRequest";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteGenerateTrustKeyRequestCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter GenerateTrustKeyRequest (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
