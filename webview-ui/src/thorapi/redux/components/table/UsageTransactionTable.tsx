...
import UsageTransactionForm from '@thorapi/redux/components/form/UsageTransactionForm';
...
import {
  ...
  useDeleteUsageTransactionCascadeMutation
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `UsageTransaction update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/UsageTransaction/${rowId}`;
...
const UsageTransactionTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): UsageTransaction[] => {
    if (Array.isArray(result)) return result as UsageTransaction[];
    ...
    return Array.isArray(candidate) ? (candidate as UsageTransaction[]) : [];
  };

  const mergeFetchedRows = (incomingRows: UsageTransaction[], requestedPage: number) => {
  ...
  const [deleteUsageTransactionCascade] = useDeleteUsageTransactionCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<UsageTransaction | null> => {
    ...
    try {
      ...
      return (await response.json()) as UsageTransaction;
    } catch (err) {
      console.warn('Could not reload UsageTransaction row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<UsageTransaction>,
    successField?: string,
  ) => {
    try {
      const saved = await updateUsageTransaction(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as UsageTransaction[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update UsageTransaction';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteUsageTransactionCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `UsageTransaction-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addUsageTransaction(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateUsageTransaction(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as UsageTransaction[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as UsageTransaction[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as UsageTransaction[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as UsageTransaction[]);
    }
  };
