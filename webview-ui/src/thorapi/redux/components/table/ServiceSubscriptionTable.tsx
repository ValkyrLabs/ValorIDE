...
const columnSchema: Record<string, ColumnSchema> = {
  subscriptionType: {
    type: "enum",
    enumValues: ["MONTHLY", "YEARLY", "PAY_AS_YOU_GO"],
...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ServiceSubscription/${rowId}`;
...
const ServiceSubscriptionTable: React.FC = () => {
  ...
  const [deleteServiceSubscriptionCascade] =
    useDeleteServiceSubscriptionCascadeMutation();
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateServiceSubscription(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ServiceSubscription";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteServiceSubscriptionCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter ServiceSubscription (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
