...
import { StripeWebhookEvent } from '@thorapi/model';
import StripeWebhookEventForm from '@thorapi/redux/components/form/StripeWebhookEventForm';
...
import {
  useLazyGetStripeWebhookEventsPagedQuery,
...
} from '@thorapi/redux/services/StripeWebhookEventService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'status': { type: 'enum', enumValues: [
    'received', 'processing', 'processed', 'duplicate', 'ignored', 'failed', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `StripeWebhookEvent update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/StripeWebhookEvent/${rowId}`;

const StripeWebhookEventTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<StripeWebhookEvent[]>([]);
  ...
  const normalizePageRows = (result: unknown): StripeWebhookEvent[] => {
    if (Array.isArray(result)) return result as StripeWebhookEvent[];
    ...
    return Array.isArray(candidate) ? (candidate as StripeWebhookEvent[]) : [];
  };

  const mergeFetchedRows = (incomingRows: StripeWebhookEvent[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addStripeWebhookEvent] = useAddStripeWebhookEventMutation();
  const [updateStripeWebhookEvent] = useUpdateStripeWebhookEventMutation();
  const [deleteStripeWebhookEvent] = useDeleteStripeWebhookEventMutation();
  const [deleteStripeWebhookEventCascade] = useDeleteStripeWebhookEventCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<StripeWebhookEvent>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<StripeWebhookEvent | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<StripeWebhookEvent | null> => {
    ...
    try {
      ...
      return (await response.json()) as StripeWebhookEvent;
    } catch (err) {
      console.warn('Could not reload StripeWebhookEvent row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<StripeWebhookEvent>,
    successField?: string,
  ) => {
    try {
      const saved = await updateStripeWebhookEvent(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as StripeWebhookEvent[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update StripeWebhookEvent';
      console.error('Failed to update StripeWebhookEvent:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteStripeWebhookEventCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteStripeWebhookEvent(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<StripeWebhookEvent> = { ...source };
      delete (payload as any).id;
      const created = await addStripeWebhookEvent(payload).unwrap();
      setAllData((prev) => [created as StripeWebhookEvent, ...prev]);
    } catch (error) {
      console.error('Failed to add StripeWebhookEvent:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<StripeWebhookEvent> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addStripeWebhookEvent(payload).unwrap();
      setAllData((prev) => [created as StripeWebhookEvent, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) StripeWebhookEvent:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `StripeWebhookEvent-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addStripeWebhookEvent(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateStripeWebhookEvent(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as StripeWebhookEvent[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as StripeWebhookEvent[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as StripeWebhookEvent[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as StripeWebhookEvent[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="StripeWebhookEventTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="StripeWebhookEvent Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create StripeWebhookEvent</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <StripeWebhookEventForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter StripeWebhookEvent (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing StripeWebhookEvent</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof StripeWebhookEvent;
                ...
                await updateStripeWebhookEvent(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of StripeWebhookEvents.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'StripeWebhookEvent'}
...
export default StripeWebhookEventTable;
