...
import { Deployment } from '@thorapi/model';
import DeploymentForm from '@thorapi/redux/components/form/DeploymentForm';
...
import {
  useLazyGetDeploymentsPagedQuery,
  useAddDeploymentMutation,
  useUpdateDeploymentMutation,
  useDeleteDeploymentMutation,
  useDeleteDeploymentCascadeMutation
} from '@thorapi/redux/services/DeploymentService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'status': { type: 'enum', enumValues: [
    'pending', 'provisioning', 'running', 'updating', 'failed', 'stopped', 'terminated', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Deployment update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Deployment/${rowId}`;
...
const DeploymentTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Deployment[]>([]);
  ...
  const normalizePageRows = (result: unknown): Deployment[] => {
    if (Array.isArray(result)) return result as Deployment[];
    ...
    return Array.isArray(candidate) ? (candidate as Deployment[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Deployment[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addDeployment] = useAddDeploymentMutation();
  const [updateDeployment] = useUpdateDeploymentMutation();
  const [deleteDeployment] = useDeleteDeploymentMutation();
  const [deleteDeploymentCascade] = useDeleteDeploymentCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<Deployment | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<Deployment | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Deployment row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Deployment>,
    successField?: string,
  ) => {
    try {
      const saved = await updateDeployment(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Deployment[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Deployment';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteDeploymentCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteDeployment(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Deployment> = { ...source };
      delete (payload as any).id;
      const created = await addDeployment(payload).unwrap();
      setAllData((prev) => [created as Deployment, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Deployment> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addDeployment(payload).unwrap();
      setAllData((prev) => [created as Deployment, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) Deployment:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Deployment-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addDeployment(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateDeployment(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Deployment[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Deployment[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Deployment[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Deployment[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter Deployment (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing Deployment</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof Deployment;
                ...
                await updateDeployment(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of Deployments.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'Deployment'}
