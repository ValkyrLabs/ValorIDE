...
import { RewrapTrustKeyRequest } from '@thorapi/model';
import RewrapTrustKeyRequestForm from '@thorapi/redux/components/form/RewrapTrustKeyRequestForm';
...
import {
  useLazyGetRewrapTrustKeyRequestsPagedQuery,
...
} from '@thorapi/redux/services/RewrapTrustKeyRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RewrapTrustKeyRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RewrapTrustKeyRequest/${rowId}`;

const RewrapTrustKeyRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RewrapTrustKeyRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): RewrapTrustKeyRequest[] => {
    if (Array.isArray(result)) return result as RewrapTrustKeyRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as RewrapTrustKeyRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RewrapTrustKeyRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addRewrapTrustKeyRequest] = useAddRewrapTrustKeyRequestMutation();
  const [updateRewrapTrustKeyRequest] = useUpdateRewrapTrustKeyRequestMutation();
  const [deleteRewrapTrustKeyRequest] = useDeleteRewrapTrustKeyRequestMutation();
  const [deleteRewrapTrustKeyRequestCascade] = useDeleteRewrapTrustKeyRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<RewrapTrustKeyRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RewrapTrustKeyRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<RewrapTrustKeyRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as RewrapTrustKeyRequest;
    } catch (err) {
      console.warn('Could not reload RewrapTrustKeyRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RewrapTrustKeyRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRewrapTrustKeyRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RewrapTrustKeyRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RewrapTrustKeyRequest';
      console.error('Failed to update RewrapTrustKeyRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRewrapTrustKeyRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteRewrapTrustKeyRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RewrapTrustKeyRequest> = { ...source };
      delete (payload as any).id;
      const created = await addRewrapTrustKeyRequest(payload).unwrap();
      setAllData((prev) => [created as RewrapTrustKeyRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add RewrapTrustKeyRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RewrapTrustKeyRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRewrapTrustKeyRequest(payload).unwrap();
      setAllData((prev) => [created as RewrapTrustKeyRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) RewrapTrustKeyRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RewrapTrustKeyRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRewrapTrustKeyRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRewrapTrustKeyRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RewrapTrustKeyRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RewrapTrustKeyRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RewrapTrustKeyRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RewrapTrustKeyRequest[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.RewrapTrustKeyRequest',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="RewrapTrustKeyRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="RewrapTrustKeyRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RewrapTrustKeyRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RewrapTrustKeyRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RewrapTrustKeyRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RewrapTrustKeyRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RewrapTrustKeyRequest;
                ...
                await updateRewrapTrustKeyRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RewrapTrustKeyRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RewrapTrustKeyRequest'}
...
export default RewrapTrustKeyRequestTable;
