...
import { ContextPageCompileRequest } from '@thorapi/model';
import ContextPageCompileRequestForm from '@thorapi/redux/components/form/ContextPageCompileRequestForm';
...
import {
  useLazyGetContextPageCompileRequestsPagedQuery,
...
} from '@thorapi/redux/services/ContextPageCompileRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ContextPageCompileRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ContextPageCompileRequest/${rowId}`;

const ContextPageCompileRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ContextPageCompileRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): ContextPageCompileRequest[] => {
    if (Array.isArray(result)) return result as ContextPageCompileRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as ContextPageCompileRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ContextPageCompileRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addContextPageCompileRequest] = useAddContextPageCompileRequestMutation();
  const [updateContextPageCompileRequest] = useUpdateContextPageCompileRequestMutation();
  const [deleteContextPageCompileRequest] = useDeleteContextPageCompileRequestMutation();
  const [deleteContextPageCompileRequestCascade] = useDeleteContextPageCompileRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ContextPageCompileRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ContextPageCompileRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ContextPageCompileRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as ContextPageCompileRequest;
    } catch (err) {
      console.warn('Could not reload ContextPageCompileRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ContextPageCompileRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateContextPageCompileRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ContextPageCompileRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ContextPageCompileRequest';
      console.error('Failed to update ContextPageCompileRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteContextPageCompileRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteContextPageCompileRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ContextPageCompileRequest> = { ...source };
      delete (payload as any).id;
      const created = await addContextPageCompileRequest(payload).unwrap();
      setAllData((prev) => [created as ContextPageCompileRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add ContextPageCompileRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ContextPageCompileRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addContextPageCompileRequest(payload).unwrap();
      setAllData((prev) => [created as ContextPageCompileRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ContextPageCompileRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ContextPageCompileRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addContextPageCompileRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateContextPageCompileRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ContextPageCompileRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ContextPageCompileRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ContextPageCompileRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ContextPageCompileRequest[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ContextPageCompileRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ContextPageCompileRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ContextPageCompileRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ContextPageCompileRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ContextPageCompileRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ContextPageCompileRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ContextPageCompileRequest;
                ...
                await updateContextPageCompileRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ContextPageCompileRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ContextPageCompileRequest'}
...
export default ContextPageCompileRequestTable;
