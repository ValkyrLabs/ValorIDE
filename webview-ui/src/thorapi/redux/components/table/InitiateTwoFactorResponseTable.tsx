...
import InitiateTwoFactorResponseForm from '@thorapi/redux/components/form/InitiateTwoFactorResponseForm';
...
import {
  useLazyGetInitiateTwoFactorResponsesPagedQuery,
...
} from '@thorapi/redux/services/InitiateTwoFactorResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `InitiateTwoFactorResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/InitiateTwoFactorResponse/${rowId}`;

const InitiateTwoFactorResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<InitiateTwoFactorResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): InitiateTwoFactorResponse[] => {
    if (Array.isArray(result)) return result as InitiateTwoFactorResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as InitiateTwoFactorResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: InitiateTwoFactorResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addInitiateTwoFactorResponse] = useAddInitiateTwoFactorResponseMutation();
  const [updateInitiateTwoFactorResponse] = useUpdateInitiateTwoFactorResponseMutation();
  const [deleteInitiateTwoFactorResponse] = useDeleteInitiateTwoFactorResponseMutation();
  const [deleteInitiateTwoFactorResponseCascade] = useDeleteInitiateTwoFactorResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<InitiateTwoFactorResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<InitiateTwoFactorResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<InitiateTwoFactorResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as InitiateTwoFactorResponse;
    } catch (err) {
      console.warn('Could not reload InitiateTwoFactorResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<InitiateTwoFactorResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateInitiateTwoFactorResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as InitiateTwoFactorResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update InitiateTwoFactorResponse';
      console.error('Failed to update InitiateTwoFactorResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteInitiateTwoFactorResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteInitiateTwoFactorResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<InitiateTwoFactorResponse> = { ...source };
      delete (payload as any).id;
      const created = await addInitiateTwoFactorResponse(payload).unwrap();
      setAllData((prev) => [created as InitiateTwoFactorResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add InitiateTwoFactorResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<InitiateTwoFactorResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addInitiateTwoFactorResponse(payload).unwrap();
      setAllData((prev) => [created as InitiateTwoFactorResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) InitiateTwoFactorResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `InitiateTwoFactorResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addInitiateTwoFactorResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateInitiateTwoFactorResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as InitiateTwoFactorResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as InitiateTwoFactorResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as InitiateTwoFactorResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as InitiateTwoFactorResponse[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.InitiateTwoFactorResponse',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="InitiateTwoFactorResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="InitiateTwoFactorResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create InitiateTwoFactorResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <InitiateTwoFactorResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter InitiateTwoFactorResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing InitiateTwoFactorResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof InitiateTwoFactorResponse;
                ...
                await updateInitiateTwoFactorResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of InitiateTwoFactorResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'InitiateTwoFactorResponse'}
...
export default InitiateTwoFactorResponseTable;
