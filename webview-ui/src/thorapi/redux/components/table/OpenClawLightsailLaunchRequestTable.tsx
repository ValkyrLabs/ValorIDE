...
import { OpenClawLightsailLaunchRequest } from '@thorapi/model';
import OpenClawLightsailLaunchRequestForm from '@thorapi/redux/components/form/OpenClawLightsailLaunchRequestForm';
...
import {
  useLazyGetOpenClawLightsailLaunchRequestsPagedQuery,
...
} from '@thorapi/redux/services/OpenClawLightsailLaunchRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `OpenClawLightsailLaunchRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/OpenClawLightsailLaunchRequest/${rowId}`;

const OpenClawLightsailLaunchRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<OpenClawLightsailLaunchRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): OpenClawLightsailLaunchRequest[] => {
    if (Array.isArray(result)) return result as OpenClawLightsailLaunchRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as OpenClawLightsailLaunchRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: OpenClawLightsailLaunchRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addOpenClawLightsailLaunchRequest] = useAddOpenClawLightsailLaunchRequestMutation();
  const [updateOpenClawLightsailLaunchRequest] = useUpdateOpenClawLightsailLaunchRequestMutation();
  const [deleteOpenClawLightsailLaunchRequest] = useDeleteOpenClawLightsailLaunchRequestMutation();
  const [deleteOpenClawLightsailLaunchRequestCascade] = useDeleteOpenClawLightsailLaunchRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<OpenClawLightsailLaunchRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<OpenClawLightsailLaunchRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<OpenClawLightsailLaunchRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as OpenClawLightsailLaunchRequest;
    } catch (err) {
      console.warn('Could not reload OpenClawLightsailLaunchRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<OpenClawLightsailLaunchRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateOpenClawLightsailLaunchRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as OpenClawLightsailLaunchRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update OpenClawLightsailLaunchRequest';
      console.error('Failed to update OpenClawLightsailLaunchRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOpenClawLightsailLaunchRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteOpenClawLightsailLaunchRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<OpenClawLightsailLaunchRequest> = { ...source };
      delete (payload as any).id;
      const created = await addOpenClawLightsailLaunchRequest(payload).unwrap();
      setAllData((prev) => [created as OpenClawLightsailLaunchRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add OpenClawLightsailLaunchRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<OpenClawLightsailLaunchRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addOpenClawLightsailLaunchRequest(payload).unwrap();
      setAllData((prev) => [created as OpenClawLightsailLaunchRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) OpenClawLightsailLaunchRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `OpenClawLightsailLaunchRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addOpenClawLightsailLaunchRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateOpenClawLightsailLaunchRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as OpenClawLightsailLaunchRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as OpenClawLightsailLaunchRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as OpenClawLightsailLaunchRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as OpenClawLightsailLaunchRequest[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="OpenClawLightsailLaunchRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="OpenClawLightsailLaunchRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create OpenClawLightsailLaunchRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <OpenClawLightsailLaunchRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter OpenClawLightsailLaunchRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing OpenClawLightsailLaunchRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof OpenClawLightsailLaunchRequest;
                ...
                await updateOpenClawLightsailLaunchRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of OpenClawLightsailLaunchRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'OpenClawLightsailLaunchRequest'}
...
export default OpenClawLightsailLaunchRequestTable;
