...
import { ExecModuleConfig } from '@thorapi/model';
import ExecModuleConfigForm from '@thorapi/redux/components/form/ExecModuleConfigForm';
...
import {
  useLazyGetExecModuleConfigsPagedQuery,
...
} from '@thorapi/redux/services/ExecModuleConfigService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ExecModuleConfig update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ExecModuleConfig/${rowId}`;

const ExecModuleConfigTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ExecModuleConfig[]>([]);
  ...
  const normalizePageRows = (result: unknown): ExecModuleConfig[] => {
    if (Array.isArray(result)) return result as ExecModuleConfig[];
    ...
    return Array.isArray(candidate) ? (candidate as ExecModuleConfig[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ExecModuleConfig[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addExecModuleConfig] = useAddExecModuleConfigMutation();
  const [updateExecModuleConfig] = useUpdateExecModuleConfigMutation();
  const [deleteExecModuleConfig] = useDeleteExecModuleConfigMutation();
  const [deleteExecModuleConfigCascade] = useDeleteExecModuleConfigCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ExecModuleConfig>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ExecModuleConfig | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ExecModuleConfig | null> => {
    ...
    try {
      ...
      return (await response.json()) as ExecModuleConfig;
    } catch (err) {
      console.warn('Could not reload ExecModuleConfig row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ExecModuleConfig>,
    successField?: string,
  ) => {
    try {
      const saved = await updateExecModuleConfig(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ExecModuleConfig[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ExecModuleConfig';
      console.error('Failed to update ExecModuleConfig:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecModuleConfigCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteExecModuleConfig(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ExecModuleConfig> = { ...source };
      delete (payload as any).id;
      const created = await addExecModuleConfig(payload).unwrap();
      setAllData((prev) => [created as ExecModuleConfig, ...prev]);
    } catch (error) {
      console.error('Failed to add ExecModuleConfig:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ExecModuleConfig> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addExecModuleConfig(payload).unwrap();
      setAllData((prev) => [created as ExecModuleConfig, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ExecModuleConfig:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ExecModuleConfig-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addExecModuleConfig(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateExecModuleConfig(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ExecModuleConfig[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ExecModuleConfig[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ExecModuleConfig[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ExecModuleConfig[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.ExecModuleConfig',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ExecModuleConfigTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ExecModuleConfig Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ExecModuleConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ExecModuleConfig (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ExecModuleConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ExecModuleConfig;
                ...
                await updateExecModuleConfig(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ExecModuleConfigs.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ExecModuleConfig'}
...
export default ExecModuleConfigTable;
