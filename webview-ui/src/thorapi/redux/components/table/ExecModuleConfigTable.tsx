...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ExecModuleConfig/${rowId}`;
...
const ExecModuleConfigTable: React.FC = () => {
  ...
  const [deleteExecModuleConfigCascade] =
    useDeleteExecModuleConfigCascadeMutation();
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ExecModuleConfig";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecModuleConfigCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
