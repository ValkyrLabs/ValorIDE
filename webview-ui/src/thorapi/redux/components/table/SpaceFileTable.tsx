...
import SpaceFileForm from '@thorapi/redux/components/form/SpaceFileForm';
...
} from '@thorapi/redux/services/SpaceFileService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SpaceFile update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SpaceFile/${rowId}`;
...
const SpaceFileTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SpaceFile[] => {
    if (Array.isArray(result)) return result as SpaceFile[];
    ...
    return Array.isArray(candidate) ? (candidate as SpaceFile[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SpaceFile[], requestedPage: number) => {
  ...
  const [deleteSpaceFileCascade] = useDeleteSpaceFileCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SpaceFile | null> => {
    ...
    try {
      ...
      console.warn('Could not reload SpaceFile row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SpaceFile>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSpaceFile(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SpaceFile[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SpaceFile';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSpaceFileCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SpaceFile-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSpaceFile(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSpaceFile(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SpaceFile[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SpaceFile[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SpaceFile[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SpaceFile[]);
    }
  };
