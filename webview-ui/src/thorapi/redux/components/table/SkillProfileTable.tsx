...
import { SkillProfile } from '@thorapi/model';
import SkillProfileForm from '@thorapi/redux/components/form/SkillProfileForm';
...
} from '@thorapi/redux/services/SkillProfileService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'proficiency': { type: 'enum', enumValues: [
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SkillProfile update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SkillProfile/${rowId}`;
...
const SkillProfileTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SkillProfile[] => {
    if (Array.isArray(result)) return result as SkillProfile[];
    ...
    return Array.isArray(candidate) ? (candidate as SkillProfile[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SkillProfile[], requestedPage: number) => {
  ...
  const [deleteSkillProfileCascade] = useDeleteSkillProfileCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SkillProfile | null> => {
    ...
    try {
      ...
      return (await response.json()) as SkillProfile;
    } catch (err) {
      console.warn('Could not reload SkillProfile row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SkillProfile>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSkillProfile(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SkillProfile[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SkillProfile';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSkillProfileCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SkillProfile-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSkillProfile(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSkillProfile(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SkillProfile[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SkillProfile[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SkillProfile[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SkillProfile[]);
    }
  };
