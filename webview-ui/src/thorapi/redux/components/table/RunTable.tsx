...
import { Run } from '@thorapi/model';
import RunForm from '@thorapi/redux/components/form/RunForm';
...
} from '@thorapi/redux/services/RunService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Run update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Run/${rowId}`;
...
const RunTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Run[] => {
    if (Array.isArray(result)) return result as Run[];
    ...
    return Array.isArray(candidate) ? (candidate as Run[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Run[], requestedPage: number) => {
  ...
  const [deleteRunCascade] = useDeleteRunCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Run | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Run row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Run>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRun(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Run[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Run';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRunCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Run-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        try {
          ...
          try {
            await updateRun(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Run[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Run[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Run[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Run[]);
    }
  };
