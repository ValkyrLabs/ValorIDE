...
import { TenantSchemaOperation } from '@thorapi/model';
import TenantSchemaOperationForm from '@thorapi/redux/components/form/TenantSchemaOperationForm';
...
import {
  useLazyGetTenantSchemaOperationsPagedQuery,
...
} from '@thorapi/redux/services/TenantSchemaOperationService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'operationType': { type: 'enum', enumValues: [
    'provision', 'validate', 'bind_runtime', 'promote_data_plane', 'apply_bundle', 'schema_reconcile', 'backup', 'export', 'clone', 'suspend', 'restore', 'delete', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TenantSchemaOperation update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TenantSchemaOperation/${rowId}`;

const TenantSchemaOperationTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TenantSchemaOperation[]>([]);
  ...
  const normalizePageRows = (result: unknown): TenantSchemaOperation[] => {
    if (Array.isArray(result)) return result as TenantSchemaOperation[];
    ...
    return Array.isArray(candidate) ? (candidate as TenantSchemaOperation[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TenantSchemaOperation[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addTenantSchemaOperation] = useAddTenantSchemaOperationMutation();
  const [updateTenantSchemaOperation] = useUpdateTenantSchemaOperationMutation();
  const [deleteTenantSchemaOperation] = useDeleteTenantSchemaOperationMutation();
  const [deleteTenantSchemaOperationCascade] = useDeleteTenantSchemaOperationCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TenantSchemaOperation>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TenantSchemaOperation | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TenantSchemaOperation | null> => {
    ...
    try {
      ...
      return (await response.json()) as TenantSchemaOperation;
    } catch (err) {
      console.warn('Could not reload TenantSchemaOperation row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TenantSchemaOperation>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTenantSchemaOperation(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TenantSchemaOperation[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TenantSchemaOperation';
      console.error('Failed to update TenantSchemaOperation:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTenantSchemaOperationCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTenantSchemaOperation(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TenantSchemaOperation> = { ...source };
      delete (payload as any).id;
      const created = await addTenantSchemaOperation(payload).unwrap();
      setAllData((prev) => [created as TenantSchemaOperation, ...prev]);
    } catch (error) {
      console.error('Failed to add TenantSchemaOperation:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TenantSchemaOperation> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTenantSchemaOperation(payload).unwrap();
      setAllData((prev) => [created as TenantSchemaOperation, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TenantSchemaOperation:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TenantSchemaOperation-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTenantSchemaOperation(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTenantSchemaOperation(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TenantSchemaOperation[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TenantSchemaOperation[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TenantSchemaOperation[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TenantSchemaOperation[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="TenantSchemaOperationTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="TenantSchemaOperation Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TenantSchemaOperation</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TenantSchemaOperationForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TenantSchemaOperation (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TenantSchemaOperation</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TenantSchemaOperation;
                ...
                await updateTenantSchemaOperation(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TenantSchemaOperations.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TenantSchemaOperation'}
...
export default TenantSchemaOperationTable;
