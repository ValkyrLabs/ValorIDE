...
import { Chart } from '@thorapi/model';
import ChartForm from '@thorapi/redux/components/form/ChartForm';
...
} from '@thorapi/redux/services/ChartService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Chart update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Chart/${rowId}`;
...
const ChartTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Chart[] => {
    if (Array.isArray(result)) return result as Chart[];
    ...
    return Array.isArray(candidate) ? (candidate as Chart[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Chart[], requestedPage: number) => {
  ...
  const [deleteChartCascade] = useDeleteChartCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Chart | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Chart row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Chart>,
    successField?: string,
  ) => {
    try {
      const saved = await updateChart(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Chart[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Chart';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteChartCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Chart-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        try {
          ...
          try {
            await updateChart(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Chart[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Chart[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Chart[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Chart[]);
    }
  };
