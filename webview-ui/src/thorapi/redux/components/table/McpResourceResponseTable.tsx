...
import McpResourceResponseForm from '@thorapi/redux/components/form/McpResourceResponseForm';
...
import {
  ...
  useDeleteMcpResourceResponseCascadeMutation
} from '@thorapi/redux/services/McpResourceResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `McpResourceResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/McpResourceResponse/${rowId}`;
...
const McpResourceResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): McpResourceResponse[] => {
    if (Array.isArray(result)) return result as McpResourceResponse[];
  ...
  const mergeFetchedRows = (incomingRows: McpResourceResponse[], requestedPage: number) => {
  ...
  const [deleteMcpResourceResponseCascade] = useDeleteMcpResourceResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<McpResourceResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as McpResourceResponse;
    } catch (err) {
      console.warn('Could not reload McpResourceResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<McpResourceResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMcpResourceResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as McpResourceResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update McpResourceResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpResourceResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `McpResourceResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMcpResourceResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMcpResourceResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as McpResourceResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as McpResourceResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as McpResourceResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as McpResourceResponse[]);
    }
  };
