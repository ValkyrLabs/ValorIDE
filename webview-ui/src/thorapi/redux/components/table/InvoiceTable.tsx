...
import { Invoice } from '@thorapi/model';
import InvoiceForm from '@thorapi/redux/components/form/InvoiceForm';
...
} from '@thorapi/redux/services/InvoiceService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Invoice update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Invoice/${rowId}`;
...
const InvoiceTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Invoice[] => {
    if (Array.isArray(result)) return result as Invoice[];
  ...
  const mergeFetchedRows = (incomingRows: Invoice[], requestedPage: number) => {
  ...
  const reloadRowById = async (rowId: string): Promise<Invoice | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Invoice row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Invoice>,
    successField?: string,
  ) => {
    try {
      const saved = await updateInvoice(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Invoice[]);
      if (successField) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteInvoiceCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Invoice-export-${stamp}.json`;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Invoice[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Invoice[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Invoice[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Invoice[]);
    }
  };
