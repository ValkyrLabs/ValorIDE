...
import { BackupConfig } from '@thorapi/model';
import BackupConfigForm from '@thorapi/redux/components/form/BackupConfigForm';
...
} from '@thorapi/redux/services/BackupConfigService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `BackupConfig update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/BackupConfig/${rowId}`;
...
const BackupConfigTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): BackupConfig[] => {
    if (Array.isArray(result)) return result as BackupConfig[];
    ...
    return Array.isArray(candidate) ? (candidate as BackupConfig[]) : [];
  };

  const mergeFetchedRows = (incomingRows: BackupConfig[], requestedPage: number) => {
  ...
  const [deleteBackupConfigCascade] = useDeleteBackupConfigCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<BackupConfig | null> => {
    ...
    try {
      ...
      return (await response.json()) as BackupConfig;
    } catch (err) {
      console.warn('Could not reload BackupConfig row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<BackupConfig>,
    successField?: string,
  ) => {
    try {
      const saved = await updateBackupConfig(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as BackupConfig[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update BackupConfig';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteBackupConfigCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `BackupConfig-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addBackupConfig(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateBackupConfig(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as BackupConfig[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as BackupConfig[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as BackupConfig[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as BackupConfig[]);
    }
  };
