...
import { InitUploadResponse } from '@thorapi/model';
import InitUploadResponseForm from '@thorapi/redux/components/form/InitUploadResponseForm';
...
} from '@thorapi/redux/services/InitUploadResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `InitUploadResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/InitUploadResponse/${rowId}`;
...
const InitUploadResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): InitUploadResponse[] => {
    if (Array.isArray(result)) return result as InitUploadResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as InitUploadResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: InitUploadResponse[], requestedPage: number) => {
  ...
  const [deleteInitUploadResponseCascade] = useDeleteInitUploadResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<InitUploadResponse | null> => {
    ...
    try {
      ...
      console.warn('Could not reload InitUploadResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<InitUploadResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateInitUploadResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as InitUploadResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update InitUploadResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteInitUploadResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `InitUploadResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addInitUploadResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateInitUploadResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as InitUploadResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as InitUploadResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as InitUploadResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as InitUploadResponse[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter InitUploadResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
