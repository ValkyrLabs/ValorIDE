...
import { CompleteUploadRequest } from '@thorapi/model';
import CompleteUploadRequestForm from '@thorapi/redux/components/form/CompleteUploadRequestForm';
...
} from '@thorapi/redux/services/CompleteUploadRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `CompleteUploadRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/CompleteUploadRequest/${rowId}`;
...
const CompleteUploadRequestTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): CompleteUploadRequest[] => {
    if (Array.isArray(result)) return result as CompleteUploadRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as CompleteUploadRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: CompleteUploadRequest[], requestedPage: number) => {
  ...
  const [deleteCompleteUploadRequestCascade] = useDeleteCompleteUploadRequestCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<CompleteUploadRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<CompleteUploadRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as CompleteUploadRequest;
    } catch (err) {
      console.warn('Could not reload CompleteUploadRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<CompleteUploadRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateCompleteUploadRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as CompleteUploadRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update CompleteUploadRequest';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCompleteUploadRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `CompleteUploadRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addCompleteUploadRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateCompleteUploadRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as CompleteUploadRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as CompleteUploadRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as CompleteUploadRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as CompleteUploadRequest[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter CompleteUploadRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
