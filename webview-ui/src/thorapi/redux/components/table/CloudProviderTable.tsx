...
import { CloudProvider } from '@thorapi/model';
import CloudProviderForm from '@thorapi/redux/components/form/CloudProviderForm';
...
import {
  useLazyGetCloudProvidersPagedQuery,
...
} from '@thorapi/redux/services/CloudProviderService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `CloudProvider update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/CloudProvider/${rowId}`;

const CloudProviderTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<CloudProvider[]>([]);
  ...
  const normalizePageRows = (result: unknown): CloudProvider[] => {
    if (Array.isArray(result)) return result as CloudProvider[];
    ...
    return Array.isArray(candidate) ? (candidate as CloudProvider[]) : [];
  };

  const mergeFetchedRows = (incomingRows: CloudProvider[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addCloudProvider] = useAddCloudProviderMutation();
  const [updateCloudProvider] = useUpdateCloudProviderMutation();
  const [deleteCloudProvider] = useDeleteCloudProviderMutation();
  const [deleteCloudProviderCascade] = useDeleteCloudProviderCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<CloudProvider | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<CloudProvider | null> => {
    ...
    try {
      ...
      return (await response.json()) as CloudProvider;
    } catch (err) {
      console.warn('Could not reload CloudProvider row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<CloudProvider>,
    successField?: string,
  ) => {
    try {
      const saved = await updateCloudProvider(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as CloudProvider[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update CloudProvider';
      console.error('Failed to update CloudProvider:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCloudProviderCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteCloudProvider(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<CloudProvider> = { ...source };
      delete (payload as any).id;
      const created = await addCloudProvider(payload).unwrap();
      setAllData((prev) => [created as CloudProvider, ...prev]);
    } catch (error) {
      console.error('Failed to add CloudProvider:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<CloudProvider> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addCloudProvider(payload).unwrap();
      setAllData((prev) => [created as CloudProvider, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) CloudProvider:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `CloudProvider-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addCloudProvider(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateCloudProvider(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as CloudProvider[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as CloudProvider[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as CloudProvider[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as CloudProvider[]);
    }
  };
    ...
    <>
          ...
          >
            ...
            <FloatingControlPanel
              description="CloudProvider Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create CloudProvider</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <CloudProviderForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter CloudProvider (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing CloudProvider</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof CloudProvider;
                ...
                await updateCloudProvider(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of CloudProviders.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'CloudProvider'}
