...
import { SalesPipeline } from '@thorapi/model';
import SalesPipelineForm from '@thorapi/redux/components/form/SalesPipelineForm';
...
} from '@thorapi/redux/services/SalesPipelineService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SalesPipeline update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SalesPipeline/${rowId}`;
...
const SalesPipelineTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SalesPipeline[] => {
    if (Array.isArray(result)) return result as SalesPipeline[];
    ...
    return Array.isArray(candidate) ? (candidate as SalesPipeline[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SalesPipeline[], requestedPage: number) => {
  ...
  const [deleteSalesPipelineCascade] = useDeleteSalesPipelineCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SalesPipeline | null> => {
    ...
    try {
      ...
      return (await response.json()) as SalesPipeline;
    } catch (err) {
      console.warn('Could not reload SalesPipeline row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SalesPipeline>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSalesPipeline(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SalesPipeline[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SalesPipeline';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSalesPipelineCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SalesPipeline-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSalesPipeline(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSalesPipeline(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SalesPipeline[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SalesPipeline[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SalesPipeline[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SalesPipeline[]);
    }
  };
