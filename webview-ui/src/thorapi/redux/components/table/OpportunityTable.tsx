...
import { Opportunity } from '@thorapi/model';
import OpportunityForm from '@thorapi/redux/components/form/OpportunityForm';
...
} from '@thorapi/redux/services/OpportunityService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Opportunity update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Opportunity/${rowId}`;
...
const OpportunityTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Opportunity[] => {
    if (Array.isArray(result)) return result as Opportunity[];
    ...
    return Array.isArray(candidate) ? (candidate as Opportunity[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Opportunity[], requestedPage: number) => {
  ...
  const [deleteOpportunityCascade] = useDeleteOpportunityCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Opportunity | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Opportunity row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Opportunity>,
    successField?: string,
  ) => {
    try {
      const saved = await updateOpportunity(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Opportunity[]);
      if (successField) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOpportunityCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Opportunity-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addOpportunity(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Opportunity[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Opportunity[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Opportunity[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Opportunity[]);
    }
  };
