...
import WorkflowGraphResponseForm from '@thorapi/redux/components/form/WorkflowGraphResponseForm';
...
import {
  useLazyGetWorkflowGraphResponsesPagedQuery,
...
} from '@thorapi/redux/services/WorkflowGraphResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `WorkflowGraphResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/WorkflowGraphResponse/${rowId}`;

const WorkflowGraphResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<WorkflowGraphResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): WorkflowGraphResponse[] => {
    if (Array.isArray(result)) return result as WorkflowGraphResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as WorkflowGraphResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: WorkflowGraphResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addWorkflowGraphResponse] = useAddWorkflowGraphResponseMutation();
  const [updateWorkflowGraphResponse] = useUpdateWorkflowGraphResponseMutation();
  const [deleteWorkflowGraphResponse] = useDeleteWorkflowGraphResponseMutation();
  const [deleteWorkflowGraphResponseCascade] = useDeleteWorkflowGraphResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<WorkflowGraphResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<WorkflowGraphResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<WorkflowGraphResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as WorkflowGraphResponse;
    } catch (err) {
      console.warn('Could not reload WorkflowGraphResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<WorkflowGraphResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWorkflowGraphResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as WorkflowGraphResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update WorkflowGraphResponse';
      console.error('Failed to update WorkflowGraphResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteWorkflowGraphResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<WorkflowGraphResponse> = { ...source };
      delete (payload as any).id;
      const created = await addWorkflowGraphResponse(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add WorkflowGraphResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<WorkflowGraphResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addWorkflowGraphResponse(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) WorkflowGraphResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `WorkflowGraphResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWorkflowGraphResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWorkflowGraphResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as WorkflowGraphResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as WorkflowGraphResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as WorkflowGraphResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as WorkflowGraphResponse[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="WorkflowGraphResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="WorkflowGraphResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create WorkflowGraphResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <WorkflowGraphResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter WorkflowGraphResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing WorkflowGraphResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof WorkflowGraphResponse;
                ...
                await updateWorkflowGraphResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of WorkflowGraphResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'WorkflowGraphResponse'}
...
export default WorkflowGraphResponseTable;
