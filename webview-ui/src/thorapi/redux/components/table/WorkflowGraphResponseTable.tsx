...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/WorkflowGraphResponse/${rowId}`;
...
const WorkflowGraphResponseTable: React.FC = () => {
  ...
  const [updateWorkflowGraphResponse] =
    useUpdateWorkflowGraphResponseMutation();
  const [deleteWorkflowGraphResponse] =
    useDeleteWorkflowGraphResponseMutation();
  const [deleteWorkflowGraphResponseCascade] =
    useDeleteWorkflowGraphResponseCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<WorkflowGraphResponse | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<WorkflowGraphResponse | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateWorkflowGraphResponse(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update WorkflowGraphResponse";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphResponseCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter WorkflowGraphResponse (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
