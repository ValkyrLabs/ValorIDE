...
import { ChatMessage } from '@thorapi/model';
import ChatMessageForm from '@thorapi/redux/components/form/ChatMessageForm';
...
} from '@thorapi/redux/services/ChatMessageService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ChatMessage update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ChatMessage/${rowId}`;
...
const ChatMessageTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ChatMessage[] => {
    if (Array.isArray(result)) return result as ChatMessage[];
    ...
    return Array.isArray(candidate) ? (candidate as ChatMessage[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ChatMessage[], requestedPage: number) => {
  ...
  const [deleteChatMessageCascade] = useDeleteChatMessageCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ChatMessage | null> => {
    ...
    try {
      ...
      console.warn('Could not reload ChatMessage row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ChatMessage>,
    successField?: string,
  ) => {
    try {
      const saved = await updateChatMessage(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ChatMessage[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ChatMessage';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteChatMessageCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ChatMessage-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addChatMessage(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateChatMessage(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ChatMessage[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ChatMessage[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ChatMessage[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ChatMessage[]);
    }
  };
