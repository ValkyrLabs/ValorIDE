...
import { SheetColumn } from '@thorapi/model';
import SheetColumnForm from '@thorapi/redux/components/form/SheetColumnForm';
...
} from '@thorapi/redux/services/SheetColumnService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SheetColumn update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SheetColumn/${rowId}`;
...
const SheetColumnTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SheetColumn[] => {
    if (Array.isArray(result)) return result as SheetColumn[];
    ...
    return Array.isArray(candidate) ? (candidate as SheetColumn[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SheetColumn[], requestedPage: number) => {
  ...
  const [deleteSheetColumnCascade] = useDeleteSheetColumnCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SheetColumn | null> => {
    ...
    try {
      ...
      return (await response.json()) as SheetColumn;
    } catch (err) {
      console.warn('Could not reload SheetColumn row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SheetColumn>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSheetColumn(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SheetColumn[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SheetColumn';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSheetColumnCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SheetColumn-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSheetColumn(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSheetColumn(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SheetColumn[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SheetColumn[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SheetColumn[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SheetColumn[]);
    }
  };
