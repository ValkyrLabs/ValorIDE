...
import { AgentBillingCharge } from '@thorapi/model';
import AgentBillingChargeForm from '@thorapi/redux/components/form/AgentBillingChargeForm';
...
import {
  ...
  useDeleteAgentBillingChargeCascadeMutation
} from '@thorapi/redux/services/AgentBillingChargeService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'chargeType': { type: 'enum', enumValues: [
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AgentBillingCharge update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AgentBillingCharge/${rowId}`;
...
const AgentBillingChargeTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): AgentBillingCharge[] => {
    if (Array.isArray(result)) return result as AgentBillingCharge[];
    ...
    return Array.isArray(candidate) ? (candidate as AgentBillingCharge[]) : [];
  };

  const mergeFetchedRows = (incomingRows: AgentBillingCharge[], requestedPage: number) => {
  ...
  const [deleteAgentBillingChargeCascade] = useDeleteAgentBillingChargeCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<AgentBillingCharge | null> => {
    ...
    try {
      ...
      return (await response.json()) as AgentBillingCharge;
    } catch (err) {
      console.warn('Could not reload AgentBillingCharge row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AgentBillingCharge>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAgentBillingCharge(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AgentBillingCharge[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AgentBillingCharge';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAgentBillingChargeCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AgentBillingCharge-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAgentBillingCharge(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAgentBillingCharge(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AgentBillingCharge[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AgentBillingCharge[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AgentBillingCharge[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AgentBillingCharge[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter AgentBillingCharge (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
