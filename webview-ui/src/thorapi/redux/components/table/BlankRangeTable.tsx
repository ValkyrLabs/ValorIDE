...
import { BlankRange } from '@thorapi/model';
import BlankRangeForm from '@thorapi/redux/components/form/BlankRangeForm';
...
} from '@thorapi/redux/services/BlankRangeService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `BlankRange update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/BlankRange/${rowId}`;
...
const BlankRangeTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): BlankRange[] => {
    if (Array.isArray(result)) return result as BlankRange[];
    ...
    return Array.isArray(candidate) ? (candidate as BlankRange[]) : [];
  };

  const mergeFetchedRows = (incomingRows: BlankRange[], requestedPage: number) => {
  ...
  const [deleteBlankRangeCascade] = useDeleteBlankRangeCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<BlankRange | null> => {
    ...
    try {
      ...
      return (await response.json()) as BlankRange;
    } catch (err) {
      console.warn('Could not reload BlankRange row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<BlankRange>,
    successField?: string,
  ) => {
    try {
      const saved = await updateBlankRange(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as BlankRange[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update BlankRange';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteBlankRangeCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `BlankRange-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addBlankRange(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateBlankRange(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as BlankRange[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as BlankRange[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as BlankRange[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as BlankRange[]);
    }
  };
