...
import { JobMetadata } from '@thorapi/model';
import JobMetadataForm from '@thorapi/redux/components/form/JobMetadataForm';
...
} from '@thorapi/redux/services/JobMetadataService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `JobMetadata update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/JobMetadata/${rowId}`;
...
const JobMetadataTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): JobMetadata[] => {
    if (Array.isArray(result)) return result as JobMetadata[];
    ...
    return Array.isArray(candidate) ? (candidate as JobMetadata[]) : [];
  };

  const mergeFetchedRows = (incomingRows: JobMetadata[], requestedPage: number) => {
  ...
  const [deleteJobMetadataCascade] = useDeleteJobMetadataCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<JobMetadata | null> => {
    ...
    try {
      ...
      console.warn('Could not reload JobMetadata row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<JobMetadata>,
    successField?: string,
  ) => {
    try {
      const saved = await updateJobMetadata(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as JobMetadata[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update JobMetadata';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteJobMetadataCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `JobMetadata-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addJobMetadata(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateJobMetadata(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as JobMetadata[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as JobMetadata[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as JobMetadata[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as JobMetadata[]);
    }
  };
