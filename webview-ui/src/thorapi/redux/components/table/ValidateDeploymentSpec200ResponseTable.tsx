...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ValidateDeploymentSpec200Response/${rowId}`;
...
const ValidateDeploymentSpec200ResponseTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addValidateDeploymentSpec200Response] =
    useAddValidateDeploymentSpec200ResponseMutation();
  const [updateValidateDeploymentSpec200Response] =
    useUpdateValidateDeploymentSpec200ResponseMutation();
  const [deleteValidateDeploymentSpec200Response] =
    useDeleteValidateDeploymentSpec200ResponseMutation();
  const [deleteValidateDeploymentSpec200ResponseCascade] =
    useDeleteValidateDeploymentSpec200ResponseCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] =
    useState<ValidateDeploymentSpec200Response | null>(null);
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<ValidateDeploymentSpec200Response | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload ValidateDeploymentSpec200Response row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateValidateDeploymentSpec200Response(
        updatedItem as any,
      ).unwrap();
      ...
      setAllData(
        (prev) =>
          mergeUpdatedRow(
            ...
            rowId,
            hydrated as any,
          ) as ValidateDeploymentSpec200Response[],
      );
      if (successField) {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ValidateDeploymentSpec200Response";
      console.error(
        "Failed to update ValidateDeploymentSpec200Response:",
        error,
      );
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteValidateDeploymentSpec200ResponseCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handleAddRow = async () => {
    try {
      ...
      setAllData((prev) => [
        created as ValidateDeploymentSpec200Response,
        ...prev,
      ]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ValidateDeploymentSpec200Response> = {
        ...copiedRow,
      };
      delete (payload as any).id;
      const created =
      ...
      console.error(
        "Failed to paste (create) ValidateDeploymentSpec200Response:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter ValidateDeploymentSpec200Response (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
      ...
      <MarkdownEditorModal
        ...
        onSave={async (updatedText) => {
          try {
            ...
            const field =
              richModal.field as keyof ValidateDeploymentSpec200Response;
            if (!rid || !field) {
