...
import { SkillOptRouteResponse } from '@thorapi/model';
import SkillOptRouteResponseForm from '@thorapi/redux/components/form/SkillOptRouteResponseForm';
...
import {
  useLazyGetSkillOptRouteResponsesPagedQuery,
...
} from '@thorapi/redux/services/SkillOptRouteResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SkillOptRouteResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SkillOptRouteResponse/${rowId}`;

const SkillOptRouteResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SkillOptRouteResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): SkillOptRouteResponse[] => {
    if (Array.isArray(result)) return result as SkillOptRouteResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as SkillOptRouteResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SkillOptRouteResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addSkillOptRouteResponse] = useAddSkillOptRouteResponseMutation();
  const [updateSkillOptRouteResponse] = useUpdateSkillOptRouteResponseMutation();
  const [deleteSkillOptRouteResponse] = useDeleteSkillOptRouteResponseMutation();
  const [deleteSkillOptRouteResponseCascade] = useDeleteSkillOptRouteResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<SkillOptRouteResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SkillOptRouteResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<SkillOptRouteResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as SkillOptRouteResponse;
    } catch (err) {
      console.warn('Could not reload SkillOptRouteResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SkillOptRouteResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSkillOptRouteResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SkillOptRouteResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SkillOptRouteResponse';
      console.error('Failed to update SkillOptRouteResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSkillOptRouteResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteSkillOptRouteResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SkillOptRouteResponse> = { ...source };
      delete (payload as any).id;
      const created = await addSkillOptRouteResponse(payload).unwrap();
      setAllData((prev) => [created as SkillOptRouteResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add SkillOptRouteResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SkillOptRouteResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSkillOptRouteResponse(payload).unwrap();
      setAllData((prev) => [created as SkillOptRouteResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) SkillOptRouteResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SkillOptRouteResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSkillOptRouteResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSkillOptRouteResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SkillOptRouteResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SkillOptRouteResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SkillOptRouteResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SkillOptRouteResponse[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="SkillOptRouteResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="SkillOptRouteResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SkillOptRouteResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SkillOptRouteResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SkillOptRouteResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SkillOptRouteResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SkillOptRouteResponse;
                ...
                await updateSkillOptRouteResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SkillOptRouteResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SkillOptRouteResponse'}
...
export default SkillOptRouteResponseTable;
