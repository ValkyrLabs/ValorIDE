...
import { Task } from '@thorapi/model';
import TaskForm from '@thorapi/redux/components/form/TaskForm';
...
} from '@thorapi/redux/services/TaskService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'role': { type: 'enum', enumValues: [
    'user', 'assistant', 'system', 'admin', 'operator', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Task update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Task/${rowId}`;
...
const TaskTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Task[] => {
    if (Array.isArray(result)) return result as Task[];
    ...
    return Array.isArray(candidate) ? (candidate as Task[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Task[], requestedPage: number) => {
  ...
  const [deleteTaskCascade] = useDeleteTaskCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Task | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Task row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Task>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTask(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Task[]);
      if (successField) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTaskCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Task-export-${stamp}.json`;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Task[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Task[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Task[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Task[]);
    }
  };
