...
import { MemoryStats } from '@thorapi/model';
import MemoryStatsForm from '@thorapi/redux/components/form/MemoryStatsForm';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `MemoryStats update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/MemoryStats/${rowId}`;

const MemoryStatsTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<MemoryStats[]>([]);
  ...
  const normalizePageRows = (result: unknown): MemoryStats[] => {
    if (Array.isArray(result)) return result as MemoryStats[];
    ...
    return Array.isArray(candidate) ? (candidate as MemoryStats[]) : [];
  };

  const mergeFetchedRows = (incomingRows: MemoryStats[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addMemoryStats] = useAddMemoryStatsMutation();
  const [updateMemoryStats] = useUpdateMemoryStatsMutation();
  const [deleteMemoryStats] = useDeleteMemoryStatsMutation();
  const [deleteMemoryStatsCascade] = useDeleteMemoryStatsCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<MemoryStats>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<MemoryStats | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<MemoryStats | null> => {
    ...
    try {
      ...
      console.warn('Could not reload MemoryStats row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<MemoryStats>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMemoryStats(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as MemoryStats[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update MemoryStats';
      console.error('Failed to update MemoryStats:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMemoryStatsCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteMemoryStats(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<MemoryStats> = { ...source };
      delete (payload as any).id;
      const created = await addMemoryStats(payload).unwrap();
      setAllData((prev) => [created as MemoryStats, ...prev]);
    } catch (error) {
      console.error('Failed to add MemoryStats:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<MemoryStats> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMemoryStats(payload).unwrap();
      setAllData((prev) => [created as MemoryStats, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) MemoryStats:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `MemoryStats-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMemoryStats(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMemoryStats(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as MemoryStats[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as MemoryStats[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as MemoryStats[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as MemoryStats[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.MemoryStats',
        objectId: rowId,
      });
    ...
    <>
          ...
          >
            ...
            <FloatingControlPanel
              description="MemoryStats Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create MemoryStats</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <MemoryStatsForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter MemoryStats (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing MemoryStats</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof MemoryStats;
                ...
                await updateMemoryStats(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of MemoryStatss.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'MemoryStats'}
...
export default MemoryStatsTable;
