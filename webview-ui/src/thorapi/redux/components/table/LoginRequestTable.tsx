...
import { LoginRequest } from '@thorapi/model';
import LoginRequestForm from '@thorapi/redux/components/form/LoginRequestForm';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `LoginRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/LoginRequest/${rowId}`;

const LoginRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<LoginRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): LoginRequest[] => {
    if (Array.isArray(result)) return result as LoginRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as LoginRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: LoginRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addLoginRequest] = useAddLoginRequestMutation();
  const [updateLoginRequest] = useUpdateLoginRequestMutation();
  const [deleteLoginRequest] = useDeleteLoginRequestMutation();
  const [deleteLoginRequestCascade] = useDeleteLoginRequestCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<LoginRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<LoginRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as LoginRequest;
    } catch (err) {
      console.warn('Could not reload LoginRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<LoginRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateLoginRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as LoginRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update LoginRequest';
      console.error('Failed to update LoginRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteLoginRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteLoginRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<LoginRequest> = { ...source };
      delete (payload as any).id;
      const created = await addLoginRequest(payload).unwrap();
      setAllData((prev) => [created as LoginRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add LoginRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<LoginRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addLoginRequest(payload).unwrap();
      setAllData((prev) => [created as LoginRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) LoginRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `LoginRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addLoginRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateLoginRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as LoginRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as LoginRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as LoginRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as LoginRequest[]);
    }
  };
    ...
    <>
          ...
          >
            ...
            <FloatingControlPanel
              description="LoginRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create LoginRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <LoginRequestForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter LoginRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing LoginRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof LoginRequest;
                ...
                await updateLoginRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of LoginRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'LoginRequest'}
