...
import ApiTrafficEventForm from '@thorapi/redux/components/form/ApiTrafficEventForm';
...
} from '@thorapi/redux/services/ApiTrafficEventService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ApiTrafficEvent update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ApiTrafficEvent/${rowId}`;
...
const ApiTrafficEventTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ApiTrafficEvent[] => {
    if (Array.isArray(result)) return result as ApiTrafficEvent[];
    ...
    return Array.isArray(candidate) ? (candidate as ApiTrafficEvent[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ApiTrafficEvent[], requestedPage: number) => {
  ...
  const [deleteApiTrafficEventCascade] = useDeleteApiTrafficEventCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ApiTrafficEvent | null> => {
    ...
    try {
      ...
      return (await response.json()) as ApiTrafficEvent;
    } catch (err) {
      console.warn('Could not reload ApiTrafficEvent row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ApiTrafficEvent>,
    successField?: string,
  ) => {
    try {
      const saved = await updateApiTrafficEvent(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ApiTrafficEvent[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ApiTrafficEvent';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteApiTrafficEventCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ApiTrafficEvent-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addApiTrafficEvent(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateApiTrafficEvent(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ApiTrafficEvent[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ApiTrafficEvent[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ApiTrafficEvent[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ApiTrafficEvent[]);
    }
  };
