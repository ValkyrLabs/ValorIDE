...
import { AppGenerationTraceResponse } from '@thorapi/model';
import AppGenerationTraceResponseForm from '@thorapi/redux/components/form/AppGenerationTraceResponseForm';
...
import {
  useLazyGetAppGenerationTraceResponsesPagedQuery,
...
} from '@thorapi/redux/services/AppGenerationTraceResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AppGenerationTraceResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AppGenerationTraceResponse/${rowId}`;

const AppGenerationTraceResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<AppGenerationTraceResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): AppGenerationTraceResponse[] => {
    if (Array.isArray(result)) return result as AppGenerationTraceResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as AppGenerationTraceResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: AppGenerationTraceResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addAppGenerationTraceResponse] = useAddAppGenerationTraceResponseMutation();
  const [updateAppGenerationTraceResponse] = useUpdateAppGenerationTraceResponseMutation();
  const [deleteAppGenerationTraceResponse] = useDeleteAppGenerationTraceResponseMutation();
  const [deleteAppGenerationTraceResponseCascade] = useDeleteAppGenerationTraceResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<AppGenerationTraceResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<AppGenerationTraceResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<AppGenerationTraceResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as AppGenerationTraceResponse;
    } catch (err) {
      console.warn('Could not reload AppGenerationTraceResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AppGenerationTraceResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAppGenerationTraceResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AppGenerationTraceResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AppGenerationTraceResponse';
      console.error('Failed to update AppGenerationTraceResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAppGenerationTraceResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteAppGenerationTraceResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<AppGenerationTraceResponse> = { ...source };
      delete (payload as any).id;
      const created = await addAppGenerationTraceResponse(payload).unwrap();
      setAllData((prev) => [created as AppGenerationTraceResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add AppGenerationTraceResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<AppGenerationTraceResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addAppGenerationTraceResponse(payload).unwrap();
      setAllData((prev) => [created as AppGenerationTraceResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) AppGenerationTraceResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AppGenerationTraceResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAppGenerationTraceResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAppGenerationTraceResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AppGenerationTraceResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AppGenerationTraceResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AppGenerationTraceResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AppGenerationTraceResponse[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="AppGenerationTraceResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="AppGenerationTraceResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create AppGenerationTraceResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <AppGenerationTraceResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter AppGenerationTraceResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing AppGenerationTraceResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof AppGenerationTraceResponse;
                ...
                await updateAppGenerationTraceResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of AppGenerationTraceResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'AppGenerationTraceResponse'}
...
export default AppGenerationTraceResponseTable;
