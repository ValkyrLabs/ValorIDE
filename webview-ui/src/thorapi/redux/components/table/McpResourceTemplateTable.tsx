...
import { McpResourceTemplate } from '@thorapi/model';
import McpResourceTemplateForm from '@thorapi/redux/components/form/McpResourceTemplateForm';
...
import {
  ...
  useDeleteMcpResourceTemplateCascadeMutation
} from '@thorapi/redux/services/McpResourceTemplateService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `McpResourceTemplate update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/McpResourceTemplate/${rowId}`;
...
const McpResourceTemplateTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): McpResourceTemplate[] => {
    if (Array.isArray(result)) return result as McpResourceTemplate[];
    ...
    return Array.isArray(candidate) ? (candidate as McpResourceTemplate[]) : [];
  };

  const mergeFetchedRows = (incomingRows: McpResourceTemplate[], requestedPage: number) => {
  ...
  const [deleteMcpResourceTemplateCascade] = useDeleteMcpResourceTemplateCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<McpResourceTemplate | null> => {
    ...
    try {
      ...
      return (await response.json()) as McpResourceTemplate;
    } catch (err) {
      console.warn('Could not reload McpResourceTemplate row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<McpResourceTemplate>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMcpResourceTemplate(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as McpResourceTemplate[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update McpResourceTemplate';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpResourceTemplateCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `McpResourceTemplate-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMcpResourceTemplate(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMcpResourceTemplate(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as McpResourceTemplate[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as McpResourceTemplate[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as McpResourceTemplate[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as McpResourceTemplate[]);
    }
  };
