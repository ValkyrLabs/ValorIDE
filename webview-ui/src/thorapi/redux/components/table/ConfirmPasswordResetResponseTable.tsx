...
import { ConfirmPasswordResetResponse } from '@thorapi/model';
import ConfirmPasswordResetResponseForm from '@thorapi/redux/components/form/ConfirmPasswordResetResponseForm';
...
import {
  useLazyGetConfirmPasswordResetResponsesPagedQuery,
...
} from '@thorapi/redux/services/ConfirmPasswordResetResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ConfirmPasswordResetResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ConfirmPasswordResetResponse/${rowId}`;

const ConfirmPasswordResetResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ConfirmPasswordResetResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): ConfirmPasswordResetResponse[] => {
    if (Array.isArray(result)) return result as ConfirmPasswordResetResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as ConfirmPasswordResetResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ConfirmPasswordResetResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addConfirmPasswordResetResponse] = useAddConfirmPasswordResetResponseMutation();
  const [updateConfirmPasswordResetResponse] = useUpdateConfirmPasswordResetResponseMutation();
  const [deleteConfirmPasswordResetResponse] = useDeleteConfirmPasswordResetResponseMutation();
  const [deleteConfirmPasswordResetResponseCascade] = useDeleteConfirmPasswordResetResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ConfirmPasswordResetResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ConfirmPasswordResetResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ConfirmPasswordResetResponse | null> => {
    ...
    try {
      ...
      console.warn('Could not reload ConfirmPasswordResetResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ConfirmPasswordResetResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateConfirmPasswordResetResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ConfirmPasswordResetResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ConfirmPasswordResetResponse';
      console.error('Failed to update ConfirmPasswordResetResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteConfirmPasswordResetResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteConfirmPasswordResetResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ConfirmPasswordResetResponse> = { ...source };
      delete (payload as any).id;
      const created = await addConfirmPasswordResetResponse(payload).unwrap();
      setAllData((prev) => [created as ConfirmPasswordResetResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add ConfirmPasswordResetResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ConfirmPasswordResetResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addConfirmPasswordResetResponse(payload).unwrap();
      setAllData((prev) => [created as ConfirmPasswordResetResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ConfirmPasswordResetResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ConfirmPasswordResetResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addConfirmPasswordResetResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateConfirmPasswordResetResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ConfirmPasswordResetResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ConfirmPasswordResetResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ConfirmPasswordResetResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ConfirmPasswordResetResponse[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.ConfirmPasswordResetResponse',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ConfirmPasswordResetResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ConfirmPasswordResetResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ConfirmPasswordResetResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ConfirmPasswordResetResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ConfirmPasswordResetResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ConfirmPasswordResetResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ConfirmPasswordResetResponse;
                ...
                await updateConfirmPasswordResetResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ConfirmPasswordResetResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ConfirmPasswordResetResponse'}
...
export default ConfirmPasswordResetResponseTable;
