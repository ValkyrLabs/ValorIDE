...
import { KeyMetric } from '@thorapi/model';
import KeyMetricForm from '@thorapi/redux/components/form/KeyMetricForm';
...
} from '@thorapi/redux/services/KeyMetricService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `KeyMetric update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/KeyMetric/${rowId}`;
...
const KeyMetricTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): KeyMetric[] => {
    if (Array.isArray(result)) return result as KeyMetric[];
    ...
    return Array.isArray(candidate) ? (candidate as KeyMetric[]) : [];
  };

  const mergeFetchedRows = (incomingRows: KeyMetric[], requestedPage: number) => {
  ...
  const [deleteKeyMetricCascade] = useDeleteKeyMetricCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<KeyMetric | null> => {
    ...
    try {
      ...
      return (await response.json()) as KeyMetric;
    } catch (err) {
      console.warn('Could not reload KeyMetric row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<KeyMetric>,
    successField?: string,
  ) => {
    try {
      const saved = await updateKeyMetric(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as KeyMetric[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update KeyMetric';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteKeyMetricCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `KeyMetric-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addKeyMetric(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateKeyMetric(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as KeyMetric[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as KeyMetric[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as KeyMetric[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as KeyMetric[]);
    }
  };
