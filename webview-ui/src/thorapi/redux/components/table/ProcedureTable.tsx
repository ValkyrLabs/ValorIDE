...
import { Procedure } from '@thorapi/model';
import ProcedureForm from '@thorapi/redux/components/form/ProcedureForm';
...
import {
  useLazyGetProceduresPagedQuery,
  useAddProcedureMutation,
  useUpdateProcedureMutation,
  useDeleteProcedureMutation,
  useDeleteProcedureCascadeMutation
} from '@thorapi/redux/services/ProcedureService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Procedure update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Procedure/${rowId}`;
...
const ProcedureTable: React.FC = () => {
  ...
  const [allData, setAllData] = useState<Procedure[]>([]);
  ...
  const normalizePageRows = (result: unknown): Procedure[] => {
    if (Array.isArray(result)) return result as Procedure[];
    ...
    return Array.isArray(candidate) ? (candidate as Procedure[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Procedure[], requestedPage: number) => {
  ...
  const [deleteProcedureCascade] = useDeleteProcedureCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<Procedure | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<Procedure | null> => {
    ...
    try {
      ...
      return (await response.json()) as Procedure;
    } catch (err) {
      console.warn('Could not reload Procedure row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Procedure>,
    successField?: string,
  ) => {
    try {
      const saved = await updateProcedure(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Procedure[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Procedure';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteProcedureCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteProcedure(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<Procedure> = { ...source };
      delete (payload as any).id;
      const created = await addProcedure(payload).unwrap();
      setAllData((prev) => [created as Procedure, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<Procedure> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addProcedure(payload).unwrap();
      setAllData((prev) => [created as Procedure, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) Procedure:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Procedure-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addProcedure(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateProcedure(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Procedure[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Procedure[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Procedure[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Procedure[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter Procedure (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing Procedure</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof Procedure;
                ...
                await updateProcedure(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of Procedures.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'Procedure'}
