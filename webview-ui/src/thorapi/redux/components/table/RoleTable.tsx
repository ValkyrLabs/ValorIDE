...
import { Role } from '@thorapi/model';
import RoleForm from '@thorapi/redux/components/form/RoleForm';
...
} from '@thorapi/redux/services/RoleService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Role update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Role/${rowId}`;
...
const RoleTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Role[] => {
    if (Array.isArray(result)) return result as Role[];
    ...
    return Array.isArray(candidate) ? (candidate as Role[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Role[], requestedPage: number) => {
  ...
  const reloadRowById = async (rowId: string): Promise<Role | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Role row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Role>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRole(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Role[]);
      if (successField) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRoleCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Role-export-${stamp}.json`;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Role[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Role[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Role[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Role[]);
    }
  };
