...
import { RefundRecord } from '@thorapi/model';
import RefundRecordForm from '@thorapi/redux/components/form/RefundRecordForm';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RefundRecord update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RefundRecord/${rowId}`;

const RefundRecordTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RefundRecord[]>([]);
  ...
  const normalizePageRows = (result: unknown): RefundRecord[] => {
    if (Array.isArray(result)) return result as RefundRecord[];
    ...
    return Array.isArray(candidate) ? (candidate as RefundRecord[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RefundRecord[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addRefundRecord] = useAddRefundRecordMutation();
  const [updateRefundRecord] = useUpdateRefundRecordMutation();
  const [deleteRefundRecord] = useDeleteRefundRecordMutation();
  const [deleteRefundRecordCascade] = useDeleteRefundRecordCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<RefundRecord>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RefundRecord | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<RefundRecord | null> => {
    ...
    try {
      ...
      return (await response.json()) as RefundRecord;
    } catch (err) {
      console.warn('Could not reload RefundRecord row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RefundRecord>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRefundRecord(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RefundRecord[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RefundRecord';
      console.error('Failed to update RefundRecord:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRefundRecordCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteRefundRecord(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RefundRecord> = { ...source };
      delete (payload as any).id;
      const created = await addRefundRecord(payload).unwrap();
      setAllData((prev) => [created as RefundRecord, ...prev]);
    } catch (error) {
      console.error('Failed to add RefundRecord:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RefundRecord> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRefundRecord(payload).unwrap();
      setAllData((prev) => [created as RefundRecord, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) RefundRecord:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RefundRecord-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRefundRecord(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRefundRecord(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RefundRecord[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RefundRecord[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RefundRecord[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RefundRecord[]);
    }
  };
    ...
    <>
          ...
          >
            ...
            <FloatingControlPanel
              description="RefundRecord Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RefundRecord</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RefundRecordForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RefundRecord (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RefundRecord</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RefundRecord;
                ...
                await updateRefundRecord(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RefundRecords.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RefundRecord'}
