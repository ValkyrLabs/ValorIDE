...
import { ContextPageHydrateRequest } from '@thorapi/model';
import ContextPageHydrateRequestForm from '@thorapi/redux/components/form/ContextPageHydrateRequestForm';
...
import {
  useLazyGetContextPageHydrateRequestsPagedQuery,
...
} from '@thorapi/redux/services/ContextPageHydrateRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ContextPageHydrateRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ContextPageHydrateRequest/${rowId}`;

const ContextPageHydrateRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ContextPageHydrateRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): ContextPageHydrateRequest[] => {
    if (Array.isArray(result)) return result as ContextPageHydrateRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as ContextPageHydrateRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ContextPageHydrateRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addContextPageHydrateRequest] = useAddContextPageHydrateRequestMutation();
  const [updateContextPageHydrateRequest] = useUpdateContextPageHydrateRequestMutation();
  const [deleteContextPageHydrateRequest] = useDeleteContextPageHydrateRequestMutation();
  const [deleteContextPageHydrateRequestCascade] = useDeleteContextPageHydrateRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ContextPageHydrateRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ContextPageHydrateRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ContextPageHydrateRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as ContextPageHydrateRequest;
    } catch (err) {
      console.warn('Could not reload ContextPageHydrateRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ContextPageHydrateRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateContextPageHydrateRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ContextPageHydrateRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ContextPageHydrateRequest';
      console.error('Failed to update ContextPageHydrateRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteContextPageHydrateRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteContextPageHydrateRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ContextPageHydrateRequest> = { ...source };
      delete (payload as any).id;
      const created = await addContextPageHydrateRequest(payload).unwrap();
      setAllData((prev) => [created as ContextPageHydrateRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add ContextPageHydrateRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ContextPageHydrateRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addContextPageHydrateRequest(payload).unwrap();
      setAllData((prev) => [created as ContextPageHydrateRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ContextPageHydrateRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ContextPageHydrateRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addContextPageHydrateRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateContextPageHydrateRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ContextPageHydrateRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ContextPageHydrateRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ContextPageHydrateRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ContextPageHydrateRequest[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.ContextPageHydrateRequest',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ContextPageHydrateRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ContextPageHydrateRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ContextPageHydrateRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ContextPageHydrateRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ContextPageHydrateRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ContextPageHydrateRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ContextPageHydrateRequest;
                ...
                await updateContextPageHydrateRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ContextPageHydrateRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ContextPageHydrateRequest'}
...
export default ContextPageHydrateRequestTable;
