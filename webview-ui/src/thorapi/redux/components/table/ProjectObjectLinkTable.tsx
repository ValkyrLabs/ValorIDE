...
import { ProjectObjectLink } from '@thorapi/model';
import ProjectObjectLinkForm from '@thorapi/redux/components/form/ProjectObjectLinkForm';
...
import {
  useLazyGetProjectObjectLinksPagedQuery,
...
} from '@thorapi/redux/services/ProjectObjectLinkService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'objectType': { type: 'enum', enumValues: [
    'Application', 'Workflow', 'Task', 'Goal', 'Deployment', 'Mcp', 'MemoryEntry', 'OasOpenAPISpec', 'Workbook', 'ContentData', 'Customer', 'Product', 'IntegrationAccount', 'External', 
  ], enumValueType: 'string' },
  'relation': { type: 'enum', enumValues: [
    'achieved', 'todo', 'dependency', 'reference', 'generated-artifact', 'evidence', 
  ], enumValueType: 'string' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ProjectObjectLink update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ProjectObjectLink/${rowId}`;

const ProjectObjectLinkTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ProjectObjectLink[]>([]);
  ...
  const normalizePageRows = (result: unknown): ProjectObjectLink[] => {
    if (Array.isArray(result)) return result as ProjectObjectLink[];
    ...
    return Array.isArray(candidate) ? (candidate as ProjectObjectLink[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ProjectObjectLink[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addProjectObjectLink] = useAddProjectObjectLinkMutation();
  const [updateProjectObjectLink] = useUpdateProjectObjectLinkMutation();
  const [deleteProjectObjectLink] = useDeleteProjectObjectLinkMutation();
  const [deleteProjectObjectLinkCascade] = useDeleteProjectObjectLinkCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ProjectObjectLink>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ProjectObjectLink | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ProjectObjectLink | null> => {
    ...
    try {
      ...
      return (await response.json()) as ProjectObjectLink;
    } catch (err) {
      console.warn('Could not reload ProjectObjectLink row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ProjectObjectLink>,
    successField?: string,
  ) => {
    try {
      const saved = await updateProjectObjectLink(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ProjectObjectLink[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ProjectObjectLink';
      console.error('Failed to update ProjectObjectLink:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteProjectObjectLinkCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteProjectObjectLink(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ProjectObjectLink> = { ...source };
      delete (payload as any).id;
      const created = await addProjectObjectLink(payload).unwrap();
      setAllData((prev) => [created as ProjectObjectLink, ...prev]);
    } catch (error) {
      console.error('Failed to add ProjectObjectLink:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ProjectObjectLink> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addProjectObjectLink(payload).unwrap();
      setAllData((prev) => [created as ProjectObjectLink, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ProjectObjectLink:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ProjectObjectLink-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addProjectObjectLink(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateProjectObjectLink(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ProjectObjectLink[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ProjectObjectLink[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ProjectObjectLink[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ProjectObjectLink[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ProjectObjectLinkTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ProjectObjectLink Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ProjectObjectLink</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ProjectObjectLinkForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ProjectObjectLink (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ProjectObjectLink</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ProjectObjectLink;
                ...
                await updateProjectObjectLink(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ProjectObjectLinks.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ProjectObjectLink'}
...
export default ProjectObjectLinkTable;
