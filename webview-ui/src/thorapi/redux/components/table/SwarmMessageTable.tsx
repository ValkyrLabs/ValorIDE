...
import { SwarmMessage } from '@thorapi/model';
import SwarmMessageForm from '@thorapi/redux/components/form/SwarmMessageForm';
...
} from '@thorapi/redux/services/SwarmMessageService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SwarmMessage update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SwarmMessage/${rowId}`;
...
const SwarmMessageTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SwarmMessage[] => {
    if (Array.isArray(result)) return result as SwarmMessage[];
    ...
    return Array.isArray(candidate) ? (candidate as SwarmMessage[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SwarmMessage[], requestedPage: number) => {
  ...
  const [deleteSwarmMessageCascade] = useDeleteSwarmMessageCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SwarmMessage | null> => {
    ...
    try {
      ...
      return (await response.json()) as SwarmMessage;
    } catch (err) {
      console.warn('Could not reload SwarmMessage row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SwarmMessage>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSwarmMessage(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SwarmMessage[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SwarmMessage';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmMessageCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SwarmMessage-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSwarmMessage(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSwarmMessage(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SwarmMessage[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SwarmMessage[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SwarmMessage[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SwarmMessage[]);
    }
  };
