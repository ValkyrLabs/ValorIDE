...
import WorkflowGraphEdgeForm from '@thorapi/redux/components/form/WorkflowGraphEdgeForm';
...
import {
  useLazyGetWorkflowGraphEdgesPagedQuery,
...
} from '@thorapi/redux/services/WorkflowGraphEdgeService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `WorkflowGraphEdge update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/WorkflowGraphEdge/${rowId}`;

const WorkflowGraphEdgeTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<WorkflowGraphEdge[]>([]);
  ...
  const normalizePageRows = (result: unknown): WorkflowGraphEdge[] => {
    if (Array.isArray(result)) return result as WorkflowGraphEdge[];
    ...
    return Array.isArray(candidate) ? (candidate as WorkflowGraphEdge[]) : [];
  };

  const mergeFetchedRows = (incomingRows: WorkflowGraphEdge[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addWorkflowGraphEdge] = useAddWorkflowGraphEdgeMutation();
  const [updateWorkflowGraphEdge] = useUpdateWorkflowGraphEdgeMutation();
  const [deleteWorkflowGraphEdge] = useDeleteWorkflowGraphEdgeMutation();
  const [deleteWorkflowGraphEdgeCascade] = useDeleteWorkflowGraphEdgeCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<WorkflowGraphEdge>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<WorkflowGraphEdge | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<WorkflowGraphEdge | null> => {
    ...
    try {
      ...
      return (await response.json()) as WorkflowGraphEdge;
    } catch (err) {
      console.warn('Could not reload WorkflowGraphEdge row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<WorkflowGraphEdge>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWorkflowGraphEdge(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as WorkflowGraphEdge[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update WorkflowGraphEdge';
      console.error('Failed to update WorkflowGraphEdge:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphEdgeCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteWorkflowGraphEdge(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<WorkflowGraphEdge> = { ...source };
      delete (payload as any).id;
      const created = await addWorkflowGraphEdge(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphEdge, ...prev]);
    } catch (error) {
      console.error('Failed to add WorkflowGraphEdge:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<WorkflowGraphEdge> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addWorkflowGraphEdge(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphEdge, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) WorkflowGraphEdge:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `WorkflowGraphEdge-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWorkflowGraphEdge(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWorkflowGraphEdge(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as WorkflowGraphEdge[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as WorkflowGraphEdge[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as WorkflowGraphEdge[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as WorkflowGraphEdge[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="WorkflowGraphEdgeTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="WorkflowGraphEdge Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create WorkflowGraphEdge</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter WorkflowGraphEdge (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing WorkflowGraphEdge</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof WorkflowGraphEdge;
                ...
                await updateWorkflowGraphEdge(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of WorkflowGraphEdges.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'WorkflowGraphEdge'}
...
export default WorkflowGraphEdgeTable;
