...
import { DeadLetterQueue } from '@thorapi/model';
import DeadLetterQueueForm from '@thorapi/redux/components/form/DeadLetterQueueForm';
...
import {
  ...
  useDeleteDeadLetterQueueCascadeMutation
} from '@thorapi/redux/services/DeadLetterQueueService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `DeadLetterQueue update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/DeadLetterQueue/${rowId}`;
...
const DeadLetterQueueTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): DeadLetterQueue[] => {
    if (Array.isArray(result)) return result as DeadLetterQueue[];
    ...
    return Array.isArray(candidate) ? (candidate as DeadLetterQueue[]) : [];
  };

  const mergeFetchedRows = (incomingRows: DeadLetterQueue[], requestedPage: number) => {
  ...
  const [deleteDeadLetterQueueCascade] = useDeleteDeadLetterQueueCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<DeadLetterQueue | null> => {
    ...
    try {
      ...
      return (await response.json()) as DeadLetterQueue;
    } catch (err) {
      console.warn('Could not reload DeadLetterQueue row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<DeadLetterQueue>,
    successField?: string,
  ) => {
    try {
      const saved = await updateDeadLetterQueue(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as DeadLetterQueue[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update DeadLetterQueue';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteDeadLetterQueueCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `DeadLetterQueue-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addDeadLetterQueue(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateDeadLetterQueue(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as DeadLetterQueue[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as DeadLetterQueue[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as DeadLetterQueue[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as DeadLetterQueue[]);
    }
  };
