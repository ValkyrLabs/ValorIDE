...
import {
  FileProcessingJob,
  FileProcessingJobTypeEnum,
  FileProcessingJobStatusEnum,
} from '@thorapi/model';
...
/* -----------------------------------------------------
   COMPONENT
-------------------------------------------------------- */
const FileProcessingJobForm: React.FC = () => {
  ...
  -------------------------------------------------------- */
  const initialValues: Partial<FileProcessingJob> = {
        type: undefined,
        status: undefined,
          progress: 0,
          metadata: '',
  ...
  /* SUBMIT HANDLER */
  const handleSubmit = async (values: FormikValues, { setSubmitting }: FormikHelpers<FileProcessingJob>) => {
    try {
      ...
      const result = await addFileProcessingJob(values as any).unwrap();

      if (result && result.id && currentUser.permissions.canGrantPermissions) {
        const shouldSetPermissions = window.confirm(
          `FileProcessingJob created successfully! Would you like to set permissions for this object?`
        );
  ...
  return (
    <div>
      <Formik
        validateOnBlur={true}
        initialValues={initialValues as FileProcessingJob}
          ...
          <form onSubmit={handleSubmit} className="form">
            <Accordion defaultActiveKey="1">
              ...
              <Accordion.Item eventKey="1">
                <Accordion.Header>
                  <FaRegPlusSquare size={28} /> &nbsp; Add New FileProcessingJob
                </Accordion.Header>
                <Accordion.Body>
                  ...
                  >
                    ...
                    <FaCheckCircle size={28} /> Create New FileProcessingJob
                  </CoolButton>

                  {(addFileProcessingJobResult.isError || errorMessage) && (
                    <Alert variant="danger" className="mt-3">
                      {errorMessage ||
                        JSON.stringify('data' in (addFileProcessingJobResult as any).error ? (addFileProcessingJobResult as any).error.data : (addFileProcessingJobResult as any).error)}
                    </Alert>
                  )}

                  {(addFileProcessingJobResult.isSuccess || successMessage) && (
            ...
            {/* Debug/Dev Accordion */}
              <Accordion.Item eventKey="0">
                ...
                <Accordion.Body>
                  ...
                  <br />
                  addFileProcessingJobResult: {JSON.stringify(addFileProcessingJobResult)}
                </Accordion.Body>
              </Accordion.Item>
