...
export const TrustKeyVersionService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getTrustKeyVersionsPaged: build.query<TrustKeyVersionResponse, { page: number; size?: number; example?: Partial<TrustKeyVersion> }>({
    ...
    getTrustKeyVersions: build.query<TrustKeyVersionResponse, { example?: Partial<TrustKeyVersion> } | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateTrustKeyVersion: build.mutation<void, Pick<TrustKeyVersion, 'id'> & Partial<TrustKeyVersion>>({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            TrustKeyVersionService.util.updateQueryData('getTrustKeyVersion', id, (draft) => {
    ...
    deleteTrustKeyVersionCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'TrustKeyVersion', id }, { type: 'TrustKeyVersion', id: 'LIST' }],
    }),
  }),
})
