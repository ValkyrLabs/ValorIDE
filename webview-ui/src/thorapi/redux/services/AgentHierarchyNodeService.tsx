...
export const AgentHierarchyNodeService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getAgentHierarchyNodesPaged: build.query<AgentHierarchyNodeResponse, { page: number; size?: number; example?: Partial<AgentHierarchyNode> }>({
    ...
    getAgentHierarchyNodes: build.query<AgentHierarchyNodeResponse, { example?: Partial<AgentHierarchyNode> } | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateAgentHierarchyNode: build.mutation<void, Pick<AgentHierarchyNode, 'id'> & Partial<AgentHierarchyNode>>({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            AgentHierarchyNodeService.util.updateQueryData('getAgentHierarchyNode', id, (draft) => {
    ...
    deleteAgentHierarchyNode: build.mutation<{ success: boolean; id: string }, number>({
      ...
      invalidatesTags: (result, error, id) => [{ type: 'AgentHierarchyNode', id }],
    }),
    ...
    deleteAgentHierarchyNodeCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'AgentHierarchyNode', id }, { type: 'AgentHierarchyNode', id: 'LIST' }],
    }),
  }),
})
