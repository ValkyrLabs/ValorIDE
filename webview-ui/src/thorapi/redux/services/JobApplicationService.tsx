...
export const JobApplicationService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getJobApplicationsPaged: build.query<JobApplicationResponse, { page: number; size?: number; example?: Partial<JobApplication> }>({
    ...
    getJobApplications: build.query<JobApplicationResponse, { example?: Partial<JobApplication> } | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateJobApplication: build.mutation<void, Pick<JobApplication, 'id'> & Partial<JobApplication>>({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            JobApplicationService.util.updateQueryData('getJobApplication', id, (draft) => {
    ...
    deleteJobApplicationCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'JobApplication', id }, { type: 'JobApplication', id: 'LIST' }],
    }),
  }),
})
