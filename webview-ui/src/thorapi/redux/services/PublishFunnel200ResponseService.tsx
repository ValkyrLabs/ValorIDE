// tslint:disable
/**
############################## DO NOT EDIT: GENERATED FILE ##############################

Generated by Valkyr Labs ThorAPI: https://valkyrlabs.com/thorapi
Powered by Swagger Codegen: http://swagger.io

Generated Details:
**GENERATOR VERSION:** 7.5.0
**GENERATOR CLASS:** org.openapitools.codegen.languages.TypeScriptReduxQueryClientCodegen

Template file: typescript-redux-query/modelService.mustache

############################## DO NOT EDIT: GENERATED FILE ##############################
*/
import { createApi } from '@reduxjs/toolkit/query/react'
import { PublishFunnel200Response } from '@thorapi/model/PublishFunnel200Response'
import customBaseQuery from '../customBaseQuery'; // Import the custom base query

type PublishFunnel200ResponseResponse = PublishFunnel200Response[]
type PublishFunnel200ResponsePagedQueryArg = {
  page: number
  size?: number
  example?: Partial<PublishFunnel200Response>
  /**
   * Cache discriminator only. Do not send this to ThorAPI; callers pass the
   * authenticated principal id/username so RBAC-filtered pages cannot be
   * reused across login boundaries by RTK Query.
   */
  authSessionKey?: string
}

type PublishFunnel200ResponseListQueryArg = {
  example?: Partial<PublishFunnel200Response>
  /**
   * Cache discriminator only. Do not send this to ThorAPI.
   */
  authSessionKey?: string
}

const toPublishFunnel200ResponseList = (result: unknown): PublishFunnel200ResponseResponse => {
  if (Array.isArray(result)) {
    return result as PublishFunnel200ResponseResponse
  }

  const candidate = (result as any)?.content ?? (result as any)?.items ?? (result as any)?.results ?? (result as any)?.data
  return Array.isArray(candidate) ? (candidate as PublishFunnel200ResponseResponse) : []
}

export const PublishFunnel200ResponseService = createApi({
  reducerPath: 'PublishFunnel200Response', // This should remain unique
  baseQuery: customBaseQuery,
  tagTypes: ['PublishFunnel200Response'],
  endpoints: (build) => ({
    // 1) Paged Query Endpoint
    // Standardized pagination: page (0-based), size (page size)
    getPublishFunnel200ResponsesPaged: build.query<PublishFunnel200ResponseResponse, PublishFunnel200ResponsePagedQueryArg>({
      query: ({ page, size = 20, example }) => {
        const q: string[] = [`page=${page}`, `size=${size}`];
        if (example) q.push(`example=${encodeURIComponent(JSON.stringify(example))}`);
        return `PublishFunnel200Response?${q.join('&')}`;
      },
      providesTags: (result, error, { page }) => {
        const rows = toPublishFunnel200ResponseList(result)
        return [
          ...rows
            .filter((row) => row?.id != null)
            .map(({ id }) => ({ type: 'PublishFunnel200Response' as const, id })),
          { type: 'PublishFunnel200Response', id: `PAGE_${page}` },
          { type: 'PublishFunnel200Response', id: 'PARTIAL-LIST' },
        ]
      },
    }),

    // 2) Simple "get all" Query (optional)
    getPublishFunnel200Responses: build.query<PublishFunnel200ResponseResponse, PublishFunnel200ResponseListQueryArg | void>({
      query: (arg) => {
        if (arg && (arg as any).example) {
          const ex = (arg as any).example;
          return `PublishFunnel200Response?example=${encodeURIComponent(JSON.stringify(ex))}`;
        }
        return `PublishFunnel200Response`;
      },
      providesTags: (result) => {
        const rows = toPublishFunnel200ResponseList(result)
        return [
          ...rows
            .filter((row) => row?.id != null)
            .map(({ id }) => ({ type: 'PublishFunnel200Response' as const, id })),
          { type: 'PublishFunnel200Response', id: 'LIST' },
          { type: 'PublishFunnel200Response', id: 'PARTIAL-LIST' },
        ]
      },
    }),

    // 3) Create
    addPublishFunnel200Response: build.mutation<PublishFunnel200Response, Partial<PublishFunnel200Response>>({
      query: (body) => ({
        url: `PublishFunnel200Response`,
        method: 'POST',
        body,
      }),
      invalidatesTags: [
        { type: 'PublishFunnel200Response', id: 'LIST' },
        { type: 'PublishFunnel200Response', id: 'PARTIAL-LIST' },
      ],
    }),

    // 4) Get single by ID
    getPublishFunnel200Response: build.query<PublishFunnel200Response, string>({
      query: (id) => `PublishFunnel200Response/${id}`,
      providesTags: (result, error, id) => [{ type: 'PublishFunnel200Response', id }],
    }),

    // 5) Update
    updatePublishFunnel200Response: build.mutation<PublishFunnel200Response, Pick<PublishFunnel200Response, 'id'> & Partial<PublishFunnel200Response>>({
      query: ({ id, ...patch }) => ({
        url: `PublishFunnel200Response/${id}`,
        method: 'PUT',
        body: patch,
      }),
      invalidatesTags: (result, error, { id }: Pick<PublishFunnel200Response, 'id'>) => [
        { type: 'PublishFunnel200Response', id },
        { type: 'PublishFunnel200Response', id: 'LIST' },
        { type: 'PublishFunnel200Response', id: 'PARTIAL-LIST' },
      ],
    }),

    // 6) Delete
    deletePublishFunnel200Response: build.mutation<{ success: boolean; id: string }, number>({
      query(id) {
        return {
          url: `PublishFunnel200Response/${id}`,
          method: 'DELETE',
        }
      },
      invalidatesTags: (result, error, id) => [
        { type: 'PublishFunnel200Response', id },
        { type: 'PublishFunnel200Response', id: 'LIST' },
        { type: 'PublishFunnel200Response', id: 'PARTIAL-LIST' },
      ],
    }),

    // 7) Cascade / soft-delete (marks trashed, cascades children)
    deletePublishFunnel200ResponseCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      query({ id, cascade = true, trash = true }) {
        const params = [`cascade=${cascade}`, `trash=${trash}`].join('&');
        return {
          url: `PublishFunnel200Response/${id}?${params}`,
          method: 'DELETE',
        }
      },
      invalidatesTags: (result, error, { id }) => [
        { type: 'PublishFunnel200Response', id },
        { type: 'PublishFunnel200Response', id: 'LIST' },
        { type: 'PublishFunnel200Response', id: 'PARTIAL-LIST' },
      ],
    }),
  }),
})

// Notice we now also export `useLazyGetPublishFunnel200ResponsesPagedQuery`
export const {
  useGetPublishFunnel200ResponsesPagedQuery,     // immediate fetch
  useLazyGetPublishFunnel200ResponsesPagedQuery, // lazy fetch
  useGetPublishFunnel200ResponseQuery,
  useGetPublishFunnel200ResponsesQuery,
  useAddPublishFunnel200ResponseMutation,
  useUpdatePublishFunnel200ResponseMutation,
  useDeletePublishFunnel200ResponseMutation,
  useDeletePublishFunnel200ResponseCascadeMutation,
} = PublishFunnel200ResponseService
