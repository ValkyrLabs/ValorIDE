...
export const TrustPolicyBindingService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getTrustPolicyBindingsPaged: build.query<TrustPolicyBindingResponse, { page: number; size?: number; example?: Partial<TrustPolicyBinding> }>({
    ...
    getTrustPolicyBindings: build.query<TrustPolicyBindingResponse, { example?: Partial<TrustPolicyBinding> } | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateTrustPolicyBinding: build.mutation<void, Pick<TrustPolicyBinding, 'id'> & Partial<TrustPolicyBinding>>({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            TrustPolicyBindingService.util.updateQueryData('getTrustPolicyBinding', id, (draft) => {
    ...
    deleteTrustPolicyBinding: build.mutation<{ success: boolean; id: string }, number>({
      ...
      invalidatesTags: (result, error, id) => [{ type: 'TrustPolicyBinding', id }],
    }),
    ...
    deleteTrustPolicyBindingCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'TrustPolicyBinding', id }, { type: 'TrustPolicyBinding', id: 'LIST' }],
    }),
  }),
})
