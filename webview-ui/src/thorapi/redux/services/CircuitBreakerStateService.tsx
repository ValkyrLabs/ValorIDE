...
type CircuitBreakerStatePagedQueryArg = {
  page: number
  size?: number
  example?: Partial<CircuitBreakerState>
  /**
...
type CircuitBreakerStateListQueryArg = {
  example?: Partial<CircuitBreakerState>
  /**
...
const toCircuitBreakerStateList = (result: unknown): CircuitBreakerStateResponse => {
  if (Array.isArray(result)) {
    return result as CircuitBreakerStateResponse
  }

  const candidate = (result as any)?.content ?? (result as any)?.items ?? (result as any)?.results ?? (result as any)?.data
  return Array.isArray(candidate) ? (candidate as CircuitBreakerStateResponse) : []
}
...
export const CircuitBreakerStateService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getCircuitBreakerStatesPaged: build.query<CircuitBreakerStateResponse, CircuitBreakerStatePagedQueryArg>({
      ...
      providesTags: (result, error, { page }) => {
        ...
        return [
          ...
          { type: 'CircuitBreakerState', id: 'PARTIAL-LIST' },
        ]
      },
    }),

    // 2) Simple "get all" Query (optional)
    getCircuitBreakerStates: build.query<CircuitBreakerStateResponse, CircuitBreakerStateListQueryArg | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateCircuitBreakerState: build.mutation<CircuitBreakerState, Pick<CircuitBreakerState, 'id'> & Partial<CircuitBreakerState>>({
      query: ({ id, ...patch }) => ({
