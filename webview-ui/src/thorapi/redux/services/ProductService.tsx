// tslint:disable
/**
############################## DO NOT EDIT: GENERATED FILE ##############################

Generated by Valkyr Labs ThorAPI: https://valkyrlabs.com/thorapi
Powered by Swagger Codegen: http://swagger.io

Generated Details:
**GENERATOR VERSION:** 7.5.0
**GENERATOR CLASS:** org.openapitools.codegen.languages.TypeScriptReduxQueryClientCodegen

Template file: typescript-redux-query/modelService.mustache

############################## DO NOT EDIT: GENERATED FILE ##############################
*/
import { createApi } from '@reduxjs/toolkit/query/react'
import { Product } from '@thorapi/model/Product'
import customBaseQuery from '../customBaseQuery'; // Import the custom base query

type ProductResponse = Product[]
type ProductPagedQueryArg = {
  page: number
  size?: number
  example?: Partial<Product>
  /**
   * Cache discriminator only. Do not send this to ThorAPI; callers pass the
   * authenticated principal id/username so RBAC-filtered pages cannot be
   * reused across login boundaries by RTK Query.
   */
  authSessionKey?: string
}

type ProductListQueryArg = {
  example?: Partial<Product>
  /**
   * Cache discriminator only. Do not send this to ThorAPI.
   */
  authSessionKey?: string
}

const toProductList = (result: unknown): ProductResponse => {
  if (Array.isArray(result)) {
    return result as ProductResponse
  }

  const candidate = (result as any)?.content ?? (result as any)?.items ?? (result as any)?.results ?? (result as any)?.data
  return Array.isArray(candidate) ? (candidate as ProductResponse) : []
}

export const ProductService = createApi({
  reducerPath: 'Product', // This should remain unique
  baseQuery: customBaseQuery,
  tagTypes: ['Product'],
  endpoints: (build) => ({
    // 1) Paged Query Endpoint
    // Standardized pagination: page (0-based), size (page size)
    getProductsPaged: build.query<ProductResponse, ProductPagedQueryArg>({
      query: ({ page, size = 20, example }) => {
        const q: string[] = [`page=${page}`, `size=${size}`];
        if (example) q.push(`example=${encodeURIComponent(JSON.stringify(example))}`);
        return `Product?${q.join('&')}`;
      },
      providesTags: (result, error, { page }) => {
        const rows = toProductList(result)
        return [
          ...rows
            .filter((row) => row?.id != null)
            .map(({ id }) => ({ type: 'Product' as const, id })),
          { type: 'Product', id: `PAGE_${page}` },
          { type: 'Product', id: 'PARTIAL-LIST' },
        ]
      },
    }),

    // 2) Simple "get all" Query (optional)
    getProducts: build.query<ProductResponse, ProductListQueryArg | void>({
      query: (arg) => {
        if (arg && (arg as any).example) {
          const ex = (arg as any).example;
          return `Product?example=${encodeURIComponent(JSON.stringify(ex))}`;
        }
        return `Product`;
      },
      providesTags: (result) => {
        const rows = toProductList(result)
        return [
          ...rows
            .filter((row) => row?.id != null)
            .map(({ id }) => ({ type: 'Product' as const, id })),
          { type: 'Product', id: 'LIST' },
          { type: 'Product', id: 'PARTIAL-LIST' },
        ]
      },
    }),

    // 3) Create
    addProduct: build.mutation<Product, Partial<Product>>({
      query: (body) => ({
        url: `Product`,
        method: 'POST',
        body,
      }),
      invalidatesTags: [
        { type: 'Product', id: 'LIST' },
        { type: 'Product', id: 'PARTIAL-LIST' },
      ],
    }),

    // 4) Get single by ID
    getProduct: build.query<Product, string>({
      query: (id) => `Product/${id}`,
      providesTags: (result, error, id) => [{ type: 'Product', id }],
    }),

    // 5) Update
    updateProduct: build.mutation<Product, Pick<Product, 'id'> & Partial<Product>>({
      query: ({ id, ...patch }) => ({
        url: `Product/${id}`,
        method: 'PUT',
        body: patch,
      }),
      invalidatesTags: (result, error, { id }: Pick<Product, 'id'>) => [
        { type: 'Product', id },
        { type: 'Product', id: 'LIST' },
        { type: 'Product', id: 'PARTIAL-LIST' },
      ],
    }),

    // 6) Delete
    deleteProduct: build.mutation<{ success: boolean; id: string }, number>({
      query(id) {
        return {
          url: `Product/${id}`,
          method: 'DELETE',
        }
      },
      invalidatesTags: (result, error, id) => [
        { type: 'Product', id },
        { type: 'Product', id: 'LIST' },
        { type: 'Product', id: 'PARTIAL-LIST' },
      ],
    }),

    // 7) Cascade / soft-delete (marks trashed, cascades children)
    deleteProductCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      query({ id, cascade = true, trash = true }) {
        const params = [`cascade=${cascade}`, `trash=${trash}`].join('&');
        return {
          url: `Product/${id}?${params}`,
          method: 'DELETE',
        }
      },
      invalidatesTags: (result, error, { id }) => [
        { type: 'Product', id },
        { type: 'Product', id: 'LIST' },
        { type: 'Product', id: 'PARTIAL-LIST' },
      ],
    }),
  }),
})

// Notice we now also export `useLazyGetProductsPagedQuery`
export const {
  useGetProductsPagedQuery,     // immediate fetch
  useLazyGetProductsPagedQuery, // lazy fetch
  useGetProductQuery,
  useGetProductsQuery,
  useAddProductMutation,
  useUpdateProductMutation,
  useDeleteProductMutation,
  useDeleteProductCascadeMutation,
} = ProductService
