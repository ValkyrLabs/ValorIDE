...
export const GraphLinkService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getGraphLinksPaged: build.query<GraphLinkResponse, { page: number; size?: number; example?: Partial<GraphLink> }>({
    ...
    getGraphLinks: build.query<GraphLinkResponse, { example?: Partial<GraphLink> } | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateGraphLink: build.mutation<void, Pick<GraphLink, 'id'> & Partial<GraphLink>>({
      query: ({ id, ...patch }) => ({
        url: `GraphLink/${id}`,
        method: 'PUT',
        body: patch,
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            GraphLinkService.util.updateQueryData('getGraphLink', id, (draft) => {
    ...
    deleteGraphLinkCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'GraphLink', id }, { type: 'GraphLink', id: 'LIST' }],
    }),
  }),
})
