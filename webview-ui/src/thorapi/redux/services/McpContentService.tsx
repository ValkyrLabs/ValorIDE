...
const toMcpContentList = (result: unknown): McpContentResponse => {
  if (Array.isArray(result)) {
    return result as McpContentResponse
  }

  const candidate = (result as any)?.content ?? (result as any)?.items ?? (result as any)?.results ?? (result as any)?.data
  return Array.isArray(candidate) ? (candidate as McpContentResponse) : []
}
  ...
  endpoints: (build) => ({
    ...
    getMcpContentsPaged: build.query<McpContentResponse, { page: number; size?: number; example?: Partial<McpContent> }>({
      ...
      providesTags: (result, error, { page }) => {
        const rows = toMcpContentList(result)
        return [
          ...rows
            .filter((row) => row?.id != null)
            .map(({ id }) => ({ type: 'McpContent' as const, id })),
    ...
    deleteMcpContentCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      query({ id, cascade = true, trash = true }) {
        const params = [`cascade=${cascade}`, `trash=${trash}`].join('&');
        return {
          url: `McpContent/${id}?${params}`,
          method: 'DELETE',
        }
      },
      invalidatesTags: (result, error, { id }) => [{ type: 'McpContent', id }, { type: 'McpContent', id: 'LIST' }],
    }),
  }),
})
