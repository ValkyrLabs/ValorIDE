// tslint:disable
/**
############################## DO NOT EDIT: GENERATED FILE ##############################

Generated by Valkyr Labs ThorAPI: https://valkyrlabs.com/thorapi
Powered by Swagger Codegen: http://swagger.io

Generated Details:
**GENERATOR VERSION:** 7.5.0
**GENERATOR CLASS:** org.openapitools.codegen.languages.TypeScriptReduxQueryClientCodegen

Template file: typescript-redux-query/modelService.mustache

############################## DO NOT EDIT: GENERATED FILE ##############################
*/
import { createApi } from '@reduxjs/toolkit/query/react'
import { Border } from '@thorapi/model/Border'
import customBaseQuery from '../customBaseQuery'; // Import the custom base query

type BorderResponse = Border[]
type BorderPagedQueryArg = {
  page: number
  size?: number
  example?: Partial<Border>
  /**
   * Cache discriminator only. Do not send this to ThorAPI; callers pass the
   * authenticated principal id/username so RBAC-filtered pages cannot be
   * reused across login boundaries by RTK Query.
   */
  authSessionKey?: string
}

type BorderListQueryArg = {
  example?: Partial<Border>
  /**
   * Cache discriminator only. Do not send this to ThorAPI.
   */
  authSessionKey?: string
}

const toBorderList = (result: unknown): BorderResponse => {
  if (Array.isArray(result)) {
    return result as BorderResponse
  }

  const candidate = (result as any)?.content ?? (result as any)?.items ?? (result as any)?.results ?? (result as any)?.data
  return Array.isArray(candidate) ? (candidate as BorderResponse) : []
}

export const BorderService = createApi({
  reducerPath: 'Border', // This should remain unique
  baseQuery: customBaseQuery,
  tagTypes: ['Border'],
  endpoints: (build) => ({
    // 1) Paged Query Endpoint
    // Standardized pagination: page (0-based), size (page size)
    getBordersPaged: build.query<BorderResponse, BorderPagedQueryArg>({
      query: ({ page, size = 20, example }) => {
        const q: string[] = [`page=${page}`, `size=${size}`];
        if (example) q.push(`example=${encodeURIComponent(JSON.stringify(example))}`);
        return `Border?${q.join('&')}`;
      },
      providesTags: (result, error, { page }) => {
        const rows = toBorderList(result)
        return [
          ...rows
            .filter((row) => row?.id != null)
            .map(({ id }) => ({ type: 'Border' as const, id })),
          { type: 'Border', id: `PAGE_${page}` },
          { type: 'Border', id: 'PARTIAL-LIST' },
        ]
      },
    }),

    // 2) Simple "get all" Query (optional)
    getBorders: build.query<BorderResponse, BorderListQueryArg | void>({
      query: (arg) => {
        if (arg && (arg as any).example) {
          const ex = (arg as any).example;
          return `Border?example=${encodeURIComponent(JSON.stringify(ex))}`;
        }
        return `Border`;
      },
      providesTags: (result) => {
        const rows = toBorderList(result)
        return [
          ...rows
            .filter((row) => row?.id != null)
            .map(({ id }) => ({ type: 'Border' as const, id })),
          { type: 'Border', id: 'LIST' },
          { type: 'Border', id: 'PARTIAL-LIST' },
        ]
      },
    }),

    // 3) Create
    addBorder: build.mutation<Border, Partial<Border>>({
      query: (body) => ({
        url: `Border`,
        method: 'POST',
        body,
      }),
      invalidatesTags: [
        { type: 'Border', id: 'LIST' },
        { type: 'Border', id: 'PARTIAL-LIST' },
      ],
    }),

    // 4) Get single by ID
    getBorder: build.query<Border, string>({
      query: (id) => `Border/${id}`,
      providesTags: (result, error, id) => [{ type: 'Border', id }],
    }),

    // 5) Update
    updateBorder: build.mutation<Border, Pick<Border, 'id'> & Partial<Border>>({
      query: ({ id, ...patch }) => ({
        url: `Border/${id}`,
        method: 'PUT',
        body: patch,
      }),
      invalidatesTags: (result, error, { id }: Pick<Border, 'id'>) => [
        { type: 'Border', id },
        { type: 'Border', id: 'LIST' },
        { type: 'Border', id: 'PARTIAL-LIST' },
      ],
    }),

    // 6) Delete
    deleteBorder: build.mutation<{ success: boolean; id: string }, number>({
      query(id) {
        return {
          url: `Border/${id}`,
          method: 'DELETE',
        }
      },
      invalidatesTags: (result, error, id) => [
        { type: 'Border', id },
        { type: 'Border', id: 'LIST' },
        { type: 'Border', id: 'PARTIAL-LIST' },
      ],
    }),

    // 7) Cascade / soft-delete (marks trashed, cascades children)
    deleteBorderCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      query({ id, cascade = true, trash = true }) {
        const params = [`cascade=${cascade}`, `trash=${trash}`].join('&');
        return {
          url: `Border/${id}?${params}`,
          method: 'DELETE',
        }
      },
      invalidatesTags: (result, error, { id }) => [
        { type: 'Border', id },
        { type: 'Border', id: 'LIST' },
        { type: 'Border', id: 'PARTIAL-LIST' },
      ],
    }),
  }),
})

// Notice we now also export `useLazyGetBordersPagedQuery`
export const {
  useGetBordersPagedQuery,     // immediate fetch
  useLazyGetBordersPagedQuery, // lazy fetch
  useGetBorderQuery,
  useGetBordersQuery,
  useAddBorderMutation,
  useUpdateBorderMutation,
  useDeleteBorderMutation,
  useDeleteBorderCascadeMutation,
} = BorderService
