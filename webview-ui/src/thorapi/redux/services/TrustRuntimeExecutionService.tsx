...
export const TrustRuntimeExecutionService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getTrustRuntimeExecutionsPaged: build.query<TrustRuntimeExecutionResponse, { page: number; size?: number; example?: Partial<TrustRuntimeExecution> }>({
    ...
    getTrustRuntimeExecutions: build.query<TrustRuntimeExecutionResponse, { example?: Partial<TrustRuntimeExecution> } | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateTrustRuntimeExecution: build.mutation<void, Pick<TrustRuntimeExecution, 'id'> & Partial<TrustRuntimeExecution>>({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            TrustRuntimeExecutionService.util.updateQueryData('getTrustRuntimeExecution', id, (draft) => {
    ...
    deleteTrustRuntimeExecution: build.mutation<{ success: boolean; id: string }, number>({
      ...
      invalidatesTags: (result, error, id) => [{ type: 'TrustRuntimeExecution', id }],
    }),
    ...
    deleteTrustRuntimeExecutionCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'TrustRuntimeExecution', id }, { type: 'TrustRuntimeExecution', id: 'LIST' }],
    }),
  }),
})
