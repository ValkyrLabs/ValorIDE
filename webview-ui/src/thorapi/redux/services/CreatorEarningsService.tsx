...
export const CreatorEarningsService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getCreatorEarningssPaged: build.query<CreatorEarningsResponse, { page: number; size?: number; example?: Partial<CreatorEarnings> }>({
    ...
    getCreatorEarningss: build.query<CreatorEarningsResponse, { example?: Partial<CreatorEarnings> } | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateCreatorEarnings: build.mutation<void, Pick<CreatorEarnings, 'id'> & Partial<CreatorEarnings>>({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            CreatorEarningsService.util.updateQueryData('getCreatorEarnings', id, (draft) => {
    ...
    deleteCreatorEarningsCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'CreatorEarnings', id }, { type: 'CreatorEarnings', id: 'LIST' }],
    }),
  }),
})
