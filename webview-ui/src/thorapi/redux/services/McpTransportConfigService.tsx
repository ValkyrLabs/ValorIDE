...
export const McpTransportConfigService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getMcpTransportConfigsPaged: build.query<McpTransportConfigResponse, { page: number; size?: number; example?: Partial<McpTransportConfig> }>({
    ...
    getMcpTransportConfigs: build.query<McpTransportConfigResponse, { example?: Partial<McpTransportConfig> } | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateMcpTransportConfig: build.mutation<void, Pick<McpTransportConfig, 'id'> & Partial<McpTransportConfig>>({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            McpTransportConfigService.util.updateQueryData('getMcpTransportConfig', id, (draft) => {
    ...
    deleteMcpTransportConfig: build.mutation<{ success: boolean; id: string }, number>({
      ...
      invalidatesTags: (result, error, id) => [{ type: 'McpTransportConfig', id }],
    }),
    ...
    deleteMcpTransportConfigCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'McpTransportConfig', id }, { type: 'McpTransportConfig', id: 'LIST' }],
    }),
  }),
})
