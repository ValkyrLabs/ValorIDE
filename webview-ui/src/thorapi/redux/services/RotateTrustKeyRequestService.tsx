...
export const RotateTrustKeyRequestService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getRotateTrustKeyRequestsPaged: build.query<RotateTrustKeyRequestResponse, { page: number; size?: number; example?: Partial<RotateTrustKeyRequest> }>({
    ...
    getRotateTrustKeyRequests: build.query<RotateTrustKeyRequestResponse, { example?: Partial<RotateTrustKeyRequest> } | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateRotateTrustKeyRequest: build.mutation<void, Pick<RotateTrustKeyRequest, 'id'> & Partial<RotateTrustKeyRequest>>({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            RotateTrustKeyRequestService.util.updateQueryData('getRotateTrustKeyRequest', id, (draft) => {
    ...
    deleteRotateTrustKeyRequest: build.mutation<{ success: boolean; id: string }, number>({
      ...
      invalidatesTags: (result, error, id) => [{ type: 'RotateTrustKeyRequest', id }],
    }),
    ...
    deleteRotateTrustKeyRequestCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'RotateTrustKeyRequest', id }, { type: 'RotateTrustKeyRequest', id: 'LIST' }],
    }),
  }),
})
