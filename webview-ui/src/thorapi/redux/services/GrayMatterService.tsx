...
type GrayMatterPagedQueryArg = {
  page: number
  size?: number
  example?: Partial<GrayMatter>
  /**
...
export const GrayMatterService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getGrayMattersPaged: build.query<GrayMatterResponse, GrayMatterPagedQueryArg>({
      ...
      providesTags: (result, error, { page }) => {
        ...
        return [
          ...
          { type: 'GrayMatter', id: 'PARTIAL-LIST' },
        ]
      },
    }),

    // 2) Simple "get all" Query (optional)
    getGrayMatters: build.query<GrayMatterResponse, GrayMatterListQueryArg | void>({
      query: (arg) => {
        if (arg && (arg as any).example) {
          const ex = (arg as any).example;
          return `GrayMatter?example=${encodeURIComponent(JSON.stringify(ex))}`;
        }
        return `GrayMatter`;
      },
      providesTags: (result) => {
    ...
    // 5) Update
    updateGrayMatter: build.mutation<GrayMatter, Pick<GrayMatter, 'id'> & Partial<GrayMatter>>({
      query: ({ id, ...patch }) => ({
        url: `GrayMatter/${id}`,
        method: 'PUT',
        body: patch,
    ...
    // 6) Delete
    deleteGrayMatter: build.mutation<{ success: boolean; id: string }, number>({
      query(id) {
        return {
          url: `GrayMatter/${id}`,
          method: 'DELETE',
