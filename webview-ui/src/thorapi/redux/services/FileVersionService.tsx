// tslint:disable
/**
############################## DO NOT EDIT: GENERATED FILE ##############################

Generated by Valkyr Labs ThorAPI: https://valkyrlabs.com/thorapi
Powered by Swagger Codegen: http://swagger.io

Generated Details:
**GENERATOR VERSION:** 7.5.0
**GENERATOR CLASS:** org.openapitools.codegen.languages.TypeScriptReduxQueryClientCodegen

Template file: typescript-redux-query/modelService.mustache

############################## DO NOT EDIT: GENERATED FILE ##############################
*/
import { createApi } from '@reduxjs/toolkit/query/react'
import { FileVersion } from '@thorapi/model/FileVersion'
import customBaseQuery from '../customBaseQuery'; // Import the custom base query

type FileVersionResponse = FileVersion[]
type FileVersionPagedQueryArg = {
  page: number
  size?: number
  example?: Partial<FileVersion>
  /**
   * Cache discriminator only. Do not send this to ThorAPI; callers pass the
   * authenticated principal id/username so RBAC-filtered pages cannot be
   * reused across login boundaries by RTK Query.
   */
  authSessionKey?: string
}

type FileVersionListQueryArg = {
  example?: Partial<FileVersion>
  /**
   * Cache discriminator only. Do not send this to ThorAPI.
   */
  authSessionKey?: string
}

const toFileVersionList = (result: unknown): FileVersionResponse => {
  if (Array.isArray(result)) {
    return result as FileVersionResponse
  }

  const candidate = (result as any)?.content ?? (result as any)?.items ?? (result as any)?.results ?? (result as any)?.data
  return Array.isArray(candidate) ? (candidate as FileVersionResponse) : []
}

export const FileVersionService = createApi({
  reducerPath: 'FileVersion', // This should remain unique
  baseQuery: customBaseQuery,
  tagTypes: ['FileVersion'],
  endpoints: (build) => ({
    // 1) Paged Query Endpoint
    // Standardized pagination: page (0-based), size (page size)
    getFileVersionsPaged: build.query<FileVersionResponse, FileVersionPagedQueryArg>({
      query: ({ page, size = 20, example }) => {
        const q: string[] = [`page=${page}`, `size=${size}`];
        if (example) q.push(`example=${encodeURIComponent(JSON.stringify(example))}`);
        return `FileVersion?${q.join('&')}`;
      },
      providesTags: (result, error, { page }) => {
        const rows = toFileVersionList(result)
        return [
          ...rows
            .filter((row) => row?.id != null)
            .map(({ id }) => ({ type: 'FileVersion' as const, id })),
          { type: 'FileVersion', id: `PAGE_${page}` },
          { type: 'FileVersion', id: 'PARTIAL-LIST' },
        ]
      },
    }),

    // 2) Simple "get all" Query (optional)
    getFileVersions: build.query<FileVersionResponse, FileVersionListQueryArg | void>({
      query: (arg) => {
        if (arg && (arg as any).example) {
          const ex = (arg as any).example;
          return `FileVersion?example=${encodeURIComponent(JSON.stringify(ex))}`;
        }
        return `FileVersion`;
      },
      providesTags: (result) => {
        const rows = toFileVersionList(result)
        return [
          ...rows
            .filter((row) => row?.id != null)
            .map(({ id }) => ({ type: 'FileVersion' as const, id })),
          { type: 'FileVersion', id: 'LIST' },
          { type: 'FileVersion', id: 'PARTIAL-LIST' },
        ]
      },
    }),

    // 3) Create
    addFileVersion: build.mutation<FileVersion, Partial<FileVersion>>({
      query: (body) => ({
        url: `FileVersion`,
        method: 'POST',
        body,
      }),
      invalidatesTags: [
        { type: 'FileVersion', id: 'LIST' },
        { type: 'FileVersion', id: 'PARTIAL-LIST' },
      ],
    }),

    // 4) Get single by ID
    getFileVersion: build.query<FileVersion, string>({
      query: (id) => `FileVersion/${id}`,
      providesTags: (result, error, id) => [{ type: 'FileVersion', id }],
    }),

    // 5) Update
    updateFileVersion: build.mutation<FileVersion, Pick<FileVersion, 'id'> & Partial<FileVersion>>({
      query: ({ id, ...patch }) => ({
        url: `FileVersion/${id}`,
        method: 'PUT',
        body: patch,
      }),
      invalidatesTags: (result, error, { id }: Pick<FileVersion, 'id'>) => [
        { type: 'FileVersion', id },
        { type: 'FileVersion', id: 'LIST' },
        { type: 'FileVersion', id: 'PARTIAL-LIST' },
      ],
    }),

    // 6) Delete
    deleteFileVersion: build.mutation<{ success: boolean; id: string }, number>({
      query(id) {
        return {
          url: `FileVersion/${id}`,
          method: 'DELETE',
        }
      },
      invalidatesTags: (result, error, id) => [
        { type: 'FileVersion', id },
        { type: 'FileVersion', id: 'LIST' },
        { type: 'FileVersion', id: 'PARTIAL-LIST' },
      ],
    }),

    // 7) Cascade / soft-delete (marks trashed, cascades children)
    deleteFileVersionCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      query({ id, cascade = true, trash = true }) {
        const params = [`cascade=${cascade}`, `trash=${trash}`].join('&');
        return {
          url: `FileVersion/${id}?${params}`,
          method: 'DELETE',
        }
      },
      invalidatesTags: (result, error, { id }) => [
        { type: 'FileVersion', id },
        { type: 'FileVersion', id: 'LIST' },
        { type: 'FileVersion', id: 'PARTIAL-LIST' },
      ],
    }),
  }),
})

// Notice we now also export `useLazyGetFileVersionsPagedQuery`
export const {
  useGetFileVersionsPagedQuery,     // immediate fetch
  useLazyGetFileVersionsPagedQuery, // lazy fetch
  useGetFileVersionQuery,
  useGetFileVersionsQuery,
  useAddFileVersionMutation,
  useUpdateFileVersionMutation,
  useDeleteFileVersionMutation,
  useDeleteFileVersionCascadeMutation,
} = FileVersionService
