// tslint:disable
/**
############################## DO NOT EDIT: GENERATED FILE ##############################

Generated by Valkyr Labs ThorAPI: https://valkyrlabs.com/thorapi
Powered by Swagger Codegen: http://swagger.io

Generated Details:
**GENERATOR VERSION:** 7.5.0
**GENERATOR CLASS:** org.openapitools.codegen.languages.TypeScriptReduxQueryClientCodegen

Template file: typescript-redux-query/modelService.mustache

############################## DO NOT EDIT: GENERATED FILE ##############################
*/
import { createApi } from '@reduxjs/toolkit/query/react'
import { PtgRef } from '@thorapi/model/PtgRef'
import customBaseQuery from '../customBaseQuery'; // Import the custom base query

type PtgRefResponse = PtgRef[]
type PtgRefPagedQueryArg = {
  page: number
  size?: number
  example?: Partial<PtgRef>
  /**
   * Cache discriminator only. Do not send this to ThorAPI; callers pass the
   * authenticated principal id/username so RBAC-filtered pages cannot be
   * reused across login boundaries by RTK Query.
   */
  authSessionKey?: string
}

type PtgRefListQueryArg = {
  example?: Partial<PtgRef>
  /**
   * Cache discriminator only. Do not send this to ThorAPI.
   */
  authSessionKey?: string
}

const toPtgRefList = (result: unknown): PtgRefResponse => {
  if (Array.isArray(result)) {
    return result as PtgRefResponse
  }

  const candidate = (result as any)?.content ?? (result as any)?.items ?? (result as any)?.results ?? (result as any)?.data
  return Array.isArray(candidate) ? (candidate as PtgRefResponse) : []
}

export const PtgRefService = createApi({
  reducerPath: 'PtgRef', // This should remain unique
  baseQuery: customBaseQuery,
  tagTypes: ['PtgRef'],
  endpoints: (build) => ({
    // 1) Paged Query Endpoint
    // Standardized pagination: page (0-based), size (page size)
    getPtgRefsPaged: build.query<PtgRefResponse, PtgRefPagedQueryArg>({
      query: ({ page, size = 20, example }) => {
        const q: string[] = [`page=${page}`, `size=${size}`];
        if (example) q.push(`example=${encodeURIComponent(JSON.stringify(example))}`);
        return `PtgRef?${q.join('&')}`;
      },
      providesTags: (result, error, { page }) => {
        const rows = toPtgRefList(result)
        return [
          ...rows
            .filter((row) => row?.id != null)
            .map(({ id }) => ({ type: 'PtgRef' as const, id })),
          { type: 'PtgRef', id: `PAGE_${page}` },
          { type: 'PtgRef', id: 'PARTIAL-LIST' },
        ]
      },
    }),

    // 2) Simple "get all" Query (optional)
    getPtgRefs: build.query<PtgRefResponse, PtgRefListQueryArg | void>({
      query: (arg) => {
        if (arg && (arg as any).example) {
          const ex = (arg as any).example;
          return `PtgRef?example=${encodeURIComponent(JSON.stringify(ex))}`;
        }
        return `PtgRef`;
      },
      providesTags: (result) => {
        const rows = toPtgRefList(result)
        return [
          ...rows
            .filter((row) => row?.id != null)
            .map(({ id }) => ({ type: 'PtgRef' as const, id })),
          { type: 'PtgRef', id: 'LIST' },
          { type: 'PtgRef', id: 'PARTIAL-LIST' },
        ]
      },
    }),

    // 3) Create
    addPtgRef: build.mutation<PtgRef, Partial<PtgRef>>({
      query: (body) => ({
        url: `PtgRef`,
        method: 'POST',
        body,
      }),
      invalidatesTags: [
        { type: 'PtgRef', id: 'LIST' },
        { type: 'PtgRef', id: 'PARTIAL-LIST' },
      ],
    }),

    // 4) Get single by ID
    getPtgRef: build.query<PtgRef, string>({
      query: (id) => `PtgRef/${id}`,
      providesTags: (result, error, id) => [{ type: 'PtgRef', id }],
    }),

    // 5) Update
    updatePtgRef: build.mutation<PtgRef, Pick<PtgRef, 'id'> & Partial<PtgRef>>({
      query: ({ id, ...patch }) => ({
        url: `PtgRef/${id}`,
        method: 'PUT',
        body: patch,
      }),
      invalidatesTags: (result, error, { id }: Pick<PtgRef, 'id'>) => [
        { type: 'PtgRef', id },
        { type: 'PtgRef', id: 'LIST' },
        { type: 'PtgRef', id: 'PARTIAL-LIST' },
      ],
    }),

    // 6) Delete
    deletePtgRef: build.mutation<{ success: boolean; id: string }, number>({
      query(id) {
        return {
          url: `PtgRef/${id}`,
          method: 'DELETE',
        }
      },
      invalidatesTags: (result, error, id) => [
        { type: 'PtgRef', id },
        { type: 'PtgRef', id: 'LIST' },
        { type: 'PtgRef', id: 'PARTIAL-LIST' },
      ],
    }),

    // 7) Cascade / soft-delete (marks trashed, cascades children)
    deletePtgRefCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      query({ id, cascade = true, trash = true }) {
        const params = [`cascade=${cascade}`, `trash=${trash}`].join('&');
        return {
          url: `PtgRef/${id}?${params}`,
          method: 'DELETE',
        }
      },
      invalidatesTags: (result, error, { id }) => [
        { type: 'PtgRef', id },
        { type: 'PtgRef', id: 'LIST' },
        { type: 'PtgRef', id: 'PARTIAL-LIST' },
      ],
    }),
  }),
})

// Notice we now also export `useLazyGetPtgRefsPagedQuery`
export const {
  useGetPtgRefsPagedQuery,     // immediate fetch
  useLazyGetPtgRefsPagedQuery, // lazy fetch
  useGetPtgRefQuery,
  useGetPtgRefsQuery,
  useAddPtgRefMutation,
  useUpdatePtgRefMutation,
  useDeletePtgRefMutation,
  useDeletePtgRefCascadeMutation,
} = PtgRefService
