...
import { HostInstanceStatusEnum } from "@thorapi/model/HostInstance";
export const ThorHostingService = createApi({
    ...
    endpoints: (build) => ({
        ...
        provisionHostInstance: build.mutation({
        ...
        listHostedInstances: build.query({
            query: () => ({
        ...
        startHostInstance: build.mutation({
            ...
            async onQueryStarted(appName, { dispatch, queryFulfilled }) {
                const patch = dispatch(ThorHostingService.util.updateQueryData("listHostedInstances", undefined, (draft) => {
                    draft.forEach((d) => {
                        if (d.name === appName)
        ...
        getHostInstanceMetrics: build.query({
            query: (appName) => ({
...
export const { useProvisionHostInstanceMutation, useListHostedInstancesQuery, useStartHostInstanceMutation, useStopHostInstanceMutation, useRestartHostInstanceMutation, useLazyGetHostInstanceMetricsQuery, } = ThorHostingService;
//# sourceMappingURL=ThorHostingService.js.map