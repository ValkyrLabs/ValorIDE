...
export const WorkflowService = createApi({
    ...
    tagTypes: ["Workflow"],
    endpoints: (build) => ({
        lazyGetWorkflows: build.query({
            query: ({ page, limit = 20 }) => `Workflow?page=${page}&limit=${limit}`,
            providesTags: (result, page) => result
                ? [
...
export const { useLoadOasMutation, useReloadWorkflowMutation, useStartWorkflowMutation, useStopWorkflowMutation, useLazyGetWorkflowsQuery, useGetWorkflowQuery, useGetWorkflowsQuery, useAddWorkflowMutation, useUpdateWorkflowMutation, useDeleteWorkflowMutation, } = WorkflowService;
