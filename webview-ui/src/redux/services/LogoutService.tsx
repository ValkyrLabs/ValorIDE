...
export const LogoutService = createApi({
	...
	endpoints: (build) => ({
		getLogouts: build.query<LogoutResponse, void>({
			...
			providesTags: (result) =>
				result
					? [...result.map(({ id }) => ({ type: "Logout" as const, id })), { type: "Logout", id: "LIST" }]
					: [{ type: "Logout", id: "LIST" }],
		}),
...
export const { useGetLogoutQuery, useGetLogoutsQuery, useAddLogoutMutation, useUpdateLogoutMutation, useDeleteLogoutMutation } =
	LogoutService
