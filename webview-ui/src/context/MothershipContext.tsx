...
export const MothershipProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  const { jwtToken, userInfo, authenticatedUser } = useExtensionState();
  const [mothershipService, setMothershipService] =
    useState<MothershipService | null>(null);
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "jwt-token-updated",
      handleJwtUpdate as EventListener,
    );
  ...
  const initializeMothershipService = useCallback(
    async (token: string) => {
      try {
  ...
  const sendValorIDEAction = useCallback(
    (taskId: string, action: string, data: any) => {
