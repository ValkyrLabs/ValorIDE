...
export const ExtensionStateContextProvider: React.FC<{
	...
	const [mcpMarketplaceCatalogError, setMcpMarketplaceCatalogError] = useState<any>(null)

	// Authentication state - prioritize backend state over sessionStorage
	const [jwtToken, setJwtToken] = useState<string | undefined>()
	const [authenticatedPrincipal, setAuthenticatedPrincipal] = useState<Principal | undefined>()
	...
	const handleMessage = useCallback((event: MessageEvent) => {
		...
		switch (message.type) {
			case "state": {
				setState((prevState) => {
					...
					if (incoming.authenticatedPrincipal) {
						...
						sessionStorage.setItem("authenticatedPrincipal", JSON.stringify(incoming.authenticatedPrincipal))
					} else {
			...
			case "loginSuccess": {
				...
				const authenticatedPrincipalStr = (message as any).authenticatedPrincipal
				...
				if (authenticatedPrincipalStr) {
					sessionStorage.setItem("authenticatedPrincipal", authenticatedPrincipalStr)
	...
	const isAuthenticated = state.isLoggedIn ?? !!(jwtToken || authenticatedPrincipal)
	const currentUser = state.authenticatedPrincipal || authenticatedPrincipal || state.userInfo
