...
export const ExtensionStateContextProvider = ({ children }) => {
    ...
    const [didHydrateState, setDidHydrateState] = useState(() => {
        ...
        const maybeVsCodeWindow = window;
    ...
    const handleMessage = useCallback((event) => {
        ...
        switch (message.type) {
            case "state": {
                ...
                if (!incoming) {
                    console.warn("ExtensionStateContext: received state message without payload");
                ...
                try {
                    const nextHost = incoming.apiConfiguration?.valkyraiHost || DEFAULT_VALKYRAI_HOST;
    ...
    useEffect(() => {
        ...
        const timeoutId = window.setTimeout(() => {
            console.warn("ExtensionStateContext: timed out waiting for extension state, continuing with local defaults");
            setDidHydrateState(true);
        }, 4000);
