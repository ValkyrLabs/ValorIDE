...
import { deriveWsUrlFromHost, getValkyraiHost, subscribeToValkyraiHost, } from "@/utils/valkyraiHost";
const getFallbackWsBase = () => deriveWsUrlFromHost(getValkyraiHost()) ?? "ws://localhost:8080";
...
if (typeof window !== "undefined") {
    try {
        ...
        hostSubscription = subscribeToValkyraiHost(() => {
            try {
                console.log("thorBridge: ValkyrAI host changed, reconnecting broker...");
                if (connectionRetryInterval) {
                    clearInterval(connectionRetryInterval);
                    connectionRetryInterval = null;
                }
