...
- After each codegen, always:
  - Run `npm run sync:thorapi` from the ValorIDE repo to copy the latest generated ThorAPI client from the sibling ValkyrAI checkout into `webview-ui/src/thorapi`.
