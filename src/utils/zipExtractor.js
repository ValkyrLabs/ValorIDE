...
export function isZipBuffer(data) {
    if (!data || data.length < 4) {
        return false;
    }
    return ZIP_SIGNATURES.some((signature) => signature.every((byte, index) => data[index] === byte));
}
/**
 ...
 */
export async function downloadAndExtractZip(url, targetDir, applicationName) {
 ...
 * @param applicationName Optional application name to create a subfolder for extraction
 */
export async function extractLocalZip(zipFilePath, targetDir, applicationName) {
