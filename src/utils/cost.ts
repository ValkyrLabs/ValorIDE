...
function calculateApiCostInternal(
  ...
  if (
    ...
    const tier = sortedInputTiers.find(
      (t) => totalInputTokensForPricing! <= t.tokenLimit,
    );
    if (tier) {
      ...
      effectiveInputPrice =
        sortedInputTiers[sortedInputTiers.length - 1]?.price || 0;
    }
  }
  ...
  if (
    ...
    const tier = sortedOutputTiers.find(
      (t) => totalInputTokensForPricing! <= t.tokenLimit,
    );
    if (tier) {
      ...
      effectiveOutputPrice =
        sortedOutputTiers[sortedOutputTiers.length - 1]?.price || 0;
    }
  }
