...
describe("ChatErrorBoundary", () => {
    it("offers a Get Started recovery path that clears the current chat", async () => {
        const { rerender } = render(_jsx(ChatErrorBoundary, { errorTitle: "Chat failed to render", children: _jsx(ProblemChild, { shouldThrow: true }) }));
        const recoveryButton = screen.getByRole("button", {
            name: /get started/i,
        });
        fireEvent.click(recoveryButton);
        await waitFor(() => expect(TaskServiceClient.clearTask).toHaveBeenCalledTimes(1));
        rerender(_jsx(ChatErrorBoundary, { errorTitle: "Chat failed to render", children: _jsx(ProblemChild, { shouldThrow: false }) }));
