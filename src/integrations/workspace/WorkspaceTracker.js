...
class WorkspaceTracker {
    ...
    disposables = [];
    filePaths = new Set();
    constructor(postMessageToWebview) {
        this.postMessageToWebview = postMessageToWebview;
        this.postMessageToWebview = postMessageToWebview;
        this.registerListeners();
    }
    async populateFilePaths() {
        // should not auto get filepaths for desktop since it would immediately show permission popup before valoride ever creates a file
        if (!cwd) {
            return;
        }
    ...
    registerListeners() {
        ...
        /*
             ...
             */
        // In other words, we don't have to worry about the root workspace folder ([0]) changing since the extension will be restarted and our cwd will be updated to reflect the new workspace folder. (We don't care about non root workspace folders, since valoride will only be working within the root folder cwd)
    ...
    async addFilePath(filePath) {
        ...
        try {
            ...
            const pathWithSlash = isDirectory && !normalizedPath.endsWith("/")
                ? normalizedPath + "/"
                : normalizedPath;
    ...
    async removeFilePath(filePath) {
        const normalizedPath = this.normalizeFilePath(filePath);
        return (this.filePaths.delete(normalizedPath) ||
