...
class ProjectRootDecorationProvider {
...
export function registerProjectExplorerIntegrations(context, output) {
    ...
    const openActions = vscode.commands.registerCommand("valoride.projectActions", async (uri) => {
        if (!uri || !isThorProjectRoot(uri)) {
            vscode.window.showInformationMessage("Select a ValorIDE ThorAPI project folder under the configured ThorAPI output directory to use this.");
            return;
        }
        const items = [
            {
                label: "Open in New Window",
                action: () => vscode.commands.executeCommand("vscode.openFolder", uri, true),
            },
            {
        ...
        const pick = await vscode.window.showQuickPick(items.map((i) => i.label), { placeHolder: path.basename(uri.fsPath) });
...
//# sourceMappingURL=ProjectDecorationProvider.js.map