...
export class DiffViewProvider {
    ...
    async open(relPath) {
        ...
        // get diagnostics before editing the file, we'll compare to diagnostics after editing to see if valoride needs to fix anything
    ...
    async saveChanges() {
        ...
        /*
            ...
            Since these are new problems resulting from ValorIDE's edit, we know they're
            directly related to the work he's doing. This eliminates the risk of ValorIDE
            going off-task or getting distracted by unrelated issues, which was a problem
            with the previous auto-debug approach. Some users' machines may be slow to
            update diagnostics, so this approach provides a good balance between automation
            and avoiding potential issues where ValorIDE might get stuck in loops due to
            outdated problem information. If no new problems show up by the time the user
            accepts the changes, they can always debug later using the '@problems' mention.
            This way, ValorIDE only becomes aware of new problems resulting from his edits
            and can address them accordingly. If problems don't change immediately after
            applying a fix, ValorIDE won't be notified, which is generally fine since the
        ...
        if (normalizedPreSaveContent !== normalizedNewContent) {
            ...
            // no changes to valoride's edits
    ...
    async replaceDocumentContent(document, content) {
        const fullRange = document.validateRange(new vscode.Range(new vscode.Position(0, 0), new vscode.Position(Number.MAX_SAFE_INTEGER, Number.MAX_SAFE_INTEGER)));
        const edit = new vscode.WorkspaceEdit();
        ...
        const fallbackApplied = await editor.edit((editBuilder) => {
    ...
    async openDiffEditor() {
        ...
        return new Promise((resolve, reject) => {
            ...
            }), uri, `${fileName}: ${fileExists ? "Original ↔ ValorIDE's Changes" : "New File"} (Editable)`);
