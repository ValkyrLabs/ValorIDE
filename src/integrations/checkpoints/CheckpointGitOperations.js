 ...
 * GitOperations Class
 *
 * Handles git-specific operations for ValorIDE's Checkpoints system.
 *
    ...
    async initShadowGit(gitPath, cwd, taskId) {
        ...
        if (await fileExistsAtPath(gitPath)) {
            ...
            if (worktree.value !== cwd) {
                throw new Error("Checkpoints can only be used in the original workspace: " +
                    worktree.value);
            }
        ...
        await git.addConfig("user.email", "checkpoint@valkyrlabs.com/v1");
        // Set up LFS patterns
