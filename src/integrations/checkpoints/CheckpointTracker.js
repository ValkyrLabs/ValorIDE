 ...
 *
 * Core implementation of ValorIDE's Checkpoints system that provides version control
...
class CheckpointTracker {
     ...
     *
     * Configuration:
     * - Respects 'valoride.enableCheckpoints' VS Code setting
     */
    static async create(taskId, globalStoragePath) {
        ...
        try {
            ...
            const enableCheckpoints = vscode.workspace
                .getConfiguration("valoride")
                .get("enableCheckpoints") ?? true;
    ...
    async getDiffSummary(lhsHash, rhsHash) {
        const startTime = performance.now();
        const gitPath = await getShadowGitPath(this.globalStoragePath, this.taskId, this.cwdHash);
        const git = simpleGit(path.dirname(gitPath));
        console.info(`Building diff summary: ${lhsHash || "initial"} -> ${rhsHash || "working directory"}`);
        await this.gitOperations.addCheckpointFiles(git);
        const cleanRhs = rhsHash ? this.cleanCommitHash(rhsHash) : undefined;
        const diffRange = cleanRhs
            ? `${this.cleanCommitHash(lhsHash)}..${cleanRhs}`
        ...
        const files = diffSummary.files.map((file) => {
            ...
            const status = this.mapStatusCodeToSummary(statusInfo?.code, !!file.binary);
            return {
                relativePath,
                ...
                status,
                previousRelativePath: statusInfo?.previousPath ??
    ...
    parseNameStatusOutput(rawOutput) {
        const map = new Map();
        if (!rawOutput) {
            return map;
        }
    ...
    mapStatusCodeToSummary(code, isBinary) {
