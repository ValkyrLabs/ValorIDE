...
import { Controller as ValorIDEProvider } from "@core/controller"
import { fileExistsAtPath } from "@utils/fs"
import { globby } from "globby"

class CheckpointTracker {
	private providerRef: WeakRef<ValorIDEProvider>
	...
	private constructor(provider: ValorIDEProvider, taskId: string, cwd: string) {
		this.providerRef = new WeakRef(provider)
		this.taskId = taskId
		this.cwd = cwd
	}

	public static async create(taskId: string, provider?: ValorIDEProvider): Promise<CheckpointTracker | undefined> {
		try {
			if (!provider) {
			...
			const enableCheckpoints = vscode.workspace.getConfiguration("valoride").get<boolean>("enableCheckpoints") ?? true
	...
	public static async doesShadowGitExist(taskId: string, provider?: ValorIDEProvider): Promise<boolean> {
	...
	public async initShadowGit(): Promise<string> {
		...
		if (await fileExistsAtPath(gitPath)) {
			...
			await git.addConfig("user.name", "ValorIDE Checkpoint")
