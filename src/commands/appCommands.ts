...
import { getApps, generateApp, pollAppStatus, getAppDownloadUrl } from "../services/appService"
 ...
 */
export function registerAppCommands(context: vscode.ExtensionContext, controller: { postMessageToWebview: (msg: any) => void }) {
	context.subscriptions.push(
		vscode.commands.registerCommand("valor.listMyApps", async () => {
			try {
				...
				if (!apps.length) {
					vscode.window.showInformationMessage("No applications found for your account.")
					return
				}
...
async function handleGenerateApp(context: vscode.ExtensionContext, jwt: string, app: Application) {
	const appId = app.id
	vscode.window.showInformationMessage(`Triggering code generation for "${app.name}"...`)
	...
	if (status === "completed") {
		...
		try {
			const sanitizedAppName = app.name?.replace(/[^a-zA-Z0-9-_]/g, "_") || "unknown-app"
			...
			vscode.window.showInformationMessage(`App "${app.name}" downloaded and extracted to ${extractedPath}.`)
			const open = await vscode.window.showInformationMessage(`Open "${app.name}" in a new VS Code window?`, "Open Folder")
			if (open === "Open Folder") {
				vscode.commands.executeCommand("vscode.openFolder", vscode.Uri.file(extractedPath), true)
			}
		} catch (err: any) {
...
async function handleOpenAppFolder(app: Application) {
	const homeDir = require("os").homedir()
	const targetDir = require("path").join(homeDir, "valor-projects", app.name.replace(/[^a-zA-Z0-9-_]/g, "_"))
	const fs = require("fs")
	if (!fs.existsSync(targetDir)) {
		vscode.window.showErrorMessage(`Folder for "${app.name}" does not exist: ${targetDir}`)
		return
	}
...
async function handleDeployApp(app: Application) {
	...
	vscode.window.showInformationMessage(`Deploying "${app.name}"... Logs are streaming in the terminal.`)
}
