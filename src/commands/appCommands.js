 ...
 */
export function registerAppCommands(context, controller) {
    context.subscriptions.push(vscode.commands.registerCommand("valor.listMyApps", async () => {
        try {
...
async function handleGenerateApp(context, jwt, app) {
    const appId = app.id;
    ...
    if (status === "completed") {
        ...
        try {
            ...
            const extractedPath = await downloadAndExtractZip(url, targetDir, app.name || "unknown-app");
...
async function handleOpenAppFolder(app) {
    const homeDir = require("os").homedir();
    ...
    if (candidates.length === 0) {
        vscode.window.showErrorMessage(`No versioned folder found for "${app.name}" in ${baseDir}`);
        return;
    }
...
//# sourceMappingURL=appCommands.js.map