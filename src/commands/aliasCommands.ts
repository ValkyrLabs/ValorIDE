...
async function updateTsconfigPaths(
  ...
  rootAliasTargets: {
    componentLib: string;
    reduxServices: string;
    thorAll: string;
  },
  options?: { includeSrc?: string | string[] }
) {
  ...
  if (options?.includeSrc) {
    ...
    for (const pat of toAdd) {
      if (!include.includes(pat)) include.push(pat);
    }
...
export function registerAliasCommands(context: vscode.ExtensionContext) {
  const disposable = vscode.commands.registerCommand(
    ...
    async (resourceUri?: vscode.Uri) => {
      ...
      const picks = await vscode.window.showQuickPick(
        allTsconfigs.map((u) => ({ label: path.basename(u.fsPath), description: u.fsPath, uri: u })),
      ...
      const doInclude = optionPicks?.some((p) => (p as any).key === "include") !== false;
      const doPreview = optionPicks?.some((p) => (p as any).key === "preview") === true;

      // 4) Compute and apply updates
      for (const pick of picks) {
        const tsUri = (pick as any).uri as vscode.Uri;
        ...
        for (const folder of selectedFolders) {
          ...
          if (doInclude) includePatterns.push(path.join(relToFolder, "src").replace(/\\/g, "/"));
        }
        ...
        if (doPreview) {
          ...
          let json: any = {};
          try { json = JSON.parse(original); } catch (e) { void e; }
          if (doPaths) {
            json.compilerOptions = json.compilerOptions || {};
            json.compilerOptions.baseUrl = json.compilerOptions.baseUrl || ".";
            json.compilerOptions.paths = json.compilerOptions.paths || {};
          ...
          if (doInclude && includePatterns.length) {
            const includeArr: string[] = Array.isArray(json.include) ? json.include : [];
            for (const pat of includePatterns) if (!includeArr.includes(pat)) includeArr.push(pat);
            if (includeArr.length) json.include = includeArr;
          }
