...
async function updateTsconfigPaths(
  ...
  if (options?.includeSrc) {
    const toAdd = Array.isArray(options.includeSrc)
      ? options.includeSrc
      : [options.includeSrc];
    ...
    async (resourceUri?: vscode.Uri) => {
      ...
      >(
        [
          ...
          {
            label: "Add include for selected src folders",
            picked: true,
            key: "include",
          },
          {
            label: "Preview updated tsconfig(s) before saving",
            picked: false,
            key: "preview",
          },
        ],
      ...
      for (const pick of picks) {
        ...
        if (doPreview) {
          ...
          const original = await vscode.workspace.fs
            .readFile(tsUri)
          ...
          if (doInclude && includePatterns.length) {
            const includeArr: string[] = Array.isArray(json.include)
              ? json.include
              : [];
            for (const pat of includePatterns)
              if (!includeArr.includes(pat)) includeArr.push(pat);
