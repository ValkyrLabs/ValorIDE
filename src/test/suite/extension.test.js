...
describe("Extension Tests", function () {
	...
	it("should handle basic commands", async () => {
		...
		await vscode.commands.executeCommand("valoride.historyButtonClicked")
		// Success if no error thrown
	})

	it("should handle advanced settings configuration", async () => {
		// Test browser session setting
		await vscode.workspace.getConfiguration().update("valoride.disableBrowserTool", true, true)
		const updatedConfig = vscode.workspace.getConfiguration("valoride")
		expect(updatedConfig.get("disableBrowserTool")).to.be.true

		// Reset settings
		await vscode.workspace.getConfiguration().update("valoride.disableBrowserTool", undefined, true)
	})
})
