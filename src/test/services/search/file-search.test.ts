...
describe("File Search", function () {
  ...
  beforeEach(function () {
    ...
    const spawnWrapper: typeof childProcess.spawn = function (
      command,
      options,
    ) {
  ...
  describe("executeRipgrepForFiles", function () {
    it("should correctly process and return file and folder results", async function () {
      ...
      sandbox
        .stub(fileSearch, "executeRipgrepForFiles")
        .resolves(expectedResult);

      const result = await fileSearch.executeRipgrepForFiles(
        "mock/path",
        "/workspace",
        5000,
      );
    ...
    it("should handle errors from ripgrep", async function () {
      ...
      await should(
        fileSearch.executeRipgrepForFiles("mock/path", "/workspace", 5000),
      ).be.rejectedWith(`ripgrep process error: ${mockError}`);
    });
  });
  ...
  describe("searchWorkspaceFiles", function () {
    ...
    it("should apply fuzzy matching for non-empty query", async function () {
      ...
      sandbox
        .stub(fileSearch, "searchWorkspaceFiles")
        .callsFake(async (query, workspacePath, limit) => {
          if (!query.trim()) {
      ...
      const result = await fileSearch.searchWorkspaceFiles(
        "imp",
        "/workspace",
        2,
      );
