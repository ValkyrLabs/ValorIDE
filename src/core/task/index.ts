...
import { ValorIDEAskResponse, ValorIDECheckpointRestore } from "@shared/WebviewMessage"
...
import {
	...
	saveValorIDEMessages,
} from "@core/storage/disk"
import {
	getGlobalValorIDERules,
...
export class Task {
	...
	private askResponse?: ValorIDEAskResponse
	...
	private async addToValorIDEMessages(message: ValorIDEMessage) {
	...
	private async overwriteValorIDEMessages(newMessages: ValorIDEMessage[]) {
		this.valorideMessages = newMessages
		await this.saveValorIDEMessagesAndUpdateHistory()
	}

	private async saveValorIDEMessagesAndUpdateHistory() {
		try {
			...
			const lastRelevantMessage =
				this.valorideMessages[
					findLastIndex(this.valorideMessages, (m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task"))
				]
	...
	async restoreCheckpoint(messageTs: number, restoreType: ValorIDECheckpointRestore, offset?: number) {
	...
	async handleWebviewAskResponse(askResponse: ValorIDEAskResponse, text?: string, images?: string[]) {
		this.askResponse = askResponse
		this.askResponseText = text
		this.askResponseImages = images
	}

	async say(type: ValorIDESay, text?: string, images?: string[], partial?: boolean): Promise<undefined> {
		if (this.abort) {
			throw new Error("ValorIDE instance aborted")
		}
	...
	async removeLastPartialMessageIfExistsWithType(type: "ask" | "say", askOrSay: ValorIDEAsk | ValorIDESay) {
		const lastMessage = this.valorideMessages.at(-1)
	...
	private async resumeTaskFromHistory() {
		...
		if (lastApiReqStartedIndex !== -1) {
			...
			const { cost, cancelReason }: ValorIDEApiReqInfo = JSON.parse(lastApiReqStarted.text || "{}")
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		const disableBrowserTool = vscode.workspace.getConfiguration("valoride").get<boolean>("disableBrowserTool") ?? false
		...
		const preferredLanguage = getLanguageKey(
			vscode.workspace.getConfiguration("valoride").get<LanguageDisplay>("preferredLanguage"),
		)
		...
		if (valorideIgnoreContent) {
			valorideIgnoreInstructions = formatResponse.valorideIgnoreInstructions(valorideIgnoreContent)
		}
		...
		try {
			...
			const isOpenRouter = this.api instanceof OpenRouterHandler || this.api instanceof ValorIDEHandler
	...
	async presentAssistantMessage() {
		...
		switch (block.type) {
			...
			case "tool_use":
				...
				const askApproval = async (type: ValorIDEAsk, partialMessage?: string) => {
				...
				switch (block.name) {
					...
					case "plan_mode_respond": {
						...
						const sharedMessage = {
							...
							options: parsePartialArrayString(removeClosingTag("options", optionsRaw)),
						} satisfies ValorIDEPlanModeResponse
						try {
							if (block.partial) {
	...
	async recursivelyMakeValorIDERequests(userContent: UserContent, includeFileDetails: boolean = false): Promise<boolean> {
		if (this.abort) {
			throw new Error("ValorIDE instance aborted")
		}
		...
		if (
			...
			await this.ask(
				...
				`ValorIDE has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests. Would you like to reset the count and proceed with the task?`,
			)
		...
		const isFirstRequest = this.valorideMessages.filter((m) => m.say === "api_req_started").length === 0
		if (isFirstRequest) {
		...
		try {
			...
			const updateApiReqMsg = (cancelReason?: ValorIDEApiReqCancelReason, streamingFailedMessage?: string) => {
				this.valorideMessages[lastApiReqIndex].text = JSON.stringify({
			...
			const abortStream = async (cancelReason: ValorIDEApiReqCancelReason, streamingFailedMessage?: string) => {
				if (this.diffViewProvider.isEditing) {
