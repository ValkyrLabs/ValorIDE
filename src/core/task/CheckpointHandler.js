...
export class CheckpointHandler {
    ...
    constructor(taskId, context, messageHandler, updateTaskHistory, postStateToWebview, postMessageToWebview, reinitExistingTaskFromId, cancelTask, saveValorIDEMessagesAndUpdateHistory) {
        this.taskId = taskId;
    ...
    getCheckpointTrackerErrorMessage() {
        return this.checkpointTrackerErrorMessage;
    }
    setCheckpointTrackerErrorMessage(message) {
    ...
    async saveCheckpoint(isAttemptCompletionMessage = false) {
        ...
        if (!isAttemptCompletionMessage) {
            ...
            this.checkpointTracker?.commit().then(async (commitHash) => {
                const lastCheckpointMessage = findLast(valorideMessages, (m) => m.say === "checkpoint_created");
            ...
            const lastCompletionResultMessage = findLast(valorideMessages, (m) => m.say === "completion_result" || m.ask === "completion_result");
    ...
    async restoreCheckpoint(messageTs, restoreType, offset) {
            ...
            case "workspace":
                ...
                if (message.lastCheckpointHash && this.checkpointTracker) {
                    ...
                    try {
                        ...
                        const errorMessage = error instanceof Error ? error.message : "Unknown error";
                        vscode.window.showErrorMessage("Failed to restore offset checkpoint: " + errorMessage);
                        didWorkspaceRestoreFail = true;
                    }
                }
                break;
        ...
        if (!didWorkspaceRestoreFail) {
            ...
            if (restoreType !== "task") {
                ...
                const checkpointMessages = valorideMessages.filter((m) => m.say === "checkpoint_created");
    ...
    async presentMultifileDiff(messageTs, seeNewChangesSinceLastTaskCompletion) {
        ...
        try {
            if (seeNewChangesSinceLastTaskCompletion) {
                ...
                const lastTaskCompletedMessageCheckpointHash = findLast(valorideMessages.slice(0, messageIndex), (m) => m.say === "completion_result")?.lastCheckpointHash;
                const firstCheckpointMessageCheckpointHash = valorideMessages.find((m) => m.say === "checkpoint_created")?.lastCheckpointHash;
    ...
    async doesLatestTaskCompletionHaveNewChanges() {
        const valorideMessages = this.messageHandler.getValorIDEMessages();
        const messageIndex = findLastIndex(valorideMessages, (m) => m.say === "completion_result");
        const message = valorideMessages[messageIndex];
        if (!message) {
        ...
        await this.initializeCheckpointTracker();
        // Get last task completed
        const lastTaskCompletedMessage = findLast(valorideMessages.slice(0, messageIndex), (m) => m.say === "completion_result");
        try {
...
//# sourceMappingURL=CheckpointHandler.js.map