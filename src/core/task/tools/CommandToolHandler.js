        ...
        try {
            if (partial) {
                if (this.context.shouldAutoApproveTool(block.name)) {
                    ...
                    await this.context
                        .ask("command", this.removeClosingTag("command", command, partial), partial)
                        .catch(() => { });
                }
                return { shouldContinue: false };
            }
                ...
                if ((!requiresApprovalPerLLM && autoApproveSafe) ||
                    ...
                    const { response, text, images } = await this.context.ask("command", command +
    ...
    async executeCommand(command) {
        ...
        const flushBuffer = async (force = false) => {
            ...
            try {
                ...
                const { response, text, images } = await awaitAskWithTimeout(askPromise, COMMAND_OUTPUT_TIMEOUT_MS);
...
//# sourceMappingURL=CommandToolHandler.js.map