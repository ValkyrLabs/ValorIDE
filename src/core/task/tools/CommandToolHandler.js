...
export class CommandToolHandler extends BaseToolHandler {
  async execute(block, partial) {
    ...
    Logger.info(
      `[CommandToolHandler] Received execute_command tool partial=${partial} params=${JSON.stringify(block.params)}`,
    );
    ...
    try {
      ...
      Logger.error(
        `[CommandToolHandler] Error executing command: ${error.message}`,
        error,
      );
      return {
        shouldContinue: true,
