...
export class BrowserToolHandler extends BaseToolHandler {
    async execute(block, partial) {
        ...
        if (!action) {
            ...
            return {
                shouldContinue: true,
                toolResponse: await this.context.sayAndCreateMissingParamError("browser_action", "action"),
            };
        }
        ...
        if (action === "launch" && !url) {
            ...
            return {
                shouldContinue: true,
                toolResponse: await this.context.sayAndCreateMissingParamError("browser_action", "url"),
            };
        }
        if (action === "click" && !coordinate) {
            ...
            return {
                shouldContinue: true,
                toolResponse: await this.context.sayAndCreateMissingParamError("browser_action", "coordinate"),
            };
        }
        if (action === "type" && !text) {
            this.context.consecutiveMistakeCount++;
            return {
                shouldContinue: true,
                toolResponse: await this.context.sayAndCreateMissingParamError("browser_action", "text"),
            };
        }
        ...
        if (this.context.shouldAutoApproveTool(block.name)) {
            this.context.removeLastPartialMessageIfExistsWithType("ask", "browser_action");
            await this.context.say("browser_action", message, undefined, false);
            this.context.consecutiveAutoApprovedRequestsCount++;
        }
        else {
            this.context.removeLastPartialMessageIfExistsWithType("say", "browser_action");
        ...
        telemetryService.captureToolUsage(this.context.taskId, block.name, true, // assuming auto-approve or user approved
        true);
        try {
            let result;
            switch (action) {
            ...
            return {
                shouldContinue: true,
                toolResponse: await this.handleError("executing browser action", error),
            };
        }
    }
}
