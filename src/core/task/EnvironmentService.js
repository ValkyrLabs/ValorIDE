...
export class EnvironmentService {
    ...
    fileContextTracker;
    urlContentFetcher;
    constructor(cwd, valorideIgnoreController, terminalManager, fileContextTracker, urlContentFetcher) {
        this.cwd = cwd;
        this.valorideIgnoreController = valorideIgnoreController;
            ...
            Promise.all(userContent.map(async (block) => {
                if (block.type === "text") {
                    ...
                    if (block.text.includes("<feedback>") ||
                        ...
                        let parsedText = await parseMentions(block.text, this.cwd, this.urlContentFetcher, this.fileContextTracker);
    ...
    async getEnvironmentDetails(includeFileDetails = false, didEditFile = false, valorideMessages = [], api, chatSettings) {
        let details = "";
        ...
        if (includeFileDetails) {
            ...
            if (isDesktop) {
                ...
                const result = formatResponse.formatFilesList(this.cwd, files, didHitLimit, this.valorideIgnoreController);
                details += result;
            }
        }
        ...
        if (api && valorideMessages.length > 0) {
            ...
            const modifiedMessages = combineApiRequests(combineCommandSequences(valorideMessages.slice(1)));
