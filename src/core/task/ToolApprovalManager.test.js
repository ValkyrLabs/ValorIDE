...
jest.mock("execa", () => ({ __esModule: true, default: jest.fn(), execa: jest.fn() }), { virtual: true });
...
describe("ToolApprovalManager.normalizeApprovalResponse", () => {
    it("approves yes button clicks and preserves feedback", () => {
        const result = ToolApprovalManager.normalizeApprovalResponse("yesButtonClicked", "Looks good", ["img.png"]);
    ...
    it("rejects non-affirmative responses and returns feedback", () => {
        const result = ToolApprovalManager.normalizeApprovalResponse("messageResponse", "no", undefined);
        expect(result.approved).toBe(false);
        expect(result.feedback).toEqual({ text: "no", images: undefined });
    });
});
//# sourceMappingURL=ToolApprovalManager.test.js.map