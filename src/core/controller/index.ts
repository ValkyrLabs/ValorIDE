...
import { ValorIDECheckpointRestore, WebviewMessage } from "@shared/WebviewMessage"
...
export class Controller {
	...
	*/
	...
	async setUserInfo(info?: { username: string | null; email: string | null; avatarUrl: string | null }) {
		...
		switch (message.type) {
			...
			case "toggleValorIDERule": {
				...
				if (rulePath && typeof enabled === "boolean" && typeof isGlobal === "boolean") {
					if (isGlobal) {
						const toggles =
							((await getGlobalState(this.context, "globalValorIDERulesToggles")) as ValorIDERulesToggles) || {}
						toggles[rulePath] = enabled
						await updateGlobalState(this.context, "globalValorIDERulesToggles", toggles)
					} else {
						const toggles =
							((await getWorkspaceState(this.context, "localValorIDERulesToggles")) as ValorIDERulesToggles) || {}
						toggles[rulePath] = enabled
			...
			case "getThorapiFolderContents": {
				try {
					...
					await this.postMessageToWebview({
						type: "workspaceFiles",
						files: [],
						error: error instanceof Error ? error.message : "Failed to load thorapi folder contents",
						mentionsRequestId: message.mentionsRequestId,
					})
				}
				break
			...
			case "streamToThorapi": {
				try {
					...
					const finalFilename = filename || `application-${applicationId}-${Date.now()}.zip`
					...
					await this.postMessageToWebview({
						...
						streamToThorapiResult: {
							...
							error: error instanceof Error ? error.message : "Failed to stream to thorapi",
						},
					})
				}
				break
	...
	async handleAuthCallback(customToken: string, apiKey: string, authenticatedPrincipal?: any) {
		try {
			...
			await this.postMessageToWebview({
				type: "loginSuccess",
				token: customToken,
				authenticatedPrincipal: authenticatedPrincipal ? JSON.stringify(authenticatedPrincipal) : undefined,
			})
		...
		try {
			...
			if (axios.isAxiosError(error)) {
				if (error.code === "ECONNABORTED") {
					...
					errorMessage = "MCP marketplace service is not available (404). Please check if the service is running."
				} else if (error.response?.status === 500) {
					errorMessage = "MCP marketplace service encountered an internal error. Please try again later."
				} else if (error.response?.status === 503) {
					errorMessage = "MCP marketplace service is temporarily unavailable. Please try again later."
				} else if (!error.response && error.request) {
					errorMessage = "Cannot connect to MCP marketplace service. Please check your network connection."
...
Here is the project's README to help you get started:\n\n${mcpDetails.readmeContent}\n${mcpDetails.llmsInstallationContent}`
	...
	async fixWithValorIDE(code: string, filePath: string, languageId: string, diagnostics: vscode.Diagnostic[]) {
		// Ensure the sidebar view is visible
		await vscode.commands.executeCommand("valoride-dev.SidebarProvider.focus")
		await setTimeoutPromise(100)
