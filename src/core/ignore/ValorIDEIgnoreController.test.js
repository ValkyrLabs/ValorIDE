import { ValorIDEIgnoreController } from "./ValorIDEIgnoreController";
...
describe("ValorIDEIgnoreController", () => {
    ...
    beforeEach(async () => {
        ...
        // Create default .valorideignore file
        await fs.writeFile(path.join(tempDir, ".valorideignore"), [
            ".env",
            "*.secret",
            "private/",
            "# This is a comment",
        ...
        controller = new ValorIDEIgnoreController(tempDir);
    ...
    describe("Default Patterns", () => {
        ...
        it("should block access to .valorideignore file", async () => {
            const result = controller.validateAccess(".valorideignore");
    ...
    describe("Custom Patterns", () => {
        ...
        it("should handle pattern edge cases", async () => {
            await fs.writeFile(path.join(tempDir, ".valorideignore"), ["*.secret", "private/", "*.tmp", "data-*.json", "temp/*"].join("\n"));
            controller = new ValorIDEIgnoreController(tempDir);
            await controller.initialize();
            const results = [
        ...
        // 	await fs.writeFile(
        // 		path.join(tempDir, ".valorideignore"),
        // 		[
        ...
        // })
        it("should handle comments in .valorideignore", async () => {
            // Create a new .valorideignore with comments
            await fs.writeFile(path.join(tempDir, ".valorideignore"), ["# Comment line", "*.secret", "private/", "temp.*"].join("\n"));
            controller = new ValorIDEIgnoreController(tempDir);
            await controller.initialize();
    ...
    describe("Error Handling", () => {
        ...
        it("should handle missing .valorideignore gracefully", async () => {
            // Create a new controller in a directory without .valorideignore
            const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`);
            await fs.mkdir(emptyDir);
            try {
                const controller = new ValorIDEIgnoreController(emptyDir);
                await controller.initialize();
        ...
        it("should handle empty .valorideignore", async () => {
            await fs.writeFile(path.join(tempDir, ".valorideignore"), "");
            controller = new ValorIDEIgnoreController(tempDir);
            await controller.initialize();
    ...
    describe("Include Directive", () => {
        it("should load patterns from an included file", async () => {
            ...
            // Create a .valorideignore file that includes .gitignore and adds an extra pattern "secret.txt"
            await fs.writeFile(path.join(tempDir, ".valorideignore"), ["!include .gitignore", "secret.txt"].join("\n"));
            // Initialize the controller to load the updated .valorideignore
            controller = new ValorIDEIgnoreController(tempDir);
            await controller.initialize();
            ...
            // "secret.txt" should be ignored as specified directly in .valorideignore
        ...
        it("should handle non-existent included file gracefully", async () => {
            // Create a .valorideignore file that includes a non-existent file
            await fs.writeFile(path.join(tempDir, ".valorideignore"), ["!include missing-file.txt"].join("\n"));
        ...
        it("should handle non-existent included file gracefully alongside a valid pattern", async () => {
            ...
            await fs.writeFile(path.join(tempDir, ".valorideignore"), ["!include non-existent.txt", "*.tmp"].join("\n"));
            controller = new ValorIDEIgnoreController(tempDir);
            await controller.initialize();
...
//# sourceMappingURL=ValorIDEIgnoreController.test.js.map