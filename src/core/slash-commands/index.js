 ...
 */
export function parseSlashCommands(text) {
    ...
    const tagPatterns = [
        {
            tag: "task",
            regex: /<task>(\s*\/([a-zA-Z0-9_-]+))(\s+.+?)?\s*<\/task>/is,
        },
        {
            tag: "feedback",
            regex: /<feedback>(\s*\/([a-zA-Z0-9_-]+))(\s+.+?)?\s*<\/feedback>/is,
        },
        {
            tag: "answer",
            regex: /<answer>(\s*\/([a-zA-Z0-9_-]+))(\s+.+?)?\s*<\/answer>/is,
        },
        {
            tag: "user_message",
