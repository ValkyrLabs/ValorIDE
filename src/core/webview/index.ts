...
export class WebviewProvider implements vscode.WebviewViewProvider {
  public static readonly sideBarId = "valoride-dev.SidebarProvider"; // used in package.json as the view's id. This value cannot be changed due to how vscode caches views based on their id, and updating the id would break existing instances of the extension.
  public static readonly tabPanelId = "valoride-dev.TabPanelProvider";
  ...
  async resolveWebviewView(
    ...
    try {
      ...
      ContentDataBridge.getInstance().setWebviewPanel(
        webviewView as vscode.WebviewPanel,
      );
    } catch (e) {
  ...
  private getHtmlContent(webview: vscode.Webview): string {
    ...
    const telecomConfig = {
      turnServers: cfg.get<any[]>("P2P.turnServers", [
  ...
  private async getHMRHtmlContent(webview: vscode.Webview): Promise<string> {
    ...
    const { httpOrigin: devValkyraiOrigin, wsOrigin: devValkyraiWsOrigin } =
      deriveValkyraiOrigins(valkyraiBasePath);
    const connectSrcParts = new Set<string>([
      devValkyraiOrigin || "",
      "https://*",
      `ws://${localServerUrl}`,
