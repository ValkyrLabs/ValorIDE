...
export const GlobalFileNames = {
	...
	mcpSettings: "valoride_mcp_settings.json",
	valorideRules: ".valoriderules",
...
export async function ensureRulesDirectoryExists(): Promise<string> {
	...
	const valorideRulesDir = path.join(userDocumentsPath, "ValorIDE", "Rules")
	try {
		await fs.mkdir(valorideRulesDir, { recursive: true })
	} catch (error) {
		return path.join(os.homedir(), "Documents", "ValorIDE", "Rules") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return valorideRulesDir
}

export async function ensureMcpServersDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const mcpServersDir = path.join(userDocumentsPath, "ValorIDE", "MCP")
	try {
		await fs.mkdir(mcpServersDir, { recursive: true })
	} catch (error) {
		return "~/Documents/ValorIDE/MCP" // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine since this path is only ever used in the system prompt
	}
...
export async function getSavedValorIDEMessages(context: vscode.ExtensionContext, taskId: string): Promise<ValorIDEMessage[]> {
...
export async function saveValorIDEMessages(context: vscode.ExtensionContext, taskId: string, uiMessages: ValorIDEMessage[]) {
	try {
