...
import { ValorIDEIgnoreController, LOCK_TEXT_SYMBOL } from "../ignore/ValorIDEIgnoreController"
...
(This is an automated message, so do not respond to it conversationally.)`,
	...
	formatFilesList: (
		...
		} else if (valorideIgnoreParsed.length === 0 || (valorideIgnoreParsed.length === 1 && valorideIgnoreParsed[0] === "")) {
...
(Remember: If it seems the user wants you to use tools only available in Act Mode, you should ask the user to "toggle to Act mode" (use those words) - they will have to manually do this themselves with the Plan/Act toggle button below. You do not have the ability to switch to Act Mode yourself, and must wait for the user to do it themselves once they are satisfied with the plan. You also cannot present an option to toggle to Act mode, as this will be something you need to direct the user to do manually themselves.)`
	...
	fileEditWithoutUserChanges: (
		...
		`The content was successfully saved to ${relPath.toString()}.\n\n` +
		(autoFormattingEdits
	...
	valorideIgnoreInstructions: (content: string) =>
		`# .valorideignore\n\n(The following is provided by a root-level .valorideignore file where the user has specified files and directories that should not be accessed. When using list_files, you'll notice a ${LOCK_TEXT_SYMBOL} next to files that are blocked. Attempting to access the file's contents e.g. through read_file will result in an error.)\n\n${content}\n.valorideignore`,

	valorideRulesGlobalDirectoryInstructions: (globalValorIDERulesFilePath: string, content: string) =>
	...
	valorideRulesLocalDirectoryInstructions: (cwd: string, content: string) =>
	...
	valorideRulesLocalFileInstructions: (cwd: string, content: string) =>
