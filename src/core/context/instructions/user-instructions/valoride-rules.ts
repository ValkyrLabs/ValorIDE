...
export const getGlobalValorIDERules = async (globalValorIDERulesFilePath: string, toggles: ValorIDERulesToggles) => {
	if (await fileExistsAtPath(globalValorIDERulesFilePath)) {
		if (await isDirectory(globalValorIDERulesFilePath)) {
			try {
				...
				const rulesFilesTotalContent = await getValorIDERulesFilesTotalContent(
					rulesFilePaths,
					globalValorIDERulesFilePath,
					toggles,
				)
				if (rulesFilesTotalContent) {
					const valorideRulesFileInstructions = formatResponse.valorideRulesGlobalDirectoryInstructions(
						globalValorIDERulesFilePath,
...
export const getLocalValorIDERules = async (cwd: string, toggles: ValorIDERulesToggles) => {
	const valorideRulesFilePath = path.resolve(cwd, GlobalFileNames.valorideRules)

	let valorideRulesFileInstructions: string | undefined
	...
	if (await fileExistsAtPath(valorideRulesFilePath)) {
		if (await isDirectory(valorideRulesFilePath)) {
			try {
				...
				if (rulesFilesTotalContent) {
					valorideRulesFileInstructions = formatResponse.valorideRulesLocalDirectoryInstructions(
						cwd,
						rulesFilesTotalContent,
					)
				}
			...
			try {
				if (valorideRulesFilePath in toggles && toggles[valorideRulesFilePath] !== false) {
					const ruleFileContent = (await fs.readFile(valorideRulesFilePath, "utf8")).trim()
					if (ruleFileContent) {
						valorideRulesFileInstructions = formatResponse.valorideRulesLocalFileInstructions(cwd, ruleFileContent)
					}
				}
			} catch {
...
const getValorIDERulesFilesTotalContent = async (rulesFilePaths: string[], basePath: string, toggles: ValorIDERulesToggles) => {
...
export async function synchronizeRuleToggles(
	...
	currentToggles: ValorIDERulesToggles,
): Promise<ValorIDERulesToggles> {
...
export async function refreshValorIDERulesToggles(
	context: vscode.ExtensionContext,
	workingDirectory: string,
): Promise<{
	globalToggles: ValorIDERulesToggles
	localToggles: ValorIDERulesToggles
}> {
	// Global toggles
	const globalValorIDERulesToggles =
		((await getGlobalState(context, "globalValorIDERulesToggles")) as ValorIDERulesToggles) || {}
	...
	const localValorIDERulesToggles =
		((await getWorkspaceState(context, "localValorIDERulesToggles")) as ValorIDERulesToggles) || {}
...
export async function deleteRuleFile(
	...
	try {
		...
		if (isGlobal) {
			const toggles = ((await getGlobalState(context, "globalValorIDERulesToggles")) as ValorIDERulesToggles) || {}
			delete toggles[rulePath]
			await updateGlobalState(context, "globalValorIDERulesToggles", toggles)
		} else {
			const toggles = ((await getWorkspaceState(context, "localValorIDERulesToggles")) as ValorIDERulesToggles) || {}
			delete toggles[rulePath]
