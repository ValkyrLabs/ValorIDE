...
export const getGlobalValorIDERules = async (
  globalValorIDERulesFilePath: string,
  toggles: ValorIDERulesToggles,
) => {
    ...
    if (await isDirectory(globalValorIDERulesFilePath)) {
      try {
        ...
        if (rulesFilesTotalContent) {
          const valorideRulesFileInstructions =
            formatResponse.valorideRulesGlobalDirectoryInstructions(
              globalValorIDERulesFilePath,
        ...
        console.error(
          `Failed to read .valoriderules directory at ${globalValorIDERulesFilePath}`,
        );
      }
    } else {
...
export const getLocalValorIDERules = async (
  cwd: string,
  toggles: ValorIDERulesToggles,
) => {
  const valorideRulesFilePath = path.resolve(
    cwd,
    GlobalFileNames.valorideRules,
  );
      ...
      try {
        ...
        if (rulesFilesTotalContent) {
          valorideRulesFileInstructions =
            formatResponse.valorideRulesLocalDirectoryInstructions(
              cwd,
              rulesFilesTotalContent,
            );
        }
        ...
        console.error(
          `Failed to read .valoriderules directory at ${valorideRulesFilePath}`,
        );
      }
    } else {
      try {
        if (
          valorideRulesFilePath in toggles &&
          toggles[valorideRulesFilePath] !== false
        ) {
          const ruleFileContent = (
            await fs.readFile(valorideRulesFilePath, "utf8")
          ).trim();
          if (ruleFileContent) {
            valorideRulesFileInstructions =
              formatResponse.valorideRulesLocalFileInstructions(
                cwd,
                ruleFileContent,
              );
          }
        ...
        console.error(
          `Failed to read .valoriderules file at ${valorideRulesFilePath}`,
        );
      }
    }
  }
...
const getValorIDERulesFilesTotalContent = async (
...
export async function synchronizeRuleToggles(
  ...
  try {
    ...
    console.error(
      `Failed to synchronize rule toggles for path: ${rulesDirectoryPath}`,
      error,
    );
  }
...
export async function refreshValorIDERulesToggles(
  ...
  const localValorIDERulesFilePath = path.resolve(
    workingDirectory,
    GlobalFileNames.valorideRules,
  );
  const updatedLocalToggles = await synchronizeRuleToggles(
    localValorIDERulesFilePath,
    localValorIDERulesToggles,
  );
