...
export class ContextManager {
    ...
    async saveContextHistory(taskDirectory) {
        try {
            const serializedUpdates = Array.from(this.contextHistoryUpdates.entries()).map(([messageIndex, [numberValue, innerMap]]) => [
                messageIndex,
     ...
     */
    async getNewContextMessagesAndMetadata(apiConversationHistory, valorideMessages, api, conversationHistoryDeletedRange, previousApiReqIndex, taskDirectory) {
        ...
        if (previousApiReqIndex >= 0) {
            ...
            if (previousRequest && previousRequest.text) {
                ...
                if (totalTokens >= maxAllowedSize) {
                    ...
                    let [anyContextUpdates, uniqueFileReadIndices] = this.applyContextOptimizations(apiConversationHistory, conversationHistoryDeletedRange
            ...
            if (message.role === "user" &&
                ...
                if (firstBlock.type === "text") {
                    ...
                    if (matchTup) {
                        ...
                        else if (matchTup[0] === "replace_in_file" ||
    ...
    calculateContextOptimizationMetrics(apiMessages, conversationHistoryDeletedRange, uniqueFileReadIndices) {
        ...
        const secondChunkResult = this.countCharactersAndSavingsInRange(apiMessages, conversationHistoryDeletedRange
            ? conversationHistoryDeletedRange[1] + 1
            : 2, apiMessages.length, uniqueFileReadIndices);
