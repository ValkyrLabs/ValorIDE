...
export class ContextManager {
  ...
  private contextHistoryUpdates: Map<
    number,
    [number, Map<number, ContextUpdate[]>]
  >;
  ...
  async initializeContextHistory(taskDirectory: string) {
    this.contextHistoryUpdates =
      await this.getSavedContextHistory(taskDirectory);
  }
      ...
      if (await fileExistsAtPath(filePath)) {
        ...
        return new Map(
          serializedUpdates.map(
            ([messageIndex, [numberValue, innerMapArray]]) => [
              messageIndex,
  ...
  async getNewContextMessagesAndMetadata(
    ...
    if (previousApiReqIndex >= 0) {
      const previousRequest = valorideMessages[previousApiReqIndex];
      if (previousRequest && previousRequest.text) {
        ...
        if (totalTokens >= maxAllowedSize) {
          // Since the user may switch between models with different context windows, truncating half may not be enough (ie if switching from valoride 200k to deepseek 64k, half truncation will only remove 100k tokens, but we need to remove much more)
          // So if totalTokens/2 is greater than maxAllowedSize, we truncate 3/4 instead of 1/2
          const keep = totalTokens / 2 > maxAllowedSize ? "quarter" : "half";

          // we later check how many chars we trim to determine if we should still truncate history
          let [anyContextUpdates, uniqueFileReadIndices] =
          ...
          if (anyContextUpdates) {
            ...
            const charactersSavedPercentage =
  ...
  public getNextTruncationRange(
    ...
    if (keep === "none") {
      ...
      // Note that this will also always be an even number.
      messagesToRemove =
        Math.floor(((apiMessages.length - startOfRest) * 3) / 4 / 2) * 2;
    }
  ...
  private applyContextHistoryUpdates(
    ...
    for (
      let arrayIndex = 0;
      arrayIndex < messagesToUpdate.length;
      arrayIndex++
    ) {
  ...
  private applyStandardContextTruncationNoticeChange(
    ...
    if (!this.contextHistoryUpdates.has(1)) {
      ...
      innerMap.set(0, [
        [timestamp, "text", [formatResponse.contextTruncationNotice()], []],
      ]);
  ...
  private findAndPotentiallySaveFileReadContextHistoryUpdates(
    ...
    const [fileReadIndices, messageFilePaths] =
      this.getPossibleDuplicateFileReads(apiMessages, startFromIndex);
            ...
            if (blockUpdates && blockUpdates.length > 0) {
              ...
              // to do so we need to keep track of which files we've already replaced so we don't replace them again
                ...
                thisExistingFileReads =
                  blockUpdates[blockUpdates.length - 1][3][0];
              }
            }
          } else {
        ...
        if (firstBlock.type === "text") {
          ...
          if (matchTup) {
            if (matchTup[0] === "read_file") {
              ...
              matchTup[0] === "replace_in_file" ||
              matchTup[0] === "write_to_file"
            ) {
  ...
  private handlePotentialFileMentionCalls(
    ...
    const pattern = new RegExp(
      `<file_content path="([^"]*)">([\\s\\S]*?)</file_content>`,
      "g",
    );
  ...
  private applyFileReadContextHistoryUpdates(
          ...
          if (messageType === EditType.FILE_MENTION) {
            if (!fileMentionUpdates.has(messageIndex)) {
              ...
              if (innerTuple) {
                ...
                if (blockUpdates && blockUpdates.length > 0) {
                  ...
                  prevFilesReplaced =
                    blockUpdates[blockUpdates.length - 1][3][0]; // previously overwritten file reads in this text
                }
              }
              ...
              fileMentionUpdates.set(messageIndex, [
                baseText,
                prevFilesReplaced,
              ]);
            }
            ...
            if (searchText) {
              ...
              if (currentTuple[0]) {
                ...
                const updatedText = currentTuple[0].replace(
                  searchText,
                  messageString,
                );
            ...
            if (!innerTuple) {
              ...
              this.contextHistoryUpdates.set(messageIndex, [
                messageType,
                innerMap,
              ]);
            } else {
    ...
    for (const [
      messageIndex,
      [updatedText, filePathsUpdated],
    ] of fileMentionUpdates.entries()) {
      let innerTuple = this.contextHistoryUpdates.get(messageIndex);
      let innerMap: Map<number, ContextUpdate[]>;
  ...
  private countCharactersAndSavingsInRange(
            ...
            if (hasExistingAlterations) {
              ...
              if (updates && updates.length > 0) {
                ...
                if (hasNewAlterations) {
                  ...
                  if (updates.length > 1) {
                    originalTextLength =
                      updates[updates.length - 2][2][0].length; // handles case if we have multiple updates for same text block
                  } else {
    ...
    return {
      ...
      charactersSaved: totalCharactersSaved,
    };
  }
  ...
  private calculateContextOptimizationMetrics(
    ...
    const totalCharacters =
      firstChunkResult.totalCharacters + secondChunkResult.totalCharacters;
