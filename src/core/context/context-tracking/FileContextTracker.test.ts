...
describe("FileContextTracker", () => {
  ...
  beforeEach(() => {
    ...
    getTaskMetadataStub = sandbox
      .stub(diskModule, "getTaskMetadata")
      .resolves(mockTaskMetadata);
    saveTaskMetadataStub = sandbox
      .stub(diskModule, "saveTaskMetadata")
      .resolves();
  ...
  it("should add a record when a file is read by a tool", async () => {
    ...
    expect(fileEntry.valoride_read_date).to.be.a("number");
    expect(fileEntry.valoride_edit_date).to.be.null;
  });

  it("should add a record when a file is edited by ValorIDE", async () => {
    const filePath = "src/test-file.ts";

    await tracker.trackFileContext(filePath, "valoride_edited");
    ...
    const activeEntry = savedMetadata.files_in_context.find(
      (entry: FileMetadataEntry) =>
        entry.path === filePath && entry.record_state === "active",
    );
    ...
    expect(activeEntry.record_source).to.equal("valoride_edited");
    expect(activeEntry.valoride_read_date).to.be.a("number");
    expect(activeEntry.valoride_edit_date).to.be.a("number");
  });
  ...
  it("should mark existing entries as stale when adding a new entry for the same file", async () => {
    ...
    mockTaskMetadata.files_in_context = [
      {
        ...
        valoride_read_date: Date.now() - 1000, // 1 second ago
        valoride_edit_date: null,
        user_edit_date: null,
      },
    ];
    ...
    expect(newEntry.record_source).to.equal("valoride_edited");
  });
  ...
  it("should not track ValorIDE edits as user edits", async () => {
    const filePath = "src/test-file.ts";

    // First track the file to set up the watcher
    await tracker.trackFileContext(filePath, "read_tool");

    // Mark the file as edited by ValorIDE
    tracker.markFileAsEditedByValorIDE(filePath);
