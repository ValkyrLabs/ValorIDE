...
describe("convertToAnthropicRole", () => {
  it("should convert VSCode roles to Anthropic roles", () => {
    ...
    (
      convertToAnthropicRole(vscode.LanguageModelChatMessageRole.Assistant) ===
      "assistant"
    ).should.be.true();
    (
      convertToAnthropicRole(vscode.LanguageModelChatMessageRole.User) ===
      "user"
    ).should.be.true();
...
describe("convertToVsCodeLmMessages", () => {
  ...
  it("should convert complex user messages with tool results", () => {
    ...
    result[0].content[0].should.be.instanceof(
      vscode.LanguageModelToolResultPart,
    );
    const toolResultPart = result[0]
  ...
  it("should convert complex assistant messages with tool calls", () => {
    ...
    const toolCallPart = result[0]
      .content[0] as vscode.LanguageModelToolCallPart;
  ...
  it("should handle image blocks with appropriate placeholders", () => {
    ...
    textPart.value.should.match(
      /Image \(base64\): image\/jpeg not supported by VSCode LM API/,
    );
  });
});
    ...
    if (result.content && result.content.length >= 2) {
      ...
      if (toolContent) {
        ...
        if (toolContent.type === "tool_use") {
          ...
          toolContent.should.have
            .property("input")
            .which.deepEqual({ param: "value" });
        }
      }
    }
  });
