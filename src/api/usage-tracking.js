 ...
 */
export async function trackApiUsage(provider, model, inputTokens, outputTokens, costPerInputToken, costPerOutputToken) {
    try {
        ...
        switch (provider.toLowerCase()) {
            case 'openai':
                await usageTrackingService.trackOpenAIUsage(model, inputTokens, outputTokens, costPerInputToken, costPerOutputToken);
                return;
            case 'anthropic':
                await usageTrackingService.trackAnthropicUsage(model, inputTokens, outputTokens, costPerInputToken, costPerOutputToken);
                return;
            default:
                ...
                await usageTrackingService.trackGenericUsage(provider, model, inputTokens, outputTokens, credits);
                break;
        }
    }
    catch (error) {
 ...
 */
export async function trackApiUsageWithPricing(provider, model, inputTokens, outputTokens) {
    const pricing = getModelPricing(provider, model);
    await trackApiUsage(provider, model, inputTokens, outputTokens, pricing.inputCost, pricing.outputCost);
}
