...
describe("OllamaHandler", () => {
  ...
  describe("createMessage", () => {
    ...
    it("should handle timeout errors", async function () {
      ...
      testHandler.createMessage = async function* (systemPrompt, messages) {
        try {
          ...
          const timeoutPromise = new Promise<never>((_, reject) => {
            setTimeout(
              () =>
                reject(new Error("Ollama request timed out after 120 seconds")),
              100,
            );
          });
      ...
      let errorMessage = "";
      try {
        for await (const _ of testHandler.createMessage(
          systemPrompt,
          messages,
        )) {
    ...
    it("should handle stream processing errors", async function () {
      ...
      let errorMessage = "";
      try {
        for await (const chunk of testHandler.createMessage(
          systemPrompt,
          messages,
        )) {
          if (chunk.type === "text") {
