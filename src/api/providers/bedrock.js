...
// https://docs.anthropic.com/en/api/valoride-on-amazon-bedrock
export class AwsBedrockHandler {
  ...
  async getAnthropicClient() {
    ...
    return new AnthropicBedrock({
      ...
      ...(this.options.awsBedrockEndpoint && {
        baseURL: this.options.awsBedrockEndpoint,
      }),
    });
  }
  /**
        ...
        content = message.content
          .map((item) => {
            ...
            if (item.type === "image") {
              ...
              if ("media_type" in item.source) {
                // Extract format from media_type (e.g., "image/jpeg" -> "jpeg")
                const formatMatch =
                  item.source.media_type.match(/image\/(\w+)/);
              ...
              try {
                if (
                  ...
                  const base64Data = item.source.data.replace(
                    /^data:image\/\w+;base64,/,
                    "",
                  );
                  ...
                  "data" in item.source &&
                  typeof item.source.data === "object"
                ) {
                ...
                console.error(
                  "Could not convert image data to Uint8Array:",
                  error,
                );
...
__decorate([withRetry()], AwsBedrockHandler.prototype, "createMessage", null);
