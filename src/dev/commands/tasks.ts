        ...
        vscode.window.withProgress(
          ...
          async (progress) => {
            for (let i = 0; i < tasksCount; i++) {
              ...
              const messages = createRealisticMessageSequence(
                timestamp,
                taskName,
                i,
              );
              ...
              const historyItem: HistoryItem = {
                ...
                cacheWrites:
                  i % 3 === 0
                    ...
                    : undefined, // Only add cache writes to every 3rd task
                cacheReads:
                  i % 3 === 0 ? Math.floor(20 + Math.random() * 80) : undefined, // Only add cache reads to every 3rd task
...
function createRealisticMessageSequence(
  ...
  const messages: ValorIDEMessage[] = [
    // Initial task message - uses "say" with "text" which is the format used in ValorIDE.ts
    {
      ts: baseTimestamp,
      type: "say",
      say: "text",
