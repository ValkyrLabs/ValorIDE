...
export interface PromptBroadcastMessage {
  type: "PROMPT_SELECT";
  supervisorId: string;
  selectedLlmDetailsId: string;
  scope: "ALL_WORKERS" | "TAG_FILTER";
  tags?: string[];
  timestamp: number;
}
...
export class SwarmPromptBroadcaster {
  ...
  private registeredWorkers: Map<string, WorkerRegistration> = new Map();
  private broadcastHistory: PromptBroadcastMessage[] = [];
  private pendingPromptChanges: Map<string, string> = new Map(); // workerId -> llmDetailsId
   ...
   */
  registerWorker(workerId: string, tags: string[] = []): void {
    ...
    this.logger.appendLine(
      `[SwarmPromptBroadcaster] Worker registered: ${workerId}`,
    );
  }
  ...
  unregisterWorker(workerId: string): void {
    ...
    this.logger.appendLine(
      `[SwarmPromptBroadcaster] Worker unregistered: ${workerId}`,
    );
  }
  ...
  selectPromptForTask(
    ...
    llmDetailsId: string,
    tags: string[] = [],
  ): PromptBroadcastMessage {
    const message: PromptBroadcastMessage = {
      type: "PROMPT_SELECT",
      supervisorId,
    ...
    this.logger.appendLine(
      `[SwarmPromptBroadcaster] Task intent: ${taskIntent} → LLMDetails: ${llmDetailsId}`,
    );
   ...
   */
  broadcastPromptSelection(message: PromptBroadcastMessage): string[] {
    ...
    this.logger.appendLine(
      `[SwarmPromptBroadcaster] Broadcast to ${targetWorkers.length} workers: ${message.selectedLlmDetailsId}`,
    );
   ...
   */
  acknowledgePromptReload(workerId: string, llmDetailsId: string): boolean {
    ...
    if (pending === llmDetailsId) {
      ...
      this.pendingPromptChanges.delete(workerId);
      this.logger.appendLine(
        `[SwarmPromptBroadcaster] Worker ${workerId} loaded prompt: ${llmDetailsId}`,
      );
   ...
   */
  getPendingPromptChange(workerId: string): string | undefined {
   ...
   */
  getRegisteredWorkers(): WorkerRegistration[] {
   ...
   */
  getWorkersByTags(tags: string[]): WorkerRegistration[] {
    return Array.from(this.registeredWorkers.values()).filter((worker) =>
      tags.some((tag) => worker.tags.includes(tag)),
    );
  }
   ...
   */
  getBroadcastHistory(): PromptBroadcastMessage[] {
   ...
   * Get worker status
   */
  getWorkerStatus(workerId: string): WorkerRegistration | undefined {
  ...
  logSwarmState(): void {
    ...
    this.logger.appendLine(
      `  Pending prompt changes: ${this.pendingPromptChanges.size}`,
    );
    this.logger.appendLine(
      `  Broadcast history: ${this.broadcastHistory.length}`,
    );

    for (const [workerId, worker] of this.registeredWorkers) {
      const timeSinceHeartbeat = Date.now() - worker.lastHeartbeat;
      this.logger.appendLine(
        `  - ${workerId}: tags=${worker.tags.join(",")}, prompt=${worker.promptId || "none"}, heartbeat=${timeSinceHeartbeat}ms`,
      );
    }
  }
}

let swarmPromptBroadcaster: SwarmPromptBroadcaster | null = null;

/**
 * Initialize global SwarmPromptBroadcaster
 */
export function initializeSwarmPromptBroadcaster(
 ...
 */
export function getSwarmPromptBroadcaster(): SwarmPromptBroadcaster {
