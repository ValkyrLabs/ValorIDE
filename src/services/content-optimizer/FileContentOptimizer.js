...
export class FileContentOptimizer {
  ...
  static DEFAULT_MAX_CONTENT_LENGTH = 10000; // 10k chars
  static LARGE_FILE_WARNING =
   ...
   */
  static async extractRelevantContent(filePath, content, config) {
    ...
    if (this.isSourceCodeFile(ext)) {
      try {
        ...
        const relevantParts = this.extractRelevantCodeParts(
          content,
          definitions,
        );
   ...
   */
  static truncateIntelligently(content, config) {
    const maxLength =
      config.maxContentLength || this.DEFAULT_MAX_CONTENT_LENGTH;
   ...
   */
  static findLogicalTruncatePoint(content, maxLength) {
   ...
   */
  static async shouldOptimizeFile(filePath) {
    try {
...
//# sourceMappingURL=FileContentOptimizer.js.map
