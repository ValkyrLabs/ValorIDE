  ...
  "mcpServers": {
	...
	private async fetchToolsList(serverName: string): Promise<McpTool[]> {
		try {
			...
			const tools = (response?.tools || [])
				.filter((tool) => tool.name) // Ensure the tool has a name
				.map((tool) => ({
					...tool,
	...
	private async fetchResourcesList(serverName: string): Promise<McpResource[]> {
		try {
			...
			return (response?.resources || []).filter((resource) => resource.uri) as McpResource[]
		} catch (error) {
	...
	private async fetchResourceTemplatesList(serverName: string): Promise<McpResourceTemplate[]> {
		try {
			...
			return (response?.resourceTemplates || []).map((template) => ({
				...
				name: template.name || "Unnamed Template", // Ensure 'name' is always provided
			}))
		} catch (error) {
	...
	private setupFileWatcher(name: string, config: Extract<McpServerConfig, { transportType: "stdio" }>) {
		...
		if (filePath) {
			// we use chokidar instead of onDidSaveTextDocument because it doesn't require the file to be open in the editor. The settings config is better suited for onDidSave since that will be manually updated by the user or ValorIDE (and we want to detect save events, not every file change)
	...
	async readResource(serverName: string, uri: string): Promise<McpResourceResponse> {
		...
		// Transform the contents to match the expected type
		return {
			...response,
	...
	async callTool(serverName: string, toolName: string, toolArguments?: Record<string, unknown>): Promise<McpToolCallResponse> {
		...
		// Transform the content to match the expected type
