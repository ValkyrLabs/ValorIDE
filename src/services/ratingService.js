...
export class RatingServiceImpl {
    ...
    recordRating(metrics, toolFailures = []) {
        ...
        this.logger.appendLine(`[RatingService] Recorded rating: ${rating.ratingId} (${metrics.taskType}, success=${metrics.success})`);
        return rating;
    }
    /**
     * Analyze ratings to identify prompt weaknesses
    ...
    analyzePromptWeaknesses() {
        ...
        this.ratings.forEach((rating) => {
            ...
            if (psrFail) {
                psrFailures++;
                if (rating.llmDetailsId)
                    psrFailedPrompts.add(rating.llmDetailsId);
            }
            psrTotal++;
            ...
            if (rating.tokensCost > 10) {
                highCostTasks++;
                if (rating.llmDetailsId)
                    highCostPrompts.add(rating.llmDetailsId);
            }
            // Low rating (<3 stars)
            if (rating.userRating && rating.userRating < 3) {
                lowRatedTasks++;
                if (rating.llmDetailsId)
                    lowRatedPrompts.add(rating.llmDetailsId);
            }
        });
        // PSR weakness
        if (psrTotal > 0 && psrFailures / psrTotal > 0.1) {
            weaknesses.push({
                ...
                suggestion: "Strengthen AST-first protocol; add more contextual regex examples",
                impact: "high",
            });
        }
        // High cost weakness
        if (highCostTasks > 0) {
            weaknesses.push({
                ...
                suggestion: "Reduce prompt verbosity; use cheaper models for QA stages; add prompt caching",
                impact: "medium",
            });
        }
        // Low rating weakness
        if (lowRatedTasks > 0) {
            weaknesses.push({
                ...
                suggestion: "Gather user feedback; analyze which sections caused issues; refine examples",
                impact: "high",
            });
        }
        this.logger.appendLine(`[RatingService] Identified ${weaknesses.length} prompt weaknesses`);
        return weaknesses;
    }
    /**
    ...
    getPromptStats(llmDetailsId) {
        const promptRatings = Array.from(this.ratings.values()).filter((r) => r.llmDetailsId === llmDetailsId);
        ...
        const ratingsWithScore = promptRatings.filter((r) => r.userRating !== undefined);
    ...
    getRatingsByTimeRange(startTime, endTime) {
        return Array.from(this.ratings.values()).filter((r) => r.createdAt >= startTime && r.createdAt <= endTime);
    }
    /**
    ...
    logSummary() {
        ...
        this.logger.appendLine(`  Success rate: ${((successCount / all.length) * 100).toFixed(1)}%`);
        this.logger.appendLine(`  Avg cost per task: $${avgCost.toFixed(2)}`);
        this.logger.appendLine(`  Avg user rating: ${avgRating.toFixed(2)}/5`);
        this.logger.appendLine(`  Tool failures: ${all.filter((r) => r.toolFailures.length > 0).length}`);
    }
}
