...
export class MemoryBankLoader {
    ...
    async initialize() {
        ...
        try {
            if (!fs.existsSync(this.memoryBankDir)) {
                this.logger.appendLine(`[MemoryBankLoader] Memory bank directory not found at ${this.memoryBankDir}`);
                return;
            }
            const context = {
            ...
            this.logger.appendLine(`[MemoryBankLoader] ❌ Initialization failed: ${error}`);
        }
    }
    /**
    ...
    logState() {
        ...
        this.logger.appendLine(`  - projectContext: ${this.memoryBank.projectContext.content ? "✅" : "❌"}`);
        this.logger.appendLine(`  - activeContext: ${this.memoryBank.activeContext.content ? "✅" : "❌"}`);
        this.logger.appendLine(`  - techContext: ${this.memoryBank.techContext.content ? "✅" : "❌"}`);
        this.logger.appendLine(`  - systemPatterns: ${this.memoryBank.systemPatterns.content ? "✅" : "❌"}`);
        this.logger.appendLine(`  - progress: ${this.memoryBank.progress.content ? "✅" : "❌"}`);
    }
    /**
    ...
    async updateActiveContext(taskLog) {
        ...
        try {
            ...
            this.logger.appendLine(`[MemoryBankLoader] ❌ Failed to update activeContext.md: ${error}`);
        }
    }
    /**
    ...
    async updateProgress(milestoneLog) {
        ...
        try {
            ...
            this.logger.appendLine(`[MemoryBankLoader] ❌ Failed to update progress.md: ${error}`);
        }
    }
    /**
