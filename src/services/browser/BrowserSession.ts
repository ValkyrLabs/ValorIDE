...
export class BrowserSession {
  ...
  async getDetectedChromePath(): Promise<{ path: string; isBundled: boolean }> {
    ...
    try {
      ...
      if (
        systemPath &&
        !systemPath.includes(".Trash") &&
        (await fileExistsAtPath(systemPath))
      ) {
  ...
  async launchBrowser() {
    ...
    if (this.browserSettings.remoteBrowserEnabled) {
      ...
      try {
        ...
        if (this.taskId) {
          telemetryService.captureBrowserToolStart(
            this.taskId,
  ...
  async launchRemoteBrowser() {
    ...
    if (
      browserWSEndpoint &&
      Date.now() - this.lastConnectionAttempt < 3600000
    ) {
      try {
        console.info(
          `Attempting to connect using cached WebSocket endpoint: ${browserWSEndpoint}`,
        );
    ...
    if (remoteBrowserHost) {
      try {
        ...
        if (!browserWSEndpoint) {
          throw new Error(
            "Could not find webSocketDebuggerUrl in the response",
          );
        }
