...
export class BrowserErrorCapture {
    ...
    initializeErrorPatterns() {
        ...
        // Missing dependency
        this.errorPatterns.set(/Cannot find module|MODULE_NOT_FOUND|not installed/i, {
        ...
        this.errorPatterns.set(/Failed to fetch|Network error|Connection refused|ECONNREFUSED/i, {
        ...
        this.errorPatterns.set(/Cannot read propert|Cannot read properties|undefined is not|null is not/i, {
        ...
        // Type errors
        this.errorPatterns.set(/is not a function|is not a type|Type .* is not assignable/i, {
    ...
    captureError(error) {
        ...
        this.logger.appendLine(`[BrowserErrorCapture] ${error.severity.toUpperCase()}: ${error.message}`);
        if (error.stack) {
    ...
    analyzeError(error) {
        for (const [pattern, analysis] of this.errorPatterns) {
            if (pattern.test(error.message)) {
                this.logger.appendLine(`[BrowserErrorCapture] Matched error pattern: ${analysis.errorType}`);
                return analysis;
            }
        }
