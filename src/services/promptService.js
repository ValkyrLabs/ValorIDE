...
export class PromptService {
    ...
    async initialize() {
        ...
        try {
            ...
            this.logger.appendLine(`[PromptService] ❌ Initialization failed: ${error}`);
            throw error;
        }
    }
    /**
    ...
    async loadSystemPrompt() {
        ...
        this.logger.appendLine(`[PromptService] Loading system.json from ${filePath}`);
        if (!fs.existsSync(filePath)) {
            throw new Error(`system.json not found at ${filePath}`);
        }
        const content = fs.readFileSync(filePath, "utf-8");
        this.systemPrompt = JSON.parse(content);
        this.logger.appendLine(`[PromptService] ✅ Loaded system.json (${this.systemPrompt.sections.length} sections)`);
    }
    /**
    ...
    async loadThorAPICatalog() {
        ...
        this.logger.appendLine(`[PromptService] Loading thorapi-catalog.json from ${filePath}`);
        if (!fs.existsSync(filePath)) {
            throw new Error(`thorapi-catalog.json not found at ${filePath}`);
        }
        const content = fs.readFileSync(filePath, "utf-8");
        this.thorapiCatalog = JSON.parse(content);
        this.logger.appendLine(`[PromptService] ✅ Loaded thorapi-catalog.json (${this.thorapiCatalog.services.length} services, ${this.thorapiCatalog.models.length} models)`);
    }
    /**
    ...
    async loadSwarmRules() {
        ...
        this.logger.appendLine(`[PromptService] Loading swarm-rules.json from ${filePath}`);
        if (!fs.existsSync(filePath)) {
            throw new Error(`swarm-rules.json not found at ${filePath}`);
        }
        const content = fs.readFileSync(filePath, "utf-8");
        this.swarmRules = JSON.parse(content);
        this.logger.appendLine(`[PromptService] ✅ Loaded swarm-rules.json (${this.swarmRules.worker_agents.length} workers)`);
    }
    /**
     ...
     */
    promptsExist() {
        return (fs.existsSync(path.join(this.promptDir, "system.json")) &&
    ...
    logState() {
        ...
        this.logger.appendLine(`  - System prompt: ${this.systemPrompt ? "✅" : "❌"}`);
        this.logger.appendLine(`  - ThorAPI catalog: ${this.thorapiCatalog ? "✅" : "❌"}`);
