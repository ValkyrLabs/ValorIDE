...
export class ToolRelayService {
  ...
  private readonly allowedTools = loadToolAllowlist(
    process.env.VALORIDE_TOOL_ALLOWLIST,
  );
  private readonly commandAllowlist = loadCommandAllowlist(
    process.env.VALORIDE_TOOL_COMMAND_ALLOWLIST,
  );
  ...
  private async handleIncomingToolCommand(command: ToolCommand) {
    ...
    try {
      ...
      this.sendToolResult(
        command.commandId,
        true,
        toResultCard(command.toolName, result),
      );
    } catch (error: any) {
      console.error(
