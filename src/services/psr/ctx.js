 ...
 */
export async function applyContextualPatches(ctx, edits) {
    if (!edits.length)
        return ctx;
    let text = ctx.text;
        ...
        if (!regexValid || !matches.length) {
            ...
            if (/^[\w\s.,:;\-_'"()[\]{}<>!?@#$%^&*+=/\\|~`]+$/.test(edit.find)) {
                ...
                if (fallbackResult.replaced) {
                    ...
                    ctx.warnings.push(`contextual edit ${index}: regex ${regexValid ? "no match" : "invalid"}, used literal fallback (${fallbackResult.count} replacement${fallbackResult.count !== 1 ? "s" : ""})`);
                    usedFallback = true;
                    continue;
                }
            }
 ...
 */
function applyLiteralFallback(text, find, replace, occurrence) {
    let replaced = false;
    let count = 0;
    let out = text;
...
function selectMatches(matches, occurrence) {
    if (occurrence === "first")
...
function buildReplacement(template, match, input) {
...
function detectTagMutation(original, replacement, start) {
...
function applyTagMutations(text, mutations) {
    if (!mutations.length) {
...
function applyHtmlTagMutation(text, mutation) {
    ...
    if (mutation.orientation === "open") {
        ...
        if (tagEnd === -1) {
            warnings.push(`tag_balance_failed: unable to locate end of <${mutation.newName}> while enforcing closing tag rename`);
            return { text: out, warnings, adjustments };
        }
        const closing = findMatchingHtmlClosing(out, tagEnd + 1, new Set([mutation.oldName, mutation.newName]));
        if (!closing) {
            warnings.push(`tag_balance_failed: no closing tag found for <${mutation.oldName}> when renaming to <${mutation.newName}>`);
            return { text: out, warnings, adjustments };
        }
    ...
    if (!open) {
        warnings.push(`tag_balance_failed: no opening tag found for </${mutation.newName}> while renaming from </${mutation.oldName}>`);
        return { text: out, warnings, adjustments };
    }
...
function applyMustacheTagMutation(text, mutation) {
    const warnings = [];
    const adjustments = [];
    let out = text;
    if (mutation.orientation === "open") {
        ...
        if (tagEnd === -1) {
            warnings.push(`tag_balance_failed: unterminated mustache tag for ${mutation.oldName} when renaming to ${mutation.newName}`);
            return { text: out, warnings, adjustments };
        }
        const closing = findMatchingMustacheClosing(out, tagEnd, new Set([mutation.oldName, mutation.newName]));
        if (!closing) {
            warnings.push(`tag_balance_failed: no closing mustache tag found for ${mutation.oldName} when renaming to ${mutation.newName}`);
            return { text: out, warnings, adjustments };
        }
        if (closing.currentName === mutation.newName) {
            return { text: out, warnings, adjustments };
    ...
    if (!opening) {
        warnings.push(`tag_balance_failed: no opening mustache tag found for ${mutation.oldName} when renaming to ${mutation.newName}`);
        return { text: out, warnings, adjustments };
    }
...
function isSelfClosingSnippet(snippet) {
...
function isHtmlClosingAt(text, index) {
    let i = index + 1;
...
function isHtmlSelfClosingAt(text, index) {
...
function findHtmlNameStart(text, index) {
    let i = index + 1;
    while (i < text.length && /\s/.test(text[i])) {
        i++;
    }
...
function findMatchingHtmlClosing(text, start, names) {
...
function findMatchingHtmlOpen(text, closingIndex, names) {
    const tagRegex = /<\s*\/?\s*([A-Za-z0-9_.$:-]+)/g;
    const tokens = [];
    let match;
...
function findMustacheEnd(text, start) {
    const marker = text.indexOf("}}", start);
    return marker === -1 ? -1 : marker + 2;
}
function findMatchingMustacheClosing(text, start, names) {
...
function findMatchingMustacheOpen(text, closingIndex, names) {
    const tokenRegex = /{{\s*([#/^])\s*([^\s{}]+)[^}]*}}/g;
    const tokens = [];
    let match;
...
function mustacheNameBounds(match, name) {
    const full = match[0];
