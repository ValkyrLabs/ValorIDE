...
import { SwarmOrchestrator, TaskIntent, WorkerType, SwarmPriority, } from "../swarmOrchestrator";
// Mock logger
const mockLogger = {
    appendLine: vi.fn(),
};
describe("SwarmOrchestrator", () => {
    ...
    describe("Load Balancing", () => {
        ...
        it("should calculate score correctly", async () => {
            ...
            const cliStatus = status.find((s) => s.name === WorkerType.CLI_TEST_RUNNER);
    ...
    describe("Checkpoint Management", () => {
        it("should save checkpoint", () => {
            const checkpoint = orchestrator.saveCheckpoint("task-1", "STAGE_4_TESTING", "worker-1", {
        ...
        it("should retrieve checkpoint", () => {
            const saved = orchestrator.saveCheckpoint("task-2", "STAGE_5_STAGING", "worker-2", {
    ...
    describe("Rollback", () => {
        it("should rollback to checkpoint", async () => {
            const checkpoint = orchestrator.saveCheckpoint("task-3", "STAGE_3_ASSEMBLY", "worker-3", {
                modified: ["src/app.ts", "src/config.ts"],
                created: ["dist/app.js", "dist/config.js"],
                deleted: ["tmp/old-file.ts"],
            }, { duration: 120 });
            const success = await orchestrator.rollbackToCheckpoint(checkpoint.checkpointId);
            expect(success).toBe(true);
            expect(mockLogger.appendLine).toHaveBeenCalledWith(expect.stringContaining("Restore 2 modified files"));
        });
    ...
    describe("Logging", () => {
        it("should log state", () => {
            ...
            expect(mockLogger.appendLine).toHaveBeenCalledWith("[SwarmOrchestrator] State:");
            expect(mockLogger.appendLine).toHaveBeenCalledWith(expect.stringContaining("Task Queue Length"));
        });
    });
});
