...
export class UsageTrackingService {
  ...
  async trackUsage(usageData) {
    if (!this.webview) {
      console.warn(
        "UsageTrackingService: No webview available for tracking usage",
      );
      return false;
    }
  ...
  async requestBalance() {
    if (!this.webview) {
      console.warn(
        "UsageTrackingService: No webview available for balance request",
      );
      return;
    }
    try {
  ...
  async trackOpenAIUsage(
    ...
    promptTokens,
    completionTokens,
    costPerPromptToken = 0.00001, // Default pricing, should be configurable
   ...
   */
  async trackAnthropicUsage(
    model,
    promptTokens,
    completionTokens,
    costPerPromptToken = 0.000008, // Default pricing for Claude
