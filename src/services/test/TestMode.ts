...
function checkForTestMode(): boolean {
  ...
  for (const folder of workspaceFolders) {
    ...
    if (fs.existsSync(evalsEnvPath)) {
      Logger.log(
        `Found evals.env file at ${evalsEnvPath}, activating test mode`,
      );
      return true;
    }
  }
...
export function initializeTestMode(
  ...
  if (IS_TEST) {
    ...
    setTestMode(true);
    vscode.commands.executeCommand("setContext", "valoride.isTestMode", true);
  ...
  const evalsEnvWatcher =
    vscode.workspace.createFileSystemWatcher("**/evals.env");
