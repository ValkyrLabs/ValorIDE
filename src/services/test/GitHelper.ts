...
export function getWorkspacePath(visibleWebview: WebviewProvider): string {
  ...
  let workspacePath =
    visibleWebview.controller.context.workspaceState.get<string>("cwd") || "";
  ...
  if (!workspacePath) {
    ...
    if (workspaceFolders && workspaceFolders.length > 0) {
      ...
      Logger.log(
        `No workspace folder found, using extension directory: ${workspacePath}`,
      );
    }
  }
...
export async function validateWorkspacePath(
  ...
  if (!workspacePath || workspacePath === "/") {
    throw new Error(
      `Invalid workspace path: ${workspacePath}. Cannot initialize Git repository.`,
    );
  }
  ...
  try {
    const testFile = path.join(workspacePath, ".valoride_write_test");
    await execa("touch", [testFile]);
    await execa("rm", [testFile]);
  } catch (error) {
    throw new Error(
      `Workspace path is not writable: ${workspacePath}. Error: ${error.message}`,
    );
  }
...
export async function cleanupPreviousGit(workspacePath: string): Promise<void> {
  ...
  try {
    ...
    Logger.log(
      `Warning: Failed to remove existing Git repository: ${error.message}`,
    );
  }
}
...
export async function initializeGitRepository(
  ...
  try {
    ...
    await execa("git", ["config", "user.name", "ValorIDE Evaluation"], {
      cwd: workspacePath,
    });
    await execa("git", ["config", "user.email", "valoride@example.com"], {
      cwd: workspacePath,
    });
    ...
    try {
        ...
        try {
          ...
          await execa(
            ...
            [
              ...
              "--allow-empty",
              "-m",
              "Initial empty commit for evaluation",
            ],
            {
              cwd: workspacePath,
            },
          ...
          Logger.log(
            `Warning: Failed to create empty commit: ${emptyCommitError.message}`,
          );
        }
      }
    } catch (commitError) {
      ...
      Logger.log(
        `Warning: Failed to create initial commit: ${commitError.message}`,
      );
...
export async function getFileChanges(workspacePath: string): Promise<{
  ...
  try {
    ...
    const { stdout: diffOutput } = await execa("git", ["diff", "--staged"], {
      cwd: workspacePath,
    });
    ...
    if (!finalDiff) {
      ...
      finalDiff = unstaged;
      Logger.log(
        `Unstaged git diff output length: ${unstaged.length} characters`,
      );
    }
