...
import { ValorIDEAsk, ExtensionMessage } from "@shared/ExtensionMessage";
...
function createToolCallTracker(webviewProvider: WebviewProvider): {
  ...
  const originalPostMessageToWebview =
    webviewProvider.controller.postMessageToWebview;
  webviewProvider.controller.postMessageToWebview = async (
    message: ExtensionMessage,
  ) => {
    // Track tool calls
    if (
      message.type === "partialMessage" &&
      message.partialMessage?.say === "tool"
    ) {
    ...
    if (
      message.type === "partialMessage" &&
      message.partialMessage?.say === "error"
    ) {
      ...
      if (errorText && errorText.includes("Error executing tool")) {
        ...
        if (match && match[1]) {
          ...
          tracker.toolFailures[toolName] =
            (tracker.toolFailures[toolName] || 0) + 1;
        }
      }
    }

    return originalPostMessageToWebview.call(
...
export function createTestServer(
  ...
  // Try to show the ValorIDE sidebar
  Logger.log("[createTestServer] Opening ValorIDE in sidebar...");
  vscode.commands.executeCommand("workbench.view.valoride-dev-ActivityBar");
  ...
  if (webviewProvider?.controller?.context) {
    updateAutoApprovalSettings(
      webviewProvider.controller.context,
      webviewProvider,
    );
  }
  const PORT = 9876;
    ...
    req.on("end", async () => {
      try {
        ...
        if (!visibleWebview || !visibleWebview.controller) {
          res.writeHead(500);
          res.end(
            JSON.stringify({ error: "No active ValorIDE instance found" }),
          );
          return;
        }
        ...
        try {
          ...
          try {
            const wasNewlyInitialized =
              await initializeGitRepository(workspacePath);
            if (wasNewlyInitialized) {
              Logger.log(
                `Initialized new Git repository in ${workspacePath} before task start`,
              );
            } else {
              Logger.log(
                `Using existing Git repository in ${workspacePath} before task start`,
              );
            }

            // Log directory contents before task start
            try {
              const { stdout: lsOutput } = await execa("ls", [
                "-la",
                workspacePath,
              ]);
              ...
              Logger.log(
                `Warning: Failed to list directory contents: ${lsError.message}`,
              );
            }
          } catch (gitError) {
            Logger.log(
              `Warning: Git initialization failed: ${gitError.message}`,
            );
          ...
          if (apiKey) {
            ...
            const updatedConfig = {
              ...apiConfiguration,
              apiProvider: "valoride" as ApiProvider,
            ...
            // Update global state to use valoride provider
            await updateGlobalState(
              visibleWebview.controller.context,
              "apiProvider",
              "valoride" as ApiProvider,
            );
          ...
          const { chatSettings } =
            await visibleWebview.controller.getStateToPostToWebview();
          ...
          try {
            ...
            try {
              if (typeof taskId === "string") {
                messages = await getSavedValorIDEMessages(
                  visibleWebview.controller.context,
                  taskId,
                );
              }
            } catch (error) {
              Logger.log(`Error getting saved ValorIDE messages: ${error}`);
            }
            ...
            try {
              ...
              Logger.log(
                `Error getting saved API conversation history: ${error}`,
              );
            }
            ...
            try {
              ...
              const workspacePath = getWorkspacePath(visibleWebview);
              Logger.log(
                `Getting file changes from workspace path: ${workspacePath}`,
              );
              ...
              try {
                ...
                Logger.log(
                  `Directory contents after task completion:\n${lsOutput}`,
                );
              } catch (lsError) {
                Logger.log(
                  `Warning: Failed to list directory contents: ${lsError.message}`,
              ...
              if (
                ...
                Logger.log(
                  "No changes detected by Git, using fallback directory scan",
                );
                ...
                try {
                  ...
                  fileChanges.created = files.map((file) =>
                    path.relative(workspacePath, file),
                  );
                  Logger.log(
                    `Fallback found ${fileChanges.created.length} files`,
                  );
                } catch (findError) {
                  Logger.log(
                    `Warning: Fallback directory scan failed: ${findError.message}`,
                  );
                }
              }
            } catch (fileChangeError) {
              Logger.log(
                `Error getting file changes: ${fileChangeError.message}`,
              );
              throw new Error(
                `Error getting file changes: ${fileChangeError.message}`,
              );
            ...
            const toolMetrics = {
              ...
              totalToolCalls: Object.values(toolTracker.toolCalls).reduce(
                (a, b) => a + b,
                0,
              ),
              totalToolFailures: Object.values(toolTracker.toolFailures).reduce(
                (a, b) => a + b,
                0,
              ),
          ...
          res.writeHead(500);
          res.end(
            JSON.stringify({ error: `Failed to initiate task: ${error}` }),
          );
        }
      } catch (error) {
        res.writeHead(400);
 ...
 */
export function createMessageCatcher(
  webviewProvider: WebviewProvider,
): vscode.Disposable {
  Logger.log("ValorIDE message catcher registered");
  ...
  if (webviewProvider && webviewProvider.controller) {
    ...
    webviewProvider.controller.postMessageToWebview = async (
      ...
      if (
        ...
        const askType = message.partialMessage.ask as ValorIDEAsk;
  ...
  return new vscode.Disposable(() => {
    ...
    Logger.log("ValorIDE message catcher disposed");
  });
}
...
function autoRespondToAsk(
  ...
  switch (askType) {
    case "followup":
      ...
      response.text =
        "I can't answer any questions right now, use your best judgment.";
      break;
    ...
    case "new_task":
      // Devaloride creating a new task to keep the current task running
      response.askResponse = "messageResponse";
