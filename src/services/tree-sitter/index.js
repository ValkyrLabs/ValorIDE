...
export async function parseSourceCodeForDefinitionsTopLevel(dirPath, valorideIgnoreController) {
    ...
    const allowedFilesToParse = valorideIgnoreController
        ? valorideIgnoreController.filterPaths(filesToParse)
        : filesToParse;
    for (const filePath of allowedFilesToParse) {
        const definitions = await parseFile(filePath, languageParsers, valorideIgnoreController);
        if (definitions) {
...
*/
async function parseFile(filePath, languageParsers, valorideIgnoreController) {
    if (valorideIgnoreController &&
        !valorideIgnoreController.validateAccess(filePath)) {
        return null;
    }
