...
import { GeneratedArtifact } from '@thorapi/model';
import GeneratedArtifactForm from '@thorapi/redux/components/form/GeneratedArtifactForm';
...
import {
  useLazyGetGeneratedArtifactsPagedQuery,
...
} from '@thorapi/redux/services/GeneratedArtifactService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'artifactType': { type: 'enum', enumValues: [
    'backend', 'frontend', 'openapi_spec', 'package_zip', 'build_output', 'test_output', 'manifest', 'other', 
  ], enumValueType: 'string' },
  'status': { type: 'enum', enumValues: [
    'created', 'packaged', 'validated', 'failed', 'superseded', 
  ], enumValueType: 'string' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `GeneratedArtifact update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/GeneratedArtifact/${rowId}`;

const GeneratedArtifactTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<GeneratedArtifact[]>([]);
  ...
  const normalizePageRows = (result: unknown): GeneratedArtifact[] => {
    if (Array.isArray(result)) return result as GeneratedArtifact[];
    ...
    return Array.isArray(candidate) ? (candidate as GeneratedArtifact[]) : [];
  };

  const mergeFetchedRows = (incomingRows: GeneratedArtifact[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addGeneratedArtifact] = useAddGeneratedArtifactMutation();
  const [updateGeneratedArtifact] = useUpdateGeneratedArtifactMutation();
  const [deleteGeneratedArtifact] = useDeleteGeneratedArtifactMutation();
  const [deleteGeneratedArtifactCascade] = useDeleteGeneratedArtifactCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<GeneratedArtifact>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<GeneratedArtifact | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<GeneratedArtifact | null> => {
    ...
    try {
      ...
      return (await response.json()) as GeneratedArtifact;
    } catch (err) {
      console.warn('Could not reload GeneratedArtifact row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<GeneratedArtifact>,
    successField?: string,
  ) => {
    try {
      const saved = await updateGeneratedArtifact(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as GeneratedArtifact[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update GeneratedArtifact';
      console.error('Failed to update GeneratedArtifact:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteGeneratedArtifactCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteGeneratedArtifact(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<GeneratedArtifact> = { ...source };
      delete (payload as any).id;
      const created = await addGeneratedArtifact(payload).unwrap();
      setAllData((prev) => [created as GeneratedArtifact, ...prev]);
    } catch (error) {
      console.error('Failed to add GeneratedArtifact:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<GeneratedArtifact> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addGeneratedArtifact(payload).unwrap();
      setAllData((prev) => [created as GeneratedArtifact, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) GeneratedArtifact:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `GeneratedArtifact-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addGeneratedArtifact(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateGeneratedArtifact(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as GeneratedArtifact[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as GeneratedArtifact[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as GeneratedArtifact[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as GeneratedArtifact[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="GeneratedArtifactTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="GeneratedArtifact Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create GeneratedArtifact</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <GeneratedArtifactForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter GeneratedArtifact (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing GeneratedArtifact</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof GeneratedArtifact;
                ...
                await updateGeneratedArtifact(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of GeneratedArtifacts.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'GeneratedArtifact'}
...
export default GeneratedArtifactTable;
