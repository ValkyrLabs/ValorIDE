...
import ActivateAgent200ResponseForm from '@thorapi/redux/components/form/ActivateAgent200ResponseForm';
...
} from '@thorapi/redux/services/ActivateAgent200ResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ActivateAgent200Response update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ActivateAgent200Response/${rowId}`;
...
const ActivateAgent200ResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ActivateAgent200Response[] => {
    if (Array.isArray(result)) return result as ActivateAgent200Response[];
    ...
    return Array.isArray(candidate) ? (candidate as ActivateAgent200Response[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ActivateAgent200Response[], requestedPage: number) => {
  ...
  const [deleteActivateAgent200ResponseCascade] = useDeleteActivateAgent200ResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ActivateAgent200Response | null> => {
    ...
    try {
      ...
      return (await response.json()) as ActivateAgent200Response;
    } catch (err) {
      console.warn('Could not reload ActivateAgent200Response row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ActivateAgent200Response>,
    successField?: string,
  ) => {
    try {
      const saved = await updateActivateAgent200Response(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ActivateAgent200Response[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ActivateAgent200Response';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteActivateAgent200ResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ActivateAgent200Response-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addActivateAgent200Response(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateActivateAgent200Response(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ActivateAgent200Response[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ActivateAgent200Response[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ActivateAgent200Response[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ActivateAgent200Response[]);
    }
  };
