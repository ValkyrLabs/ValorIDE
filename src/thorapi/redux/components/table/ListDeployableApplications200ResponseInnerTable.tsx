...
const stringFieldCandidates = ["applicationId", "name", "description"];
...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ListDeployableApplications200ResponseInner/${rowId}`;
...
const ListDeployableApplications200ResponseInnerTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addListDeployableApplications200ResponseInner] =
    useAddListDeployableApplications200ResponseInnerMutation();
  const [updateListDeployableApplications200ResponseInner] =
    useUpdateListDeployableApplications200ResponseInnerMutation();
  const [deleteListDeployableApplications200ResponseInner] =
    useDeleteListDeployableApplications200ResponseInnerMutation();
  const [deleteListDeployableApplications200ResponseInnerCascade] =
    useDeleteListDeployableApplications200ResponseInnerCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] =
    useState<ListDeployableApplications200ResponseInner | null>(null);
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<ListDeployableApplications200ResponseInner | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload ListDeployableApplications200ResponseInner row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateListDeployableApplications200ResponseInner(
        updatedItem as any,
      ).unwrap();
      ...
      setAllData(
        (prev) =>
          mergeUpdatedRow(
            ...
            rowId,
            hydrated as any,
          ) as ListDeployableApplications200ResponseInner[],
      );
      if (successField) {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ListDeployableApplications200ResponseInner";
      console.error(
        "Failed to update ListDeployableApplications200ResponseInner:",
        error,
      );
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteListDeployableApplications200ResponseInnerCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
            ...
            try {
              await deleteListDeployableApplications200ResponseInner(
                id as any,
              ).unwrap();
            } catch (inner) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ListDeployableApplications200ResponseInner> = {
        ...source,
      };
      ...
      setAllData((prev) => [
        created as ListDeployableApplications200ResponseInner,
        ...prev,
      ]);
    } catch (error) {
      console.error(
        "Failed to add ListDeployableApplications200ResponseInner:",
        error,
      );
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) ListDeployableApplications200ResponseInner:",
        error,
      );
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === "") {
          delete payload.id;
          try {
            await addListDeployableApplications200ResponseInner(
              payload,
            ).unwrap();
            created += 1;
          } catch (err) {
        ...
        try {
          ...
          try {
            await updateListDeployableApplications200ResponseInner(
              payload,
            ).unwrap();
            updated += 1;
          } catch (updateErr) {
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType:
          "com.valkyrlabs.model.ListDeployableApplications200ResponseInner",
        objectId: rowId,
      });
    ...
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Create ListDeployableApplications200ResponseInner
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter ListDeployableApplications200ResponseInner (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>
            Editing ListDeployableApplications200ResponseInner
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
      ...
      <MarkdownEditorModal
        ...
        onSave={async (updatedText) => {
          try {
            ...
            const field =
              richModal.field as keyof ListDeployableApplications200ResponseInner;
            if (!rid || !field) {
      ...
      {isError && (
        <div style={{ color: "red", marginTop: 12 }}>
          Error fetching page {page} of
          ListDeployableApplications200ResponseInners.
        </div>
      )}
      ...
      <QBEPicker
        show={qbePicker.show}
        refType={
          qbePicker.refType || "ListDeployableApplications200ResponseInner"
        }
