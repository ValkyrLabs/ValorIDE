...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/McpMarketplaceCatalog/${rowId}`;
...
const McpMarketplaceCatalogTable: React.FC = () => {
  ...
  const [deleteMcpMarketplaceCatalog] =
    useDeleteMcpMarketplaceCatalogMutation();
  const [deleteMcpMarketplaceCatalogCascade] =
    useDeleteMcpMarketplaceCatalogCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<McpMarketplaceCatalog | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateMcpMarketplaceCatalog(
        updatedItem as any,
      ).unwrap();
