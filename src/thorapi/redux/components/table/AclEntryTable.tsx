...
import AclEntryForm from '@thorapi/redux/components/form/AclEntryForm';
...
import {
  ...
  useDeleteAclEntryCascadeMutation
} from '@thorapi/redux/services/AclEntryService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AclEntry update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AclEntry/${rowId}`;
...
const AclEntryTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): AclEntry[] => {
    if (Array.isArray(result)) return result as AclEntry[];
    ...
    return Array.isArray(candidate) ? (candidate as AclEntry[]) : [];
  };

  const mergeFetchedRows = (incomingRows: AclEntry[], requestedPage: number) => {
  ...
  const [deleteAclEntryCascade] = useDeleteAclEntryCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<AclEntry | null> => {
    ...
    try {
      ...
      return (await response.json()) as AclEntry;
    } catch (err) {
      console.warn('Could not reload AclEntry row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AclEntry>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAclEntry(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AclEntry[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AclEntry';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAclEntryCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AclEntry-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAclEntry(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAclEntry(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AclEntry[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AclEntry[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AclEntry[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AclEntry[]);
    }
  };
