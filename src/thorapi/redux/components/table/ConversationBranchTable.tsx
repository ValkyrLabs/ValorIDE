...
import { ConversationBranch } from '@thorapi/model';
import ConversationBranchForm from '@thorapi/redux/components/form/ConversationBranchForm';
...
} from '@thorapi/redux/services/ConversationBranchService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ConversationBranch update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ConversationBranch/${rowId}`;
...
const ConversationBranchTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ConversationBranch[] => {
    if (Array.isArray(result)) return result as ConversationBranch[];
    ...
    return Array.isArray(candidate) ? (candidate as ConversationBranch[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ConversationBranch[], requestedPage: number) => {
  ...
  const [deleteConversationBranchCascade] = useDeleteConversationBranchCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ConversationBranch | null> => {
    ...
    try {
      ...
      return (await response.json()) as ConversationBranch;
    } catch (err) {
      console.warn('Could not reload ConversationBranch row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ConversationBranch>,
    successField?: string,
  ) => {
    try {
      const saved = await updateConversationBranch(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ConversationBranch[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ConversationBranch';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteConversationBranchCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ConversationBranch-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addConversationBranch(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateConversationBranch(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ConversationBranch[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ConversationBranch[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ConversationBranch[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ConversationBranch[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ConversationBranch (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
