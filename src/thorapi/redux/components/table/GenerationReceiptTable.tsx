...
import { GenerationReceipt } from '@thorapi/model';
import GenerationReceiptForm from '@thorapi/redux/components/form/GenerationReceiptForm';
...
import {
  useLazyGetGenerationReceiptsPagedQuery,
...
} from '@thorapi/redux/services/GenerationReceiptService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `GenerationReceipt update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/GenerationReceipt/${rowId}`;

const GenerationReceiptTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<GenerationReceipt[]>([]);
  ...
  const normalizePageRows = (result: unknown): GenerationReceipt[] => {
    if (Array.isArray(result)) return result as GenerationReceipt[];
    ...
    return Array.isArray(candidate) ? (candidate as GenerationReceipt[]) : [];
  };

  const mergeFetchedRows = (incomingRows: GenerationReceipt[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addGenerationReceipt] = useAddGenerationReceiptMutation();
  const [updateGenerationReceipt] = useUpdateGenerationReceiptMutation();
  const [deleteGenerationReceipt] = useDeleteGenerationReceiptMutation();
  const [deleteGenerationReceiptCascade] = useDeleteGenerationReceiptCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<GenerationReceipt>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<GenerationReceipt | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<GenerationReceipt | null> => {
    ...
    try {
      ...
      console.warn('Could not reload GenerationReceipt row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<GenerationReceipt>,
    successField?: string,
  ) => {
    try {
      const saved = await updateGenerationReceipt(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as GenerationReceipt[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update GenerationReceipt';
      console.error('Failed to update GenerationReceipt:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteGenerationReceiptCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteGenerationReceipt(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<GenerationReceipt> = { ...source };
      delete (payload as any).id;
      const created = await addGenerationReceipt(payload).unwrap();
      setAllData((prev) => [created as GenerationReceipt, ...prev]);
    } catch (error) {
      console.error('Failed to add GenerationReceipt:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<GenerationReceipt> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addGenerationReceipt(payload).unwrap();
      setAllData((prev) => [created as GenerationReceipt, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) GenerationReceipt:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `GenerationReceipt-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addGenerationReceipt(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateGenerationReceipt(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as GenerationReceipt[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as GenerationReceipt[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as GenerationReceipt[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as GenerationReceipt[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="GenerationReceiptTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="GenerationReceipt Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create GenerationReceipt</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <GenerationReceiptForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter GenerationReceipt (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing GenerationReceipt</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof GenerationReceipt;
                ...
                await updateGenerationReceipt(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of GenerationReceipts.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'GenerationReceipt'}
...
export default GenerationReceiptTable;
