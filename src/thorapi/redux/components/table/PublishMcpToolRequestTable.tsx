...
import { PublishMcpToolRequest } from '@thorapi/model';
import PublishMcpToolRequestForm from '@thorapi/redux/components/form/PublishMcpToolRequestForm';
...
import {
  useLazyGetPublishMcpToolRequestsPagedQuery,
...
} from '@thorapi/redux/services/PublishMcpToolRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `PublishMcpToolRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/PublishMcpToolRequest/${rowId}`;

const PublishMcpToolRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<PublishMcpToolRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): PublishMcpToolRequest[] => {
    if (Array.isArray(result)) return result as PublishMcpToolRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as PublishMcpToolRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: PublishMcpToolRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addPublishMcpToolRequest] = useAddPublishMcpToolRequestMutation();
  const [updatePublishMcpToolRequest] = useUpdatePublishMcpToolRequestMutation();
  const [deletePublishMcpToolRequest] = useDeletePublishMcpToolRequestMutation();
  const [deletePublishMcpToolRequestCascade] = useDeletePublishMcpToolRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<PublishMcpToolRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<PublishMcpToolRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<PublishMcpToolRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as PublishMcpToolRequest;
    } catch (err) {
      console.warn('Could not reload PublishMcpToolRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<PublishMcpToolRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updatePublishMcpToolRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as PublishMcpToolRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update PublishMcpToolRequest';
      console.error('Failed to update PublishMcpToolRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePublishMcpToolRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deletePublishMcpToolRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<PublishMcpToolRequest> = { ...source };
      delete (payload as any).id;
      const created = await addPublishMcpToolRequest(payload).unwrap();
      setAllData((prev) => [created as PublishMcpToolRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add PublishMcpToolRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<PublishMcpToolRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addPublishMcpToolRequest(payload).unwrap();
      setAllData((prev) => [created as PublishMcpToolRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) PublishMcpToolRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `PublishMcpToolRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addPublishMcpToolRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updatePublishMcpToolRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as PublishMcpToolRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as PublishMcpToolRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as PublishMcpToolRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as PublishMcpToolRequest[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="PublishMcpToolRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="PublishMcpToolRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create PublishMcpToolRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <PublishMcpToolRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter PublishMcpToolRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing PublishMcpToolRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof PublishMcpToolRequest;
                ...
                await updatePublishMcpToolRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of PublishMcpToolRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'PublishMcpToolRequest'}
...
export default PublishMcpToolRequestTable;
