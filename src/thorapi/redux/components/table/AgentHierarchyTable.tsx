...
import { AgentHierarchy } from '@thorapi/model';
import AgentHierarchyForm from '@thorapi/redux/components/form/AgentHierarchyForm';
...
} from '@thorapi/redux/services/AgentHierarchyService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AgentHierarchy update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AgentHierarchy/${rowId}`;
...
const AgentHierarchyTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): AgentHierarchy[] => {
    if (Array.isArray(result)) return result as AgentHierarchy[];
    ...
    return Array.isArray(candidate) ? (candidate as AgentHierarchy[]) : [];
  };

  const mergeFetchedRows = (incomingRows: AgentHierarchy[], requestedPage: number) => {
  ...
  const [deleteAgentHierarchyCascade] = useDeleteAgentHierarchyCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<AgentHierarchy | null> => {
    ...
    try {
      ...
      return (await response.json()) as AgentHierarchy;
    } catch (err) {
      console.warn('Could not reload AgentHierarchy row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AgentHierarchy>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAgentHierarchy(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AgentHierarchy[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AgentHierarchy';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAgentHierarchyCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AgentHierarchy-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAgentHierarchy(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAgentHierarchy(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AgentHierarchy[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AgentHierarchy[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AgentHierarchy[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AgentHierarchy[]);
    }
  };
