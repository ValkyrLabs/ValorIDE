...
import { AgentChatMessageRequest } from '@thorapi/model';
import AgentChatMessageRequestForm from '@thorapi/redux/components/form/AgentChatMessageRequestForm';
...
} from '@thorapi/redux/services/AgentChatMessageRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AgentChatMessageRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AgentChatMessageRequest/${rowId}`;
...
const AgentChatMessageRequestTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): AgentChatMessageRequest[] => {
    if (Array.isArray(result)) return result as AgentChatMessageRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as AgentChatMessageRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: AgentChatMessageRequest[], requestedPage: number) => {
  ...
  const [deleteAgentChatMessageRequestCascade] = useDeleteAgentChatMessageRequestCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<AgentChatMessageRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<AgentChatMessageRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as AgentChatMessageRequest;
    } catch (err) {
      console.warn('Could not reload AgentChatMessageRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AgentChatMessageRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAgentChatMessageRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AgentChatMessageRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AgentChatMessageRequest';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAgentChatMessageRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AgentChatMessageRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAgentChatMessageRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAgentChatMessageRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AgentChatMessageRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AgentChatMessageRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AgentChatMessageRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AgentChatMessageRequest[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter AgentChatMessageRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
