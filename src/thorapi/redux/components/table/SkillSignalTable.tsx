...
import { SkillSignal } from '@thorapi/model';
import SkillSignalForm from '@thorapi/redux/components/form/SkillSignalForm';
...
const MODEL_FIELDS = [
  ...
  'actualCreditCost',
  'confidenceDelta',
  'procedureConfidenceBefore',
  'procedureConfidenceAfter',
  'notes',
  'metadataJson',
  'createdAt',
  'id',
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'procedureConfidenceBefore': { type: 'number' },
  'procedureConfidenceAfter': { type: 'number' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SkillSignal update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SkillSignal/${rowId}`;

const SkillSignalTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SkillSignal[]>([]);
  ...
  const normalizePageRows = (result: unknown): SkillSignal[] => {
    if (Array.isArray(result)) return result as SkillSignal[];
    ...
    return Array.isArray(candidate) ? (candidate as SkillSignal[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SkillSignal[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addSkillSignal] = useAddSkillSignalMutation();
  const [updateSkillSignal] = useUpdateSkillSignalMutation();
  const [deleteSkillSignal] = useDeleteSkillSignalMutation();
  const [deleteSkillSignalCascade] = useDeleteSkillSignalCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<SkillSignal>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SkillSignal | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<SkillSignal | null> => {
    ...
    try {
      ...
      return (await response.json()) as SkillSignal;
    } catch (err) {
      console.warn('Could not reload SkillSignal row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SkillSignal>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSkillSignal(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SkillSignal[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SkillSignal';
      console.error('Failed to update SkillSignal:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSkillSignalCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteSkillSignal(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SkillSignal> = { ...source };
      delete (payload as any).id;
      const created = await addSkillSignal(payload).unwrap();
      setAllData((prev) => [created as SkillSignal, ...prev]);
    } catch (error) {
      console.error('Failed to add SkillSignal:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SkillSignal> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSkillSignal(payload).unwrap();
      setAllData((prev) => [created as SkillSignal, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) SkillSignal:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SkillSignal-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSkillSignal(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSkillSignal(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SkillSignal[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SkillSignal[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SkillSignal[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SkillSignal[]);
    }
  };
    ...
    <>
          ...
          >
            ...
            <FloatingControlPanel
              description="SkillSignal Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SkillSignal</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SkillSignalForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SkillSignal (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SkillSignal</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SkillSignal;
                ...
                await updateSkillSignal(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SkillSignals.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SkillSignal'}
...
export default SkillSignalTable;
