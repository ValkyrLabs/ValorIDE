...
import { ServiceSubscription } from '@thorapi/model';
import ServiceSubscriptionForm from '@thorapi/redux/components/form/ServiceSubscriptionForm';
...
import {
  useLazyGetServiceSubscriptionsPagedQuery,
...
} from '@thorapi/redux/services/ServiceSubscriptionService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'subscriptionType': { type: 'enum', enumValues: [
    'MONTHLY', 'YEARLY', 'PAY_AS_YOU_GO', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ServiceSubscription update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ServiceSubscription/${rowId}`;

const ServiceSubscriptionTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ServiceSubscription[]>([]);
  ...
  const normalizePageRows = (result: unknown): ServiceSubscription[] => {
    if (Array.isArray(result)) return result as ServiceSubscription[];
    ...
    return Array.isArray(candidate) ? (candidate as ServiceSubscription[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ServiceSubscription[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addServiceSubscription] = useAddServiceSubscriptionMutation();
  const [updateServiceSubscription] = useUpdateServiceSubscriptionMutation();
  const [deleteServiceSubscription] = useDeleteServiceSubscriptionMutation();
  const [deleteServiceSubscriptionCascade] = useDeleteServiceSubscriptionCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ServiceSubscription>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ServiceSubscription | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ServiceSubscription | null> => {
    ...
    try {
      ...
      return (await response.json()) as ServiceSubscription;
    } catch (err) {
      console.warn('Could not reload ServiceSubscription row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ServiceSubscription>,
    successField?: string,
  ) => {
    try {
      const saved = await updateServiceSubscription(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ServiceSubscription[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ServiceSubscription';
      console.error('Failed to update ServiceSubscription:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteServiceSubscriptionCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteServiceSubscription(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ServiceSubscription> = { ...source };
      delete (payload as any).id;
      const created = await addServiceSubscription(payload).unwrap();
      setAllData((prev) => [created as ServiceSubscription, ...prev]);
    } catch (error) {
      console.error('Failed to add ServiceSubscription:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ServiceSubscription> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addServiceSubscription(payload).unwrap();
      setAllData((prev) => [created as ServiceSubscription, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ServiceSubscription:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ServiceSubscription-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addServiceSubscription(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateServiceSubscription(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ServiceSubscription[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ServiceSubscription[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ServiceSubscription[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ServiceSubscription[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ServiceSubscriptionTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ServiceSubscription Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ServiceSubscription</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ServiceSubscription (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ServiceSubscription</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ServiceSubscription;
                ...
                await updateServiceSubscription(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ServiceSubscriptions.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ServiceSubscription'}
...
export default ServiceSubscriptionTable;
