...
import ExecModuleForm from '@thorapi/redux/components/form/ExecModuleForm';
...
} from '@thorapi/redux/services/ExecModuleService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'status': { type: 'enum', enumValues: [
    'ready', 'running', 'stopped', 'good', 'warning', 'error', 'disabled', 
  ], enumValueType: 'string' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ExecModule update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ExecModule/${rowId}`;
...
const ExecModuleTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ExecModule[] => {
    if (Array.isArray(result)) return result as ExecModule[];
    ...
    return Array.isArray(candidate) ? (candidate as ExecModule[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ExecModule[], requestedPage: number) => {
  ...
  const [deleteExecModuleCascade] = useDeleteExecModuleCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ExecModule | null> => {
    ...
    try {
      ...
      return (await response.json()) as ExecModule;
    } catch (err) {
      console.warn('Could not reload ExecModule row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ExecModule>,
    successField?: string,
  ) => {
    try {
      const saved = await updateExecModule(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ExecModule[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ExecModule';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecModuleCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ExecModule-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addExecModule(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateExecModule(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ExecModule[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ExecModule[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ExecModule[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ExecModule[]);
    }
  };
