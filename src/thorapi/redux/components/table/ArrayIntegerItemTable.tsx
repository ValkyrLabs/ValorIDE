...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ArrayIntegerItem/${rowId}`;
...
const ArrayIntegerItemTable: React.FC = () => {
  ...
  const [deleteArrayIntegerItemCascade] =
    useDeleteArrayIntegerItemCascadeMutation();
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ArrayIntegerItem";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteArrayIntegerItemCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
