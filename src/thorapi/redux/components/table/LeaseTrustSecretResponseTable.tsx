...
import { LeaseTrustSecretResponse } from '@thorapi/model';
import LeaseTrustSecretResponseForm from '@thorapi/redux/components/form/LeaseTrustSecretResponseForm';
...
import {
  useLazyGetLeaseTrustSecretResponsesPagedQuery,
...
} from '@thorapi/redux/services/LeaseTrustSecretResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `LeaseTrustSecretResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/LeaseTrustSecretResponse/${rowId}`;

const LeaseTrustSecretResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<LeaseTrustSecretResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): LeaseTrustSecretResponse[] => {
    if (Array.isArray(result)) return result as LeaseTrustSecretResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as LeaseTrustSecretResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: LeaseTrustSecretResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addLeaseTrustSecretResponse] = useAddLeaseTrustSecretResponseMutation();
  const [updateLeaseTrustSecretResponse] = useUpdateLeaseTrustSecretResponseMutation();
  const [deleteLeaseTrustSecretResponse] = useDeleteLeaseTrustSecretResponseMutation();
  const [deleteLeaseTrustSecretResponseCascade] = useDeleteLeaseTrustSecretResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<LeaseTrustSecretResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<LeaseTrustSecretResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<LeaseTrustSecretResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as LeaseTrustSecretResponse;
    } catch (err) {
      console.warn('Could not reload LeaseTrustSecretResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<LeaseTrustSecretResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateLeaseTrustSecretResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as LeaseTrustSecretResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update LeaseTrustSecretResponse';
      console.error('Failed to update LeaseTrustSecretResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteLeaseTrustSecretResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteLeaseTrustSecretResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<LeaseTrustSecretResponse> = { ...source };
      delete (payload as any).id;
      const created = await addLeaseTrustSecretResponse(payload).unwrap();
      setAllData((prev) => [created as LeaseTrustSecretResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add LeaseTrustSecretResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<LeaseTrustSecretResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addLeaseTrustSecretResponse(payload).unwrap();
      setAllData((prev) => [created as LeaseTrustSecretResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) LeaseTrustSecretResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `LeaseTrustSecretResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addLeaseTrustSecretResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateLeaseTrustSecretResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as LeaseTrustSecretResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as LeaseTrustSecretResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as LeaseTrustSecretResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as LeaseTrustSecretResponse[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.LeaseTrustSecretResponse',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="LeaseTrustSecretResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="LeaseTrustSecretResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create LeaseTrustSecretResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <LeaseTrustSecretResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter LeaseTrustSecretResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing LeaseTrustSecretResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof LeaseTrustSecretResponse;
                ...
                await updateLeaseTrustSecretResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of LeaseTrustSecretResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'LeaseTrustSecretResponse'}
...
export default LeaseTrustSecretResponseTable;
