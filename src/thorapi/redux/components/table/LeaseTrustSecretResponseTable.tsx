...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/LeaseTrustSecretResponse/${rowId}`;
...
const LeaseTrustSecretResponseTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addLeaseTrustSecretResponse] =
    useAddLeaseTrustSecretResponseMutation();
  const [updateLeaseTrustSecretResponse] =
    useUpdateLeaseTrustSecretResponseMutation();
  const [deleteLeaseTrustSecretResponse] =
    useDeleteLeaseTrustSecretResponseMutation();
  const [deleteLeaseTrustSecretResponseCascade] =
    useDeleteLeaseTrustSecretResponseCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<LeaseTrustSecretResponse | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<LeaseTrustSecretResponse | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateLeaseTrustSecretResponse(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update LeaseTrustSecretResponse";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteLeaseTrustSecretResponseCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) LeaseTrustSecretResponse:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter LeaseTrustSecretResponse (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
