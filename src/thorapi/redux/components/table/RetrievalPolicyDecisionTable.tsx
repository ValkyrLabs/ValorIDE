...
import { RetrievalPolicyDecision } from '@thorapi/model';
import RetrievalPolicyDecisionForm from '@thorapi/redux/components/form/RetrievalPolicyDecisionForm';
...
import {
  useLazyGetRetrievalPolicyDecisionsPagedQuery,
...
} from '@thorapi/redux/services/RetrievalPolicyDecisionService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'secureFieldRedactions': { type: 'number' },
  'tenantScopeVerified': { type: 'boolean' },
  'restrictedRecordsExcluded': { type: 'number' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RetrievalPolicyDecision update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RetrievalPolicyDecision/${rowId}`;

const RetrievalPolicyDecisionTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RetrievalPolicyDecision[]>([]);
  ...
  const normalizePageRows = (result: unknown): RetrievalPolicyDecision[] => {
    if (Array.isArray(result)) return result as RetrievalPolicyDecision[];
    ...
    return Array.isArray(candidate) ? (candidate as RetrievalPolicyDecision[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RetrievalPolicyDecision[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addRetrievalPolicyDecision] = useAddRetrievalPolicyDecisionMutation();
  const [updateRetrievalPolicyDecision] = useUpdateRetrievalPolicyDecisionMutation();
  const [deleteRetrievalPolicyDecision] = useDeleteRetrievalPolicyDecisionMutation();
  const [deleteRetrievalPolicyDecisionCascade] = useDeleteRetrievalPolicyDecisionCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<RetrievalPolicyDecision>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RetrievalPolicyDecision | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<RetrievalPolicyDecision | null> => {
    ...
    try {
      ...
      return (await response.json()) as RetrievalPolicyDecision;
    } catch (err) {
      console.warn('Could not reload RetrievalPolicyDecision row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RetrievalPolicyDecision>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRetrievalPolicyDecision(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RetrievalPolicyDecision[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RetrievalPolicyDecision';
      console.error('Failed to update RetrievalPolicyDecision:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRetrievalPolicyDecisionCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteRetrievalPolicyDecision(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RetrievalPolicyDecision> = { ...source };
      delete (payload as any).id;
      const created = await addRetrievalPolicyDecision(payload).unwrap();
      setAllData((prev) => [created as RetrievalPolicyDecision, ...prev]);
    } catch (error) {
      console.error('Failed to add RetrievalPolicyDecision:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RetrievalPolicyDecision> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRetrievalPolicyDecision(payload).unwrap();
      setAllData((prev) => [created as RetrievalPolicyDecision, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) RetrievalPolicyDecision:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RetrievalPolicyDecision-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRetrievalPolicyDecision(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRetrievalPolicyDecision(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RetrievalPolicyDecision[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RetrievalPolicyDecision[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RetrievalPolicyDecision[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RetrievalPolicyDecision[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="RetrievalPolicyDecisionTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="RetrievalPolicyDecision Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RetrievalPolicyDecision</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RetrievalPolicyDecisionForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RetrievalPolicyDecision (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RetrievalPolicyDecision</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RetrievalPolicyDecision;
                ...
                await updateRetrievalPolicyDecision(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RetrievalPolicyDecisions.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RetrievalPolicyDecision'}
...
export default RetrievalPolicyDecisionTable;
