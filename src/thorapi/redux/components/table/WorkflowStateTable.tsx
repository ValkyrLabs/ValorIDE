...
import { WorkflowState } from '@thorapi/model';
import WorkflowStateForm from '@thorapi/redux/components/form/WorkflowStateForm';
...
import {
  ...
  useDeleteWorkflowStateCascadeMutation
} from '@thorapi/redux/services/WorkflowStateService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `WorkflowState update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/WorkflowState/${rowId}`;
...
const WorkflowStateTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): WorkflowState[] => {
    if (Array.isArray(result)) return result as WorkflowState[];
    ...
    return Array.isArray(candidate) ? (candidate as WorkflowState[]) : [];
  };

  const mergeFetchedRows = (incomingRows: WorkflowState[], requestedPage: number) => {
  ...
  const [deleteWorkflowStateCascade] = useDeleteWorkflowStateCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<WorkflowState | null> => {
    ...
    try {
      ...
      console.warn('Could not reload WorkflowState row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<WorkflowState>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWorkflowState(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as WorkflowState[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update WorkflowState';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowStateCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `WorkflowState-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWorkflowState(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWorkflowState(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as WorkflowState[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as WorkflowState[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as WorkflowState[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as WorkflowState[]);
    }
  };
