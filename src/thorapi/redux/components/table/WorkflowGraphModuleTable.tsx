...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/WorkflowGraphModule/${rowId}`;
...
const WorkflowGraphModuleTable: React.FC = () => {
  ...
  const [deleteWorkflowGraphModuleCascade] =
    useDeleteWorkflowGraphModuleCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<WorkflowGraphModule | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateWorkflowGraphModule(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update WorkflowGraphModule";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphModuleCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter WorkflowGraphModule (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
