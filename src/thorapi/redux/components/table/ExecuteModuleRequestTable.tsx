...
import { ExecuteModuleRequest } from '@thorapi/model';
import ExecuteModuleRequestForm from '@thorapi/redux/components/form/ExecuteModuleRequestForm';
...
} from '@thorapi/redux/services/ExecuteModuleRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ExecuteModuleRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ExecuteModuleRequest/${rowId}`;
...
const ExecuteModuleRequestTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ExecuteModuleRequest[] => {
    if (Array.isArray(result)) return result as ExecuteModuleRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as ExecuteModuleRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ExecuteModuleRequest[], requestedPage: number) => {
  ...
  const [deleteExecuteModuleRequestCascade] = useDeleteExecuteModuleRequestCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ExecuteModuleRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as ExecuteModuleRequest;
    } catch (err) {
      console.warn('Could not reload ExecuteModuleRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ExecuteModuleRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateExecuteModuleRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ExecuteModuleRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ExecuteModuleRequest';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecuteModuleRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ExecuteModuleRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addExecuteModuleRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateExecuteModuleRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ExecuteModuleRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ExecuteModuleRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ExecuteModuleRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ExecuteModuleRequest[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ExecuteModuleRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
