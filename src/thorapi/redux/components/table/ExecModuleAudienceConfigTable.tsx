...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ExecModuleAudienceConfig/${rowId}`;
...
const ExecModuleAudienceConfigTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addExecModuleAudienceConfig] =
    useAddExecModuleAudienceConfigMutation();
  const [updateExecModuleAudienceConfig] =
    useUpdateExecModuleAudienceConfigMutation();
  const [deleteExecModuleAudienceConfig] =
    useDeleteExecModuleAudienceConfigMutation();
  const [deleteExecModuleAudienceConfigCascade] =
    useDeleteExecModuleAudienceConfigCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<ExecModuleAudienceConfig | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<ExecModuleAudienceConfig | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateExecModuleAudienceConfig(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ExecModuleAudienceConfig";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecModuleAudienceConfigCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) ExecModuleAudienceConfig:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter ExecModuleAudienceConfig (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
