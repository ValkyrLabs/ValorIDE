...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/SwarmCommandRequest/${rowId}`;
...
const SwarmCommandRequestTable: React.FC = () => {
  ...
  const [deleteSwarmCommandRequestCascade] =
    useDeleteSwarmCommandRequestCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<SwarmCommandRequest | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateSwarmCommandRequest(
        updatedItem as any,
      ).unwrap();
