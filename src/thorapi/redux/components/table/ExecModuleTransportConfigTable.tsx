...
import ExecModuleTransportConfigForm from '@thorapi/redux/components/form/ExecModuleTransportConfigForm';
...
import {
  useLazyGetExecModuleTransportConfigsPagedQuery,
...
} from '@thorapi/redux/services/ExecModuleTransportConfigService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ExecModuleTransportConfig update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ExecModuleTransportConfig/${rowId}`;

const ExecModuleTransportConfigTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ExecModuleTransportConfig[]>([]);
  ...
  const normalizePageRows = (result: unknown): ExecModuleTransportConfig[] => {
    if (Array.isArray(result)) return result as ExecModuleTransportConfig[];
    ...
    return Array.isArray(candidate) ? (candidate as ExecModuleTransportConfig[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ExecModuleTransportConfig[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addExecModuleTransportConfig] = useAddExecModuleTransportConfigMutation();
  const [updateExecModuleTransportConfig] = useUpdateExecModuleTransportConfigMutation();
  const [deleteExecModuleTransportConfig] = useDeleteExecModuleTransportConfigMutation();
  const [deleteExecModuleTransportConfigCascade] = useDeleteExecModuleTransportConfigCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ExecModuleTransportConfig>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ExecModuleTransportConfig | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ExecModuleTransportConfig | null> => {
    ...
    try {
      ...
      return (await response.json()) as ExecModuleTransportConfig;
    } catch (err) {
      console.warn('Could not reload ExecModuleTransportConfig row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ExecModuleTransportConfig>,
    successField?: string,
  ) => {
    try {
      const saved = await updateExecModuleTransportConfig(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ExecModuleTransportConfig[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ExecModuleTransportConfig';
      console.error('Failed to update ExecModuleTransportConfig:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecModuleTransportConfigCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteExecModuleTransportConfig(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ExecModuleTransportConfig> = { ...source };
      delete (payload as any).id;
      const created = await addExecModuleTransportConfig(payload).unwrap();
      setAllData((prev) => [created as ExecModuleTransportConfig, ...prev]);
    } catch (error) {
      console.error('Failed to add ExecModuleTransportConfig:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ExecModuleTransportConfig> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addExecModuleTransportConfig(payload).unwrap();
      setAllData((prev) => [created as ExecModuleTransportConfig, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ExecModuleTransportConfig:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ExecModuleTransportConfig-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addExecModuleTransportConfig(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateExecModuleTransportConfig(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ExecModuleTransportConfig[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ExecModuleTransportConfig[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ExecModuleTransportConfig[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ExecModuleTransportConfig[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ExecModuleTransportConfigTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ExecModuleTransportConfig Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ExecModuleTransportConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ExecModuleTransportConfigForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ExecModuleTransportConfig (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ExecModuleTransportConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ExecModuleTransportConfig;
                ...
                await updateExecModuleTransportConfig(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ExecModuleTransportConfigs.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ExecModuleTransportConfig'}
...
export default ExecModuleTransportConfigTable;
