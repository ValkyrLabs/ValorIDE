...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/StartTrustExecutionRequest/${rowId}`;
...
const StartTrustExecutionRequestTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addStartTrustExecutionRequest] =
    useAddStartTrustExecutionRequestMutation();
  const [updateStartTrustExecutionRequest] =
    useUpdateStartTrustExecutionRequestMutation();
  const [deleteStartTrustExecutionRequest] =
    useDeleteStartTrustExecutionRequestMutation();
  const [deleteStartTrustExecutionRequestCascade] =
    useDeleteStartTrustExecutionRequestCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<StartTrustExecutionRequest | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<StartTrustExecutionRequest | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload StartTrustExecutionRequest row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateStartTrustExecutionRequest(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update StartTrustExecutionRequest";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteStartTrustExecutionRequestCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) StartTrustExecutionRequest:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter StartTrustExecutionRequest (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
