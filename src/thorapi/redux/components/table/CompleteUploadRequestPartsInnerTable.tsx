...
import CompleteUploadRequestPartsInnerForm from '@thorapi/redux/components/form/CompleteUploadRequestPartsInnerForm';
...
} from '@thorapi/redux/services/CompleteUploadRequestPartsInnerService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `CompleteUploadRequestPartsInner update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/CompleteUploadRequestPartsInner/${rowId}`;
...
const CompleteUploadRequestPartsInnerTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): CompleteUploadRequestPartsInner[] => {
    if (Array.isArray(result)) return result as CompleteUploadRequestPartsInner[];
    ...
    return Array.isArray(candidate) ? (candidate as CompleteUploadRequestPartsInner[]) : [];
  };

  const mergeFetchedRows = (incomingRows: CompleteUploadRequestPartsInner[], requestedPage: number) => {
  ...
  const [deleteCompleteUploadRequestPartsInnerCascade] = useDeleteCompleteUploadRequestPartsInnerCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<CompleteUploadRequestPartsInner>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<CompleteUploadRequestPartsInner | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<CompleteUploadRequestPartsInner | null> => {
    ...
    try {
      ...
      return (await response.json()) as CompleteUploadRequestPartsInner;
    } catch (err) {
      console.warn('Could not reload CompleteUploadRequestPartsInner row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<CompleteUploadRequestPartsInner>,
    successField?: string,
  ) => {
    try {
      const saved = await updateCompleteUploadRequestPartsInner(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as CompleteUploadRequestPartsInner[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update CompleteUploadRequestPartsInner';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCompleteUploadRequestPartsInnerCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      setAllData((prev) => [created as CompleteUploadRequestPartsInner, ...prev]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<CompleteUploadRequestPartsInner> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addCompleteUploadRequestPartsInner(payload).unwrap();
      setAllData((prev) => [created as CompleteUploadRequestPartsInner, ...prev]);
    } catch (error) {
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `CompleteUploadRequestPartsInner-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        try {
          ...
          try {
            await updateCompleteUploadRequestPartsInner(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as CompleteUploadRequestPartsInner[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as CompleteUploadRequestPartsInner[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as CompleteUploadRequestPartsInner[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as CompleteUploadRequestPartsInner[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter CompleteUploadRequestPartsInner (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof CompleteUploadRequestPartsInner;
