...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/CompleteUploadRequestPartsInner/${rowId}`;
...
const CompleteUploadRequestPartsInnerTable: React.FC = () => {
  ...
  const [deleteCompleteUploadRequestPartsInner] =
    useDeleteCompleteUploadRequestPartsInnerMutation();
  const [deleteCompleteUploadRequestPartsInnerCascade] =
    useDeleteCompleteUploadRequestPartsInnerCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<CompleteUploadRequestPartsInner | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload CompleteUploadRequestPartsInner row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateCompleteUploadRequestPartsInner(
        updatedItem as any,
      ).unwrap();
      ...
      setAllData(
        (prev) =>
          mergeUpdatedRow(
            ...
            rowId,
            hydrated as any,
          ) as CompleteUploadRequestPartsInner[],
      );
      if (successField) {
