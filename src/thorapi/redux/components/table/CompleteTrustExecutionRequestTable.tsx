...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/CompleteTrustExecutionRequest/${rowId}`;
...
const CompleteTrustExecutionRequestTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addCompleteTrustExecutionRequest] =
    useAddCompleteTrustExecutionRequestMutation();
  const [updateCompleteTrustExecutionRequest] =
    useUpdateCompleteTrustExecutionRequestMutation();
  const [deleteCompleteTrustExecutionRequest] =
    useDeleteCompleteTrustExecutionRequestMutation();
  const [deleteCompleteTrustExecutionRequestCascade] =
    useDeleteCompleteTrustExecutionRequestCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] =
    useState<CompleteTrustExecutionRequest | null>(null);
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<CompleteTrustExecutionRequest | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload CompleteTrustExecutionRequest row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateCompleteTrustExecutionRequest(
        updatedItem as any,
      ).unwrap();
      ...
      setAllData(
        (prev) =>
          mergeUpdatedRow(
            ...
            rowId,
            hydrated as any,
          ) as CompleteTrustExecutionRequest[],
      );
      if (successField) {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update CompleteTrustExecutionRequest";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCompleteTrustExecutionRequestCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) CompleteTrustExecutionRequest:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter CompleteTrustExecutionRequest (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
      ...
      <MarkdownEditorModal
        ...
        onSave={async (updatedText) => {
          try {
            ...
            const field =
              richModal.field as keyof CompleteTrustExecutionRequest;
            if (!rid || !field) {
