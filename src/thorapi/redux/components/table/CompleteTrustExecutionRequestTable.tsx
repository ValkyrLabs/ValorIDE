...
import CompleteTrustExecutionRequestForm from '@thorapi/redux/components/form/CompleteTrustExecutionRequestForm';
...
import {
  useLazyGetCompleteTrustExecutionRequestsPagedQuery,
...
} from '@thorapi/redux/services/CompleteTrustExecutionRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `CompleteTrustExecutionRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/CompleteTrustExecutionRequest/${rowId}`;

const CompleteTrustExecutionRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<CompleteTrustExecutionRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): CompleteTrustExecutionRequest[] => {
    if (Array.isArray(result)) return result as CompleteTrustExecutionRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as CompleteTrustExecutionRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: CompleteTrustExecutionRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addCompleteTrustExecutionRequest] = useAddCompleteTrustExecutionRequestMutation();
  const [updateCompleteTrustExecutionRequest] = useUpdateCompleteTrustExecutionRequestMutation();
  const [deleteCompleteTrustExecutionRequest] = useDeleteCompleteTrustExecutionRequestMutation();
  const [deleteCompleteTrustExecutionRequestCascade] = useDeleteCompleteTrustExecutionRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<CompleteTrustExecutionRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<CompleteTrustExecutionRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<CompleteTrustExecutionRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as CompleteTrustExecutionRequest;
    } catch (err) {
      console.warn('Could not reload CompleteTrustExecutionRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<CompleteTrustExecutionRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateCompleteTrustExecutionRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as CompleteTrustExecutionRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update CompleteTrustExecutionRequest';
      console.error('Failed to update CompleteTrustExecutionRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCompleteTrustExecutionRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteCompleteTrustExecutionRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<CompleteTrustExecutionRequest> = { ...source };
      delete (payload as any).id;
      const created = await addCompleteTrustExecutionRequest(payload).unwrap();
      setAllData((prev) => [created as CompleteTrustExecutionRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add CompleteTrustExecutionRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<CompleteTrustExecutionRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addCompleteTrustExecutionRequest(payload).unwrap();
      setAllData((prev) => [created as CompleteTrustExecutionRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) CompleteTrustExecutionRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `CompleteTrustExecutionRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addCompleteTrustExecutionRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateCompleteTrustExecutionRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as CompleteTrustExecutionRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as CompleteTrustExecutionRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as CompleteTrustExecutionRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as CompleteTrustExecutionRequest[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.CompleteTrustExecutionRequest',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="CompleteTrustExecutionRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="CompleteTrustExecutionRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create CompleteTrustExecutionRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <CompleteTrustExecutionRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter CompleteTrustExecutionRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing CompleteTrustExecutionRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof CompleteTrustExecutionRequest;
                ...
                await updateCompleteTrustExecutionRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of CompleteTrustExecutionRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'CompleteTrustExecutionRequest'}
...
export default CompleteTrustExecutionRequestTable;
