...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ExecModulePayloadConfig/${rowId}`;
...
const ExecModulePayloadConfigTable: React.FC = () => {
  ...
  const [updateExecModulePayloadConfig] =
    useUpdateExecModulePayloadConfigMutation();
  const [deleteExecModulePayloadConfig] =
    useDeleteExecModulePayloadConfigMutation();
  const [deleteExecModulePayloadConfigCascade] =
    useDeleteExecModulePayloadConfigCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<ExecModulePayloadConfig | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<ExecModulePayloadConfig | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateExecModulePayloadConfig(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ExecModulePayloadConfig";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecModulePayloadConfigCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter ExecModulePayloadConfig (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
