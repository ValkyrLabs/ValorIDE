...
import { SkillOptRouteReceipt } from '@thorapi/model';
import SkillOptRouteReceiptForm from '@thorapi/redux/components/form/SkillOptRouteReceiptForm';
...
import {
  useLazyGetSkillOptRouteReceiptsPagedQuery,
...
} from '@thorapi/redux/services/SkillOptRouteReceiptService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'requiredHydration': { type: 'boolean' },
  'requiredApproval': { type: 'boolean' },
  'billable': { type: 'boolean' },
  'createdAt': { type: 'datetime' },
  'outcome': { type: 'enum', enumValues: [
    'pending', 'success', 'failure', 'partial', 'canceled', 
  ], enumValueType: 'string' },
  'outcomeAt': { type: 'datetime' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SkillOptRouteReceipt update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SkillOptRouteReceipt/${rowId}`;

const SkillOptRouteReceiptTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SkillOptRouteReceipt[]>([]);
  ...
  const normalizePageRows = (result: unknown): SkillOptRouteReceipt[] => {
    if (Array.isArray(result)) return result as SkillOptRouteReceipt[];
    ...
    return Array.isArray(candidate) ? (candidate as SkillOptRouteReceipt[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SkillOptRouteReceipt[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addSkillOptRouteReceipt] = useAddSkillOptRouteReceiptMutation();
  const [updateSkillOptRouteReceipt] = useUpdateSkillOptRouteReceiptMutation();
  const [deleteSkillOptRouteReceipt] = useDeleteSkillOptRouteReceiptMutation();
  const [deleteSkillOptRouteReceiptCascade] = useDeleteSkillOptRouteReceiptCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<SkillOptRouteReceipt>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SkillOptRouteReceipt | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<SkillOptRouteReceipt | null> => {
    ...
    try {
      ...
      return (await response.json()) as SkillOptRouteReceipt;
    } catch (err) {
      console.warn('Could not reload SkillOptRouteReceipt row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SkillOptRouteReceipt>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSkillOptRouteReceipt(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SkillOptRouteReceipt[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SkillOptRouteReceipt';
      console.error('Failed to update SkillOptRouteReceipt:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSkillOptRouteReceiptCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteSkillOptRouteReceipt(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SkillOptRouteReceipt> = { ...source };
      delete (payload as any).id;
      const created = await addSkillOptRouteReceipt(payload).unwrap();
      setAllData((prev) => [created as SkillOptRouteReceipt, ...prev]);
    } catch (error) {
      console.error('Failed to add SkillOptRouteReceipt:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SkillOptRouteReceipt> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSkillOptRouteReceipt(payload).unwrap();
      setAllData((prev) => [created as SkillOptRouteReceipt, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) SkillOptRouteReceipt:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SkillOptRouteReceipt-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSkillOptRouteReceipt(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSkillOptRouteReceipt(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SkillOptRouteReceipt[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SkillOptRouteReceipt[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SkillOptRouteReceipt[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SkillOptRouteReceipt[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="SkillOptRouteReceiptTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="SkillOptRouteReceipt Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SkillOptRouteReceipt</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SkillOptRouteReceiptForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SkillOptRouteReceipt (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SkillOptRouteReceipt</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SkillOptRouteReceipt;
                ...
                await updateSkillOptRouteReceipt(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SkillOptRouteReceipts.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SkillOptRouteReceipt'}
...
export default SkillOptRouteReceiptTable;
