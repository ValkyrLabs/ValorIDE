...
import { ChatCompletionResponse } from '@thorapi/model';
import ChatCompletionResponseForm from '@thorapi/redux/components/form/ChatCompletionResponseForm';
...
} from '@thorapi/redux/services/ChatCompletionResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ChatCompletionResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ChatCompletionResponse/${rowId}`;
...
const ChatCompletionResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ChatCompletionResponse[] => {
    if (Array.isArray(result)) return result as ChatCompletionResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as ChatCompletionResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ChatCompletionResponse[], requestedPage: number) => {
  ...
  const [deleteChatCompletionResponseCascade] = useDeleteChatCompletionResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ChatCompletionResponse | null> => {
    ...
    try {
      ...
      console.warn('Could not reload ChatCompletionResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ChatCompletionResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateChatCompletionResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ChatCompletionResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ChatCompletionResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteChatCompletionResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ChatCompletionResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addChatCompletionResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateChatCompletionResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ChatCompletionResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ChatCompletionResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ChatCompletionResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ChatCompletionResponse[]);
    }
  };
