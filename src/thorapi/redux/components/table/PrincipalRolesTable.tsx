...
const ICON_SIZE = 22;
import { PrincipalRoles } from '@thorapi/model';
import PrincipalRolesForm from '@thorapi/redux/components/form/PrincipalRolesForm';
...
import {
  ...
  useDeletePrincipalRolesCascadeMutation
} from '@thorapi/redux/services/PrincipalRolesService';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePrincipalRolesCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update PrincipalRoles';
      console.error('Failed to update PrincipalRoles:', error);
