...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ExecModuleAuthConfig/${rowId}`;
...
const ExecModuleAuthConfigTable: React.FC = () => {
  ...
  const [deleteExecModuleAuthConfigCascade] =
    useDeleteExecModuleAuthConfigCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<ExecModuleAuthConfig | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateExecModuleAuthConfig(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ExecModuleAuthConfig";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecModuleAuthConfigCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter ExecModuleAuthConfig (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
