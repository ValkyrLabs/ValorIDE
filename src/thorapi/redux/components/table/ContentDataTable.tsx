...
import ContentDataForm from '@thorapi/redux/components/form/ContentDataForm';
...
} from '@thorapi/redux/services/ContentDataService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'category': { type: 'enum', enumValues: [
    'CodeGen', 'slides', 'spreadsheets', 'docs', 'BLOG', 'Product', 'SECURITY', 'Event', 'Tutorial', 'News', 'Announcement', 'Guide', 'Review', 'Case Study', 'Whitepaper', 'EBOOK', 'FUNNEL', 'video', 'audio', 'image', 'podcast', 'webinar', 'course', 'policy', 'research', 'report', 'memory', 'other', 
  ], enumValueType: 'string' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ContentData update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ContentData/${rowId}`;
...
const ContentDataTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ContentData[] => {
    if (Array.isArray(result)) return result as ContentData[];
    ...
    return Array.isArray(candidate) ? (candidate as ContentData[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ContentData[], requestedPage: number) => {
  ...
  const [deleteContentDataCascade] = useDeleteContentDataCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ContentData | null> => {
    ...
    try {
      ...
      console.warn('Could not reload ContentData row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ContentData>,
    successField?: string,
  ) => {
    try {
      const saved = await updateContentData(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ContentData[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ContentData';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteContentDataCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ContentData-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addContentData(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateContentData(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ContentData[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ContentData[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ContentData[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ContentData[]);
    }
  };
