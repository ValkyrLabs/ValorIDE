...
import { Budget } from '@thorapi/model';
import BudgetForm from '@thorapi/redux/components/form/BudgetForm';
...
} from '@thorapi/redux/services/BudgetService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Budget update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Budget/${rowId}`;
...
const BudgetTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Budget[] => {
    if (Array.isArray(result)) return result as Budget[];
    ...
    return Array.isArray(candidate) ? (candidate as Budget[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Budget[], requestedPage: number) => {
  ...
  const reloadRowById = async (rowId: string): Promise<Budget | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Budget row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Budget>,
    successField?: string,
  ) => {
    try {
      const saved = await updateBudget(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Budget[]);
      if (successField) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteBudgetCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Budget-export-${stamp}.json`;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Budget[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Budget[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Budget[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Budget[]);
    }
  };
