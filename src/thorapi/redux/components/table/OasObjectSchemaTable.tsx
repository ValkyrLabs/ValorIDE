...
import { OasObjectSchema } from '@thorapi/model';
import OasObjectSchemaForm from '@thorapi/redux/components/form/OasObjectSchemaForm';
...
} from '@thorapi/redux/services/OasObjectSchemaService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `OasObjectSchema update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/OasObjectSchema/${rowId}`;
...
const OasObjectSchemaTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): OasObjectSchema[] => {
    if (Array.isArray(result)) return result as OasObjectSchema[];
    ...
    return Array.isArray(candidate) ? (candidate as OasObjectSchema[]) : [];
  };

  const mergeFetchedRows = (incomingRows: OasObjectSchema[], requestedPage: number) => {
  ...
  const [deleteOasObjectSchemaCascade] = useDeleteOasObjectSchemaCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<OasObjectSchema | null> => {
    ...
    try {
      ...
      return (await response.json()) as OasObjectSchema;
    } catch (err) {
      console.warn('Could not reload OasObjectSchema row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<OasObjectSchema>,
    successField?: string,
  ) => {
    try {
      const saved = await updateOasObjectSchema(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as OasObjectSchema[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update OasObjectSchema';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOasObjectSchemaCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `OasObjectSchema-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addOasObjectSchema(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateOasObjectSchema(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as OasObjectSchema[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as OasObjectSchema[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as OasObjectSchema[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as OasObjectSchema[]);
    }
  };
