...
import { UpdateFileRequest } from '@thorapi/model';
import UpdateFileRequestForm from '@thorapi/redux/components/form/UpdateFileRequestForm';
...
} from '@thorapi/redux/services/UpdateFileRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `UpdateFileRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/UpdateFileRequest/${rowId}`;
...
const UpdateFileRequestTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): UpdateFileRequest[] => {
    if (Array.isArray(result)) return result as UpdateFileRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as UpdateFileRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: UpdateFileRequest[], requestedPage: number) => {
  ...
  const [deleteUpdateFileRequestCascade] = useDeleteUpdateFileRequestCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<UpdateFileRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as UpdateFileRequest;
    } catch (err) {
      console.warn('Could not reload UpdateFileRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<UpdateFileRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateUpdateFileRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as UpdateFileRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update UpdateFileRequest';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteUpdateFileRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `UpdateFileRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addUpdateFileRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateUpdateFileRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as UpdateFileRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as UpdateFileRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as UpdateFileRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as UpdateFileRequest[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter UpdateFileRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
