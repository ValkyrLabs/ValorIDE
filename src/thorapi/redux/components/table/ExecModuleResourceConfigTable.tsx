...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ExecModuleResourceConfig/${rowId}`;
...
const ExecModuleResourceConfigTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addExecModuleResourceConfig] =
    useAddExecModuleResourceConfigMutation();
  const [updateExecModuleResourceConfig] =
    useUpdateExecModuleResourceConfigMutation();
  const [deleteExecModuleResourceConfig] =
    useDeleteExecModuleResourceConfigMutation();
  const [deleteExecModuleResourceConfigCascade] =
    useDeleteExecModuleResourceConfigCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<ExecModuleResourceConfig | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<ExecModuleResourceConfig | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateExecModuleResourceConfig(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ExecModuleResourceConfig";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecModuleResourceConfigCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) ExecModuleResourceConfig:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter ExecModuleResourceConfig (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
