...
import { TrustKeyProvider } from '@thorapi/model';
import TrustKeyProviderForm from '@thorapi/redux/components/form/TrustKeyProviderForm';
...
import {
  useLazyGetTrustKeyProvidersPagedQuery,
...
} from '@thorapi/redux/services/TrustKeyProviderService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'providerType': { type: 'enum', enumValues: [
    'LOCAL_JAVA_KEYSTORE', 'MACOS_KEYCHAIN', 'HASHICORP_VAULT', 'AWS_KMS', 'AZURE_KEY_VAULT', 'GCP_KMS', 'HSM', 'PASSWORD_SERVICE', 'TRUST_KMS_MICROSERVICE', 
  ], enumValueType: 'string' },
  'providerStatus': { type: 'enum', enumValues: [
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TrustKeyProvider update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TrustKeyProvider/${rowId}`;

const TrustKeyProviderTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TrustKeyProvider[]>([]);
  ...
  const normalizePageRows = (result: unknown): TrustKeyProvider[] => {
    if (Array.isArray(result)) return result as TrustKeyProvider[];
    ...
    return Array.isArray(candidate) ? (candidate as TrustKeyProvider[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TrustKeyProvider[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addTrustKeyProvider] = useAddTrustKeyProviderMutation();
  const [updateTrustKeyProvider] = useUpdateTrustKeyProviderMutation();
  const [deleteTrustKeyProvider] = useDeleteTrustKeyProviderMutation();
  const [deleteTrustKeyProviderCascade] = useDeleteTrustKeyProviderCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TrustKeyProvider>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TrustKeyProvider | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TrustKeyProvider | null> => {
    ...
    try {
      ...
      return (await response.json()) as TrustKeyProvider;
    } catch (err) {
      console.warn('Could not reload TrustKeyProvider row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TrustKeyProvider>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTrustKeyProvider(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TrustKeyProvider[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TrustKeyProvider';
      console.error('Failed to update TrustKeyProvider:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustKeyProviderCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTrustKeyProvider(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TrustKeyProvider> = { ...source };
      delete (payload as any).id;
      const created = await addTrustKeyProvider(payload).unwrap();
      setAllData((prev) => [created as TrustKeyProvider, ...prev]);
    } catch (error) {
      console.error('Failed to add TrustKeyProvider:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TrustKeyProvider> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTrustKeyProvider(payload).unwrap();
      setAllData((prev) => [created as TrustKeyProvider, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TrustKeyProvider:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TrustKeyProvider-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTrustKeyProvider(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTrustKeyProvider(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TrustKeyProvider[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TrustKeyProvider[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TrustKeyProvider[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TrustKeyProvider[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="TrustKeyProviderTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="TrustKeyProvider Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TrustKeyProvider</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TrustKeyProviderForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TrustKeyProvider (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TrustKeyProvider</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TrustKeyProvider;
                ...
                await updateTrustKeyProvider(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TrustKeyProviders.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TrustKeyProvider'}
...
export default TrustKeyProviderTable;
