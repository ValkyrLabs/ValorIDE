...
const columnSchema: Record<string, ColumnSchema> = {
  envelopeAlgorithm: {
    type: "enum",
    enumValues: ["AES_256_GCM", "XCHACHA20_POLY1305"],
...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/TrustObjectKeyEnvelope/${rowId}`;
...
const TrustObjectKeyEnvelopeTable: React.FC = () => {
  ...
  const [updateTrustObjectKeyEnvelope] =
    useUpdateTrustObjectKeyEnvelopeMutation();
  const [deleteTrustObjectKeyEnvelope] =
    useDeleteTrustObjectKeyEnvelopeMutation();
  const [deleteTrustObjectKeyEnvelopeCascade] =
    useDeleteTrustObjectKeyEnvelopeCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<TrustObjectKeyEnvelope | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<TrustObjectKeyEnvelope | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateTrustObjectKeyEnvelope(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update TrustObjectKeyEnvelope";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustObjectKeyEnvelopeCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter TrustObjectKeyEnvelope (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
