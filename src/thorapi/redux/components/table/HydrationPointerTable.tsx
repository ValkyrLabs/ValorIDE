...
import { HydrationPointer } from '@thorapi/model';
import HydrationPointerForm from '@thorapi/redux/components/form/HydrationPointerForm';
...
import {
  useLazyGetHydrationPointersPagedQuery,
...
} from '@thorapi/redux/services/HydrationPointerService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'targetType': { type: 'enum', enumValues: [
    'file', 'model', 'schema', 'receipt', 'memory', 'procedure', 'workflow', 'generated_artifact', 'tenant_app_record', 'external_source', 'other', 
  ], enumValueType: 'string' },
  'status': { type: 'enum', enumValues: [
    'available', 'hydrated', 'denied', 'stale', 'missing', 
  ], enumValueType: 'string' },
  'estimatedTokens': { type: 'number' },
  'lastHydratedAt': { type: 'datetime' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `HydrationPointer update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/HydrationPointer/${rowId}`;

const HydrationPointerTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<HydrationPointer[]>([]);
  ...
  const normalizePageRows = (result: unknown): HydrationPointer[] => {
    if (Array.isArray(result)) return result as HydrationPointer[];
    ...
    return Array.isArray(candidate) ? (candidate as HydrationPointer[]) : [];
  };

  const mergeFetchedRows = (incomingRows: HydrationPointer[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addHydrationPointer] = useAddHydrationPointerMutation();
  const [updateHydrationPointer] = useUpdateHydrationPointerMutation();
  const [deleteHydrationPointer] = useDeleteHydrationPointerMutation();
  const [deleteHydrationPointerCascade] = useDeleteHydrationPointerCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<HydrationPointer>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<HydrationPointer | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<HydrationPointer | null> => {
    ...
    try {
      ...
      return (await response.json()) as HydrationPointer;
    } catch (err) {
      console.warn('Could not reload HydrationPointer row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<HydrationPointer>,
    successField?: string,
  ) => {
    try {
      const saved = await updateHydrationPointer(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as HydrationPointer[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update HydrationPointer';
      console.error('Failed to update HydrationPointer:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteHydrationPointerCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteHydrationPointer(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<HydrationPointer> = { ...source };
      delete (payload as any).id;
      const created = await addHydrationPointer(payload).unwrap();
      setAllData((prev) => [created as HydrationPointer, ...prev]);
    } catch (error) {
      console.error('Failed to add HydrationPointer:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<HydrationPointer> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addHydrationPointer(payload).unwrap();
      setAllData((prev) => [created as HydrationPointer, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) HydrationPointer:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `HydrationPointer-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addHydrationPointer(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateHydrationPointer(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as HydrationPointer[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as HydrationPointer[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as HydrationPointer[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as HydrationPointer[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="HydrationPointerTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="HydrationPointer Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create HydrationPointer</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <HydrationPointerForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter HydrationPointer (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing HydrationPointer</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof HydrationPointer;
                ...
                await updateHydrationPointer(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of HydrationPointers.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'HydrationPointer'}
...
export default HydrationPointerTable;
