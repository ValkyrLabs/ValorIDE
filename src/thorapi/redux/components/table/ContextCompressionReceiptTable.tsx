...
import { ContextCompressionReceipt } from '@thorapi/model';
import ContextCompressionReceiptForm from '@thorapi/redux/components/form/ContextCompressionReceiptForm';
...
import {
  useLazyGetContextCompressionReceiptsPagedQuery,
...
} from '@thorapi/redux/services/ContextCompressionReceiptService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ContextCompressionReceipt update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ContextCompressionReceipt/${rowId}`;

const ContextCompressionReceiptTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ContextCompressionReceipt[]>([]);
  ...
  const normalizePageRows = (result: unknown): ContextCompressionReceipt[] => {
    if (Array.isArray(result)) return result as ContextCompressionReceipt[];
    ...
    return Array.isArray(candidate) ? (candidate as ContextCompressionReceipt[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ContextCompressionReceipt[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addContextCompressionReceipt] = useAddContextCompressionReceiptMutation();
  const [updateContextCompressionReceipt] = useUpdateContextCompressionReceiptMutation();
  const [deleteContextCompressionReceipt] = useDeleteContextCompressionReceiptMutation();
  const [deleteContextCompressionReceiptCascade] = useDeleteContextCompressionReceiptCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ContextCompressionReceipt>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ContextCompressionReceipt | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ContextCompressionReceipt | null> => {
    ...
    try {
      ...
      return (await response.json()) as ContextCompressionReceipt;
    } catch (err) {
      console.warn('Could not reload ContextCompressionReceipt row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ContextCompressionReceipt>,
    successField?: string,
  ) => {
    try {
      const saved = await updateContextCompressionReceipt(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ContextCompressionReceipt[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ContextCompressionReceipt';
      console.error('Failed to update ContextCompressionReceipt:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteContextCompressionReceiptCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteContextCompressionReceipt(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ContextCompressionReceipt> = { ...source };
      delete (payload as any).id;
      const created = await addContextCompressionReceipt(payload).unwrap();
      setAllData((prev) => [created as ContextCompressionReceipt, ...prev]);
    } catch (error) {
      console.error('Failed to add ContextCompressionReceipt:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ContextCompressionReceipt> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addContextCompressionReceipt(payload).unwrap();
      setAllData((prev) => [created as ContextCompressionReceipt, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ContextCompressionReceipt:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ContextCompressionReceipt-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addContextCompressionReceipt(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateContextCompressionReceipt(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ContextCompressionReceipt[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ContextCompressionReceipt[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ContextCompressionReceipt[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ContextCompressionReceipt[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ContextCompressionReceiptTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ContextCompressionReceipt Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ContextCompressionReceipt</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ContextCompressionReceiptForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ContextCompressionReceipt (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ContextCompressionReceipt</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ContextCompressionReceipt;
                ...
                await updateContextCompressionReceipt(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ContextCompressionReceipts.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ContextCompressionReceipt'}
...
export default ContextCompressionReceiptTable;
