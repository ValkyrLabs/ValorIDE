...
import { CreditPricingMatrix } from '@thorapi/model';
import CreditPricingMatrixForm from '@thorapi/redux/components/form/CreditPricingMatrixForm';
...
import {
  useLazyGetCreditPricingMatrixsPagedQuery,
...
} from '@thorapi/redux/services/CreditPricingMatrixService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'unitType': { type: 'enum', enumValues: [
    'cpu_hour', 'memory_gb_hour', 'generated_thorapi_component', 'valoride_sign_in', 'lightsail_instance_hour', 'schema_object', 'api_call', 'llm_token_1k', 'storage_gb_month', 'workflow_run', 'custom', 
  ], enumValueType: 'string' },
  'creditCost': { type: 'number' },
  'billingScope': { type: 'enum', enumValues: [
    'global', 'plan', 'customer', 'organization', 'deployment', 
  ], enumValueType: 'string' },
  'status': { type: 'enum', enumValues: [
    'active', 'draft', 'retired', 
  ], enumValueType: 'string' },
  'effectiveAt': { type: 'datetime' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `CreditPricingMatrix update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/CreditPricingMatrix/${rowId}`;

const CreditPricingMatrixTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<CreditPricingMatrix[]>([]);
  ...
  const normalizePageRows = (result: unknown): CreditPricingMatrix[] => {
    if (Array.isArray(result)) return result as CreditPricingMatrix[];
    ...
    return Array.isArray(candidate) ? (candidate as CreditPricingMatrix[]) : [];
  };

  const mergeFetchedRows = (incomingRows: CreditPricingMatrix[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addCreditPricingMatrix] = useAddCreditPricingMatrixMutation();
  const [updateCreditPricingMatrix] = useUpdateCreditPricingMatrixMutation();
  const [deleteCreditPricingMatrix] = useDeleteCreditPricingMatrixMutation();
  const [deleteCreditPricingMatrixCascade] = useDeleteCreditPricingMatrixCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<CreditPricingMatrix>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<CreditPricingMatrix | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<CreditPricingMatrix | null> => {
    ...
    try {
      ...
      return (await response.json()) as CreditPricingMatrix;
    } catch (err) {
      console.warn('Could not reload CreditPricingMatrix row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<CreditPricingMatrix>,
    successField?: string,
  ) => {
    try {
      const saved = await updateCreditPricingMatrix(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as CreditPricingMatrix[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update CreditPricingMatrix';
      console.error('Failed to update CreditPricingMatrix:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCreditPricingMatrixCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteCreditPricingMatrix(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<CreditPricingMatrix> = { ...source };
      delete (payload as any).id;
      const created = await addCreditPricingMatrix(payload).unwrap();
      setAllData((prev) => [created as CreditPricingMatrix, ...prev]);
    } catch (error) {
      console.error('Failed to add CreditPricingMatrix:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<CreditPricingMatrix> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addCreditPricingMatrix(payload).unwrap();
      setAllData((prev) => [created as CreditPricingMatrix, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) CreditPricingMatrix:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `CreditPricingMatrix-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addCreditPricingMatrix(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateCreditPricingMatrix(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as CreditPricingMatrix[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as CreditPricingMatrix[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as CreditPricingMatrix[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as CreditPricingMatrix[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="CreditPricingMatrixTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="CreditPricingMatrix Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create CreditPricingMatrix</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <CreditPricingMatrixForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter CreditPricingMatrix (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing CreditPricingMatrix</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof CreditPricingMatrix;
                ...
                await updateCreditPricingMatrix(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of CreditPricingMatrixs.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'CreditPricingMatrix'}
...
export default CreditPricingMatrixTable;
