...
import { McpContent } from '@thorapi/model';
import McpContentForm from '@thorapi/redux/components/form/McpContentForm';
...
import {
  ...
  useDeleteMcpContentCascadeMutation
} from '@thorapi/redux/services/McpContentService';
...
const buildResourcePath = (rowId: string) => `${BASE_PATH}/McpContent/${rowId}`;
...
const McpContentTable: React.FC = () => {
  ...
  const reloadRowById = async (rowId: string): Promise<McpContent | null> => {
    ...
    try {
      ...
      return (await response.json()) as McpContent;
    } catch (err) {
      console.warn('Could not reload McpContent row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<McpContent>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMcpContent(updatedItem as any).unwrap();
      const persistedId = String((saved as any)?.id ?? rowId);
      const hydrated = (await reloadRowById(persistedId)) ?? (saved as any) ?? updatedItem;
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as McpContent[]);
      if (successField) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpContentCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `McpContent-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMcpContent(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMcpContent(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
