...
const columnSchema: Record<string, ColumnSchema> = {
  executionStatus: {
    type: "enum",
    enumValues: ["STARTED", "RUNNING", "COMPLETED", "FAILED", "CANCELLED"],
...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/TrustRuntimeExecution/${rowId}`;
...
const TrustRuntimeExecutionTable: React.FC = () => {
  ...
  const [updateTrustRuntimeExecution] =
    useUpdateTrustRuntimeExecutionMutation();
  const [deleteTrustRuntimeExecution] =
    useDeleteTrustRuntimeExecutionMutation();
  const [deleteTrustRuntimeExecutionCascade] =
    useDeleteTrustRuntimeExecutionCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<TrustRuntimeExecution | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<TrustRuntimeExecution | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateTrustRuntimeExecution(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update TrustRuntimeExecution";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustRuntimeExecutionCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter TrustRuntimeExecution (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
