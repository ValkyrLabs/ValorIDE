...
import { TrustRuntimeExecution } from '@thorapi/model';
import TrustRuntimeExecutionForm from '@thorapi/redux/components/form/TrustRuntimeExecutionForm';
...
import {
  useLazyGetTrustRuntimeExecutionsPagedQuery,
...
} from '@thorapi/redux/services/TrustRuntimeExecutionService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'executionStatus': { type: 'enum', enumValues: [
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TrustRuntimeExecution update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TrustRuntimeExecution/${rowId}`;

const TrustRuntimeExecutionTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TrustRuntimeExecution[]>([]);
  ...
  const normalizePageRows = (result: unknown): TrustRuntimeExecution[] => {
    if (Array.isArray(result)) return result as TrustRuntimeExecution[];
    ...
    return Array.isArray(candidate) ? (candidate as TrustRuntimeExecution[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TrustRuntimeExecution[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addTrustRuntimeExecution] = useAddTrustRuntimeExecutionMutation();
  const [updateTrustRuntimeExecution] = useUpdateTrustRuntimeExecutionMutation();
  const [deleteTrustRuntimeExecution] = useDeleteTrustRuntimeExecutionMutation();
  const [deleteTrustRuntimeExecutionCascade] = useDeleteTrustRuntimeExecutionCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TrustRuntimeExecution>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TrustRuntimeExecution | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TrustRuntimeExecution | null> => {
    ...
    try {
      ...
      return (await response.json()) as TrustRuntimeExecution;
    } catch (err) {
      console.warn('Could not reload TrustRuntimeExecution row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TrustRuntimeExecution>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTrustRuntimeExecution(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TrustRuntimeExecution[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TrustRuntimeExecution';
      console.error('Failed to update TrustRuntimeExecution:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustRuntimeExecutionCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTrustRuntimeExecution(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TrustRuntimeExecution> = { ...source };
      delete (payload as any).id;
      const created = await addTrustRuntimeExecution(payload).unwrap();
      setAllData((prev) => [created as TrustRuntimeExecution, ...prev]);
    } catch (error) {
      console.error('Failed to add TrustRuntimeExecution:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TrustRuntimeExecution> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTrustRuntimeExecution(payload).unwrap();
      setAllData((prev) => [created as TrustRuntimeExecution, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TrustRuntimeExecution:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TrustRuntimeExecution-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTrustRuntimeExecution(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTrustRuntimeExecution(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TrustRuntimeExecution[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TrustRuntimeExecution[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TrustRuntimeExecution[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TrustRuntimeExecution[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="TrustRuntimeExecutionTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="TrustRuntimeExecution Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TrustRuntimeExecution</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TrustRuntimeExecutionForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TrustRuntimeExecution (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TrustRuntimeExecution</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TrustRuntimeExecution;
                ...
                await updateTrustRuntimeExecution(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TrustRuntimeExecutions.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TrustRuntimeExecution'}
...
export default TrustRuntimeExecutionTable;
