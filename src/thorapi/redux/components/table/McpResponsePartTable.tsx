...
import { McpResponsePart } from '@thorapi/model';
import McpResponsePartForm from '@thorapi/redux/components/form/McpResponsePartForm';
...
import {
  useLazyGetMcpResponsePartsPagedQuery,
...
} from '@thorapi/redux/services/McpResponsePartService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `McpResponsePart update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/McpResponsePart/${rowId}`;

const McpResponsePartTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<McpResponsePart[]>([]);
  ...
  const normalizePageRows = (result: unknown): McpResponsePart[] => {
    if (Array.isArray(result)) return result as McpResponsePart[];
    ...
    return Array.isArray(candidate) ? (candidate as McpResponsePart[]) : [];
  };

  const mergeFetchedRows = (incomingRows: McpResponsePart[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addMcpResponsePart] = useAddMcpResponsePartMutation();
  const [updateMcpResponsePart] = useUpdateMcpResponsePartMutation();
  const [deleteMcpResponsePart] = useDeleteMcpResponsePartMutation();
  const [deleteMcpResponsePartCascade] = useDeleteMcpResponsePartCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<McpResponsePart>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<McpResponsePart | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<McpResponsePart | null> => {
    ...
    try {
      ...
      return (await response.json()) as McpResponsePart;
    } catch (err) {
      console.warn('Could not reload McpResponsePart row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<McpResponsePart>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMcpResponsePart(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as McpResponsePart[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update McpResponsePart';
      console.error('Failed to update McpResponsePart:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpResponsePartCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteMcpResponsePart(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<McpResponsePart> = { ...source };
      delete (payload as any).id;
      const created = await addMcpResponsePart(payload).unwrap();
      setAllData((prev) => [created as McpResponsePart, ...prev]);
    } catch (error) {
      console.error('Failed to add McpResponsePart:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<McpResponsePart> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMcpResponsePart(payload).unwrap();
      setAllData((prev) => [created as McpResponsePart, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) McpResponsePart:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `McpResponsePart-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMcpResponsePart(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMcpResponsePart(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as McpResponsePart[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as McpResponsePart[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as McpResponsePart[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as McpResponsePart[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="McpResponsePartTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="McpResponsePart Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create McpResponsePart</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <McpResponsePartForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter McpResponsePart (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing McpResponsePart</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof McpResponsePart;
                ...
                await updateMcpResponsePart(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of McpResponseParts.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'McpResponsePart'}
...
export default McpResponsePartTable;
