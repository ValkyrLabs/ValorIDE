...
import SwarmGraphEdgeForm from '@thorapi/redux/components/form/SwarmGraphEdgeForm';
...
} from '@thorapi/redux/services/SwarmGraphEdgeService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SwarmGraphEdge update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SwarmGraphEdge/${rowId}`;
...
const SwarmGraphEdgeTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SwarmGraphEdge[] => {
    if (Array.isArray(result)) return result as SwarmGraphEdge[];
    ...
    return Array.isArray(candidate) ? (candidate as SwarmGraphEdge[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SwarmGraphEdge[], requestedPage: number) => {
  ...
  const [deleteSwarmGraphEdgeCascade] = useDeleteSwarmGraphEdgeCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SwarmGraphEdge | null> => {
    ...
    try {
      ...
      return (await response.json()) as SwarmGraphEdge;
    } catch (err) {
      console.warn('Could not reload SwarmGraphEdge row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SwarmGraphEdge>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSwarmGraphEdge(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SwarmGraphEdge[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SwarmGraphEdge';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmGraphEdgeCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SwarmGraphEdge-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSwarmGraphEdge(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSwarmGraphEdge(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SwarmGraphEdge[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SwarmGraphEdge[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SwarmGraphEdge[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SwarmGraphEdge[]);
    }
  };
