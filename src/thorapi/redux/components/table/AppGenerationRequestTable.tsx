...
import { AppGenerationRequest } from '@thorapi/model';
import AppGenerationRequestForm from '@thorapi/redux/components/form/AppGenerationRequestForm';
...
import {
  useLazyGetAppGenerationRequestsPagedQuery,
...
} from '@thorapi/redux/services/AppGenerationRequestService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'status': { type: 'enum', enumValues: [
    'draft', 'clarifying', 'ready', 'running', 'succeeded', 'failed', 'canceled', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AppGenerationRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AppGenerationRequest/${rowId}`;

const AppGenerationRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<AppGenerationRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): AppGenerationRequest[] => {
    if (Array.isArray(result)) return result as AppGenerationRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as AppGenerationRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: AppGenerationRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addAppGenerationRequest] = useAddAppGenerationRequestMutation();
  const [updateAppGenerationRequest] = useUpdateAppGenerationRequestMutation();
  const [deleteAppGenerationRequest] = useDeleteAppGenerationRequestMutation();
  const [deleteAppGenerationRequestCascade] = useDeleteAppGenerationRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<AppGenerationRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<AppGenerationRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<AppGenerationRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as AppGenerationRequest;
    } catch (err) {
      console.warn('Could not reload AppGenerationRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AppGenerationRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAppGenerationRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AppGenerationRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AppGenerationRequest';
      console.error('Failed to update AppGenerationRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAppGenerationRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteAppGenerationRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<AppGenerationRequest> = { ...source };
      delete (payload as any).id;
      const created = await addAppGenerationRequest(payload).unwrap();
      setAllData((prev) => [created as AppGenerationRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add AppGenerationRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<AppGenerationRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addAppGenerationRequest(payload).unwrap();
      setAllData((prev) => [created as AppGenerationRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) AppGenerationRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AppGenerationRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAppGenerationRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAppGenerationRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AppGenerationRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AppGenerationRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AppGenerationRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AppGenerationRequest[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="AppGenerationRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="AppGenerationRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create AppGenerationRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <AppGenerationRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter AppGenerationRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing AppGenerationRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof AppGenerationRequest;
                ...
                await updateAppGenerationRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of AppGenerationRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'AppGenerationRequest'}
...
export default AppGenerationRequestTable;
