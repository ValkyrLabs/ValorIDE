...
const columnSchema: Record<string, ColumnSchema> = {
  verificationStatus: {
    type: "enum",
    enumValues: ["VALID", "INVALID", "INCONCLUSIVE", "UNAUTHORIZED"],
...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/TrustVerificationResult/${rowId}`;
...
const TrustVerificationResultTable: React.FC = () => {
  ...
  const [updateTrustVerificationResult] =
    useUpdateTrustVerificationResultMutation();
  const [deleteTrustVerificationResult] =
    useDeleteTrustVerificationResultMutation();
  const [deleteTrustVerificationResultCascade] =
    useDeleteTrustVerificationResultCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<TrustVerificationResult | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<TrustVerificationResult | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateTrustVerificationResult(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update TrustVerificationResult";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustVerificationResultCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter TrustVerificationResult (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
