...
import ScanBuildArtifactsRequestForm from '@thorapi/redux/components/form/ScanBuildArtifactsRequestForm';
...
import {
  useLazyGetScanBuildArtifactsRequestsPagedQuery,
...
} from '@thorapi/redux/services/ScanBuildArtifactsRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ScanBuildArtifactsRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ScanBuildArtifactsRequest/${rowId}`;

const ScanBuildArtifactsRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ScanBuildArtifactsRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): ScanBuildArtifactsRequest[] => {
    if (Array.isArray(result)) return result as ScanBuildArtifactsRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as ScanBuildArtifactsRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ScanBuildArtifactsRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addScanBuildArtifactsRequest] = useAddScanBuildArtifactsRequestMutation();
  const [updateScanBuildArtifactsRequest] = useUpdateScanBuildArtifactsRequestMutation();
  const [deleteScanBuildArtifactsRequest] = useDeleteScanBuildArtifactsRequestMutation();
  const [deleteScanBuildArtifactsRequestCascade] = useDeleteScanBuildArtifactsRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ScanBuildArtifactsRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ScanBuildArtifactsRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ScanBuildArtifactsRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as ScanBuildArtifactsRequest;
    } catch (err) {
      console.warn('Could not reload ScanBuildArtifactsRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ScanBuildArtifactsRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateScanBuildArtifactsRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ScanBuildArtifactsRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ScanBuildArtifactsRequest';
      console.error('Failed to update ScanBuildArtifactsRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteScanBuildArtifactsRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteScanBuildArtifactsRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ScanBuildArtifactsRequest> = { ...source };
      delete (payload as any).id;
      const created = await addScanBuildArtifactsRequest(payload).unwrap();
      setAllData((prev) => [created as ScanBuildArtifactsRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add ScanBuildArtifactsRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ScanBuildArtifactsRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addScanBuildArtifactsRequest(payload).unwrap();
      setAllData((prev) => [created as ScanBuildArtifactsRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ScanBuildArtifactsRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ScanBuildArtifactsRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addScanBuildArtifactsRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateScanBuildArtifactsRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ScanBuildArtifactsRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ScanBuildArtifactsRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ScanBuildArtifactsRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ScanBuildArtifactsRequest[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ScanBuildArtifactsRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ScanBuildArtifactsRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ScanBuildArtifactsRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ScanBuildArtifactsRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ScanBuildArtifactsRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ScanBuildArtifactsRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ScanBuildArtifactsRequest;
                ...
                await updateScanBuildArtifactsRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ScanBuildArtifactsRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ScanBuildArtifactsRequest'}
...
export default ScanBuildArtifactsRequestTable;
