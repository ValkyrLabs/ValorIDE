...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/RequestPasswordResetRequest/${rowId}`;
...
const RequestPasswordResetRequestTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addRequestPasswordResetRequest] =
    useAddRequestPasswordResetRequestMutation();
  const [updateRequestPasswordResetRequest] =
    useUpdateRequestPasswordResetRequestMutation();
  const [deleteRequestPasswordResetRequest] =
    useDeleteRequestPasswordResetRequestMutation();
  const [deleteRequestPasswordResetRequestCascade] =
    useDeleteRequestPasswordResetRequestCascadeMutation();
  ...
  const [formData, setFormData] = useState<
    Partial<RequestPasswordResetRequest>
  >({});
  ...
  const [copiedRow, setCopiedRow] =
    useState<RequestPasswordResetRequest | null>(null);
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<RequestPasswordResetRequest | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload RequestPasswordResetRequest row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateRequestPasswordResetRequest(
        updatedItem as any,
      ).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) RequestPasswordResetRequest:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter RequestPasswordResetRequest (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
