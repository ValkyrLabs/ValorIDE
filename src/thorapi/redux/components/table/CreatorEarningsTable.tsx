...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  payoutStatus: {
    type: "enum",
    enumValues: ["PENDING", "PROCESSING", "PAID", "FAILED"],
    enumValueType: "string",
  },
  payoutMethod: {
    type: "enum",
    enumValues: ["STRIPE", "CRYPTO_WALLET", "BANK_TRANSFER"],
    enumValueType: "string",
  },
...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/CreatorEarnings/${rowId}`;
...
const CreatorEarningsTable: React.FC = () => {
  ...
  const [deleteCreatorEarningsCascade] =
    useDeleteCreatorEarningsCascadeMutation();
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update CreatorEarnings";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCreatorEarningsCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
