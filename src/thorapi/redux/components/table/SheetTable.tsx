...
import { Sheet } from '@thorapi/model';
import SheetForm from '@thorapi/redux/components/form/SheetForm';
...
} from '@thorapi/redux/services/SheetService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Sheet update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Sheet/${rowId}`;
...
const SheetTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Sheet[] => {
    if (Array.isArray(result)) return result as Sheet[];
    ...
    return Array.isArray(candidate) ? (candidate as Sheet[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Sheet[], requestedPage: number) => {
  ...
  const [deleteSheetCascade] = useDeleteSheetCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Sheet | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Sheet row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Sheet>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSheet(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Sheet[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Sheet';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSheetCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Sheet-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        try {
          ...
          try {
            await updateSheet(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Sheet[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Sheet[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Sheet[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Sheet[]);
    }
  };
