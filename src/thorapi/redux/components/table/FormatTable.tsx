...
import { Format } from '@thorapi/model';
import FormatForm from '@thorapi/redux/components/form/FormatForm';
...
} from '@thorapi/redux/services/FormatService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Format update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Format/${rowId}`;
...
const FormatTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Format[] => {
    if (Array.isArray(result)) return result as Format[];
    ...
    return Array.isArray(candidate) ? (candidate as Format[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Format[], requestedPage: number) => {
  ...
  const [deleteFormatCascade] = useDeleteFormatCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Format | null> => {
    ...
    try {
      ...
      return (await response.json()) as Format;
    } catch (err) {
      console.warn('Could not reload Format row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Format>,
    successField?: string,
  ) => {
    try {
      const saved = await updateFormat(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Format[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Format';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteFormatCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Format-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addFormat(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateFormat(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Format[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Format[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Format[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Format[]);
    }
  };
