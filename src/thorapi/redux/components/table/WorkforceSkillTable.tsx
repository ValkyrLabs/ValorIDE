...
import { WorkforceSkill } from '@thorapi/model';
import WorkforceSkillForm from '@thorapi/redux/components/form/WorkforceSkillForm';
...
import {
  useLazyGetWorkforceSkillsPagedQuery,
...
} from '@thorapi/redux/services/WorkforceSkillService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'level': { type: 'enum', enumValues: [
    'FOUNDATIONAL', 'PRACTITIONER', 'ADVANCED', 'EXPERT', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `WorkforceSkill update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/WorkforceSkill/${rowId}`;

const WorkforceSkillTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<WorkforceSkill[]>([]);
  ...
  const normalizePageRows = (result: unknown): WorkforceSkill[] => {
    if (Array.isArray(result)) return result as WorkforceSkill[];
    ...
    return Array.isArray(candidate) ? (candidate as WorkforceSkill[]) : [];
  };

  const mergeFetchedRows = (incomingRows: WorkforceSkill[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addWorkforceSkill] = useAddWorkforceSkillMutation();
  const [updateWorkforceSkill] = useUpdateWorkforceSkillMutation();
  const [deleteWorkforceSkill] = useDeleteWorkforceSkillMutation();
  const [deleteWorkforceSkillCascade] = useDeleteWorkforceSkillCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<WorkforceSkill>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<WorkforceSkill | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<WorkforceSkill | null> => {
    ...
    try {
      ...
      return (await response.json()) as WorkforceSkill;
    } catch (err) {
      console.warn('Could not reload WorkforceSkill row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<WorkforceSkill>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWorkforceSkill(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as WorkforceSkill[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update WorkforceSkill';
      console.error('Failed to update WorkforceSkill:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkforceSkillCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteWorkforceSkill(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<WorkforceSkill> = { ...source };
      delete (payload as any).id;
      const created = await addWorkforceSkill(payload).unwrap();
      setAllData((prev) => [created as WorkforceSkill, ...prev]);
    } catch (error) {
      console.error('Failed to add WorkforceSkill:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<WorkforceSkill> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addWorkforceSkill(payload).unwrap();
      setAllData((prev) => [created as WorkforceSkill, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) WorkforceSkill:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `WorkforceSkill-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWorkforceSkill(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWorkforceSkill(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as WorkforceSkill[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as WorkforceSkill[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as WorkforceSkill[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as WorkforceSkill[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="WorkforceSkillTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="WorkforceSkill Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create WorkforceSkill</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <WorkforceSkillForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter WorkforceSkill (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing WorkforceSkill</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof WorkforceSkill;
                ...
                await updateWorkforceSkill(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of WorkforceSkills.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'WorkforceSkill'}
...
export default WorkforceSkillTable;
