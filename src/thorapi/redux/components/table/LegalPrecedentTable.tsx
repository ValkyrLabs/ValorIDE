...
const ICON_SIZE = 22;
import { LegalPrecedent } from '@thorapi/model';
import LegalPrecedentForm from '@thorapi/redux/components/form/LegalPrecedentForm';
...
} from '@thorapi/redux/services/LegalPrecedentService';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteLegalPrecedentCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update LegalPrecedent';
      console.error('Failed to update LegalPrecedent:', error);
