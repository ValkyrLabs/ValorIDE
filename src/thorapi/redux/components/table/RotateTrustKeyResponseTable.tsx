...
import { RotateTrustKeyResponse } from '@thorapi/model';
import RotateTrustKeyResponseForm from '@thorapi/redux/components/form/RotateTrustKeyResponseForm';
...
import {
  useLazyGetRotateTrustKeyResponsesPagedQuery,
...
} from '@thorapi/redux/services/RotateTrustKeyResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RotateTrustKeyResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RotateTrustKeyResponse/${rowId}`;

const RotateTrustKeyResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RotateTrustKeyResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): RotateTrustKeyResponse[] => {
    if (Array.isArray(result)) return result as RotateTrustKeyResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as RotateTrustKeyResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RotateTrustKeyResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addRotateTrustKeyResponse] = useAddRotateTrustKeyResponseMutation();
  const [updateRotateTrustKeyResponse] = useUpdateRotateTrustKeyResponseMutation();
  const [deleteRotateTrustKeyResponse] = useDeleteRotateTrustKeyResponseMutation();
  const [deleteRotateTrustKeyResponseCascade] = useDeleteRotateTrustKeyResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<RotateTrustKeyResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RotateTrustKeyResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<RotateTrustKeyResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as RotateTrustKeyResponse;
    } catch (err) {
      console.warn('Could not reload RotateTrustKeyResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RotateTrustKeyResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRotateTrustKeyResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RotateTrustKeyResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RotateTrustKeyResponse';
      console.error('Failed to update RotateTrustKeyResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRotateTrustKeyResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteRotateTrustKeyResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RotateTrustKeyResponse> = { ...source };
      delete (payload as any).id;
      const created = await addRotateTrustKeyResponse(payload).unwrap();
      setAllData((prev) => [created as RotateTrustKeyResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add RotateTrustKeyResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RotateTrustKeyResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRotateTrustKeyResponse(payload).unwrap();
      setAllData((prev) => [created as RotateTrustKeyResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) RotateTrustKeyResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RotateTrustKeyResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRotateTrustKeyResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRotateTrustKeyResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RotateTrustKeyResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RotateTrustKeyResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RotateTrustKeyResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RotateTrustKeyResponse[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.RotateTrustKeyResponse',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="RotateTrustKeyResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="RotateTrustKeyResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RotateTrustKeyResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RotateTrustKeyResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RotateTrustKeyResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RotateTrustKeyResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RotateTrustKeyResponse;
                ...
                await updateRotateTrustKeyResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RotateTrustKeyResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RotateTrustKeyResponse'}
...
export default RotateTrustKeyResponseTable;
