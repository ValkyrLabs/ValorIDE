...
import { McpServiceResponse } from '@thorapi/model';
import McpServiceResponseForm from '@thorapi/redux/components/form/McpServiceResponseForm';
...
} from '@thorapi/redux/services/McpServiceResponseService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'status': { type: 'enum', enumValues: [
    'published', 'pending_approval', 'draft', 'failed', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `McpServiceResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/McpServiceResponse/${rowId}`;
...
const McpServiceResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): McpServiceResponse[] => {
    if (Array.isArray(result)) return result as McpServiceResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as McpServiceResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: McpServiceResponse[], requestedPage: number) => {
  ...
  const [deleteMcpServiceResponseCascade] = useDeleteMcpServiceResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<McpServiceResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as McpServiceResponse;
    } catch (err) {
      console.warn('Could not reload McpServiceResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<McpServiceResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMcpServiceResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as McpServiceResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update McpServiceResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpServiceResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `McpServiceResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMcpServiceResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMcpServiceResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as McpServiceResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as McpServiceResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as McpServiceResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as McpServiceResponse[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter McpServiceResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
