...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/WorkflowGraphNode/${rowId}`;
...
const WorkflowGraphNodeTable: React.FC = () => {
  ...
  const [deleteWorkflowGraphNodeCascade] =
    useDeleteWorkflowGraphNodeCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<WorkflowGraphNode | null> => {
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update WorkflowGraphNode";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphNodeCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
