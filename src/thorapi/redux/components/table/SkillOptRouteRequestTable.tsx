...
import { SkillOptRouteRequest } from '@thorapi/model';
import SkillOptRouteRequestForm from '@thorapi/redux/components/form/SkillOptRouteRequestForm';
...
import {
  useLazyGetSkillOptRouteRequestsPagedQuery,
...
} from '@thorapi/redux/services/SkillOptRouteRequestService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'estimatedCreditCost': { type: 'number' },
  'creditBalance': { type: 'number' },
  'contextSufficiency': { type: 'number' },
  'confidence': { type: 'number' },
  'latencyBudgetMs': { type: 'number' },
  'riskLevel': { type: 'enum', enumValues: [
    'low', 'medium', 'high', 'critical', 
  ], enumValueType: 'string' },
  'privacyPolicy': { type: 'enum', enumValues: [
    'public', 'internal', 'confidential', 'restricted', 
  ], enumValueType: 'string' },
  'localAvailable': { type: 'boolean' },
  'billable': { type: 'boolean' },
  'requiresAudit': { type: 'boolean' },
  'humanApprovalAvailable': { type: 'boolean' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SkillOptRouteRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SkillOptRouteRequest/${rowId}`;

const SkillOptRouteRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SkillOptRouteRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): SkillOptRouteRequest[] => {
    if (Array.isArray(result)) return result as SkillOptRouteRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as SkillOptRouteRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SkillOptRouteRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addSkillOptRouteRequest] = useAddSkillOptRouteRequestMutation();
  const [updateSkillOptRouteRequest] = useUpdateSkillOptRouteRequestMutation();
  const [deleteSkillOptRouteRequest] = useDeleteSkillOptRouteRequestMutation();
  const [deleteSkillOptRouteRequestCascade] = useDeleteSkillOptRouteRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<SkillOptRouteRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SkillOptRouteRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<SkillOptRouteRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as SkillOptRouteRequest;
    } catch (err) {
      console.warn('Could not reload SkillOptRouteRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SkillOptRouteRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSkillOptRouteRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SkillOptRouteRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SkillOptRouteRequest';
      console.error('Failed to update SkillOptRouteRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSkillOptRouteRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteSkillOptRouteRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SkillOptRouteRequest> = { ...source };
      delete (payload as any).id;
      const created = await addSkillOptRouteRequest(payload).unwrap();
      setAllData((prev) => [created as SkillOptRouteRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add SkillOptRouteRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SkillOptRouteRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSkillOptRouteRequest(payload).unwrap();
      setAllData((prev) => [created as SkillOptRouteRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) SkillOptRouteRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SkillOptRouteRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSkillOptRouteRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSkillOptRouteRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SkillOptRouteRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SkillOptRouteRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SkillOptRouteRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SkillOptRouteRequest[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="SkillOptRouteRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="SkillOptRouteRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SkillOptRouteRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SkillOptRouteRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SkillOptRouteRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SkillOptRouteRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SkillOptRouteRequest;
                ...
                await updateSkillOptRouteRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SkillOptRouteRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SkillOptRouteRequest'}
...
export default SkillOptRouteRequestTable;
