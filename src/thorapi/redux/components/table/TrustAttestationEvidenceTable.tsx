...
import { TrustAttestationEvidence } from '@thorapi/model';
import TrustAttestationEvidenceForm from '@thorapi/redux/components/form/TrustAttestationEvidenceForm';
...
import {
  useLazyGetTrustAttestationEvidencesPagedQuery,
...
} from '@thorapi/redux/services/TrustAttestationEvidenceService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'evidenceType': { type: 'enum', enumValues: [
    'SOFTWARE_DECLARATION', 'CONTAINER_IMAGE', 'CI_BUILD', 'TPM', 'SGX', 'SEV_SNP', 'NITRO_ENCLAVE', 
  ], enumValueType: 'string' },
  'evidenceStatus': { type: 'enum', enumValues: [
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TrustAttestationEvidence update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TrustAttestationEvidence/${rowId}`;

const TrustAttestationEvidenceTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TrustAttestationEvidence[]>([]);
  ...
  const normalizePageRows = (result: unknown): TrustAttestationEvidence[] => {
    if (Array.isArray(result)) return result as TrustAttestationEvidence[];
    ...
    return Array.isArray(candidate) ? (candidate as TrustAttestationEvidence[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TrustAttestationEvidence[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addTrustAttestationEvidence] = useAddTrustAttestationEvidenceMutation();
  const [updateTrustAttestationEvidence] = useUpdateTrustAttestationEvidenceMutation();
  const [deleteTrustAttestationEvidence] = useDeleteTrustAttestationEvidenceMutation();
  const [deleteTrustAttestationEvidenceCascade] = useDeleteTrustAttestationEvidenceCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TrustAttestationEvidence>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TrustAttestationEvidence | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TrustAttestationEvidence | null> => {
    ...
    try {
      ...
      return (await response.json()) as TrustAttestationEvidence;
    } catch (err) {
      console.warn('Could not reload TrustAttestationEvidence row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TrustAttestationEvidence>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTrustAttestationEvidence(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TrustAttestationEvidence[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TrustAttestationEvidence';
      console.error('Failed to update TrustAttestationEvidence:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustAttestationEvidenceCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTrustAttestationEvidence(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TrustAttestationEvidence> = { ...source };
      delete (payload as any).id;
      const created = await addTrustAttestationEvidence(payload).unwrap();
      setAllData((prev) => [created as TrustAttestationEvidence, ...prev]);
    } catch (error) {
      console.error('Failed to add TrustAttestationEvidence:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TrustAttestationEvidence> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTrustAttestationEvidence(payload).unwrap();
      setAllData((prev) => [created as TrustAttestationEvidence, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TrustAttestationEvidence:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TrustAttestationEvidence-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTrustAttestationEvidence(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTrustAttestationEvidence(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TrustAttestationEvidence[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TrustAttestationEvidence[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TrustAttestationEvidence[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TrustAttestationEvidence[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="TrustAttestationEvidenceTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="TrustAttestationEvidence Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TrustAttestationEvidence</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TrustAttestationEvidenceForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TrustAttestationEvidence (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TrustAttestationEvidence</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TrustAttestationEvidence;
                ...
                await updateTrustAttestationEvidence(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TrustAttestationEvidences.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TrustAttestationEvidence'}
...
export default TrustAttestationEvidenceTable;
