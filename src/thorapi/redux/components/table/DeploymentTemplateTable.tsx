...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/DeploymentTemplate/${rowId}`;
...
const DeploymentTemplateTable: React.FC = () => {
  ...
  const [deleteDeploymentTemplateCascade] =
    useDeleteDeploymentTemplateCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<DeploymentTemplate | null> => {
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update DeploymentTemplate";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteDeploymentTemplateCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter DeploymentTemplate (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
