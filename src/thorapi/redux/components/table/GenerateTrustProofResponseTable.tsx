...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/GenerateTrustProofResponse/${rowId}`;
...
const GenerateTrustProofResponseTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addGenerateTrustProofResponse] =
    useAddGenerateTrustProofResponseMutation();
  const [updateGenerateTrustProofResponse] =
    useUpdateGenerateTrustProofResponseMutation();
  const [deleteGenerateTrustProofResponse] =
    useDeleteGenerateTrustProofResponseMutation();
  const [deleteGenerateTrustProofResponseCascade] =
    useDeleteGenerateTrustProofResponseCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<GenerateTrustProofResponse | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<GenerateTrustProofResponse | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload GenerateTrustProofResponse row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateGenerateTrustProofResponse(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update GenerateTrustProofResponse";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteGenerateTrustProofResponseCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) GenerateTrustProofResponse:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter GenerateTrustProofResponse (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
