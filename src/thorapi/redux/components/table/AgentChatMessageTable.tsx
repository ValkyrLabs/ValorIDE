...
import { AgentChatMessage } from '@thorapi/model';
import AgentChatMessageForm from '@thorapi/redux/components/form/AgentChatMessageForm';
...
} from '@thorapi/redux/services/AgentChatMessageService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'senderType': { type: 'enum', enumValues: [
    'AGENT', 'USER', 'SYSTEM', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AgentChatMessage update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AgentChatMessage/${rowId}`;
...
const AgentChatMessageTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): AgentChatMessage[] => {
    if (Array.isArray(result)) return result as AgentChatMessage[];
    ...
    return Array.isArray(candidate) ? (candidate as AgentChatMessage[]) : [];
  };

  const mergeFetchedRows = (incomingRows: AgentChatMessage[], requestedPage: number) => {
  ...
  const [deleteAgentChatMessageCascade] = useDeleteAgentChatMessageCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<AgentChatMessage | null> => {
    ...
    try {
      ...
      console.warn('Could not reload AgentChatMessage row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AgentChatMessage>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAgentChatMessage(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AgentChatMessage[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AgentChatMessage';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAgentChatMessageCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AgentChatMessage-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAgentChatMessage(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAgentChatMessage(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AgentChatMessage[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AgentChatMessage[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AgentChatMessage[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AgentChatMessage[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter AgentChatMessage (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
