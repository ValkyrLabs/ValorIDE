...
import { DigitalAsset } from '@thorapi/model';
import DigitalAssetForm from '@thorapi/redux/components/form/DigitalAssetForm';
...
} from '@thorapi/redux/services/DigitalAssetService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `DigitalAsset update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/DigitalAsset/${rowId}`;
...
const DigitalAssetTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): DigitalAsset[] => {
    if (Array.isArray(result)) return result as DigitalAsset[];
    ...
    return Array.isArray(candidate) ? (candidate as DigitalAsset[]) : [];
  };

  const mergeFetchedRows = (incomingRows: DigitalAsset[], requestedPage: number) => {
  ...
  const [deleteDigitalAssetCascade] = useDeleteDigitalAssetCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<DigitalAsset | null> => {
    ...
    try {
      ...
      return (await response.json()) as DigitalAsset;
    } catch (err) {
      console.warn('Could not reload DigitalAsset row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<DigitalAsset>,
    successField?: string,
  ) => {
    try {
      const saved = await updateDigitalAsset(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as DigitalAsset[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update DigitalAsset';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteDigitalAssetCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `DigitalAsset-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addDigitalAsset(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateDigitalAsset(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as DigitalAsset[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as DigitalAsset[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as DigitalAsset[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as DigitalAsset[]);
    }
  };
