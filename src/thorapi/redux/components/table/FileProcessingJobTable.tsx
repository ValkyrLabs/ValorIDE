...
import { FileProcessingJob } from '@thorapi/model';
import FileProcessingJobForm from '@thorapi/redux/components/form/FileProcessingJobForm';
...
} from '@thorapi/redux/services/FileProcessingJobService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `FileProcessingJob update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/FileProcessingJob/${rowId}`;
...
const FileProcessingJobTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): FileProcessingJob[] => {
    if (Array.isArray(result)) return result as FileProcessingJob[];
    ...
    return Array.isArray(candidate) ? (candidate as FileProcessingJob[]) : [];
  };

  const mergeFetchedRows = (incomingRows: FileProcessingJob[], requestedPage: number) => {
  ...
  const [deleteFileProcessingJobCascade] = useDeleteFileProcessingJobCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<FileProcessingJob | null> => {
    ...
    try {
      ...
      return (await response.json()) as FileProcessingJob;
    } catch (err) {
      console.warn('Could not reload FileProcessingJob row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<FileProcessingJob>,
    successField?: string,
  ) => {
    try {
      const saved = await updateFileProcessingJob(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as FileProcessingJob[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update FileProcessingJob';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteFileProcessingJobCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `FileProcessingJob-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addFileProcessingJob(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateFileProcessingJob(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as FileProcessingJob[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as FileProcessingJob[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as FileProcessingJob[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as FileProcessingJob[]);
    }
  };
