...
import { OasPath } from '@thorapi/model';
import OasPathForm from '@thorapi/redux/components/form/OasPathForm';
...
} from '@thorapi/redux/services/OasPathService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `OasPath update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/OasPath/${rowId}`;
...
const OasPathTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): OasPath[] => {
    if (Array.isArray(result)) return result as OasPath[];
    ...
    return Array.isArray(candidate) ? (candidate as OasPath[]) : [];
  };

  const mergeFetchedRows = (incomingRows: OasPath[], requestedPage: number) => {
  ...
  const [deleteOasPathCascade] = useDeleteOasPathCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<OasPath | null> => {
    ...
    try {
      ...
      return (await response.json()) as OasPath;
    } catch (err) {
      console.warn('Could not reload OasPath row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<OasPath>,
    successField?: string,
  ) => {
    try {
      const saved = await updateOasPath(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as OasPath[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update OasPath';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOasPathCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `OasPath-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addOasPath(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateOasPath(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as OasPath[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as OasPath[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as OasPath[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as OasPath[]);
    }
  };
