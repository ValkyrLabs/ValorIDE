...
import { VerifyTwoFactorRequest } from '@thorapi/model';
import VerifyTwoFactorRequestForm from '@thorapi/redux/components/form/VerifyTwoFactorRequestForm';
...
import {
  useLazyGetVerifyTwoFactorRequestsPagedQuery,
...
} from '@thorapi/redux/services/VerifyTwoFactorRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `VerifyTwoFactorRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/VerifyTwoFactorRequest/${rowId}`;

const VerifyTwoFactorRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<VerifyTwoFactorRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): VerifyTwoFactorRequest[] => {
    if (Array.isArray(result)) return result as VerifyTwoFactorRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as VerifyTwoFactorRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: VerifyTwoFactorRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addVerifyTwoFactorRequest] = useAddVerifyTwoFactorRequestMutation();
  const [updateVerifyTwoFactorRequest] = useUpdateVerifyTwoFactorRequestMutation();
  const [deleteVerifyTwoFactorRequest] = useDeleteVerifyTwoFactorRequestMutation();
  const [deleteVerifyTwoFactorRequestCascade] = useDeleteVerifyTwoFactorRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<VerifyTwoFactorRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<VerifyTwoFactorRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<VerifyTwoFactorRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as VerifyTwoFactorRequest;
    } catch (err) {
      console.warn('Could not reload VerifyTwoFactorRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<VerifyTwoFactorRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateVerifyTwoFactorRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as VerifyTwoFactorRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update VerifyTwoFactorRequest';
      console.error('Failed to update VerifyTwoFactorRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteVerifyTwoFactorRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteVerifyTwoFactorRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<VerifyTwoFactorRequest> = { ...source };
      delete (payload as any).id;
      const created = await addVerifyTwoFactorRequest(payload).unwrap();
      setAllData((prev) => [created as VerifyTwoFactorRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add VerifyTwoFactorRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<VerifyTwoFactorRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addVerifyTwoFactorRequest(payload).unwrap();
      setAllData((prev) => [created as VerifyTwoFactorRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) VerifyTwoFactorRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `VerifyTwoFactorRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addVerifyTwoFactorRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateVerifyTwoFactorRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as VerifyTwoFactorRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as VerifyTwoFactorRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as VerifyTwoFactorRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as VerifyTwoFactorRequest[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="VerifyTwoFactorRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="VerifyTwoFactorRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create VerifyTwoFactorRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <VerifyTwoFactorRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter VerifyTwoFactorRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing VerifyTwoFactorRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof VerifyTwoFactorRequest;
                ...
                await updateVerifyTwoFactorRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of VerifyTwoFactorRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'VerifyTwoFactorRequest'}
...
export default VerifyTwoFactorRequestTable;
