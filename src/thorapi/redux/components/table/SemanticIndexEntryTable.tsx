...
import { SemanticIndexEntry } from '@thorapi/model';
import SemanticIndexEntryForm from '@thorapi/redux/components/form/SemanticIndexEntryForm';
...
import {
  useLazyGetSemanticIndexEntrysPagedQuery,
...
} from '@thorapi/redux/services/SemanticIndexEntryService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'compressionStrategy': { type: 'enum', enumValues: [
    'none', 'summary_v1', 'hierarchical_summary_v1', 
  ], enumValueType: 'string' },
  'embeddingDimensions': { type: 'number' },
  'vectorEncoding': { type: 'enum', enumValues: [
    'float32_le', 'uint8_quantized', 'summary_only', 
  ], enumValueType: 'string' },
  ...
  'lastHitAt': { type: 'datetime' },
  'semanticDeletedAt': { type: 'datetime' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SemanticIndexEntry update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SemanticIndexEntry/${rowId}`;

const SemanticIndexEntryTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SemanticIndexEntry[]>([]);
  ...
  const normalizePageRows = (result: unknown): SemanticIndexEntry[] => {
    if (Array.isArray(result)) return result as SemanticIndexEntry[];
    ...
    return Array.isArray(candidate) ? (candidate as SemanticIndexEntry[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SemanticIndexEntry[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addSemanticIndexEntry] = useAddSemanticIndexEntryMutation();
  const [updateSemanticIndexEntry] = useUpdateSemanticIndexEntryMutation();
  const [deleteSemanticIndexEntry] = useDeleteSemanticIndexEntryMutation();
  const [deleteSemanticIndexEntryCascade] = useDeleteSemanticIndexEntryCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<SemanticIndexEntry>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SemanticIndexEntry | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<SemanticIndexEntry | null> => {
    ...
    try {
      ...
      return (await response.json()) as SemanticIndexEntry;
    } catch (err) {
      console.warn('Could not reload SemanticIndexEntry row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SemanticIndexEntry>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSemanticIndexEntry(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SemanticIndexEntry[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SemanticIndexEntry';
      console.error('Failed to update SemanticIndexEntry:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSemanticIndexEntryCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteSemanticIndexEntry(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SemanticIndexEntry> = { ...source };
      delete (payload as any).id;
      const created = await addSemanticIndexEntry(payload).unwrap();
      setAllData((prev) => [created as SemanticIndexEntry, ...prev]);
    } catch (error) {
      console.error('Failed to add SemanticIndexEntry:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SemanticIndexEntry> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSemanticIndexEntry(payload).unwrap();
      setAllData((prev) => [created as SemanticIndexEntry, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) SemanticIndexEntry:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SemanticIndexEntry-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSemanticIndexEntry(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSemanticIndexEntry(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SemanticIndexEntry[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SemanticIndexEntry[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SemanticIndexEntry[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SemanticIndexEntry[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="SemanticIndexEntryTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="SemanticIndexEntry Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SemanticIndexEntry</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SemanticIndexEntryForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SemanticIndexEntry (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SemanticIndexEntry</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SemanticIndexEntry;
                ...
                await updateSemanticIndexEntry(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SemanticIndexEntrys.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SemanticIndexEntry'}
...
export default SemanticIndexEntryTable;
