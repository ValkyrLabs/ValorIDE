...
const columnSchema: Record<string, ColumnSchema> = {
  compressionStrategy: {
    type: "enum",
    enumValues: ["none", "summary_v1", "hierarchical_summary_v1"],
    enumValueType: "string",
  },
  embeddingDimensions: { type: "number" },
  vectorEncoding: {
    type: "enum",
    enumValues: ["float32_le", "uint8_quantized", "summary_only"],
    enumValueType: "string",
  },
  status: {
    type: "enum",
    enumValues: ["active", "stale", "deleted"],
    enumValueType: "string",
  },
...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/SemanticIndexEntry/${rowId}`;
...
const SemanticIndexEntryTable: React.FC = () => {
  ...
  const [deleteSemanticIndexEntryCascade] =
    useDeleteSemanticIndexEntryCascadeMutation();
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update SemanticIndexEntry";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSemanticIndexEntryCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter SemanticIndexEntry (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
