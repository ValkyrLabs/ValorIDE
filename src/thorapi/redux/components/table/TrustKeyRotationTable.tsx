...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/TrustKeyRotation/${rowId}`;
...
const TrustKeyRotationTable: React.FC = () => {
  ...
  const [deleteTrustKeyRotationCascade] =
    useDeleteTrustKeyRotationCascadeMutation();
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update TrustKeyRotation";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustKeyRotationCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
