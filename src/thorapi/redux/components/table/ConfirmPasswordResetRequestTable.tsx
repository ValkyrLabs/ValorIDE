...
import { ConfirmPasswordResetRequest } from '@thorapi/model';
import ConfirmPasswordResetRequestForm from '@thorapi/redux/components/form/ConfirmPasswordResetRequestForm';
...
import {
  useLazyGetConfirmPasswordResetRequestsPagedQuery,
...
} from '@thorapi/redux/services/ConfirmPasswordResetRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ConfirmPasswordResetRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ConfirmPasswordResetRequest/${rowId}`;

const ConfirmPasswordResetRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ConfirmPasswordResetRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): ConfirmPasswordResetRequest[] => {
    if (Array.isArray(result)) return result as ConfirmPasswordResetRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as ConfirmPasswordResetRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ConfirmPasswordResetRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addConfirmPasswordResetRequest] = useAddConfirmPasswordResetRequestMutation();
  const [updateConfirmPasswordResetRequest] = useUpdateConfirmPasswordResetRequestMutation();
  const [deleteConfirmPasswordResetRequest] = useDeleteConfirmPasswordResetRequestMutation();
  const [deleteConfirmPasswordResetRequestCascade] = useDeleteConfirmPasswordResetRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ConfirmPasswordResetRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ConfirmPasswordResetRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ConfirmPasswordResetRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as ConfirmPasswordResetRequest;
    } catch (err) {
      console.warn('Could not reload ConfirmPasswordResetRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ConfirmPasswordResetRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateConfirmPasswordResetRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ConfirmPasswordResetRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ConfirmPasswordResetRequest';
      console.error('Failed to update ConfirmPasswordResetRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteConfirmPasswordResetRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteConfirmPasswordResetRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ConfirmPasswordResetRequest> = { ...source };
      delete (payload as any).id;
      const created = await addConfirmPasswordResetRequest(payload).unwrap();
      setAllData((prev) => [created as ConfirmPasswordResetRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add ConfirmPasswordResetRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ConfirmPasswordResetRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addConfirmPasswordResetRequest(payload).unwrap();
      setAllData((prev) => [created as ConfirmPasswordResetRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ConfirmPasswordResetRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ConfirmPasswordResetRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addConfirmPasswordResetRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateConfirmPasswordResetRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ConfirmPasswordResetRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ConfirmPasswordResetRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ConfirmPasswordResetRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ConfirmPasswordResetRequest[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ConfirmPasswordResetRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ConfirmPasswordResetRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ConfirmPasswordResetRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ConfirmPasswordResetRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ConfirmPasswordResetRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ConfirmPasswordResetRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ConfirmPasswordResetRequest;
                ...
                await updateConfirmPasswordResetRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ConfirmPasswordResetRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ConfirmPasswordResetRequest'}
...
export default ConfirmPasswordResetRequestTable;
