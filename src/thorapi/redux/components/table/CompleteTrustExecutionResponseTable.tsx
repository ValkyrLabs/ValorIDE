...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/CompleteTrustExecutionResponse/${rowId}`;
...
const CompleteTrustExecutionResponseTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addCompleteTrustExecutionResponse] =
    useAddCompleteTrustExecutionResponseMutation();
  const [updateCompleteTrustExecutionResponse] =
    useUpdateCompleteTrustExecutionResponseMutation();
  const [deleteCompleteTrustExecutionResponse] =
    useDeleteCompleteTrustExecutionResponseMutation();
  const [deleteCompleteTrustExecutionResponseCascade] =
    useDeleteCompleteTrustExecutionResponseCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] =
    useState<CompleteTrustExecutionResponse | null>(null);
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<CompleteTrustExecutionResponse | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload CompleteTrustExecutionResponse row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateCompleteTrustExecutionResponse(
        updatedItem as any,
      ).unwrap();
      ...
      setAllData(
        (prev) =>
          mergeUpdatedRow(
            ...
            rowId,
            hydrated as any,
          ) as CompleteTrustExecutionResponse[],
      );
      if (successField) {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update CompleteTrustExecutionResponse";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCompleteTrustExecutionResponseCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handleAddRow = async () => {
    try {
      ...
      setAllData((prev) => [
        created as CompleteTrustExecutionResponse,
        ...prev,
      ]);
    } catch (error) {
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) CompleteTrustExecutionResponse:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter CompleteTrustExecutionResponse (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
      ...
      <MarkdownEditorModal
        ...
        onSave={async (updatedText) => {
          try {
            ...
            const field =
              richModal.field as keyof CompleteTrustExecutionResponse;
            if (!rid || !field) {
