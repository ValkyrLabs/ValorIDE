...
import { CompleteTrustExecutionResponse } from '@thorapi/model';
import CompleteTrustExecutionResponseForm from '@thorapi/redux/components/form/CompleteTrustExecutionResponseForm';
...
import {
  useLazyGetCompleteTrustExecutionResponsesPagedQuery,
...
} from '@thorapi/redux/services/CompleteTrustExecutionResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `CompleteTrustExecutionResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/CompleteTrustExecutionResponse/${rowId}`;

const CompleteTrustExecutionResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<CompleteTrustExecutionResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): CompleteTrustExecutionResponse[] => {
    if (Array.isArray(result)) return result as CompleteTrustExecutionResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as CompleteTrustExecutionResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: CompleteTrustExecutionResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addCompleteTrustExecutionResponse] = useAddCompleteTrustExecutionResponseMutation();
  const [updateCompleteTrustExecutionResponse] = useUpdateCompleteTrustExecutionResponseMutation();
  const [deleteCompleteTrustExecutionResponse] = useDeleteCompleteTrustExecutionResponseMutation();
  const [deleteCompleteTrustExecutionResponseCascade] = useDeleteCompleteTrustExecutionResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<CompleteTrustExecutionResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<CompleteTrustExecutionResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<CompleteTrustExecutionResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as CompleteTrustExecutionResponse;
    } catch (err) {
      console.warn('Could not reload CompleteTrustExecutionResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<CompleteTrustExecutionResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateCompleteTrustExecutionResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as CompleteTrustExecutionResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update CompleteTrustExecutionResponse';
      console.error('Failed to update CompleteTrustExecutionResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteCompleteTrustExecutionResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteCompleteTrustExecutionResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<CompleteTrustExecutionResponse> = { ...source };
      delete (payload as any).id;
      const created = await addCompleteTrustExecutionResponse(payload).unwrap();
      setAllData((prev) => [created as CompleteTrustExecutionResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add CompleteTrustExecutionResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<CompleteTrustExecutionResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addCompleteTrustExecutionResponse(payload).unwrap();
      setAllData((prev) => [created as CompleteTrustExecutionResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) CompleteTrustExecutionResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `CompleteTrustExecutionResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addCompleteTrustExecutionResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateCompleteTrustExecutionResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as CompleteTrustExecutionResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as CompleteTrustExecutionResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as CompleteTrustExecutionResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as CompleteTrustExecutionResponse[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.CompleteTrustExecutionResponse',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="CompleteTrustExecutionResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="CompleteTrustExecutionResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create CompleteTrustExecutionResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <CompleteTrustExecutionResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter CompleteTrustExecutionResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing CompleteTrustExecutionResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof CompleteTrustExecutionResponse;
                ...
                await updateCompleteTrustExecutionResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of CompleteTrustExecutionResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'CompleteTrustExecutionResponse'}
...
export default CompleteTrustExecutionResponseTable;
