...
import { McpTransportConfig } from '@thorapi/model';
import McpTransportConfigForm from '@thorapi/redux/components/form/McpTransportConfigForm';
...
} from '@thorapi/redux/services/McpTransportConfigService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'transportType': { type: 'enum', enumValues: [
    'STDIO', 'HTTP', 'WEBSOCKET', 'SSE', 'LAMBDA', 'CUSTOM', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `McpTransportConfig update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/McpTransportConfig/${rowId}`;
...
const McpTransportConfigTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): McpTransportConfig[] => {
    if (Array.isArray(result)) return result as McpTransportConfig[];
    ...
    return Array.isArray(candidate) ? (candidate as McpTransportConfig[]) : [];
  };

  const mergeFetchedRows = (incomingRows: McpTransportConfig[], requestedPage: number) => {
  ...
  const [deleteMcpTransportConfigCascade] = useDeleteMcpTransportConfigCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<McpTransportConfig | null> => {
    ...
    try {
      ...
      return (await response.json()) as McpTransportConfig;
    } catch (err) {
      console.warn('Could not reload McpTransportConfig row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<McpTransportConfig>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMcpTransportConfig(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as McpTransportConfig[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update McpTransportConfig';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpTransportConfigCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `McpTransportConfig-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMcpTransportConfig(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMcpTransportConfig(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as McpTransportConfig[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as McpTransportConfig[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as McpTransportConfig[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as McpTransportConfig[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter McpTransportConfig (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
