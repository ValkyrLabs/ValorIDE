...
import { OasOperation } from '@thorapi/model';
import OasOperationForm from '@thorapi/redux/components/form/OasOperationForm';
...
} from '@thorapi/redux/services/OasOperationService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `OasOperation update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/OasOperation/${rowId}`;
...
const OasOperationTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): OasOperation[] => {
    if (Array.isArray(result)) return result as OasOperation[];
    ...
    return Array.isArray(candidate) ? (candidate as OasOperation[]) : [];
  };

  const mergeFetchedRows = (incomingRows: OasOperation[], requestedPage: number) => {
  ...
  const reloadRowById = async (rowId: string): Promise<OasOperation | null> => {
    ...
    try {
      ...
      return (await response.json()) as OasOperation;
    } catch (err) {
      console.warn('Could not reload OasOperation row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<OasOperation>,
    successField?: string,
  ) => {
    try {
      const saved = await updateOasOperation(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as OasOperation[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update OasOperation';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOasOperationCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `OasOperation-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addOasOperation(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateOasOperation(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as OasOperation[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as OasOperation[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as OasOperation[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as OasOperation[]);
    }
  };
