...
import { ValkyrJob } from '@thorapi/model';
import ValkyrJobForm from '@thorapi/redux/components/form/ValkyrJobForm';
...
} from '@thorapi/redux/services/ValkyrJobService';
...
const buildResourcePath = (rowId: string) => `${BASE_PATH}/ValkyrJob/${rowId}`;
...
const ValkyrJobTable: React.FC = () => {
  ...
  const reloadRowById = async (rowId: string): Promise<ValkyrJob | null> => {
    ...
    try {
      ...
      return (await response.json()) as ValkyrJob;
    } catch (err) {
      console.warn('Could not reload ValkyrJob row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ValkyrJob>,
    successField?: string,
  ) => {
    try {
      const saved = await updateValkyrJob(updatedItem as any).unwrap();
      const persistedId = String((saved as any)?.id ?? rowId);
      const hydrated = (await reloadRowById(persistedId)) ?? (saved as any) ?? updatedItem;
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ValkyrJob[]);
      if (successField) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteValkyrJobCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ValkyrJob-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addValkyrJob(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateValkyrJob(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
