...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ExecModuleExecutionConfig/${rowId}`;
...
const ExecModuleExecutionConfigTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addExecModuleExecutionConfig] =
    useAddExecModuleExecutionConfigMutation();
  const [updateExecModuleExecutionConfig] =
    useUpdateExecModuleExecutionConfigMutation();
  const [deleteExecModuleExecutionConfig] =
    useDeleteExecModuleExecutionConfigMutation();
  const [deleteExecModuleExecutionConfigCascade] =
    useDeleteExecModuleExecutionConfigCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<ExecModuleExecutionConfig | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<ExecModuleExecutionConfig | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload ExecModuleExecutionConfig row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateExecModuleExecutionConfig(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ExecModuleExecutionConfig";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecModuleExecutionConfigCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) ExecModuleExecutionConfig:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter ExecModuleExecutionConfig (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
