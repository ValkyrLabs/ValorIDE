...
import { ExecModuleExecutionConfig } from '@thorapi/model';
import ExecModuleExecutionConfigForm from '@thorapi/redux/components/form/ExecModuleExecutionConfigForm';
...
import {
  useLazyGetExecModuleExecutionConfigsPagedQuery,
...
} from '@thorapi/redux/services/ExecModuleExecutionConfigService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ExecModuleExecutionConfig update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ExecModuleExecutionConfig/${rowId}`;

const ExecModuleExecutionConfigTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ExecModuleExecutionConfig[]>([]);
  ...
  const normalizePageRows = (result: unknown): ExecModuleExecutionConfig[] => {
    if (Array.isArray(result)) return result as ExecModuleExecutionConfig[];
    ...
    return Array.isArray(candidate) ? (candidate as ExecModuleExecutionConfig[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ExecModuleExecutionConfig[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addExecModuleExecutionConfig] = useAddExecModuleExecutionConfigMutation();
  const [updateExecModuleExecutionConfig] = useUpdateExecModuleExecutionConfigMutation();
  const [deleteExecModuleExecutionConfig] = useDeleteExecModuleExecutionConfigMutation();
  const [deleteExecModuleExecutionConfigCascade] = useDeleteExecModuleExecutionConfigCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ExecModuleExecutionConfig>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ExecModuleExecutionConfig | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ExecModuleExecutionConfig | null> => {
    ...
    try {
      ...
      return (await response.json()) as ExecModuleExecutionConfig;
    } catch (err) {
      console.warn('Could not reload ExecModuleExecutionConfig row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ExecModuleExecutionConfig>,
    successField?: string,
  ) => {
    try {
      const saved = await updateExecModuleExecutionConfig(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ExecModuleExecutionConfig[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ExecModuleExecutionConfig';
      console.error('Failed to update ExecModuleExecutionConfig:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExecModuleExecutionConfigCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteExecModuleExecutionConfig(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ExecModuleExecutionConfig> = { ...source };
      delete (payload as any).id;
      const created = await addExecModuleExecutionConfig(payload).unwrap();
      setAllData((prev) => [created as ExecModuleExecutionConfig, ...prev]);
    } catch (error) {
      console.error('Failed to add ExecModuleExecutionConfig:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ExecModuleExecutionConfig> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addExecModuleExecutionConfig(payload).unwrap();
      setAllData((prev) => [created as ExecModuleExecutionConfig, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ExecModuleExecutionConfig:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ExecModuleExecutionConfig-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addExecModuleExecutionConfig(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateExecModuleExecutionConfig(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ExecModuleExecutionConfig[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ExecModuleExecutionConfig[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ExecModuleExecutionConfig[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ExecModuleExecutionConfig[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ExecModuleExecutionConfigTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ExecModuleExecutionConfig Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ExecModuleExecutionConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ExecModuleExecutionConfigForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ExecModuleExecutionConfig (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ExecModuleExecutionConfig</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ExecModuleExecutionConfig;
                ...
                await updateExecModuleExecutionConfig(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ExecModuleExecutionConfigs.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ExecModuleExecutionConfig'}
...
export default ExecModuleExecutionConfigTable;
