...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ApiSpineKpiSnapshot/${rowId}`;
...
const ApiSpineKpiSnapshotTable: React.FC = () => {
  ...
  const [deleteApiSpineKpiSnapshotCascade] =
    useDeleteApiSpineKpiSnapshotCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<ApiSpineKpiSnapshot | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateApiSpineKpiSnapshot(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ApiSpineKpiSnapshot";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteApiSpineKpiSnapshotCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter ApiSpineKpiSnapshot (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
