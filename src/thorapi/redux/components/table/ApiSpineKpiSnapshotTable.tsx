...
import ApiSpineKpiSnapshotForm from '@thorapi/redux/components/form/ApiSpineKpiSnapshotForm';
...
import {
  useLazyGetApiSpineKpiSnapshotsPagedQuery,
...
} from '@thorapi/redux/services/ApiSpineKpiSnapshotService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ApiSpineKpiSnapshot update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ApiSpineKpiSnapshot/${rowId}`;

const ApiSpineKpiSnapshotTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ApiSpineKpiSnapshot[]>([]);
  ...
  const normalizePageRows = (result: unknown): ApiSpineKpiSnapshot[] => {
    if (Array.isArray(result)) return result as ApiSpineKpiSnapshot[];
    ...
    return Array.isArray(candidate) ? (candidate as ApiSpineKpiSnapshot[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ApiSpineKpiSnapshot[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addApiSpineKpiSnapshot] = useAddApiSpineKpiSnapshotMutation();
  const [updateApiSpineKpiSnapshot] = useUpdateApiSpineKpiSnapshotMutation();
  const [deleteApiSpineKpiSnapshot] = useDeleteApiSpineKpiSnapshotMutation();
  const [deleteApiSpineKpiSnapshotCascade] = useDeleteApiSpineKpiSnapshotCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ApiSpineKpiSnapshot>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ApiSpineKpiSnapshot | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ApiSpineKpiSnapshot | null> => {
    ...
    try {
      ...
      return (await response.json()) as ApiSpineKpiSnapshot;
    } catch (err) {
      console.warn('Could not reload ApiSpineKpiSnapshot row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ApiSpineKpiSnapshot>,
    successField?: string,
  ) => {
    try {
      const saved = await updateApiSpineKpiSnapshot(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ApiSpineKpiSnapshot[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ApiSpineKpiSnapshot';
      console.error('Failed to update ApiSpineKpiSnapshot:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteApiSpineKpiSnapshotCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteApiSpineKpiSnapshot(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ApiSpineKpiSnapshot> = { ...source };
      delete (payload as any).id;
      const created = await addApiSpineKpiSnapshot(payload).unwrap();
      setAllData((prev) => [created as ApiSpineKpiSnapshot, ...prev]);
    } catch (error) {
      console.error('Failed to add ApiSpineKpiSnapshot:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ApiSpineKpiSnapshot> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addApiSpineKpiSnapshot(payload).unwrap();
      setAllData((prev) => [created as ApiSpineKpiSnapshot, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ApiSpineKpiSnapshot:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ApiSpineKpiSnapshot-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addApiSpineKpiSnapshot(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateApiSpineKpiSnapshot(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ApiSpineKpiSnapshot[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ApiSpineKpiSnapshot[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ApiSpineKpiSnapshot[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ApiSpineKpiSnapshot[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ApiSpineKpiSnapshotTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ApiSpineKpiSnapshot Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ApiSpineKpiSnapshot</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ApiSpineKpiSnapshotForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ApiSpineKpiSnapshot (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ApiSpineKpiSnapshot</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ApiSpineKpiSnapshot;
                ...
                await updateApiSpineKpiSnapshot(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ApiSpineKpiSnapshots.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ApiSpineKpiSnapshot'}
...
export default ApiSpineKpiSnapshotTable;
