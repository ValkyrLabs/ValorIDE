...
import BuildOutputForm from '@thorapi/redux/components/form/BuildOutputForm';
...
} from '@thorapi/redux/services/BuildOutputService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `BuildOutput update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/BuildOutput/${rowId}`;
...
const BuildOutputTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): BuildOutput[] => {
    if (Array.isArray(result)) return result as BuildOutput[];
    ...
    return Array.isArray(candidate) ? (candidate as BuildOutput[]) : [];
  };

  const mergeFetchedRows = (incomingRows: BuildOutput[], requestedPage: number) => {
  ...
  const [deleteBuildOutputCascade] = useDeleteBuildOutputCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<BuildOutput | null> => {
    ...
    try {
      ...
      return (await response.json()) as BuildOutput;
    } catch (err) {
      console.warn('Could not reload BuildOutput row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<BuildOutput>,
    successField?: string,
  ) => {
    try {
      const saved = await updateBuildOutput(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as BuildOutput[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update BuildOutput';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteBuildOutputCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `BuildOutput-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addBuildOutput(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateBuildOutput(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as BuildOutput[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as BuildOutput[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as BuildOutput[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as BuildOutput[]);
    }
  };
