...
import { JobApplication } from '@thorapi/model';
import JobApplicationForm from '@thorapi/redux/components/form/JobApplicationForm';
...
} from '@thorapi/redux/services/JobApplicationService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'status': { type: 'enum', enumValues: [
    'SUBMITTED', 'REVIEWED', 'INTERVIEW_SCHEDULED', 'REJECTED', 'OFFER_SENT', 'HIRED', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `JobApplication update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/JobApplication/${rowId}`;
...
const JobApplicationTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): JobApplication[] => {
    if (Array.isArray(result)) return result as JobApplication[];
    ...
    return Array.isArray(candidate) ? (candidate as JobApplication[]) : [];
  };

  const mergeFetchedRows = (incomingRows: JobApplication[], requestedPage: number) => {
  ...
  const [deleteJobApplicationCascade] = useDeleteJobApplicationCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<JobApplication | null> => {
    ...
    try {
      ...
      console.warn('Could not reload JobApplication row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<JobApplication>,
    successField?: string,
  ) => {
    try {
      const saved = await updateJobApplication(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as JobApplication[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update JobApplication';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteJobApplicationCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `JobApplication-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addJobApplication(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateJobApplication(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as JobApplication[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as JobApplication[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as JobApplication[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as JobApplication[]);
    }
  };
