...
import { ConsultingProfile } from '@thorapi/model';
import ConsultingProfileForm from '@thorapi/redux/components/form/ConsultingProfileForm';
...
} from '@thorapi/redux/services/ConsultingProfileService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'consultingType': { type: 'enum', enumValues: [
    'INDEPENDENT_CONSULTANT', 'AGENCY_OWNER', 'FREELANCER', 'RESELLER_PARTNER', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ConsultingProfile update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ConsultingProfile/${rowId}`;
...
const ConsultingProfileTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ConsultingProfile[] => {
    if (Array.isArray(result)) return result as ConsultingProfile[];
    ...
    return Array.isArray(candidate) ? (candidate as ConsultingProfile[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ConsultingProfile[], requestedPage: number) => {
  ...
  const [deleteConsultingProfileCascade] = useDeleteConsultingProfileCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ConsultingProfile | null> => {
    ...
    try {
      ...
      return (await response.json()) as ConsultingProfile;
    } catch (err) {
      console.warn('Could not reload ConsultingProfile row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ConsultingProfile>,
    successField?: string,
  ) => {
    try {
      const saved = await updateConsultingProfile(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ConsultingProfile[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ConsultingProfile';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteConsultingProfileCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ConsultingProfile-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addConsultingProfile(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateConsultingProfile(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ConsultingProfile[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ConsultingProfile[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ConsultingProfile[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ConsultingProfile[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ConsultingProfile (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
