...
import { EventLog } from '@thorapi/model';
import EventLogForm from '@thorapi/redux/components/form/EventLogForm';
...
} from '@thorapi/redux/services/EventLogService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `EventLog update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/EventLog/${rowId}`;
...
const EventLogTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): EventLog[] => {
    if (Array.isArray(result)) return result as EventLog[];
    ...
    return Array.isArray(candidate) ? (candidate as EventLog[]) : [];
  };

  const mergeFetchedRows = (incomingRows: EventLog[], requestedPage: number) => {
  ...
  const [deleteEventLogCascade] = useDeleteEventLogCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<EventLog | null> => {
    ...
    try {
      ...
      return (await response.json()) as EventLog;
    } catch (err) {
      console.warn('Could not reload EventLog row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<EventLog>,
    successField?: string,
  ) => {
    try {
      const saved = await updateEventLog(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as EventLog[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update EventLog';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteEventLogCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `EventLog-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addEventLog(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateEventLog(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as EventLog[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as EventLog[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as EventLog[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as EventLog[]);
    }
  };
