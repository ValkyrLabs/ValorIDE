...
import { PresignRequest } from '@thorapi/model';
import PresignRequestForm from '@thorapi/redux/components/form/PresignRequestForm';
...
} from '@thorapi/redux/services/PresignRequestService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'disposition': { type: 'enum', enumValues: [
    'inline', 'attachment', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `PresignRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/PresignRequest/${rowId}`;
...
const PresignRequestTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): PresignRequest[] => {
    if (Array.isArray(result)) return result as PresignRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as PresignRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: PresignRequest[], requestedPage: number) => {
  ...
  const [deletePresignRequestCascade] = useDeletePresignRequestCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<PresignRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as PresignRequest;
    } catch (err) {
      console.warn('Could not reload PresignRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<PresignRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updatePresignRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as PresignRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update PresignRequest';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePresignRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `PresignRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addPresignRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updatePresignRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as PresignRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as PresignRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as PresignRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as PresignRequest[]);
    }
  };
