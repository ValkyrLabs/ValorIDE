...
import { SalesActivity } from '@thorapi/model';
import SalesActivityForm from '@thorapi/redux/components/form/SalesActivityForm';
...
} from '@thorapi/redux/services/SalesActivityService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SalesActivity update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SalesActivity/${rowId}`;
...
const SalesActivityTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SalesActivity[] => {
    if (Array.isArray(result)) return result as SalesActivity[];
    ...
    return Array.isArray(candidate) ? (candidate as SalesActivity[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SalesActivity[], requestedPage: number) => {
  ...
  const [deleteSalesActivityCascade] = useDeleteSalesActivityCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SalesActivity | null> => {
    ...
    try {
      ...
      return (await response.json()) as SalesActivity;
    } catch (err) {
      console.warn('Could not reload SalesActivity row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SalesActivity>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSalesActivity(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SalesActivity[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SalesActivity';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSalesActivityCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SalesActivity-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSalesActivity(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSalesActivity(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SalesActivity[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SalesActivity[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SalesActivity[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SalesActivity[]);
    }
  };
