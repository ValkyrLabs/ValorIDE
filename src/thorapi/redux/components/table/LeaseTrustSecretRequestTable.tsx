...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/LeaseTrustSecretRequest/${rowId}`;
...
const LeaseTrustSecretRequestTable: React.FC = () => {
  ...
  const [updateLeaseTrustSecretRequest] =
    useUpdateLeaseTrustSecretRequestMutation();
  const [deleteLeaseTrustSecretRequest] =
    useDeleteLeaseTrustSecretRequestMutation();
  const [deleteLeaseTrustSecretRequestCascade] =
    useDeleteLeaseTrustSecretRequestCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<LeaseTrustSecretRequest | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<LeaseTrustSecretRequest | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateLeaseTrustSecretRequest(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update LeaseTrustSecretRequest";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteLeaseTrustSecretRequestCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter LeaseTrustSecretRequest (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
