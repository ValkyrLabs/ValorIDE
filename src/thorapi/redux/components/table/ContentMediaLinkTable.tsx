...
import ContentMediaLinkForm from '@thorapi/redux/components/form/ContentMediaLinkForm';
...
} from '@thorapi/redux/services/ContentMediaLinkService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'contentUse': { type: 'enum', enumValues: [
    'none', 'screenshot', 'selfie', 'portrait', 'banner', 'hero', 'header', 'cover', 'promotional', 'informational', 'entertainment', 'product', 'support', 'training', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ContentMediaLink update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ContentMediaLink/${rowId}`;
...
const ContentMediaLinkTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ContentMediaLink[] => {
    if (Array.isArray(result)) return result as ContentMediaLink[];
    ...
    return Array.isArray(candidate) ? (candidate as ContentMediaLink[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ContentMediaLink[], requestedPage: number) => {
  ...
  const [deleteContentMediaLinkCascade] = useDeleteContentMediaLinkCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ContentMediaLink | null> => {
    ...
    try {
      ...
      return (await response.json()) as ContentMediaLink;
    } catch (err) {
      console.warn('Could not reload ContentMediaLink row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ContentMediaLink>,
    successField?: string,
  ) => {
    try {
      const saved = await updateContentMediaLink(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ContentMediaLink[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ContentMediaLink';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteContentMediaLinkCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ContentMediaLink-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addContentMediaLink(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateContentMediaLink(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ContentMediaLink[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ContentMediaLink[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ContentMediaLink[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ContentMediaLink[]);
    }
  };
