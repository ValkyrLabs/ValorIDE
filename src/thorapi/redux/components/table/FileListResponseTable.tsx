...
import { FileListResponse } from '@thorapi/model';
import FileListResponseForm from '@thorapi/redux/components/form/FileListResponseForm';
...
import {
  useLazyGetFileListResponsesPagedQuery,
...
} from '@thorapi/redux/services/FileListResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `FileListResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/FileListResponse/${rowId}`;

const FileListResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<FileListResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): FileListResponse[] => {
    if (Array.isArray(result)) return result as FileListResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as FileListResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: FileListResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addFileListResponse] = useAddFileListResponseMutation();
  const [updateFileListResponse] = useUpdateFileListResponseMutation();
  const [deleteFileListResponse] = useDeleteFileListResponseMutation();
  const [deleteFileListResponseCascade] = useDeleteFileListResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<FileListResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<FileListResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<FileListResponse | null> => {
    ...
    try {
      ...
      console.warn('Could not reload FileListResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<FileListResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateFileListResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as FileListResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update FileListResponse';
      console.error('Failed to update FileListResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteFileListResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteFileListResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<FileListResponse> = { ...source };
      delete (payload as any).id;
      const created = await addFileListResponse(payload).unwrap();
      setAllData((prev) => [created as FileListResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add FileListResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<FileListResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addFileListResponse(payload).unwrap();
      setAllData((prev) => [created as FileListResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) FileListResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `FileListResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addFileListResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateFileListResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as FileListResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as FileListResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as FileListResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as FileListResponse[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.FileListResponse',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="FileListResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="FileListResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create FileListResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <FileListResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter FileListResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing FileListResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof FileListResponse;
                ...
                await updateFileListResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of FileListResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'FileListResponse'}
...
export default FileListResponseTable;
