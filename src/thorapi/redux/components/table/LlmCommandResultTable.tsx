...
import { LlmCommandResult } from '@thorapi/model';
import LlmCommandResultForm from '@thorapi/redux/components/form/LlmCommandResultForm';
...
import {
  useLazyGetLlmCommandResultsPagedQuery,
...
} from '@thorapi/redux/services/LlmCommandResultService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `LlmCommandResult update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/LlmCommandResult/${rowId}`;

const LlmCommandResultTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<LlmCommandResult[]>([]);
  ...
  const normalizePageRows = (result: unknown): LlmCommandResult[] => {
    if (Array.isArray(result)) return result as LlmCommandResult[];
    ...
    return Array.isArray(candidate) ? (candidate as LlmCommandResult[]) : [];
  };

  const mergeFetchedRows = (incomingRows: LlmCommandResult[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addLlmCommandResult] = useAddLlmCommandResultMutation();
  const [updateLlmCommandResult] = useUpdateLlmCommandResultMutation();
  const [deleteLlmCommandResult] = useDeleteLlmCommandResultMutation();
  const [deleteLlmCommandResultCascade] = useDeleteLlmCommandResultCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<LlmCommandResult>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<LlmCommandResult | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<LlmCommandResult | null> => {
    ...
    try {
      ...
      return (await response.json()) as LlmCommandResult;
    } catch (err) {
      console.warn('Could not reload LlmCommandResult row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<LlmCommandResult>,
    successField?: string,
  ) => {
    try {
      const saved = await updateLlmCommandResult(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as LlmCommandResult[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update LlmCommandResult';
      console.error('Failed to update LlmCommandResult:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteLlmCommandResultCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteLlmCommandResult(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<LlmCommandResult> = { ...source };
      delete (payload as any).id;
      const created = await addLlmCommandResult(payload).unwrap();
      setAllData((prev) => [created as LlmCommandResult, ...prev]);
    } catch (error) {
      console.error('Failed to add LlmCommandResult:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<LlmCommandResult> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addLlmCommandResult(payload).unwrap();
      setAllData((prev) => [created as LlmCommandResult, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) LlmCommandResult:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `LlmCommandResult-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addLlmCommandResult(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateLlmCommandResult(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as LlmCommandResult[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as LlmCommandResult[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as LlmCommandResult[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as LlmCommandResult[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.LlmCommandResult',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="LlmCommandResultTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="LlmCommandResult Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create LlmCommandResult</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <LlmCommandResultForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter LlmCommandResult (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing LlmCommandResult</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof LlmCommandResult;
                ...
                await updateLlmCommandResult(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of LlmCommandResults.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'LlmCommandResult'}
...
export default LlmCommandResultTable;
