...
import { OasResponse } from '@thorapi/model';
import OasResponseForm from '@thorapi/redux/components/form/OasResponseForm';
...
} from '@thorapi/redux/services/OasResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `OasResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/OasResponse/${rowId}`;
...
const OasResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): OasResponse[] => {
    if (Array.isArray(result)) return result as OasResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as OasResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: OasResponse[], requestedPage: number) => {
  ...
  const [deleteOasResponseCascade] = useDeleteOasResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<OasResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as OasResponse;
    } catch (err) {
      console.warn('Could not reload OasResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<OasResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateOasResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as OasResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update OasResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOasResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `OasResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addOasResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateOasResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as OasResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as OasResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as OasResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as OasResponse[]);
    }
  };
