...
import { Stack } from '@thorapi/model';
import StackForm from '@thorapi/redux/components/form/StackForm';
...
} from '@thorapi/redux/services/StackService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Stack update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Stack/${rowId}`;
...
const StackTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Stack[] => {
    if (Array.isArray(result)) return result as Stack[];
    ...
    return Array.isArray(candidate) ? (candidate as Stack[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Stack[], requestedPage: number) => {
  ...
  const reloadRowById = async (rowId: string): Promise<Stack | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Stack row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Stack>,
    successField?: string,
  ) => {
    try {
      const saved = await updateStack(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Stack[]);
      if (successField) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteStackCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Stack-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        try {
          ...
          try {
            await updateStack(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Stack[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Stack[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Stack[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Stack[]);
    }
  };
