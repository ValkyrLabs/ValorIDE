...
const columnSchema: Record<string, ColumnSchema> = {
  action: {
    type: "enum",
    enumValues: ["compact", "reindex", "prune", "expand"],
...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/MemoryActionResponse/${rowId}`;
...
const MemoryActionResponseTable: React.FC = () => {
  ...
  const [deleteMemoryActionResponseCascade] =
    useDeleteMemoryActionResponseCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<MemoryActionResponse | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateMemoryActionResponse(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update MemoryActionResponse";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMemoryActionResponseCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter MemoryActionResponse (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
