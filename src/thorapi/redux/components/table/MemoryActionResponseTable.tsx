...
import { MemoryActionResponse } from '@thorapi/model';
import MemoryActionResponseForm from '@thorapi/redux/components/form/MemoryActionResponseForm';
...
import {
  useLazyGetMemoryActionResponsesPagedQuery,
...
} from '@thorapi/redux/services/MemoryActionResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `MemoryActionResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/MemoryActionResponse/${rowId}`;

const MemoryActionResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<MemoryActionResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): MemoryActionResponse[] => {
    if (Array.isArray(result)) return result as MemoryActionResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as MemoryActionResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: MemoryActionResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addMemoryActionResponse] = useAddMemoryActionResponseMutation();
  const [updateMemoryActionResponse] = useUpdateMemoryActionResponseMutation();
  const [deleteMemoryActionResponse] = useDeleteMemoryActionResponseMutation();
  const [deleteMemoryActionResponseCascade] = useDeleteMemoryActionResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<MemoryActionResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<MemoryActionResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<MemoryActionResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as MemoryActionResponse;
    } catch (err) {
      console.warn('Could not reload MemoryActionResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<MemoryActionResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMemoryActionResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as MemoryActionResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update MemoryActionResponse';
      console.error('Failed to update MemoryActionResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMemoryActionResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteMemoryActionResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<MemoryActionResponse> = { ...source };
      delete (payload as any).id;
      const created = await addMemoryActionResponse(payload).unwrap();
      setAllData((prev) => [created as MemoryActionResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add MemoryActionResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<MemoryActionResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMemoryActionResponse(payload).unwrap();
      setAllData((prev) => [created as MemoryActionResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) MemoryActionResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `MemoryActionResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMemoryActionResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMemoryActionResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as MemoryActionResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as MemoryActionResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as MemoryActionResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as MemoryActionResponse[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="MemoryActionResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="MemoryActionResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create MemoryActionResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <MemoryActionResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter MemoryActionResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing MemoryActionResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof MemoryActionResponse;
                ...
                await updateMemoryActionResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of MemoryActionResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'MemoryActionResponse'}
...
export default MemoryActionResponseTable;
