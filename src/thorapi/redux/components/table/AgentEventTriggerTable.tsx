...
import { AgentEventTrigger } from '@thorapi/model';
import AgentEventTriggerForm from '@thorapi/redux/components/form/AgentEventTriggerForm';
...
} from '@thorapi/redux/services/AgentEventTriggerService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AgentEventTrigger update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AgentEventTrigger/${rowId}`;
...
const AgentEventTriggerTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): AgentEventTrigger[] => {
    if (Array.isArray(result)) return result as AgentEventTrigger[];
    ...
    return Array.isArray(candidate) ? (candidate as AgentEventTrigger[]) : [];
  };

  const mergeFetchedRows = (incomingRows: AgentEventTrigger[], requestedPage: number) => {
  ...
  const [deleteAgentEventTriggerCascade] = useDeleteAgentEventTriggerCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<AgentEventTrigger | null> => {
    ...
    try {
      ...
      return (await response.json()) as AgentEventTrigger;
    } catch (err) {
      console.warn('Could not reload AgentEventTrigger row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AgentEventTrigger>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAgentEventTrigger(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AgentEventTrigger[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AgentEventTrigger';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAgentEventTriggerCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AgentEventTrigger-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAgentEventTrigger(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAgentEventTrigger(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AgentEventTrigger[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AgentEventTrigger[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AgentEventTrigger[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AgentEventTrigger[]);
    }
  };
