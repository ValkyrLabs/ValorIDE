...
import { ApplicationDataPlaneBinding } from '@thorapi/model';
import ApplicationDataPlaneBindingForm from '@thorapi/redux/components/form/ApplicationDataPlaneBindingForm';
...
import {
  useLazyGetApplicationDataPlaneBindingsPagedQuery,
...
} from '@thorapi/redux/services/ApplicationDataPlaneBindingService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'isolationMode': { type: 'enum', enumValues: [
    'org_schema', 'app_schema', 'app_database', 'byoc_database', 
  ], enumValueType: 'string' },
  'status': { type: 'enum', enumValues: [
    'requested', 'binding', 'active', 'validating', 'suspended', 'failed', 'retired', 
  ], enumValueType: 'string' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ApplicationDataPlaneBinding update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ApplicationDataPlaneBinding/${rowId}`;

const ApplicationDataPlaneBindingTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ApplicationDataPlaneBinding[]>([]);
  ...
  const normalizePageRows = (result: unknown): ApplicationDataPlaneBinding[] => {
    if (Array.isArray(result)) return result as ApplicationDataPlaneBinding[];
    ...
    return Array.isArray(candidate) ? (candidate as ApplicationDataPlaneBinding[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ApplicationDataPlaneBinding[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addApplicationDataPlaneBinding] = useAddApplicationDataPlaneBindingMutation();
  const [updateApplicationDataPlaneBinding] = useUpdateApplicationDataPlaneBindingMutation();
  const [deleteApplicationDataPlaneBinding] = useDeleteApplicationDataPlaneBindingMutation();
  const [deleteApplicationDataPlaneBindingCascade] = useDeleteApplicationDataPlaneBindingCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ApplicationDataPlaneBinding>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ApplicationDataPlaneBinding | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ApplicationDataPlaneBinding | null> => {
    ...
    try {
      ...
      return (await response.json()) as ApplicationDataPlaneBinding;
    } catch (err) {
      console.warn('Could not reload ApplicationDataPlaneBinding row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ApplicationDataPlaneBinding>,
    successField?: string,
  ) => {
    try {
      const saved = await updateApplicationDataPlaneBinding(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ApplicationDataPlaneBinding[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ApplicationDataPlaneBinding';
      console.error('Failed to update ApplicationDataPlaneBinding:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteApplicationDataPlaneBindingCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteApplicationDataPlaneBinding(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ApplicationDataPlaneBinding> = { ...source };
      delete (payload as any).id;
      const created = await addApplicationDataPlaneBinding(payload).unwrap();
      setAllData((prev) => [created as ApplicationDataPlaneBinding, ...prev]);
    } catch (error) {
      console.error('Failed to add ApplicationDataPlaneBinding:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ApplicationDataPlaneBinding> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addApplicationDataPlaneBinding(payload).unwrap();
      setAllData((prev) => [created as ApplicationDataPlaneBinding, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ApplicationDataPlaneBinding:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ApplicationDataPlaneBinding-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addApplicationDataPlaneBinding(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateApplicationDataPlaneBinding(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ApplicationDataPlaneBinding[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ApplicationDataPlaneBinding[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ApplicationDataPlaneBinding[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ApplicationDataPlaneBinding[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ApplicationDataPlaneBindingTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ApplicationDataPlaneBinding Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ApplicationDataPlaneBinding</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ApplicationDataPlaneBindingForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ApplicationDataPlaneBinding (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ApplicationDataPlaneBinding</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ApplicationDataPlaneBinding;
                ...
                await updateApplicationDataPlaneBinding(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ApplicationDataPlaneBindings.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ApplicationDataPlaneBinding'}
...
export default ApplicationDataPlaneBindingTable;
