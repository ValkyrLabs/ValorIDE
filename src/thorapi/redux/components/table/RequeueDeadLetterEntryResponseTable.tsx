...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/RequeueDeadLetterEntryResponse/${rowId}`;
...
const RequeueDeadLetterEntryResponseTable: React.FC = () => {
  ...
  const [deleteRequeueDeadLetterEntryResponse] =
    useDeleteRequeueDeadLetterEntryResponseMutation();
  const [deleteRequeueDeadLetterEntryResponseCascade] =
    useDeleteRequeueDeadLetterEntryResponseCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<RequeueDeadLetterEntryResponse | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload RequeueDeadLetterEntryResponse row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateRequeueDeadLetterEntryResponse(
        updatedItem as any,
      ).unwrap();
      ...
      setAllData(
        (prev) =>
          mergeUpdatedRow(
            ...
            rowId,
            hydrated as any,
          ) as RequeueDeadLetterEntryResponse[],
      );
      if (successField) {
