...
import RequeueDeadLetterEntryResponseForm from '@thorapi/redux/components/form/RequeueDeadLetterEntryResponseForm';
...
} from '@thorapi/redux/services/RequeueDeadLetterEntryResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RequeueDeadLetterEntryResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RequeueDeadLetterEntryResponse/${rowId}`;
...
const RequeueDeadLetterEntryResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): RequeueDeadLetterEntryResponse[] => {
    if (Array.isArray(result)) return result as RequeueDeadLetterEntryResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as RequeueDeadLetterEntryResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RequeueDeadLetterEntryResponse[], requestedPage: number) => {
  ...
  const [deleteRequeueDeadLetterEntryResponseCascade] = useDeleteRequeueDeadLetterEntryResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<RequeueDeadLetterEntryResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as RequeueDeadLetterEntryResponse;
    } catch (err) {
      console.warn('Could not reload RequeueDeadLetterEntryResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RequeueDeadLetterEntryResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRequeueDeadLetterEntryResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RequeueDeadLetterEntryResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RequeueDeadLetterEntryResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRequeueDeadLetterEntryResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RequeueDeadLetterEntryResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRequeueDeadLetterEntryResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRequeueDeadLetterEntryResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RequeueDeadLetterEntryResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RequeueDeadLetterEntryResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RequeueDeadLetterEntryResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RequeueDeadLetterEntryResponse[]);
    }
  };
