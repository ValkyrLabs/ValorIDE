...
import { RetrievalProvenance } from '@thorapi/model';
import RetrievalProvenanceForm from '@thorapi/redux/components/form/RetrievalProvenanceForm';
...
import {
  useLazyGetRetrievalProvenancesPagedQuery,
...
} from '@thorapi/redux/services/RetrievalProvenanceService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'oldestSourceAt': { type: 'datetime' },
  'newestSourceAt': { type: 'datetime' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RetrievalProvenance update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RetrievalProvenance/${rowId}`;

const RetrievalProvenanceTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RetrievalProvenance[]>([]);
  ...
  const normalizePageRows = (result: unknown): RetrievalProvenance[] => {
    if (Array.isArray(result)) return result as RetrievalProvenance[];
    ...
    return Array.isArray(candidate) ? (candidate as RetrievalProvenance[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RetrievalProvenance[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addRetrievalProvenance] = useAddRetrievalProvenanceMutation();
  const [updateRetrievalProvenance] = useUpdateRetrievalProvenanceMutation();
  const [deleteRetrievalProvenance] = useDeleteRetrievalProvenanceMutation();
  const [deleteRetrievalProvenanceCascade] = useDeleteRetrievalProvenanceCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<RetrievalProvenance>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RetrievalProvenance | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<RetrievalProvenance | null> => {
    ...
    try {
      ...
      return (await response.json()) as RetrievalProvenance;
    } catch (err) {
      console.warn('Could not reload RetrievalProvenance row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RetrievalProvenance>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRetrievalProvenance(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RetrievalProvenance[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RetrievalProvenance';
      console.error('Failed to update RetrievalProvenance:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRetrievalProvenanceCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteRetrievalProvenance(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RetrievalProvenance> = { ...source };
      delete (payload as any).id;
      const created = await addRetrievalProvenance(payload).unwrap();
      setAllData((prev) => [created as RetrievalProvenance, ...prev]);
    } catch (error) {
      console.error('Failed to add RetrievalProvenance:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RetrievalProvenance> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRetrievalProvenance(payload).unwrap();
      setAllData((prev) => [created as RetrievalProvenance, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) RetrievalProvenance:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RetrievalProvenance-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRetrievalProvenance(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRetrievalProvenance(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RetrievalProvenance[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RetrievalProvenance[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RetrievalProvenance[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RetrievalProvenance[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="RetrievalProvenanceTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="RetrievalProvenance Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RetrievalProvenance</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RetrievalProvenanceForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RetrievalProvenance (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RetrievalProvenance</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RetrievalProvenance;
                ...
                await updateRetrievalProvenance(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RetrievalProvenances.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RetrievalProvenance'}
...
export default RetrievalProvenanceTable;
