...
import ApiIdentityKpiSnapshotForm from '@thorapi/redux/components/form/ApiIdentityKpiSnapshotForm';
...
import {
  useLazyGetApiIdentityKpiSnapshotsPagedQuery,
...
} from '@thorapi/redux/services/ApiIdentityKpiSnapshotService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ApiIdentityKpiSnapshot update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ApiIdentityKpiSnapshot/${rowId}`;

const ApiIdentityKpiSnapshotTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ApiIdentityKpiSnapshot[]>([]);
  ...
  const normalizePageRows = (result: unknown): ApiIdentityKpiSnapshot[] => {
    if (Array.isArray(result)) return result as ApiIdentityKpiSnapshot[];
    ...
    return Array.isArray(candidate) ? (candidate as ApiIdentityKpiSnapshot[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ApiIdentityKpiSnapshot[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addApiIdentityKpiSnapshot] = useAddApiIdentityKpiSnapshotMutation();
  const [updateApiIdentityKpiSnapshot] = useUpdateApiIdentityKpiSnapshotMutation();
  const [deleteApiIdentityKpiSnapshot] = useDeleteApiIdentityKpiSnapshotMutation();
  const [deleteApiIdentityKpiSnapshotCascade] = useDeleteApiIdentityKpiSnapshotCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ApiIdentityKpiSnapshot>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ApiIdentityKpiSnapshot | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ApiIdentityKpiSnapshot | null> => {
    ...
    try {
      ...
      return (await response.json()) as ApiIdentityKpiSnapshot;
    } catch (err) {
      console.warn('Could not reload ApiIdentityKpiSnapshot row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ApiIdentityKpiSnapshot>,
    successField?: string,
  ) => {
    try {
      const saved = await updateApiIdentityKpiSnapshot(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ApiIdentityKpiSnapshot[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ApiIdentityKpiSnapshot';
      console.error('Failed to update ApiIdentityKpiSnapshot:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteApiIdentityKpiSnapshotCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteApiIdentityKpiSnapshot(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ApiIdentityKpiSnapshot> = { ...source };
      delete (payload as any).id;
      const created = await addApiIdentityKpiSnapshot(payload).unwrap();
      setAllData((prev) => [created as ApiIdentityKpiSnapshot, ...prev]);
    } catch (error) {
      console.error('Failed to add ApiIdentityKpiSnapshot:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ApiIdentityKpiSnapshot> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addApiIdentityKpiSnapshot(payload).unwrap();
      setAllData((prev) => [created as ApiIdentityKpiSnapshot, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ApiIdentityKpiSnapshot:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ApiIdentityKpiSnapshot-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addApiIdentityKpiSnapshot(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateApiIdentityKpiSnapshot(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ApiIdentityKpiSnapshot[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ApiIdentityKpiSnapshot[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ApiIdentityKpiSnapshot[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ApiIdentityKpiSnapshot[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.ApiIdentityKpiSnapshot',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ApiIdentityKpiSnapshotTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ApiIdentityKpiSnapshot Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ApiIdentityKpiSnapshot</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ApiIdentityKpiSnapshotForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ApiIdentityKpiSnapshot (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ApiIdentityKpiSnapshot</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ApiIdentityKpiSnapshot;
                ...
                await updateApiIdentityKpiSnapshot(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ApiIdentityKpiSnapshots.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ApiIdentityKpiSnapshot'}
...
export default ApiIdentityKpiSnapshotTable;
