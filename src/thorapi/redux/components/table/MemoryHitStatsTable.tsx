...
import MemoryHitStatsForm from '@thorapi/redux/components/form/MemoryHitStatsForm';
...
import {
  useLazyGetMemoryHitStatssPagedQuery,
...
} from '@thorapi/redux/services/MemoryHitStatsService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `MemoryHitStats update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/MemoryHitStats/${rowId}`;

const MemoryHitStatsTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<MemoryHitStats[]>([]);
  ...
  const normalizePageRows = (result: unknown): MemoryHitStats[] => {
    if (Array.isArray(result)) return result as MemoryHitStats[];
    ...
    return Array.isArray(candidate) ? (candidate as MemoryHitStats[]) : [];
  };

  const mergeFetchedRows = (incomingRows: MemoryHitStats[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addMemoryHitStats] = useAddMemoryHitStatsMutation();
  const [updateMemoryHitStats] = useUpdateMemoryHitStatsMutation();
  const [deleteMemoryHitStats] = useDeleteMemoryHitStatsMutation();
  const [deleteMemoryHitStatsCascade] = useDeleteMemoryHitStatsCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<MemoryHitStats>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<MemoryHitStats | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<MemoryHitStats | null> => {
    ...
    try {
      ...
      return (await response.json()) as MemoryHitStats;
    } catch (err) {
      console.warn('Could not reload MemoryHitStats row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<MemoryHitStats>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMemoryHitStats(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as MemoryHitStats[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update MemoryHitStats';
      console.error('Failed to update MemoryHitStats:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMemoryHitStatsCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteMemoryHitStats(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<MemoryHitStats> = { ...source };
      delete (payload as any).id;
      const created = await addMemoryHitStats(payload).unwrap();
      setAllData((prev) => [created as MemoryHitStats, ...prev]);
    } catch (error) {
      console.error('Failed to add MemoryHitStats:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<MemoryHitStats> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMemoryHitStats(payload).unwrap();
      setAllData((prev) => [created as MemoryHitStats, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) MemoryHitStats:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `MemoryHitStats-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMemoryHitStats(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMemoryHitStats(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as MemoryHitStats[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as MemoryHitStats[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as MemoryHitStats[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as MemoryHitStats[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="MemoryHitStatsTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="MemoryHitStats Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create MemoryHitStats</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <MemoryHitStatsForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter MemoryHitStats (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing MemoryHitStats</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof MemoryHitStats;
                ...
                await updateMemoryHitStats(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of MemoryHitStatss.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'MemoryHitStats'}
...
export default MemoryHitStatsTable;
