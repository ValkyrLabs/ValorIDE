...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/MemoryHitStats/${rowId}`;
...
const MemoryHitStatsTable: React.FC = () => {
  ...
  const [deleteMemoryHitStatsCascade] =
    useDeleteMemoryHitStatsCascadeMutation();
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update MemoryHitStats";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMemoryHitStatsCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
