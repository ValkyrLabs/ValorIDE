...
import DownloadAccessForm from '@thorapi/redux/components/form/DownloadAccessForm';
...
} from '@thorapi/redux/services/DownloadAccessService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `DownloadAccess update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/DownloadAccess/${rowId}`;
...
const DownloadAccessTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): DownloadAccess[] => {
    if (Array.isArray(result)) return result as DownloadAccess[];
    ...
    return Array.isArray(candidate) ? (candidate as DownloadAccess[]) : [];
  };

  const mergeFetchedRows = (incomingRows: DownloadAccess[], requestedPage: number) => {
  ...
  const [deleteDownloadAccessCascade] = useDeleteDownloadAccessCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<DownloadAccess | null> => {
    ...
    try {
      ...
      return (await response.json()) as DownloadAccess;
    } catch (err) {
      console.warn('Could not reload DownloadAccess row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<DownloadAccess>,
    successField?: string,
  ) => {
    try {
      const saved = await updateDownloadAccess(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as DownloadAccess[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update DownloadAccess';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteDownloadAccessCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `DownloadAccess-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addDownloadAccess(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateDownloadAccess(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as DownloadAccess[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as DownloadAccess[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as DownloadAccess[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as DownloadAccess[]);
    }
  };
