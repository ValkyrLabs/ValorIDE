...
import { McpTool } from '@thorapi/model';
import McpToolForm from '@thorapi/redux/components/form/McpToolForm';
...
} from '@thorapi/redux/services/McpToolService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'category': { type: 'enum', enumValues: [
    'uncategorized', 'cloud_platform', 'devops', 'security', 'observability', 'data_engineering', 'ai_assistant', 'productivity', 'customization', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `McpTool update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/McpTool/${rowId}`;
...
const McpToolTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): McpTool[] => {
    if (Array.isArray(result)) return result as McpTool[];
    ...
    return Array.isArray(candidate) ? (candidate as McpTool[]) : [];
  };

  const mergeFetchedRows = (incomingRows: McpTool[], requestedPage: number) => {
  ...
  const [deleteMcpToolCascade] = useDeleteMcpToolCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<McpTool | null> => {
    ...
    try {
      ...
      console.warn('Could not reload McpTool row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<McpTool>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMcpTool(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as McpTool[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update McpTool';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpToolCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `McpTool-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMcpTool(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMcpTool(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as McpTool[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as McpTool[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as McpTool[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as McpTool[]);
    }
  };
