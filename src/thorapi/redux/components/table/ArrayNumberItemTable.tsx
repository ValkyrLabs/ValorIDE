...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ArrayNumberItem/${rowId}`;
...
const ArrayNumberItemTable: React.FC = () => {
  ...
  const [deleteArrayNumberItemCascade] =
    useDeleteArrayNumberItemCascadeMutation();
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ArrayNumberItem";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteArrayNumberItemCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
