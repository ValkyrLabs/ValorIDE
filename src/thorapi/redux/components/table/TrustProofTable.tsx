...
import { TrustProof } from '@thorapi/model';
import TrustProofForm from '@thorapi/redux/components/form/TrustProofForm';
...
const columnSchema: Record<string, ColumnSchema> = {
  'proofStatus': { type: 'enum', enumValues: [
    'GENERATED', 'VERIFIED', 'FAILED', 'REVOKED', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TrustProof update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TrustProof/${rowId}`;

const TrustProofTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TrustProof[]>([]);
  ...
  const normalizePageRows = (result: unknown): TrustProof[] => {
    if (Array.isArray(result)) return result as TrustProof[];
    ...
    return Array.isArray(candidate) ? (candidate as TrustProof[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TrustProof[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addTrustProof] = useAddTrustProofMutation();
  const [updateTrustProof] = useUpdateTrustProofMutation();
  const [deleteTrustProof] = useDeleteTrustProofMutation();
  const [deleteTrustProofCascade] = useDeleteTrustProofCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TrustProof>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TrustProof | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TrustProof | null> => {
    ...
    try {
      ...
      return (await response.json()) as TrustProof;
    } catch (err) {
      console.warn('Could not reload TrustProof row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TrustProof>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTrustProof(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TrustProof[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TrustProof';
      console.error('Failed to update TrustProof:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustProofCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTrustProof(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TrustProof> = { ...source };
      delete (payload as any).id;
      const created = await addTrustProof(payload).unwrap();
      setAllData((prev) => [created as TrustProof, ...prev]);
    } catch (error) {
      console.error('Failed to add TrustProof:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TrustProof> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTrustProof(payload).unwrap();
      setAllData((prev) => [created as TrustProof, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TrustProof:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TrustProof-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTrustProof(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTrustProof(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TrustProof[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TrustProof[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TrustProof[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TrustProof[]);
    }
  };
    ...
    <>
          ...
          >
            ...
            <FloatingControlPanel
              description="TrustProof Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TrustProof</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TrustProofForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TrustProof (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TrustProof</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TrustProof;
                ...
                await updateTrustProof(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TrustProofs.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TrustProof'}
...
export default TrustProofTable;
