...
import ValidateDeploymentSpec200ResponseForm from '@thorapi/redux/components/form/ValidateDeploymentSpec200ResponseForm';
...
import {
  useLazyGetValidateDeploymentSpec200ResponsesPagedQuery,
...
} from '@thorapi/redux/services/ValidateDeploymentSpec200ResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ValidateDeploymentSpec200Response update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ValidateDeploymentSpec200Response/${rowId}`;

const ValidateDeploymentSpec200ResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ValidateDeploymentSpec200Response[]>([]);
  ...
  const normalizePageRows = (result: unknown): ValidateDeploymentSpec200Response[] => {
    if (Array.isArray(result)) return result as ValidateDeploymentSpec200Response[];
    ...
    return Array.isArray(candidate) ? (candidate as ValidateDeploymentSpec200Response[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ValidateDeploymentSpec200Response[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addValidateDeploymentSpec200Response] = useAddValidateDeploymentSpec200ResponseMutation();
  const [updateValidateDeploymentSpec200Response] = useUpdateValidateDeploymentSpec200ResponseMutation();
  const [deleteValidateDeploymentSpec200Response] = useDeleteValidateDeploymentSpec200ResponseMutation();
  const [deleteValidateDeploymentSpec200ResponseCascade] = useDeleteValidateDeploymentSpec200ResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ValidateDeploymentSpec200Response>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ValidateDeploymentSpec200Response | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ValidateDeploymentSpec200Response | null> => {
    ...
    try {
      ...
      return (await response.json()) as ValidateDeploymentSpec200Response;
    } catch (err) {
      console.warn('Could not reload ValidateDeploymentSpec200Response row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ValidateDeploymentSpec200Response>,
    successField?: string,
  ) => {
    try {
      const saved = await updateValidateDeploymentSpec200Response(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ValidateDeploymentSpec200Response[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ValidateDeploymentSpec200Response';
      console.error('Failed to update ValidateDeploymentSpec200Response:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteValidateDeploymentSpec200ResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteValidateDeploymentSpec200Response(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ValidateDeploymentSpec200Response> = { ...source };
      delete (payload as any).id;
      const created = await addValidateDeploymentSpec200Response(payload).unwrap();
      setAllData((prev) => [created as ValidateDeploymentSpec200Response, ...prev]);
    } catch (error) {
      console.error('Failed to add ValidateDeploymentSpec200Response:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ValidateDeploymentSpec200Response> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addValidateDeploymentSpec200Response(payload).unwrap();
      setAllData((prev) => [created as ValidateDeploymentSpec200Response, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ValidateDeploymentSpec200Response:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ValidateDeploymentSpec200Response-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addValidateDeploymentSpec200Response(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateValidateDeploymentSpec200Response(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ValidateDeploymentSpec200Response[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ValidateDeploymentSpec200Response[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ValidateDeploymentSpec200Response[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ValidateDeploymentSpec200Response[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.ValidateDeploymentSpec200Response',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ValidateDeploymentSpec200ResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ValidateDeploymentSpec200Response Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ValidateDeploymentSpec200Response</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ValidateDeploymentSpec200ResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ValidateDeploymentSpec200Response (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ValidateDeploymentSpec200Response</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ValidateDeploymentSpec200Response;
                ...
                await updateValidateDeploymentSpec200Response(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ValidateDeploymentSpec200Responses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ValidateDeploymentSpec200Response'}
...
export default ValidateDeploymentSpec200ResponseTable;
