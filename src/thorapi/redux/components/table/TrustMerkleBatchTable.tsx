...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/TrustMerkleBatch/${rowId}`;
...
const TrustMerkleBatchTable: React.FC = () => {
  ...
  const [deleteTrustMerkleBatchCascade] =
    useDeleteTrustMerkleBatchCascadeMutation();
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update TrustMerkleBatch";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustMerkleBatchCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
