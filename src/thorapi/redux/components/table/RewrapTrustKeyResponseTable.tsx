...
import { RewrapTrustKeyResponse } from '@thorapi/model';
import RewrapTrustKeyResponseForm from '@thorapi/redux/components/form/RewrapTrustKeyResponseForm';
...
import {
  useLazyGetRewrapTrustKeyResponsesPagedQuery,
...
} from '@thorapi/redux/services/RewrapTrustKeyResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RewrapTrustKeyResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RewrapTrustKeyResponse/${rowId}`;

const RewrapTrustKeyResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RewrapTrustKeyResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): RewrapTrustKeyResponse[] => {
    if (Array.isArray(result)) return result as RewrapTrustKeyResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as RewrapTrustKeyResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RewrapTrustKeyResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addRewrapTrustKeyResponse] = useAddRewrapTrustKeyResponseMutation();
  const [updateRewrapTrustKeyResponse] = useUpdateRewrapTrustKeyResponseMutation();
  const [deleteRewrapTrustKeyResponse] = useDeleteRewrapTrustKeyResponseMutation();
  const [deleteRewrapTrustKeyResponseCascade] = useDeleteRewrapTrustKeyResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<RewrapTrustKeyResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RewrapTrustKeyResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<RewrapTrustKeyResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as RewrapTrustKeyResponse;
    } catch (err) {
      console.warn('Could not reload RewrapTrustKeyResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RewrapTrustKeyResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRewrapTrustKeyResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RewrapTrustKeyResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RewrapTrustKeyResponse';
      console.error('Failed to update RewrapTrustKeyResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRewrapTrustKeyResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteRewrapTrustKeyResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RewrapTrustKeyResponse> = { ...source };
      delete (payload as any).id;
      const created = await addRewrapTrustKeyResponse(payload).unwrap();
      setAllData((prev) => [created as RewrapTrustKeyResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add RewrapTrustKeyResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RewrapTrustKeyResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRewrapTrustKeyResponse(payload).unwrap();
      setAllData((prev) => [created as RewrapTrustKeyResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) RewrapTrustKeyResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RewrapTrustKeyResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRewrapTrustKeyResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRewrapTrustKeyResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RewrapTrustKeyResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RewrapTrustKeyResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RewrapTrustKeyResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RewrapTrustKeyResponse[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="RewrapTrustKeyResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="RewrapTrustKeyResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RewrapTrustKeyResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RewrapTrustKeyResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RewrapTrustKeyResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RewrapTrustKeyResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RewrapTrustKeyResponse;
                ...
                await updateRewrapTrustKeyResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RewrapTrustKeyResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RewrapTrustKeyResponse'}
...
export default RewrapTrustKeyResponseTable;
