...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/GrantPermissionRequest/${rowId}`;
...
const GrantPermissionRequestTable: React.FC = () => {
  ...
  const [deleteGrantPermissionRequest] =
    useDeleteGrantPermissionRequestMutation();
  const [deleteGrantPermissionRequestCascade] =
    useDeleteGrantPermissionRequestCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<GrantPermissionRequest | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateGrantPermissionRequest(
        updatedItem as any,
      ).unwrap();
