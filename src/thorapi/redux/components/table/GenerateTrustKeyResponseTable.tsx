...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/GenerateTrustKeyResponse/${rowId}`;
...
const GenerateTrustKeyResponseTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addGenerateTrustKeyResponse] =
    useAddGenerateTrustKeyResponseMutation();
  const [updateGenerateTrustKeyResponse] =
    useUpdateGenerateTrustKeyResponseMutation();
  const [deleteGenerateTrustKeyResponse] =
    useDeleteGenerateTrustKeyResponseMutation();
  const [deleteGenerateTrustKeyResponseCascade] =
    useDeleteGenerateTrustKeyResponseCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<GenerateTrustKeyResponse | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<GenerateTrustKeyResponse | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateGenerateTrustKeyResponse(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update GenerateTrustKeyResponse";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteGenerateTrustKeyResponseCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) GenerateTrustKeyResponse:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter GenerateTrustKeyResponse (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
