...
import { Ptg } from '@thorapi/model';
import PtgForm from '@thorapi/redux/components/form/PtgForm';
...
} from '@thorapi/redux/services/PtgService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Ptg update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Ptg/${rowId}`;
...
const PtgTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Ptg[] => {
    if (Array.isArray(result)) return result as Ptg[];
    ...
    return Array.isArray(candidate) ? (candidate as Ptg[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Ptg[], requestedPage: number) => {
  ...
  const [deletePtgCascade] = useDeletePtgCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Ptg | null> => {
    ...
    try {
      ...
      return (await response.json()) as Ptg;
    } catch (err) {
      console.warn('Could not reload Ptg row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Ptg>,
    successField?: string,
  ) => {
    try {
      const saved = await updatePtg(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Ptg[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Ptg';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePtgCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Ptg-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        try {
          ...
          try {
            await updatePtg(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Ptg[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Ptg[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Ptg[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Ptg[]);
    }
  };
