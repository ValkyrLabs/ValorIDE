...
import { Expense } from '@thorapi/model';
import ExpenseForm from '@thorapi/redux/components/form/ExpenseForm';
...
} from '@thorapi/redux/services/ExpenseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Expense update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Expense/${rowId}`;
...
const ExpenseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Expense[] => {
    if (Array.isArray(result)) return result as Expense[];
    ...
    return Array.isArray(candidate) ? (candidate as Expense[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Expense[], requestedPage: number) => {
  ...
  const [deleteExpenseCascade] = useDeleteExpenseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Expense | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Expense row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Expense>,
    successField?: string,
  ) => {
    try {
      const saved = await updateExpense(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Expense[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Expense';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteExpenseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Expense-export-${stamp}.json`;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Expense[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Expense[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Expense[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Expense[]);
    }
  };
