...
const columnSchema: Record<string, ColumnSchema> = {
  outcome: {
    type: "enum",
    enumValues: ["ALLOWED", "DENIED", "FAILED"],
...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/TrustSecretAccessEvent/${rowId}`;
...
const TrustSecretAccessEventTable: React.FC = () => {
  ...
  const [updateTrustSecretAccessEvent] =
    useUpdateTrustSecretAccessEventMutation();
  const [deleteTrustSecretAccessEvent] =
    useDeleteTrustSecretAccessEventMutation();
  const [deleteTrustSecretAccessEventCascade] =
    useDeleteTrustSecretAccessEventCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<TrustSecretAccessEvent | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<TrustSecretAccessEvent | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateTrustSecretAccessEvent(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update TrustSecretAccessEvent";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustSecretAccessEventCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter TrustSecretAccessEvent (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
