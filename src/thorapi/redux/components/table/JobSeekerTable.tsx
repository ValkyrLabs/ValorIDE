...
import JobSeekerForm from '@thorapi/redux/components/form/JobSeekerForm';
...
} from '@thorapi/redux/services/JobSeekerService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `JobSeeker update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/JobSeeker/${rowId}`;
...
const JobSeekerTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): JobSeeker[] => {
    if (Array.isArray(result)) return result as JobSeeker[];
    ...
    return Array.isArray(candidate) ? (candidate as JobSeeker[]) : [];
  };

  const mergeFetchedRows = (incomingRows: JobSeeker[], requestedPage: number) => {
  ...
  const [deleteJobSeekerCascade] = useDeleteJobSeekerCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<JobSeeker | null> => {
    ...
    try {
      ...
      return (await response.json()) as JobSeeker;
    } catch (err) {
      console.warn('Could not reload JobSeeker row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<JobSeeker>,
    successField?: string,
  ) => {
    try {
      const saved = await updateJobSeeker(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as JobSeeker[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update JobSeeker';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteJobSeekerCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `JobSeeker-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addJobSeeker(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateJobSeeker(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as JobSeeker[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as JobSeeker[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as JobSeeker[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as JobSeeker[]);
    }
  };
