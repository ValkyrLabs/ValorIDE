...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/OrderFulfillmentTask/${rowId}`;
...
const OrderFulfillmentTaskTable: React.FC = () => {
  ...
  const [deleteOrderFulfillmentTaskCascade] =
    useDeleteOrderFulfillmentTaskCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<OrderFulfillmentTask | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateOrderFulfillmentTask(
        updatedItem as any,
      ).unwrap();
