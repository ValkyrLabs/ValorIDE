...
import { WorkflowGraphWorkflowData } from '@thorapi/model';
import WorkflowGraphWorkflowDataForm from '@thorapi/redux/components/form/WorkflowGraphWorkflowDataForm';
...
import {
  useLazyGetWorkflowGraphWorkflowDatasPagedQuery,
...
} from '@thorapi/redux/services/WorkflowGraphWorkflowDataService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'status': { type: 'enum', enumValues: [
    'ready', 'running', 'paused', 'stopped', 'warning', 'error', 'disabled', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `WorkflowGraphWorkflowData update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/WorkflowGraphWorkflowData/${rowId}`;

const WorkflowGraphWorkflowDataTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<WorkflowGraphWorkflowData[]>([]);
  ...
  const normalizePageRows = (result: unknown): WorkflowGraphWorkflowData[] => {
    if (Array.isArray(result)) return result as WorkflowGraphWorkflowData[];
    ...
    return Array.isArray(candidate) ? (candidate as WorkflowGraphWorkflowData[]) : [];
  };

  const mergeFetchedRows = (incomingRows: WorkflowGraphWorkflowData[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addWorkflowGraphWorkflowData] = useAddWorkflowGraphWorkflowDataMutation();
  const [updateWorkflowGraphWorkflowData] = useUpdateWorkflowGraphWorkflowDataMutation();
  const [deleteWorkflowGraphWorkflowData] = useDeleteWorkflowGraphWorkflowDataMutation();
  const [deleteWorkflowGraphWorkflowDataCascade] = useDeleteWorkflowGraphWorkflowDataCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<WorkflowGraphWorkflowData>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<WorkflowGraphWorkflowData | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<WorkflowGraphWorkflowData | null> => {
    ...
    try {
      ...
      return (await response.json()) as WorkflowGraphWorkflowData;
    } catch (err) {
      console.warn('Could not reload WorkflowGraphWorkflowData row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<WorkflowGraphWorkflowData>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWorkflowGraphWorkflowData(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as WorkflowGraphWorkflowData[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update WorkflowGraphWorkflowData';
      console.error('Failed to update WorkflowGraphWorkflowData:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphWorkflowDataCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteWorkflowGraphWorkflowData(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<WorkflowGraphWorkflowData> = { ...source };
      delete (payload as any).id;
      const created = await addWorkflowGraphWorkflowData(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphWorkflowData, ...prev]);
    } catch (error) {
      console.error('Failed to add WorkflowGraphWorkflowData:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<WorkflowGraphWorkflowData> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addWorkflowGraphWorkflowData(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphWorkflowData, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) WorkflowGraphWorkflowData:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `WorkflowGraphWorkflowData-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWorkflowGraphWorkflowData(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWorkflowGraphWorkflowData(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as WorkflowGraphWorkflowData[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as WorkflowGraphWorkflowData[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as WorkflowGraphWorkflowData[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as WorkflowGraphWorkflowData[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.WorkflowGraphWorkflowData',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="WorkflowGraphWorkflowDataTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="WorkflowGraphWorkflowData Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create WorkflowGraphWorkflowData</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <WorkflowGraphWorkflowDataForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter WorkflowGraphWorkflowData (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing WorkflowGraphWorkflowData</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof WorkflowGraphWorkflowData;
                ...
                await updateWorkflowGraphWorkflowData(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of WorkflowGraphWorkflowDatas.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'WorkflowGraphWorkflowData'}
...
export default WorkflowGraphWorkflowDataTable;
