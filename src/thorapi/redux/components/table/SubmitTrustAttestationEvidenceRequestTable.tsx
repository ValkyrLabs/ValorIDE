...
import { SubmitTrustAttestationEvidenceRequest } from '@thorapi/model';
import SubmitTrustAttestationEvidenceRequestForm from '@thorapi/redux/components/form/SubmitTrustAttestationEvidenceRequestForm';
...
import {
  useLazyGetSubmitTrustAttestationEvidenceRequestsPagedQuery,
...
} from '@thorapi/redux/services/SubmitTrustAttestationEvidenceRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SubmitTrustAttestationEvidenceRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SubmitTrustAttestationEvidenceRequest/${rowId}`;

const SubmitTrustAttestationEvidenceRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<SubmitTrustAttestationEvidenceRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): SubmitTrustAttestationEvidenceRequest[] => {
    if (Array.isArray(result)) return result as SubmitTrustAttestationEvidenceRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as SubmitTrustAttestationEvidenceRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SubmitTrustAttestationEvidenceRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addSubmitTrustAttestationEvidenceRequest] = useAddSubmitTrustAttestationEvidenceRequestMutation();
  const [updateSubmitTrustAttestationEvidenceRequest] = useUpdateSubmitTrustAttestationEvidenceRequestMutation();
  const [deleteSubmitTrustAttestationEvidenceRequest] = useDeleteSubmitTrustAttestationEvidenceRequestMutation();
  const [deleteSubmitTrustAttestationEvidenceRequestCascade] = useDeleteSubmitTrustAttestationEvidenceRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<SubmitTrustAttestationEvidenceRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SubmitTrustAttestationEvidenceRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<SubmitTrustAttestationEvidenceRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as SubmitTrustAttestationEvidenceRequest;
    } catch (err) {
      console.warn('Could not reload SubmitTrustAttestationEvidenceRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SubmitTrustAttestationEvidenceRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSubmitTrustAttestationEvidenceRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SubmitTrustAttestationEvidenceRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SubmitTrustAttestationEvidenceRequest';
      console.error('Failed to update SubmitTrustAttestationEvidenceRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSubmitTrustAttestationEvidenceRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteSubmitTrustAttestationEvidenceRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SubmitTrustAttestationEvidenceRequest> = { ...source };
      delete (payload as any).id;
      const created = await addSubmitTrustAttestationEvidenceRequest(payload).unwrap();
      setAllData((prev) => [created as SubmitTrustAttestationEvidenceRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add SubmitTrustAttestationEvidenceRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<SubmitTrustAttestationEvidenceRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addSubmitTrustAttestationEvidenceRequest(payload).unwrap();
      setAllData((prev) => [created as SubmitTrustAttestationEvidenceRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) SubmitTrustAttestationEvidenceRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SubmitTrustAttestationEvidenceRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSubmitTrustAttestationEvidenceRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSubmitTrustAttestationEvidenceRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SubmitTrustAttestationEvidenceRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SubmitTrustAttestationEvidenceRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SubmitTrustAttestationEvidenceRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SubmitTrustAttestationEvidenceRequest[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.SubmitTrustAttestationEvidenceRequest',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="SubmitTrustAttestationEvidenceRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="SubmitTrustAttestationEvidenceRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create SubmitTrustAttestationEvidenceRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <SubmitTrustAttestationEvidenceRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter SubmitTrustAttestationEvidenceRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing SubmitTrustAttestationEvidenceRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof SubmitTrustAttestationEvidenceRequest;
                ...
                await updateSubmitTrustAttestationEvidenceRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of SubmitTrustAttestationEvidenceRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'SubmitTrustAttestationEvidenceRequest'}
...
export default SubmitTrustAttestationEvidenceRequestTable;
