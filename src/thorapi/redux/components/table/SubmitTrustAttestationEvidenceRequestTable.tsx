...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/SubmitTrustAttestationEvidenceRequest/${rowId}`;
...
const SubmitTrustAttestationEvidenceRequestTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addSubmitTrustAttestationEvidenceRequest] =
    useAddSubmitTrustAttestationEvidenceRequestMutation();
  const [updateSubmitTrustAttestationEvidenceRequest] =
    useUpdateSubmitTrustAttestationEvidenceRequestMutation();
  const [deleteSubmitTrustAttestationEvidenceRequest] =
    useDeleteSubmitTrustAttestationEvidenceRequestMutation();
  const [deleteSubmitTrustAttestationEvidenceRequestCascade] =
    useDeleteSubmitTrustAttestationEvidenceRequestCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] =
    useState<SubmitTrustAttestationEvidenceRequest | null>(null);
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<SubmitTrustAttestationEvidenceRequest | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload SubmitTrustAttestationEvidenceRequest row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateSubmitTrustAttestationEvidenceRequest(
        updatedItem as any,
      ).unwrap();
      ...
      setAllData(
        (prev) =>
          mergeUpdatedRow(
            ...
            rowId,
            hydrated as any,
          ) as SubmitTrustAttestationEvidenceRequest[],
      );
      if (successField) {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update SubmitTrustAttestationEvidenceRequest";
      console.error(
        "Failed to update SubmitTrustAttestationEvidenceRequest:",
        error,
      );
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSubmitTrustAttestationEvidenceRequestCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
            ...
            try {
              await deleteSubmitTrustAttestationEvidenceRequest(
                id as any,
              ).unwrap();
            } catch (inner) {
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<SubmitTrustAttestationEvidenceRequest> = {
        ...source,
      };
      ...
      setAllData((prev) => [
        created as SubmitTrustAttestationEvidenceRequest,
        ...prev,
      ]);
    } catch (error) {
      console.error(
        "Failed to add SubmitTrustAttestationEvidenceRequest:",
        error,
      );
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) SubmitTrustAttestationEvidenceRequest:",
        error,
      );
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType:
          "com.valkyrlabs.model.SubmitTrustAttestationEvidenceRequest",
        objectId: rowId,
      });
    ...
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Create SubmitTrustAttestationEvidenceRequest
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter SubmitTrustAttestationEvidenceRequest (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>
            Editing SubmitTrustAttestationEvidenceRequest
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
      ...
      <MarkdownEditorModal
        ...
        onSave={async (updatedText) => {
          try {
            ...
            const field =
              richModal.field as keyof SubmitTrustAttestationEvidenceRequest;
            if (!rid || !field) {
