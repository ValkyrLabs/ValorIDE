...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/WorkflowGraphEdge/${rowId}`;
...
const WorkflowGraphEdgeTable: React.FC = () => {
  ...
  const [deleteWorkflowGraphEdgeCascade] =
    useDeleteWorkflowGraphEdgeCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<WorkflowGraphEdge | null> => {
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update WorkflowGraphEdge";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphEdgeCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
