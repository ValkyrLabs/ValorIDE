...
import { Mcp } from '@thorapi/model';
import McpForm from '@thorapi/redux/components/form/McpForm';
...
} from '@thorapi/redux/services/McpService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Mcp update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Mcp/${rowId}`;
...
const McpTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Mcp[] => {
    if (Array.isArray(result)) return result as Mcp[];
    ...
    return Array.isArray(candidate) ? (candidate as Mcp[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Mcp[], requestedPage: number) => {
  ...
  const [deleteMcpCascade] = useDeleteMcpCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Mcp | null> => {
    ...
    try {
      ...
      return (await response.json()) as Mcp;
    } catch (err) {
      console.warn('Could not reload Mcp row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Mcp>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMcp(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Mcp[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update Mcp';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Mcp-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        try {
          ...
          try {
            await updateMcp(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Mcp[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Mcp[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Mcp[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Mcp[]);
    }
  };
