...
import { LineItem } from '@thorapi/model';
import LineItemForm from '@thorapi/redux/components/form/LineItemForm';
...
import {
  ...
  useDeleteLineItemCascadeMutation
} from '@thorapi/redux/services/LineItemService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `LineItem update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/LineItem/${rowId}`;
...
const LineItemTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): LineItem[] => {
    if (Array.isArray(result)) return result as LineItem[];
    ...
    return Array.isArray(candidate) ? (candidate as LineItem[]) : [];
  };

  const mergeFetchedRows = (incomingRows: LineItem[], requestedPage: number) => {
  ...
  const [deleteLineItemCascade] = useDeleteLineItemCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<LineItem | null> => {
    ...
    try {
      ...
      console.warn('Could not reload LineItem row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<LineItem>,
    successField?: string,
  ) => {
    try {
      const saved = await updateLineItem(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as LineItem[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update LineItem';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteLineItemCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `LineItem-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addLineItem(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateLineItem(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as LineItem[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as LineItem[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as LineItem[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as LineItem[]);
    }
  };
