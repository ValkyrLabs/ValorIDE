...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/JobApplicationRequest/${rowId}`;
...
const JobApplicationRequestTable: React.FC = () => {
  ...
  const [deleteJobApplicationRequest] =
    useDeleteJobApplicationRequestMutation();
  const [deleteJobApplicationRequestCascade] =
    useDeleteJobApplicationRequestCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<JobApplicationRequest | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateJobApplicationRequest(
        updatedItem as any,
      ).unwrap();
