...
import { JobApplicationRequest } from '@thorapi/model';
import JobApplicationRequestForm from '@thorapi/redux/components/form/JobApplicationRequestForm';
...
} from '@thorapi/redux/services/JobApplicationRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `JobApplicationRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/JobApplicationRequest/${rowId}`;
...
const JobApplicationRequestTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): JobApplicationRequest[] => {
    if (Array.isArray(result)) return result as JobApplicationRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as JobApplicationRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: JobApplicationRequest[], requestedPage: number) => {
  ...
  const [deleteJobApplicationRequestCascade] = useDeleteJobApplicationRequestCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<JobApplicationRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<JobApplicationRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as JobApplicationRequest;
    } catch (err) {
      console.warn('Could not reload JobApplicationRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<JobApplicationRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateJobApplicationRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as JobApplicationRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update JobApplicationRequest';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteJobApplicationRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `JobApplicationRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addJobApplicationRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateJobApplicationRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as JobApplicationRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as JobApplicationRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as JobApplicationRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as JobApplicationRequest[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter JobApplicationRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
