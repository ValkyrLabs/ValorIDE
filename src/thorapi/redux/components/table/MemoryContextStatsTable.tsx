...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/MemoryContextStats/${rowId}`;
...
const MemoryContextStatsTable: React.FC = () => {
  ...
  const [deleteMemoryContextStatsCascade] =
    useDeleteMemoryContextStatsCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<MemoryContextStats | null> => {
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update MemoryContextStats";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMemoryContextStatsCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter MemoryContextStats (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
