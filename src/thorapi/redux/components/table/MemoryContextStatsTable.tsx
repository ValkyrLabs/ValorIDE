...
import MemoryContextStatsForm from '@thorapi/redux/components/form/MemoryContextStatsForm';
...
import {
  useLazyGetMemoryContextStatssPagedQuery,
...
} from '@thorapi/redux/services/MemoryContextStatsService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `MemoryContextStats update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/MemoryContextStats/${rowId}`;

const MemoryContextStatsTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<MemoryContextStats[]>([]);
  ...
  const normalizePageRows = (result: unknown): MemoryContextStats[] => {
    if (Array.isArray(result)) return result as MemoryContextStats[];
    ...
    return Array.isArray(candidate) ? (candidate as MemoryContextStats[]) : [];
  };

  const mergeFetchedRows = (incomingRows: MemoryContextStats[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addMemoryContextStats] = useAddMemoryContextStatsMutation();
  const [updateMemoryContextStats] = useUpdateMemoryContextStatsMutation();
  const [deleteMemoryContextStats] = useDeleteMemoryContextStatsMutation();
  const [deleteMemoryContextStatsCascade] = useDeleteMemoryContextStatsCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<MemoryContextStats>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<MemoryContextStats | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<MemoryContextStats | null> => {
    ...
    try {
      ...
      return (await response.json()) as MemoryContextStats;
    } catch (err) {
      console.warn('Could not reload MemoryContextStats row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<MemoryContextStats>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMemoryContextStats(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as MemoryContextStats[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update MemoryContextStats';
      console.error('Failed to update MemoryContextStats:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMemoryContextStatsCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteMemoryContextStats(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<MemoryContextStats> = { ...source };
      delete (payload as any).id;
      const created = await addMemoryContextStats(payload).unwrap();
      setAllData((prev) => [created as MemoryContextStats, ...prev]);
    } catch (error) {
      console.error('Failed to add MemoryContextStats:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<MemoryContextStats> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMemoryContextStats(payload).unwrap();
      setAllData((prev) => [created as MemoryContextStats, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) MemoryContextStats:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `MemoryContextStats-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMemoryContextStats(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMemoryContextStats(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as MemoryContextStats[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as MemoryContextStats[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as MemoryContextStats[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as MemoryContextStats[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="MemoryContextStatsTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="MemoryContextStats Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create MemoryContextStats</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <MemoryContextStatsForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter MemoryContextStats (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing MemoryContextStats</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof MemoryContextStats;
                ...
                await updateMemoryContextStats(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of MemoryContextStatss.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'MemoryContextStats'}
...
export default MemoryContextStatsTable;
