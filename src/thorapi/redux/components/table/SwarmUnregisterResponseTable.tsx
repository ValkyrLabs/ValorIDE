...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/SwarmUnregisterResponse/${rowId}`;
...
const SwarmUnregisterResponseTable: React.FC = () => {
  ...
  const [deleteSwarmUnregisterResponse] =
    useDeleteSwarmUnregisterResponseMutation();
  const [deleteSwarmUnregisterResponseCascade] =
    useDeleteSwarmUnregisterResponseCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<SwarmUnregisterResponse | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateSwarmUnregisterResponse(
        updatedItem as any,
      ).unwrap();
