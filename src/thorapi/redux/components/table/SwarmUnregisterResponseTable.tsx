...
import { SwarmUnregisterResponse } from '@thorapi/model';
import SwarmUnregisterResponseForm from '@thorapi/redux/components/form/SwarmUnregisterResponseForm';
...
} from '@thorapi/redux/services/SwarmUnregisterResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SwarmUnregisterResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SwarmUnregisterResponse/${rowId}`;
...
const SwarmUnregisterResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SwarmUnregisterResponse[] => {
    if (Array.isArray(result)) return result as SwarmUnregisterResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as SwarmUnregisterResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SwarmUnregisterResponse[], requestedPage: number) => {
  ...
  const [deleteSwarmUnregisterResponseCascade] = useDeleteSwarmUnregisterResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SwarmUnregisterResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as SwarmUnregisterResponse;
    } catch (err) {
      console.warn('Could not reload SwarmUnregisterResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SwarmUnregisterResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSwarmUnregisterResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SwarmUnregisterResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SwarmUnregisterResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmUnregisterResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SwarmUnregisterResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSwarmUnregisterResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSwarmUnregisterResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SwarmUnregisterResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SwarmUnregisterResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SwarmUnregisterResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SwarmUnregisterResponse[]);
    }
  };
