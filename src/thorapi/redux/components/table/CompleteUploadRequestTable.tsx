...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/CompleteUploadRequest/${rowId}`;
...
const CompleteUploadRequestTable: React.FC = () => {
  ...
  const [deleteCompleteUploadRequest] =
    useDeleteCompleteUploadRequestMutation();
  const [deleteCompleteUploadRequestCascade] =
    useDeleteCompleteUploadRequestCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<CompleteUploadRequest | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateCompleteUploadRequest(
        updatedItem as any,
      ).unwrap();
