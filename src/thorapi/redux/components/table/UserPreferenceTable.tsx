...
import { UserPreference } from '@thorapi/model';
import UserPreferenceForm from '@thorapi/redux/components/form/UserPreferenceForm';
...
const columnSchema: Record<string, ColumnSchema> = {
  'preferenceType': { type: 'enum', enumValues: [
    'ux-layout', 'ux-mode', 'persona-mode', 'ux-theme', 'measurement', 'chatmemory', 
  ], enumValueType: 'string' },
  'personaMode': { type: 'enum', enumValues: [
    'cxo', 'solopreneur', 'engineer', 'student', 'reseller', 'content_publisher', 'workflow_designer', 'graymatter_user', 
  ], enumValueType: 'string' },
  'personaComplexity': { type: 'enum', enumValues: [
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `UserPreference update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/UserPreference/${rowId}`;
...
const UserPreferenceTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): UserPreference[] => {
    if (Array.isArray(result)) return result as UserPreference[];
    ...
    return Array.isArray(candidate) ? (candidate as UserPreference[]) : [];
  };

  const mergeFetchedRows = (incomingRows: UserPreference[], requestedPage: number) => {
  ...
  const [deleteUserPreferenceCascade] = useDeleteUserPreferenceCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<UserPreference | null> => {
    ...
    try {
      ...
      return (await response.json()) as UserPreference;
    } catch (err) {
      console.warn('Could not reload UserPreference row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<UserPreference>,
    successField?: string,
  ) => {
    try {
      const saved = await updateUserPreference(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as UserPreference[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update UserPreference';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteUserPreferenceCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `UserPreference-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addUserPreference(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as UserPreference[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as UserPreference[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as UserPreference[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as UserPreference[]);
    }
  };
