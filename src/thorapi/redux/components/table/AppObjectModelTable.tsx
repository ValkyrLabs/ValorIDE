...
import { AppObjectModel } from '@thorapi/model';
import AppObjectModelForm from '@thorapi/redux/components/form/AppObjectModelForm';
...
import {
  useLazyGetAppObjectModelsPagedQuery,
...
} from '@thorapi/redux/services/AppObjectModelService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'status': { type: 'enum', enumValues: [
    'draft', 'generated', 'accepted', 'failed', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AppObjectModel update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AppObjectModel/${rowId}`;

const AppObjectModelTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<AppObjectModel[]>([]);
  ...
  const normalizePageRows = (result: unknown): AppObjectModel[] => {
    if (Array.isArray(result)) return result as AppObjectModel[];
    ...
    return Array.isArray(candidate) ? (candidate as AppObjectModel[]) : [];
  };

  const mergeFetchedRows = (incomingRows: AppObjectModel[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addAppObjectModel] = useAddAppObjectModelMutation();
  const [updateAppObjectModel] = useUpdateAppObjectModelMutation();
  const [deleteAppObjectModel] = useDeleteAppObjectModelMutation();
  const [deleteAppObjectModelCascade] = useDeleteAppObjectModelCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<AppObjectModel>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<AppObjectModel | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<AppObjectModel | null> => {
    ...
    try {
      ...
      return (await response.json()) as AppObjectModel;
    } catch (err) {
      console.warn('Could not reload AppObjectModel row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AppObjectModel>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAppObjectModel(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AppObjectModel[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AppObjectModel';
      console.error('Failed to update AppObjectModel:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAppObjectModelCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteAppObjectModel(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<AppObjectModel> = { ...source };
      delete (payload as any).id;
      const created = await addAppObjectModel(payload).unwrap();
      setAllData((prev) => [created as AppObjectModel, ...prev]);
    } catch (error) {
      console.error('Failed to add AppObjectModel:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<AppObjectModel> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addAppObjectModel(payload).unwrap();
      setAllData((prev) => [created as AppObjectModel, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) AppObjectModel:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AppObjectModel-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAppObjectModel(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAppObjectModel(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AppObjectModel[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AppObjectModel[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AppObjectModel[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AppObjectModel[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.AppObjectModel',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="AppObjectModelTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="AppObjectModel Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create AppObjectModel</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <AppObjectModelForm />
            </Modal.Body>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter AppObjectModel (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing AppObjectModel</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof AppObjectModel;
                ...
                await updateAppObjectModel(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of AppObjectModels.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'AppObjectModel'}
...
export default AppObjectModelTable;
