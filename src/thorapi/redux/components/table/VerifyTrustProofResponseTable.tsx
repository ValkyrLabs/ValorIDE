...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/VerifyTrustProofResponse/${rowId}`;
...
const VerifyTrustProofResponseTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addVerifyTrustProofResponse] =
    useAddVerifyTrustProofResponseMutation();
  const [updateVerifyTrustProofResponse] =
    useUpdateVerifyTrustProofResponseMutation();
  const [deleteVerifyTrustProofResponse] =
    useDeleteVerifyTrustProofResponseMutation();
  const [deleteVerifyTrustProofResponseCascade] =
    useDeleteVerifyTrustProofResponseCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<VerifyTrustProofResponse | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<VerifyTrustProofResponse | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateVerifyTrustProofResponse(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update VerifyTrustProofResponse";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteVerifyTrustProofResponseCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) VerifyTrustProofResponse:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter VerifyTrustProofResponse (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
