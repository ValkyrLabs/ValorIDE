...
import { VerifyTrustProofResponse } from '@thorapi/model';
import VerifyTrustProofResponseForm from '@thorapi/redux/components/form/VerifyTrustProofResponseForm';
...
import {
  useLazyGetVerifyTrustProofResponsesPagedQuery,
...
} from '@thorapi/redux/services/VerifyTrustProofResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `VerifyTrustProofResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/VerifyTrustProofResponse/${rowId}`;

const VerifyTrustProofResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<VerifyTrustProofResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): VerifyTrustProofResponse[] => {
    if (Array.isArray(result)) return result as VerifyTrustProofResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as VerifyTrustProofResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: VerifyTrustProofResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addVerifyTrustProofResponse] = useAddVerifyTrustProofResponseMutation();
  const [updateVerifyTrustProofResponse] = useUpdateVerifyTrustProofResponseMutation();
  const [deleteVerifyTrustProofResponse] = useDeleteVerifyTrustProofResponseMutation();
  const [deleteVerifyTrustProofResponseCascade] = useDeleteVerifyTrustProofResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<VerifyTrustProofResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<VerifyTrustProofResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<VerifyTrustProofResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as VerifyTrustProofResponse;
    } catch (err) {
      console.warn('Could not reload VerifyTrustProofResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<VerifyTrustProofResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateVerifyTrustProofResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as VerifyTrustProofResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update VerifyTrustProofResponse';
      console.error('Failed to update VerifyTrustProofResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteVerifyTrustProofResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteVerifyTrustProofResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<VerifyTrustProofResponse> = { ...source };
      delete (payload as any).id;
      const created = await addVerifyTrustProofResponse(payload).unwrap();
      setAllData((prev) => [created as VerifyTrustProofResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add VerifyTrustProofResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<VerifyTrustProofResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addVerifyTrustProofResponse(payload).unwrap();
      setAllData((prev) => [created as VerifyTrustProofResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) VerifyTrustProofResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `VerifyTrustProofResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addVerifyTrustProofResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateVerifyTrustProofResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as VerifyTrustProofResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as VerifyTrustProofResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as VerifyTrustProofResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as VerifyTrustProofResponse[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.VerifyTrustProofResponse',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="VerifyTrustProofResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="VerifyTrustProofResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create VerifyTrustProofResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <VerifyTrustProofResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter VerifyTrustProofResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing VerifyTrustProofResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof VerifyTrustProofResponse;
                ...
                await updateVerifyTrustProofResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of VerifyTrustProofResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'VerifyTrustProofResponse'}
...
export default VerifyTrustProofResponseTable;
