...
import { Goal } from '@thorapi/model';
import GoalForm from '@thorapi/redux/components/form/GoalForm';
...
} from '@thorapi/redux/services/GoalService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `Goal update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/Goal/${rowId}`;
...
const GoalTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): Goal[] => {
    if (Array.isArray(result)) return result as Goal[];
    ...
    return Array.isArray(candidate) ? (candidate as Goal[]) : [];
  };

  const mergeFetchedRows = (incomingRows: Goal[], requestedPage: number) => {
  ...
  const [deleteGoalCascade] = useDeleteGoalCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<Goal | null> => {
    ...
    try {
      ...
      console.warn('Could not reload Goal row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<Goal>,
    successField?: string,
  ) => {
    try {
      const saved = await updateGoal(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as Goal[]);
      if (successField) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteGoalCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Goal-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        try {
          ...
          try {
            await updateGoal(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as Goal[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as Goal[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as Goal[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as Goal[]);
    }
  };
