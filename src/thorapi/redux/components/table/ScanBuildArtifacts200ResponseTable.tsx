...
import ScanBuildArtifacts200ResponseForm from '@thorapi/redux/components/form/ScanBuildArtifacts200ResponseForm';
...
import {
  useLazyGetScanBuildArtifacts200ResponsesPagedQuery,
...
} from '@thorapi/redux/services/ScanBuildArtifacts200ResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ScanBuildArtifacts200Response update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ScanBuildArtifacts200Response/${rowId}`;

const ScanBuildArtifacts200ResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<ScanBuildArtifacts200Response[]>([]);
  ...
  const normalizePageRows = (result: unknown): ScanBuildArtifacts200Response[] => {
    if (Array.isArray(result)) return result as ScanBuildArtifacts200Response[];
    ...
    return Array.isArray(candidate) ? (candidate as ScanBuildArtifacts200Response[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ScanBuildArtifacts200Response[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addScanBuildArtifacts200Response] = useAddScanBuildArtifacts200ResponseMutation();
  const [updateScanBuildArtifacts200Response] = useUpdateScanBuildArtifacts200ResponseMutation();
  const [deleteScanBuildArtifacts200Response] = useDeleteScanBuildArtifacts200ResponseMutation();
  const [deleteScanBuildArtifacts200ResponseCascade] = useDeleteScanBuildArtifacts200ResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ScanBuildArtifacts200Response>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ScanBuildArtifacts200Response | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<ScanBuildArtifacts200Response | null> => {
    ...
    try {
      ...
      return (await response.json()) as ScanBuildArtifacts200Response;
    } catch (err) {
      console.warn('Could not reload ScanBuildArtifacts200Response row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ScanBuildArtifacts200Response>,
    successField?: string,
  ) => {
    try {
      const saved = await updateScanBuildArtifacts200Response(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ScanBuildArtifacts200Response[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ScanBuildArtifacts200Response';
      console.error('Failed to update ScanBuildArtifacts200Response:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteScanBuildArtifacts200ResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteScanBuildArtifacts200Response(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<ScanBuildArtifacts200Response> = { ...source };
      delete (payload as any).id;
      const created = await addScanBuildArtifacts200Response(payload).unwrap();
      setAllData((prev) => [created as ScanBuildArtifacts200Response, ...prev]);
    } catch (error) {
      console.error('Failed to add ScanBuildArtifacts200Response:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<ScanBuildArtifacts200Response> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addScanBuildArtifacts200Response(payload).unwrap();
      setAllData((prev) => [created as ScanBuildArtifacts200Response, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) ScanBuildArtifacts200Response:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ScanBuildArtifacts200Response-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addScanBuildArtifacts200Response(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateScanBuildArtifacts200Response(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ScanBuildArtifacts200Response[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ScanBuildArtifacts200Response[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ScanBuildArtifacts200Response[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ScanBuildArtifacts200Response[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="ScanBuildArtifacts200ResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="ScanBuildArtifacts200Response Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create ScanBuildArtifacts200Response</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <ScanBuildArtifacts200ResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter ScanBuildArtifacts200Response (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing ScanBuildArtifacts200Response</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof ScanBuildArtifacts200Response;
                ...
                await updateScanBuildArtifacts200Response(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of ScanBuildArtifacts200Responses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'ScanBuildArtifacts200Response'}
...
export default ScanBuildArtifacts200ResponseTable;
