...
import PublishRestEndpointRequestForm from '@thorapi/redux/components/form/PublishRestEndpointRequestForm';
...
} from '@thorapi/redux/services/PublishRestEndpointRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `PublishRestEndpointRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/PublishRestEndpointRequest/${rowId}`;
...
const PublishRestEndpointRequestTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): PublishRestEndpointRequest[] => {
    if (Array.isArray(result)) return result as PublishRestEndpointRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as PublishRestEndpointRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: PublishRestEndpointRequest[], requestedPage: number) => {
  ...
  const [deletePublishRestEndpointRequestCascade] = useDeletePublishRestEndpointRequestCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<PublishRestEndpointRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<PublishRestEndpointRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as PublishRestEndpointRequest;
    } catch (err) {
      console.warn('Could not reload PublishRestEndpointRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<PublishRestEndpointRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updatePublishRestEndpointRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as PublishRestEndpointRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update PublishRestEndpointRequest';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deletePublishRestEndpointRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `PublishRestEndpointRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addPublishRestEndpointRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updatePublishRestEndpointRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as PublishRestEndpointRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as PublishRestEndpointRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as PublishRestEndpointRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as PublishRestEndpointRequest[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter PublishRestEndpointRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
