...
import { ApiMetricsResponse } from '@thorapi/model';
import ApiMetricsResponseForm from '@thorapi/redux/components/form/ApiMetricsResponseForm';
...
} from '@thorapi/redux/services/ApiMetricsResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ApiMetricsResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ApiMetricsResponse/${rowId}`;
...
const ApiMetricsResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ApiMetricsResponse[] => {
    if (Array.isArray(result)) return result as ApiMetricsResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as ApiMetricsResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ApiMetricsResponse[], requestedPage: number) => {
  ...
  const [deleteApiMetricsResponseCascade] = useDeleteApiMetricsResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ApiMetricsResponse | null> => {
    ...
    try {
      ...
      console.warn('Could not reload ApiMetricsResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ApiMetricsResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateApiMetricsResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ApiMetricsResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ApiMetricsResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteApiMetricsResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ApiMetricsResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addApiMetricsResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateApiMetricsResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ApiMetricsResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ApiMetricsResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ApiMetricsResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ApiMetricsResponse[]);
    }
  };
