...
import { GeneralLedgerEntry } from '@thorapi/model';
import GeneralLedgerEntryForm from '@thorapi/redux/components/form/GeneralLedgerEntryForm';
...
} from '@thorapi/redux/services/GeneralLedgerEntryService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `GeneralLedgerEntry update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/GeneralLedgerEntry/${rowId}`;
...
const GeneralLedgerEntryTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): GeneralLedgerEntry[] => {
    if (Array.isArray(result)) return result as GeneralLedgerEntry[];
    ...
    return Array.isArray(candidate) ? (candidate as GeneralLedgerEntry[]) : [];
  };

  const mergeFetchedRows = (incomingRows: GeneralLedgerEntry[], requestedPage: number) => {
  ...
  const [deleteGeneralLedgerEntryCascade] = useDeleteGeneralLedgerEntryCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<GeneralLedgerEntry | null> => {
    ...
    try {
      ...
      return (await response.json()) as GeneralLedgerEntry;
    } catch (err) {
      console.warn('Could not reload GeneralLedgerEntry row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<GeneralLedgerEntry>,
    successField?: string,
  ) => {
    try {
      const saved = await updateGeneralLedgerEntry(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as GeneralLedgerEntry[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update GeneralLedgerEntry';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteGeneralLedgerEntryCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `GeneralLedgerEntry-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addGeneralLedgerEntry(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateGeneralLedgerEntry(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as GeneralLedgerEntry[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as GeneralLedgerEntry[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as GeneralLedgerEntry[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as GeneralLedgerEntry[]);
    }
  };
