...
import { ChartSeries } from '@thorapi/model';
import ChartSeriesForm from '@thorapi/redux/components/form/ChartSeriesForm';
...
} from '@thorapi/redux/services/ChartSeriesService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ChartSeries update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ChartSeries/${rowId}`;
...
const ChartSeriesTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ChartSeries[] => {
    if (Array.isArray(result)) return result as ChartSeries[];
    ...
    return Array.isArray(candidate) ? (candidate as ChartSeries[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ChartSeries[], requestedPage: number) => {
  ...
  const [deleteChartSeriesCascade] = useDeleteChartSeriesCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ChartSeries | null> => {
    ...
    try {
      ...
      return (await response.json()) as ChartSeries;
    } catch (err) {
      console.warn('Could not reload ChartSeries row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ChartSeries>,
    successField?: string,
  ) => {
    try {
      const saved = await updateChartSeries(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ChartSeries[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ChartSeries';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteChartSeriesCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ChartSeries-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addChartSeries(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateChartSeries(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ChartSeries[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ChartSeries[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ChartSeries[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ChartSeries[]);
    }
  };
