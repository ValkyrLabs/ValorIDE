...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  leaseStatus: {
    type: "enum",
    enumValues: ["ISSUED", "USED", "EXPIRED", "REVOKED"],
    enumValueType: "string",
  },
...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/TrustSecretLease/${rowId}`;
...
const TrustSecretLeaseTable: React.FC = () => {
  ...
  const [deleteTrustSecretLeaseCascade] =
    useDeleteTrustSecretLeaseCascadeMutation();
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update TrustSecretLease";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustSecretLeaseCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
