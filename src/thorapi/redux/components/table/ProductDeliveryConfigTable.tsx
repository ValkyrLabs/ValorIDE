...
import { ProductDeliveryConfig } from '@thorapi/model';
import ProductDeliveryConfigForm from '@thorapi/redux/components/form/ProductDeliveryConfigForm';
...
import {
  ...
  useDeleteProductDeliveryConfigCascadeMutation
} from '@thorapi/redux/services/ProductDeliveryConfigService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ProductDeliveryConfig update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ProductDeliveryConfig/${rowId}`;
...
const ProductDeliveryConfigTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ProductDeliveryConfig[] => {
    if (Array.isArray(result)) return result as ProductDeliveryConfig[];
    ...
    return Array.isArray(candidate) ? (candidate as ProductDeliveryConfig[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ProductDeliveryConfig[], requestedPage: number) => {
  ...
  const [deleteProductDeliveryConfigCascade] = useDeleteProductDeliveryConfigCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ProductDeliveryConfig | null> => {
    ...
    try {
      ...
      return (await response.json()) as ProductDeliveryConfig;
    } catch (err) {
      console.warn('Could not reload ProductDeliveryConfig row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ProductDeliveryConfig>,
    successField?: string,
  ) => {
    try {
      const saved = await updateProductDeliveryConfig(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ProductDeliveryConfig[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ProductDeliveryConfig';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteProductDeliveryConfigCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ProductDeliveryConfig-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addProductDeliveryConfig(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateProductDeliveryConfig(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ProductDeliveryConfig[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ProductDeliveryConfig[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ProductDeliveryConfig[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ProductDeliveryConfig[]);
    }
  };
