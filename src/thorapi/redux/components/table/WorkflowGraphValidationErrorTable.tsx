...
import { WorkflowGraphValidationError } from '@thorapi/model';
import WorkflowGraphValidationErrorForm from '@thorapi/redux/components/form/WorkflowGraphValidationErrorForm';
...
import {
  useLazyGetWorkflowGraphValidationErrorsPagedQuery,
...
} from '@thorapi/redux/services/WorkflowGraphValidationErrorService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `WorkflowGraphValidationError update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/WorkflowGraphValidationError/${rowId}`;

const WorkflowGraphValidationErrorTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<WorkflowGraphValidationError[]>([]);
  ...
  const normalizePageRows = (result: unknown): WorkflowGraphValidationError[] => {
    if (Array.isArray(result)) return result as WorkflowGraphValidationError[];
    ...
    return Array.isArray(candidate) ? (candidate as WorkflowGraphValidationError[]) : [];
  };

  const mergeFetchedRows = (incomingRows: WorkflowGraphValidationError[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addWorkflowGraphValidationError] = useAddWorkflowGraphValidationErrorMutation();
  const [updateWorkflowGraphValidationError] = useUpdateWorkflowGraphValidationErrorMutation();
  const [deleteWorkflowGraphValidationError] = useDeleteWorkflowGraphValidationErrorMutation();
  const [deleteWorkflowGraphValidationErrorCascade] = useDeleteWorkflowGraphValidationErrorCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<WorkflowGraphValidationError>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<WorkflowGraphValidationError | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<WorkflowGraphValidationError | null> => {
    ...
    try {
      ...
      return (await response.json()) as WorkflowGraphValidationError;
    } catch (err) {
      console.warn('Could not reload WorkflowGraphValidationError row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<WorkflowGraphValidationError>,
    successField?: string,
  ) => {
    try {
      const saved = await updateWorkflowGraphValidationError(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as WorkflowGraphValidationError[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update WorkflowGraphValidationError';
      console.error('Failed to update WorkflowGraphValidationError:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteWorkflowGraphValidationErrorCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteWorkflowGraphValidationError(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<WorkflowGraphValidationError> = { ...source };
      delete (payload as any).id;
      const created = await addWorkflowGraphValidationError(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphValidationError, ...prev]);
    } catch (error) {
      console.error('Failed to add WorkflowGraphValidationError:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<WorkflowGraphValidationError> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addWorkflowGraphValidationError(payload).unwrap();
      setAllData((prev) => [created as WorkflowGraphValidationError, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) WorkflowGraphValidationError:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `WorkflowGraphValidationError-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addWorkflowGraphValidationError(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateWorkflowGraphValidationError(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as WorkflowGraphValidationError[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as WorkflowGraphValidationError[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as WorkflowGraphValidationError[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as WorkflowGraphValidationError[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="WorkflowGraphValidationErrorTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="WorkflowGraphValidationError Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create WorkflowGraphValidationError</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <WorkflowGraphValidationErrorForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter WorkflowGraphValidationError (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing WorkflowGraphValidationError</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof WorkflowGraphValidationError;
                ...
                await updateWorkflowGraphValidationError(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of WorkflowGraphValidationErrors.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'WorkflowGraphValidationError'}
...
export default WorkflowGraphValidationErrorTable;
