...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/ArrayStringItem/${rowId}`;
...
const ArrayStringItemTable: React.FC = () => {
  ...
  const [deleteArrayStringItemCascade] =
    useDeleteArrayStringItemCascadeMutation();
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update ArrayStringItem";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteArrayStringItemCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
