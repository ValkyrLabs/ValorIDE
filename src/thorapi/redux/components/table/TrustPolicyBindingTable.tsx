...
const columnSchema: Record<string, ColumnSchema> = {
  bindingStatus: {
    type: "enum",
    enumValues: ["ACTIVE", "SUSPENDED", "RETIRED"],
...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/TrustPolicyBinding/${rowId}`;
...
const TrustPolicyBindingTable: React.FC = () => {
  ...
  const [deleteTrustPolicyBindingCascade] =
    useDeleteTrustPolicyBindingCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<TrustPolicyBinding | null> => {
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update TrustPolicyBinding";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTrustPolicyBindingCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter TrustPolicyBinding (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
