...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/RewrapTrustKeyRequest/${rowId}`;
...
const RewrapTrustKeyRequestTable: React.FC = () => {
  ...
  const [updateRewrapTrustKeyRequest] =
    useUpdateRewrapTrustKeyRequestMutation();
  const [deleteRewrapTrustKeyRequest] =
    useDeleteRewrapTrustKeyRequestMutation();
  const [deleteRewrapTrustKeyRequestCascade] =
    useDeleteRewrapTrustKeyRequestCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<RewrapTrustKeyRequest | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<RewrapTrustKeyRequest | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateRewrapTrustKeyRequest(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update RewrapTrustKeyRequest";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRewrapTrustKeyRequestCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter RewrapTrustKeyRequest (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
