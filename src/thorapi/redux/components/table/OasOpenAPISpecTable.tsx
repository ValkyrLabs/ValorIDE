...
import { OasOpenAPISpec } from '@thorapi/model';
import OasOpenAPISpecForm from '@thorapi/redux/components/form/OasOpenAPISpecForm';
...
} from '@thorapi/redux/services/OasOpenAPISpecService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `OasOpenAPISpec update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/OasOpenAPISpec/${rowId}`;
...
const OasOpenAPISpecTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): OasOpenAPISpec[] => {
    if (Array.isArray(result)) return result as OasOpenAPISpec[];
    ...
    return Array.isArray(candidate) ? (candidate as OasOpenAPISpec[]) : [];
  };

  const mergeFetchedRows = (incomingRows: OasOpenAPISpec[], requestedPage: number) => {
  ...
  const [deleteOasOpenAPISpecCascade] = useDeleteOasOpenAPISpecCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<OasOpenAPISpec | null> => {
    ...
    try {
      ...
      return (await response.json()) as OasOpenAPISpec;
    } catch (err) {
      console.warn('Could not reload OasOpenAPISpec row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<OasOpenAPISpec>,
    successField?: string,
  ) => {
    try {
      const saved = await updateOasOpenAPISpec(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as OasOpenAPISpec[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update OasOpenAPISpec';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOasOpenAPISpecCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `OasOpenAPISpec-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addOasOpenAPISpec(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateOasOpenAPISpec(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as OasOpenAPISpec[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as OasOpenAPISpec[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as OasOpenAPISpec[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as OasOpenAPISpec[]);
    }
  };
