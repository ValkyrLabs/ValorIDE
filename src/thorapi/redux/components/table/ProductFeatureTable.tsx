...
import { ProductFeature } from '@thorapi/model';
import ProductFeatureForm from '@thorapi/redux/components/form/ProductFeatureForm';
...
} from '@thorapi/redux/services/ProductFeatureService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'featureScope': { type: 'enum', enumValues: [
    'credits', 'platform', 'organization', 'application', 'agent', 'billing', 'ux', 
  ], enumValueType: 'string' },
  'valueType': { type: 'enum', enumValues: [
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `ProductFeature update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/ProductFeature/${rowId}`;
...
const ProductFeatureTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): ProductFeature[] => {
    if (Array.isArray(result)) return result as ProductFeature[];
    ...
    return Array.isArray(candidate) ? (candidate as ProductFeature[]) : [];
  };

  const mergeFetchedRows = (incomingRows: ProductFeature[], requestedPage: number) => {
  ...
  const [deleteProductFeatureCascade] = useDeleteProductFeatureCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<ProductFeature | null> => {
    ...
    try {
      ...
      return (await response.json()) as ProductFeature;
    } catch (err) {
      console.warn('Could not reload ProductFeature row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<ProductFeature>,
    successField?: string,
  ) => {
    try {
      const saved = await updateProductFeature(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as ProductFeature[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update ProductFeature';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteProductFeatureCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `ProductFeature-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addProductFeature(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateProductFeature(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as ProductFeature[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as ProductFeature[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as ProductFeature[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as ProductFeature[]);
    }
  };
