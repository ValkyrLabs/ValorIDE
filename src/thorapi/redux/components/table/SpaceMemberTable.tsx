...
import { SpaceMember } from '@thorapi/model';
import SpaceMemberForm from '@thorapi/redux/components/form/SpaceMemberForm';
...
} from '@thorapi/redux/services/SpaceMemberService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SpaceMember update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SpaceMember/${rowId}`;
...
const SpaceMemberTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SpaceMember[] => {
    if (Array.isArray(result)) return result as SpaceMember[];
    ...
    return Array.isArray(candidate) ? (candidate as SpaceMember[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SpaceMember[], requestedPage: number) => {
  ...
  const [deleteSpaceMemberCascade] = useDeleteSpaceMemberCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SpaceMember | null> => {
    ...
    try {
      ...
      return (await response.json()) as SpaceMember;
    } catch (err) {
      console.warn('Could not reload SpaceMember row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SpaceMember>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSpaceMember(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SpaceMember[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SpaceMember';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSpaceMemberCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SpaceMember-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSpaceMember(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSpaceMember(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SpaceMember[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SpaceMember[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SpaceMember[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SpaceMember[]);
    }
  };
