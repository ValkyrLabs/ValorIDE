...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/MemoryRetentionPolicy/${rowId}`;
...
const MemoryRetentionPolicyTable: React.FC = () => {
  ...
  const [updateMemoryRetentionPolicy] =
    useUpdateMemoryRetentionPolicyMutation();
  const [deleteMemoryRetentionPolicy] =
    useDeleteMemoryRetentionPolicyMutation();
  const [deleteMemoryRetentionPolicyCascade] =
    useDeleteMemoryRetentionPolicyCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<MemoryRetentionPolicy | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<MemoryRetentionPolicy | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateMemoryRetentionPolicy(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update MemoryRetentionPolicy";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMemoryRetentionPolicyCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter MemoryRetentionPolicy (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
