...
import MemoryRetentionPolicyForm from '@thorapi/redux/components/form/MemoryRetentionPolicyForm';
...
import {
  useLazyGetMemoryRetentionPolicysPagedQuery,
...
} from '@thorapi/redux/services/MemoryRetentionPolicyService';
...
const columnSchema: Record<string, ColumnSchema> = {
  ...
  'hardDeleteEnabled': { type: 'boolean' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `MemoryRetentionPolicy update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/MemoryRetentionPolicy/${rowId}`;

const MemoryRetentionPolicyTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<MemoryRetentionPolicy[]>([]);
  ...
  const normalizePageRows = (result: unknown): MemoryRetentionPolicy[] => {
    if (Array.isArray(result)) return result as MemoryRetentionPolicy[];
    ...
    return Array.isArray(candidate) ? (candidate as MemoryRetentionPolicy[]) : [];
  };

  const mergeFetchedRows = (incomingRows: MemoryRetentionPolicy[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addMemoryRetentionPolicy] = useAddMemoryRetentionPolicyMutation();
  const [updateMemoryRetentionPolicy] = useUpdateMemoryRetentionPolicyMutation();
  const [deleteMemoryRetentionPolicy] = useDeleteMemoryRetentionPolicyMutation();
  const [deleteMemoryRetentionPolicyCascade] = useDeleteMemoryRetentionPolicyCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<MemoryRetentionPolicy>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<MemoryRetentionPolicy | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<MemoryRetentionPolicy | null> => {
    ...
    try {
      ...
      return (await response.json()) as MemoryRetentionPolicy;
    } catch (err) {
      console.warn('Could not reload MemoryRetentionPolicy row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<MemoryRetentionPolicy>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMemoryRetentionPolicy(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as MemoryRetentionPolicy[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update MemoryRetentionPolicy';
      console.error('Failed to update MemoryRetentionPolicy:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMemoryRetentionPolicyCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteMemoryRetentionPolicy(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<MemoryRetentionPolicy> = { ...source };
      delete (payload as any).id;
      const created = await addMemoryRetentionPolicy(payload).unwrap();
      setAllData((prev) => [created as MemoryRetentionPolicy, ...prev]);
    } catch (error) {
      console.error('Failed to add MemoryRetentionPolicy:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<MemoryRetentionPolicy> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMemoryRetentionPolicy(payload).unwrap();
      setAllData((prev) => [created as MemoryRetentionPolicy, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) MemoryRetentionPolicy:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `MemoryRetentionPolicy-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMemoryRetentionPolicy(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMemoryRetentionPolicy(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as MemoryRetentionPolicy[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as MemoryRetentionPolicy[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as MemoryRetentionPolicy[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as MemoryRetentionPolicy[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="MemoryRetentionPolicyTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="MemoryRetentionPolicy Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create MemoryRetentionPolicy</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <MemoryRetentionPolicyForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter MemoryRetentionPolicy (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing MemoryRetentionPolicy</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof MemoryRetentionPolicy;
                ...
                await updateMemoryRetentionPolicy(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of MemoryRetentionPolicys.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'MemoryRetentionPolicy'}
...
export default MemoryRetentionPolicyTable;
