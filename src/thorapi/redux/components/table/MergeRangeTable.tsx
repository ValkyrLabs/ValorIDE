...
import { MergeRange } from '@thorapi/model';
import MergeRangeForm from '@thorapi/redux/components/form/MergeRangeForm';
...
} from '@thorapi/redux/services/MergeRangeService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `MergeRange update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/MergeRange/${rowId}`;
...
const MergeRangeTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): MergeRange[] => {
    if (Array.isArray(result)) return result as MergeRange[];
    ...
    return Array.isArray(candidate) ? (candidate as MergeRange[]) : [];
  };

  const mergeFetchedRows = (incomingRows: MergeRange[], requestedPage: number) => {
  ...
  const [deleteMergeRangeCascade] = useDeleteMergeRangeCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<MergeRange | null> => {
    ...
    try {
      ...
      return (await response.json()) as MergeRange;
    } catch (err) {
      console.warn('Could not reload MergeRange row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<MergeRange>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMergeRange(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as MergeRange[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update MergeRange';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMergeRangeCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `MergeRange-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMergeRange(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMergeRange(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as MergeRange[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as MergeRange[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as MergeRange[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as MergeRange[]);
    }
  };
