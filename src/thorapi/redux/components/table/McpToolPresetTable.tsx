...
import { McpToolPreset } from '@thorapi/model';
import McpToolPresetForm from '@thorapi/redux/components/form/McpToolPresetForm';
...
} from '@thorapi/redux/services/McpToolPresetService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `McpToolPreset update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/McpToolPreset/${rowId}`;
...
const McpToolPresetTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): McpToolPreset[] => {
    if (Array.isArray(result)) return result as McpToolPreset[];
    ...
    return Array.isArray(candidate) ? (candidate as McpToolPreset[]) : [];
  };

  const mergeFetchedRows = (incomingRows: McpToolPreset[], requestedPage: number) => {
  ...
  const [deleteMcpToolPresetCascade] = useDeleteMcpToolPresetCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<McpToolPreset | null> => {
    ...
    try {
      ...
      return (await response.json()) as McpToolPreset;
    } catch (err) {
      console.warn('Could not reload McpToolPreset row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<McpToolPreset>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMcpToolPreset(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as McpToolPreset[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update McpToolPreset';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpToolPresetCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `McpToolPreset-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMcpToolPreset(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMcpToolPreset(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as McpToolPreset[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as McpToolPreset[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as McpToolPreset[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as McpToolPreset[]);
    }
  };
