...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/RequestPasswordResetResponse/${rowId}`;
...
const RequestPasswordResetResponseTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addRequestPasswordResetResponse] =
    useAddRequestPasswordResetResponseMutation();
  const [updateRequestPasswordResetResponse] =
    useUpdateRequestPasswordResetResponseMutation();
  const [deleteRequestPasswordResetResponse] =
    useDeleteRequestPasswordResetResponseMutation();
  const [deleteRequestPasswordResetResponseCascade] =
    useDeleteRequestPasswordResetResponseCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] =
    useState<RequestPasswordResetResponse | null>(null);
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<RequestPasswordResetResponse | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload RequestPasswordResetResponse row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateRequestPasswordResetResponse(
        updatedItem as any,
      ).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) RequestPasswordResetResponse:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter RequestPasswordResetResponse (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
