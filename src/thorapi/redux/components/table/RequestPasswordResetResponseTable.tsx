...
import { RequestPasswordResetResponse } from '@thorapi/model';
import RequestPasswordResetResponseForm from '@thorapi/redux/components/form/RequestPasswordResetResponseForm';
...
import {
  useLazyGetRequestPasswordResetResponsesPagedQuery,
...
} from '@thorapi/redux/services/RequestPasswordResetResponseService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'status': { type: 'enum', enumValues: [
    'SENT', 'USER_NOT_FOUND', 'ACCOUNT_LOCKED', 
  ], enumValueType: 'string' },
  'requiresPhoneVerification': { type: 'boolean' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RequestPasswordResetResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RequestPasswordResetResponse/${rowId}`;

const RequestPasswordResetResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RequestPasswordResetResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): RequestPasswordResetResponse[] => {
    if (Array.isArray(result)) return result as RequestPasswordResetResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as RequestPasswordResetResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RequestPasswordResetResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addRequestPasswordResetResponse] = useAddRequestPasswordResetResponseMutation();
  const [updateRequestPasswordResetResponse] = useUpdateRequestPasswordResetResponseMutation();
  const [deleteRequestPasswordResetResponse] = useDeleteRequestPasswordResetResponseMutation();
  const [deleteRequestPasswordResetResponseCascade] = useDeleteRequestPasswordResetResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<RequestPasswordResetResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RequestPasswordResetResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<RequestPasswordResetResponse | null> => {
    ...
    try {
      ...
      console.warn('Could not reload RequestPasswordResetResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RequestPasswordResetResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRequestPasswordResetResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RequestPasswordResetResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RequestPasswordResetResponse';
      console.error('Failed to update RequestPasswordResetResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRequestPasswordResetResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteRequestPasswordResetResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RequestPasswordResetResponse> = { ...source };
      delete (payload as any).id;
      const created = await addRequestPasswordResetResponse(payload).unwrap();
      setAllData((prev) => [created as RequestPasswordResetResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add RequestPasswordResetResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RequestPasswordResetResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRequestPasswordResetResponse(payload).unwrap();
      setAllData((prev) => [created as RequestPasswordResetResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) RequestPasswordResetResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RequestPasswordResetResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRequestPasswordResetResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRequestPasswordResetResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RequestPasswordResetResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RequestPasswordResetResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RequestPasswordResetResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RequestPasswordResetResponse[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="RequestPasswordResetResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="RequestPasswordResetResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RequestPasswordResetResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RequestPasswordResetResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RequestPasswordResetResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RequestPasswordResetResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RequestPasswordResetResponse;
                ...
                await updateRequestPasswordResetResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RequestPasswordResetResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RequestPasswordResetResponse'}
...
export default RequestPasswordResetResponseTable;
