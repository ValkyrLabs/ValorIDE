...
import { McpMarketplaceItemTag } from '@thorapi/model';
import McpMarketplaceItemTagForm from '@thorapi/redux/components/form/McpMarketplaceItemTagForm';
...
} from '@thorapi/redux/services/McpMarketplaceItemTagService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `McpMarketplaceItemTag update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/McpMarketplaceItemTag/${rowId}`;
...
const McpMarketplaceItemTagTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): McpMarketplaceItemTag[] => {
    if (Array.isArray(result)) return result as McpMarketplaceItemTag[];
    ...
    return Array.isArray(candidate) ? (candidate as McpMarketplaceItemTag[]) : [];
  };

  const mergeFetchedRows = (incomingRows: McpMarketplaceItemTag[], requestedPage: number) => {
  ...
  const [deleteMcpMarketplaceItemTagCascade] = useDeleteMcpMarketplaceItemTagCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<McpMarketplaceItemTag | null> => {
    ...
    try {
      ...
      return (await response.json()) as McpMarketplaceItemTag;
    } catch (err) {
      console.warn('Could not reload McpMarketplaceItemTag row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<McpMarketplaceItemTag>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMcpMarketplaceItemTag(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as McpMarketplaceItemTag[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update McpMarketplaceItemTag';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpMarketplaceItemTagCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `McpMarketplaceItemTag-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMcpMarketplaceItemTag(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMcpMarketplaceItemTag(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as McpMarketplaceItemTag[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as McpMarketplaceItemTag[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as McpMarketplaceItemTag[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as McpMarketplaceItemTag[]);
    }
  };
