...
import { InitUploadRequest } from '@thorapi/model';
import InitUploadRequestForm from '@thorapi/redux/components/form/InitUploadRequestForm';
...
} from '@thorapi/redux/services/InitUploadRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `InitUploadRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/InitUploadRequest/${rowId}`;
...
const InitUploadRequestTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): InitUploadRequest[] => {
    if (Array.isArray(result)) return result as InitUploadRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as InitUploadRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: InitUploadRequest[], requestedPage: number) => {
  ...
  const [deleteInitUploadRequestCascade] = useDeleteInitUploadRequestCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<InitUploadRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as InitUploadRequest;
    } catch (err) {
      console.warn('Could not reload InitUploadRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<InitUploadRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateInitUploadRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as InitUploadRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update InitUploadRequest';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteInitUploadRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `InitUploadRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addInitUploadRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateInitUploadRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as InitUploadRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as InitUploadRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as InitUploadRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as InitUploadRequest[]);
    }
  };
  ...
  return (
    <>
          ...
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter InitUploadRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
