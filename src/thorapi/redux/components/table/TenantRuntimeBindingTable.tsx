...
import { TenantRuntimeBinding } from '@thorapi/model';
import TenantRuntimeBindingForm from '@thorapi/redux/components/form/TenantRuntimeBindingForm';
...
import {
  useLazyGetTenantRuntimeBindingsPagedQuery,
  useAddTenantRuntimeBindingMutation,
  useUpdateTenantRuntimeBindingMutation,
  useDeleteTenantRuntimeBindingMutation,
  useDeleteTenantRuntimeBindingCascadeMutation
} from '@thorapi/redux/services/TenantRuntimeBindingService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'runtimeKind': { type: 'enum', enumValues: [
    'valkyr_hosted', 'lightsail', 'fargate', 'external', 'local_dev', 
  ], enumValueType: 'string' },
  'authMode': { type: 'enum', enumValues: [
    'valkyr_rbac', 'tenant_rbac', 'service_token', 'delegated_oidc', 
  ], enumValueType: 'string' },
  'rbacMode': { type: 'enum', enumValues: [
    'valkyr_managed', 'tenant_managed', 'hybrid', 
  ], enumValueType: 'string' },
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `TenantRuntimeBinding update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/TenantRuntimeBinding/${rowId}`;

const TenantRuntimeBindingTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<TenantRuntimeBinding[]>([]);
  ...
  const normalizePageRows = (result: unknown): TenantRuntimeBinding[] => {
    if (Array.isArray(result)) return result as TenantRuntimeBinding[];
    ...
    return Array.isArray(candidate) ? (candidate as TenantRuntimeBinding[]) : [];
  };

  const mergeFetchedRows = (incomingRows: TenantRuntimeBinding[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addTenantRuntimeBinding] = useAddTenantRuntimeBindingMutation();
  const [updateTenantRuntimeBinding] = useUpdateTenantRuntimeBindingMutation();
  const [deleteTenantRuntimeBinding] = useDeleteTenantRuntimeBindingMutation();
  const [deleteTenantRuntimeBindingCascade] = useDeleteTenantRuntimeBindingCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<TenantRuntimeBinding>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<TenantRuntimeBinding | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<TenantRuntimeBinding | null> => {
    ...
    try {
      ...
      return (await response.json()) as TenantRuntimeBinding;
    } catch (err) {
      console.warn('Could not reload TenantRuntimeBinding row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<TenantRuntimeBinding>,
    successField?: string,
  ) => {
    try {
      const saved = await updateTenantRuntimeBinding(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as TenantRuntimeBinding[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update TenantRuntimeBinding';
      console.error('Failed to update TenantRuntimeBinding:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteTenantRuntimeBindingCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteTenantRuntimeBinding(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<TenantRuntimeBinding> = { ...source };
      delete (payload as any).id;
      const created = await addTenantRuntimeBinding(payload).unwrap();
      setAllData((prev) => [created as TenantRuntimeBinding, ...prev]);
    } catch (error) {
      console.error('Failed to add TenantRuntimeBinding:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<TenantRuntimeBinding> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addTenantRuntimeBinding(payload).unwrap();
      setAllData((prev) => [created as TenantRuntimeBinding, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) TenantRuntimeBinding:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `TenantRuntimeBinding-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addTenantRuntimeBinding(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateTenantRuntimeBinding(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as TenantRuntimeBinding[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as TenantRuntimeBinding[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as TenantRuntimeBinding[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as TenantRuntimeBinding[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="TenantRuntimeBindingTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="TenantRuntimeBinding Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create TenantRuntimeBinding</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <TenantRuntimeBindingForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter TenantRuntimeBinding (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing TenantRuntimeBinding</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof TenantRuntimeBinding;
                ...
                await updateTenantRuntimeBinding(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of TenantRuntimeBindings.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'TenantRuntimeBinding'}
...
export default TenantRuntimeBindingTable;
