...
import { GenerateTrustProofRequest } from '@thorapi/model';
import GenerateTrustProofRequestForm from '@thorapi/redux/components/form/GenerateTrustProofRequestForm';
...
import {
  useLazyGetGenerateTrustProofRequestsPagedQuery,
...
} from '@thorapi/redux/services/GenerateTrustProofRequestService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'proofFormat': { type: 'enum', enumValues: [
    'JCS_JSON', 'CANONICAL_CBOR', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `GenerateTrustProofRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/GenerateTrustProofRequest/${rowId}`;

const GenerateTrustProofRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<GenerateTrustProofRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): GenerateTrustProofRequest[] => {
    if (Array.isArray(result)) return result as GenerateTrustProofRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as GenerateTrustProofRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: GenerateTrustProofRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addGenerateTrustProofRequest] = useAddGenerateTrustProofRequestMutation();
  const [updateGenerateTrustProofRequest] = useUpdateGenerateTrustProofRequestMutation();
  const [deleteGenerateTrustProofRequest] = useDeleteGenerateTrustProofRequestMutation();
  const [deleteGenerateTrustProofRequestCascade] = useDeleteGenerateTrustProofRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<GenerateTrustProofRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<GenerateTrustProofRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<GenerateTrustProofRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as GenerateTrustProofRequest;
    } catch (err) {
      console.warn('Could not reload GenerateTrustProofRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<GenerateTrustProofRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateGenerateTrustProofRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as GenerateTrustProofRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update GenerateTrustProofRequest';
      console.error('Failed to update GenerateTrustProofRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteGenerateTrustProofRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteGenerateTrustProofRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<GenerateTrustProofRequest> = { ...source };
      delete (payload as any).id;
      const created = await addGenerateTrustProofRequest(payload).unwrap();
      setAllData((prev) => [created as GenerateTrustProofRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add GenerateTrustProofRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<GenerateTrustProofRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addGenerateTrustProofRequest(payload).unwrap();
      setAllData((prev) => [created as GenerateTrustProofRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) GenerateTrustProofRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `GenerateTrustProofRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addGenerateTrustProofRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateGenerateTrustProofRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as GenerateTrustProofRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as GenerateTrustProofRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as GenerateTrustProofRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as GenerateTrustProofRequest[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.GenerateTrustProofRequest',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="GenerateTrustProofRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="GenerateTrustProofRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create GenerateTrustProofRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <GenerateTrustProofRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter GenerateTrustProofRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing GenerateTrustProofRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof GenerateTrustProofRequest;
                ...
                await updateGenerateTrustProofRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of GenerateTrustProofRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'GenerateTrustProofRequest'}
...
export default GenerateTrustProofRequestTable;
