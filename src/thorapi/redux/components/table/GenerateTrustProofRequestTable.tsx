...
const columnSchema: Record<string, ColumnSchema> = {
  proofFormat: {
    type: "enum",
    enumValues: ["JCS_JSON", "CANONICAL_CBOR"],
...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/GenerateTrustProofRequest/${rowId}`;
...
const GenerateTrustProofRequestTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addGenerateTrustProofRequest] =
    useAddGenerateTrustProofRequestMutation();
  const [updateGenerateTrustProofRequest] =
    useUpdateGenerateTrustProofRequestMutation();
  const [deleteGenerateTrustProofRequest] =
    useDeleteGenerateTrustProofRequestMutation();
  const [deleteGenerateTrustProofRequestCascade] =
    useDeleteGenerateTrustProofRequestCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<GenerateTrustProofRequest | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<GenerateTrustProofRequest | null> => {
    ...
    try {
      ...
      console.warn(
        "Could not reload GenerateTrustProofRequest row",
        rowId,
        err,
      );
      return null;
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateGenerateTrustProofRequest(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update GenerateTrustProofRequest";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteGenerateTrustProofRequestCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) GenerateTrustProofRequest:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter GenerateTrustProofRequest (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
