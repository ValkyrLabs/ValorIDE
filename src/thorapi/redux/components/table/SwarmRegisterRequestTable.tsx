...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/SwarmRegisterRequest/${rowId}`;
...
const SwarmRegisterRequestTable: React.FC = () => {
  ...
  const [deleteSwarmRegisterRequestCascade] =
    useDeleteSwarmRegisterRequestCascadeMutation();
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<SwarmRegisterRequest | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateSwarmRegisterRequest(
        updatedItem as any,
      ).unwrap();
