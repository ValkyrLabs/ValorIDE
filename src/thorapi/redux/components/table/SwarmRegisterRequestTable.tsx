...
import { SwarmRegisterRequest } from '@thorapi/model';
import SwarmRegisterRequestForm from '@thorapi/redux/components/form/SwarmRegisterRequestForm';
...
} from '@thorapi/redux/services/SwarmRegisterRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `SwarmRegisterRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/SwarmRegisterRequest/${rowId}`;
...
const SwarmRegisterRequestTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): SwarmRegisterRequest[] => {
    if (Array.isArray(result)) return result as SwarmRegisterRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as SwarmRegisterRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: SwarmRegisterRequest[], requestedPage: number) => {
  ...
  const [deleteSwarmRegisterRequestCascade] = useDeleteSwarmRegisterRequestCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<SwarmRegisterRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as SwarmRegisterRequest;
    } catch (err) {
      console.warn('Could not reload SwarmRegisterRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<SwarmRegisterRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateSwarmRegisterRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as SwarmRegisterRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update SwarmRegisterRequest';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteSwarmRegisterRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `SwarmRegisterRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addSwarmRegisterRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateSwarmRegisterRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as SwarmRegisterRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as SwarmRegisterRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as SwarmRegisterRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as SwarmRegisterRequest[]);
    }
  };
