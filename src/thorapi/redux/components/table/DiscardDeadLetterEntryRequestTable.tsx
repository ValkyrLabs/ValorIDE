...
import { DiscardDeadLetterEntryRequest } from '@thorapi/model';
import DiscardDeadLetterEntryRequestForm from '@thorapi/redux/components/form/DiscardDeadLetterEntryRequestForm';
...
} from '@thorapi/redux/services/DiscardDeadLetterEntryRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `DiscardDeadLetterEntryRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/DiscardDeadLetterEntryRequest/${rowId}`;
...
const DiscardDeadLetterEntryRequestTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): DiscardDeadLetterEntryRequest[] => {
    if (Array.isArray(result)) return result as DiscardDeadLetterEntryRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as DiscardDeadLetterEntryRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: DiscardDeadLetterEntryRequest[], requestedPage: number) => {
  ...
  const [deleteDiscardDeadLetterEntryRequestCascade] = useDeleteDiscardDeadLetterEntryRequestCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<DiscardDeadLetterEntryRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as DiscardDeadLetterEntryRequest;
    } catch (err) {
      console.warn('Could not reload DiscardDeadLetterEntryRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<DiscardDeadLetterEntryRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateDiscardDeadLetterEntryRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as DiscardDeadLetterEntryRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update DiscardDeadLetterEntryRequest';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteDiscardDeadLetterEntryRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `DiscardDeadLetterEntryRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addDiscardDeadLetterEntryRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateDiscardDeadLetterEntryRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as DiscardDeadLetterEntryRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as DiscardDeadLetterEntryRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as DiscardDeadLetterEntryRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as DiscardDeadLetterEntryRequest[]);
    }
  };
