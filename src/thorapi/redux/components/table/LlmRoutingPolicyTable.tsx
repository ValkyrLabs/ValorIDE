...
const columnSchema: Record<string, ColumnSchema> = {
  strategy: {
    type: "enum",
    enumValues: ["single_provider", "fallback_chain"],
...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/LlmRoutingPolicy/${rowId}`;
...
const LlmRoutingPolicyTable: React.FC = () => {
  ...
  const [deleteLlmRoutingPolicyCascade] =
    useDeleteLlmRoutingPolicyCascadeMutation();
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update LlmRoutingPolicy";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteLlmRoutingPolicyCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
