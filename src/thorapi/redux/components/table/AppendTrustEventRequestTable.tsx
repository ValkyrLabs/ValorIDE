...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/AppendTrustEventRequest/${rowId}`;
...
const AppendTrustEventRequestTable: React.FC = () => {
  ...
  const [updateAppendTrustEventRequest] =
    useUpdateAppendTrustEventRequestMutation();
  const [deleteAppendTrustEventRequest] =
    useDeleteAppendTrustEventRequestMutation();
  const [deleteAppendTrustEventRequestCascade] =
    useDeleteAppendTrustEventRequestCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<AppendTrustEventRequest | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<AppendTrustEventRequest | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateAppendTrustEventRequest(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update AppendTrustEventRequest";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAppendTrustEventRequestCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter AppendTrustEventRequest (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
