...
import { AppendTrustEventRequest } from '@thorapi/model';
import AppendTrustEventRequestForm from '@thorapi/redux/components/form/AppendTrustEventRequestForm';
...
import {
  useLazyGetAppendTrustEventRequestsPagedQuery,
...
} from '@thorapi/redux/services/AppendTrustEventRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `AppendTrustEventRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/AppendTrustEventRequest/${rowId}`;

const AppendTrustEventRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<AppendTrustEventRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): AppendTrustEventRequest[] => {
    if (Array.isArray(result)) return result as AppendTrustEventRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as AppendTrustEventRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: AppendTrustEventRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addAppendTrustEventRequest] = useAddAppendTrustEventRequestMutation();
  const [updateAppendTrustEventRequest] = useUpdateAppendTrustEventRequestMutation();
  const [deleteAppendTrustEventRequest] = useDeleteAppendTrustEventRequestMutation();
  const [deleteAppendTrustEventRequestCascade] = useDeleteAppendTrustEventRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<AppendTrustEventRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<AppendTrustEventRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<AppendTrustEventRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as AppendTrustEventRequest;
    } catch (err) {
      console.warn('Could not reload AppendTrustEventRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<AppendTrustEventRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateAppendTrustEventRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as AppendTrustEventRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update AppendTrustEventRequest';
      console.error('Failed to update AppendTrustEventRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAppendTrustEventRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteAppendTrustEventRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<AppendTrustEventRequest> = { ...source };
      delete (payload as any).id;
      const created = await addAppendTrustEventRequest(payload).unwrap();
      setAllData((prev) => [created as AppendTrustEventRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add AppendTrustEventRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<AppendTrustEventRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addAppendTrustEventRequest(payload).unwrap();
      setAllData((prev) => [created as AppendTrustEventRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) AppendTrustEventRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `AppendTrustEventRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addAppendTrustEventRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateAppendTrustEventRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as AppendTrustEventRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as AppendTrustEventRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as AppendTrustEventRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as AppendTrustEventRequest[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.AppendTrustEventRequest',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="AppendTrustEventRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="AppendTrustEventRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create AppendTrustEventRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <AppendTrustEventRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter AppendTrustEventRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing AppendTrustEventRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof AppendTrustEventRequest;
                ...
                await updateAppendTrustEventRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of AppendTrustEventRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'AppendTrustEventRequest'}
...
export default AppendTrustEventRequestTable;
