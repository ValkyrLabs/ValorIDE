...
import { VerifyTrustProofRequest } from '@thorapi/model';
import VerifyTrustProofRequestForm from '@thorapi/redux/components/form/VerifyTrustProofRequestForm';
...
import {
  useLazyGetVerifyTrustProofRequestsPagedQuery,
...
} from '@thorapi/redux/services/VerifyTrustProofRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `VerifyTrustProofRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/VerifyTrustProofRequest/${rowId}`;

const VerifyTrustProofRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<VerifyTrustProofRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): VerifyTrustProofRequest[] => {
    if (Array.isArray(result)) return result as VerifyTrustProofRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as VerifyTrustProofRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: VerifyTrustProofRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addVerifyTrustProofRequest] = useAddVerifyTrustProofRequestMutation();
  const [updateVerifyTrustProofRequest] = useUpdateVerifyTrustProofRequestMutation();
  const [deleteVerifyTrustProofRequest] = useDeleteVerifyTrustProofRequestMutation();
  const [deleteVerifyTrustProofRequestCascade] = useDeleteVerifyTrustProofRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<VerifyTrustProofRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<VerifyTrustProofRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<VerifyTrustProofRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as VerifyTrustProofRequest;
    } catch (err) {
      console.warn('Could not reload VerifyTrustProofRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<VerifyTrustProofRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateVerifyTrustProofRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as VerifyTrustProofRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update VerifyTrustProofRequest';
      console.error('Failed to update VerifyTrustProofRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteVerifyTrustProofRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteVerifyTrustProofRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<VerifyTrustProofRequest> = { ...source };
      delete (payload as any).id;
      const created = await addVerifyTrustProofRequest(payload).unwrap();
      setAllData((prev) => [created as VerifyTrustProofRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add VerifyTrustProofRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<VerifyTrustProofRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addVerifyTrustProofRequest(payload).unwrap();
      setAllData((prev) => [created as VerifyTrustProofRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) VerifyTrustProofRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `VerifyTrustProofRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addVerifyTrustProofRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateVerifyTrustProofRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as VerifyTrustProofRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as VerifyTrustProofRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as VerifyTrustProofRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as VerifyTrustProofRequest[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.VerifyTrustProofRequest',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="VerifyTrustProofRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="VerifyTrustProofRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create VerifyTrustProofRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <VerifyTrustProofRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter VerifyTrustProofRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing VerifyTrustProofRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof VerifyTrustProofRequest;
                ...
                await updateVerifyTrustProofRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of VerifyTrustProofRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'VerifyTrustProofRequest'}
...
export default VerifyTrustProofRequestTable;
