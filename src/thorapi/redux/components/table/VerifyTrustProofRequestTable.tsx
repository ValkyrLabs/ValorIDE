...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/VerifyTrustProofRequest/${rowId}`;
...
const VerifyTrustProofRequestTable: React.FC = () => {
  ...
  const [updateVerifyTrustProofRequest] =
    useUpdateVerifyTrustProofRequestMutation();
  const [deleteVerifyTrustProofRequest] =
    useDeleteVerifyTrustProofRequestMutation();
  const [deleteVerifyTrustProofRequestCascade] =
    useDeleteVerifyTrustProofRequestCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<VerifyTrustProofRequest | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<VerifyTrustProofRequest | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateVerifyTrustProofRequest(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update VerifyTrustProofRequest";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteVerifyTrustProofRequestCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter VerifyTrustProofRequest (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
