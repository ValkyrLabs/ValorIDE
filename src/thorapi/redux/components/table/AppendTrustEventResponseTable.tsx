...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/AppendTrustEventResponse/${rowId}`;
...
const AppendTrustEventResponseTable: React.FC = () => {
  ...
  // -------------------------------------------------------------------
  const [addAppendTrustEventResponse] =
    useAddAppendTrustEventResponseMutation();
  const [updateAppendTrustEventResponse] =
    useUpdateAppendTrustEventResponseMutation();
  const [deleteAppendTrustEventResponse] =
    useDeleteAppendTrustEventResponseMutation();
  const [deleteAppendTrustEventResponseCascade] =
    useDeleteAppendTrustEventResponseCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<AppendTrustEventResponse | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<AppendTrustEventResponse | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateAppendTrustEventResponse(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update AppendTrustEventResponse";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteAppendTrustEventResponseCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  const handlePasteRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to paste (create) AppendTrustEventResponse:",
        error,
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter AppendTrustEventResponse (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
