...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/RotateTrustKeyRequest/${rowId}`;
...
const RotateTrustKeyRequestTable: React.FC = () => {
  ...
  const [updateRotateTrustKeyRequest] =
    useUpdateRotateTrustKeyRequestMutation();
  const [deleteRotateTrustKeyRequest] =
    useDeleteRotateTrustKeyRequestMutation();
  const [deleteRotateTrustKeyRequestCascade] =
    useDeleteRotateTrustKeyRequestCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<RotateTrustKeyRequest | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<RotateTrustKeyRequest | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateRotateTrustKeyRequest(
        updatedItem as any,
      ).unwrap();
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Failed to update RotateTrustKeyRequest";
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRotateTrustKeyRequestCascade({
              id,
              cascade: true,
              trash: true,
            } as any).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter RotateTrustKeyRequest (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
