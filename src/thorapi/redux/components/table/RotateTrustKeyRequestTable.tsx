...
import { RotateTrustKeyRequest } from '@thorapi/model';
import RotateTrustKeyRequestForm from '@thorapi/redux/components/form/RotateTrustKeyRequestForm';
...
import {
  useLazyGetRotateTrustKeyRequestsPagedQuery,
...
} from '@thorapi/redux/services/RotateTrustKeyRequestService';
...
const columnSchema: Record<string, ColumnSchema> = {
  'rotationType': { type: 'enum', enumValues: [
    'SCHEDULED', 'MANUAL', 'COMPROMISE_RESPONSE', 'PROVIDER_MIGRATION', 'ALGORITHM_MIGRATION', 
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `RotateTrustKeyRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/RotateTrustKeyRequest/${rowId}`;

const RotateTrustKeyRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<RotateTrustKeyRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): RotateTrustKeyRequest[] => {
    if (Array.isArray(result)) return result as RotateTrustKeyRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as RotateTrustKeyRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: RotateTrustKeyRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addRotateTrustKeyRequest] = useAddRotateTrustKeyRequestMutation();
  const [updateRotateTrustKeyRequest] = useUpdateRotateTrustKeyRequestMutation();
  const [deleteRotateTrustKeyRequest] = useDeleteRotateTrustKeyRequestMutation();
  const [deleteRotateTrustKeyRequestCascade] = useDeleteRotateTrustKeyRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<RotateTrustKeyRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<RotateTrustKeyRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<RotateTrustKeyRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as RotateTrustKeyRequest;
    } catch (err) {
      console.warn('Could not reload RotateTrustKeyRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<RotateTrustKeyRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateRotateTrustKeyRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as RotateTrustKeyRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update RotateTrustKeyRequest';
      console.error('Failed to update RotateTrustKeyRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteRotateTrustKeyRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteRotateTrustKeyRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<RotateTrustKeyRequest> = { ...source };
      delete (payload as any).id;
      const created = await addRotateTrustKeyRequest(payload).unwrap();
      setAllData((prev) => [created as RotateTrustKeyRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add RotateTrustKeyRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<RotateTrustKeyRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addRotateTrustKeyRequest(payload).unwrap();
      setAllData((prev) => [created as RotateTrustKeyRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) RotateTrustKeyRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `RotateTrustKeyRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addRotateTrustKeyRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateRotateTrustKeyRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as RotateTrustKeyRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as RotateTrustKeyRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as RotateTrustKeyRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as RotateTrustKeyRequest[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="RotateTrustKeyRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="RotateTrustKeyRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create RotateTrustKeyRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <RotateTrustKeyRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter RotateTrustKeyRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing RotateTrustKeyRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof RotateTrustKeyRequest;
                ...
                await updateRotateTrustKeyRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of RotateTrustKeyRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'RotateTrustKeyRequest'}
...
export default RotateTrustKeyRequestTable;
