...
import { DefaultResponse } from '@thorapi/model';
import DefaultResponseForm from '@thorapi/redux/components/form/DefaultResponseForm';
...
} from '@thorapi/redux/services/DefaultResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `DefaultResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/DefaultResponse/${rowId}`;
...
const DefaultResponseTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): DefaultResponse[] => {
    if (Array.isArray(result)) return result as DefaultResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as DefaultResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: DefaultResponse[], requestedPage: number) => {
  ...
  const [deleteDefaultResponseCascade] = useDeleteDefaultResponseCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<DefaultResponse | null> => {
    ...
    try {
      ...
      console.warn('Could not reload DefaultResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<DefaultResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateDefaultResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as DefaultResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update DefaultResponse';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteDefaultResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `DefaultResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addDefaultResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateDefaultResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as DefaultResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as DefaultResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as DefaultResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as DefaultResponse[]);
    }
  };
