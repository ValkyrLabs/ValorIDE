...
import { VerifyPhoneOTPRequest } from '@thorapi/model';
import VerifyPhoneOTPRequestForm from '@thorapi/redux/components/form/VerifyPhoneOTPRequestForm';
...
import {
  useLazyGetVerifyPhoneOTPRequestsPagedQuery,
...
} from '@thorapi/redux/services/VerifyPhoneOTPRequestService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `VerifyPhoneOTPRequest update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/VerifyPhoneOTPRequest/${rowId}`;

const VerifyPhoneOTPRequestTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<VerifyPhoneOTPRequest[]>([]);
  ...
  const normalizePageRows = (result: unknown): VerifyPhoneOTPRequest[] => {
    if (Array.isArray(result)) return result as VerifyPhoneOTPRequest[];
    ...
    return Array.isArray(candidate) ? (candidate as VerifyPhoneOTPRequest[]) : [];
  };

  const mergeFetchedRows = (incomingRows: VerifyPhoneOTPRequest[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addVerifyPhoneOTPRequest] = useAddVerifyPhoneOTPRequestMutation();
  const [updateVerifyPhoneOTPRequest] = useUpdateVerifyPhoneOTPRequestMutation();
  const [deleteVerifyPhoneOTPRequest] = useDeleteVerifyPhoneOTPRequestMutation();
  const [deleteVerifyPhoneOTPRequestCascade] = useDeleteVerifyPhoneOTPRequestCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<VerifyPhoneOTPRequest>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<VerifyPhoneOTPRequest | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<VerifyPhoneOTPRequest | null> => {
    ...
    try {
      ...
      return (await response.json()) as VerifyPhoneOTPRequest;
    } catch (err) {
      console.warn('Could not reload VerifyPhoneOTPRequest row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<VerifyPhoneOTPRequest>,
    successField?: string,
  ) => {
    try {
      const saved = await updateVerifyPhoneOTPRequest(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as VerifyPhoneOTPRequest[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update VerifyPhoneOTPRequest';
      console.error('Failed to update VerifyPhoneOTPRequest:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteVerifyPhoneOTPRequestCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteVerifyPhoneOTPRequest(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<VerifyPhoneOTPRequest> = { ...source };
      delete (payload as any).id;
      const created = await addVerifyPhoneOTPRequest(payload).unwrap();
      setAllData((prev) => [created as VerifyPhoneOTPRequest, ...prev]);
    } catch (error) {
      console.error('Failed to add VerifyPhoneOTPRequest:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<VerifyPhoneOTPRequest> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addVerifyPhoneOTPRequest(payload).unwrap();
      setAllData((prev) => [created as VerifyPhoneOTPRequest, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) VerifyPhoneOTPRequest:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `VerifyPhoneOTPRequest-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addVerifyPhoneOTPRequest(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateVerifyPhoneOTPRequest(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as VerifyPhoneOTPRequest[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as VerifyPhoneOTPRequest[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as VerifyPhoneOTPRequest[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as VerifyPhoneOTPRequest[]);
    }
  };
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="VerifyPhoneOTPRequestTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="VerifyPhoneOTPRequest Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create VerifyPhoneOTPRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <VerifyPhoneOTPRequestForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter VerifyPhoneOTPRequest (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing VerifyPhoneOTPRequest</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof VerifyPhoneOTPRequest;
                ...
                await updateVerifyPhoneOTPRequest(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of VerifyPhoneOTPRequests.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'VerifyPhoneOTPRequest'}
...
export default VerifyPhoneOTPRequestTable;
