...
const buildResourcePath = (rowId: string) =>
  `${BASE_PATH}/VerifyPhoneOTPRequest/${rowId}`;
...
const VerifyPhoneOTPRequestTable: React.FC = () => {
  ...
  const [updateVerifyPhoneOTPRequest] =
    useUpdateVerifyPhoneOTPRequestMutation();
  const [deleteVerifyPhoneOTPRequest] =
    useDeleteVerifyPhoneOTPRequestMutation();
  const [deleteVerifyPhoneOTPRequestCascade] =
    useDeleteVerifyPhoneOTPRequestCascadeMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<VerifyPhoneOTPRequest | null>(
    null,
  );
  ...
  const reloadRowById = async (
    rowId: string,
  ): Promise<VerifyPhoneOTPRequest | null> => {
  ...
  const persistRowUpdate = async (
    ...
    try {
      const saved = await updateVerifyPhoneOTPRequest(
        updatedItem as any,
      ).unwrap();
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Filter VerifyPhoneOTPRequest (Query By Example)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
