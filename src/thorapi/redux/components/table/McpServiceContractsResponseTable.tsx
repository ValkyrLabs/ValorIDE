...
import { McpServiceContractsResponse } from '@thorapi/model';
import McpServiceContractsResponseForm from '@thorapi/redux/components/form/McpServiceContractsResponseForm';
...
import {
  useLazyGetMcpServiceContractsResponsesPagedQuery,
...
} from '@thorapi/redux/services/McpServiceContractsResponseService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `McpServiceContractsResponse update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/McpServiceContractsResponse/${rowId}`;

const McpServiceContractsResponseTable: React.FC = () => {
  // -------------------------------------------------------------------
  ...
  const [allData, setAllData] = useState<McpServiceContractsResponse[]>([]);
  ...
  const normalizePageRows = (result: unknown): McpServiceContractsResponse[] => {
    if (Array.isArray(result)) return result as McpServiceContractsResponse[];
    ...
    return Array.isArray(candidate) ? (candidate as McpServiceContractsResponse[]) : [];
  };

  const mergeFetchedRows = (incomingRows: McpServiceContractsResponse[], requestedPage: number) => {
  ...
  // -------------------------------------------------------------------
  const [addMcpServiceContractsResponse] = useAddMcpServiceContractsResponseMutation();
  const [updateMcpServiceContractsResponse] = useUpdateMcpServiceContractsResponseMutation();
  const [deleteMcpServiceContractsResponse] = useDeleteMcpServiceContractsResponseMutation();
  const [deleteMcpServiceContractsResponseCascade] = useDeleteMcpServiceContractsResponseCascadeMutation();

  // -------------------------------------------------------------------
  // UI states (editing, modals, etc.)
  // -------------------------------------------------------------------
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<McpServiceContractsResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<McpServiceContractsResponse | null>(null);
  ...
  const reloadRowById = async (rowId: string): Promise<McpServiceContractsResponse | null> => {
    ...
    try {
      ...
      return (await response.json()) as McpServiceContractsResponse;
    } catch (err) {
      console.warn('Could not reload McpServiceContractsResponse row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<McpServiceContractsResponse>,
    successField?: string,
  ) => {
    try {
      const saved = await updateMcpServiceContractsResponse(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as McpServiceContractsResponse[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update McpServiceContractsResponse';
      console.error('Failed to update McpServiceContractsResponse:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteMcpServiceContractsResponseCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
            console.error('Failed to cascade delete id', id, e);
            try {
              await deleteMcpServiceContractsResponse(id as any).unwrap();
            } catch (inner) {
...
// AddressToJSON(value?: Address)
  ...
  const handleAddRow = async () => {
    try {
      ...
      const payload: Partial<McpServiceContractsResponse> = { ...source };
      delete (payload as any).id;
      const created = await addMcpServiceContractsResponse(payload).unwrap();
      setAllData((prev) => [created as McpServiceContractsResponse, ...prev]);
    } catch (error) {
      console.error('Failed to add McpServiceContractsResponse:', error);
  ...
  const handlePasteRow = async () => {
    ...
    try {
      const payload: Partial<McpServiceContractsResponse> = { ...copiedRow };
      delete (payload as any).id;
      const created = await addMcpServiceContractsResponse(payload).unwrap();
      setAllData((prev) => [created as McpServiceContractsResponse, ...prev]);
    } catch (error) {
      console.error('Failed to paste (create) McpServiceContractsResponse:', error);
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `McpServiceContractsResponse-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addMcpServiceContractsResponse(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateMcpServiceContractsResponse(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as McpServiceContractsResponse[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as McpServiceContractsResponse[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as McpServiceContractsResponse[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as McpServiceContractsResponse[]);
    }
  };
  ...
  const handleManagePermissions = () => {
    if (selectedRows.size === 1) {
      ...
      setSelectedObjectForPermissions({
        objectType: 'com.valkyrlabs.model.McpServiceContractsResponse',
        objectId: rowId,
      });
  ...
  return (
    <>
            ...
            <RBGrid
              ...
              storageKey="McpServiceContractsResponseTable"
          ...
          >
            ...
            <FloatingControlPanel
              description="McpServiceContractsResponse Actions"
          ...
          <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Create McpServiceContractsResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <McpServiceContractsResponseForm />
            </Modal.Body>
          </Modal>
          {/* QBE Modal */}
          <Modal show={showQbeModal} onHide={() => setShowQbeModal(false)} centered size="lg">
            <Modal.Header closeButton>
              <Modal.Title>Filter McpServiceContractsResponse (Query By Example)</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
            <Modal.Header closeButton>
              <Modal.Title>Editing McpServiceContractsResponse</Modal.Title>
            </Modal.Header>
            <Modal.Body>
          ...
          <MarkdownEditorModal
            ...
            onSave={async (updatedText) => {
              try {
                ...
                const field = richModal.field as keyof McpServiceContractsResponse;
                ...
                await updateMcpServiceContractsResponse(updated).unwrap();
          ...
          {isError && (
            <div style={ { color: 'red', marginTop: 12 }}>
              Error fetching page {page} of McpServiceContractsResponses.
            </div>
          )}
        ...
        <QBEPicker
          show={qbePicker.show}
          refType={qbePicker.refType || 'McpServiceContractsResponse'}
...
export default McpServiceContractsResponseTable;
