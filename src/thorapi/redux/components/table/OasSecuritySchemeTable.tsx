...
import OasSecuritySchemeForm from '@thorapi/redux/components/form/OasSecuritySchemeForm';
...
} from '@thorapi/redux/services/OasSecuritySchemeService';
...
const reconcileSavedRow = (
  ...
  throw new Error(
    `OasSecurityScheme update did not return persisted value for ${successField}; refusing to show an unverified edit.`,
  );
};

const buildResourcePath = (rowId: string) => `${BASE_PATH}/OasSecurityScheme/${rowId}`;
...
const OasSecuritySchemeTable: React.FC = () => {
  ...
  const normalizePageRows = (result: unknown): OasSecurityScheme[] => {
    if (Array.isArray(result)) return result as OasSecurityScheme[];
    ...
    return Array.isArray(candidate) ? (candidate as OasSecurityScheme[]) : [];
  };

  const mergeFetchedRows = (incomingRows: OasSecurityScheme[], requestedPage: number) => {
  ...
  const [deleteOasSecuritySchemeCascade] = useDeleteOasSecuritySchemeCascadeMutation();
  ...
  const reloadRowById = async (rowId: string): Promise<OasSecurityScheme | null> => {
    ...
    try {
      ...
      return (await response.json()) as OasSecurityScheme;
    } catch (err) {
      console.warn('Could not reload OasSecurityScheme row', rowId, err);
      return null;
    }
  };
  ...
  const persistRowUpdate = async (
    rowId: string,
    updatedItem: Partial<OasSecurityScheme>,
    successField?: string,
  ) => {
    try {
      const saved = await updateOasSecurityScheme(updatedItem as any).unwrap();
      ...
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, hydrated as any) as OasSecurityScheme[]);
      if (successField) {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to update OasSecurityScheme';
  ...
  const confirmDelete = async () => {
    ...
    try {
      await Promise.all(
        ids.map(async (id) => {
          try {
            await deleteOasSecuritySchemeCascade({ id, cascade: true, trash: true } as any).unwrap();
          } catch (e) {
...
// AddressToJSON(value?: Address)
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `OasSecurityScheme-export-${stamp}.json`;
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (incomingId == null || incomingId === '') {
          delete payload.id;
          try {
            await addOasSecurityScheme(payload).unwrap();
            created += 1;
          } catch (err) {
            failed += 1;
        ...
        try {
          ...
          try {
            await updateOasSecurityScheme(payload).unwrap();
            updated += 1;
          } catch (updateErr) {
            failed += 1;
      ...
      setQbePicker({ 
        ...
        resolve: async (picked) => {
          if (picked) {
            ...
            setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, updated as any) as OasSecurityScheme[]);
            try {
              await persistRowUpdate(idToUpdate, updated, keyToUpdate);
              setEditRowId(null);
              setFormData({});
            } catch (error) {
              setAllData((prev) => mergeUpdatedRow(prev as any[], idToUpdate, row as any) as OasSecurityScheme[]);
            }
          }
        }, 
        allowCreate: true 
  ...
  const handleBooleanCellToggle = async (rowId: string, key: string, nextValue: boolean) => {
    ...
    setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, updatedItem as any) as OasSecurityScheme[]);
    try {
      await persistRowUpdate(rowId, updatedItem, key);
      setEditRowId(null);
      setFormData({});
    } catch (error) {
      setAllData((prev) => mergeUpdatedRow(prev as any[], rowId, row as any) as OasSecurityScheme[]);
    }
  };
