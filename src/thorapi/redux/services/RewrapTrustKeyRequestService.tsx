...
export const RewrapTrustKeyRequestService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getRewrapTrustKeyRequestsPaged: build.query<RewrapTrustKeyRequestResponse, { page: number; size?: number; example?: Partial<RewrapTrustKeyRequest> }>({
    ...
    getRewrapTrustKeyRequests: build.query<RewrapTrustKeyRequestResponse, { example?: Partial<RewrapTrustKeyRequest> } | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateRewrapTrustKeyRequest: build.mutation<void, Pick<RewrapTrustKeyRequest, 'id'> & Partial<RewrapTrustKeyRequest>>({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            RewrapTrustKeyRequestService.util.updateQueryData('getRewrapTrustKeyRequest', id, (draft) => {
    ...
    deleteRewrapTrustKeyRequest: build.mutation<{ success: boolean; id: string }, number>({
      ...
      invalidatesTags: (result, error, id) => [{ type: 'RewrapTrustKeyRequest', id }],
    }),
    ...
    deleteRewrapTrustKeyRequestCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'RewrapTrustKeyRequest', id }, { type: 'RewrapTrustKeyRequest', id: 'LIST' }],
    }),
  }),
})
