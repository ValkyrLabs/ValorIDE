...
export const AgentChatMessageService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getAgentChatMessagesPaged: build.query<AgentChatMessageResponse, { page: number; size?: number; example?: Partial<AgentChatMessage> }>({
      ...
      providesTags: (result, error, { page }) =>
        result
          ? [
              ...result.map(({ id }) => ({ type: 'AgentChatMessage' as const, id })),
    ...
    getAgentChatMessages: build.query<AgentChatMessageResponse, { example?: Partial<AgentChatMessage> } | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateAgentChatMessage: build.mutation<void, Pick<AgentChatMessage, 'id'> & Partial<AgentChatMessage>>({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            AgentChatMessageService.util.updateQueryData('getAgentChatMessage', id, (draft) => {
    ...
    deleteAgentChatMessage: build.mutation<{ success: boolean; id: string }, number>({
      ...
      invalidatesTags: (result, error, id) => [{ type: 'AgentChatMessage', id }],
    }),
    ...
    deleteAgentChatMessageCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'AgentChatMessage', id }, { type: 'AgentChatMessage', id: 'LIST' }],
    }),
  }),
})
