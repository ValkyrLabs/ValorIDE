// tslint:disable
/**
############################## DO NOT EDIT: GENERATED FILE ##############################

Generated by Valkyr Labs ThorAPI: https://valkyrlabs.com/thorapi
Powered by Swagger Codegen: http://swagger.io

Generated Details:
**GENERATOR VERSION:** 7.5.0
**GENERATOR CLASS:** org.openapitools.codegen.languages.TypeScriptReduxQueryClientCodegen

Template file: typescript-redux-query/modelService.mustache

############################## DO NOT EDIT: GENERATED FILE ##############################
*/
import { createApi } from '@reduxjs/toolkit/query/react'
import { GoalDependency } from '@thorapi/model/GoalDependency'
import customBaseQuery from '../customBaseQuery'; // Import the custom base query

type GoalDependencyResponse = GoalDependency[]
type GoalDependencyPagedQueryArg = {
  page: number
  size?: number
  example?: Partial<GoalDependency>
  /**
   * Cache discriminator only. Do not send this to ThorAPI; callers pass the
   * authenticated principal id/username so RBAC-filtered pages cannot be
   * reused across login boundaries by RTK Query.
   */
  authSessionKey?: string
}

type GoalDependencyListQueryArg = {
  example?: Partial<GoalDependency>
  /**
   * Cache discriminator only. Do not send this to ThorAPI.
   */
  authSessionKey?: string
}

const toGoalDependencyList = (result: unknown): GoalDependencyResponse => {
  if (Array.isArray(result)) {
    return result as GoalDependencyResponse
  }

  const candidate = (result as any)?.content ?? (result as any)?.items ?? (result as any)?.results ?? (result as any)?.data
  return Array.isArray(candidate) ? (candidate as GoalDependencyResponse) : []
}

export const GoalDependencyService = createApi({
  reducerPath: 'GoalDependency', // This should remain unique
  baseQuery: customBaseQuery,
  tagTypes: ['GoalDependency'],
  endpoints: (build) => ({
    // 1) Paged Query Endpoint
    // Standardized pagination: page (0-based), size (page size)
    getGoalDependencysPaged: build.query<GoalDependencyResponse, GoalDependencyPagedQueryArg>({
      query: ({ page, size = 20, example }) => {
        const q: string[] = [`page=${page}`, `size=${size}`];
        if (example) q.push(`example=${encodeURIComponent(JSON.stringify(example))}`);
        return `GoalDependency?${q.join('&')}`;
      },
      providesTags: (result, error, { page }) => {
        const rows = toGoalDependencyList(result)
        return [
          ...rows
            .filter((row) => row?.id != null)
            .map(({ id }) => ({ type: 'GoalDependency' as const, id })),
          { type: 'GoalDependency', id: `PAGE_${page}` },
          { type: 'GoalDependency', id: 'PARTIAL-LIST' },
        ]
      },
    }),

    // 2) Simple "get all" Query (optional)
    getGoalDependencys: build.query<GoalDependencyResponse, GoalDependencyListQueryArg | void>({
      query: (arg) => {
        if (arg && (arg as any).example) {
          const ex = (arg as any).example;
          return `GoalDependency?example=${encodeURIComponent(JSON.stringify(ex))}`;
        }
        return `GoalDependency`;
      },
      providesTags: (result) => {
        const rows = toGoalDependencyList(result)
        return [
          ...rows
            .filter((row) => row?.id != null)
            .map(({ id }) => ({ type: 'GoalDependency' as const, id })),
          { type: 'GoalDependency', id: 'LIST' },
          { type: 'GoalDependency', id: 'PARTIAL-LIST' },
        ]
      },
    }),

    // 3) Create
    addGoalDependency: build.mutation<GoalDependency, Partial<GoalDependency>>({
      query: (body) => ({
        url: `GoalDependency`,
        method: 'POST',
        body,
      }),
      invalidatesTags: [
        { type: 'GoalDependency', id: 'LIST' },
        { type: 'GoalDependency', id: 'PARTIAL-LIST' },
      ],
    }),

    // 4) Get single by ID
    getGoalDependency: build.query<GoalDependency, string>({
      query: (id) => `GoalDependency/${id}`,
      providesTags: (result, error, id) => [{ type: 'GoalDependency', id }],
    }),

    // 5) Update
    updateGoalDependency: build.mutation<GoalDependency, Pick<GoalDependency, 'id'> & Partial<GoalDependency>>({
      query: ({ id, ...patch }) => ({
        url: `GoalDependency/${id}`,
        method: 'PUT',
        body: patch,
      }),
      invalidatesTags: (result, error, { id }: Pick<GoalDependency, 'id'>) => [
        { type: 'GoalDependency', id },
        { type: 'GoalDependency', id: 'LIST' },
        { type: 'GoalDependency', id: 'PARTIAL-LIST' },
      ],
    }),

    // 6) Delete
    deleteGoalDependency: build.mutation<{ success: boolean; id: string }, number>({
      query(id) {
        return {
          url: `GoalDependency/${id}`,
          method: 'DELETE',
        }
      },
      invalidatesTags: (result, error, id) => [
        { type: 'GoalDependency', id },
        { type: 'GoalDependency', id: 'LIST' },
        { type: 'GoalDependency', id: 'PARTIAL-LIST' },
      ],
    }),

    // 7) Cascade / soft-delete (marks trashed, cascades children)
    deleteGoalDependencyCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      query({ id, cascade = true, trash = true }) {
        const params = [`cascade=${cascade}`, `trash=${trash}`].join('&');
        return {
          url: `GoalDependency/${id}?${params}`,
          method: 'DELETE',
        }
      },
      invalidatesTags: (result, error, { id }) => [
        { type: 'GoalDependency', id },
        { type: 'GoalDependency', id: 'LIST' },
        { type: 'GoalDependency', id: 'PARTIAL-LIST' },
      ],
    }),
  }),
})

// Notice we now also export `useLazyGetGoalDependencysPagedQuery`
export const {
  useGetGoalDependencysPagedQuery,     // immediate fetch
  useLazyGetGoalDependencysPagedQuery, // lazy fetch
  useGetGoalDependencyQuery,
  useGetGoalDependencysQuery,
  useAddGoalDependencyMutation,
  useUpdateGoalDependencyMutation,
  useDeleteGoalDependencyMutation,
  useDeleteGoalDependencyCascadeMutation,
} = GoalDependencyService
