...
export const RotateTrustKeyResponseService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getRotateTrustKeyResponsesPaged: build.query<RotateTrustKeyResponseResponse, { page: number; size?: number; example?: Partial<RotateTrustKeyResponse> }>({
    ...
    getRotateTrustKeyResponses: build.query<RotateTrustKeyResponseResponse, { example?: Partial<RotateTrustKeyResponse> } | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateRotateTrustKeyResponse: build.mutation<void, Pick<RotateTrustKeyResponse, 'id'> & Partial<RotateTrustKeyResponse>>({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            RotateTrustKeyResponseService.util.updateQueryData('getRotateTrustKeyResponse', id, (draft) => {
    ...
    deleteRotateTrustKeyResponse: build.mutation<{ success: boolean; id: string }, number>({
      ...
      invalidatesTags: (result, error, id) => [{ type: 'RotateTrustKeyResponse', id }],
    }),
    ...
    deleteRotateTrustKeyResponseCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'RotateTrustKeyResponse', id }, { type: 'RotateTrustKeyResponse', id: 'LIST' }],
    }),
  }),
})
