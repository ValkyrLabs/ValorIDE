...
export const DeploymentService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getDeploymentsPaged: build.query<DeploymentResponse, { page: number; size?: number; example?: Partial<Deployment> }>({
    ...
    getDeployments: build.query<DeploymentResponse, { example?: Partial<Deployment> } | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateDeployment: build.mutation<void, Pick<Deployment, 'id'> & Partial<Deployment>>({
      query: ({ id, ...patch }) => ({
        url: `Deployment/${id}`,
        method: 'PUT',
        body: patch,
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            DeploymentService.util.updateQueryData('getDeployment', id, (draft) => {
    ...
    deleteDeploymentCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'Deployment', id }, { type: 'Deployment', id: 'LIST' }],
    }),
  }),
})
