...
export const SubscriptionPlanService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getSubscriptionPlansPaged: build.query<SubscriptionPlanResponse, { page: number; size?: number; example?: Partial<SubscriptionPlan> }>({
    ...
    getSubscriptionPlans: build.query<SubscriptionPlanResponse, { example?: Partial<SubscriptionPlan> } | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateSubscriptionPlan: build.mutation<void, Pick<SubscriptionPlan, 'id'> & Partial<SubscriptionPlan>>({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            SubscriptionPlanService.util.updateQueryData('getSubscriptionPlan', id, (draft) => {
    ...
    deleteSubscriptionPlanCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'SubscriptionPlan', id }, { type: 'SubscriptionPlan', id: 'LIST' }],
    }),
  }),
})
