...
export const JobMetadataService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getJobMetadatasPaged: build.query<JobMetadataResponse, { page: number; size?: number; example?: Partial<JobMetadata> }>({
    ...
    getJobMetadatas: build.query<JobMetadataResponse, { example?: Partial<JobMetadata> } | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateJobMetadata: build.mutation<void, Pick<JobMetadata, 'id'> & Partial<JobMetadata>>({
      query: ({ id, ...patch }) => ({
        url: `JobMetadata/${id}`,
        method: 'PUT',
        body: patch,
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            JobMetadataService.util.updateQueryData('getJobMetadata', id, (draft) => {
    ...
    deleteJobMetadataCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'JobMetadata', id }, { type: 'JobMetadata', id: 'LIST' }],
    }),
  }),
})
