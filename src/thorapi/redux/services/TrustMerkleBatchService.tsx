...
export const TrustMerkleBatchService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getTrustMerkleBatchsPaged: build.query<TrustMerkleBatchResponse, { page: number; size?: number; example?: Partial<TrustMerkleBatch> }>({
    ...
    getTrustMerkleBatchs: build.query<TrustMerkleBatchResponse, { example?: Partial<TrustMerkleBatch> } | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateTrustMerkleBatch: build.mutation<void, Pick<TrustMerkleBatch, 'id'> & Partial<TrustMerkleBatch>>({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            TrustMerkleBatchService.util.updateQueryData('getTrustMerkleBatch', id, (draft) => {
    ...
    deleteTrustMerkleBatch: build.mutation<{ success: boolean; id: string }, number>({
      ...
      invalidatesTags: (result, error, id) => [{ type: 'TrustMerkleBatch', id }],
    }),
    ...
    deleteTrustMerkleBatchCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'TrustMerkleBatch', id }, { type: 'TrustMerkleBatch', id: 'LIST' }],
    }),
  }),
})
