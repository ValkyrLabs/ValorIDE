// tslint:disable
/**
############################## DO NOT EDIT: GENERATED FILE ##############################

Generated by Valkyr Labs ThorAPI: https://valkyrlabs.com/thorapi
Powered by Swagger Codegen: http://swagger.io

Generated Details:
**GENERATOR VERSION:** 7.5.0
**GENERATOR CLASS:** org.openapitools.codegen.languages.TypeScriptReduxQueryClientCodegen

Template file: typescript-redux-query/modelService.mustache

############################## DO NOT EDIT: GENERATED FILE ##############################
*/
import { createApi } from '@reduxjs/toolkit/query/react'
import { WizardStatusResponse } from '@thorapi/model/WizardStatusResponse'
import customBaseQuery from '../customBaseQuery'; // Import the custom base query

type WizardStatusResponseResponse = WizardStatusResponse[]
type WizardStatusResponsePagedQueryArg = {
  page: number
  size?: number
  example?: Partial<WizardStatusResponse>
  /**
   * Cache discriminator only. Do not send this to ThorAPI; callers pass the
   * authenticated principal id/username so RBAC-filtered pages cannot be
   * reused across login boundaries by RTK Query.
   */
  authSessionKey?: string
}

type WizardStatusResponseListQueryArg = {
  example?: Partial<WizardStatusResponse>
  /**
   * Cache discriminator only. Do not send this to ThorAPI.
   */
  authSessionKey?: string
}

const toWizardStatusResponseList = (result: unknown): WizardStatusResponseResponse => {
  if (Array.isArray(result)) {
    return result as WizardStatusResponseResponse
  }

  const candidate = (result as any)?.content ?? (result as any)?.items ?? (result as any)?.results ?? (result as any)?.data
  return Array.isArray(candidate) ? (candidate as WizardStatusResponseResponse) : []
}

export const WizardStatusResponseService = createApi({
  reducerPath: 'WizardStatusResponse', // This should remain unique
  baseQuery: customBaseQuery,
  tagTypes: ['WizardStatusResponse'],
  endpoints: (build) => ({
    // 1) Paged Query Endpoint
    // Standardized pagination: page (0-based), size (page size)
    getWizardStatusResponsesPaged: build.query<WizardStatusResponseResponse, WizardStatusResponsePagedQueryArg>({
      query: ({ page, size = 20, example }) => {
        const q: string[] = [`page=${page}`, `size=${size}`];
        if (example) q.push(`example=${encodeURIComponent(JSON.stringify(example))}`);
        return `WizardStatusResponse?${q.join('&')}`;
      },
      providesTags: (result, error, { page }) => {
        const rows = toWizardStatusResponseList(result)
        return [
          ...rows
            .filter((row) => row?.id != null)
            .map(({ id }) => ({ type: 'WizardStatusResponse' as const, id })),
          { type: 'WizardStatusResponse', id: `PAGE_${page}` },
          { type: 'WizardStatusResponse', id: 'PARTIAL-LIST' },
        ]
      },
    }),

    // 2) Simple "get all" Query (optional)
    getWizardStatusResponses: build.query<WizardStatusResponseResponse, WizardStatusResponseListQueryArg | void>({
      query: (arg) => {
        if (arg && (arg as any).example) {
          const ex = (arg as any).example;
          return `WizardStatusResponse?example=${encodeURIComponent(JSON.stringify(ex))}`;
        }
        return `WizardStatusResponse`;
      },
      providesTags: (result) => {
        const rows = toWizardStatusResponseList(result)
        return [
          ...rows
            .filter((row) => row?.id != null)
            .map(({ id }) => ({ type: 'WizardStatusResponse' as const, id })),
          { type: 'WizardStatusResponse', id: 'LIST' },
          { type: 'WizardStatusResponse', id: 'PARTIAL-LIST' },
        ]
      },
    }),

    // 3) Create
    addWizardStatusResponse: build.mutation<WizardStatusResponse, Partial<WizardStatusResponse>>({
      query: (body) => ({
        url: `WizardStatusResponse`,
        method: 'POST',
        body,
      }),
      invalidatesTags: [
        { type: 'WizardStatusResponse', id: 'LIST' },
        { type: 'WizardStatusResponse', id: 'PARTIAL-LIST' },
      ],
    }),

    // 4) Get single by ID
    getWizardStatusResponse: build.query<WizardStatusResponse, string>({
      query: (id) => `WizardStatusResponse/${id}`,
      providesTags: (result, error, id) => [{ type: 'WizardStatusResponse', id }],
    }),

    // 5) Update
    updateWizardStatusResponse: build.mutation<WizardStatusResponse, Pick<WizardStatusResponse, 'id'> & Partial<WizardStatusResponse>>({
      query: ({ id, ...patch }) => ({
        url: `WizardStatusResponse/${id}`,
        method: 'PUT',
        body: patch,
      }),
      invalidatesTags: (result, error, { id }: Pick<WizardStatusResponse, 'id'>) => [
        { type: 'WizardStatusResponse', id },
        { type: 'WizardStatusResponse', id: 'LIST' },
        { type: 'WizardStatusResponse', id: 'PARTIAL-LIST' },
      ],
    }),

    // 6) Delete
    deleteWizardStatusResponse: build.mutation<{ success: boolean; id: string }, number>({
      query(id) {
        return {
          url: `WizardStatusResponse/${id}`,
          method: 'DELETE',
        }
      },
      invalidatesTags: (result, error, id) => [
        { type: 'WizardStatusResponse', id },
        { type: 'WizardStatusResponse', id: 'LIST' },
        { type: 'WizardStatusResponse', id: 'PARTIAL-LIST' },
      ],
    }),

    // 7) Cascade / soft-delete (marks trashed, cascades children)
    deleteWizardStatusResponseCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      query({ id, cascade = true, trash = true }) {
        const params = [`cascade=${cascade}`, `trash=${trash}`].join('&');
        return {
          url: `WizardStatusResponse/${id}?${params}`,
          method: 'DELETE',
        }
      },
      invalidatesTags: (result, error, { id }) => [
        { type: 'WizardStatusResponse', id },
        { type: 'WizardStatusResponse', id: 'LIST' },
        { type: 'WizardStatusResponse', id: 'PARTIAL-LIST' },
      ],
    }),
  }),
})

// Notice we now also export `useLazyGetWizardStatusResponsesPagedQuery`
export const {
  useGetWizardStatusResponsesPagedQuery,     // immediate fetch
  useLazyGetWizardStatusResponsesPagedQuery, // lazy fetch
  useGetWizardStatusResponseQuery,
  useGetWizardStatusResponsesQuery,
  useAddWizardStatusResponseMutation,
  useUpdateWizardStatusResponseMutation,
  useDeleteWizardStatusResponseMutation,
  useDeleteWizardStatusResponseCascadeMutation,
} = WizardStatusResponseService
