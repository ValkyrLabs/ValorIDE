...
export const WorkflowGraphValidationErrorService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getWorkflowGraphValidationErrorsPaged: build.query<WorkflowGraphValidationErrorResponse, { page: number; size?: number; example?: Partial<WorkflowGraphValidationError> }>({
    ...
    getWorkflowGraphValidationErrors: build.query<WorkflowGraphValidationErrorResponse, { example?: Partial<WorkflowGraphValidationError> } | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateWorkflowGraphValidationError: build.mutation<void, Pick<WorkflowGraphValidationError, 'id'> & Partial<WorkflowGraphValidationError>>({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            WorkflowGraphValidationErrorService.util.updateQueryData('getWorkflowGraphValidationError', id, (draft) => {
    ...
    deleteWorkflowGraphValidationError: build.mutation<{ success: boolean; id: string }, number>({
      ...
      invalidatesTags: (result, error, id) => [{ type: 'WorkflowGraphValidationError', id }],
    }),
    ...
    deleteWorkflowGraphValidationErrorCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'WorkflowGraphValidationError', id }, { type: 'WorkflowGraphValidationError', id: 'LIST' }],
    }),
  }),
})
