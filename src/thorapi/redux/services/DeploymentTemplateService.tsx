...
export const DeploymentTemplateService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getDeploymentTemplatesPaged: build.query<DeploymentTemplateResponse, { page: number; size?: number; example?: Partial<DeploymentTemplate> }>({
    ...
    getDeploymentTemplates: build.query<DeploymentTemplateResponse, { example?: Partial<DeploymentTemplate> } | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateDeploymentTemplate: build.mutation<void, Pick<DeploymentTemplate, 'id'> & Partial<DeploymentTemplate>>({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            DeploymentTemplateService.util.updateQueryData('getDeploymentTemplate', id, (draft) => {
    ...
    deleteDeploymentTemplate: build.mutation<{ success: boolean; id: string }, number>({
      ...
      invalidatesTags: (result, error, id) => [{ type: 'DeploymentTemplate', id }],
    }),
    ...
    deleteDeploymentTemplateCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'DeploymentTemplate', id }, { type: 'DeploymentTemplate', id: 'LIST' }],
    }),
  }),
})
