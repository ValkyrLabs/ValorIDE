...
export const TagService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getTagsPaged: build.query<TagResponse, TagPagedQueryArg>({
    ...
    getTags: build.query<TagResponse, TagListQueryArg | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateTag: build.mutation<Tag, Pick<Tag, 'id'> & Partial<Tag>>({
      query: ({ id, ...patch }) => ({
        url: `Tag/${id}`,
        method: 'PUT',
        body: patch,
