...
export const TrustSecretAccessEventService = createApi({
  ...
  endpoints: (build) => ({
    ...
    getTrustSecretAccessEventsPaged: build.query<TrustSecretAccessEventResponse, { page: number; size?: number; example?: Partial<TrustSecretAccessEvent> }>({
    ...
    getTrustSecretAccessEvents: build.query<TrustSecretAccessEventResponse, { example?: Partial<TrustSecretAccessEvent> } | void>({
      query: (arg) => {
    ...
    // 5) Update
    updateTrustSecretAccessEvent: build.mutation<void, Pick<TrustSecretAccessEvent, 'id'> & Partial<TrustSecretAccessEvent>>({
      query: ({ id, ...patch }) => ({
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        if (id) {
          const patchResult = dispatch(
            TrustSecretAccessEventService.util.updateQueryData('getTrustSecretAccessEvent', id, (draft) => {
    ...
    deleteTrustSecretAccessEvent: build.mutation<{ success: boolean; id: string }, number>({
      ...
      invalidatesTags: (result, error, id) => [{ type: 'TrustSecretAccessEvent', id }],
    }),
    ...
    deleteTrustSecretAccessEventCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      ...
      invalidatesTags: (result, error, { id }) => [{ type: 'TrustSecretAccessEvent', id }, { type: 'TrustSecretAccessEvent', id: 'LIST' }],
    }),
  }),
})
