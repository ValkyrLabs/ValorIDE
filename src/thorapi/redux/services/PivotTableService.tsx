// tslint:disable
/**
############################## DO NOT EDIT: GENERATED FILE ##############################

Generated by Valkyr Labs ThorAPI: https://valkyrlabs.com/thorapi
Powered by Swagger Codegen: http://swagger.io

Generated Details:
**GENERATOR VERSION:** 7.5.0
**GENERATOR CLASS:** org.openapitools.codegen.languages.TypeScriptReduxQueryClientCodegen

Template file: typescript-redux-query/modelService.mustache

############################## DO NOT EDIT: GENERATED FILE ##############################
*/
import { createApi } from '@reduxjs/toolkit/query/react'
import { PivotTable } from '@thorapi/model/PivotTable'
import customBaseQuery from '../customBaseQuery'; // Import the custom base query

type PivotTableResponse = PivotTable[]
type PivotTablePagedQueryArg = {
  page: number
  size?: number
  example?: Partial<PivotTable>
  /**
   * Cache discriminator only. Do not send this to ThorAPI; callers pass the
   * authenticated principal id/username so RBAC-filtered pages cannot be
   * reused across login boundaries by RTK Query.
   */
  authSessionKey?: string
}

type PivotTableListQueryArg = {
  example?: Partial<PivotTable>
  /**
   * Cache discriminator only. Do not send this to ThorAPI.
   */
  authSessionKey?: string
}

const toPivotTableList = (result: unknown): PivotTableResponse => {
  if (Array.isArray(result)) {
    return result as PivotTableResponse
  }

  const candidate = (result as any)?.content ?? (result as any)?.items ?? (result as any)?.results ?? (result as any)?.data
  return Array.isArray(candidate) ? (candidate as PivotTableResponse) : []
}

export const PivotTableService = createApi({
  reducerPath: 'PivotTable', // This should remain unique
  baseQuery: customBaseQuery,
  tagTypes: ['PivotTable'],
  endpoints: (build) => ({
    // 1) Paged Query Endpoint
    // Standardized pagination: page (0-based), size (page size)
    getPivotTablesPaged: build.query<PivotTableResponse, PivotTablePagedQueryArg>({
      query: ({ page, size = 20, example }) => {
        const q: string[] = [`page=${page}`, `size=${size}`];
        if (example) q.push(`example=${encodeURIComponent(JSON.stringify(example))}`);
        return `PivotTable?${q.join('&')}`;
      },
      providesTags: (result, error, { page }) => {
        const rows = toPivotTableList(result)
        return [
          ...rows
            .filter((row) => row?.id != null)
            .map(({ id }) => ({ type: 'PivotTable' as const, id })),
          { type: 'PivotTable', id: `PAGE_${page}` },
          { type: 'PivotTable', id: 'PARTIAL-LIST' },
        ]
      },
    }),

    // 2) Simple "get all" Query (optional)
    getPivotTables: build.query<PivotTableResponse, PivotTableListQueryArg | void>({
      query: (arg) => {
        if (arg && (arg as any).example) {
          const ex = (arg as any).example;
          return `PivotTable?example=${encodeURIComponent(JSON.stringify(ex))}`;
        }
        return `PivotTable`;
      },
      providesTags: (result) => {
        const rows = toPivotTableList(result)
        return [
          ...rows
            .filter((row) => row?.id != null)
            .map(({ id }) => ({ type: 'PivotTable' as const, id })),
          { type: 'PivotTable', id: 'LIST' },
          { type: 'PivotTable', id: 'PARTIAL-LIST' },
        ]
      },
    }),

    // 3) Create
    addPivotTable: build.mutation<PivotTable, Partial<PivotTable>>({
      query: (body) => ({
        url: `PivotTable`,
        method: 'POST',
        body,
      }),
      invalidatesTags: [
        { type: 'PivotTable', id: 'LIST' },
        { type: 'PivotTable', id: 'PARTIAL-LIST' },
      ],
    }),

    // 4) Get single by ID
    getPivotTable: build.query<PivotTable, string>({
      query: (id) => `PivotTable/${id}`,
      providesTags: (result, error, id) => [{ type: 'PivotTable', id }],
    }),

    // 5) Update
    updatePivotTable: build.mutation<PivotTable, Pick<PivotTable, 'id'> & Partial<PivotTable>>({
      query: ({ id, ...patch }) => ({
        url: `PivotTable/${id}`,
        method: 'PUT',
        body: patch,
      }),
      invalidatesTags: (result, error, { id }: Pick<PivotTable, 'id'>) => [
        { type: 'PivotTable', id },
        { type: 'PivotTable', id: 'LIST' },
        { type: 'PivotTable', id: 'PARTIAL-LIST' },
      ],
    }),

    // 6) Delete
    deletePivotTable: build.mutation<{ success: boolean; id: string }, number>({
      query(id) {
        return {
          url: `PivotTable/${id}`,
          method: 'DELETE',
        }
      },
      invalidatesTags: (result, error, id) => [
        { type: 'PivotTable', id },
        { type: 'PivotTable', id: 'LIST' },
        { type: 'PivotTable', id: 'PARTIAL-LIST' },
      ],
    }),

    // 7) Cascade / soft-delete (marks trashed, cascades children)
    deletePivotTableCascade: build.mutation<{ success: boolean; id: string }, { id: string; cascade?: boolean; trash?: boolean }>({
      query({ id, cascade = true, trash = true }) {
        const params = [`cascade=${cascade}`, `trash=${trash}`].join('&');
        return {
          url: `PivotTable/${id}?${params}`,
          method: 'DELETE',
        }
      },
      invalidatesTags: (result, error, { id }) => [
        { type: 'PivotTable', id },
        { type: 'PivotTable', id: 'LIST' },
        { type: 'PivotTable', id: 'PARTIAL-LIST' },
      ],
    }),
  }),
})

// Notice we now also export `useLazyGetPivotTablesPagedQuery`
export const {
  useGetPivotTablesPagedQuery,     // immediate fetch
  useLazyGetPivotTablesPagedQuery, // lazy fetch
  useGetPivotTableQuery,
  useGetPivotTablesQuery,
  useAddPivotTableMutation,
  useUpdatePivotTableMutation,
  useDeletePivotTableMutation,
  useDeletePivotTableCascadeMutation,
} = PivotTableService
