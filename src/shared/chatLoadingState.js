...
export function deriveChatLoadingState({ messages, lastMessage, textAreaDisabled, enableButtons, }) {
    const inlineSources = [];
    ...
    const apiReqInFlight = (lastApiStatus?.say === "api_req_started" ||
        lastApiStatus?.say === "api_req_retried") &&
        (() => {
            if (!lastApiStatus?.text)
                return true;
            try {
                const parsed = JSON.parse(lastApiStatus.text);
                const hasCompletionDetails = parsed?.cancelReason != null ||
    ...
    const isCommandFlow = lastMessage?.ask === "command" ||
        lastMessage?.say === "command" ||
        lastMessage?.ask === "command_output" ||
        lastMessage?.say === "command_output";
    const hasCommandOutput = typeof lastMessage?.text === "string" &&
        lastMessage.text.includes(COMMAND_OUTPUT_STRING);
    const isCommandStillRunning = isCommandFlow &&
        (lastMessage?.ask === "command_output" ||
            lastMessage?.partial ||
            !hasCommandOutput);
    ...
    const isWaitingForResponse = apiReqInFlight ||
...
export function computeIsChatLoadingState({ messages, lastMessage, textAreaDisabled, enableButtons, }) {
