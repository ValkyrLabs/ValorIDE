...
export function validateFileProcessingConfig(
  config: Partial<FileProcessingConfig>,
): FileProcessingConfig {
  return {
    maxFileSize: Math.max(
      1024,
      config.maxFileSize ?? DEFAULT_FILE_PROCESSING_CONFIG.maxFileSize,
    ),
    chunkSize: Math.max(
      512,
      Math.min(
        config.chunkSize ?? DEFAULT_FILE_PROCESSING_CONFIG.chunkSize,
        10240,
      ),
    ),
    streamingDelay: Math.max(
      1,
      Math.min(
        config.streamingDelay ?? DEFAULT_FILE_PROCESSING_CONFIG.streamingDelay,
        100,
      ),
    ),
    ...
    warnLargeFiles:
      config.warnLargeFiles ?? DEFAULT_FILE_PROCESSING_CONFIG.warnLargeFiles,
    largeFileThreshold: Math.max(
      1024,
...
export function validateMatchingConfig(
  ...
  return {
    enableFuzzyMatching:
      config.enableFuzzyMatching ?? DEFAULT_MATCHING_CONFIG.enableFuzzyMatching,
    fuzzyThreshold: Math.max(
      0.1,
      Math.min(
        config.fuzzyThreshold ?? DEFAULT_MATCHING_CONFIG.fuzzyThreshold,
        1.0,
      ),
    ),
    maxSearchAttempts: Math.max(
      1,
      Math.min(
        config.maxSearchAttempts ?? DEFAULT_MATCHING_CONFIG.maxSearchAttempts,
        10,
      ),
    ),
    enableContextualMatching:
      config.enableContextualMatching ??
      DEFAULT_MATCHING_CONFIG.enableContextualMatching,
    enableWhitespaceNormalization:
      config.enableWhitespaceNormalization ??
      DEFAULT_MATCHING_CONFIG.enableWhitespaceNormalization,
    enableSemanticMatching:
      config.enableSemanticMatching ??
      DEFAULT_MATCHING_CONFIG.enableSemanticMatching,
  };
}

export function validateOutputFilterConfig(
  config: Partial<OutputFilterConfig>,
): OutputFilterConfig {
  return {
    maxOutputLength: Math.max(
      100,
      config.maxOutputLength ?? DEFAULT_OUTPUT_FILTER_CONFIG.maxOutputLength,
    ),
    enableSmartSummarization:
      config.enableSmartSummarization ??
      DEFAULT_OUTPUT_FILTER_CONFIG.enableSmartSummarization,
    verbosityLevel: ["minimal", "normal", "verbose"].includes(
      config.verbosityLevel ?? "",
    )
      ? (config.verbosityLevel as "minimal" | "normal" | "verbose")
      : DEFAULT_OUTPUT_FILTER_CONFIG.verbosityLevel,
    enableProgressFiltering:
      config.enableProgressFiltering ??
      DEFAULT_OUTPUT_FILTER_CONFIG.enableProgressFiltering,
    enableStackTraceFiltering:
      config.enableStackTraceFiltering ??
    ...
    enableCommandSpecificFiltering:
      config.enableCommandSpecificFiltering ??
      DEFAULT_OUTPUT_FILTER_CONFIG.enableCommandSpecificFiltering,
  };
}

export function validateThorApiConfig(
  config: Partial<ThorApiConfig>,
): ThorApiConfig {
  return {
    outputFolder:
      config.outputFolder?.trim() || DEFAULT_THORAPI_CONFIG.outputFolder,
  };
}

export function validateAdvancedSettings(
  settings: Partial<ValorIDEAdvancedSettings>,
  ...
  return {
    ...
    performance: {
      enableParallelProcessing:
        settings.performance?.enableParallelProcessing ??
        DEFAULT_PERFORMANCE_CONFIG.enableParallelProcessing,
      maxConcurrentOperations: Math.max(
        1,
        Math.min(
          settings.performance?.maxConcurrentOperations ??
            DEFAULT_PERFORMANCE_CONFIG.maxConcurrentOperations,
          10,
        ),
      ),
